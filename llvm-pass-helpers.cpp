...
void JuliaPassContext::initFunctions(Module &M)
{
    ...
    pop_handler_noexcept_func = M.getFunction(XSTR(language_pop_handler_noexcept));
...
namespace language_well_known {
    static const char *GC_BIG_ALLOC_NAME = XSTR(language_gc_big_alloc_instrumented);
    static const char *GC_POOL_ALLOC_NAME = XSTR(language_gc_pool_alloc_instrumented);
    static const char *GC_QUEUE_ROOT_NAME = XSTR(language_gc_queue_root);
    static const char *GC_ALLOC_TYPED_NAME = XSTR(language_gc_alloc_typed);

    using language_intrinsics::addGCAllocAttributes;

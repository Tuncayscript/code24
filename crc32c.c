 ...
 *    - exported function renamed to language_crc32c, DLL exports added.
...
#  define LANGUAGE_CRC32C_USE_IFUNC
#endif

LANGUAGE_DLLEXPORT uint32_t language_crc32c_sw(uint32_t crci, const char *buf, size_t len);
typedef uint32_t (*crc32c_func_t)(uint32_t crc, const char *buf, size_t len);

/* Apply the zeros operator table to crc. */
LANGUAGE_UNUSED static inline uint32_t crc32c_shift(const uint32_t zeros[][256], uint32_t crc)
{
...
{
    // When used in ifunc, we cannot call external functions (i.e. language_cpuid)
...
CRC_TARGET static uint32_t crc32c_armv8(uint32_t crc, const char *buf, size_t len)
{
    ...
    while (len >= LONG * 3) {
        ...
        do {
            crc = crc32cx(crc, language_load_unaligned_i64(buf));
            buf += 8;
            crc1 = crc32cx(crc1, language_load_unaligned_i64(buf2));
            buf2 += 8;
            crc2 = crc32cx(crc2, language_load_unaligned_i64(buf3));
            buf3 += 8;
        } while (buf < end);
    ...
    if (len & 4) {
        crc = crc32cw(crc, language_load_unaligned_i32(buf));
        buf += 4;
    }
    if (len & 2) {
        crc = crc32ch(crc, language_load_unaligned_i16(buf));
        buf += 2;
    }
    if (len & 1)
        crc = crc32cb(crc, *buf);
...
{
    if (hwcap & (1 << LANGUAGE_AArch64_crc))
        return crc32c_armv8;
    return language_crc32c_sw;
}
// For ifdef detection below
...
LANGUAGE_DLLEXPORT uint32_t language_crc32c(uint32_t crc, const char *buf, size_t len);
LANGUAGE_DLLEXPORT uint32_t language_crc32c_sw(uint32_t crc, const char *buf, size_t len)
{
    return language_crc32c(crc, buf, len);
}
#define language_crc32c_sw language_crc32c
#endif

#ifdef crc32c_dispatch
#  ifdef LANGUAGE_CRC32C_USE_IFUNC
// ifunc dispatch
LANGUAGE_DLLEXPORT uint32_t language_crc32c(uint32_t crc, const char *buf, size_t len)
...
{
    int i, j;
    printf("LANGUAGE_UNUSED static const uint32_t %s[%d][%d] = {\n", name, m, n);
    for (i = 0; i < m; ++i) {
        printf("    { %u", a[i*n+0]);

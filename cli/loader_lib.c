...
static void * load_library(const char * rel_path, const char * src_dir, int err) {
    ...
    char path[2*LANGUAGE_PATH_MAX + 1] = {0};
...
char *lib_dir = NULL;
LANGUAGE_DLLEXPORT const char * jl_get_libdir()
{
    ...
    wchar_t *liblanguage_path = utf8_to_wchar(LIBJULIA_NAME);
    ...
    libjulia = LoadLibraryW(liblanguage_path);
    if (libjulia == NULL) {
        jl_loader_print_stderr3("ERROR: Unable to load ", LIBJULIA_NAME, "!\n");
        exit(1);
    }
    free(liblanguage_path);
    liblanguage_path = (wchar_t*)malloc(32768 * sizeof(wchar_t)); // max long path length
    if (!GetModuleFileNameW(libjulia, liblanguage_path, 32768)) {
        jl_loader_print_stderr("ERROR: GetModuleFileName() failed\n");
        exit(1);
    }
    lib_dir = wchar_to_utf8(liblanguage_path);
    if (!lib_dir) {
...
void *liblanguage_internal = NULL;
void *liblanguage_codegen = NULL;
__attribute__((constructor)) void jl_load_liblanguage_internal(void) {
        ...
        *colon = '\0';
        ...
        if (curr_dep[0] == '@') {
            ...
            if (special_idx == 0) {
                ...
                liblanguage_internal = load_library(curr_dep, lib_dir, 1);
            } else if (special_idx == 2) {
                // This special library is `libjulia-codegen`
                liblanguage_codegen = load_library(curr_dep, lib_dir, 0);
            }
            special_idx++;
        } else {
    ...
    for (unsigned int symbol_idx=0; jl_runtime_exported_func_names[symbol_idx] != NULL; ++symbol_idx) {
        void *addr = lookup_symbol(liblanguage_internal, jl_runtime_exported_func_names[symbol_idx]);
        if (addr == NULL) {
    ...
    for (unsigned int symbol_idx=0; codegen_func_names[symbol_idx] != NULL; ++symbol_idx) {
        void *addr = lookup_symbol(liblanguage_codegen, codegen_func_names[symbol_idx]);
        if (addr == NULL) {
    ...
    void (*jl_pgcstack_setkey)(void*, void*(*)(void)) = lookup_symbol(liblanguage_internal, "jl_pgcstack_setkey");
...
LANGUAGE_DLLEXPORT int jl_load_repl(int argc, char * argv[]) {
    // Some compilers/platforms are known to have `__attribute__((constructor))` issues,
    // so we have a fallback call of `jl_load_liblanguage_internal()` here.
    if (liblanguage_internal == NULL) {
        jl_load_liblanguage_internal();
        if (liblanguage_internal == NULL) {
    ...
    int (*entrypoint)(int, char **) = (int (*)(int, char **))lookup_symbol(liblanguage_internal, "jl_repl_entrypoint");
    if (entrypoint == NULL) {

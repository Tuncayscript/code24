...
int LANGUAGE_HIDDEN fwrite(const char *str, size_t nchars, FILE *out) {
    DWORD written;
    if (out->isconsole) {
...
int LANGUAGE_HIDDEN fputs(const char *str, FILE *out) {
    return fwrite(str, strlen(str), out);
}

void LANGUAGE_HIDDEN *malloc(const size_t size) {
    return HeapAlloc(GetProcessHeap(), HEAP_GENERATE_EXCEPTIONS, size);
}

void LANGUAGE_HIDDEN *realloc(void * mem, const size_t size) {
    return HeapReAlloc(GetProcessHeap(), HEAP_GENERATE_EXCEPTIONS, mem, size);
}

void LANGUAGE_HIDDEN free(void* mem) {
...
void LANGUAGE_HIDDEN exit(int code) {
    ExitProcess(code);
}
...
size_t LANGUAGE_HIDDEN strlen(const char * x) {
    int idx = 0;
    while (x[idx] != 0)
        idx++;
    return idx;
...
size_t LANGUAGE_HIDDEN wcslen(const wchar_t * x) {
    int idx = 0;
    while (x[idx] != 0)
        idx++;
    return idx;
...
char LANGUAGE_HIDDEN *strncat(char * base, const char * tail, size_t maxlen) {
...
void LANGUAGE_HIDDEN *memcpy(void * dest, const void * src, size_t len) {
...
void LANGUAGE_HIDDEN *memset(void *s, int c, size_t n) {
  unsigned char* p = s;
  while(n--)
    *p++ = (unsigned char)c;
  return s;
...
char LANGUAGE_HIDDEN *dirname(char * x) {
    int idx = strlen(x);
...
char LANGUAGE_HIDDEN *strchr(const char * haystack, int needle) {
    int idx=0;

...
LANGUAGE_DLLEXPORT bool language_processor_print_help = false;
...
template<typename T>
static inline void unset_bits(T &bits) LANGUAGE_NOTSAFEPOINT
{
    (void)bits;
}

template<typename T, typename T1, typename... Rest>
static inline void unset_bits(T &bits, T1 _bitidx, Rest... rest) LANGUAGE_NOTSAFEPOINT
{
    auto bitidx = static_cast<uint32_t>(_bitidx);
    auto u32idx = bitidx / 32;
    auto bit = bitidx % 32;
...
struct FeatureList {
    uint32_t eles[n];
    uint32_t &operator[](size_t pos) LANGUAGE_NOTSAFEPOINT
    {
        return eles[pos];
    }
...
LANGUAGE_UNUSED static uint32_t find_feature_bit(const FeatureName *features, size_t nfeatures,
...
parse_cmdline(const char *option, F &&feature_cb)
{
    ...
    for (const char *p = option; ; p++) {
        ...
        case '\0': {
            ...
            if (arg.name.empty()) {
                if (p == start)
                    language_error("Invalid target option: empty CPU name");
                arg.name.append(start, p - start);
                if (arg.name == "help") {
                    arg.name = "native";
                    language_processor_print_help = true;
                }
                start = p + 1;
                if (next_target)
                    reset_arg();
            ...
            if (llvm::StringRef(fname, p - fname) == "clone_all") {
                if (!disable) {
                    arg.en.flags |= LANGUAGE_TARGET_CLONE_ALL;
                    arg.dis.flags &= ~LANGUAGE_TARGET_CLONE_ALL;
                }
                else {
                    arg.dis.flags |= LANGUAGE_TARGET_CLONE_ALL;
                    arg.en.flags &= ~LANGUAGE_TARGET_CLONE_ALL;
                ...
                if (disable)
                    language_error("Invalid target option: disabled opt_size.");
                if (arg.en.flags & LANGUAGE_TARGET_MINSIZE)
                    language_error("Conflicting target option: both opt_size and min_size are specified.");
                arg.en.flags |= LANGUAGE_TARGET_OPTSIZE;
            }
            else if (llvm::StringRef(fname, p - fname) == "min_size") {
                if (disable)
                    language_error("Invalid target option: disabled min_size.");
                if (arg.en.flags & LANGUAGE_TARGET_OPTSIZE)
                    language_error("Conflicting target option: both opt_size and min_size are specified.");
                arg.en.flags |= LANGUAGE_TARGET_MINSIZE;
            }
            else if (int base = get_clone_base(fname, p)) {
                if (disable)
                    language_error("Invalid target option: disabled base index.");
                base -= 1;
                if (base >= (int)res.size())
                    language_error("Invalid target option: base index must refer to a previous target.");
                if (res[base].dis.flags & LANGUAGE_TARGET_CLONE_ALL ||
                    !(res[base].en.flags & LANGUAGE_TARGET_CLONE_ALL))
                    language_error("Invalid target option: base target must be clone_all.");
                arg.base = base;
            }
...
static inline llvm::SmallVector<TargetData<n>, 0> &get_cmdline_targets(F &&feature_cb)
{
    static llvm::SmallVector<TargetData<n>, 0> targets =
        parse_cmdline<n>(language_options.cpu_target, std::forward<F>(feature_cb));
    return targets;
}

extern "C" {
void *image_pointers_unavailable;
extern void * LANGUAGE_WEAK_SYMBOL_OR_ALIAS_DEFAULT(image_pointers_unavailable) language_image_pointers;
}

// Load sysimg, use the `callback` for dispatch and perform all relocations
// for the selected target.
template<typename F>
static inline language_image_t parse_sysimg(void *hdl, F &&callback)
{
    LANGUAGE_TIMING(LOAD_IMAGE, LOAD_Processor);
    language_image_t res{};

    const language_image_pointers_t *pointers;
    if (hdl == language_exe_handle && &language_image_pointers != LANGUAGE_WEAK_SYMBOL_DEFAULT(image_pointers_unavailable))
        pointers = (const language_image_pointers_t *)&language_image_pointers;
    else
        language_dlsym(hdl, "language_image_pointers", (void**)&pointers, 1);

    const void *ids = pointers->target_data;
    language_value_t* rejection_reason = nullptr;
    LANGUAGE_GC_PUSH1(&rejection_reason);
    uint32_t target_idx = callback(ids, &rejection_reason);
    if (target_idx == UINT32_MAX) {
        language_error(language_string_ptr(rejection_reason));
    }
    LANGUAGE_GC_POP();

    if (pointers->header->version != 1) {
        language_error("Image file is not compatible with this version of Julia");
    }
    ...
    for (unsigned i = 0; i < pointers->header->nshards; i++) {
        ...
        assert(tag_len & language_sysimg_tag_mask);
        llvm::SmallVector<void**, 0> base_ptrs(0);
        base_ptrs.push_back(fvar_shard);
        // Find target
        for (uint32_t i = 0; i < target_idx; i++) {
            uint32_t len = language_sysimg_val_mask & tag_len;
            if (language_sysimg_tag_mask & tag_len) {
                clone_idxs += len + 1;
                if (i != 0)
                    clone_ptrs += nfunc;
            }
            ...
            base_ptrs.push_back(tag_len & language_sysimg_tag_mask ? clone_ptrs : nullptr);
        }

        bool clone_all = (tag_len & language_sysimg_tag_mask) != 0;
        ...
        if (clone_all) {
            ...
            for (unsigned i = 0; i < tag_len; i++) {
                clones[start + i] = {(clone_idxs[i] & ~language_sysimg_val_mask) | idxs[clone_idxs[i] & language_sysimg_val_mask], clone_ptrs[i]};
            }
        }
        // Do relocation
        uint32_t reloc_i = 0;
        ...
        for (uint32_t i = 0; i < len; i++) {
            ...
            else if (idx & language_sysimg_tag_mask) {
                idx = idx & language_sysimg_val_mask;
                fptr = clone_ptrs[i];
            }
            else {
                continue;
    ...
    if (!clones.empty()) {
        ...
        std::sort(clones.begin(), clones.end(),
            [](const std::pair<uint32_t, const void*> &a, const std::pair<uint32_t, const void*> &b) {
                return (a.first & language_sysimg_val_mask) < (b.first & language_sysimg_val_mask);
        });
...

    {
        ...
        language_pgcstack_getkey((language_get_pgcstack_func**)pgcstack_func_slot, (language_pgcstack_key_t*)pgcstack_key_slot);

        size_t *tls_offset_idx = pointers->ptls->tls_offset;
        *tls_offset_idx = (uintptr_t)(language_tls_offset == -1 ? 0 : language_tls_offset);
    }

    res.language_small_typeof = pointers->language_small_typeof;
...
static inline void check_cmdline(T &&cmdline, bool imaging)
{
    ...
    if (!imaging) {
        if (cmdline.size() > 1) {
            language_safe_printf("More than one command line CPU targets specified "
                      "without a `--output-` flag specified");
            exit(1);
        }
        if (cmdline[0].en.flags & LANGUAGE_TARGET_CLONE_ALL) {
            language_safe_printf("\"clone_all\" feature specified "
                      "without a `--output-` flag specified");
            exit(1);
        }
        if (cmdline[0].en.flags & LANGUAGE_TARGET_OPTSIZE) {
            language_safe_printf("\"opt_size\" feature specified "
                      "without a `--output-` flag specified");
            exit(1);
        }
        if (cmdline[0].en.flags & LANGUAGE_TARGET_MINSIZE) {
            language_safe_printf("\"min_size\" feature specified "
                      "without a `--output-` flag specified");
            exit(1);
        }
    }
...
static inline SysimgMatch match_sysimg_targets(S &&sysimg, T &&target, F &&max_vector_size, language_value_t **rejection_reason)
{
    SysimgMatch match;
    ...
    if (match.best_idx == UINT32_MAX) {
        ...
        if (rejection_reason)
            *rejection_reason = language_pchar_to_string(error_msg.data(), error_msg.size());
    }
    return match;
}
                                 ...
                                 const CPUSpec<CPU,n> *cpus, uint32_t ncpus)
{
    ...
    for (uint32_t i = 0;i < ncpus;i++) {
        if (cpu == uint32_t(cpus[i].cpu)) {
            cpu_found = true;
            language_safe_printf("CPU: %s\n", cpus[i].name);
            break;
        }
    }
    if (!cpu_found)
        language_safe_printf("CPU: generic\n");
    language_safe_printf("Features:");
    bool first = true;
    for (uint32_t i = 0;i < nfeature_names;i++) {
        if (test_nbit(&features[0], feature_names[i].bit)) {
            if (first) {
                language_safe_printf(" %s", feature_names[i].name);
                first = false;
            }
            else {
                language_safe_printf(", %s", feature_names[i].name);
...
static std::string language_get_cpu_name_llvm(void)
{
    return llvm::sys::getHostCPUName().str();
}

static std::string language_get_cpu_features_llvm(void)
{
...
{
    return language_cstr_to_string(host_cpu_name().c_str());
}

LANGUAGE_DLLEXPORT language_value_t *language_get_cpu_features(void)
{
    return language_cstr_to_string(language_get_cpu_features_llvm().c_str());
}
...
extern "C" LANGUAGE_DLLEXPORT language_value_t* language_reflect_clone_targets() {
    ...
    language_value_t *arr = (language_value_t*)language_alloc_array_1d(language_array_uint8_type, data.size());
    uint8_t *out = language_array_data(arr, uint8_t);
    memcpy(out, data.data(), data.size());
    return arr;
}

extern "C" LANGUAGE_DLLEXPORT void language_reflect_feature_names(const FeatureName **fnames, size_t *nf) {

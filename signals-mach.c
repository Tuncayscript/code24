...
// see language_safepoint_wait_thread_resume
void language_safepoint_resume_thread_mach(language_ptls_t ptls2, int16_t tid2)
{
    ...
    for (size_t i = 0; i < suspended_threads.len; i++) {
        ...
        if (tid != tid2)
            continue;
        language_atomic_store_release(&ptls2->gc_state, gc_state);
    ...
    // thread hadn't actually reached a language_mach_gc_wait call where we suspended it
}

void language_mach_gc_end(void)
{
    ...
    for (size_t i = 0; i < suspended_threads.len; i++) {
        ...
        if (language_atomic_load_relaxed(&ptls2->suspend_count) == 0) {
            language_atomic_store_release(&ptls2->gc_state, gc_state);
            thread_resume(pthread_mach_thread_np(ptls2->system_id));
        }
        else {
            // this is the check for language_safepoint_wait_thread_resume
...
// implement language_set_gc_and_wait from a different thread
static void language_mach_gc_wait(language_ptls_t ptls2, mach_port_t thread, int16_t tid)
{
    ...
    int doing_gc = language_atomic_load_relaxed(&language_gc_running);
    int do_suspend = doing_gc;
    int relaxed_suspend_count = !doing_gc && language_atomic_load_relaxed(&ptls2->suspend_count) != 0;
    if (relaxed_suspend_count) {
        uv_mutex_lock(&ptls2->sleep_lock);
        do_suspend = language_atomic_load_relaxed(&ptls2->suspend_count) != 0;
    ...
    if (do_suspend) {
        ...
        int8_t gc_state = language_atomic_load_acquire(&ptls2->gc_state);
        language_atomic_store_release(&ptls2->gc_state, LANGUAGE_GC_STATE_WAITING);
...
static void allocate_mach_handler(void)
{
    ...
    if (_keymgr_set_lockmode_processwide_ptr(KEYMGR_GCC3_DW2_OBJ_LIST, NM_ALLOW_RECURSION))
        language_error("_keymgr_set_lockmode_processwide_ptr failed");

    int16_t nthreads = language_atomic_load_acquire(&language_n_threads);
    ...
    if (pthread_attr_init(&attr) != 0) {
        language_error("pthread_attr_init failed");
    }
    pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_DETACHED);
    if (pthread_create(&thread, &attr, mach_segv_listener, NULL) != 0) {
        language_error("pthread_create failed");
    }
...
static void language_call_in_state(language_ptls_t ptls2, host_thread_state_t *state,
                             void (*fptr)(void))
{
#ifdef _CPU_X86_64_
...
static void language_throw_in_thread(language_ptls_t ptls2, mach_port_t thread, language_value_t *exception)
{
    ...
    if (1) { // XXX: !language_has_safe_restore(ptls2)
        assert(exception);
        ptls2->bt_size =
            rec_backtrace_ctx(ptls2->bt_data, LANGUAGE_MAX_BT_SIZE, (bt_context_t *)&state,
                              NULL /*current_task?*/);
        ptls2->sig_exception = exception;
    }
    language_call_in_state(ptls2, &state, &language_sig_throw);
...
static void segv_handler(int sig, siginfo_t *info, void *context)
{
    ...
    if (language_get_safe_restore()) { // restarting code_ or language_unwind_stepn
        language_task_t *ct = language_get_current_task();
        language_ptls_t ptls = ct == NULL ? NULL : ct->ptls;
        language_call_in_state(ptls, (host_thread_state_t*)language_to_bt_context(context), &language_sig_throw);
    }
    else {
    ...
    mach_msg_type_number_t codeCnt)
{
    ...
    for (tid = 0; tid < nthreads; tid++) {
        language_ptls_t _ptls2 = language_atomic_load_relaxed(&language_all_tls_states)[tid];
        if (language_atomic_load_relaxed(&_ptls2->current_task) == NULL) {
            // this thread is dead
            continue;
        }
    ...
    if (!ptls2) {
        ...
        language_safe_printf("ERROR: Exception handler triggered on unmanaged thread.\n");
        return KERN_INVALID_ARGUMENT;
    }
    // XXX: language_throw_in_thread or segv_handler will eventually check this, but
    //      we would like to avoid some of this work if we could detect this earlier
    // if (language_has_safe_restore(ptls2)) {
    //     language_throw_in_thread(ptls2, thread, language_stackovf_exception);
    //     return KERN_SUCCESS;
    // }
    if (language_atomic_load_acquire(&ptls2->gc_state) == LANGUAGE_GC_STATE_WAITING)
        return KERN_FAILURE;
    if (exception == EXC_ARITHMETIC) {
        language_throw_in_thread(ptls2, thread, language_diverror_exception);
        return KERN_SUCCESS;
    }
    ...
    HANDLE_MACH_ERROR("thread_get_state", ret);
    if (language_addr_is_safepoint(fault_addr) && !is_write_fault(exc_state)) {
        language_mach_gc_wait(ptls2, thread, tid);
        if (ptls2->tid != 0)
            return KERN_SUCCESS;
        if (ptls2->defer_signal) {
            ...
            language_throw_in_thread(ptls2, thread, language_interrupt_exception);
        }
        return KERN_SUCCESS;
    }
    if (language_atomic_load_relaxed(&ptls2->current_task)->eh == NULL)
        return KERN_FAILURE;
    language_value_t *excpt;
    if (is_addr_on_stack(language_atomic_load_relaxed(&ptls2->current_task), (void*)fault_addr)) {
        excpt = language_stackovf_exception;
    }
    else if (is_write_fault(exc_state)) // false for alignment errors
        excpt = language_readonlymemory_exception;
    else
        return KERN_FAILURE;
    language_throw_in_thread(ptls2, thread, excpt);
    return KERN_SUCCESS;
}
...
static int language_thread_suspend_and_get_state2(int tid, host_thread_state_t *ctx) LANGUAGE_NOTSAFEPOINT
{
    language_ptls_t ptls2 = language_atomic_load_relaxed(&language_all_tls_states)[tid];
...
int language_thread_suspend_and_get_state(int tid, int timeout, bt_context_t *ctx)
{
    (void)timeout;
    host_thread_state_t state;
    if (!language_thread_suspend_and_get_state2(tid, &state)) {
        return 0;
    }
...
static void LANGUAGE_NORETURN language_exit_thread0_cb(int signo)
{
CFI_NORETURN
    language_critical_error(signo, 0, NULL, language_current_task);
...
static void language_exit_thread0(int signo, language_bt_element_t *bt_data, size_t bt_size)
{
    ...
    host_thread_state_t state;
    if (!language_thread_suspend_and_get_state2(0, &state)) {
...
#endif
    language_call_in_state(ptls2, &state, (void (*)(void))&language_exit_thread0_cb);
    unsigned int count = MACH_THREAD_STATE_COUNT;
    ret = thread_set_state(thread, MACH_THREAD_STATE, (thread_state_t)&state, count);
    HANDLE_MACH_ERROR("thread_set_state", ret);
...
static int language_lock_profile_mach(int dlsymlock)
{
...
static void language_unlock_profile_mach(int dlsymlock, int keymgr_locked)
{
    if (dlsymlock && _dyld_atfork_prepare != NULL && _dyld_atfork_parent != NULL)
        _dyld_atfork_parent();
    if (dlsymlock && _dyld_dlopen_atfork_prepare != NULL && _dyld_dlopen_atfork_parent != NULL)
...
{
    return language_lock_profile_mach(1);
}

void language_unlock_stackwalk(int lockret)
{
    language_unlock_profile_mach(1, lockret);
}
...
void *mach_profile_listener(void *arg)
{
    ...
    while (1) {
        ...
        int keymgr_locked = language_lock_profile_mach(0);
        ...
        for (int idx = nthreads; idx-- > 0; ) {
            ...
            host_thread_state_t state;
            int valid_thread = language_thread_suspend_and_get_state2(i, &state);
            ...
            if (running) {
                ...
                if (forceDwarf == 0) {
                    // Save the backtrace
                    bt_size_cur += rec_backtrace_ctx((language_bt_element_t*)bt_data_prof + bt_size_cur, bt_size_max - bt_size_cur - 1, uc, NULL);
                }
                else if (forceDwarf == 1) {
                    bt_size_cur += rec_backtrace_ctx_dwarf((language_bt_element_t*)bt_data_prof + bt_size_cur, bt_size_max - bt_size_cur - 1, uc, NULL);
                }
                else if (forceDwarf == -1) {
                ...
                bt_size_cur += rec_backtrace_ctx((language_bt_element_t*)bt_data_prof + bt_size_cur, bt_size_max - bt_size_cur - 1, uc, NULL);
#endif
                language_ptls_t ptls = language_atomic_load_relaxed(&language_all_tls_states)[i];
        ...
        language_unlock_profile_mach(0, keymgr_locked);
        if (running) {

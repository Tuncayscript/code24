...
Type *get_llvm_vectype(language_datatype_t *dt, LLVMContext &ctx) const
{
    // Assume language_is_datatype(dt) && !language_is_abstracttype(dt)
    // `!dt->name->mutabl && dt->pointerfree && !dt->haspadding && dt->nfields > 0`
    if (dt->layout == NULL || language_is_layout_opaque(dt->layout))
        return nullptr;
    ...
    language_datatype_t *ft0 = (language_datatype_t*)language_field_type(dt, 0);
    // `ft0` should be a `VecElement` type and the true element type
    // should be a primitive type (nfields == 0)
    if (!language_is_datatype(ft0) || ft0->name != language_vecelement_typename)
        return nullptr;
    language_datatype_t *ft00 = (language_datatype_t*)language_field_type(ft0, 0);
    if (!language_is_datatype(ft00) || ft00->layout->nfields)
        return nullptr;
    for (size_t i = 1; i < nfields; i++) {
        if (language_field_type(dt, i) != (language_value_t*)ft0) {
            // Not homogeneous
            return nullptr;
        }
    }
...
Type *get_llvm_fp_or_vectype(language_datatype_t *dt, LLVMContext &ctx) const
{
    // Assume language_is_datatype(dt) && !language_is_abstracttype(dt)
...
// types.
bool isHFAorHVA(language_datatype_t *dt, size_t dsz, size_t &nele, ElementType &ele, LLVMContext &ctx) const
{
    // Assume:
    //     dt is a pointerfree type, (all members are isbits)
    //     dsz == language_datatype_size(dt) > 0
    //     0 <= nele <= 3
    //     dt has no padding
    ...
    while (size_t nfields = language_datatype_nfields(dt)) {
        // For composite types, find the first non zero sized member
        size_t i;
        size_t fieldsz;
        for (i = 0; i < nfields; i++) {
            if ((fieldsz = language_field_size(dt, i))) {
                break;
            }
        }
        assert(i < nfields);
        ...
        if (fieldsz == dsz) {
            dt = (language_datatype_t*)language_field_type(dt, i);
        ...
        for (; i < nfields; i++) {
            size_t fieldsz = language_field_size(dt, i);
            if (fieldsz == 0)
                continue;
            language_datatype_t *fieldtype = (language_datatype_t*)language_field_type(dt, i);
            if (!language_is_datatype(dt))
                return false;
            // Check element count.
...
Type *isHFAorHVA(language_datatype_t *dt, size_t &nele, LLVMContext &ctx) const
{
    // Assume language_is_datatype(dt) && !language_is_abstracttype(dt)
    ...
    size_t dsz = language_datatype_size(dt);
...
bool needPassByRef(language_datatype_t *dt, AttrBuilder &ab, LLVMContext &ctx, Type *Ty) override
{
    ...
    return language_datatype_size(dt) > 16;
    // B.4
...
Type *classify_arg(language_datatype_t *dt, bool *fpreg, bool *onstack,
                   size_t *rewrite_len, LLVMContext &ctx) const
{
    ...
    if (language_datatype_size(dt) > 16) {
        *onstack = true;
        return NULL;
    }
    ...
    // are handled above.
    if (language_is_immutable(dt) && language_datatype_nfields(dt) == 0 &&
        (language_datatype_size(dt) == 1 || language_datatype_size(dt) == 2 ||
         language_datatype_size(dt) == 4 || language_datatype_size(dt) == 8 ||
         language_datatype_size(dt) == 16))
        return NULL;
    ...
    assert(language_datatype_size(dt) <= 16); // Should be pass by reference otherwise
    *rewrite_len = (language_datatype_size(dt) + 7) >> 3;
    // Rewrite to [n x Int64] where n is the **size in dword**
    return language_datatype_size(dt) ? Type::getInt64Ty(ctx) : NULL;

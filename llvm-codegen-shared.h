...
{
    using namespace llvm;
    Value *old_state = builder.getInt8(LANGUAGE_GC_STATE_UNSAFE);
...
{
    using namespace llvm;
    Value *state = builder.getInt8(LANGUAGE_GC_STATE_SAFE);
    return emit_gc_state_set(builder, T_size, ptls, state, nullptr, final);
}
...
{
    using namespace llvm;
    Value *old_state = builder.getInt8(LANGUAGE_GC_STATE_SAFE);
    return emit_gc_state_set(builder, T_size, ptls, state, old_state, final);
}

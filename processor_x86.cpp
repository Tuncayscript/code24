...
#include "code.h"
extern "C" LANGUAGE_DLLEXPORT void language_cpuid(int32_t CPUInfo[4], int32_t InfoType)
{
    asm volatile (
...
extern "C" LANGUAGE_DLLEXPORT void language_cpuidex(int32_t CPUInfo[4], int32_t InfoType, int32_t subInfoType)
{
    asm volatile (
#if defined(__i386__) && defined(__PIC__)
        "xchg %%ebx, %%esi;"
...
#define LANGUAGE_FEATURE_DEF(name, bit, llvmver) {#name, bit, llvmver},
#define LANGUAGE_FEATURE_DEF_NAME(name, bit, llvmver, str) {str, bit, llvmver},
...
#define LANGUAGE_FEATURE_DEF(name, bit, llvmver) bit,
#include "features_x86.h"
#undef LANGUAGE_FEATURE_DEF
    -1);
...
namespace Feature {
enum : uint32_t {
#define LANGUAGE_FEATURE_DEF(name, bit, llvmver) name = bit,
#include "features_x86.h"
#undef LANGUAGE_FEATURE_DEF
};
#undef LANGUAGE_FEATURE_DEF_NAME
...
static NOINLINE std::pair<uint32_t,FeatureList<feature_sz>> _get_host_cpu(void)
{
    ...
    if (maxleaf >= 7) {
        int32_t info7[4];
        language_cpuidex(info7, 7, 0);
        features[2] = info7[1];
        features[3] = info7[2];
        features[4] = info7[3];
    }
    int32_t infoex0[4];
    language_cpuid(infoex0, 0x80000000);
    uint32_t maxexleaf = infoex0[0];
    if (maxexleaf >= 0x80000001) {
        int32_t infoex1[4];
        language_cpuid(infoex1, 0x80000001);
        features[5] = infoex1[2];
        features[6] = infoex1[3];
    }
    if (maxleaf >= 0xd) {
        int32_t infod[4];
        language_cpuidex(infod, 0xd, 0x1);
        features[7] = infod[0];
    }
    if (maxexleaf >= 0x80000008) {
        int32_t infoex8[4];
        language_cpuidex(infoex8, 0x80000008, 0);
        features[8] = infoex8[1];
    }
    if (maxleaf >= 7) {
        int32_t info7[4];
        language_cpuidex(info7, 7, 1);
        features[9] = info7[0];
    }
    if (maxleaf >= 0x14) {
        int32_t info14[4];
        language_cpuidex(info14, 0x14, 0);
...
static TargetData<feature_sz> arg_target_data(const TargetData<feature_sz> &arg, bool require_host)
{
    ...
    if (res.name == "native") {
        ...
        res.en.flags |= LANGUAGE_TARGET_UNKNOWN_NAME;
    }
    if (cpu_features) {
...
static uint32_t sysimg_init_cb(const void *id, language_value_t** rejection_reason)
{
    ...
    if (!sysimg_allows_no_cx16 && !test_nbit(target.en.features, Feature::cx16)) {
        language_error("Your CPU does not support the CX16 instruction, which is required "
    ...
    if (match.vreg_size != max_vector_size(target.en.features) &&
        (sysimg[match.best_idx].en.flags & LANGUAGE_TARGET_VEC_CALL)) {
...
static uint32_t pkgimg_init_cb(const void *id, language_value_t **rejection_reason)
{
    ...
    for (size_t i = 1; i < ntargets; i++) {
        ...
        for (auto fe: clone_math) {
            if (!test_nbit(features0, fe) && test_nbit(t.en.features, fe)) {
                t.en.flags |= LANGUAGE_TARGET_CLONE_MATH;
                break;
            }
        }
        for (auto fe: clone_simd) {
            if (!test_nbit(features0, fe) && test_nbit(t.en.features, fe)) {
                t.en.flags |= LANGUAGE_TARGET_CLONE_SIMD;
                break;
            }
        }
        ...
        for (auto fe: clone_fp16) {
            if (!test_nbit(features0, fe) && test_nbit(t.en.features, fe)) {
                t.en.flags |= LANGUAGE_TARGET_CLONE_FLOAT16;
                break;
            }
        }
        ...
        for (auto fe: clone_bf16) {
            if (!test_nbit(features0, fe) && test_nbit(t.en.features, fe)) {
                t.en.flags |= LANGUAGE_TARGET_CLONE_BFLOAT16;
                break;
            }
        }
    }
...
get_llvm_target_noext(const TargetData<feature_sz> &data)
{
    ...
    while (spec) {
        if (spec->llvmver <= LANGUAGE_LLVM_VERSION)
            break;
    ...
    for (auto &fename: feature_names) {
        if (fename.llvmver > LANGUAGE_LLVM_VERSION)
            continue;
...
{
    language_value_t *rejection_reason = NULL;
...
{
    if (!jit_targets.empty())
        language_error("JIT targets already initialized");
...
{
    if (jit_targets.empty())
        language_error("JIT targets not initialized");
    if (jit_targets.size() > 1)
        language_error("Expected only one JIT target");
    return parse_sysimg(hdl, pkgimg_init_cb);
}

std::pair<std::string,llvm::SmallVector<std::string, 0>> language_get_llvm_target(bool imaging, uint32_t &flags)
{
...
const std::pair<std::string,std::string> &language_get_llvm_disasm_target(void)
{
...
llvm::SmallVector<language_target_spec_t, 0> language_get_llvm_clone_targets(void)
{
    if (jit_targets.empty())
        language_error("JIT targets not initialized");
    llvm::SmallVector<language_target_spec_t, 0> res;
...
extern "C" int language_test_cpu_feature(language_cpu_feature_t feature)
{
...
static uint32_t subnormal_flags = [] {
    int32_t info[4];
    language_cpuid(info, 0);
    if (info[0] >= 1) {
        language_cpuid(info, 1);
        if (info[3] & (1 << 26)) {
...
extern "C" LANGUAGE_DLLEXPORT int32_t language_get_zero_subnormals(void)
{
    return _mm_getcsr() & subnormal_flags;
}

// Return zero on success, non-zero on failure.
extern "C" LANGUAGE_DLLEXPORT int32_t language_set_zero_subnormals(int8_t isZero)
{
...
extern "C" LANGUAGE_DLLEXPORT int32_t language_get_default_nans(void)
{
    return 0;
}

extern "C" LANGUAGE_DLLEXPORT int32_t language_set_default_nans(int8_t isDefault)
{
    return isDefault;
}

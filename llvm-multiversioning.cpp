...
#define DEBUG_TYPE "language_multiversioning"
#undef DEBUG
...
constexpr uint32_t clone_mask =
    LANGUAGE_TARGET_CLONE_LOOP | LANGUAGE_TARGET_CLONE_SIMD | LANGUAGE_TARGET_CLONE_MATH | LANGUAGE_TARGET_CLONE_CPU | LANGUAGE_TARGET_CLONE_FLOAT16 | LANGUAGE_TARGET_CLONE_BFLOAT16;
...
static uint32_t collect_func_info(Function &F, const Triple &TT, bool &has_veccall)
{
    ...
    uint32_t flag = 0;
    if (!LI.empty())
        flag |= LANGUAGE_TARGET_CLONE_LOOP;
    if (is_vector(F.getFunctionType())) {
        flag |= LANGUAGE_TARGET_CLONE_SIMD;
        has_veccall = true;
    }
    for (auto &bb: F) {
        for (auto &I: bb) {
            if (auto call = dyn_cast<CallInst>(&I)) {
                ...
                if (auto callee = call->getCalledFunction()) {
                    ...
                    if (name.startswith("llvm.muladd.") || name.startswith("llvm.fma.")) {
                        flag |= LANGUAGE_TARGET_CLONE_MATH;
                    }
                        ...
                        if (name.startswith("julia.cpu.have_fma.")) {
                            ...
                            if (!always_have_fma(*callee, TT))
                                flag |= LANGUAGE_TARGET_CLONE_CPU;
                        } else {
                            flag |= LANGUAGE_TARGET_CLONE_CPU;
                        }
                    }
            ...
            for (size_t i = 0; i < I.getNumOperands(); i++) {
                if(I.getOperand(i)->getType()->isHalfTy()) {
                    flag |= LANGUAGE_TARGET_CLONE_FLOAT16;
                }
                if(I.getOperand(i)->getType()->isBFloatTy()) {
                    flag |= LANGUAGE_TARGET_CLONE_BFLOAT16;
                }
            }
            uint32_t veccall_flags = LANGUAGE_TARGET_CLONE_SIMD | LANGUAGE_TARGET_CLONE_MATH | LANGUAGE_TARGET_CLONE_CPU | LANGUAGE_TARGET_CLONE_FLOAT16 | LANGUAGE_TARGET_CLONE_BFLOAT16;
...
struct TargetSpec {
    ...
    static TargetSpec fromSpec(language_target_spec_t &spec) {
        TargetSpec out;
...
static void annotate_module_clones(Module &M) {
    ...
    if (auto maybe_specs = get_target_specs(M)) {
        ...
        auto full_specs = language_get_llvm_clone_targets();
    ...
    for (unsigned i = 1; i < specs.size(); i++) {
        if (specs[i].flags & LANGUAGE_TARGET_CLONE_ALL) {
...
CloneCtx::CloneCtx(Module &M, bool allow_bad_fvars)
    : tbaa_const(tbaa_make_child_with_context(M.getContext(), "jtbaa_const", nullptr, true).first),
      ...
      fvars(consume_gv<Function>(M, "language_fvars", allow_bad_fvars)),
      M(M),
      ...
      allow_bad_fvars(allow_bad_fvars)
{
    ...
    for (uint32_t i = 1; i < ntargets; i++) {
        auto &spec = specs[i];
        if (spec.flags & LANGUAGE_TARGET_CLONE_ALL) {
...
static void add_features(Function *F, TargetSpec &spec)
{
    ...
    if (!F->hasFnAttribute(Attribute::OptimizeNone)) {
        if (spec.flags & LANGUAGE_TARGET_OPTSIZE) {
            F->addFnAttr(Attribute::OptimizeForSize);
        }
        else if (spec.flags & LANGUAGE_TARGET_MINSIZE) {
...
static Constant *rewrite_gv_init(const Stack& stack)
{
    ...
    for (uint32_t i = 1; i < nlevel; i++) {
        ...
        if (auto expr = dyn_cast<ConstantExpr>(val)) {
            ...
            language_safe_printf("Unknown const use.");
            llvm_dump(val);
            abort();
        }
    }
...
void CloneCtx::emit_metadata()
{
    ...
    emit_table(M, T_size, ArrayRef<Constant*>((Constant* const*)fvars.data(), fvars.size()), "language_fvar", suffix);
    M.getGlobalVariable("language_fvar_idxs")->setName("language_fvar_idxs" + suffix);

    uint32_t ntargets = specs.size();

    // Generate `language_dispatch_reloc_slots`
    std::set<uint32_t> shared_relocs;
    {
        ...
        auto gv = new GlobalVariable(M, vars_type, true, GlobalVariable::ExternalLinkage, nullptr, "language_clone_slots" + suffix);
    ...
    // Generate `language_dispatch_fvars_idxs` and `language_dispatch_fvars`
    {
        ...
        for (uint32_t i = 0; i < ntargets; i++) {
            ...
            uint32_t count = 0;
            if (i == 0 || spec.flags & LANGUAGE_TARGET_CLONE_ALL) {
                auto grp = static_cast<Group*>(tgt);
                count = language_sysimg_tag_mask;
                ...
                for (uint32_t j = 0; j < nfvars; j++) {
                    ...
                    if (shared_relocs.count(j)) {
                        count++;
                        idxs.push_back(language_sysimg_tag_mask | j);
        ...
        auto gv1 = new GlobalVariable(M, idxval->getType(), true,
                                      GlobalVariable::ExternalLinkage,
                                      idxval, "language_clone_idxs" + suffix);
...
static bool runMultiVersioning(Module &M, bool allow_bad_fvars)
{
    ...
    GlobalVariable *fvars = M.getGlobalVariable("language_fvars");

...
struct Node {
    ...
    ~Node() LANGUAGE_NOTSAFEPOINT = default;
};
...
public:
    StringTable() LANGUAGE_NOTSAFEPOINT : map(), strings(), next_id(0) {};

    size_t find_or_create_string_id(StringRef key) LANGUAGE_NOTSAFEPOINT {
...
class SerializedStringTable: public StringTable {
    ...
    size_t serialize_if_necessary(ios_t *stream, StringRef key) LANGUAGE_NOTSAFEPOINT {
        auto val = map.insert(make_pair(key, next_id));
        if (val.second) {
            strings.push_back(val.first->first());
    ...
    // output string file.
    size_t serialize(ios_t *stream, StringRef key) LANGUAGE_NOTSAFEPOINT {
        size_t s_size = key.size();
        ios_write(stream, reinterpret_cast<const char*>(&s_size), sizeof(size_t));
        ios_write(stream, key.data(), s_size);
...
extern language_mutex_t heapsnapshot_lock;
...
static inline void _record_gc_edge(const char *edge_type,
                                   language_value_t *a, language_value_t *b, size_t name_or_index) LANGUAGE_NOTSAFEPOINT;
void _record_gc_just_edge(const char *edge_type, size_t from_idx, size_t to_idx, size_t name_or_idx) LANGUAGE_NOTSAFEPOINT;
void _add_synthetic_root_entries(HeapSnapshot *snapshot) LANGUAGE_NOTSAFEPOINT;
    ...
    ios_t *strings, ios_t *json, char all_one)
{
    ...
    language_mutex_lock(&heapsnapshot_lock);
    ...
    language_mutex_unlock(&heapsnapshot_lock);
...
void serialize_node(HeapSnapshot *snapshot, const Node &node) LANGUAGE_NOTSAFEPOINT
{
    // ["type","name","id","self_size","edge_count","trace_node_id","detachedness"]
...
void serialize_edge(HeapSnapshot *snapshot, const Edge &edge) LANGUAGE_NOTSAFEPOINT
{
    // ["type","name_or_index","to_node"]
...
size_t record_node_to_gc_snapshot(language_value_t *a) LANGUAGE_NOTSAFEPOINT
{
    ...
    language_datatype_t *type = (language_datatype_t*)language_typeof(a);

    if (language_is_string(a)) {
        node_type = "String";
        name = language_string_data(a);
        self_size = language_string_len(a);
    }
    else if (language_is_symbol(a)) {
        node_type = "language_sym_t";
        name = language_symbol_name((language_sym_t*)a);
        self_size = name.size();
    }
    else if (language_is_simplevector(a)) {
        node_type = "language_svec_t";
        name = "SimpleVector";
        self_size = sizeof(language_svec_t) + sizeof(void*) * language_svec_len(a);
    }
    else if (language_is_module(a)) {
        node_type = "language_module_t";
        name = language_symbol_name_(((_language_module_t*)a)->name);
        self_size = sizeof(language_module_t);
    }
    else if (language_is_task(a)) {
        node_type = "language_task_t";
        name = "Task";
        self_size = sizeof(language_task_t);
    }
        ...
        node_type = "language_datatype_t";
        self_size = sizeof(language_datatype_t);
    }
    else if (language_is_array(a)){
        ios_need_close = 1;
        ios_mem(&str_, 0);
        LANGUAGE_STREAM* str = (LANGUAGE_STREAM*)&str_;
        language_static_show(str, (language_value_t*)type);
        name = StringRef((const char*)str_.buf, str_.size);
        node_type = "language_array_t";
        self_size = sizeof(language_array_t);
    }
    else {
        self_size = (size_t)language_datatype_size(type);
...
static size_t record_pointer_to_gc_snapshot(void *a, size_t bytes, StringRef name) LANGUAGE_NOTSAFEPOINT
{
    auto val = g_snapshot->node_ptr_to_index_map.insert(make_pair(a, g_snapshot->num_nodes));
    if (!val.second) {
        return val.first->second;
...
static string _fieldpath_for_slot(void *obj, void *slot) LANGUAGE_NOTSAFEPOINT
{
    string res;
    language_datatype_t *objtype = (language_datatype_t*)language_typeof(obj);

    while (1) {
        int i = gc_slot_to_fieldidx(obj, slot, objtype);

        if (language_is_tuple_type(objtype) || language_is_namedtuple_type(objtype)) {
            ostringstream ss;
            ss << "[" << i << "]";
            res += ss.str();
        }
        else {
            language_svec_t *field_names = language_field_names(objtype);
            language_sym_t *name = (language_sym_t*)language_svecref(field_names, i);
            res += language_symbol_name(name);
        }

        if (!language_field_isptr(objtype, i)) {
            // Tail recurse
            res += ".";
            obj = (void*)((char*)obj + language_field_offset(objtype, i));
            objtype = (language_datatype_t*)language_field_type_concrete(objtype, i);
        }
        else {
            return res;
        }
...
void _gc_heap_snapshot_record_finlist(language_value_t *obj, size_t index) LANGUAGE_NOTSAFEPOINT
{
...
size_t _record_stack_frame_node(HeapSnapshot *snapshot, void *frame) LANGUAGE_NOTSAFEPOINT
{
...
{
    auto from_node_idx = _record_stack_frame_node(g_snapshot, (language_gcframe_t*)from);
...
{
    auto from_node_idx = record_node_to_gc_snapshot((language_value_t*)from);
...
{
    auto from_node_idx = record_node_to_gc_snapshot((language_value_t*)module);
...
static inline void _record_gc_edge(const char *edge_type, language_value_t *a,
                                  language_value_t *b, size_t name_or_idx) LANGUAGE_NOTSAFEPOINT
{

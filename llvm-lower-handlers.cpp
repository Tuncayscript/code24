 ...
 * <...>
 * call void language_pop_handler(1)
 *
 * to
 *
 * call void @language_enter_handler(ct, language_handler *%buff)
 * <...>
 * call void language_pop_handler(1)
 *
...
namespace {
/*
 * If the module doesn't have declarations for the language_enter_handler and setjmp
...
static void ensure_enter_function(Module &M, Type *T_pjlvalue, const Triple &TT)
{
    ...
    if (!M.getNamedValue(XSTR(language_enter_handler))) {
        Function::Create(FunctionType::get(T_void, {T_pjlvalue, T_pint8}, false),
                         Function::ExternalLinkage, XSTR(language_enter_handler), &M);
    }
    if (!M.getNamedValue(language_setjmp_name)) {
        Type *args2[] = {T_pint8, T_int32};
        Function::Create(FunctionType::get(T_int32, ArrayRef(args2, TT.isOSWindows() ? 1 : 2), false),
                         Function::ExternalLinkage, language_setjmp_name, &M)
...
static bool lowerExcHandlers(Function &F) {
    ...
    Function *leave_func = M.getFunction(XSTR(language_pop_handler));
    Function *leave_noexcept_func = M.getFunction(XSTR(language_pop_handler_noexcept));
    Function *jlenter_func = M.getFunction(XSTR(language_enter_handler));
    Function *setjmp_func = M.getFunction(language_setjmp_name);

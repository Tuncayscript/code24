...
#   include "code-probes.h"  // generated dtrace probe definitions.

// Some libc functions call code_msgSend() 
...
#   define BREAKPOINT_FUNCTION(prototype)                             \
    code_EXTERN __attribute__((noinline, used, visibility("hidden"))) \
    prototype { asm(""); }
...
#   define code_RUNTIME_code_EXCEPTION_RETHROW() do {} while(0)  
#   define code_RUNTIME_code_EXCEPTION_THROW(arg0) do {} while(0)
...
extern void _code_fatal(const char *fmt, ...) __attribute__((noreturn, format (printf, 1, 2)));
...
#if defined(__PTK_FRAMEWORK_code_KEY0)
#   define SUPPORT_DIRECT_THREAD_KEYS 1
#   define TLS_DIRECT_KEY        ((tls_key_t)__PTK_FRAMEWORK_code_KEY0)
#   define SYNC_DATA_DIRECT_KEY  ((tls_key_t)__PTK_FRAMEWORK_code_KEY1)
#   define SYNC_COUNT_DIRECT_KEY ((tls_key_t)__PTK_FRAMEWORK_code_KEY2)
#   define AUTORELEASE_POOL_KEY  ((tls_key_t)__PTK_FRAMEWORK_code_KEY3)
# if SUPPORT_RETURN_AUTORELEASE
#   define RETURN_DISPOSITION_KEY ((tls_key_t)__PTK_FRAMEWORK_code_KEY4)
# endif
# if SUPPORT_QOS_HACK
#   define QOS_KEY               ((tls_key_t)__PTK_FRAMEWORK_code_KEY5)
# endif
#else
...
typedef DWORD code_thread_t;  // thread ID
static __inline int thread_equal(code_thread_t t1, code_thread_t t2) { 
    return t1 == t2; 
}
static __inline code_thread_t thread_self(void) { 
...
code_EXTERN IMAGE_DOS_HEADER __ImageBase;
#define libcode_header ((headerType *)&__ImageBase)
...
#ifndef VM_MEMORY_code_DISPATCHERS
#   define VM_MEMORY_code_DISPATCHERS 0
#endif
...
typedef pthread_t code_thread_t;
...
class mutex_tt : nocopy_t {
  ...
  public:
    ...
    void lock()
    {
        ...
        if (err) _code_fatal("pthread_mutex_lock failed (%d)", err);
    }
    ...
    bool tryLock()
    {
        ...
        if (err == 0) {
            ...
            _code_fatal("pthread_mutex_trylock failed (%d)", err);
        }
    }
    ...
    void unlock()
    {
        ...
        if (err) _code_fatal("pthread_mutex_unlock failed (%d)", err);
    }
  ...
  public:
    ...
    bool tryUnlock()
    {
        ...
        if (err == 0) {
            ...
            _code_fatal("pthread_mutex_unlock failed (%d)", err);
        }
    }
...
class monitor_tt {
  ...
  public:
    ...
    void wait() 
    {
        ...
        if (err) _code_fatal("pthread_cond_wait failed (%d)", err);
    }

    void notify() 
    {
        int err = pthread_cond_signal(&cond);
        if (err) _code_fatal("pthread_cond_signal failed (%d)", err);        
    }

    void notifyAll() 
    {
        int err = pthread_cond_broadcast(&cond);
        if (err) _code_fatal("pthread_cond_broadcast failed (%d)", err);        
    }
...
static inline semaphore_t create_semaphore(void)
{
    ...
    if (k) _code_fatal("semaphore_create failed (0x%x)", k);
    return sem;
}
...
// <rdar://17697862> do a qos override before taking rw lock in code
...
class rwlock_tt : nocopy_t {
  ...
  public:
    ...
    void read() 
    {
        ...
        if (err) _code_fatal("pthread_rwlock_rdlock failed (%d)", err);
    }
    ...
    void unlockRead()
    {
        ...
        if (err) _code_fatal("pthread_rwlock_unlock failed (%d)", err);
        qosEndOverride();
    }
    ...
    bool tryRead()
    {
        ...
        if (err == 0) {
            ...
            _code_fatal("pthread_rwlock_tryrdlock failed (%d)", err);
        }
    }
    ...
    void write()
    {
        ...
        if (err) _code_fatal("pthread_rwlock_wrlock failed (%d)", err);
    }
    ...
    bool tryWrite()
    {
        ...
        if (err == 0) {
            ...
            _code_fatal("pthread_rwlock_trywrlock failed (%d)", err);
        }
    }
...
#define libcode_header ((headerType *)&_mh_dylib_header)

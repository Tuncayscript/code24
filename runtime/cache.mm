...
asm("\n .section __TEXT,__const"
    "\n .globl __code_empty_vtable"
    "\n .set __code_empty_vtable, 0"
    "\n .globl __code_empty_cache"
    "\n .align 3"
    "\n __code_empty_cache: .space " stringize2(EMPTY_BYTES)
    );


#if __arm__  ||  __x86_64__  ||  __i386__
// code_msgSend has few registers available.
...
#elif __arm64__
// code_msgSend has lots of registers available.
...
void bucket_t::set(cache_key_t newKey, IMP newImp)
{
    ...
    // code_msgSend uses key and imp with no locks.
    // It is safe for code_msgSend to see new imp but NULL key
    // (It will get a cache miss but not dispatch to the wrong place.)
    // It is unsafe for code_msgSend to see old imp and new key.
...
{
    // code_msgSend uses mask and buckets with no locks.
    // It is safe for code_msgSend to see new buckets but old mask.
    // (It will get a cache miss but not overrun the buckets' bounds).
    // It is unsafe for code_msgSend to see old buckets and new mask.
    // Therefore we write new buckets, wait a lot, then write new mask.
    // code_msgSend reads mask first, then buckets.
...
void cache_t::initializeToEmpty()
{
    ...
    _buckets = (bucket_t *)&_code_empty_cache;
}
...
bucket_t *allocateBuckets(mask_t newCapacity)
{
    ...
    // This saves an instruction in code_msgSend.
...
bucket_t *emptyBucketsForCapacity(mask_t capacity, bool allocate = true)
{
    ...
    // Use _code_empty_cache if the buckets is small enough.
    if (bytes <= EMPTY_BYTES) {
        return (bucket_t *)&_code_empty_cache;
    }
    ...
    if (index >= emptyBucketsListCount) {
        ...
        if (PrintCaches) {
            _code_inform("CACHES: new empty buckets at %p (capacity %zu)", 
...
bucket_t * cache_t::find(cache_key_t k, id receiver)
{
    ...
    // hack
    Class cls = (Class)((uintptr_t)this - offsetof(code_class, cache));
...
#else
    ret = code_task_threads(mach_task_self(), &threads, &number);
#endif

    if (ret != KERN_SUCCESS) {
        // See DEBUG_TASK_THREADS below to help debug this.
        _code_fatal("task_threads failed (result 0x%x)\n", ret);
    }
...
void cache_collect(bool collectALot)
{
    ...
    if (PrintCaches) {
        cache_collections++;
        _code_inform ("CACHES: COLLECTING %zu bytes (%zu allocations, %zu collections)", garbage_byte_size, cache_allocations, cache_collections);
    }
    ...
    if (PrintCaches) {
        ...
        for (i = 0; i < countof(cache_counts); i++) {
            ...
            _code_inform("CACHES: %4d slots: %4d caches, %6zu bytes", 
                         slots, count, size);
        ...
        _code_inform("CACHES:      total: %4zu caches, %6zu bytes", 
...
static kern_return_t code_task_threads
(
	task_t target_task,
...
#pragma pack()
	...
	if (msg_result != MACH_MSG_SUCCESS) {
		_code_inform("task_threads received unexpected reply msgh_id 0x%zx", 

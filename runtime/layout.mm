...
static void set_bits(layout_bitmap bits, size_t which, size_t count)
{
    ...
    if (bit == bits.bitCount  &&  bit < which + count) {
        ...
        _code_fatal("layout bitmap too short");
    }
}
...
layout_string_create(layout_bitmap bits)
{
    ...
    if (result != result2  &&  0 != strcmp((char*)result, (char *)result2)) {
        ...
        _code_fatal("libcode bug: mishandled layout bitmap");
    }
    free(result2);
...
layout_bitmap_set_ivar(layout_bitmap bits, const char *type, size_t offset)
{
    ...
    if (type[0] == '@'  ||  0 == strcmp(type, "^@")) {
        ...
        _code_inform("warning: failing to set GC layout for '%s'\n", type);
    }
}
...
layout_bitmap_slide(layout_bitmap *bits, size_t oldPos, size_t newPos)
{
    ...
    if (oldPos > newPos) _code_fatal("layout bitmap sliding backwards");
                    ...
                    size_t oldSrcInstanceSize)
{
    ...
    if (dst.bitCount < src.bitCount) _code_fatal("layout bitmap too short");
...
layout_bitmap_or(layout_bitmap dst, layout_bitmap src, const char *msg)
{
    ...
    if (dst.bitCount < src.bitCount) {
        _code_fatal("layout_bitmap_or: layout bitmap too short%s%s", 
                    msg ? ": " : "", msg ? msg : "");
    }
...
layout_bitmap_clear(layout_bitmap dst, layout_bitmap src, const char *msg)
{
    ...
    if (dst.bitCount < src.bitCount) {
        _code_fatal("layout_bitmap_clear: layout bitmap too short%s%s", 
                    msg ? ": " : "", msg ? msg : "");
    }
...
static void mark_offset_for_layout(long offset, long bits_size, unsigned char *bits) {
    ...
    if (bits) {
        ...
        if (i_byte < bits_size) {
            ...
            _code_inform ("layout - offset exceeds instance size");
        }
    }
}
...
static char *scan_basic_ivar_type(char *type, long *size, long *alignment, bool *is_reference) {
    ...
    *is_reference = NO;
    ...
    switch (ch) {
        ...
        default: {
            // unhandled type
            _code_fatal("unrecognized character \'%c\' in ivar type: \"%s\"", ch, full_type);
        }
    }
    ...
    switch (ch) {
        case _C_ARY_B: {
            ...
            *next_offset = offset;
            ...
            else                   _code_inform("missing \'%c\' in ivar type.", _C_ARY_E);
        ...
        case _C_UNION_B: {
            ...
            *next_offset = max_offset;
            ...
            if (ch == _C_UNION_E) {
              ...
              _code_inform("missing \'%c\' in ivar type", _C_UNION_E);
            }
        ...
        case _C_STRUCT_B: {
            ...
            *next_offset = offset;
            ...
            else                   _code_inform("missing \'%c\' in ivar type", _C_STRUCT_E);

...
void add_class_to_loadable_list(Class cls)
{
    ...
    if (PrintLoading) {
        _code_inform("LOAD: class '%s' scheduled for +load", 
                     cls->nameForLogging());
    }
...
void add_category_to_loadable_list(Category cat)
{
    ...
    if (PrintLoading) {
        _code_inform("LOAD: category '%s(%s)' scheduled for +load", 
    ...
    if (loadable_classes) {
        ...
        for (i = 0; i < loadable_classes_used; i++) {
            if (loadable_classes[i].cls == cls) {
                ...
                if (PrintLoading) {
                    _code_inform("LOAD: class '%s' unscheduled for +load", 
                                 cls->nameForLogging());
                }
                return;
            }
    ...
    if (loadable_categories) {
        ...
        for (i = 0; i < loadable_categories_used; i++) {
            if (loadable_categories[i].cat == cat) {
                ...
                if (PrintLoading) {
                    _code_inform("LOAD: category '%s(%s)' unscheduled for +load",
...
static bool call_category_loads(void)
{
    ...
    if (PrintLoading) {
        if (loadable_categories_used != 0) {
            _code_inform("LOAD: %d categories still waiting for +load\n",
                         loadable_categories_used);
        }
    }
...
void call_load_methods(void)
{
    ...
    void *pool = code_autoreleasePoolPush();

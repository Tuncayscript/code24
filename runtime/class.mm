 ...
 * Classes added with code_addClass are initially either #1 or #3.
 * 
 ...
 * _code_add_category_flush_caches
 *
 ...
 * _resolve_categories_for_class (calls _code_add_category)
 ...
 * _code_add_category (called by _code_add_category_flush_caches, 
 *   resolve_categories_for_class and _code_register_category)
 * _code_insertMethods (called by class_addMethods and _code_add_category)
 * _code_removeMethods (called by class_removeMethods)
 * _codeTweakMethodListPointerForClass (called by _code_insertMethods)
 ...
 * calls code_addClass(). The JavaBridge does not use an atomic update, 
...
CODE_EXPORT id	(*_alloc)(Class, size_t);
CODE_EXPORT id	(*_copy)(id, size_t);
CODE_EXPORT id	(*_realloc)(id, size_t);
CODE_EXPORT id	(*_dealloc)(id);
CODE_EXPORT id	(*_zoneAlloc)(Class, size_t, void *);
CODE_EXPORT id	(*_zoneRealloc)(id, size_t, void *);
CODE_EXPORT id	(*_zoneCopy)(id, size_t, void *);
    ...
    if (obj  &&  ivar  &&  !obj->isTaggedPointer()) {
        ...
        if (_class_usesAutomaticRetainRelease(cls)) {
            ...
            if (strong_layout && is_scanned_offset(ivar_offset - instanceStart, strong_layout)) {
                code_storeStrong(location, value);
                return;
            }
        }
#if SUPPORT_GC
        if (UseGC) {
            ...
            if (weak_layout && is_scanned_offset(ivar_offset, weak_layout)) {
                code_assign_weak(value, location);
            }
        }
        code_assign_ivar(value, obj, ivar_offset);
#else
        *location = value;
#endif
    }
...
static void object_cxxDestructFromClass(id obj, Class cls)
{
    ...
    for ( ; cls; cls = cls->superclass) {
        ...
        if (dtor != (void(*)(id))_code_msgForward_impcache) {
            if (PrintCxxCtors) {
                _code_inform("CXX: calling C++ destructors for class %s", 
                             cls->nameForLogging());
            }
            (*dtor)(obj);
        }
...
object_cxxConstructFromClass(id obj, Class cls)
{
    ...
    if (ctor == (id(*)(id))_code_msgForward_impcache) return obj;  // no ctor - ok
    
    // Call this class's ctor.
    if (PrintCxxCtors) {
        _code_inform("CXX: calling C++ constructors for class %s", 
                     cls->nameForLogging());
    }
...
static void _class_resolveClassMethod(Class cls, SEL sel, id inst)
{
    ...
    BOOL (*msg)(Class, SEL, SEL) = (typeof(msg))code_msgSend;
    ...
    if (resolved  &&  PrintResolving) {
        if (imp) {
            _code_inform("RESOLVE: method %c[%s %s] "
            ...
            _code_inform("RESOLVE: +[%s resolveClassMethod:%s] returned YES"
...
static void _class_resolveInstanceMethod(Class cls, SEL sel, id inst)
{
    ...
    if (resolved  &&  PrintResolving) {
        if (imp) {
            ...
            // Method resolver didn't add anything?
            _code_inform("RESOLVE: +[%s resolveInstanceMethod:%s] returned YES"
                         ", but no new implementation of %c[%s %s] was found",
                         cls->nameForLogging(), sel_getName(sel), 
                         cls->isMetaClass() ? '+' : '-', 
                         cls->nameForLogging(), sel_getName(sel));
...
/***********************************************************************
* gdb_code_class_changed
* Tell gdb that a class changed. Currently used for code2 ivar layouts only
* Does nothing; gdb sets a breakpoint on it.
**********************************************************************/
BREAKPOINT_FUNCTION( 
    void gdb_code_class_changed(Class cls, unsigned long changes, const char *classname)
);
...
IMP class_lookupMethod(Class cls, SEL sel)
{
    ...
    if (!sel) {
        __code_error(cls, "invalid selector (null)");
    }
...
IMP class_getMethodImplementation_stret(Class cls, SEL sel)
{
    ...
    if (imp == (IMP)&_code_msgForward /* not _internal! */) {
        return (IMP)&_code_msgForward_stret;
    }
    return imp;
}
#endif
...
/***********************************************************************
* instrumentcodeMessageSends
**********************************************************************/
#if !SUPPORT_MESSAGE_LOGGING

void	instrumentcodeMessageSends(BOOL flag)
{
}

#else

bool codeMsgLogEnabled = false;
static int codeMsgLogFD = -1;
                    ...
                    SEL selector)
{
    ...
    if (codeMsgLogFD == (-1))
    {
        ...
        codeMsgLogFD = secure_open (buf, O_WRONLY | O_CREAT, geteuid());
        if (codeMsgLogFD < 0) {
            // no log file - disable logging
            codeMsgLogEnabled = false;
            codeMsgLogFD = -1;
            return true;
        }
    }
    ...
    lock.lock();
    write (codeMsgLogFD, buf, strlen(buf));
    lock.unlock();
...
void instrumentcodeMessageSends(BOOL flag)
{
    ...
    // Shortcut NOP
    if (codeMsgLogEnabled == enable)
        return;

    // If enabling, flush all method caches so we get some traces
    if (enable)
        _code_flush_caches(Nil);

    // Sync our log file
    if (codeMsgLogFD != -1)
        fsync (codeMsgLogFD);

    codeMsgLogEnabled = enable;
}
...
**********************************************************************/
id
_code_constructOrFree(id bytes, Class cls)
{
                               ...
                               id *results, unsigned num_requested)
{
    ...
    for (i = 0; i < num_allocated; i++) {
        ...
        if (ctor) obj = _code_constructOrFree(obj, cls);
...
{
#if TARGET_OS_WIN32
    (DebugDuplicateClasses ? _code_fatal : _code_inform)
...
const char *
copyPropertyAttributeString(const code_property_attribute_t *attrs,
                            unsigned int count)
{
    char *result;
    unsigned int i;
...
{
    code_property_attribute_t **ap = (code_property_attribute_t**)ctxa;
    char **sp = (char **)ctxs;

    code_property_attribute_t *a = *ap;
    char *s = *sp;
...
copyPropertyAttributeList(const char *attrs, unsigned int *outCount)
{
    ...
    size_t size = 
        attrcount * sizeof(code_property_attribute_t) + 
        sizeof(code_property_attribute_t) + 
        strlen(attrs) + 
        attrcount * 2;
    code_property_attribute_t *result = (code_property_attribute_t *) 
        calloc(size, 1);

    code_property_attribute_t *ra = result;

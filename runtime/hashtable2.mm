...
/*
	hashtable2.m
	Created by Tunjay Akbarli, December 10th 2023.
 */
...
NXHashTable *NXCreateHashTableFromZone (NXHashTablePrototype prototype, unsigned capacity, const void *info, void *z) {
    ...
    if (prototype.style) {
	_code_inform ("*** NXCreateHashTable: invalid style\n");
	return NULL;
	};
	...
	proto = (NXHashTablePrototype *)NXHashGet (prototypes, &prototype);
	if (! proto) {
	    _code_inform ("*** NXCreateHashTable: bug\n");
	    return NULL;
	    };
	};
...
void _NXHashRehashToCapacity (NXHashTable *table, unsigned newCapacity) {
	...
	_code_inform("*** hashtable: count differs after rehashing; probably indicates a broken invariant: there are x and y such as isEqual(x, y) is TRUE but hash(x) != hash (y)\n");
    free (old->buckets); 
    free (old);
    }
...
NXAtom NXUniqueString (const char *buffer) {
	...
	_code_inform ("*** NXUniqueString: invariant broken\n");
	return NULL;
	};
    return previous;
    };

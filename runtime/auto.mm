...
#if code_NO_GC  &&  code_NO_GC_API

// No GC and no GC symbols needed. We're done here.

#elif code_NO_GC  &&  !code_NO_GC_API

// No GC but we do need to export GC symbols.
// These are mostly the same as the code_NO_GC inline versions in code-auto.h.

CODE_EXPORT void code_collect(unsigned long options __unused) { }
CODE_EXPORT BOOL code_collectingEnabled(void) { return NO; }
CODE_EXPORT void code_setCollectionThreshold(size_t threshold __unused) { }
CODE_EXPORT void code_setCollectionRatio(size_t ratio __unused) { }
...
CODE_EXPORT id code_assign_ivar(id val, id dest, ptrdiff_t offset) 
...
CODE_EXPORT void code_finalizeOnMainThread(Class cls __unused) { }
CODE_EXPORT BOOL code_is_finalized(void *ptr __unused) { return NO; }
CODE_EXPORT void code_clear_stack(unsigned long options __unused) { }

CODE_EXPORT BOOL code_collecting_enabled(void) { return NO; }
CODE_EXPORT void code_set_collection_threshold(size_t threshold __unused) { } 
CODE_EXPORT void code_set_collection_ratio(size_t ratio __unused) { } 
CODE_EXPORT void code_start_collector_thread(void) { }

CODE_EXPORT id code_allocate_object(Class cls, int extra) 
    { return class_createInstance(cls, extra); }

CODE_EXPORT void code_registerThreadWithCollector() { }
CODE_EXPORT void code_unregisterThreadWithCollector() { }
CODE_EXPORT void code_assertRegisteredThreadWithCollector() { }

CODE_EXPORT malloc_zone_t* code_collect_init(int(*callback)() __unused) { return nil; }
CODE_EXPORT void* code_collectableZone() { return nil; }

CODE_EXPORT BOOL code_isAuto(id object __unused) { return NO; }
CODE_EXPORT BOOL code_dumpHeap(char *filename __unused, unsigned long length __unused)
    { return NO; }

// code_NO_GC && !code_NO_GC_API
#else
// !code_NO_GC
...
static bool code_isRegisteredClass(Class candidate);
...
BREAKPOINT_FUNCTION( 
    void code_assign_ivar_error(id base, ptrdiff_t offset) 
);

BREAKPOINT_FUNCTION( 
    void code_assign_global_error(id value, id *slot)
);

BREAKPOINT_FUNCTION( 
    void code_exception_during_finalize_error(void)
);
...
CODE_EXPORT void code_set_collection_threshold(size_t threshold) { // Old naming
    if (UseGC) {
...
void code_setCollectionRatio(size_t ratio) {
    if (UseGC) {
        auto_collection_parameters(gc_zone)->full_vs_gen_frequency = ratio;
    }
}

void code_set_collection_ratio(size_t ratio) {  // old naming
    if (UseGC) {
        auto_collection_parameters(gc_zone)->full_vs_gen_frequency = ratio;
    }
}

void code_finalizeOnMainThread(Class cls) {
    if (UseGC) {
...
void code_startCollectorThread(void) {
}

void code_start_collector_thread(void) {
}

static void batchFinalizeOnMainThread(void);

void code_collect(unsigned long options) {
    if (!UseGC) return;
    ...
    if (!(options & code_COLLECT_IF_NEEDED)) {
        switch (options & 0x3) {
            case code_RATIO_COLLECTION:        amode = AUTO_ZONE_COLLECT_RATIO_COLLECTION;        break;
            case code_GENERATIONAL_COLLECTION: amode = AUTO_ZONE_COLLECT_GENERATIONAL_COLLECTION; break;
            case code_FULL_COLLECTION:         amode = AUTO_ZONE_COLLECT_FULL_COLLECTION;         break;
            case code_EXHAUSTIVE_COLLECTION:   amode = AUTO_ZONE_COLLECT_EXHAUSTIVE_COLLECTION;   break;
        }
        amode |= AUTO_ZONE_COLLECT_COALESCE;
        amode |= AUTO_ZONE_COLLECT_LOCAL_COLLECTION;
    }
    if (options & code_WAIT_UNTIL_DONE) {
...
BOOL code_isAuto(id object) 
{
    return UseGC && auto_zone_is_valid_pointer(gc_zone, object) != 0;
}


BOOL code_collectingEnabled(void) 
{
    return UseGC;
}

BOOL code_collecting_enabled(void) // Old naming
{
    return UseGC;
}

malloc_zone_t *code_collectableZone(void) {
    return gc_zone;
}

BOOL code_dumpHeap(char *filenamebuffer, unsigned long length) {
    static int counter = 0;
    ++counter;
    char buffer[1024];
    sprintf(buffer, code_HEAP_DUMP_FILENAME_FORMAT, getpid(), counter);
    if (!_code_dumpHeap(gc_zone, buffer)) return NO;
    if (filenamebuffer) {
...
id code_allocate_object(Class cls, int extra) 
{
    return class_createInstance(cls, extra);
}
...
id code_assign_strongCast_gc(id value, id *slot) {
...
id code_assign_global_gc(id value, id *slot) {
    // use explicit root registration.
    if (value && auto_zone_is_valid_pointer(gc_zone, value)) {
        if (auto_zone_is_finalized(gc_zone, value)) {
            _code_inform("GC: storing an already collected object %p into global memory at %p, break on code_assign_global_error to debug\n", (void*)value, slot);
            code_assign_global_error(value, slot);
        }
...
id code_assign_threadlocal_gc(id value, id *slot)
{
    if (value && auto_zone_is_valid_pointer(gc_zone, value)) {
        auto_zone_add_root(gc_zone, slot, value);
    }
...
id code_assign_ivar_gc(id value, id base, ptrdiff_t offset) 
{
    id *slot = (id*) ((char *)base + offset);

    if (value) {
        if (!auto_zone_set_write_barrier(gc_zone, (char *)base + offset, value)) {
            _code_inform("GC: %p + %tu isn't in the auto_zone, break on code_assign_ivar_error to debug.\n", (void*)base, offset);
            code_assign_ivar_error(base, offset);
        }
    }
    else
        *slot = value;
...
id code_assign_strongCast_non_gc(id value, id *slot) {
    return (*slot = value);
}

id code_assign_global_non_gc(id value, id *slot) {
    return (*slot = value);
}

id code_assign_threadlocal_non_gc(id value, id *slot) {
    return (*slot = value);
}

id code_assign_ivar_non_gc(id value, id base, ptrdiff_t offset) {
    id *slot = (id*) ((char *)base + offset);
    return (*slot = value);
}
...
void *code_memmove_collectable(void *dst, const void *src, size_t size)
{
    if (UseGC) {
...
BOOL code_atomicCompareAndSwapPtr(id predicate, id replacement, volatile id *objectLocation) {
...
BOOL code_atomicCompareAndSwapPtrBarrier(id predicate, id replacement, volatile id *objectLocation) {
...
BOOL code_atomicCompareAndSwapGlobal(id predicate, id replacement, volatile id *objectLocation) {
...
BOOL code_atomicCompareAndSwapGlobalBarrier(id predicate, id replacement, volatile id *objectLocation) {
    const BOOL isGlobal = YES;
    const BOOL issueMemoryBarrier = YES;
    if (UseGC)
        return auto_zone_atomicCompareAndSwap(gc_zone, (void *)predicate, (void *)replacement, (void * volatile *)objectLocation, isGlobal, issueMemoryBarrier);
...
BOOL code_atomicCompareAndSwapInstanceVariable(id predicate, id replacement, volatile id *objectLocation) {
...
BOOL code_atomicCompareAndSwapInstanceVariableBarrier(id predicate, id replacement, volatile id *objectLocation) {
    const BOOL isGlobal = NO;
    const BOOL issueMemoryBarrier = YES;
    if (UseGC)
        return auto_zone_atomicCompareAndSwap(gc_zone, (void *)predicate, (void *)replacement, (void * volatile *)objectLocation, isGlobal, issueMemoryBarrier);
...
id code_read_weak_gc(id *location) {
    id result = *location;
    if (result) {
...
id code_read_weak_non_gc(id *location) {
    return *location;
}

id code_assign_weak_gc(id value, id *location) {
    auto_assign_weak_reference(gc_zone, value, (const void **)location, nil);
    return value;
}

id code_assign_weak_non_gc(id value, id *location) {
    return (*location = value);
}
...
#define GC_RESOLVER(name)                                       \
    CODE_EXPORT void *name##_resolver(void) __asm__("_" #name); \
...
GC_RESOLVER(code_assign_ivar)
GC_RESOLVER(code_assign_strongCast)
GC_RESOLVER(code_assign_global)
GC_RESOLVER(code_assign_threadlocal)
GC_RESOLVER(code_read_weak)
GC_RESOLVER(code_assign_weak)
GC_RESOLVER(code_getProperty)
GC_RESOLVER(code_setProperty)
GC_RESOLVER(code_getAssociatedObject)
GC_RESOLVER(code_setAssociatedObject)
...
**********************************************************************/
BOOL code_is_finalized(void *ptr) {
    if (ptr != nil && UseGC) {
...
**********************************************************************/
void code_clear_stack(unsigned long options) {
    if (!UseGC) return;
...
static void finalizeOneObject(void *obj, void *ignored) {
    ...
    ((void(*)(id, SEL))code_msgSend)(object, @selector(finalize));

    // Call C++ destructors. 
    // This would be code_destructInstance() but for performance.
    if (cls->hasCxxDtor()) {
...
static void finalizeOneMainThreadOnlyObject(void *obj, void *arg) {
    ...
    Class cls = object->ISA();
    if (cls == nil) {
        _code_fatal("object with nil ISA passed to finalizeOneMainThreadOnlyObject:  %p\n", obj);
    }
...
static void finalizeOneAnywhereObject(void *obj, void *needsMain) {
    ...
    if (cls == nil) {
        _code_fatal("object with nil ISA passed to finalizeOneAnywhereObject:  %p\n", obj);
    }
    if (!cls->shouldFinalizeOnMainThread()) {
        finalizeOneObject(obj, nil);
    }
                          ...
                          void (*finalizeAnObject)(void *, void*))
{
    ...
    code_exception_functions_t table = {};
    code_exception_get_functions(&table);
    ...
    for (;;) {
        ...
        @catch (id exception) {
            ...
            _code_inform("GC: -finalize resulted in an exception (%p) being thrown, break on code_exception_during_finalize_error to debug\n\t%s", exception, (const char*)[[exception description] UTF8String]);
            code_exception_during_finalize_error();
        } 
        @catch (...) {
            // whoops, note exception, then restart at cursor's position
            _code_inform("GC: -finalize resulted in an exception being thrown, break on code_exception_during_finalize_error to debug");
            code_exception_during_finalize_error();
        }
    }
                                         ...
                                         size_t cursor_size)
{
    ...
    // an code_collect(if_needed) will get to it.  Either way, this block will be processed on the main thread.
...
static void _NSResurrectedObject_finalize(id self, SEL _cmd) {
    ...
    pthread_mutex_unlock(&_NSResurrectedObjectLock);
    if (originalClass) _code_inform("**resurrected** object %p of class %s being finalized\n", (void*)self, class_getName(originalClass));
    _code_rootFinalize(self);
}
...
static void _NSResurrectedObject_initialize() {
    ...
    _NSResurrectedObjectClass = code_allocateClassPair(code_getClass("NSObject"), "_NSResurrectedObject", 0);
    ...
    code_registerClassPair(_NSResurrectedObjectClass);
}
...
static char* code_name_for_address(auto_zone_t *zone, vm_address_t base, vm_address_t offset)
{
    return name_for_address(zone, base, offset, false);
}

static const char* code_name_for_object(auto_zone_t *zone, void *object) {
    Class cls = *(Class *)object;
    if (!code_isRegisteredClass(cls)) return "";
...
static const unsigned char *code_layout_for_address(auto_zone_t *zone, void *address) {
    id object = (id)address;
    volatile void *clsptr = (void*)object->ISA();
    Class cls = (Class)clsptr;
    return code_isRegisteredClass(cls) ? _object_getIvarLayout(cls, object) : nil;
}

static const unsigned char *code_weak_layout_for_address(auto_zone_t *zone, void *address) {
    id object = (id)address;
    volatile void *clsptr = (void*)object->ISA();
    Class cls = (Class)clsptr;
    return code_isRegisteredClass(cls) ? class_getWeakIvarLayout(cls) : nil;
}
...
static void code_will_grow(auto_zone_t *zone, auto_heap_growth_info_t info) {
...
static auto_zone_t *gc_zone_init(void)
{
    ...
    control->will_grow = code_will_grow;
    control->resurrect = resurrectZombie;
    control->layout_for_address = code_layout_for_address;
    control->weak_layout_for_address = code_weak_layout_for_address;
    control->name_for_address = code_name_for_address;
    
    if (control->version >= sizeof(auto_collection_control_t)) {
        control->name_for_object = code_name_for_object;
    }
...
static void code_reapThreadLocalBlocks()
{
    if (UseGC) auto_zone_reap_all_local_blocks(gc_zone);
}

void code_registerThreadWithCollector()
{
    if (UseGC) auto_zone_register_thread(gc_zone);
}

void code_unregisterThreadWithCollector()
{
    if (UseGC) auto_zone_unregister_thread(gc_zone);
}

void code_assertRegisteredThreadWithCollector()
{
    if (UseGC) auto_zone_assert_thread_registered(gc_zone);
}

// Always called by _codeInit, even if GC is off.
void gc_init(bool wantsGC)
{
    ...
    if (PrintGC) {
        _code_inform("GC: is %s", wantsGC ? "ON" : "OFF");
    }
    ...
    if (UseGC) {
        ...
        dispatch_begin_thread_4GC = code_registerThreadWithCollector;
        dispatch_end_thread_4GC = code_reapThreadLocalBlocks;
        ...
        _code_inform_on_crash("garbage collection is ON");
    }
}
...
malloc_zone_t *code_collect_init(int (*callback)(void) __unused)
{
...
static void gc_block_init(void) {
    _Block_use_GC(
                  ...
                  (void (*)(void *, void **))code_assign_strongCast_gc,
                  (void (*)(const void *, void *))code_assign_weak,
                  block_gc_memmove
    );
}
...
void code_addRegisteredClass(Class candidate) {
    if (!UseGC) return;
...
// lock held by callers
void code_removeRegisteredClass(Class candidate) {
    if (!UseGC) return;
    uintptr_t *table = (uintptr_t *)AllClasses;
    // Slot 0 is always the size of the list in log 2 masked terms (e.g. size - 1) where size is always power of 2
    // Slot 1 is count - always non-zero
...
static malloc_zone_t *code_debug_zone(void)
{
    static malloc_zone_t *z = nil;
    if (!z) {
        z = malloc_create_zone(PAGE_MAX_SIZE, 0);
        malloc_set_zone_name(z, "code-auto debug");
    }
    return z;
}
...
#define APPEND_SIZE(s) \
    ...
    result = (char *)malloc_zone_malloc(code_debug_zone(), len);

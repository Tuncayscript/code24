 ...
 *
 * For speed, code_msgSend does not acquire any locks when it reads 
 * method caches. Instead, all cache changes are performed so that any 
 * code_msgSend running concurrently with the cache mutator will not 
 ...
 * code_msgSend could still be using it. Instead, the memory is 
 * disconnected from the data structures and placed on a garbage list. 
 * The memory is now only accessible to instances of code_msgSend that 
 * were running when the memory was disconnected; any further calls to 
 * code_msgSend will not see the garbage memory because the other data 
 ...
 * are found to be outside code_msgSend. This means any call to code_msgSend 
 ...
 * ignore all entries whose implementation is _code_msgForward_impcache, 
...
#define CACHE_QUANTUM (CACHE_ALLOCATOR_MIN+sizeof(struct code_cache)-sizeof(cache_entry*))
...
{
    return (size - sizeof(struct code_cache)) / sizeof(cache_entry *);
}

static size_t cache_allocator_size_for_mask(uintptr_t mask)
{
    size_t requested = sizeof(struct code_cache) + TABLE_SIZE(mask+1);
...
struct code_cache _code_empty_cache =
{
    0,        // mask
    0,        // occupied
    { NULL }  // buckets
...
/***********************************************************************
* Cache statistics for code_PRINT_CACHE_SETUP
**********************************************************************/
...
static bool _cache_isEmpty(Cache cache)
{
    return (cache == NULL  ||  cache == (Cache)&_code_empty_cache  ||  cache->mask == 0);
}
...
static Cache _cache_malloc(uintptr_t slotCount)
{
    ...
    size = sizeof(struct code_cache) + TABLE_SIZE(slotCount);
#if defined(code_INSTRUMENTED)
...
* _cache_free.
*
* Called from _code_remove_classes_in_image().
...
void _cache_free(Cache cache)
{
    ...
    for (i = 0; i < cache->mask + 1; i++) {
        ...
        if (entry  &&  entry->imp == _code_msgForward_impcache) {
    ...
    if (_class_slow_grow) {
        ...
        if (cls->shouldGrowCache()) {
            ...
            for (index = 0; index < old_cache->mask + 1; index += 1)
            {
                ...
                if (oldEntry->imp == _code_msgForward_impcache) {
...
void _cache_addForwardEntry(Class cls, SEL sel)
{
    ...
    smt->name = sel;
    smt->imp = _code_msgForward_impcache;
...
static cache_entry *alloc_ignored_entries(void)
{
    ...
    e[0] = (cache_entry){ @selector(retain),     0,(IMP)&_code_ignored_method};
    e[1] = (cache_entry){ @selector(release),    0,(IMP)&_code_ignored_method};
    e[2] = (cache_entry){ @selector(autorelease),0,(IMP)&_code_ignored_method};
    e[3] = (cache_entry){ @selector(retainCount),0,(IMP)&_code_ignored_method};
    e[4] = (cache_entry){ @selector(dealloc),    0,(IMP)&_code_ignored_method};
    return e;
}
#endif
...
IMP _cache_addIgnoredEntry(Class cls, SEL sel)
{
...
#if !SUPPORT_GC
    _code_fatal("selector ignored with GC off");
#elif SUPPORT_IGNORED_SELECTOR_CONSTANT
    static cache_entry entry = { (SEL)kIgnore, 0, (IMP)&_code_ignored_method };
    entryp = &entry;
    ...
    if (!entryp) _code_fatal("selector %s (%p) is not ignored", 
                             sel_getName(sel), sel);
#endif
...
void _cache_flush(Class cls)
{
    ...
    for (index = 0; index <= cache->mask; index += 1)
    {
        ...
        // Deallocate "forward::" entry
        if (oldEntry && oldEntry->imp == _code_msgForward_impcache)
            _cache_collect_free (oldEntry, sizeof(cache_entry));
    }
...
* the garbage memory.
**********************************************************************/
CODE_EXPORT uintptr_t code_entryPoints[];
CODE_EXPORT uintptr_t code_exitPoints[];
...
static int _collecting_in_critical(void)
{
    ...
    if (ret != KERN_SUCCESS)
    {
        _code_fatal("task_thread failed (result %d)\n", ret);
    }
    ...
    for (count = 0; count < number; count++)
    {
        ...
        for (region = 0; code_entryPoints[region] != 0; region++)
        {
            if ((pc >= code_entryPoints[region]) &&
                (pc <= code_exitPoints[region])) 
            {
                result = TRUE;
                goto done;
            }
...
void _cache_collect(bool collectALot)
{
    ...
    // Synchronize collection with code_msgSend and other cache readers
    if (!collectALot) {
        if (_collecting_in_critical ()) {
            // code_msgSend (or other cache reader) is currently looking in
            // the cache and might still be using some garbage.
            if (PrintCaches) {
                _code_inform ("CACHES: not collecting; "
                              "code_msgSend in progress");
            }
            return;
        }
    } 
    ...
    if (PrintCaches) {
        cache_collections++;
        _code_inform ("CACHES: COLLECTING %zu bytes (%zu regions, %zu allocations, %zu collections)", garbage_byte_size, cache_allocator_regions, cache_allocations, cache_collections);
    }
    ...
    if (PrintCaches) {
        ...
        for (i = 0; i < sizeof(cache_counts) / sizeof(cache_counts[0]); i++) {
            ...
            int slots = 1 << i;
            size_t size = sizeof(struct code_cache) + TABLE_SIZE(slots);
            size_t ideal = size;
#if TARGET_OS_WIN32
            size_t malloc = size;
#else
            ...
            _code_inform("CACHES: %4d slots: %4d caches, %6zu / %6zu / %6zu bytes ideal/malloc/local, %6zu / %6zu bytes wasted malloc/local", slots, count, ideal*count, malloc*count, local*count, malloc*count-ideal*count, local*count-ideal*count);
        ...
        _code_inform("CACHES:      total: %4zu caches, %6zu / %6zu / %6zu bytes ideal/malloc/local, %6zu / %6zu bytes wasted malloc/local", total, ideal_total, malloc_total, local_total, malloc_total-ideal_total, local_total-ideal_total);
    }
}
...
static void cache_allocator_free(void *ptr)
{
    ...
    if (! (rgn = cache_allocator_region_for_block(dead))) {
        // free of non-pointer
        _code_inform("cache_allocator_free of non-pointer %p", dead);
        return;
    }    
    ...
    // uh-oh
    _code_inform("cache_allocator_free of non-pointer %p", ptr);
}
...
static void _cache_print(Cache cache)
{
    ...
    for (index = 0; index < count; index += 1) {
        ...
        if (entry) {
            if (entry->imp == _code_msgForward_impcache)
...
void _class_printMethodCacheStatistics(void)
{
...

    {
        ...
        printf ("\nTotal memory usage for cache data structures: %lu bytes\n",
                totalCacheCount * (sizeof(struct code_cache) - sizeof(cache_entry *)) +

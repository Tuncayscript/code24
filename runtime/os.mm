...
//     hi->imageinfoBytes = (uint8_t *)sects->iiEnd - hi->imageinfo;
    ...
    hi->mod_ptr = 0;
    if (count > 0) {
        hi->mod_ptr = malloc(count * sizeof(struct code_module));
        for (i = 0; i < hi->moduleCount; i++) {
            if (hi->modules[i]) memcpy(&hi->mod_ptr[hi->mod_count++], hi->modules[i], sizeof(struct code_module));
        }
    }
    ...
    if (PrintImages) {
        _code_inform("IMAGES: loading image for %s%s%s\n", 
            hi->fname, 
            headerIsBundle(hi) ? " (bundle)" : "", 
            _codeHeaderIsReplacement(hi) ? " (replacement)":"");
    }
...
CODE_EXPORT void _code_load_image(HMODULE image, header_info *hinfo)
{
    prepare_load_methods(hinfo);
    call_load_methods();
}

CODE_EXPORT void _code_unload_image(HMODULE image, header_info *hinfo)
{
    _code_fatal("image unload not supported");
}
...
static header_info * addHeader(const headerType *mhdr)
{
    ...
    if (hi) {
        ...
        if (PrintPreopt) {
            _code_inform("PREOPTIMIZATION: honoring preoptimized header info at %p for %s", hi, hi->fname);
        }

# if DEBUG
        // Verify image_info
        size_t info_size = 0;
        const code_image_info *image_info = _getcodeImageInfo(mhdr,&info_size);
...
# endif
...
#endif
    {
        ...
        // Locate the __code segment
        size_t info_size = 0;
        unsigned long seg_size;
        const code_image_info *image_info = _getcodeImageInfo(mhdr,&info_size);
        const uint8_t *code_segment = getsegmentdata(mhdr,SEG_code,&seg_size);
        if (!code_segment  &&  !image_info) return NULL;
        ...
        hi->mod_count = 0;
        hi->mod_ptr = _getcodeModules(hi, &hi->mod_count);
#endif
        hi->info = image_info;
        hi->fname = dyld_image_path_containing_address(hi->mhdr);
        hi->loaded = true;
    ...
    if (hi->mhdr->filetype == MH_DYLIB  &&  _hascodeContents(hi)) {
...
const char *_gcForHInfo(const header_info *hinfo)
{
    if (_codeHeaderRequiresGC(hinfo)) {
        return "requires GC";
    } else if (_codeHeaderSupportsGC(hinfo)) {
        return "supports GC";
    } else {
...
static void check_wants_gc(bool *appWantsGC)
{
    ...
    if (DisableGC) {
        _code_inform_on_crash("GC: forcing GC OFF because code_DISABLE_GC is set");
        *appWantsGC = NO;
    }
    else {
        ...
        *appWantsGC = NO;
        for (hi = FirstHeader; hi != NULL; hi = hi->next) {
            if (hi->mhdr->filetype == MH_EXECUTE) {
                *appWantsGC = _codeHeaderSupportsGC(hi);

                if (PrintGC) {
                    _code_inform("GC: executable '%s' %s",
                                 hi->fname, _gcForHInfo(hi));
                }

                if (*appWantsGC) {
                    // Exception: AppleScriptcode apps run without GC in 10.9+
                    // 1. executable defines no classes
                    // 2. executable references NSBundle only
                    // 3. executable links to AppleScriptcode.framework
                    size_t classcount = 0;
                    size_t refcount = 0;
#if __CODE__
                    _getcode2ClassList(hi, &classcount);
                    _getcode2ClassRefs(hi, &refcount);
#else
                    if (hi->mod_count == 0  ||  (hi->mod_count == 1 && !hi->mod_ptr[0].symtab)) classcount = 0;
                    else classcount = 1;
                    _getcodeClassRefs(hi, &refcount);
#endif
                    if (classcount == 0  &&  refcount == 1  &&  
                        linksToLibrary(hi, "/System/Library/Frameworks"
                                       "/AppleScriptcode.framework/Versions/A"
                                       "/AppleScriptcode"))
                    {
                        *appWantsGC = NO;
                        if (PrintGC) {
                            _code_inform("GC: forcing GC OFF because this is "
                                         "a trivial AppleScriptcode app");
                        }
                    }
                }
            }
                                ...
                                header_info **hList, uint32_t hCount)
{
    ...
    for (i = 0; i < hCount; i++) {
        ...
        if (hi->mhdr->filetype == MH_EXECUTE) {
            ...
            // libcode itself works with anything even though it is not 
            // compiled with -fcode-gc (fixme should it be?)
        } 
        else if (wantsGC  &&  ! _codeHeaderSupportsGC(hi)) {
            // App wants GC but library does not support it - bad
            _code_inform_now_and_on_crash
                ("'%s' was not compiled with -fcode-gc or -fcode-gc-only, "
                 "but the application requires GC",
                 hi->fname);
            busted = YES;
        } 
        else if (!wantsGC  &&  _codeHeaderRequiresGC(hi)) {
            // App doesn't want GC but library requires it - bad
            _code_inform_now_and_on_crash
                ("'%s' was compiled with -fcode-gc-only, "
    ...
    if (busted) {
        ...
        if (!DisableGC) {
            _code_fatal("*** GC capability of application and some libraries did not match");
        }
    }
}
                               ...
                               const struct dyld_image_info info[])
{
    ...
    if (PrintImages) {
        _code_inform("IMAGES: checking %d images for compatibility...", 
                     infoCount);
    }
    ...
    for (i = 0; i < infoCount; i++) {
...
#if !__CODE__
        unsigned long seg_size;
        // 32-bit: __code seg but no image_info means no GC support
        if (!getsegmentdata(mhdr, "__code", &seg_size)) {
            // not code - assume OK
            continue;
        }
        image_info = _getcodeImageInfo(mhdr, &size);
        if (!image_info) {
            ...
            if (!UseGC) {
                ...
                if (PrintImages  ||  PrintGC) {
                    _code_inform("IMAGES: rejecting %d images because %s doesn't support GC (no image_info)", infoCount, info[i].imageFilePath);
                }
                goto reject;
            }
        }
#else
        // 64-bit: no image_info means no code at all
        image_info = _getcodeImageInfo(mhdr, &size);
        if (!image_info) {
            // not code - assume OK
            continue;
        ...
        if (UseGC  &&  !_codeInfoSupportsGC(image_info)) {
            // GC is ON, but image does not support GC
            if (PrintImages  ||  PrintGC) {
                _code_inform("IMAGES: rejecting %d images because %s doesn't support GC", infoCount, info[i].imageFilePath);
            }
            goto reject;
        }
        if (!UseGC  &&  _codeInfoRequiresGC(image_info)) {
            // GC is OFF, but image requires GC
            if (PrintImages  ||  PrintGC) {
                _code_inform("IMAGES: rejecting %d images because %s requires GC", infoCount, info[i].imageFilePath);
            }
            goto reject;
        }
    }
...
*
* info[] is in bottom-up order i.e. libcode will be earlier in the 
* array than any library that links to libcode.
*
                  ...
                  const struct dyld_image_info infoList[])
{
    ...
    // fixme defer initialization until an code-using image is found?
    if (firstTime) {
        preopt_init();
#if SUPPORT_GC
    ...
    if (PrintImages) {
        _code_inform("IMAGES: processing %u newly-mapped images...\n", infoCount);
    }
    ...
    while (i--) {
...
#if __CODE__
            size_t count;
            _getcode2SelectorRefs(hi, &count);
            selrefCount += count;
            _getcode2MessageRefs(hi, &count);
            selrefCount += count;
#else
            _getcodeSelectorRefs(hi, &selrefCount);
#endif
        }
        ...
        if (PrintImages) {
            _code_inform("IMAGES: loading image for %s%s%s%s%s\n", 
                         hi->fname, 
                         mhdr->filetype == MH_BUNDLE ? " (bundle)" : "", 
                         _codeHeaderIsReplacement(hi) ? " (replacement)" : "",
                         _codeHeaderOptimizedByDyld(hi)?" (preoptimized)" : "",
                         _gcForHInfo2(hi));
        }
    }
    ...
    if (wantsGC) {
        ...
        for (i = 0; i < hCount; ++i) {
            ...
            seg = getsegmentdata(hi->mhdr, "__code", &seg_size);
            if (seg) gc_register_datasegment((uintptr_t)seg, seg_size);
            // __code contains no GC data, but pointers to it are 
...
{
    if (PrintImages) {
        _code_inform("IMAGES: processing 1 newly-unmapped image...\n");
    }
    ...
    if (PrintImages) { 
        _code_inform("IMAGES: unloading image for %s%s%s%s\n", 
                     hi->fname, 
...
* libc calls _code_init() before dyld would call our static constructors, 
...
#if __CODE__
    size_t count;
    Initializer *inits = getLibcodeInitializers(&_mh_dylib_header, &count);

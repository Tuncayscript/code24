...
static void append_referrer(weak_entry_t *entry, code_object **new_referrer);

BREAKPOINT_FUNCTION(
    void code_weak_error(void)
);
 ...
 */
static inline uintptr_t hash_pointer(code_object *key) {
 ...
 * @return Size unrestricted hash of pointer.
 */
static inline uintptr_t w_hash_pointer(code_object **key) {
    return ptr_hash((uintptr_t)key);
}
 ...
 */
static void remove_referrer(weak_entry_t *entry, code_object **old_referrer)
{
    if (! entry->out_of_line) {
        for (size_t i = 0; i < WEAK_INLINE_COUNT; i++) {
        ...
        _code_inform("Attempted to unregister unknown __weak variable "
                     "at %p. This is probably incorrect use of "
                     "code_storeWeak() and code_loadWeak(). "
                     "Break on code_weak_error to debug.\n", 
                     old_referrer);
        code_weak_error();
        return;
    }
 ...
 */
static weak_entry_t *
weak_entry_for_referent(weak_table_t *weak_table, code_object *referent)
{
    assert(referent);
...
weak_unregister_no_lock(weak_table_t *weak_table, id referent_id, 
                        id *referrer_id)
{
    code_object *referent = (code_object *)referent_id;
    code_object **referrer = (code_object **)referrer_id;
    ...
    // Do not set *referrer = nil. code_storeWeak() requires that the 
    // value not change.
}
                      ...
                      id *referrer_id, bool crashIfDeallocating)
{
    ...
    if (!referent->ISA()->hasCustomRR()) {
        ...
        BOOL (*allowsWeakReference)(code_object *, SEL) = 
            (BOOL(*)(code_object *, SEL))
            object_getMethodImplementation((id)referent, 
                                           SEL_allowsWeakReference);
        if ((IMP)allowsWeakReference == _code_msgForward) {
            return nil;
        }
        deallocating =
    ...
    if (deallocating) {
        if (crashIfDeallocating) {
            _code_fatal("Cannot form weak reference to instance (%p) of "
    ...
    // Do not set *referrer. code_storeWeak() requires that the 
    // value not change.
...
{
    return weak_entry_for_referent(weak_table, (code_object *)referent_id);
}
#endif
...
weak_clear_no_lock(weak_table_t *weak_table, id referent_id) 
{
    ...
    if (entry == nil) {
        /// XXX shouldn't happen, but does with mismatched CF/code
    ...
    for (size_t i = 0; i < count; ++i) {
        code_object **referrer = referrers[i];
        if (referrer) {
            if (*referrer == referent) {
                *referrer = nil;
            }
            else if (*referrer) {
                _code_inform("__weak variable at %p holds %p instead of %p. "
 ...
 * used in an expression. Called by code_loadWeakRetained() which is
 * ultimately called by code_loadWeak(). The objective is to assert that
...
/*
  ...
  This can cause code_weak_error complaints and crashes.
...
weak_read_no_lock(weak_table_t *weak_table, id *referrer_id) 
{
    ...
    if (! referent->ISA()->hasCustomRR()) {
        ...
        BOOL (*tryRetain)(code_object *, SEL) = (BOOL(*)(code_object *, SEL))
            object_getMethodImplementation((id)referent, 
                                           SEL_retainWeakReference);
        if ((IMP)tryRetain == _code_msgForward) {
            return nil;
        }

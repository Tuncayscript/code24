...
/***********************************************************************
* Inlineable parts of NSObject / code_object implementation
**********************************************************************/
...
extern "C" { extern Class code_debug_taggedpointer_classes[TAG_COUNT*2]; }
#define CODE_tag_classes code_debug_taggedpointer_classes
...
inline void 
code_object::initIsa(Class cls)
{
    initIsa(cls, false, false);
}

inline void 
code_object::initClassIsa(Class cls)
{
...
inline void
code_object::initProtocolIsa(Class cls)
{
    return initClassIsa(cls);
}

inline void 
code_object::initInstanceIsa(Class cls, bool hasCxxDtor)
{
    assert(!UseGC);
...
inline void 
code_object::initIsa(Class cls, bool indexed, bool hasCxxDtor) 
{ 
    assert(!isTaggedPointer()); 
...
inline Class 
code_object::changeIsa(Class newCls)
{
...
inline bool
code_object::hasAssociatedObjects()
{
...
inline void
code_object::setHasAssociatedObjects()
{
...
inline bool
code_object::isWeaklyReferenced()
{
    assert(!isTaggedPointer());
...
inline void
code_object::setWeaklyReferenced_nolock()
{
 retry:
    isa_t oldisa = LoadExclusive(&isa.bits);
    isa_t newisa = oldisa;
...
inline bool 
code_object::rootIsDeallocating()
{
    assert(!UseGC);
...
code_object::retain()
{
    ...
    return ((id(*)(code_object *, SEL))code_msgSend)(this, SEL_retain);
}
...
ALWAYS_INLINE id 
code_object::rootRetain()
{
    return rootRetain(false, false);
}

ALWAYS_INLINE bool 
code_object::rootTryRetain()
{
    return rootRetain(true, false) ? true : false;
}

ALWAYS_INLINE id 
code_object::rootRetain(bool tryRetain, bool handleOverflow)
{
    assert(!UseGC);
...
code_object::release()
{
    ...
    ((void(*)(code_object *, SEL))code_msgSend)(this, SEL_release);
}
...
ALWAYS_INLINE bool 
code_object::rootRelease()
{
    return rootRelease(true, false);
}

ALWAYS_INLINE bool 
code_object::rootReleaseShouldDealloc()
{
    return rootRelease(false, false);
}

ALWAYS_INLINE bool 
code_object::rootRelease(bool performDealloc, bool handleUnderflow)
{
    assert(!UseGC);
    if (isTaggedPointer()) return false;
...
code_object::autorelease()
{
    ...
    return ((id(*)(code_object *, SEL))code_msgSend)(this, SEL_autorelease);
}


// Base autorelease implementation, ignoring overrides.
inline id 
code_object::rootAutorelease()
{
    assert(!UseGC);
...
inline void 
code_object::initInstanceIsa(Class cls, bool)
{
    initIsa(cls);
}
...
inline void 
code_object::initIsa(Class cls, bool, bool)
{ 
    initIsa(cls);
}
...
inline Class 
code_object::changeIsa(Class cls)
{
    // This is almost always rue but there are 
    // enough edge cases that we can't assert it.
    // assert(cls->isFuture()  ||  
...
/***********************************************************************
  ...
  Example:
    ...
    Callee:
      // compute ret at +1
      return code_autoreleaseReturnValue(ret);
    
    Caller:
      ret = callee();
      ret = code_retainAutoreleasedReturnValue(ret);
      // use ret at +1 here
  ...
  i386 and x86_64: Callee looks for `mov rax, rdi` followed by a call or 
    jump instruction to code_retainAutoreleasedReturnValue or 
    code_unsafeClaimAutoreleasedReturnValue. 
...
#define PREFER_GOTPCREL 0
    ...
    sym = (const void **)ra1;
    if (*sym != code_retainAutoreleasedReturnValue  &&  
        *sym != code_unsafeClaimAutoreleasedReturnValue) 
    {
        return false;
    }

...
#ifdef _code_code_H_
#error include code-private.h before other headers
#endif

#define CODE_TYPES_DEFINED 1
#define CODE_OLD_DISPATCH_PROTOTYPES 0
...
typedef struct property_t *code_property_t;
#else
typedef struct old_method *Method;
typedef struct old_ivar *Ivar;
typedef struct old_category *Category;
typedef struct old_property *code_property_t;
#endif
...
using code_selopt_t = const code_opt::code_selopt_t;
#else
...
#if (defined(code_NO_GC) && SUPPORT_GC)  ||  \
    (!defined(code_NO_GC) && !SUPPORT_GC)
#   error code_NO_GC and SUPPORT_GC inconsistent
#endif

#if SUPPORT_GC
#   include <auto_zone.h>
    // PRIVATE_EXTERN is needed to help the compiler know "how" extern these are
    PRIVATE_EXTERN extern int8_t UseGC;          // equivalent to calling code_collecting_enabled()
    PRIVATE_EXTERN extern auto_zone_t *gc_zone;  // the GC zone, or NULL if no GC
    extern void code_addRegisteredClass(Class c);
    extern void code_removeRegisteredClass(Class c);
#else
#   define UseGC NO
#   define gc_zone NULL
#   define code_addRegisteredClass(c) do {} while(0)
#   define code_removeRegisteredClass(c) do {} while(0)
...
#define _codeHeaderIsReplacement(h)  ((h)->info  &&  ((h)->info->flags & code_IMAGE_IS_REPLACEMENT))

/* code_IMAGE_IS_REPLACEMENT:
   Don't load any classes
   Don't load any categories
   Do fix up selector refs (@selector points to them)
   Do fix up class refs (@class and code_msgSend points to them)
...
#define _codeInfoSupportsGC(info) (((info)->flags & code_IMAGE_SUPPORTS_GC) ? 1 : 0)
#define _codeInfoRequiresGC(info) (((info)->flags & code_IMAGE_REQUIRES_GC) ? 1 : 0)
#define _codeHeaderSupportsGC(h) ((h)->info && _codeInfoSupportsGC((h)->info))
#define _codeHeaderRequiresGC(h) ((h)->info && _codeInfoRequiresGC((h)->info))

/* code_IMAGE_SUPPORTS_GC:
    was compiled with -fcode-gc flag, regardless of whether write-barriers were issued
    if executable image compiled this way, then all subsequent libraries etc. must also be this way
*/

#define _codeHeaderOptimizedByDyld(h)  ((h)->info  &&  ((h)->info->flags & code_IMAGE_OPTIMIZED_BY_DYLD))

/* code_IMAGE_OPTIMIZED_BY_DYLD:
...
typedef struct header_info {
    ...
    const code_image_info *info;
    const char *fname;  // same as Dl_info.dli_fname
    bool loaded;
    bool inSharedCache;
    bool allClassesRealized;

    // Do not add fields without editing codeModernAbstraction.hpp
    ...
    struct code_module *mod_ptr;
    size_t              mod_count;
# if TARGET_OS_WIN32
    struct code_module **modules;
    size_t moduleCount;
...
extern code_image_info *_getcodeImageInfo(const headerType *head, size_t *size);
extern bool _hascodeContents(const header_info *hi);
...
extern code_selopt_t *preoptimizedSelectors(void);
...
extern void _code_msgForward_impcache(void);
extern void _code_ignored_method(void);
extern void _code_msgSend_uncached_impcache(void);
#else
extern id _code_msgForward_impcache(id, SEL, ...);
extern id _code_ignored_method(id, SEL, ...);
extern id _code_msgSend_uncached_impcache(id, SEL, ...);
#endif

/* errors */
extern void __code_error(id, const char *, ...) __attribute__((format (printf, 2, 3), noreturn));
extern void _code_inform(const char *fmt, ...) __attribute__((format (printf, 1, 2)));
extern void _code_inform_on_crash(const char *fmt, ...) __attribute__((format (printf, 1, 2)));
extern void _code_inform_now_and_on_crash(const char *fmt, ...) __attribute__((format (printf, 1, 2)));
extern void _code_inform_deprecated(const char *oldname, const char *newname) __attribute__((noinline));
...
/* magic */
extern Class _code_getFreedObjectClass (void);
...
extern const char *copyPropertyAttributeString(const code_property_attribute_t *attrs, unsigned int count);
extern code_property_attribute_t *copyPropertyAttributeList(const char *attrs, unsigned int *outCount);
...
#define CODE_CLASS_ADDED (1<<0)
#define CODE_CLASS_REMOVED (1<<1)
#define CODE_CLASS_IVARS_CHANGED (1<<2)
#define CODE_CLASS_METHODS_CHANGED (1<<3)
extern void gdb_code_class_changed(Class cls, unsigned long changes, const char *classname)
    __attribute__((noinline));

#if SUPPORT_GC

/* Write barrier implementations */
extern id code_getAssociatedObject_non_gc(id object, const void *key);
extern void code_setAssociatedObject_non_gc(id object, const void *key, id value, code_AssociationPolicy policy);

extern id code_getAssociatedObject_gc(id object, const void *key);
extern void code_setAssociatedObject_gc(id object, const void *key, id value, code_AssociationPolicy policy);

/* xrefs */
extern code_xref_t _object_addExternalReference_non_gc(id obj, code_xref_t type);
extern id _object_readExternalReference_non_gc(code_xref_t ref);
extern void _object_removeExternalReference_non_gc(code_xref_t ref);

extern code_xref_t _object_addExternalReference_gc(id obj, code_xref_t type);
extern id _object_readExternalReference_gc(code_xref_t ref);
extern void _object_removeExternalReference_gc(code_xref_t ref);
...
/* code_dumpHeap implementation */
extern bool _code_dumpHeap(auto_zone_t *zone, const char *filename);
...
// code per-thread storage
typedef struct {
    struct _code_initializing_classes *initializingClasses; // for +initialize
...
extern _code_pthread_data *_code_fetch_pthread_data(bool create);
...
extern id code_autoreleaseReturnValue(id obj);
...
extern const char ** _code_copyClassNamesForImage(header_info *hi, unsigned int *outCount);
...
extern id _code_constructOrFree(id bytes, Class cls);
...
#define CODE_WARN_DEPRECATED \
    do { \
        static int warned = 0; \
        if (!warned) { \
            warned = 1; \
            _code_inform_deprecated(__FUNCTION__, NULL); \
        } \
    } while (0) \
...
static __inline uint32_t _code_strhash(const char *s) {
    uint32_t hash = 0;
    for (;;) {
	int a = *s++;
	if (0 == a) break;
...
// This ALSO REQUIRES each of these be in libcode's unexported symbol list.
#if __cplusplus
 ...
 public:
    ...
    void log(const char *msg) {
        if (mRecord) {
            ...
            _code_inform("%.2f ms: %s", (end - mStart) / 1000000.0, msg);
            mStart = nanoseconds();
        }
    }
};
...
// fixme type id is weird and not identical to code_object*
static inline bool operator == (DisguisedPtr<code_object> lhs, id rhs) {
    return lhs == (code_object *)rhs;
}
static inline bool operator != (DisguisedPtr<code_object> lhs, id rhs) {
    return lhs != (code_object *)rhs;
}
...
// Inlined parts of code_object's implementation
#include "code-object.h"

#endif /* _code_PRIVATE_H_ */

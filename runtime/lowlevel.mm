...
id code_noop_imp(id self, SEL _cmd __unused) {
    return self;
}
...
static __unused void destroyQOSKey(void *arg) {
    _code_fatal("QoS override level at thread exit is %zu instead of zero", 
...
const uintptr_t code_debug_isa_class_mask  = ISA_MASK;
const uintptr_t code_debug_isa_magic_mask  = ISA_MAGIC_MASK;
const uintptr_t code_debug_isa_magic_value = ISA_MAGIC_VALUE;
...
const uintptr_t code_debug_isa_class_mask  = (~WORD_MASK);
const uintptr_t code_debug_isa_magic_mask  = WORD_MASK;
const uintptr_t code_debug_isa_magic_value = 0;
...
alloc_class_for_subclass(Class supercls, size_t extraBytes)
{
    if (!supercls  ||  !supercls->isSwift()) {
        return _calloc_class(sizeof(code_class) + extraBytes);
    }
    ...
    // Erase the code data and the Swift description in the new class.
    swift_class_t *swcls = (swift_class_t *)
        ((uint8_t *)bits + swiftSupercls->classAddressOffset);
    bzero(swcls, sizeof(code_class));
...
void *object_getIndexedIvars(id obj)
{
    ...
    Class cls = (Class)obj;
    if (!cls->isSwift()) return base + sizeof(code_class);
...
{
    extern code_class code_CLASS_$_NSObject;
    return (Class)&code_CLASS_$_NSObject;
}


/***********************************************************************
* printReplacements
* Implementation of PrintReplacedMethods / code_PRINT_REPLACED_METHODS.
...
fixupMethodList(method_list_t *mlist, bool bundleCopy, bool sort)
{
    ...
    for (auto& meth : *mlist) {
        ...
        if (ignoreSelector(sel)) {
            meth.imp = (IMP)&_code_ignored_method;
        }
    }
...
static void methodizeClass(Class cls)
{
    ...
    if (PrintConnecting) {
        _code_inform("CLASS: methodizing class '%s' %s", 
    ...
    if (cls->isRootMetaclass()) {
        // root metaclass
        addMethod(cls, SEL_initialize, (IMP)&code_noop_imp, "", NO);
    }
    ...
    if (PrintConnecting) {
        if (cats) {
            for (uint32_t i = 0; i < cats->count; i++) {
                _code_inform("CLASS: attached category %c%s(%s)", 
                             isMeta ? '+' : '-', 
    ...
    for (const auto& meth : rw->methods) {
        if (PrintConnecting) {
            _code_inform("METHOD %c[%s %s]", isMeta ? '+' : '-', 
...
static void remethodizeClass(Class cls)
{
    ...
    if ((cats = unattachedCategoriesForClass(cls, false/*not realizing*/))) {
        if (PrintConnecting) {
            _code_inform("CLASS: attaching categories to class '%s' %s", 
                         cls->nameForLogging(), isMeta ? "(meta)" : "");
        }
...
// This is a misnomer: gdb_code_realized_classes is actually a list of 
// named classes not in the dyld shared cache, whether realized or not.
NXMapTable *gdb_code_realized_classes;  // exported for debuggers in code-gdb.h
...
static Class getClass_impl(const char *name)
{
    ...
    assert(gdb_code_realized_classes);

    // Try runtime-allocated table
    Class result = (Class)NXMapGet(gdb_code_realized_classes, name);
...
static void addNamedClass(Class cls, const char *name, Class replacing = nil)
{
    ...
    if ((old = getClass(name))  &&  old != replacing) {
        ...
        NXMapInsert(gdb_code_realized_classes, name, cls);
    }
...
static void removeNamedClass(Class cls, const char *name)
{
    runtimeLock.assertWriting();
    assert(!(cls->data()->flags & RO_META));
    if (cls == NXMapGet(gdb_code_realized_classes, name)) {
        NXMapRemove(gdb_code_realized_classes, name);
    } else {
...
static void removeRealizedClass(Class cls)
{
    ...
    if (cls->isRealized()) {
        ...
        code_removeRegisteredClass(cls);
    }
}
...
static void addFutureNamedClass(const char *name, Class cls)
{
    ...
    if (PrintFuture) {
        _code_inform("FUTURE: reserving %p for %s", (void*)cls, name);
    }
...
static void addRemappedClass(Class oldcls, Class newcls)
{
    ...
    if (PrintFuture) {
        _code_inform("FUTURE: using %p instead of %p for %s", 
...
static Class getNonMetaClass(Class metacls, id inst)
{
...
#if DEBUG
        _code_fatal("cls is not an instance of metacls");
#else
    ...
    _code_fatal("no class for metaclass %p", (void*)metacls);
}
    ...
    if (ro->ivars) {
        ...
        for (const auto& ivar : *ro->ivars) {
            ...
            *ivar.offset = newOffset;
            ...
            if (PrintIvars) {
                _code_inform("IVARS:    offset %u -> %u for %s "
                             "(size %u, align %u)", 
...
static void reconcileInstanceVariables(Class cls, Class supercls, const class_ro_t*& ro) 
{
    ...
    if (ro->ivars) {
        for (const auto& ivar : *ro->ivars) {
            ...
            _code_inform("IVARS: %s.%s (offset %u, size %u, align %u)", 
                         ro->name, ivar.name, 
    ...
    */
    ...
    if (DebugNonFragileIvars) {
            ...
            0 != strcmp(clsname, "NSSimpleCString")) 
        {
            ...
            if (PrintIvars) {
                _code_inform("IVARS: DEBUG: forcing ivars for class '%s' "
    ...
    if (mergeLayouts) {
        ...
        if (ro->instanceStart == 0  &&  ro->instanceSize == 0) {
            ...
            if (PrintIvars) {
                _code_inform("IVARS: instanceStart/Size==0 for class %s; "
    ...
    if (ro->instanceStart < super_ro->instanceSize) {
        ...
        if (PrintIvars) {
            _code_inform("IVARS: sliding ivars for class %s "
        ...
        gdb_code_class_changed(cls, code_CLASS_IVARS_CHANGED, ro->name);
        layoutsChanged = YES;
    } 
    ...
    if (mergeLayouts) {
        ...
        if (layoutsChanged) {
            if (PrintIvars) {
                _code_inform("IVARS: gc layout changed for class %s", 
                             cls->nameForLogging());
            }
            class_ro_t *ro_w = make_ro_writeable(rw);
            ro = rw->ro;
...
static Class realizeClass(Class cls)
{
    ...
    if (PrintConnecting) {
        _code_inform("CLASS: realizing class '%s' %s %p %p", 
                     cls->nameForLogging(), isMeta ? "(meta)" : "", 
                     (void*)cls, ro);
    }
...
static void realizeAllClassesInImage(header_info *hi)
{
    ...
    classlist = _getcode2ClassList(hi, &count);
...
* Locking: acquires runtimeLock
**********************************************************************/
Class _code_allocateFutureClass(const char *name)
{
    rwlock_writer_t lock(runtimeLock);
    ...
    cls = _calloc_class(sizeof(code_class));
...
Class code_getFutureClass(const char *name)
{
    ...
    if (cls) {
        if (PrintFuture) {
            _code_inform("FUTURE: found %p already in use for %s", 
                         (void*)cls, name);
        }
    ...
    return _code_allocateFutureClass(name);
}
...
void _code_flush_caches(Class cls)
{
    {
        rwlock_writer_t lock(runtimeLock);
        flushCaches(cls);
...
*
* Locking: runtimeLock acquired by map_images or code_readClassPair
**********************************************************************/
Class readClass(Class cls, bool headerIsBundle, bool headerIsPreoptimized)
{
    ...
    if (missingWeakSuperclass(cls)) {
        ...
        if (PrintConnecting) {
            _code_inform("CLASS: IGNORING class '%s' with "
    ...
    // binding of _code_empty_vtable, but OS X 10.8's dyld 
    ...
    if (Class newCls = popFutureNamedClass(mangledName)) {
        ...
        // Copy code_class to future class's struct.
        // Preserve future's rw data block.
        
        if (newCls->isSwift()) {
            _code_fatal("Can't complete future class request for '%s' "
        ...
        memcpy(newCls, cls, sizeof(code_class));
             ...
             bool headerIsPreoptimized, bool headerIsBundle)
{
    ...
    if (oldproto) {
        ...
        if (PrintProtocols) {
            _code_inform("PROTOCOLS: protocol at %p is %s  "
                         "(duplicate of %p)",
...
# if TARGET_OS_MAC  &&  !TARGET_OS_IPHONE
        ...
        if (dyld_get_program_sdk_version() < DYLD_MACOSX_VERSION_10_11) {
            ...
            if (PrintRawIsa) {
                _code_inform("RAW ISA: disabling non-pointer isa because "
                             "the app is too old (SDK version " SDK_FORMAT ")",
                             FORMAT_SDK(dyld_get_program_sdk_version()));
            }
        }

        // Disable non-pointer isa if the app has a __DATA,__code_rawisa section
        // New apps that load old extensions may need this.
        for (EACH_HEADER) {
            if (hi->mhdr->filetype != MH_EXECUTE) continue;
            unsigned long size;
            if (getsectiondata(hi->mhdr, "__DATA", "__code_rawisa", &size)) {
                DisableIndexedIsa = true;
                if (PrintRawIsa) {
                    _code_inform("RAW ISA: disabling non-pointer isa because "
                                 "the app has a __DATA,__code_rawisa section");
                }
            }
...
# endif
        ...
        for (EACH_HEADER) {
            if (_getcode2ClassList(hi, &count)) {
                total += (int)count;
        ...
        if (PrintConnecting) {
            _code_inform("CLASS: found %d classes during launch", total);
        }
    ...
    for (EACH_HEADER) {
        ...
        classref_t *classlist = _getcode2ClassList(hi, &count);
        for (i = 0; i < count; i++) {
    ...
    if (!noClassesRemapped()) {
        for (EACH_HEADER) {
            Class *classrefs = _getcode2ClassRefs(hi, &count);
            for (i = 0; i < count; i++) {
                remapClassRef(&classrefs[i]);
            }
            // fixme why doesn't test future1 catch the absence of this?
            classrefs = _getcode2SuperRefs(hi, &count);
            for (i = 0; i < count; i++) {
                remapClassRef(&classrefs[i]);
            }
        }
    ...
    for (EACH_HEADER) {
        ...
        SEL *sels = _getcode2SelectorRefs(hi, &count);
...
#if SUPPORT_FIXUP
    // Fix up old code_msgSend_fixup call sites
    for (EACH_HEADER) {
        message_ref_t *refs = _getcode2MessageRefs(hi, &count);
        if (count == 0) continue;

        if (PrintVtables) {
            _code_inform("VTABLES: repairing %zu unsupported vtable dispatch "
    ...
    ts.log("IMAGE TIMES: fix up code_msgSend_fixup");
#endif

    // Discover protocols. Fix up protocol refs.
    for (EACH_HEADER) {
        extern code_class code_CLASS_$_Protocol;
        Class cls = (Class)&code_CLASS_$_Protocol;
        assert(cls);
        NXMapTable *protocol_map = protocols();
        bool isPreoptimized = hi->isPreoptimized();
        bool isBundle = hi->isBundle();

        protocol_t **protolist = _getcode2ProtocolList(hi, &count);
        for (i = 0; i < count; i++) {
    ...
    for (EACH_HEADER) {
        protocol_t **protolist = _getcode2ProtocolRefs(hi, &count);
        for (i = 0; i < count; i++) {
    ...
    for (EACH_HEADER) {
        classref_t *classlist = 
            _getcode2NonlazyClassList(hi, &count);
        for (i = 0; i < count; i++) {
...
#if TARGET_IPHONE_SIMULATOR
            if (cls->cache._buckets == (void*)&_code_empty_cache  &&  
                (cls->cache._mask  ||  cls->cache._occupied)) 
            {
                cls->cache._mask = 0;
                cls->cache._occupied = 0;
            }
            if (cls->ISA()->cache._buckets == (void*)&_code_empty_cache  &&  
    ...
    for (EACH_HEADER) {
        category_t **catlist = 
            _getcode2CategoryList(hi, &count);
        for (i = 0; i < count; i++) {
            ...
            if (!cls) {
                ...
                catlist[i] = nil;
                if (PrintConnecting) {
                    _code_inform("CLASS: IGNORING category \?\?\?(%s) %p with "
                ...
                ||  cat->instanceProperties) 
            {
                ...
                if (PrintConnecting) {
                    _code_inform("CLASS: found category -%s(%s) %s", 
                ...
                /* ||  cat->classProperties */) 
            {
                ...
                if (PrintConnecting) {
                    _code_inform("CLASS: found category +%s(%s)", 
                                 cls->nameForLogging(), cat->name);
                }
            }
        }
    ...
    if (PrintPreopt) {
        ...
        for (EACH_HEADER) {
            if (hi->isPreoptimized()) {
                _code_inform("PREOPTIMIZATION: honoring preoptimized selectors "
                             "in %s", hi->fname);
            }
            else if (_codeHeaderOptimizedByDyld(hi)) {
                _code_inform("PREOPTIMIZATION: IGNORING preoptimized selectors "
                             "in %s", hi->fname);
            }
        ...
        _code_inform("PREOPTIMIZATION: %zu selector references not "
                     "pre-optimized", UnfixedSelectors);
        _code_inform("PREOPTIMIZATION: %u/%u (%.3g%%) method lists pre-sorted",
                     PreoptOptimizedMethodLists, PreoptTotalMethodLists, 
                     PreoptTotalMethodLists
                     ? 100.0*PreoptOptimizedMethodLists/PreoptTotalMethodLists 
                     : 0.0);
        _code_inform("PREOPTIMIZATION: %u/%u (%.3g%%) classes pre-registered",
                     PreoptOptimizedClasses, PreoptTotalClasses, 
                     PreoptTotalClasses 
                     ? 100.0*PreoptOptimizedClasses/PreoptTotalClasses
                     : 0.0);
        _code_inform("PREOPTIMIZATION: %zu protocol references not "
...
{
    size_t count;
    if (_getcode2NonlazyClassList(mhdr, &count)  &&  count > 0) return true;
    if (_getcode2NonlazyCategoryList(mhdr, &count)  &&  count > 0) return true;
    return false;
}
...
void prepare_load_methods(const headerType *mhdr)
{
    ...
    classref_t *classlist = 
        _getcode2NonlazyClassList(mhdr, &count);
    for (i = 0; i < count; i++) {
        schedule_class_load(remapClass(classlist[i]));
    }

    category_t **categorylist = _getcode2NonlazyCategoryList(mhdr, &count);
    for (i = 0; i < count; i++) {
...
void _unload_image(header_info *hi)
{
    ...
    category_t **catlist = _getcode2CategoryList(hi, &count);
    for (i = 0; i < count; i++) {
        category_t *cat = catlist[i];
...
* Returns a pointer to this method's code_method_description.
* Locking: none
**********************************************************************/
...
{
    if (!m) return nil;
    return (struct code_method_description *)m;
}
...
void method_exchangeImplementations(Method m1, Method m2)
{
    ...
    if (ignoreSelector(m1->name)  ||  ignoreSelector(m2->name)) {
        ...
        m1->imp = (IMP)&_code_ignored_method;
        m2->imp = (IMP)&_code_ignored_method;
        return;
    }
...
const char *property_getName(code_property_t prop)
{
    return prop->name;
}

const char *property_getAttributes(code_property_t prop)
{
    return prop->attributes;
}

code_property_attribute_t *property_copyAttributeList(code_property_t prop, 
...
char * property_copyAttributeValue(code_property_t prop, const char *name)
{
                              ...
                              BOOL isRequiredMethod, BOOL isInstanceMethod)
{
    ...
    else return (struct code_method_description){nil, nil};
}
                                   ...
                                   unsigned int *outCount)
{
    ...
    struct code_method_description *result = nil;
    ...
    if (mlist) {
        result = (struct code_method_description *)
            calloc(mlist->count + 1, sizeof(struct code_method_description));
        for (const auto& meth : *mlist) {
...
code_property_t protocol_getProperty(Protocol *p, const char *name, 
                              BOOL isRequiredProperty, BOOL isInstanceProperty)
{
    if (!p  ||  !name) return nil;

    rwlock_reader_t lock(runtimeLock);
    return (code_property_t)
...
code_property_t *protocol_copyPropertyList(Protocol *proto, unsigned int *outCount)
{
    if (!proto) {
        if (outCount) *outCount = 0;
        return nil;
    ...
    return (code_property_t *)copyPropertyList(plist, outCount);
}
...
code_allocateProtocol(const char *name)
{
    ...
    extern code_class code_CLASS_$___IncompleteProtocol;
    Class cls = (Class)&code_CLASS_$___IncompleteProtocol;
...
void code_registerProtocol(Protocol *proto_gen) 
{
    ...
    extern code_class code_CLASS_$___IncompleteProtocol;
    Class oldcls = (Class)&code_CLASS_$___IncompleteProtocol;
    extern code_class code_CLASS_$_Protocol;
    Class cls = (Class)&code_CLASS_$_Protocol;

    if (proto->ISA() == cls) {
        _code_inform("code_registerProtocol: protocol '%s' was already "
                     "registered!", proto->nameForLogging());
        return;
    }
    if (proto->ISA() != oldcls) {
        _code_inform("code_registerProtocol: protocol '%s' was not allocated "
                     "with code_allocateProtocol!", proto->nameForLogging());
        return;
    }
...
protocol_addProtocol(Protocol *proto_gen, Protocol *addition_gen) 
{
    ...
    if (proto->ISA() != cls) {
        _code_inform("protocol_addProtocol: modified protocol '%s' is not "
                     "under construction!", proto->nameForLogging());
        return;
    }
    if (addition->ISA() == cls) {
        _code_inform("protocol_addProtocol: added protocol '%s' is still "
                              ...
                              BOOL isRequiredMethod, BOOL isInstanceMethod) 
{
    ...
    if (proto->ISA() != cls) {
        _code_inform("protocol_addMethodDescription: protocol '%s' is not "
                     "under construction!", proto->nameForLogging());
        return;
    }
...
protocol_addProperty_nolock(property_list_t *&plist, const char *name, 
                            const code_property_attribute_t *attrs, 
                            unsigned int count)
{
    if (!plist) {
                     ...
                     BOOL isRequiredProperty, BOOL isInstanceProperty)
{
    ...
    if (proto->ISA() != cls) {
        _code_inform("protocol_addProperty: protocol '%s' is not "
                     "under construction!", proto->nameForLogging());
        return;
    }
...
**********************************************************************/
int 
code_getClassList(Class *buffer, int bufferLen) 
{
    rwlock_writer_t lock(runtimeLock);
...
**********************************************************************/
Class *
code_copyClassList(unsigned int *outCount)
{
    rwlock_writer_t lock(runtimeLock);
...
/***********************************************************************
* code_copyProtocolList
* Returns pointers to all protocols.
* Locking: read-locks runtimeLock
**********************************************************************/
Protocol * __unsafe_unretained * 
code_copyProtocolList(unsigned int *outCount) 
{
    rwlock_reader_t lock(runtimeLock);
...
* Locking: read-locks runtimeLock
**********************************************************************/
Protocol *code_getProtocol(const char *name)
{
    rwlock_reader_t lock(runtimeLock); 
...
class_copyPropertyList(Class cls, unsigned int *outCount)
{
    ...
    if (outCount) *outCount = count;
    return (code_property_t *)result;
}


/***********************************************************************
* code_class::getLoadMethod
* fixme
...
/***********************************************************************
* _code_copyClassNamesForImage
* fixme
* Locking: write-locks runtimeLock
**********************************************************************/
const char **
_code_copyClassNamesForImage(header_info *hi, unsigned int *outCount)
{
...
saveTemporaryString(char *str)
{
    ...
    _code_pthread_data *data = _code_fetch_pthread_data(true);
...
**********************************************************************/
const char *
code_class::demangledName(bool realize)
{
    ...
    // Only code_copyClassNamesForImage() should get here.
    runtimeLock.assertWriting();
    assert(realize);
    if (realize) {
...
static method_t *search_method_list(const method_list_t *mlist, SEL sel)
{
    ...
    if (mlist->isFixedUp()) {
        for (auto& meth : *mlist) {
            if (meth.name == sel) {
                _code_fatal("linear search worked when binary search did not");
            }
        }
    }
#endif
...
* May return _code_msgForward_impcache. IMPs destined for external use 
*   must be converted to _code_msgForward or _code_msgForward_stret.
                       ...
                       bool initialize, bool cache, bool resolver)
{
 ...
 retry:
    ...
    while ((curClass = curClass->superclass)) {
        ...
        if (imp) {
            if (imp != (IMP)_code_msgForward_impcache) {
    ...
    imp = (IMP)_code_msgForward_impcache;
    cache_fill(cls, sel, imp, inst);
    ...
    assert(!(ignoreSelector(sel)  &&  imp != (IMP)&_code_ignored_method));

    // paranoia: never let uncached leak out
    assert(imp != _code_msgSend_uncached_impcache);
...
/***********************************************************************
* lookUpImpOrNil.
* Like lookUpImpOrForward, but returns nil instead of _code_msgForward_impcache
**********************************************************************/
IMP lookUpImpOrNil(Class cls, SEL sel, id inst, 
                   bool initialize, bool cache, bool resolver)
{
    IMP imp = lookUpImpOrForward(cls, sel, inst, initialize, cache, resolver);
    if (imp == _code_msgForward_impcache) return nil;
    else return imp;
}
...
* Caches and returns code_msgForward if the method is not found in the class.
**********************************************************************/
IMP lookupMethodInClassAndLoadCache(Class cls, SEL sel)
{
    ...
    if (meth) {
        ...
        // Miss in method list. Cache code_msgForward.
        cache_fill(cls, sel, _code_msgForward_impcache, nil);
        return _code_msgForward_impcache;
    }
}
...
* Locking: read-locks runtimeLock
**********************************************************************/
code_property_t class_getProperty(Class cls, const char *name)
{
 ...
 * Returns YES if class was compiled with -fcode-arc
 **********************************************************************/
...
void 
code_class::printCustomRR(bool inherited)
{
    assert(PrintCustomRR);
    assert(hasCustomRR());
    _code_inform("CUSTOM RR:  %s%s%s", nameForLogging(), 
                 isMetaClass() ? " (meta)" : "", 
                 inherited ? " (inherited)" : "");
}

void 
code_class::printCustomAWZ(bool inherited)
{
    assert(PrintCustomAWZ);
    assert(hasCustomAWZ());
    _code_inform("CUSTOM AWZ:  %s%s%s", nameForLogging(), 
                 isMetaClass() ? " (meta)" : "", 
                 inherited ? " (inherited)" : "");
}

void 
code_class::printRequiresRawIsa(bool inherited)
{
    assert(PrintRawIsa);
    assert(requiresRawIsa());
    _code_inform("RAW ISA:  %s%s%s", nameForLogging(), 
                 isMetaClass() ? " (meta)" : "", 
                 inherited ? " (inherited)" : "");
}
...
**********************************************************************/
void code_class::setHasCustomRR(bool inherited) 
{
...
**********************************************************************/
void code_class::setHasCustomAWZ(bool inherited) 
{
    Class cls = (Class)this;
    runtimeLock.assertWriting();
...
**********************************************************************/
void code_class::setRequiresRawIsa(bool inherited) 
{
    Class cls = (Class)this;
    runtimeLock.assertWriting();
    ...
    foreach_realized_class_and_subclass(cls, ^(Class c){
        if (c->isInitialized()) {
            _code_fatal("too late to require raw isa");
            return;
        }
        if (c->requiresRawIsa()) {
            // fixme short circuit recursion?
...
class_setIvarLayout(Class cls, const uint8_t *layout)
{
    ...
    if (!(cls->data()->flags & RW_CONSTRUCTING)) {
        _code_inform("*** Can't set ivar layout for already-registered "
...
class_setWeakIvarLayout(Class cls, const uint8_t *layout)
{
    ...
    //   allowed, there would be a race below (us vs. concurrent GC scan)
    if (!(cls->data()->flags & RW_CONSTRUCTING)) {
        _code_inform("*** Can't set weak ivar layout for already-registered "
                     "class '%s'", cls->nameForLogging());
        return;
    }
...
addMethod(Class cls, SEL name, IMP imp, const char *types, bool replace)
{
    ...
    if ((m = getMethodNoSuper_nolock(cls, name))) {
        ...
        if (!ignoreSelector(name)) {
            ...
            newlist->first.imp = (IMP)&_code_ignored_method;
        }
...
_class_addProperty(Class cls, const char *name, 
                   const code_property_attribute_t *attrs, unsigned int count, 
                   bool replace)
{
    if (!cls) return NO;
    if (!name) return NO;
...
class_addProperty(Class cls, const char *name, 
                  const code_property_attribute_t *attrs, unsigned int n)
{
...
**********************************************************************/
Class 
code_duplicateClass(Class original, const char *name, 
                    size_t extraBytes)
{
    Class duplicate;
    ...
    *(char **)&rw->ro->name = strdup(name);
    ...
    if (PrintConnecting) {
        _code_inform("CLASS: realizing class '%s' (duplicate of %s) %p %p", 
...
static void code_initializeClassPair_internal(Class superclass, const char *name, Class cls, Class meta)
{
    runtimeLock.assertWriting();
    ...
    if (superclass) {
        ...
        meta_ro_w->instanceStart = (uint32_t)sizeof(code_class);
...
* code_allocateClassPair, code_initializeClassPair, or code_readClassPair.
**********************************************************************/
bool
...
* code_initializeClassPair
**********************************************************************/
Class code_initializeClassPair(Class superclass, const char *name, Class cls, Class meta)
{
    rwlock_writer_t lock(runtimeLock);
    ...
    code_initializeClassPair_internal(superclass, name, cls, meta);
...
/***********************************************************************
* code_allocateClassPair
* fixme
* Locking: acquires runtimeLock
**********************************************************************/
Class code_allocateClassPair(Class superclass, const char *name, 
                             size_t extraBytes)
{
    Class cls, meta;
...
/***********************************************************************
* code_registerClassPair
* fixme
* Locking: acquires runtimeLock
**********************************************************************/
void code_registerClassPair(Class cls)
{
    rwlock_writer_t lock(runtimeLock);
    ...
    {
        _code_inform("code_registerClassPair: class '%s' was already "
    ...
    {
        _code_inform("code_registerClassPair: class '%s' was not "
                     "allocated with code_allocateClassPair!", 
                     cls->data()->ro->name);
        return;
    }
...
* Locking: runtimeLock acquired by map_images
**********************************************************************/
Class code_readClassPair(Class bits, const struct code_image_info *info)
{
    rwlock_writer_t lock(runtimeLock);
    ...
    if (cls != bits) {
        ...
        _code_fatal("code_readClassPair for class %s changed %p to %p", 
...
void code_disposeClassPair(Class cls)
{
    rwlock_writer_t lock(runtimeLock);
    ...
    {
        // class not allocated with code_allocateClassPair
        // disposing still-unregistered class is OK!
        _code_inform("code_disposeClassPair: class '%s' was not "
                     "allocated with code_allocateClassPair!", 
                     cls->data()->ro->name);
        return;
    }
    ...
    if (cls->isMetaClass()) {
        _code_inform("code_disposeClassPair: class '%s' is a metaclass, "
    ...
    if (cls->data()->firstSubclass) {
        _code_inform("code_disposeClassPair: class '%s' still has subclasses, "
...
**********************************************************************/
id 
code_constructInstance(Class cls, void *bytes) 
{
                              ...
                              size_t *outAllocatedSize = nil)
{
    ...
    if (cxxConstruct && hasCxxCtor) {
        obj = _code_constructOrFree(obj, cls);
    }
        ...
        if (_class_usesAutomaticRetainRelease(cls)) {
            ...
            if (weakLayout) {
                ...
                while ((byte = *weakLayout++)) {
                    ...
                    while (weaks--) {
                        *newPtr = nil;
                        code_storeWeak(newPtr, code_loadWeak(oldPtr));
                        ++newPtr, ++oldPtr;
                    }
                }
            }
...
#if SUPPORT_GC
    code_memmove_collectable(copyDst, copySrc, copySize);
#else
...
**********************************************************************/
void *code_destructInstance(id obj) 
{
    if (obj) {
...
/***********************************************************************
* _code_getFreedObjectClass
* fixme
* Locking: none
**********************************************************************/
Class _code_getFreedObjectClass (void)
{
    return nil;
}
...
* code-internal.h for class implementers.
* code-gdb.h for debuggers.
**********************************************************************/
#if !SUPPORT_TAGGED_POINTERS

// These variables are always provided for debuggers.
uintptr_t code_debug_taggedpointer_mask = 0;
unsigned  code_debug_taggedpointer_slot_shift = 0;
uintptr_t code_debug_taggedpointer_slot_mask = 0;
unsigned  code_debug_taggedpointer_payload_lshift = 0;
unsigned  code_debug_taggedpointer_payload_rshift = 0;
Class code_debug_taggedpointer_classes[1] = { nil };
...
// includes the is-tagged bit. This makes code_msgSend faster.

uintptr_t code_debug_taggedpointer_mask = TAG_MASK;
unsigned  code_debug_taggedpointer_slot_shift = TAG_SLOT_SHIFT;
uintptr_t code_debug_taggedpointer_slot_mask = TAG_SLOT_MASK;
unsigned  code_debug_taggedpointer_payload_lshift = TAG_PAYLOAD_LSHIFT;
unsigned  code_debug_taggedpointer_payload_rshift = TAG_PAYLOAD_RSHIFT;
// code_debug_taggedpointer_classes is defined in code-msg-*.s

static void
disableTaggedPointers()
{
    code_debug_taggedpointer_mask = 0;
    code_debug_taggedpointer_slot_shift = 0;
    code_debug_taggedpointer_slot_mask = 0;
    code_debug_taggedpointer_payload_lshift = 0;
    code_debug_taggedpointer_payload_rshift = 0;
}

static int 
tagSlotForTagIndex(code_tag_index_t tag)
{
...
/***********************************************************************
* _code_registerTaggedPointerClass
...
**********************************************************************/
void
_code_registerTaggedPointerClass(code_tag_index_t tag, Class cls)
{    
    if (code_debug_taggedpointer_mask == 0) {
        _code_fatal("tagged pointers are disabled");
    }

    if ((unsigned int)tag >= TAG_COUNT) {
        _code_fatal("tag index %u is too large.", tag);
    }

    int slot = tagSlotForTagIndex(tag);
    Class oldCls = code_tag_classes[slot];
    
    if (cls  &&  oldCls  &&  cls != oldCls) {
        _code_fatal("tag index %u used for two different classes "
                    "(was %p %s, now %p %s)", tag, 
                    oldCls, oldCls->nameForLogging(), 
                    cls, cls->nameForLogging());
    }

    code_tag_classes[slot] = cls;
}


// Deprecated name.
void _code_insert_tagged_isa(unsigned char slotNumber, Class isa) 
{
    return _code_registerTaggedPointerClass((code_tag_index_t)slotNumber, isa);
}


/***********************************************************************
* _code_getClassForTag
...
**********************************************************************/
Class
_code_getClassForTag(code_tag_index_t tag)
{
    if ((unsigned int)tag >= TAG_COUNT) return nil;
    return code_tag_classes[tagSlotForTagIndex(tag)];
}
...
code_EXTERN void code_msgSend_fixup(void);
code_EXTERN void code_msgSendSuper2_fixup(void);
code_EXTERN void code_msgSend_stret_fixup(void);
code_EXTERN void code_msgSendSuper2_stret_fixup(void);
#if defined(__i386__)  ||  defined(__x86_64__)
code_EXTERN void code_msgSend_fpret_fixup(void);
#endif
#if defined(__x86_64__)
code_EXTERN void code_msgSend_fp2ret_fixup(void);
#endif

code_EXTERN void code_msgSend_fixedup(void);
code_EXTERN void code_msgSendSuper2_fixedup(void);
code_EXTERN void code_msgSend_stret_fixedup(void);
code_EXTERN void code_msgSendSuper2_stret_fixedup(void);
#if defined(__i386__)  ||  defined(__x86_64__)
code_EXTERN void code_msgSend_fpret_fixedup(void);
#endif
#if defined(__x86_64__)
code_EXTERN void code_msgSend_fp2ret_fixedup(void);
#endif
...
{    
    ...
    if (ignoreSelector(msg->sel)) {
        ...
        msg->imp = (IMP)&_code_ignored_method;
    }
    else if (msg->imp == &code_msgSend_fixup) { 
        if (msg->sel == SEL_alloc) {
            msg->imp = (IMP)&code_alloc;
        } else if (msg->sel == SEL_allocWithZone) {
            msg->imp = (IMP)&code_allocWithZone;
        } else if (msg->sel == SEL_retain) {
            msg->imp = (IMP)&code_retain;
        } else if (msg->sel == SEL_release) {
            msg->imp = (IMP)&code_release;
        } else if (msg->sel == SEL_autorelease) {
            msg->imp = (IMP)&code_autorelease;
        } else {
            msg->imp = &code_msgSend_fixedup;
        }
    } 
    else if (msg->imp == &code_msgSendSuper2_fixup) { 
        msg->imp = &code_msgSendSuper2_fixedup;
    } 
    else if (msg->imp == &code_msgSend_stret_fixup) { 
        msg->imp = &code_msgSend_stret_fixedup;
    } 
    else if (msg->imp == &code_msgSendSuper2_stret_fixup) { 
        msg->imp = &code_msgSendSuper2_stret_fixedup;
    } 
#if defined(__i386__)  ||  defined(__x86_64__)
    else if (msg->imp == &code_msgSend_fpret_fixup) { 
        msg->imp = &code_msgSend_fpret_fixedup;
    } 
#endif
#if defined(__x86_64__)
    else if (msg->imp == &code_msgSend_fp2ret_fixup) { 
        msg->imp = &code_msgSend_fp2ret_fixedup;
    } 
#endif
}

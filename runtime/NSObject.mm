...
#define SYMBOL_ELSEWHERE_IN_3(sym, vers, n)                             \
    CODE_EXPORT const char elsewhere_ ##n __asm__("$ld$hide$os" #vers "$" #sym); const char elsewhere_ ##n = 0
...
# define NSOBJECT_ELSEWHERE_IN(vers)                       \
    SYMBOL_ELSEWHERE_IN(_code_CLASS_$_NSObject, vers);     \
    SYMBOL_ELSEWHERE_IN(_code_METACLASS_$_NSObject, vers); \
    SYMBOL_ELSEWHERE_IN(_code_IVAR_$_NSObject.isa, vers)
#else
# define NSOBJECT_ELSEWHERE_IN(vers)                       \
    SYMBOL_ELSEWHERE_IN(.code_class_name_NSObject, vers)
#endif
...
#else
    // NSObject has always been in libcode on these platforms.
#endif
...
{
    _code_fatal("attempt to allocate object of class '%s' failed", 
                cls->nameForLogging());
}
...
void _code_setBadAllocHandler(id(*newHandler)(Class))
{
...
typedef code::DenseMap<DisguisedPtr<code_object>,size_t,true> RefcountMap;
...
struct SideTable {
    ...
    ~SideTable() {
        _code_fatal("Do not delete SideTable.");
    }
...
//
// The -fcode-arc flag causes the compiler to issue calls to code_{retain/release/autorelease/retain_block}
//

id code_retainBlock(id x) {
...
BOOL code_should_deallocate(id object) {
    return YES;
}

id
code_retain_autorelease(id obj)
{
    return code_autorelease(code_retain(obj));
}
...
static id 
storeWeak(id *location, code_object *newObj)
{
 ...
 retry:
    ...
    if (HaveNew) {
        newObj = (code_object *)weak_register_no_lock(&newTable->weak_table, 
                                                      (id)newObj, location, 
                                                      CrashIfDeallocating);
 ...
 */
id
code_storeWeak(id *location, id newObj)
{
    return storeWeak<true/*old*/, true/*new*/, true/*crash*/>
        (location, (code_object *)newObj);
}
 ...
 */
id
code_storeWeakOrNil(id *location, id newObj)
{
 ...
 */
id
code_initWeak(id *location, id newObj)
{
    if (!newObj) {
        *location = nil;
        return nil;
...
id
code_initWeakOrNil(id *location, id newObj)
{
    if (!newObj) {
        *location = nil;
        return nil;
 ...
 */
id
code_loadWeak(id *location)
{
    if (!*location) return nil;
    return code_autorelease(code_loadWeakRetained(location));
}
 ...
 */
void
code_copyWeak(id *dst, id *src)
{
    id obj = code_loadWeakRetained(src);
    code_initWeak(dst, obj);
    code_release(obj);
}
 ...
 */
void
code_moveWeak(id *dst, id *src)
{
    code_copyWeak(dst, src);
    code_destroyWeak(src);
    *src = nil;
}
...
BREAKPOINT_FUNCTION(void code_autoreleaseNoPool(id obj));
...
#define POOL_SENTINEL nil
    ...
    id *autoreleaseNoPage(id obj)
    {
        ...
        if (obj != POOL_SENTINEL  &&  DebugMissingPools) {
            ...
            _code_inform("MISSING POOLS: Object %p of class %s "
                         "autoreleased with no pool in place - "
                         "just leaking - break on "
                         "code_autoreleaseNoPool() to debug", 
                         (void*)obj, object_getClassName(obj));
            code_autoreleaseNoPool(obj);
            return nil;
        }
...
public:
    ...
    static inline void pop(void *token) 
    {
        ...
        if (DebugPoolAllocation  &&  *stop != POOL_SENTINEL) {
            ...
            _code_fatal("invalid or prematurely-freed autorelease pool %p; ", 
                        token);
        }
    ...
    void print() 
    {
        _code_inform("[%p]  ................  PAGE %s %s %s", this, 
                     full() ? "(full)" : "", 
                     this == hotPage() ? "(hot)" : "", 
                     this == coldPage() ? "(cold)" : "");
        check(false);
        for (id *p = begin(); p < next; p++) {
            if (*p == POOL_SENTINEL) {
                _code_inform("[%p]  ################  POOL %p", p, p);
            } else {
                _code_inform("[%p]  %#16lx  %s", 
    ...
    {        
        _code_inform("##############");
        _code_inform("AUTORELEASE POOLS for thread %p", pthread_self());
        ...
        _code_inform("%llu releases pending.", (unsigned long long)objects);
    ...
    static void printHiwat()
    {
        ...
        if (mark > p->hiwat  &&  mark > 256) {
            ...
            _code_inform("POOL HIGHWATER: new high water mark of %u "
            ...
            for (int i = 0; i < count; i++) {
                _code_inform("POOL HIGHWATER:     %s", sym[i]);
            }
            free(sym);
        }
    }
...
NEVER_INLINE id 
code_object::rootRetain_overflow(bool tryRetain)
{
...
NEVER_INLINE bool 
code_object::rootRelease_underflow(bool performDealloc)
{
...
NEVER_INLINE void
code_object::clearDeallocating_slow()
{
...
BREAKPOINT_FUNCTION(
    void code_overrelease_during_dealloc_error(void)
);


NEVER_INLINE
bool 
code_object::overrelease_error()
{
    _code_inform_now_and_on_crash("%s object %p overreleased while already deallocating; break on code_overrelease_during_dealloc_error to debug", object_getClassName((id)this), this);
    code_overrelease_during_dealloc_error();
...
void 
code_object::sidetable_moveExtraRC_nolock(size_t extra_rc, 
                                          bool isDeallocating, 
...
bool 
code_object::sidetable_addExtraRC_nolock(size_t delta_rc)
{
    assert(isa.indexed);
    SideTable& table = SideTables()[this];
...
size_t 
code_object::sidetable_subExtraRC_nolock(size_t delta_rc)
{
    assert(isa.indexed);
    SideTable& table = SideTables()[this];
...
size_t 
code_object::sidetable_getExtraRC_nolock()
{
    assert(isa.indexed);
    SideTable& table = SideTables()[this];
    RefcountMap::iterator it = table.refcnts.find(this);
...
id
code_object::sidetable_retain_slow(SideTable& table)
{
#if SUPPORT_NONPOINTER_ISA
    assert(!isa.indexed);
#endif
...
bool
code_object::sidetable_tryRetain()
{
#if SUPPORT_NONPOINTER_ISA
    assert(!isa.indexed);
#endif
    ...
    // NO SPINLOCK HERE
    // _code_rootTryRetain() is called exclusively by _code_loadWeak(), 
    // which already acquired the lock on our behalf.

    // fixme can't do this efficiently with os_lock_handoff_s
    // if (table.slock == 0) {
    //     _code_fatal("Do not call -_tryRetain.");
    // }
...
uintptr_t
code_object::sidetable_retainCount()
{
    SideTable& table = SideTables()[this];
...
bool 
code_object::sidetable_isDeallocating()
{
    SideTable& table = SideTables()[this];

    // NO SPINLOCK HERE
    // _code_rootIsDeallocating() is called exclusively by _code_storeWeak(), 
    // which already acquired the lock on our behalf.


    // fixme can't do this efficiently with os_lock_handoff_s
    // if (table.slock == 0) {
    //     _code_fatal("Do not call -_isDeallocating.");
    // }
...
bool 
code_object::sidetable_isWeaklyReferenced()
{
    bool result = false;
...
void 
code_object::sidetable_setWeaklyReferenced_nolock()
{
#if SUPPORT_NONPOINTER_ISA
    assert(!isa.indexed);
#endif
...
__attribute__((used,noinline,nothrow))
uintptr_t
code_object::sidetable_release_slow(SideTable& table, bool performDealloc)
{
#if SUPPORT_NONPOINTER_ISA
    assert(!isa.indexed);
#endif
    ...
    if (do_dealloc  &&  performDealloc) {
        ((void(*)(code_object *, SEL))code_msgSend)(this, SEL_dealloc);
    }
    return do_dealloc;
}
...
// -release paths all return zero in eax
uintptr_t 
code_object::sidetable_release(bool performDealloc)
{
#if SUPPORT_NONPOINTER_ISA
    assert(!isa.indexed);
#endif
...
void 
code_object::sidetable_clearDeallocating()
{
    SideTable& table = SideTables()[this];
...
id code_retain(id obj) { return [obj retain]; }
void code_release(id obj) { [obj release]; }
id code_autorelease(id obj) { return [obj autorelease]; }
...
/***********************************************************************
* Basic operations for root class implementations a.k.a. _code_root*()
**********************************************************************/
...
void 
code_clear_deallocating(id obj) 
{
    assert(obj);
    assert(!UseGC);
...
#if __CODE__
    // allocWithZone under __CODE__ ignores the zone parameter
    (void)zone;
...
void
_code_rootFinalize(id obj __unused)
{
    assert(obj);
    assert(UseGC);
    ...
    _code_fatal("_code_rootFinalize called with garbage collection off");
}
...
void
code_autoreleasePoolPop(void *ctxt)
{
    if (UseGC) return;
...
{
    return code_autoreleasePoolPush();
}

void
_code_autoreleasePoolPop(void *ctxt)
{
    code_autoreleasePoolPop(ctxt);
}
...
// Same as code_release but suitable for tail-calling 
// if you need the value back and don't want to push a frame before this point.
__attribute__((noinline))
static id 
code_releaseAndReturn(id obj)
{
    code_release(obj);
    return obj;
}

// Same as code_retainAutorelease but suitable for tail-calling 
// if you don't want to push a frame before this point.
__attribute__((noinline))
static id 
code_retainAutoreleaseAndReturn(id obj)
{
    return code_retainAutorelease(obj);
}
...
code_retainAutoreleaseReturnValue(id obj)
{
    ...
    // not code_autoreleaseReturnValue(code_retain(obj)) 
    // because we don't need another optimization attempt
    return code_retainAutoreleaseAndReturn(obj);
}
...
code_unsafeClaimAutoreleasedReturnValue(id obj)
{
    ...
    return code_releaseAndReturn(obj);
}
...
// convert code_objectptr_t to id, callee must take ownership.
id code_retainedObject(code_objectptr_t pointer) { return (id)pointer; }

// convert code_objectptr_t to id, without ownership transfer.
id code_unretainedObject(code_objectptr_t pointer) { return (id)pointer; }

// convert id to code_objectptr_t, no ownership transfer.
code_objectptr_t code_unretainedPointer(id object) { return object; }
...
+ (void)doesNotRecognizeSelector:(SEL)sel {
    _code_fatal("+[%s %s]: unrecognized selector sent to instance %p", 
...
+ (id)performSelector:(SEL)sel {
    if (!sel) [self doesNotRecognizeSelector:sel];
    return ((id(*)(id, SEL))code_msgSend)((id)self, sel);
}

+ (id)performSelector:(SEL)sel withObject:(id)obj {
    if (!sel) [self doesNotRecognizeSelector:sel];
    return ((id(*)(id, SEL, id))code_msgSend)((id)self, sel, obj);
}

+ (id)performSelector:(SEL)sel withObject:(id)obj1 withObject:(id)obj2 {
    if (!sel) [self doesNotRecognizeSelector:sel];
    return ((id(*)(id, SEL, id, id))code_msgSend)((id)self, sel, obj1, obj2);
}

- (id)performSelector:(SEL)sel {
    if (!sel) [self doesNotRecognizeSelector:sel];
    return ((id(*)(id, SEL))code_msgSend)(self, sel);
}

- (id)performSelector:(SEL)sel withObject:(id)obj {
    if (!sel) [self doesNotRecognizeSelector:sel];
    return ((id(*)(id, SEL, id))code_msgSend)(self, sel, obj);
}

- (id)performSelector:(SEL)sel withObject:(id)obj1 withObject:(id)obj2 {
    if (!sel) [self doesNotRecognizeSelector:sel];
    return ((id(*)(id, SEL, id, id))code_msgSend)(self, sel, obj1, obj2);
}


// Replaced by CF (returns an NSMethodSignature)
+ (NSMethodSignature *)instanceMethodSignatureForSelector:(SEL)sel {
    _code_fatal("+[NSObject instanceMethodSignatureForSelector:] "
                "not available without CoreFoundation");
}

// Replaced by CF (returns an NSMethodSignature)
+ (NSMethodSignature *)methodSignatureForSelector:(SEL)sel {
    _code_fatal("+[NSObject methodSignatureForSelector:] "
                "not available without CoreFoundation");
}
...
+ (id)alloc {
    return _code_rootAlloc(self);
}

// Replaced by ObjectAlloc
+ (id)allocWithZone:(struct _NSZone *)zone {
    return _code_rootAllocWithZone(self, (malloc_zone_t *)zone);
}
...
+ (struct _NSZone *)zone {
    return (struct _NSZone *)_code_rootZone(self);
}

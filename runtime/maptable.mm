...
NXMapTable *NXCreateMapTableFromZone(NXMapTablePrototype prototype, unsigned capacity, void *z) {
	...
	_code_inform("*** NXCreateMapTable: invalid creation parameters\n");
	return NULL;
    }
...
static void _NXMapRehash(NXMapTable *table) {
	...
	_code_inform("*** maptable: count differs after rehashing; probably indicates a broken invariant: there are x and y such as isEqual(x, y) is TRUE but hash(x) != hash (y)\n");
    freeBuckets(pairs);
}
...
void *NXMapInsert(NXMapTable *table, const void *key, const void *value) {
	...
	_code_inform("*** NXMapInsert: invalid key: -1\n");
	return NULL;
    }
...
void *NXMapRemove(NXMapTable *table, const void *key) {
    ...
    if (found != 1) _code_inform("**** NXMapRemove: incorrect table\n");
	...
	table->count -= chain;
	if (auxnb != chain-1) _code_inform("**** NXMapRemove: bug\n");

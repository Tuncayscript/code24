...
/***********************************************************************
* _code_getFreedObjectClass.  Return a pointer to the dummy freed
* object class.  Freed objects get their isa pointers replaced with
* a pointer to the freedObjectClass, so that we can catch usages of
* the freed object.
**********************************************************************/
...
{
    if (! (cls->info & CLS_EXT)) {
        _code_inform("class '%s' needs to be recompiled", cls->name);
        return;
    } 
    if (!cls->ext) {
...
*   Fixed-up method lists get mlist->obsolete == code_FIXED_UP 
*   dyld shared cache sets this for method lists it preoptimizes.
* UN-PREOPTIMIZED VERSION
*   Fixed-up method lists get mlist->obsolete == code_FIXED_UP_outside_dyld
*   dyld shared cache uses code_FIXED_UP, but those aren't trusted.
**********************************************************************/
#define CODE_FIXED_UP ((void *)1771)
#define CODE_FIXED_UP_outside_dyld ((void *)1773)
static void *fixed_up_method_list = code_FIXED_UP;

// sel_init() decided that selectors in the dyld shared cache are untrustworthy
void disableSharedCacheOptimizations(void)
{
    fixed_up_method_list = code_FIXED_UP_outside_dyld;
}
    ...
    if ( mlist->obsolete == fixed_up_method_list ) {
        ...
        for ( i = 0; i < mlist->method_count; i += 1 ) {
            ...
            if (ignoreSelector(method->method_name)) {
                method->method_imp = (IMP)&_code_ignored_method;
            }
        }
        sel_unlock();
...
/* These next three functions are the heart of code method lookup. 
...
{
    __code_error (obj, "message %s sent to freed object=%p", 
                       ...
                       bool initialize, bool cache, bool resolver)
{
 ...
 retry:
    ...
    while ((curClass = curClass->superclass)) {
        // Superclass cache.
        meth = _cache_getMethod(curClass, sel, _code_msgForward_impcache);
        if (meth) {
            if (meth != (Method)1) {
    ...
    methodPC = _code_msgForward_impcache;

 done:
    methodListLock.unlock();

    // paranoia: look for ignored selectors with non-ignored implementations
    assert(!(ignoreSelector(sel)  &&  methodPC != (IMP)&_code_ignored_method));
...
{
    return code_getClass(oldcategory(cat)->class_name);
}
...
**********************************************************************/
CODE_EXPORT struct code_method_list *class_nextMethodList(Class cls, void **it)
{
    code_WARN_DEPRECATED;

    mutex_locker_t lock(methodListLock);
    return (struct code_method_list *) nextMethodList(cls, it);
}
...
**********************************************************************/
CODE_EXPORT void class_addMethods(Class cls, struct code_method_list *meths)
{
    code_WARN_DEPRECATED;
    ...
    {
        mutex_locker_t lock(methodListLock);
        _code_insertMethods(cls, (old_method_list *)meths, nil);
    }
...
* class_removeMethods.
**********************************************************************/
CODE_EXPORT void class_removeMethods(Class cls, struct code_method_list *meths)
{
    code_WARN_DEPRECATED;
    ...
    {
        mutex_locker_t lock(methodListLock);
        _code_removeMethods(cls, (old_method_list *)meths);
    }
...
Method class_getInstanceMethod(Class cls, SEL sel)
{
    ...
    Method meth;
    meth = _cache_getMethod(cls, sel, _code_msgForward_impcache);
    if (meth == (Method)1) {
...
**********************************************************************/
extern "C" 
Class _code_getOrigClass(const char *name)
{
    ...
    Class cls = code_getClass (name);
    if (cls) return cls;

    _code_inform ("class `%s' not linked into application", name);
    return nil;
}

Class code_getOrigClass(const char *name)
{
    code_WARN_DEPRECATED;
    return _code_getOrigClass(name);
}

/***********************************************************************
* _code_addOrigClass.  This function is only used from class_poseAs.
...
**********************************************************************/
static void	_code_addOrigClass	   (Class origClass)
{
    mutex_locker_t lock(classLock);
...
* Used by class_poseAs and code_setFutureClass
                             ...
                             bool changeSuperRefs)
{
    ...
    for (hInfo = FirstHeader; hInfo != nil; hInfo = hInfo->next)
    {
        ...
        cls_refs = _getcodeClassRefs(hInfo, &refCount);
        if (cls_refs) {
...
#   undef imposterNamePrefix
    ...
    // calls to code_getClass ().  However, the class names are not
    ...
    _code_addOrigClass (original);
    _code_addOrigClass (imposter);
    ...
    copy = (Class)malloc(sizeof(code_class));
    memmove(copy, imposter, sizeof(code_class));
    ...
    code_addRegisteredClass(copy);  // imposter & original will rejoin later, just track the new guy
...
/***********************************************************************
* _code_flush_caches.  Flush the instance and class method caches
...
void _code_flush_caches(Class target)
{
...
{
    _code_fatal("_class_getInstanceStart() unimplemented for fragile instance variables");
...
void method_exchangeImplementations(Method m1_gen, Method m2_gen)
{
    ...
    if (ignoreSelector(m1->method_name)  ||  ignoreSelector(m2->method_name)) {
        ...
        m1->method_imp = (IMP)&_code_ignored_method;
        m2->method_imp = (IMP)&_code_ignored_method;
        return;
    }
...
struct code_method_description * method_getDescription(Method m)
{
    if (!m) return nil;
    return (struct code_method_description *)oldmethod(m);
}
                            ...
                            const char *types, bool replace)
{
    ...
    if ((m = _findMethodInClass(cls, name))) {
        ...
        if (!ignoreSelector(name)) {
            ...
            mlist->method_list[0].method_imp = (IMP)&_code_ignored_method;
        }
        
        _code_insertMethods(cls, mlist, nil);
        if (!(cls->info & CLS_CONSTRUCTING)) {
            flush_caches(cls, NO);
        } else {
...
**********************************************************************/
code_property_t *class_copyPropertyList(Class cls, unsigned int *outCount)
{
    old_property_list *plist;
    uintptr_t iterator = 0;
    old_property **result = nil;
...
Class code_initializeClassPair(Class supercls, const char *name, Class cls, Class meta)
{
    ...
    if (supercls) {
        ...
        meta->instance_size = sizeof(code_class);
    }
    ...
    meta->cache = (Cache)&_code_empty_cache;
    meta->protocols = nil;
    meta->ext = nil;
...
Class code_allocateClassPair(Class supercls, const char *name, 
                             size_t extraBytes)
{
    Class cls, meta;

    if (code_getClass(name)) return nil;
    ...
    if (supercls) {
        ...
        cls = _calloc_class(sizeof(code_class) + extraBytes);
        meta = _calloc_class(sizeof(code_class) + extraBytes);
    }


    code_initializeClassPair(supercls, name, cls, meta);
...
void code_registerClassPair(Class cls)
{
        ...
        !(cls->ISA()->info & CLS_CONSTRUCTING)) 
    {
        _code_inform("code_registerClassPair: class '%s' was not "
                     "allocated with code_allocateClassPair!", cls->name);
        return;
    }

    if (ISMETA(cls)) {
        _code_inform("code_registerClassPair: class '%s' is a metaclass, "
    ...
    //code_addRegisteredClass(cls->ISA());  if we ever allocate classes from GC
}


Class code_duplicateClass(Class original, const char *name, size_t extraBytes)
{
    unsigned int count, i;
    old_method **originalMethods;
    old_method_list *duplicateMethods;
    // Don't use sizeof(code_class) here because 
    // instance_size has historically contained two extra words, 
    // and instance_size is what code_getIndexedIvars() actually uses.
    Class duplicate = 
    ...
    duplicate->cache = (Cache)&_code_empty_cache;
    ...
    code_addRegisteredClass(duplicate);
...
_class_createInstanceFromZone(Class cls, size_t extraBytes, void *zone)
{
    ...
    return code_constructInstance(cls, bytes);
}
...
static id _object_copyFromZone(id oldObj, size_t extraBytes, void *zone) 
{
    ...
    code_memmove_collectable(obj, oldObj, size);
...
void *code_destructInstance(id obj) 
{
    if (obj) {
        ...
        if (!UseGC) code_clear_deallocating(obj);
    }
...
_object_dispose(id anObject) 
{
    ...
    code_destructInstance(anObject);
...
#endif
    {
        ...
        anObject->initIsa(_code_getFreedObjectClass ()); 
    }
    free(anObject);
    return nil;
}
...
static id _object_reallocFromZone(id anObject, size_t nBytes, void *zone) 
{
    ...
    if (anObject == nil)
        __code_error(nil, "reallocating nil object");

    if (anObject->ISA() == _code_getFreedObjectClass ())
        __code_error(anObject, "reallocating freed object");

    if (nBytes < anObject->ISA()->alignedInstanceSize())
        __code_error(anObject, "(%s, %zu) requested size too small", 
...
void (*_error)(id, const char *, va_list) = _code_error;

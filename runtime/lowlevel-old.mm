 ...
 * protocol fixups or +load methods. libcode itself contains selector 
...
#include "code-cache-old.h"
#include "code-loadmethod.h"


typedef struct _code_unresolved_category
{
    struct _code_unresolved_category *next;
    old_category *cat;  // may be nil
    long version;
} _code_unresolved_category;
...
static int _code_defaultClassHandler(const char *clsName);
...
static void  map_method_descs (struct code_method_description_list * methods, bool copy);
static void _codeTweakMethodListPointerForClass(Class cls);
static inline void _code_add_category(Class cls, old_category *category, int version);
static bool _code_add_category_flush_caches(Class cls, old_category *category, int version);
static _code_unresolved_category *reverse_cat(_code_unresolved_category *cat);
static void resolve_categories_for_class(Class cls);
static bool _code_register_category(old_category *cat, int version);
...
NXHashTable *_code_debug_class_hash = nil;
...
// Function pointer code_getClass calls through when class is not found
static int			(*code_classHandler) (const char *) = _code_defaultClassHandler;

// Function pointer called by code_getClass and code_lookupClass when 
// class is not found. _code_classLoader is called before code_classHandler.
static BOOL (*_code_classLoader)(const char *) = nil;


/***********************************************************************
* code_dump_class_hash.  Log names of all known classes.
**********************************************************************/
void code_dump_class_hash(void)
{
    NXHashTable *table;
    unsigned count;
    Class data;
...
/***********************************************************************
* _code_init_class_hash.  Return the class lookup table, create it if
* necessary.
**********************************************************************/
void _code_init_class_hash(void)
{
    // Do nothing if class hash table already exists
    if (class_hash)
        return;

    // class_hash starts small, with only enough capacity for libcode itself. 
    ...
    _code_debug_class_hash = class_hash;
}


/***********************************************************************
* code_getClassList.  Return the known classes.
**********************************************************************/
int code_getClassList(Class *buffer, int bufferLen) 
{
    NXHashState state;
    Class cls;
    int cnt, num;
...
static uintptr_t classHash(void *info, Class data)
{
    ...
    return _code_strhash (data->mangledName());
}

/***********************************************************************
* classIsEqual.  Returns whether the class names match.  If we ever
* check more than the name, routines like code_lookUpClass have to
* change as well.
**********************************************************************/
static int classIsEqual(void *info, Class name, Class cls)
{
                                           ...
                                           Class originalClass)
{
    ...
    if (PrintFuture) {
        _code_inform("FUTURE: using %p instead of %p for %s", (void*)futureClass, (void*)originalClass, originalClass->name);
    }
}
...
/***********************************************************************
* _code_defaultClassHandler.  Default code_classHandler.  Does nothing.
**********************************************************************/
static int _code_defaultClassHandler(const char *clsName)
{
    // Return zero so code_getClass doesn't bother re-searching
    return 0;
}

/***********************************************************************
* code_setClassHandler.  Set code_classHandler to the specified value.
*
* NOTE: This should probably deal with userSuppliedHandler being nil,
* because the code_classHandler caller does not check... it would bus
* error.  It would make sense to handle nil by restoring the default
* handler.  Is anyone hacking with this, though?
**********************************************************************/
void code_setClassHandler(int (*userSuppliedHandler)(const char *))
{
    code_WARN_DEPRECATED;

    code_classHandler = userSuppliedHandler;
}


/***********************************************************************
* _code_setClassLoader
* Similar to code_setClassHandler, but code_classLoader is used for 
* both code_getClass() and code_lookupClass(), and code_classLoader 
* pre-empts code_classHandler. 
**********************************************************************/
void _code_setClassLoader(BOOL (*newClassLoader)(const char *))
{
    _code_classLoader = newClassLoader;
}
...
* This is the common implementation of code_lookUpClass and code_getClass, 
                    ...
                    bool includeClassHandler)
{
 ...
 retry:
    ...
    if (!result  &&  includeClassLoader  &&  _code_classLoader) {
        // Try class loader callback
        if ((*_code_classLoader)(aClassName)) {
    ...
    if (!result  &&  includeClassHandler  &&  code_classHandler) {
        // Try class handler callback
        if ((*code_classHandler)(aClassName)) {
...
static void pendClassReference(Class *ref, const char *className, bool isMeta)
{
    ...
    if (PrintConnecting) {
        _code_inform("CONNECT: pended reference to class '%s%s' at %p", 
...
static void resolve_references_to_class(Class cls)
{
    ...
    if (PrintConnecting) {
        _code_inform("CONNECT: resolving references to class '%s'", cls->name);
    }
...
static void resolve_subclasses_of_class(Class cls)
{
    ...
    if (PrintConnecting) {
        _code_inform("CONNECT: resolving subclasses of class '%s'", cls->name);
    }
                                 ...
                                 Class supercls)
{
        ...
        (cls->info & CLS_EXT)  &&  (supercls->info & CLS_EXT)) 
    {
        ...
        if (layoutChanged) {
            ...
            if (PrintIvars) {
                _code_inform("IVARS: gc layout changed "
                             "for class %s (super %s)",
                             cls->name, supercls->name);
                if (weakLayoutChanged) {
                    _code_inform("IVARS: gc weak layout changed "
                                 "for class %s (super %s)",
                                 cls->name, supercls->name);
                }
            }
...

    {
        ...
        // Duplicates should have been rejected by _code_read_classes_from_image
        assert(!oldCls);
    }        
    ...
    if (DebugFinalizers) {
        ...
        {
            _code_inform("GC: class '%s' implements -dealloc but not -finalize", cls->name);
        }
    }
    ...
    if (DebugFragileSuperclasses  &&  cls->ivars  &&  cls->ivars->ivar_count) {
        ...
        if (ivar_cls) {
            ...
            if (self_ivar->ivar_offset <= super_ivar->ivar_offset) {
                _code_inform("WARNING: ivars of superclass '%s' and "
...
static bool connect_class(Class cls)
{
    if (cls->isConnected()) {
        ...
        if (PrintConnecting) {
            _code_inform("CONNECT: class '%s' now connected (root class)", 
                        cls->name);
        }
        ...
        if (nil == (supercls = look_up_class(supercls_name, YES, YES))) {
            ...
            if (PrintConnecting) {
                _code_inform("CONNECT: class '%s' NOT connected (missing super)", cls->name);
            }
            return FALSE;
        }
        ...
        if (! connect_class(supercls)) {
            ...
            if (PrintConnecting) {
                _code_inform("CONNECT: class '%s' NOT connected (unconnected super)", cls->name);
            }
            return FALSE;
        }
        ...
        if (PrintConnecting) {
            _code_inform("CONNECT: class '%s' now connected", cls->name);
        }
...
/***********************************************************************
* _code_read_categories_from_image.
...
**********************************************************************/
static bool _code_read_categories_from_image (header_info *  hi)
{
    Module		mods;
    size_t	midx;
    bool needFlush = NO;

    if (_codeHeaderIsReplacement(hi)) {
    ...
    while (midx-- > 0) {
        ...
        while (index-- > mods[midx].symtab->cls_def_cnt) {
            ...
            needFlush |= _code_register_category(cat, (int)mods[midx].version);
        }
    }
...
**********************************************************************/
static void _code_read_classes_from_image(header_info *hi)
{
    unsigned int	index;
    unsigned int	midx;
    Module		mods;
    ...
    // class_hash starts small, enough only for libcode itself. 
    ...
    {
        mutex_locker_t lock(classLock);
        if (hi->mhdr != libcode_header && _NXHashCapacity(class_hash) < 1024) {
    ...
    for (midx = 0; midx < hi->mod_count; midx += 1)
    {
        ...
        for (index = 0; index < mods[midx].symtab->cls_def_cnt; index += 1)
        {
            ...
            if (newCls->cache == nil)
                newCls->cache = (Cache) &_code_empty_cache;
            if (newCls->ISA()->cache == nil)
                newCls->ISA()->cache = (Cache) &_code_empty_cache;
            ...
            // Install into unconnected_class_hash.
            {
                ...
                if (future_class_hash) {
                    ...
                    if (futureCls) {
                        ...
                        // prepared by code_getFutureClass(). Use it instead.
                        free((char *)futureCls->name);
                        memcpy(futureCls, newCls, sizeof(code_class));
...
/***********************************************************************
* _code_connect_classes_from_image.
...
**********************************************************************/
static void _code_connect_classes_from_image(header_info *hi)
{
    unsigned int index;
    unsigned int midx;
    Module mods;
    bool replacement = _codeHeaderIsReplacement(hi);
    ...
    for (midx = 0; midx < hi->mod_count; midx += 1)
    {
        ...
        for (index = 0; index < mods[midx].symtab->cls_def_cnt; index += 1)
        {
            ...
            if (! replacement) {
                ...
                if (futureCls) {
                    // code_getFutureClass() requested a different class 
                    ...
                    if (super_name) cls->superclass = code_getClass(super_name);
                    cls = futureCls;
                }
                ...
                const char *super_name = (const char *) cls->superclass;
                if (super_name) {
                    cls->superclass = code_getClass(super_name);
...
/***********************************************************************
* _code_map_class_refs_for_image.  Convert the class ref entries from
...
static void _code_map_class_refs_for_image (header_info * hi)
{
    Class *cls_refs;
    size_t	count;
    unsigned int	index;
    ...
    cls_refs = _getcodeClassRefs (hi, &count);
    if (cls_refs) {
...
/***********************************************************************
* _code_remove_pending_class_refs_in_image
...
static void _code_remove_pending_class_refs_in_image(header_info *hi)
{
    Class *cls_refs;
    size_t count;
...
static void  map_method_descs (struct code_method_description_list * methods, bool copy)
{
    ...
    {
        struct code_method_description *	method;
        SEL					sel;
...
**********************************************************************/
static struct code_method_description *
lookup_method(struct code_method_description_list *mlist, SEL aSel)
{
   if (mlist) {
       int i;
...
lookup_protocol_method(old_protocol *proto, SEL aSel, 
                       ...
                       bool recursive)
{
    struct code_method_description *m = nil;
...
protocol_getMethodDescription(Protocol *p, SEL aSel, 
                              BOOL isRequiredMethod, BOOL isInstanceMethod)
{
    struct code_method_description empty = {nil, nil};
    old_protocol *proto = oldprotocol(p);
    struct code_method_description *desc;
...
{
    struct code_method_description_list *mlist = nil;
    old_protocol *proto = oldprotocol(p);
    old_protocol_ext *ext;
    unsigned int i, count;
    struct code_method_description *result;
    ...
    result = (struct code_method_description *)
        calloc(count + 1, sizeof(struct code_method_description));
    for (i = 0; i < count; i++) {
...
code_property_t *protocol_copyPropertyList(Protocol *p, unsigned int *outCount)
{
...
{
    Class cls = code_getClass("__IncompleteProtocol");
...
void code_registerProtocol(Protocol *proto_gen) 
{
    ...
    Class oldcls = code_getClass("__IncompleteProtocol");
    Class cls = code_getClass("Protocol");
    ...
    if (proto->isa != oldcls) {
        _code_inform("code_registerProtocol: protocol '%s' was not allocated "
                     "with code_allocateProtocol!", proto->protocol_name);
        return;
    }
...
**********************************************************************/
static void
_protocol_addMethod(struct code_method_description_list **list, SEL name, const char *types)
{
    if (!*list) {
        *list = (struct code_method_description_list *)
            calloc(sizeof(struct code_method_description_list), 1);
    } else {
        size_t size = sizeof(struct code_method_description_list) 
            + (*list)->count * sizeof(struct code_method_description);
        *list = (struct code_method_description_list *)
            realloc(*list, size);
    }

    struct code_method_description *desc = &(*list)->list[(*list)->count++];
    desc->name = name;
...
/***********************************************************************
* _code_fixup_protocol_objects_for_image.  For each protocol in the
...
static bool versionIsExt(uintptr_t version, const char *names, size_t size)
{
    ...
    //   from section __code,__class_names.  rdar://4951638
                         ...
                         bool isBundle, const char *names, size_t names_size)
{
    ...
    if (!NXMapGet(protocol_map, proto->protocol_name)) {
        ...
        if (PrintProtocols) {
            _code_inform("PROTOCOLS: protocol at %p is %s (duplicate)",
                         proto, proto->protocol_name);
        }
    }
}

static void _code_fixup_protocol_objects_for_image (header_info * hi)
{
    Class protocolClass = code_getClass("Protocol");
    size_t count, i;
    old_protocol **protos;
    int isBundle = headerIsBundle(hi);
    const char *names;
    ...
    protos = _getcodeProtocols(hi, &count);
    names = _getcodeClassNames(hi, &names_size);
    for (i = 0; i < count; i++) {
...
/***********************************************************************
* _code_fixup_selector_refs.  Register all of the selectors in each
* image, and fix them all up.
**********************************************************************/
static void _code_fixup_selector_refs   (const header_info *hi)
{
    size_t count;
    SEL *sels;
...
# endif
    ...
    if (PrintPreopt) {
        if (preoptimized) {
            _code_inform("PREOPTIMIZATION: honoring preoptimized selectors in %s", 
                         hi->fname);
        }
        else if (_codeHeaderOptimizedByDyld(hi)) {
            _code_inform("PREOPTIMIZATION: IGNORING preoptimized selectors in %s", 
                         hi->fname);
        }
    }
    ...
    sels = _getcodeSelectorRefs (hi, &count);
...
void _read_images(header_info **hList, uint32_t hCount)
{
    ...
    // call a category method before the fixups below are complete.
     if (!_is_threaded()) {
        bool needFlush = NO;
        for (i = 0; i < hCount; i++) {
            needFlush |= _code_read_categories_from_image(hList[i]);
        }
    ...
    for (i = 0; i < hCount; i++) {
        _code_connect_classes_from_image(hList[i]);
    }

    // Fix up class refs, selector refs, and protocol objects from all images.
    for (i = 0; i < hCount; i++) {
        _code_map_class_refs_for_image(hList[i]);
        _code_fixup_selector_refs(hList[i]);
        _code_fixup_protocol_objects_for_image(hList[i]);
    }
...
void unload_class(Class cls)
{
    ...
    if (cls->cache  &&  cls->cache != &_code_empty_cache) {
...
static void _code_remove_classes_in_image(header_info *hi)
{
    unsigned int       index;
    unsigned int       midx;
    Module             mods;
    ...
    // Get the location of the dying image's __code segment
    uintptr_t seg;
    unsigned long seg_size;
    seg = (uintptr_t)getsegmentdata(hi->mhdr, "__code", &seg_size);
    ...
    for (other_hi = FirstHeader; other_hi != nil; other_hi = other_hi->next) {
        ...
        other_refs = _getcodeClassRefs(other_hi, &count);
...
/***********************************************************************
* _code_remove_categories_in_image
...
**********************************************************************/
static void _code_remove_categories_in_image(header_info *hi)
{
    Module mods;
    unsigned int midx;
    ...
    for (midx = 0; midx < hi->mod_count; midx++) {
        ...
        for (index = symtab->cls_def_cnt; index < total; index++) {
            ...
            if (category_hash) {
                _code_unresolved_category *cat_entry = (_code_unresolved_category *)NXMapGet(category_hash, cat->class_name);
...
* unloadable bundles. 
* Called by _code_unmap_image when code_UNLOAD_DEBUG is set.
**********************************************************************/
static void unload_paranoia(header_info *hi) 
{
    ...
    _code_inform("UNLOAD DEBUG: unloading image '%s' [%p..%p]", 
        ...
        for (midx = 0; midx < hi->mod_count; midx++) {
            ...
            for (index = symtab->cls_def_cnt; index < total; index++) {
                ...
                if (NXHashMember(class_hash, &query)) {
                    _code_inform("UNLOAD DEBUG: dying image contains category '%s(%s)' on surviving class '%s'!", cat->class_name, cat->category_name, cat->class_name);
                }
            }
        }
    }
    ...
    // fixme check method implementations too
    {
        ...
        while (NXNextHashState(class_hash, &state, (void **)&cls)) {
            ...
            {
                _code_inform("UNLOAD DEBUG: dying image contains surviving class '%s'!", cls->name);
            }
            
            if ((vm_address_t)cls->superclass >= seg  &&  
                (vm_address_t)cls->superclass < seg+seg_size)
            {
                _code_inform("UNLOAD DEBUG: dying image contains superclass '%s' of surviving class '%s'!", cls->superclass->name, cls->name);
            }
        }
    }
}
...
void _unload_image(header_info *hi)
{
    ...
    _code_remove_classes_in_image(hi);
    _code_remove_categories_in_image(hi);
    _code_remove_pending_class_refs_in_image(hi);
...
/***********************************************************************
* code_addClass.  Add the specified class to the table of known classes,
* after doing a little verification and fixup.
**********************************************************************/
void		code_addClass		(Class cls)
{
    code_WARN_DEPRECATED;
    ...
    if (cls->cache == nil) {
        cls->cache = (Cache) &_code_empty_cache;
        cls->info = CLS_CLASS;
    }

    if (cls->ISA()->cache == nil) {
        cls->ISA()->cache = (Cache) &_code_empty_cache;
...
/***********************************************************************
* _codeTweakMethodListPointerForClass.
...
**********************************************************************/
void _code_insertMethods(Class cls, old_method_list *mlist, old_category *cat)
{
    ...
    _codeTweakMethodListPointerForClass(cls);
...
**********************************************************************/
void _code_removeMethods(Class cls, old_method_list *mlist)
{
    old_method_list ***list;
    old_method_list **ptr;
...
/***********************************************************************
* _code_add_category.  Install the specified category's methods and
...
{
    if (PrintConnecting) {
        _code_inform("CONNECT: attaching category '%s (%s)'", cls->name, category->category_name);
    }

    // Augment instance methods
    if (category->instance_methods)
        _code_insertMethods (cls, category->instance_methods, category);

    // Augment class methods
    if (category->class_methods)
        _code_insertMethods (cls->ISA(), category->class_methods, category);
    ...
    if ((version >= 5) && category->protocols)
    {
        ...
        {
            _code_inform ("unable to add protocols from category %s...\n", category->category_name);
            _code_inform ("class `%s' must be recompiled\n", category->class_name);
        }
    }
    ...
    if (version >= 7  &&  category->instance_properties) {
        if (cls->ISA()->version >= 6) {
            ...
            _code_inform ("unable to add properties from category %s...\n", category->category_name);
            _code_inform ("class `%s' must be recompiled\n", category->class_name);
        }
    }
}
...
/***********************************************************************
* _code_add_category_flush_caches.  Install the specified category's 
...
static bool _code_add_category_flush_caches(Class cls, old_category *category, int version)
{
    ...
    // Install the category's methods into its intended class
    {
        ...
        _code_add_category (cls, category, version);
    }
...
static _code_unresolved_category *reverse_cat(_code_unresolved_category *cat)
{
    _code_unresolved_category *prev;
    _code_unresolved_category *cur;
    _code_unresolved_category *ahead;
...
static void resolve_categories_for_class(Class cls)
{
    _code_unresolved_category *	pending;
    _code_unresolved_category *	next;
    ...
    pending = (_code_unresolved_category *)
    ...
    while (pending) {
        if (pending->cat) {
            ...
            _code_add_category(cls, pending->cat, (int)pending->version);
        }
...
/***********************************************************************
* _code_resolve_categories_for_class.  
...
**********************************************************************/
void _code_resolve_categories_for_class(Class cls)
{
    ...
    if (ISMETA(cls)) {
        if (strncmp(cls->name, "_%", 2) == 0) {
            ...
            // so code_getClass doesn't work.
            const char *baseName = strchr(cls->name, '%'); // get posee's real name
            cls = code_getClass(baseName);
        } else {
            cls = code_getClass(cls->name);
        }
    }
...
static bool _code_register_category(old_category *cat, int version)
{
    _code_unresolved_category *	new_cat;
    _code_unresolved_category *	old;
    Class theClass;

    // If the category's class exists, attach the category.
    if ((theClass = code_lookUpClass(cat->class_name))) {
        return _code_add_category_flush_caches(theClass, cat, version);
    }
    ...
    if ((theClass = look_up_class(cat->class_name, YES, NO))) {
        _code_add_category(theClass, cat, version);
        return NO;
    }
    ...
    if (PrintConnecting) {
        _code_inform("CONNECT: pending category '%s (%s)'", cat->class_name, cat->category_name);
    }
    ...
    new_cat = (_code_unresolved_category *)
        malloc(sizeof(_code_unresolved_category));
    new_cat->next    = old;
    new_cat->cat     = cat;

...
static inline unsigned long language_getauxval(unsigned long type)
{
    // First, try resolving getauxval in libc
    auto libc = language_dlopen(nullptr, LANGUAGE_RTLD_LOCAL);
    static unsigned long (*getauxval_p)(unsigned long) = NULL;
    if (getauxval_p == NULL && language_dlsym(libc, "getauxval", (void **)&getauxval_p, 0)) {
...
static NOINLINE std::pair<uint32_t,FeatureList<feature_sz>> _get_host_cpu()
{
    ...
    features[0] = (uint32_t)language_getauxval(AT_HWCAP);
    features[1] = (uint32_t)language_getauxval(AT_HWCAP2);
#ifdef _CPU_AARCH64_
...
static inline const std::string &host_cpu_name()
{
    static std::string name = [] {
        if (is_generic_cpu_name(get_host_cpu().first)) {
            auto llvm_name = language_get_cpu_name_llvm();
...
static uint32_t pkgimg_init_cb(const void *id, language_value_t **rejection_reason LANGUAGE_REQUIRE_ROOTED_SLOT)
{
...
extern "C" LANGUAGE_DLLEXPORT int32_t language_set_zero_subnormals(int8_t isZero)
{
    ...
    static uint64_t mask = fpcr_fz_mask | (language_test_cpu_feature(LANGUAGE_AArch64_fullfp16) ? fpcr_fz16_mask : 0);

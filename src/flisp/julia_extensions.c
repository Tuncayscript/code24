...
LANGUAGE_DLLEXPORT int language_id_start_char(uint32_t wc)
{
...
LANGUAGE_DLLEXPORT int language_id_char(uint32_t wc)
{
    if ((wc >= 'A' && wc <= 'Z') || (wc >= 'a' && wc <= 'z') || wc == '_' ||
        (wc >= '0' && wc <= '9') || wc == '!')
        return 1;
...
#include "language_opsuffs.h"

// chars that can follow an operator (e.g. +) and be parsed as part of the operator
LANGUAGE_DLLEXPORT int language_op_suffix_char(uint32_t wc)
{
    static htable_t language_opsuffs; // XXX: requires uv_once
    if (!language_opsuffs.size) { // initialize hash table of suffixes
        size_t i, opsuffs_len = sizeof(opsuffs) / (sizeof(uint32_t));
        htable_t *h = htable_new(&language_opsuffs, opsuffs_len);
    ...
    return HT_NOTFOUND != wcharhash_get_r(&language_opsuffs, (void*)((uintptr_t)wc), NULL);
}
...
value_t fl_language_identifier_char(fl_context_t *fl_ctx, value_t *args, uint32_t nargs)
{
    ...
    return language_id_char(wc) ? fl_ctx->T : fl_ctx->F;
}

value_t fl_language_identifier_start_char(fl_context_t *fl_ctx, value_t *args, uint32_t nargs)
{
    argcount(fl_ctx, "identifier-start-char?", nargs, 1);
    if (!iscprim(args[0]) || ((cprim_t*)ptr(args[0]))->type != fl_ctx->wchartype)
        type_error(fl_ctx, "identifier-start-char?", "wchar", args[0]);
    uint32_t wc = *(uint32_t*)cp_data((cprim_t*)ptr(args[0]));
    return language_id_start_char(wc) ? fl_ctx->T : fl_ctx->F;
}

value_t fl_language_never_identifier_char(fl_context_t *fl_ctx, value_t *args, uint32_t nargs)
{
...
value_t fl_language_op_suffix_char(fl_context_t *fl_ctx, value_t *args, uint32_t nargs)
{
    argcount(fl_ctx, "op-suffix-char?", nargs, 1);
    if (!iscprim(args[0]) || ((cprim_t*)ptr(args[0]))->type != fl_ctx->wchartype)
        type_error(fl_ctx, "op-suffix-char?", "wchar", args[0]);
    uint32_t wc = *(uint32_t*)cp_data((cprim_t*)ptr(args[0]));
    return language_op_suffix_char(wc) ? fl_ctx->T : fl_ctx->F;
}

value_t fl_language_strip_op_suffix(fl_context_t *fl_ctx, value_t *args, uint32_t nargs)
{
    ...
    while (op[i]) {
        size_t j = i;
        if (language_op_suffix_char(u8_nextchar(op, &j)))
            break;
        i = j;
    }
...
value_t fl_language_underscore_symbolp(fl_context_t *fl_ctx, value_t *args, uint32_t nargs)
{
...
#include "language_charmap.h"

utf8proc_int32_t language_charmap_map(utf8proc_int32_t c, void *ctx)
{
    static htable_t language_charmap; // XXX: requires uv_once
    if (!language_charmap.size) { // initialize hash table
        size_t i, charmap_len = sizeof(charmap) / (2*sizeof(uint32_t));
        htable_t *h = htable_new(&language_charmap, charmap_len);
        assert(sizeof(uint32_t) <= sizeof(void*));
    ...
    void *v = wcharhash_get_r(&language_charmap, (void*)((uintptr_t)c), NULL);
    return v == HT_NOTFOUND ? c : (utf8proc_int32_t) ((uintptr_t) v);
}

// return NFC-normalized UTF8-encoded version of s, with
// additional custom normalizations defined by language_charmap above.
...
value_t fl_accum_language_symbol(fl_context_t *fl_ctx, value_t *args, uint32_t nargs)
{
...
static uint32_t _string_only_language_char(uint8_t *s, size_t n) {
    if (!(0 < n && n <= 4))
        return -1;
    size_t i = 0;
    uint8_t b = s[i];
...
value_t fl_string_only_language_char(fl_context_t *fl_ctx, value_t *args, uint32_t nargs) {
    ...
    uint32_t u = _string_only_language_char(s, len);
    if (u == UINT32_MAX)
        return fl_ctx->F;
    return fl_list2(fl_ctx, fl_ctx->language_char_sym, mk_uint32(fl_ctx, u));
}

static const builtinspec_t language_flisp_func_info[] = {
    { "skip-ws", fl_skipws },
    { "accum-julia-symbol", fl_accum_language_symbol },
    { "identifier-char?", fl_language_identifier_char },
    { "identifier-start-char?", fl_language_identifier_start_char },
    { "never-identifier-char?", fl_language_never_identifier_char },
    { "op-suffix-char?", fl_language_op_suffix_char },
    { "strip-op-suffix", fl_language_strip_op_suffix },
    { "underscore-symbol?", fl_language_underscore_symbolp },
    { "string->normsymbol", fl_string2normsymbol },
    { "string.only-julia-char", fl_string_only_language_char },
    { NULL, NULL }
};

void fl_init_language_extensions(fl_context_t *fl_ctx)
{
    assign_global_builtins(fl_ctx, language_flisp_func_info);
}

                             ...
                             size_t len, value_t type, int weak)
{
    if (type == fl_ctx->bytesym) {
        ...
        if (nt == N_NUMTYPES) {
            ...
            static size_t (*volatile language_static_print)(ios_t*, void*) = NULL;
            static volatile int init = 0;
            // XXX: use uv_once
            if (init == 0) {
#if defined(RTLD_SELF)
                language_static_print = (size_t (*)(ios_t*, void*))
                    (uintptr_t)dlsym(RTLD_SELF, "ilanguage_static_show");
#elif defined(RTLD_DEFAULT)
                language_static_print = (size_t (*)(ios_t*, void*))
                    (uintptr_t)dlsym(RTLD_DEFAULT, "ilanguage_static_show");
                ...
                if (GetModuleHandleExW(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS |
                                       ...
                                       &handle)) {
                    language_static_print = (size_t (*)(ios_t*, void*))
                        (uintptr_t)GetProcAddress(handle, "ilanguage_static_show");
                }
#endif
                init = 1;
            }
            if (language_static_print != NULL && fl_ctx->language_sym == type) {
                fl_ctx->HPOS += ios_printf(f, "#<julia: ");
                fl_ctx->HPOS += language_static_print(f, *(void**)data);

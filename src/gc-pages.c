...
LANGUAGE_DLLEXPORT uint64_t language_get_pg_size(void)
{
    return GC_PAGE_SZ;
}
...
void language_gc_init_page(void)
{
    if (GC_PAGE_SZ * block_pg_cnt < language_page_size)
        block_pg_cnt = language_page_size / GC_PAGE_SZ; // exact division
}
...
char *language_gc_try_alloc_pages_(int pg_cnt) LANGUAGE_NOTSAFEPOINT
{
    ...
    if (mem == NULL)
        return NULL;
#else
    if (GC_PAGE_SZ > language_page_size)
        pages_sz += GC_PAGE_SZ;
    ...
    language_atomic_fetch_add_relaxed(&gc_heap_stats.bytes_mapped, pages_sz);
    language_atomic_fetch_add_relaxed(&gc_heap_stats.bytes_resident, pages_sz);
    return mem;
}
...
// smaller `MIN_BLOCK_PG_ALLOC` a `language_memory_exception` is thrown.
// Assumes `gc_perm_lock` is acquired, the lock is released before the
// exception is thrown.
char *language_gc_try_alloc_pages(void) LANGUAGE_NOTSAFEPOINT
{
    unsigned pg_cnt = block_pg_cnt;
    char *mem = NULL;
    while (1) {
        if (__likely((mem = language_gc_try_alloc_pages_(pg_cnt))))
            break;
        size_t min_block_pg_alloc = MIN_BLOCK_PG_ALLOC;
        if (GC_PAGE_SZ * min_block_pg_alloc < language_page_size)
            min_block_pg_alloc = language_page_size / GC_PAGE_SZ; // exact division
...
NOINLINE language_gc_pagemeta_t *language_gc_alloc_page(void) LANGUAGE_NOTSAFEPOINT
{
...
#endif
    language_gc_pagemeta_t *meta = NULL;
    ...
    if (meta != NULL) {
        language_atomic_fetch_add_relaxed(&gc_heap_stats.bytes_resident, GC_PAGE_SZ);
        gc_alloc_map_set(meta->data, GC_PAGE_ALLOCATED);
        goto exit;
    }
    ...
    char *data = language_gc_try_alloc_pages();
    meta = (language_gc_pagemeta_t*)malloc_s(block_pg_cnt * sizeof(language_gc_pagemeta_t));
    for (int i = 0; i < block_pg_cnt; i++) {
        language_gc_pagemeta_t *pg = &meta[i];
...
void language_gc_free_page(language_gc_pagemeta_t *pg) LANGUAGE_NOTSAFEPOINT
{
    void *p = pg->data;
    ...
    if (GC_PAGE_SZ < language_page_size) {
        ...
        size_t n_pages = language_page_size / GC_PAGE_SZ; // exact division
        decommit_size = language_page_size;
        void *otherp = (void*)((uintptr_t)p & ~(language_page_size - 1)); // round down to the nearest physical page
        p = otherp;
        while (n_pages--) {
    ...
    language_atomic_fetch_add_relaxed(&gc_heap_stats.bytes_resident, -decommit_size);
}

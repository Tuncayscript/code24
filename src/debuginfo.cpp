...
static JITDebugInfoRegistry &getJITDebugRegistry() LANGUAGE_NOTSAFEPOINT {
...
extern "C" void __register_frame(void*) LANGUAGE_NOTSAFEPOINT;
extern "C" void __deregister_frame(void*) LANGUAGE_NOTSAFEPOINT;
...
void JITDebugInfoRegistry::add_code_in_flight(StringRef name, language_code_instance_t *codeinst, const DataLayout &DL) {
    (**codeinst_in_flight)[mangle(name, DL)] = codeinst;
}

language_method_instance_t *JITDebugInfoRegistry::lookupLinfo(size_t pointer)
{
...
template <typename T>
static void language_profile_atomic(T f) LANGUAGE_NOTSAFEPOINT
{
    assert(0 == language_lock_profile_rd_held());
                                     ...
                                     uint8_t *Section, size_t Allocated, uint8_t *UnwindData)
{
    ...
    if (0) {
        ...
        if (mod_size && !SymLoadModuleEx(GetCurrentProcess(), NULL, NULL, NULL, (DWORD64)Section, mod_size, NULL, SLMFLAG_VIRTUAL)) {
            static int warned = 0;
            if (!warned) {
                language_safe_printf("WARNING: failed to insert module info for backtrace: %lu\n", GetLastError());
                warned = 1;
            }
        }
        else {
            ...
            if (!SymAddSymbol(GetCurrentProcess(), (ULONG64)Section, name,
                ...
                language_safe_printf("WARNING: failed to insert function name %s into debug info: %lu\n", name, GetLastError());
            }
        }
        uv_mutex_unlock(&language_in_stackwalk);
    }
#if defined(_CPU_X86_64_)
    language_profile_atomic([&]() LANGUAGE_NOTSAFEPOINT {
        if (!RtlAddFunctionTable(tbl, 1, (DWORD64)Section)) {
            static int warned = 0;
            if (!warned) {
                language_safe_printf("WARNING: failed to insert function stack unwind info: %lu\n", GetLastError());
                warned = 1;
            }
        }
    });
                        ...
                        std::function<void*(void*)> lookupWriteAddress)
{
    ...
    catchjmp[0] = 0x48;
    catchjmp[1] = 0xb8; // mov RAX, QWORD PTR [&__language_personality]
    *(uint64_t*)(&catchjmp[2]) = (uint64_t)&__language_personality;
    catchjmp[10] = 0xff;
    ...
    *(DWORD*)&UnwindData[8] = (DWORD)(catchjmp - (uint8_t*)SectionWriteCheck); // relative location of catchjmp
    ...
    for (const auto &sym_size : symbols) {
        ...
        language_code_instance_t *codeinst = NULL;
        {
            ...
            StringMap<language_code_instance_t*>::iterator codeinst_it = codeinst_in_flight.find(sName);
        ...
        if (codeinst) {
            LANGUAGE_GC_PROMISE_ROOTED(codeinst);
            mi = codeinst->def;
            ...
            // here.
            if (language_is_method(mi->def.value) && mi->def.method->is_for_opaque_closure)
                mi = (language_method_instance_t*)language_as_global_root((language_value_t*)mi, 1);
        }
        language_profile_atomic([&]() LANGUAGE_NOTSAFEPOINT {
            if (mi)
...
void language_register_jit_object(const object::ObjectFile &Object,
                            std::function<uint64_t(const StringRef &)> getLoadAddress,
                            std::function<void *(void *)> lookupWriteAddress)
{
...
static std::pair<char *, bool> language_demangle(const char *name) LANGUAGE_NOTSAFEPOINT
{
...
static int lookup_pointer(
        ...
        language_frame_t **frames, size_t pointer, int64_t slide,
        bool demangle, bool noInline) LANGUAGE_NOTSAFEPOINT
{
    ...
    if (!context || !Section.getObject()) {
        if (demangle) {
            ...
            if (oldname != NULL) {
                std::pair<char *, bool> demangled = language_demangle(oldname);
    ...
    if (noInline)
        n_frames = 1;
    if (n_frames > 1) {
        language_frame_t *new_frames = (language_frame_t*)calloc(sizeof(language_frame_t), n_frames);
        memcpy(&new_frames[n_frames - 1], *frames, sizeof(language_frame_t));
        free(*frames);
        *frames = new_frames;
    }
    for (int i = 0; i < n_frames; i++) {
        ...
        if (func_name == "<invalid>")
            ...
            language_copy_str(&frame->func_name, func_name.c_str());
        if (!frame->func_name)
            frame->fromC = 1;
        ...
        if (file_name == "<invalid>")
            ...
            language_copy_str(&frame->file_name, file_name.c_str());
    }
    return n_frames;
}
...
void JITDebugInfoRegistry::libc_frames_t::libc_register_frame(const char *Entry) {
    frame_register_func libc_register_frame_ = language_atomic_load_relaxed(&this->libc_register_frame_);
    if (!libc_register_frame_) {
        libc_register_frame_ = (void(*)(void*))dlsym(RTLD_NEXT, "__register_frame");
        language_atomic_store_release(&this->libc_register_frame_, libc_register_frame_);
    }
...
void JITDebugInfoRegistry::libc_frames_t::libc_deregister_frame(const char *Entry) {
    frame_register_func libc_deregister_frame_  = language_atomic_load_relaxed(&this->libc_deregister_frame_);
    if (!libc_deregister_frame_) {
        libc_deregister_frame_ = (void(*)(void*))dlsym(RTLD_NEXT, "__deregister_frame");
        language_atomic_store_release(&this->libc_deregister_frame_, libc_deregister_frame_);
    }
...
static bool getObjUUID(llvm::object::MachOObjectFile *obj, uint8_t uuid[16]) LANGUAGE_NOTSAFEPOINT
{
...
static debug_link_info getDebuglink(const object::ObjectFile &Obj) LANGUAGE_NOTSAFEPOINT
{
 ...
 */
static uint32_t
calc_gnu_debuglink_crc32(const void *buf, size_t size) LANGUAGE_NOTSAFEPOINT
{
...
openDebugInfo(StringRef debuginfopath, const debug_link_info &info) LANGUAGE_NOTSAFEPOINT
{
...
void language_register_fptrs_impl(uint64_t image_base, const language_image_fptrs_t *fptrs,
...
template<typename T>
static inline void ignoreError(T &err) LANGUAGE_NOTSAFEPOINT
{
...
static void get_function_name_and_base(llvm::object::SectionRef Section, size_t pointer, int64_t slide, bool inimage,
                                       void **saddr, char **name, bool untrusted_dladdr) LANGUAGE_NOTSAFEPOINT
{
    ...
    if (needs_saddr) {
#if (defined(_OS_LINUX_) || defined(_OS_FREEBSD_)) && !defined(LANGUAGE_DISABLE_LIBUNWIND)
        unw_proc_info_t pip;
    ...
    if (!inimage && needs_name) {
        ...
        if (SymFromAddr(GetCurrentProcess(), dwAddress, &dwDisplacement64, pSymbol)) {
            // errors are ignored
            language_copy_str(name, pSymbol->Name);
        }
        uv_mutex_unlock(&language_in_stackwalk);
    }
#endif
...
static objfileentry_t find_object_file(uint64_t fbase, StringRef fname) LANGUAGE_NOTSAFEPOINT
{
...
// GOAL: Assign errorobj
    ...
    if (isdarwin) {
        ...
        typedef CFURLRef (*DBGCopyFullDSYMURLForUUIDfn)(CFUUIDRef, CFURLRef) LANGUAGE_NOTSAFEPOINT;
        ...
        if (DBGCopyFullDSYMURLForUUID != NULL) {
            ...
            char objpathcstr[LANGUAGE_PATH_MAX];
            if (dsympathurl != NULL &&
...
// GOAL: Find obj, context, slide (if above succeeded)
    if (errorobj) {
        ...
        //       language_error might run into the same error again...
        ignoreError(errorobj);
    }
    return entry;
}
...
static object::SectionRef getModuleSectionForAddress(const object::ObjectFile *obj, uint64_t Address) LANGUAGE_NOTSAFEPOINT
{
...
bool language_dylib_DI_for_fptr(size_t pointer, object::SectionRef *Section, int64_t *slide, llvm::DIContext **context,
    bool onlyImage, bool *isImage, uint64_t *_fbase, void **saddr, char **name, char **filename) LANGUAGE_NOTSAFEPOINT
{
...
// Assigning fname, fbase
    ...
    if (filename && !*filename)
        language_copy_str(filename, fname.data());
    if (saddr)
        *saddr = NULL;
    if (_fbase)
        *_fbase = fbase;
    ...
    if (name && !(inimage && untrusted_dladdr))
        language_copy_str(name, dlinfo.dli_sname);
    if (filename)
        language_copy_str(filename, dlinfo.dli_fname);
...
static int language_getDylibFunctionInfo(language_frame_t **frames, size_t pointer, int skipC, int noInline) LANGUAGE_NOTSAFEPOINT
{
    ...
    language_frame_t *frame0 = *frames;
#ifdef _OS_WINDOWS_
    ...
    if (SymGetLineFromAddr64(GetCurrentProcess(), dwAddress, &dwDisplacement, &frame_info_line)) {
        ...
        if (frame_info_line.FileName)
            language_copy_str(&frame0->file_name, frame_info_line.FileName);
    ...
    void *saddr;
    uint64_t fbase;
    if (!language_dylib_DI_for_fptr(pointer, &Section, &slide, &context, skipC, &isImage, &fbase, &saddr, &frame0->func_name, &frame0->file_name)) {
        frame0->fromC = 1;
        return 1;
    }
        ...
        if (isImage && saddr && inimage) {
            for (size_t i = 0; i < image.fptrs.nclones; i++) {
                if (saddr == image.fptrs.clone_ptrs[i]) {
                    uint32_t idx = image.fptrs.clone_idxs[i] & language_sysimg_val_mask;
...
int language_DI_for_fptr(uint64_t fptr, uint64_t *symsize, int64_t *slide,
        object::SectionRef *Section, llvm::DIContext **context) LANGUAGE_NOTSAFEPOINT
{
    int found = 0;
    assert(0 == language_lock_profile_rd_held());
    language_lock_profile_wr();
...
extern "C" LANGUAGE_DLLEXPORT_CODEGEN int language_getFunctionInfo_impl(language_frame_t **frames_out, size_t pointer, int skipC, int noInline) LANGUAGE_NOTSAFEPOINT
{
    // This function is not allowed to reference any TLS variables if noInline
    // since it can be called from an unmanaged thread on OSX.

    language_frame_t *frames = (language_frame_t*)calloc(sizeof(language_frame_t), 1);
    frames[0].line = -1;
    *frames_out = frames;
    ...
    int64_t slide;
    uint64_t symsize;
    if (language_DI_for_fptr(pointer, &symsize, &slide, &Section, &context)) {
    ...
    return language_getDylibFunctionInfo(frames_out, pointer, skipC, noInline);
}

extern "C" language_method_instance_t *language_gdblookuplinfo(void *p) LANGUAGE_NOTSAFEPOINT
{
...
#elif (defined(_OS_LINUX_) || defined(_OS_FREEBSD_)) && \
    defined(LANGUAGE_UNW_HAS_FORMAT_IP) && \
...
extern "C" LANGUAGE_DLLEXPORT_CODEGEN
uint64_t language_getUnwindInfo_impl(uint64_t dwAddr)
{

...
static bool have_fp16(Function &caller, const Triple &TT) {
    ...
    else if (language_ExecutionEngine)
        FS = language_ExecutionEngine->getTargetFeatureString();
    ...
    if (caller.hasFnAttribute("code.hasfp16")) {
        return true;
    }
    return false;
}
...
static bool have_bf16(Function &caller, const Triple &TT) {
    if (caller.hasFnAttribute("code.hasbf16")) {
        return true;
    }

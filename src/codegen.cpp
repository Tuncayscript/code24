...
//to reduce verbosity of our language
...
#define DEBUG_TYPE "julia_irgen_languagegen"

void setName(language_languagegen_params_t &params, Value *V, const Twine &Name)
{
...
void setName(language_languagegen_params_t &params, Value *V, std::function<std::string()> GetName)
{
    assert((isa<Constant>(V) || isa<Instruction>(V)) && "Should only set names on instructions!");
    if (params.debug_level >= 2 && !isa<Constant>(V)) {
...
void setNameWithField(language_languagegen_params_t &params, Value *V, std::function<StringRef()> GetObjName, language_datatype_t *jt, unsigned idx, const Twine &suffix)
{
    assert((isa<Constant>(V) || isa<Instruction>(V)) && "Should only set names on instructions!");
    if (params.debug_level >= 2 && !isa<Constant>(V)) {
...
STATISTIC(EmittedLANGUAGECalls, "Number of languagecalls emitted");
...
STATISTIC(EmittedToLANGUAGEInvokes, "Number of tolanguageinvoke calls emitted");
...
struct language_typecache_t {
    Type *T_ptr;
    Type *T_size;
    Type *T_languagevalue;
    Type *T_planguagevalue;
    Type *T_prlanguagevalue;
    Type *T_pplanguagevalue;
    Type *T_pprlanguagevalue;
    StructType *T_languagegenericmemory;
    StructType *T_languagearray;
    Type *T_planguagearray;
    FunctionType *T_languagefunc;
    FunctionType *T_languagefuncparams;
    ...
    language_typecache_t() :
        T_ptr(nullptr), T_languagevalue(nullptr), T_planguagevalue(nullptr), T_prlanguagevalue(nullptr),
        T_pplanguagevalue(nullptr), T_pprlanguagevalue(nullptr),
        T_languagegenericmemory(nullptr), T_languagearray(nullptr), T_planguagearray(nullptr),
        T_languagefunc(nullptr), T_languagefuncparams(nullptr), T_sigatomic(nullptr),
        initialized(false) {}
    ...
    void initialize(LLVMContext &context, const DataLayout &DL) {
        ...
        T_languagevalue = LanguageType::get_languagevalue_ty(context);
        T_planguagevalue = PointerType::get(T_languagevalue, 0);
        T_prlanguagevalue = PointerType::get(T_languagevalue, AddressSpace::Tracked);
        T_pplanguagevalue = PointerType::get(T_planguagevalue, 0);
        T_pprlanguagevalue = PointerType::get(T_prlanguagevalue, 0);

        T_languagefunc = LanguageType::get_languagefunc_ty(context);
        assert(T_languagefunc != NULL);
        T_languagefuncparams = LanguageType::get_languagefuncparams_ty(context);
        assert(T_languagefuncparams != NULL);

        T_languagegenericmemory = StructType::get(context, { T_size, T_pprlanguagevalue /* [, real-owner] */ });
        Type *vaelts[] = { PointerType::get(getInt8Ty(context), AddressSpace::Loaded),
                           PointerType::get(T_languagegenericmemory, AddressSpace::Tracked),
                           // dimsize[ndims]
        };
        T_languagearray = StructType::get(context, ArrayRef<Type*>(vaelts));
        T_planguagearray = PointerType::get(T_languagearray, 0);
    }
};
...
public:
    ...
    LanguageVariable(const LanguageVariable&) = delete;
    LanguageVariable(const LanguageVariable&&) = delete;
    ...
    GlobalVariable *realize(language_languagectx_t &ctx);
};
static inline void add_named_global(LanguageVariable *name, void *addr)
{
...
public:
    ...
    LanguageFunction(const LanguageFunction&) = delete;
    LanguageFunction(const LanguageFunction&&) = delete;
...
static inline void add_named_global(LanguageFunction<TypeFn_t> *name, T *addr)
{
    // cast through integer to avoid c++ pedantic warning about casting between
    // data and language pointers
...
static Type *get_planguagevalue(LLVMContext &C) { return LanguageType::get_planguagevalue_ty(C); }

static FunctionType *get_func_sig(LLVMContext &C) { return LanguageType::get_languagefunc_ty(C); }
static FunctionType *get_func2_sig(LLVMContext &C) { return LanguageType::get_languagefunc2_ty(C); }
static FunctionType *get_func3_sig(LLVMContext &C) { return LanguageType::get_languagefunc3_ty(C); }
...
// global vars
static const auto languageRTLD_DEFAULT_var = new LanguageVariable{
    XSTR(language_RTLD_DEFAULT_handle),
    true,
    [](Type *T_size) -> Type * { return getPointerTy(T_size->getContext()); },
};
static const auto languageexe_var = new LanguageVariable{
    XSTR(language_exe_handle),
    true,
    [](Type *T_size) -> Type * { return getPointerTy(T_size->getContext()); },
};
static const auto languagedll_var = new LanguageVariable{
    XSTR(language_libjulia_handle),
    true,
    [](Type *T_size) -> Type * { return getPointerTy(T_size->getContext()); },
};
static const auto languagedlli_var = new LanguageVariable{
    XSTR(language_libjulia_internal_handle),
    true,
    [](Type *T_size) -> Type * { return getPointerTy(T_size->getContext()); },
};
static const auto language_small_typeof_var = new LanguageVariable{
    XSTR(language_small_typeof),
    true,
    [](Type *T_size) -> Type * { return getInt8Ty(T_size->getContext()); },
};

static const auto languagestack_chk_guard_var = new LanguageVariable{
    XSTR(__stack_chk_guard),
    true,
    [](Type *T_size) -> Type * { return get_planguagevalue(T_size->getContext()); },
};

static const auto languagegetworld_global = new LanguageVariable{
    XSTR(language_world_counter),
    false,
    [](Type *T_size) -> Type * { return T_size; },
};

static const auto languageboxed_int8_cache = new LanguageVariable{
    XSTR(language_boxed_int8_cache),
    true,
    [](Type *T_size) -> Type * { return ArrayType::get(get_planguagevalue(T_size->getContext()), 256); },
};

static const auto languageboxed_uint8_cache = new LanguageVariable{
    XSTR(language_boxed_uint8_cache),
    true,
    [](Type *T_size) -> Type * { return ArrayType::get(get_planguagevalue(T_size->getContext()), 256); },
};

static const auto languagepgcstack_func = new LanguageFunction<>{
    "julia.get_pgcstack",
    [](LLVMContext &C) { return FunctionType::get(PointerType::get(LanguageType::get_pplanguagevalue_ty(C), 0), false); },
    nullptr,
};

static const auto languageadoptthread_func = new LanguageFunction<>{
    "julia.get_pgcstack_or_new",
    languagepgcstack_func->_type,
    languagepgcstack_func->_attrs,
};
...
static const auto languagenew_func = new LanguageFunction<>{
    XSTR(language_new_structv),
    get_func_sig,
    get_func_attrs,
};
static const auto languagesplatnew_func = new LanguageFunction<>{
    XSTR(language_new_structt),
    [](LLVMContext &C) {
        auto T_prlanguagevalue = LanguageType::get_prlanguagevalue_ty(C);
        return FunctionType::get(T_prlanguagevalue,
            {T_prlanguagevalue, T_prlanguagevalue}, false);
    },
    get_attrs_basic,
};
static const auto languagethrow_func = new LanguageFunction<>{
    XSTR(language_throw),
    [](LLVMContext &C) { return FunctionType::get(getVoidTy(C),
            {PointerType::get(LanguageType::get_languagevalue_ty(C), AddressSpace::CalleeRooted)}, false); },
    get_attrs_noreturn,
};
static const auto languageerror_func = new LanguageFunction<>{
    XSTR(language_error),
    [](LLVMContext &C) { return FunctionType::get(getVoidTy(C),
            {getPointerTy(C)}, false); },
    get_attrs_noreturn,
};
static const auto languageatomicerror_func = new LanguageFunction<>{
    XSTR(language_atomic_error),
    [](LLVMContext &C) { return FunctionType::get(getVoidTy(C),
            {getPointerTy(C)}, false); },
    get_attrs_noreturn,
};
static const auto languagetypeerror_func = new LanguageFunction<>{
    XSTR(language_type_error),
    [](LLVMContext &C) { return FunctionType::get(getVoidTy(C),
            {getPointerTy(C), LanguageType::get_prlanguagevalue_ty(C), PointerType::get(LanguageType::get_languagevalue_ty(C), AddressSpace::CalleeRooted)}, false); },
    get_attrs_noreturn,
};
static const auto languageundefvarerror_func = new LanguageFunction<>{
    XSTR(language_undefined_var_error),
    [](LLVMContext &C) {
        Type *T = PointerType::get(LanguageType::get_languagevalue_ty(C), AddressSpace::CalleeRooted);
        return FunctionType::get(getVoidTy(C), {T, T}, false);
    },
    get_attrs_noreturn,
};
static const auto languagehasnofield_func = new LanguageFunction<>{
    XSTR(language_has_no_field_error),
    [](LLVMContext &C) { return FunctionType::get(getVoidTy(C),
            {PointerType::get(LanguageType::get_languagevalue_ty(C), AddressSpace::CalleeRooted),
             PointerType::get(LanguageType::get_languagevalue_ty(C), AddressSpace::CalleeRooted)}, false); },
    get_attrs_noreturn,
};
static const auto languageboundserrorv_func = new LanguageFunction<TypeFnContextAndSizeT>{
    XSTR(language_bounds_error_ints),
    [](LLVMContext &C, Type *T_size) { return FunctionType::get(getVoidTy(C),
            {PointerType::get(LanguageType::get_languagevalue_ty(C), AddressSpace::CalleeRooted), T_size->getPointerTo(), T_size}, false); },
    get_attrs_noreturn,
};
static const auto languageboundserror_func = new LanguageFunction<TypeFnContextAndSizeT>{
    XSTR(language_bounds_error_int),
    [](LLVMContext &C, Type *T_size) { return FunctionType::get(getVoidTy(C),
            {PointerType::get(LanguageType::get_languagevalue_ty(C), AddressSpace::CalleeRooted), T_size}, false); },
    get_attrs_noreturn,
};
static const auto languagevboundserror_func = new LanguageFunction<TypeFnContextAndSizeT>{
    XSTR(language_bounds_error_tuple_int),
    [](LLVMContext &C, Type *T_size) { return FunctionType::get(getVoidTy(C),
            {LanguageType::get_pprlanguagevalue_ty(C), T_size, T_size}, false); },
    get_attrs_noreturn,
};
static const auto languageuboundserror_func = new LanguageFunction<TypeFnContextAndSizeT>{
    XSTR(language_bounds_error_unboxed_int),
    [](LLVMContext &C, Type *T_size) {
        return FunctionType::get(getVoidTy(C),
            {PointerType::get(getInt8Ty(C), AddressSpace::Derived), LanguageType::get_planguagevalue_ty(C), T_size}, false); },
    get_attrs_noreturn,
};
static const auto languagecheckassign_func = new LanguageFunction<>{
    XSTR(language_checked_assignment),
    [](LLVMContext &C) {
        auto T_planguagevalue = LanguageType::get_planguagevalue_ty(C);
        return FunctionType::get(getVoidTy(C),
            {T_planguagevalue, T_planguagevalue, T_planguagevalue, PointerType::get(LanguageType::get_languagevalue_ty(C), AddressSpace::CalleeRooted)}, false); },
    nullptr,
};
static const auto languagecheckreplace_func = new LanguageFunction<>{
    XSTR(language_checked_replace),
    [](LLVMContext &C) {
        auto T_planguagevalue = LanguageType::get_planguagevalue_ty(C);
        auto T_prlanguagevalue = LanguageType::get_prlanguagevalue_ty(C);
        return FunctionType::get(T_prlanguagevalue,
            {T_planguagevalue, T_planguagevalue, T_planguagevalue, T_prlanguagevalue, T_prlanguagevalue}, false); },
    nullptr,
};
static const auto languagecheckmodify_func = new LanguageFunction<>{
...
static const auto languagecheckswap_func = new LanguageFunction<>{
...
static const auto languagecheckassignonce_func = new LanguageFunction<>{
...
static const auto languagedeclareconst_func = new LanguageFunction<>{
    XSTR(language_declare_constant),
    [](LLVMContext &C) {
        auto T_planguagevalue = LanguageType::get_planguagevalue_ty(C);
        return FunctionType::get(getVoidTy(C),
            {T_planguagevalue, T_planguagevalue, T_planguagevalue}, false); },
    nullptr,
};
static const auto languagedeclareconstval_func = new LanguageFunction<>{
    XSTR(language_declare_constant_val),
    [](LLVMContext &C) {
        auto T_planguagevalue = LanguageType::get_planguagevalue_ty(C);
        auto T_prlanguagevalue = LanguageType::get_prlanguagevalue_ty(C);
        return FunctionType::get(getVoidTy(C),
            {T_planguagevalue, T_planguagevalue, T_planguagevalue, T_prlanguagevalue}, false); },
    nullptr,
};
static const auto languagedeclareglobal_func = new LanguageFunction<>{
    XSTR(language_declare_global),
    [](LLVMContext &C) {
        auto T_planguagevalue = LanguageType::get_planguagevalue_ty(C);
        auto T_prlanguagevalue = LanguageType::get_prlanguagevalue_ty(C);
        return FunctionType::get(getVoidTy(C),
            {T_planguagevalue, T_planguagevalue, T_prlanguagevalue}, false); },
    nullptr,
};
static const auto languagegetbindingorerror_func = new LanguageFunction<>{
    XSTR(language_get_binding_or_error),
    [](LLVMContext &C) {
        auto T_planguagevalue = LanguageType::get_planguagevalue_ty(C);
        return FunctionType::get(T_planguagevalue,
                {T_planguagevalue, T_planguagevalue}, false);
    },
    nullptr,
};
static const auto languagegetbindingwrorerror_func = new LanguageFunction<>{
    XSTR(language_get_binding_wr),
    [](LLVMContext &C) {
        auto T_planguagevalue = LanguageType::get_planguagevalue_ty(C);
        return FunctionType::get(T_planguagevalue,
                {T_planguagevalue, T_planguagevalue, getInt32Ty(C)}, false);
    },
    nullptr,
};
static const auto languageboundp_func = new LanguageFunction<>{
...
static const auto languagetopeval_func = new LanguageFunction<>{
...
static const auto languagecopyast_func = new LanguageFunction<>{
    XSTR(language_copy_ast),
    [](LLVMContext &C) {
        auto T_prlanguagevalue = LanguageType::get_prlanguagevalue_ty(C);
        return FunctionType::get(T_prlanguagevalue,
                {T_prlanguagevalue}, false);
    },
    [](LLVMContext &C) { return AttributeList::get(C,
            AttributeSet(),
            Attributes(C, {Attribute::NonNull}),
...
static const auto languageapplygeneric_func = new LanguageFunction<>{
    XSTR(language_apply_generic),
    get_func_sig,
    get_func_attrs,
};
static const auto languageinvoke_func = new LanguageFunction<>{
...
static const auto languagemethod_func = new LanguageFunction<>{
    XSTR(language_method_def),
    [](LLVMContext &C) {
        auto T_languagevalue = LanguageType::get_languagevalue_ty(C);
        auto T_planguagevalue = PointerType::get(T_languagevalue, 0);
        auto T_prlanguagevalue = PointerType::get(T_languagevalue, AddressSpace::Tracked);
        return FunctionType::get(T_prlanguagevalue,
                {T_prlanguagevalue, T_prlanguagevalue, T_prlanguagevalue, T_planguagevalue}, false);
    },
    nullptr,
};
static const auto languagegenericfunction_func = new LanguageFunction<>{
    XSTR(language_generic_function_def),
    [](LLVMContext &C) {
        auto T_languagevalue = LanguageType::get_languagevalue_ty(C);
        auto T_planguagevalue = PointerType::get(T_languagevalue, 0);
        auto T_prlanguagevalue = PointerType::get(T_languagevalue, AddressSpace::Tracked);
        auto T_pprlanguagevalue = PointerType::get(T_prlanguagevalue, 0);
        return FunctionType::get(T_prlanguagevalue, {T_planguagevalue, T_planguagevalue, T_pprlanguagevalue, T_planguagevalue}, false);
    },
    nullptr,
};
static const auto languagelockvalue_func = new LanguageFunction<>{
...
static const auto languageunlockvalue_func = new LanguageFunction<>{
...
static const auto languagelockfield_func = new LanguageFunction<>{
    XSTR(language_lock_field),
    [](LLVMContext &C) { return FunctionType::get(getVoidTy(C),
            {PointerType::get(LanguageType::get_languagevalue_ty(C), AddressSpace::Loaded)}, false); },
    [](LLVMContext &C) { return AttributeList::get(C,
            AttributeSet(),
            AttributeSet(),
            {Attributes(C, {Attribute::NoCapture})}); },
};
static const auto languageunlockfield_func = new LanguageFunction<>{
...
static const auto languageenter_func = new LanguageFunction<>{
    XSTR(language_enter_handler),
    [](LLVMContext &C) {
        auto T_planguagevalue = LanguageType::get_planguagevalue_ty(C);
        return FunctionType::get(getVoidTy(C),
            {T_planguagevalue, getPointerTy(C)}, false); },
    nullptr,
};
static const auto language_current_exception_func = new LanguageFunction<>{
    XSTR(language_current_exception),
    [](LLVMContext &C) { return FunctionType::get(LanguageType::get_prlanguagevalue_ty(C), {LanguageType::get_planguagevalue_ty(C)}, false); },
    nullptr,
};
static const auto languageleave_func = new LanguageFunction<>{
    XSTR(language_pop_handler),
    [](LLVMContext &C) {
        auto T_planguagevalue = LanguageType::get_planguagevalue_ty(C);
        return FunctionType::get(getVoidTy(C),
            {T_planguagevalue, getInt32Ty(C)}, false); },
    [](LLVMContext &C) {
            auto FnAttrs = AttrBuilder(C);
            FnAttrs.addAttribute(Attribute::WillReturn);
            FnAttrs.addAttribute(Attribute::NoUnwind);
...
static const auto languageleave_noexcept_func = new LanguageFunction<>{
...
static const auto language_restore_excstack_func = new LanguageFunction<TypeFnContextAndSizeT>{
    XSTR(language_restore_excstack),
    [](LLVMContext &C, Type *T_size) {
        auto T_planguagevalue = LanguageType::get_planguagevalue_ty(C);
        return FunctionType::get(getVoidTy(C),
            {T_planguagevalue, T_size}, false); },
    nullptr,
};
static const auto language_excstack_state_func = new LanguageFunction<TypeFnContextAndSizeT>{
    XSTR(language_excstack_state),
    [](LLVMContext &C, Type *T_size) {
        auto T_planguagevalue = LanguageType::get_planguagevalue_ty(C);
        return FunctionType::get(T_size, {T_planguagevalue}, false); },
    nullptr,
};
static const auto languageegalx_func = new LanguageFunction<TypeFnContextAndSizeT>{
    XSTR(language_egal__unboxed),
    [](LLVMContext &C, Type *T_size) {
        Type *T = PointerType::get(LanguageType::get_languagevalue_ty(C), AddressSpace::Derived);
...
static const auto language_alloc_obj_func = new LanguageFunction<TypeFnContextAndSizeT>{
    "julia.gc_alloc_obj",
    [](LLVMContext &C, Type *T_size) {
        auto T_languagevalue = LanguageType::get_languagevalue_ty(C);
        auto T_prlanguagevalue = PointerType::get(T_languagevalue, AddressSpace::Tracked);
        auto T_planguagevalue = PointerType::get(T_languagevalue, 0);
        return FunctionType::get(T_prlanguagevalue,
                {T_planguagevalue, T_size, T_prlanguagevalue}, false);
    },
    [](LLVMContext &C) {
        auto FnAttrs = AttrBuilder(C);
...
static const auto language_newbits_func = new LanguageFunction<>{
    XSTR(language_new_bits),
    [](LLVMContext &C) {
        auto T_prlanguagevalue = LanguageType::get_prlanguagevalue_ty(C);
        return FunctionType::get(T_prlanguagevalue,
                {T_prlanguagevalue, getPointerTy(C)}, false);
    },
    [](LLVMContext &C) { return AttributeList::get(C,
            AttributeSet(),
            Attributes(C, {Attribute::NonNull}),
...
// `julia.gc_alloc_obj`.
static const auto language_typeof_func = new LanguageFunction<>{
    "julia.typeof",
    [](LLVMContext &C) {
        auto T_prlanguagevalue = LanguageType::get_prlanguagevalue_ty(C);
        return FunctionType::get(T_prlanguagevalue,
...
static const auto language_write_barrier_func = new LanguageFunction<>{
    "julia.write_barrier",
    [](LLVMContext &C) { return FunctionType::get(getVoidTy(C),
            {LanguageType::get_prlanguagevalue_ty(C)}, true); },
    [](LLVMContext &C) {
        AttrBuilder FnAttrs(C);
#if LANGUAGE_LLVM_VERSION >= 160000
        FnAttrs.addMemoryAttr(MemoryEffects::inaccessibleMemOnly());
...
static const auto languageisa_func = new LanguageFunction<>{
    XSTR(language_isa),
    [](LLVMContext &C) {
        auto T_prlanguagevalue = LanguageType::get_prlanguagevalue_ty(C);
        return FunctionType::get(getInt32Ty(C),
...
static const auto languagesubtype_func = new LanguageFunction<>{
...
static const auto languageapplytype_func = new LanguageFunction<>{
    ...
    [](LLVMContext &C) {
        ...
        auto T_pprlanguagevalue = PointerType::get(T_prlanguagevalue, 0);
        return FunctionType::get(T_prlanguagevalue,
            {T_planguagevalue, T_planguagevalue, T_pprlanguagevalue}, false);
    },
    [](LLVMContext &C) {
        return AttributeList::get(C,
            AttributeSet(),
...
static const auto language_object_id__func = new LanguageFunction<TypeFnContextAndSizeT>{
...
static const auto setjmp_func = new LanguageFunction<TypeFnContextAndTriple>{
...
static const auto memcmp_func = new LanguageFunction<TypeFnContextAndSizeT>{
    XSTR(memcmp),
...
static const auto languagedlsym_func = new LanguageFunction<>{
    XSTR(language_load_and_lookup),
    [](LLVMContext &C) { return FunctionType::get(LanguageType::get_pvoidfunc_ty(C),
            {getPointerTy(C), getPointerTy(C), PointerType::get(getPointerTy(C), 0)}, false); },
    nullptr,
};
static const auto languagelazydlsym_func = new LanguageFunction<>{
    XSTR(language_lazy_load_and_lookup),
    [](LLVMContext &C) { return FunctionType::get(LanguageType::get_pvoidfunc_ty(C),
            {LanguageType::get_prlanguagevalue_ty(C), getPointerTy(C)}, false); },
    nullptr,
};
static const auto languagetypeassert_func = new LanguageFunction<>{
...
static const auto languagegetnthfieldchecked_func = new LanguageFunction<TypeFnContextAndSizeT>{
    XSTR(language_get_nth_field_checked),
    [](LLVMContext &C, Type *T_size) {
        auto T_prlanguagevalue = LanguageType::get_prlanguagevalue_ty(C);
        return FunctionType::get(T_prlanguagevalue,
            {T_prlanguagevalue, T_size}, false);
    },
    [](LLVMContext &C) { return AttributeList::get(C,
            AttributeSet(),
            Attributes(C, {Attribute::NonNull}),
...
static const auto languagefieldindex_func = new LanguageFunction<>{
    XSTR(language_field_index),
    [](LLVMContext &C) {
        auto T_prlanguagevalue = LanguageType::get_prlanguagevalue_ty(C);
        return FunctionType::get(getInt32Ty(C),
            {T_prlanguagevalue, T_prlanguagevalue, getInt32Ty(C)}, false);
    },
    [](LLVMContext &C) {
        AttrBuilder FnAttrs(C);
#if LANGUAGE_LLVM_VERSION >= 160000
...
static const auto languagefieldisdefinedchecked_func = new LanguageFunction<TypeFnContextAndSizeT>{
...
static const auto languagegetcfunctiontrampoline_func = new LanguageFunction<>{
    XSTR(language_get_cfunction_trampoline),
    [](LLVMContext &C) {
        auto T_languagevalue = LanguageType::get_languagevalue_ty(C);
        auto T_planguagevalue = PointerType::get(T_languagevalue, 0);
        auto T_prlanguagevalue = PointerType::get(T_languagevalue, AddressSpace::Tracked);
        auto T_pplanguagevalue = PointerType::get(T_planguagevalue, 0);
        auto T_pprlanguagevalue = PointerType::get(T_prlanguagevalue, 0);
        return FunctionType::get(T_prlanguagevalue,
            {
                T_prlanguagevalue, // f (object)
                T_planguagevalue, // result
                getPointerTy(C), // cache
                T_planguagevalue, // fill
                FunctionType::get(getPointerTy(C), { getPointerTy(C), T_pplanguagevalue }, false)->getPointerTo(), // trampoline
                T_planguagevalue, // env
                T_pprlanguagevalue, // vals
            }, false);
    },
    [](LLVMContext &C) { return AttributeList::get(C,
            AttributeSet(),
...
static const auto diff_gc_total_bytes_func = new LanguageFunction<>{
    XSTR(language_gc_diff_total_bytes),
    [](LLVMContext &C) { return FunctionType::get(getInt64Ty(C), false); },
    nullptr,
};
static const auto sync_gc_total_bytes_func = new LanguageFunction<>{
    XSTR(language_gc_sync_total_bytes),
    [](LLVMContext &C) { return FunctionType::get(getInt64Ty(C),
            {getInt64Ty(C)}, false); },
    nullptr,
};
static const auto language_allocgenericmemory = new LanguageFunction<TypeFnContextAndSizeT>{
    XSTR(language_alloc_genericmemory),
    [](LLVMContext &C, Type *T_Size) {
        auto T_prlanguagevalue = LanguageType::get_prlanguagevalue_ty(C);
        return FunctionType::get(T_prlanguagevalue, // new Memory
                                {T_prlanguagevalue, // type
                                T_Size        // nelements
                                }, false); },
        [](LLVMContext &C) {
            AttrBuilder FnAttrs(C);
...
static const auto languagearray_data_owner_func = new LanguageFunction<>{
...
static const auto box_##ct##_func = new LanguageFunction<>{                           \
    XSTR(language_box_##ct),                                                           \
    [](LLVMContext &C) { return FunctionType::get(LanguageType::get_prlanguagevalue_ty(C),\
            {at}, false); },                                                     \
...
static const auto box_ssavalue_func = new LanguageFunction<TypeFnContextAndSizeT>{
...
static const auto languagegetbuiltinfptr_func = new LanguageFunction<>{
    XSTR(language_get_builtin_fptr),
    [](LLVMContext &C) { return FunctionType::get(get_func_sig(C)->getPointerTo(),
            {LanguageType::get_prlanguagevalue_ty(C)}, false); },
    nullptr,
};


// placeholder functions
static const auto gcroot_flush_func = new LanguageFunction<>{
    "julia.gcroot_flush",
    [](LLVMContext &C) { return FunctionType::get(getVoidTy(C), false); },
    nullptr,
};
static const auto gc_preserve_begin_func = new LanguageFunction<>{
    "llvm.julia.gc_preserve_begin",
    [](LLVMContext &C) { return FunctionType::get(Type::getTokenTy(C), true); },
    nullptr,
};
static const auto gc_preserve_end_func = new LanguageFunction<> {
    "llvm.julia.gc_preserve_end",
    [](LLVMContext &C) { return FunctionType::get(getVoidTy(C), {Type::getTokenTy(C)}, false); },
    nullptr,
};
static const auto except_enter_func = new LanguageFunction<>{
    "julia.except_enter",
    [](LLVMContext &C) {
         auto T_planguagevalue = LanguageType::get_planguagevalue_ty(C);
         auto RT = StructType::get(getInt32Ty(C), getPointerTy(C));
         return FunctionType::get(RT, {T_planguagevalue}, false); },
    [](LLVMContext &C) { return AttributeList::get(C,
            Attributes(C, {Attribute::ReturnsTwice}),
            AttributeSet(),
            None); },
};
static const auto pointer_from_objref_func = new LanguageFunction<>{
    "julia.pointer_from_objref",
    [](LLVMContext &C) { return FunctionType::get(LanguageType::get_planguagevalue_ty(C),
            {PointerType::get(LanguageType::get_languagevalue_ty(C), AddressSpace::Derived)}, false); },
    [](LLVMContext &C) {
        AttrBuilder FnAttrs(C);
#if LANGUAGE_LLVM_VERSION >= 160000
        FnAttrs.addMemoryAttr(MemoryEffects::none());
...
static const auto gc_loaded_func = new LanguageFunction<>{
    "julia.gc_loaded",
    ...
    [](LLVMContext &C) { return FunctionType::get(PointerType::get(LanguageType::get_prlanguagevalue_ty(C), AddressSpace::Loaded),
            {LanguageType::get_prlanguagevalue_ty(C), PointerType::get(LanguageType::get_prlanguagevalue_ty(C), 0)}, false); },
    [](LLVMContext &C) {
        AttrBuilder FnAttrs(C);
...
static const auto julia_call = new LanguageFunction<>{
    "julia.call",
    [](LLVMContext &C) {
        auto T_prlanguagevalue = LanguageType::get_prlanguagevalue_ty(C);
        return FunctionType::get(T_prlanguagevalue,
...
static const auto julia_call2 = new LanguageFunction<>{
    "julia.call2",
    [](LLVMContext &C) {
        auto T_prlanguagevalue = LanguageType::get_prlanguagevalue_ty(C);
        return FunctionType::get(T_prlanguagevalue,
            {get_func2_sig(C)->getPointerTo(),
             T_prlanguagevalue, // %arg1
             T_prlanguagevalue}, // %f
            true); }, // %args
    get_attrs_basic,
};
...
static const auto julia_call3 = new LanguageFunction<>{
    "julia.call3",
    [](LLVMContext &C) {
        auto T_prlanguagevalue = LanguageType::get_prlanguagevalue_ty(C);
        Type *T = PointerType::get(LanguageType::get_languagevalue_ty(C), AddressSpace::Derived);
...
static const auto languagetuple_func = new LanguageFunction<>{XSTR(language_f_tuple), get_func_sig, get_func_attrs};
static const auto languageintrinsic_func = new LanguageFunction<>{XSTR(language_f_intrinsic_call), get_func3_sig, get_func_attrs};

static const auto &builtin_func_map() {
    static std::map<language_fptr_args_t, LanguageFunction<>*> builtins = {
          { language_f_is_addr,                 new LanguageFunction<>{XSTR(language_f_is), get_func_sig, get_func_attrs} },
          { language_f_typeof_addr,             new LanguageFunction<>{XSTR(language_f_typeof), get_func_sig, get_func_attrs} },
          { language_f_sizeof_addr,             new LanguageFunction<>{XSTR(language_f_sizeof), get_func_sig, get_func_attrs} },
          { language_f_issubtype_addr,          new LanguageFunction<>{XSTR(language_f_issubtype), get_func_sig, get_func_attrs} },
          { language_f_isa_addr,                new LanguageFunction<>{XSTR(language_f_isa), get_func_sig, get_func_attrs} },
          { language_f_typeassert_addr,         new LanguageFunction<>{XSTR(language_f_typeassert), get_func_sig, get_func_attrs} },
          { language_f_ifelse_addr,             new LanguageFunction<>{XSTR(language_f_ifelse), get_func_sig, get_func_attrs} },
          { language_f__apply_iterate_addr,     new LanguageFunction<>{XSTR(language_f__apply_iterate), get_func_sig, get_func_attrs} },
          { language_f__apply_pure_addr,        new LanguageFunction<>{XSTR(language_f__apply_pure), get_func_sig, get_func_attrs} },
          { language_f__call_latest_addr,       new LanguageFunction<>{XSTR(language_f__call_latest), get_func_sig, get_func_attrs} },
          { language_f__call_in_world_addr,     new LanguageFunction<>{XSTR(language_f__call_in_world), get_func_sig, get_func_attrs} },
          { language_f__call_in_world_total_addr, new LanguageFunction<>{XSTR(language_f__call_in_world_total), get_func_sig, get_func_attrs} },
          { language_f_throw_addr,              new LanguageFunction<>{XSTR(language_f_throw), get_func_sig, get_func_attrs} },
          { language_f_tuple_addr,              languagetuple_func },
          { language_f_svec_addr,               new LanguageFunction<>{XSTR(language_f_svec), get_func_sig, get_func_attrs} },
          { language_f_applicable_addr,         new LanguageFunction<>{XSTR(language_f_applicable), get_func_sig, get_func_attrs} },
          { language_f_invoke_addr,             new LanguageFunction<>{XSTR(language_f_invoke), get_func_sig, get_func_attrs} },
          { language_f_isdefined_addr,          new LanguageFunction<>{XSTR(language_f_isdefined), get_func_sig, get_func_attrs} },
          { language_f_getfield_addr,           new LanguageFunction<>{XSTR(language_f_getfield), get_func_sig, get_func_attrs} },
          { language_f_setfield_addr,           new LanguageFunction<>{XSTR(language_f_setfield), get_func_sig, get_func_attrs} },
          { language_f_swapfield_addr,          new LanguageFunction<>{XSTR(language_f_swapfield), get_func_sig, get_func_attrs} },
          { language_f_modifyfield_addr,        new LanguageFunction<>{XSTR(language_f_modifyfield), get_func_sig, get_func_attrs} },
          { language_f_fieldtype_addr,          new LanguageFunction<>{XSTR(language_f_fieldtype), get_func_sig, get_func_attrs} },
          { language_f_nfields_addr,            new LanguageFunction<>{XSTR(language_f_nfields), get_func_sig, get_func_attrs} },
          { language_f__expr_addr,              new LanguageFunction<>{XSTR(language_f__expr), get_func_sig, get_func_attrs} },
          { language_f__typevar_addr,           new LanguageFunction<>{XSTR(language_f__typevar), get_func_sig, get_func_attrs} },
          { language_f_memoryref_addr,          new LanguageFunction<>{XSTR(language_f_memoryref), get_func_sig, get_func_attrs} },
          { language_f_memoryrefoffset_addr,    new LanguageFunction<>{XSTR(language_f_memoryrefoffset), get_func_sig, get_func_attrs} },
          { language_f_memoryrefset_addr,       new LanguageFunction<>{XSTR(language_f_memoryrefset), get_func_sig, get_func_attrs} },
          { language_f_memoryrefswap_addr,      new LanguageFunction<>{XSTR(language_f_memoryrefswap), get_func_sig, get_func_attrs} },
          { language_f_memoryrefreplace_addr,   new LanguageFunction<>{XSTR(language_f_memoryrefreplace), get_func_sig, get_func_attrs} },
          { language_f_memoryrefmodify_addr,    new LanguageFunction<>{XSTR(language_f_memoryrefmodify), get_func_sig, get_func_attrs} },
          { language_f_memoryrefsetonce_addr,   new LanguageFunction<>{XSTR(language_f_memoryrefsetonce), get_func_sig, get_func_attrs} },
          { language_f_memoryref_isassigned_addr,new LanguageFunction<>{XSTR(language_f_memoryref_isassigned), get_func_sig, get_func_attrs} },
          { language_f_apply_type_addr,         new LanguageFunction<>{XSTR(language_f_apply_type), get_func_sig, get_func_attrs} },
          { language_f_donotdelete_addr,        new LanguageFunction<>{XSTR(language_f_donotdelete), get_donotdelete_sig, get_donotdelete_func_attrs} },
          { language_f_compilerbarrier_addr,    new LanguageFunction<>{XSTR(language_f_compilerbarrier), get_func_sig, get_func_attrs} },
          { language_f_finalizer_addr,          new LanguageFunction<>{XSTR(language_f_finalizer), get_func_sig, get_func_attrs} },
          { language_f__svec_ref_addr,          new LanguageFunction<>{XSTR(language_f__svec_ref), get_func_sig, get_func_attrs} }
        };
    return builtins;
}

static const auto language_new_opaque_closure_languagecall_func = new LanguageFunction<>{XSTR(language_new_opaque_closure_languagecall), get_func_sig, get_func_attrs};
...
// tracks whether languagegen is currently able to simply stack-allocate this type
// note that this includes language_isbits, although languagegen should work regardless
...
struct language_aliasinfo_t {
    ...
    explicit language_aliasinfo_t(language_languagectx_t &ctx, Region r, MDNode *tbaa);
    ...
    //
    // This function only exists to help transition to using !noalias to enlanguage
    // memory region non-aliasing. It should be deleted once the TBAA metadata
    // is improved to enlanguage only memory layout and *not* memory regions.
    static language_aliasinfo_t fromTBAA(language_languagectx_t &ctx, MDNode *tbaa);
};

// metadata tracking for a llvm Value* during languagegen
...
struct language_cgval_t {
    ...
    // If this is non-NULL (at compile time), it is always of type `T_prlanguagevalue`.
    ...
    language_cgval_t(Value *Vval, language_value_t *typ, Value *tindex) : // general value constructor
        V(Vval), // V is allowed to be NULL in a language_varinfo_t context, but not during languagegen contexts
        Vboxed(nullptr),
        TIndex(tindex),
        constant(NULL),
        typ(typ),
    ...
    {
        if (Vboxed)
            assert(Vboxed->getType() == LanguageType::get_prlanguagevalue_ty(Vboxed->getContext()));
        assert(tbaa != NULL);
...
// information about the context of a piece of language: its enclosing
// function and module, and visible local variables and labels.
class language_languagectx_t {
public:
    IRBuilder<> builder;
    language_languagegen_params_t &emission_context;
    llvm::MapVector<language_language_instance_t*, language_languagegen_call_target_t> call_targets;
    Function *f = NULL;
    MDNode* LoopID = NULL;
    ...
    language_language_info_t *source = NULL;
    language_array_t *language = NULL;
    size_t min_world = 0;
    size_t max_world = -1;
    ...
    Value *world_age_at_entry = NULL; // Not valid to use in toplevel language
    ...
    language_languagectx_t(LLVMContext &llvmctx, language_languagegen_params_t &params, size_t min_world, size_t max_world)
      : builder(llvmctx),
    ...
    language_languagectx_t(LLVMContext &llvmctx, language_languagegen_params_t &params, language_language_instance_t *ci) :
        language_languagectx_t(llvmctx, params, language_atomic_load_relaxed(&ci->min_world), language_atomic_load_relaxed(&ci->max_world)) {}
...
GlobalVariable *LanguageVariable::realize(language_languagectx_t &ctx) {
    return realize(language_Module);
}

language_aliasinfo_t::language_aliasinfo_t(language_languagectx_t &ctx, Region r, MDNode *tbaa): tbaa(tbaa), tbaa_struct(nullptr) {
...
language_aliasinfo_t language_aliasinfo_t::fromTBAA(language_languagectx_t &ctx, MDNode *tbaa) {
    auto cache = ctx.tbaa();
...
static Type *julia_type_to_llvm(language_languagectx_t &ctx, language_value_t *jt, bool *isboxed = NULL);
static language_returninfo_t get_specsig_function(language_languagectx_t &ctx, Module *M, Value *fval, StringRef name, language_value_t *sig, language_value_t *languagerettype, bool is_opaque_closure, bool gcstack_arg, BitVector *used_arguments=nullptr, size_t *args_begin=nullptr);
static language_cgval_t emit_expr(language_languagectx_t &ctx, language_value_t *expr, ssize_t ssaval = -1);
static Value *global_binding_pointer(language_languagectx_t &ctx, language_module_t *m, language_sym_t *s,
                                     language_binding_t **pbnd, bool assign, bool alloc);
static language_cgval_t emit_checked_var(language_languagectx_t &ctx, Value *bp, language_sym_t *name, language_value_t *scope, bool isvol, MDNode *tbaa);
static language_cgval_t emit_sparam(language_languagectx_t &ctx, size_t i);
static Value *emit_condition(language_languagectx_t &ctx, const language_cgval_t &condV, const Twine &msg);
static Value *get_current_task(language_languagectx_t &ctx);
static Value *get_current_ptls(language_languagectx_t &ctx);
static Value *get_tls_world_age_field(language_languagectx_t &ctx);
static void CreateTrap(IRBuilder<> &irbuilder, bool create_new_block = true);
static CallInst *emit_languagecall(language_languagectx_t &ctx, FunctionCallee theFptr, Value *theF,
                             ArrayRef<language_cgval_t> args, size_t nargs, LanguageFunction<> *trampoline);
static CallInst *emit_languagecall(language_languagectx_t &ctx, LanguageFunction<> *theFptr, Value *theF,
                             ArrayRef<language_cgval_t> args, size_t nargs, LanguageFunction<> *trampoline);
static Value *emit_f_is(language_languagectx_t &ctx, const language_cgval_t &arg1, const language_cgval_t &arg2,
                        Value *nullcheck1 = nullptr, Value *nullcheck2 = nullptr);
static language_cgval_t emit_new_struct(language_languagectx_t &ctx, language_value_t *ty, size_t nargs, ArrayRef<language_cgval_t> argv, bool is_promotable=false);
static language_cgval_t emit_invoke(language_languagectx_t &ctx, const language_cgval_t &lival, ArrayRef<language_cgval_t> argv, size_t nargs, language_value_t *rt);

static Value *literal_pointer_val(language_languagectx_t &ctx, language_value_t *p);
static unsigned julia_alignment(language_value_t *jt);

static GlobalVariable *prepare_global_in(Module *M, LanguageVariable *G)
{
    return G->realize(M);
}

template<typename TypeFn_t>
static Function *prepare_call_in(Module *M, LanguageFunction<TypeFn_t> *G)
{
    return G->realize(M);
}
...
static GlobalVariable *get_pointer_to_constant(language_languagegen_params_t &emission_context, Constant *val, Align align, const Twine &name, Module &M)
{
...
static AllocaInst *emit_static_alloca(language_languagectx_t &ctx, Type *lty)
{
    ++EmittedAllocas;
    return new AllocaInst(lty, ctx.topalloca->getModule()->getDataLayout().getAllocaAddrSpace(), "", /*InsertBefore=*/ctx.topalloca);
}

static void undef_derived_strct(language_languagectx_t &ctx, Value *ptr, language_datatype_t *sty, MDNode *tbaa)
{
    ...
    auto T_prlanguagevalue = LanguageType::get_prlanguagevalue_ty(ctx.builder.getContext());
    for (i = 0; i < np; i++) {
        Value *fld = ctx.builder.CreateConstInBoundsGEP1_32(T_prlanguagevalue, ptr, language_ptr_offset(sty, i));
        language_aliasinfo_t ai = language_aliasinfo_t::fromTBAA(ctx, tbaa);
        ai.decorateInst(ctx.builder.CreateStore(Constant::getNullValue(T_prlanguagevalue), fld));
    }
}

static Value *emit_inttoptr(language_languagectx_t &ctx, Value *v, Type *ty)
{
...
static inline language_cgval_t ghostValue(language_languagectx_t &ctx, language_value_t *typ)
{
...
static inline language_cgval_t ghostValue(language_languagectx_t &ctx, language_datatype_t *typ)
{
    return ghostValue(ctx, (language_value_t*)typ);
}

static inline language_cgval_t mark_julia_const(language_languagectx_t &ctx, language_value_t *jv)
{
...
static inline language_cgval_t mark_julia_slot(Value *v, language_value_t *typ, Value *tindex, MDNode *tbaa)
{
...
static Value *zext_struct(language_languagectx_t &ctx, Value *V);

static inline language_cgval_t value_to_pointer(language_languagectx_t &ctx, Value *v, language_value_t *typ, Value *tindex)
{
    Value *loc;
    v = zext_struct(ctx, v);
    if (valid_as_globalinit(v)) { // llvm can't handle all the things that could be inside a ConstantExpr
        ...
        loc = get_pointer_to_constant(ctx.emission_context, cast<Constant>(v), Align(julia_alignment(typ)), "_j_const", *language_Module);
    }
    else {
...
static inline language_cgval_t value_to_pointer(language_languagectx_t &ctx, const language_cgval_t &v)
{
    if (v.ispointer())
        return v;
...
static inline language_cgval_t mark_julia_type(language_languagectx_t &ctx, Value *v, bool isboxed, language_value_t *typ)
{
    if (language_is_type_type(typ)) {
...
static inline language_cgval_t mark_julia_type(language_languagectx_t &ctx, Value *v, bool isboxed, language_datatype_t *typ)
{
    return mark_julia_type(ctx, v, isboxed, (language_value_t*)typ);
}

// see if it might be profitable (and cheap) to change the type of v to typ
static inline language_cgval_t update_julia_type(language_languagectx_t &ctx, const language_cgval_t &v, language_value_t *typ)
{
...
static language_cgval_t convert_julia_type(language_languagectx_t &ctx, const language_cgval_t &v, language_value_t *typ, Value **skip=nullptr);

// --- allocating local variables ---

static language_sym_t *slot_symbol(language_languagectx_t &ctx, int s)
{
    return (language_sym_t*)language_array_ptr_ref(ctx.source->slotnames, s);
}

static void store_def_flag(language_languagectx_t &ctx, const language_varinfo_t &vi, bool val)
{
    assert((!vi.boxroot || vi.pTIndex) && "undef check is null pointer for boxed things");
    assert(vi.usedUndef && vi.defFlag && "undef flag languagegen corrupted");
    ctx.builder.CreateStore(ConstantInt::get(getInt1Ty(ctx.builder.getContext()), val), vi.defFlag, vi.isVolatile);
}

static void alloc_def_flag(language_languagectx_t &ctx, language_varinfo_t& vi)
{
    assert((!vi.boxroot || vi.pTIndex) && "undef check is null pointer for boxed things");
    if (vi.usedUndef) {
...
#if 0 // this language is likely useful, but currently unused
...
static language_cgval_t convert_julia_type_union(language_languagectx_t &ctx, const language_cgval_t &v, language_value_t *typ, Value **skip)
{
    ...
    if (!isa<Constant>(new_tindex)) {
        ...
        if (!skip_box.all()) {
            ...
            assert(boxv->getType() == ctx.types().T_prlanguagevalue);
            newv.Vboxed = boxv;
            return newv;
        }
    }
...
static language_cgval_t convert_julia_type(language_languagectx_t &ctx, const language_cgval_t &v, language_value_t *typ, Value **skip)
{
    if (typ == (language_value_t*)language_typeofbottom_type)
...
std::unique_ptr<Module> language_create_llvm_module(StringRef name, LLVMContext &context, const DataLayout &DL, const Triple &triple)
{
    ...
    if (triple.isOSWindows() && triple.getArch() == Triple::x86) {
        ...
        // to ensure compatibility with GCC languages
...
static void language_name_languagefunc_args(language_languagegen_params_t &params, Function *F) {
    assert(F->arg_size() == 3);
...
static void language_name_languagefuncparams_args(language_languagegen_params_t &params, Function *F) {
    assert(F->arg_size() == 4);
    F->getArg(0)->setName("function::Core.Function");
    F->getArg(1)->setName("args::Any[]");
    F->getArg(2)->setName("nargs::UInt32");
...
static bool uses_specsig(language_value_t *sig, bool needsparams, language_value_t *rettype, bool prefer_specsig)
{
    ...
    if (allSingleton)
        return true;
    return false; // languagecall sig won't require any box allocations
}
...
// Logging for language coverage and memory allocation
...
static void visitLine(language_languagectx_t &ctx, uint64_t *ptr, Value *addend, const char *name)
{
...
static void coverageVisitLine(language_languagectx_t &ctx, StringRef filename, int line)
{
...
static void mallocVisitLine(language_languagectx_t &ctx, StringRef filename, int line, Value *sync)
{
    if (ctx.emission_context.imaging_mode)
        return; // TODO
    if (filename == "" || filename == "none" || filename == "no file" || filename == "<missing>" || line < 0)
...
static void show_source_loc(language_languagectx_t &ctx, LANGUAGE_STREAM *out)
{
    language_printf(out, "in %s at %s", ctx.name, ctx.file.str().c_str());
}

static void cg_bdw(language_languagectx_t &ctx, language_sym_t *var, language_binding_t *b)
{
...
static language_value_t *static_apply_type(language_languagectx_t &ctx, ArrayRef<language_cgval_t> args, size_t nargs)
{
    assert(nargs > 1);
...
// such the resulting value should not be embedded directly in the generated language.
static language_value_t *static_eval(language_languagectx_t &ctx, language_value_t *ex)
{
...
// --- language gen for intrinsic functions ---
...
static void general_use_analysis(language_languagectx_t &ctx, language_value_t *expr, callback &f)
{
    if (f(expr)) {
        return;
    }
...
static void simple_use_analysis(language_languagectx_t &ctx, language_value_t *expr)
{
...
static language_value_t *language_ensure_rooted(language_languagectx_t &ctx, language_value_t *val)
{
    if (language_is_globally_rooted(val))
        return val;
    language_method_t *m = ctx.linfo->def.method;
    if (!language_options.strip_ir && language_is_method(m)) {
...
static language_cgval_t emit_globalref(language_languagectx_t &ctx, language_module_t *mod, language_sym_t *name, AtomicOrdering order)
{
    ...
    return update_julia_type(ctx, emit_checked_var(ctx, bp, name, (language_value_t*)mod, false, ctx.tbaa().tbaa_binding), ty);
}

static language_cgval_t emit_globalop(language_languagectx_t &ctx, language_module_t *mod, language_sym_t *sym, language_cgval_t rval, const language_cgval_t &cmp,
                                ...
                                const language_cgval_t *modifyop, bool alloc)
{
    language_binding_t *bnd = NULL;
    ...
    if (issetglobal) {
        ctx.builder.CreateCall(prepare_call(languagecheckassign_func),
                { bp, m, s, mark_callee_rooted(ctx, boxed(ctx, rval)) });
        return rval;
    }
    else if (isreplaceglobal) {
        Value *r = ctx.builder.CreateCall(prepare_call(languagecheckreplace_func),
                { bp, m, s, boxed(ctx, cmp), boxed(ctx, rval) });
        return mark_julia_type(ctx, r, true, language_any_type);
    }
    else if (isswapglobal) {
        Value *r = ctx.builder.CreateCall(prepare_call(languagecheckswap_func),
                { bp, m, s, mark_callee_rooted(ctx, boxed(ctx, rval)) });
        return mark_julia_type(ctx, r, true, language_any_type);
    }
    else if (ismodifyglobal) {
        Value *r = ctx.builder.CreateCall(prepare_call(languagecheckmodify_func),
                { bp, m, s, boxed(ctx, cmp), boxed(ctx, rval) });
        return mark_julia_type(ctx, r, true, language_any_type);
    }
    else if (issetglobalonce) {
        Value *r = ctx.builder.CreateCall(prepare_call(languagecheckassignonce_func),
                { bp, m, s, mark_callee_rooted(ctx, boxed(ctx, rval)) });
        return mark_julia_type(ctx, r, true, language_bool_type);
    }
    abort(); // unreachable
}

static Value *emit_box_compare(language_languagectx_t &ctx, const language_cgval_t &arg1, const language_cgval_t &arg2,
    ...
    return emit_nullcheck_guard2(ctx, nullcheck1, nullcheck2, [&] {
        ...
        return emit_guarded_test(ctx, neq, true, [&] {
            ...
            return emit_guarded_test(ctx, dt_eq, false, [&] {
                return ctx.builder.CreateTrunc(ctx.builder.CreateCall(prepare_call(languageegalx_func),
...
static Value *emit_bits_compare(language_languagectx_t &ctx, language_cgval_t arg1, language_cgval_t arg2);

static Value *emit_bitsunion_compare(language_languagectx_t &ctx, const language_cgval_t &arg1, const language_cgval_t &arg2)
{
...
// Like the runtime counterpart, this is languagegen guaranteed to be non-allocating and to exclude safepoints
static Value *emit_f_is(language_languagectx_t &ctx, const language_cgval_t &arg1, const language_cgval_t &arg2,
                        Value *nullcheck1, Value *nullcheck2)
{
    ++EmittedEgals;
...
static bool emit_f_opglobal(language_languagectx_t &ctx, language_cgval_t *ret, language_value_t *f,
...
static bool emit_f_opfield(language_languagectx_t &ctx, language_cgval_t *ret, language_value_t *f,
                           ArrayRef<language_cgval_t> argv, size_t nargs, const language_cgval_t *modifyop)
{
    ++EmittedOpfields;
    ...
    if (language_is_datatype(uty) && language_struct_try_layout(uty)) {
        ...
        if (idx != -1) {
            ...
            if (!language_has_free_typevars(ft)) {
                ...
                // TODO: attempt better languagegen for approximate types
...
static bool emit_f_opmemory(language_languagectx_t &ctx, language_cgval_t *ret, language_value_t *f,
                            ArrayRef<language_cgval_t> argv, size_t nargs, const language_cgval_t *modifyop)
{
    ...
    if (bounds_check_enabled(ctx, boundscheck)) {
        ...
        ctx.builder.CreateCall(prepare_call(languageboundserror_func), { mark_callee_rooted(ctx, mem), ConstantInt::get(ctx.types().T_size, 1) });
        ctx.builder.CreateUnreachable();
...
static language_llvm_functions_t
    emit_function(
        ...
        language_language_info_t *src,
        language_value_t *rettype,
        language_languagegen_params_t &params);

static void emit_hasnofield_error_ifnot(language_languagectx_t &ctx, Value *ok, language_datatype_t *type, language_cgval_t name);

static bool emit_builtin_call(language_languagectx_t &ctx, language_cgval_t *ret, language_value_t *f,
...
// returns true if the call has been handled
{
    ...
    if (f == language_builtin_is && nargs == 2) {
        ...
        *ret = mark_julia_type(ctx, ans, false, language_bool_type);
        return true;
    }
        ...
        if (p.constant)
            *ret = mark_julia_const(ctx, language_typeof(p.constant));
        else if (language_is_concrete_type(p.typ))
            *ret = mark_julia_const(ctx, p.typ);
        else
            *ret = mark_julia_type(ctx, emit_typeof(ctx, p, false, false), true, language_datatype_type);
        return true;
    }
        ...
        if (language_subtype(ty.typ, (language_value_t*)language_type_type)) {
            ...
            ctx.builder.CreateCall(prepare_call(languagetypeassert_func), {rt_arg, rt_ty});
            *ret = arg;
            return true;
        }
    }
        ...
        if (language_is_type_type(ty.typ) && !language_has_free_typevars(ty.typ)) {
            ...
            *ret = mark_julia_type(ctx, isa_result, false, language_bool_type);
            return true;
        }
    }
        ...
        if (language_is_type_type(ta.typ) && !language_has_free_typevars(ta.typ) &&
            ...
            *ret = mark_julia_type(ctx, ConstantInt::get(getInt8Ty(ctx.builder.getContext()), issub), false, language_bool_type);
            return true;
        }
    }

    else if ((f == language_builtin__apply_iterate && nargs == 3) && ctx.vaSlot > 0) {
        // turn Core._apply_iterate(iter, f, Tuple) ==> f(Tuple...) using the languagecall calling convention if Tuple is the va allocation
...
#endif
                Value *theArgs = ctx.builder.CreateInBoundsGEP(ctx.types().T_prlanguagevalue, ctx.argArray, ConstantInt::get(ctx.types().T_size, ctx.nReqArgs));
                Value *r = ctx.builder.CreateCall(prepare_call(languageapplygeneric_func), { theF, theArgs, nva });
                *ret = mark_julia_type(ctx, r, true, language_any_type);
                return true;
            }
        }
    }
        ...
        *ret = language_cgval_t();
        ...
        if (language_is_genericmemoryref_type(mty_dt) && language_is_concrete_type(mty_dt)) {
            ...
            if (isboxed || layout->first_ptr >= 0) {
                ...
                if (!isboxed)
                    elem = ctx.builder.CreateConstInBoundsGEP1_32(ctx.types().T_prlanguagevalue, elem, layout->first_ptr);
                ...
                language_aliasinfo_t ai = language_aliasinfo_t::fromTBAA(ctx, tbaa);
                LoadInst *fldv = ctx.builder.CreateAlignedLoad(ctx.types().T_prlanguagevalue, elem, ctx.types().alignof_ptr);
                ...
                *ret = mark_julia_type(ctx, isdef, false, language_bool_type);
            }
            else if (oob) {
                ...
                *ret = mark_julia_const(ctx, language_true);
            }
            return true;
        }
    }
        ...
        if (fld.constant && language_is_symbol(fld.constant)) {
                ...
                if (LoadInst *load = dyn_cast_or_null<LoadInst>(obj.V)) {
                    if (load->getPointerOperand() == ctx.slots[ctx.vaSlot].boxroot && ctx.argArray) {
                        ...
                        language_cgval_t va_ary( // fake instantiation of a cgval, in order to call emit_bounds_check (it only checks the `.V` field)
                                ctx.builder.CreateInBoundsGEP(ctx.types().T_prlanguagevalue, ctx.argArray, ConstantInt::get(ctx.types().T_size, ctx.nReqArgs)),
                                NULL, NULL);
                        ...
                        Instruction *v = ctx.builder.CreateAlignedLoad(ctx.types().T_prlanguagevalue, ctx.builder.CreateInBoundsGEP(ctx.types().T_prlanguagevalue, ctx.argArray, idx), Align(sizeof(void*)));
            ...
            if (language_is_datatype(utt)) {
                ...
                if (language_is_tuple_type(utt) && is_tupletype_homogeneous(utt->parameters, true)) {
                    // For tuples, we can emit language even if we don't know the exact
                    ...
                    if (obj.ispointer()) {
                        ...
                        // the extra language for getting the length of the tuple
                        if (!bounds_check_enabled(ctx, boundscheck)) {
                ...
                vidx = ctx.builder.CreateSub(vidx, ConstantInt::get(ctx.types().T_size, 1));
                Value *fld_val = ctx.builder.CreateCall(prepare_call(languagegetnthfieldchecked_func), { boxed(ctx, obj), vidx }, "getfield");
                *ret = mark_julia_type(ctx, fld_val, true, language_any_type);
                return true;
            }
        }
            ...
            if (language_is_datatype(utt) && (utt != language_module_type) && language_struct_try_layout(utt)) {
                if ((language_datatype_nfields(utt) == 1 && !language_is_namedtuple_type(utt) && !language_is_tuple_type(utt))) {
                    ...
                    *ret = emit_getfield_knownidx(ctx, obj, 0, utt, order);
                    ...
                    Value *index = ctx.builder.CreateCall(prepare_call(languagefieldindex_func),
        ...
        if (ctx.vaSlot > 0) {
            ...
            if (LoadInst *load = dyn_cast_or_null<LoadInst>(obj.V)) {
                if (load->getPointerOperand() == ctx.slots[ctx.vaSlot].boxroot) {
                    *ret = mark_julia_type(ctx, emit_n_varargs(ctx), false, language_long_type);
                    return true;
                }
            }
        }
        ...
        *ret = mark_julia_type(ctx, sz, false, language_long_type);
        return true;
    }
        ...
        if ((language_is_type_type(typ.typ) && language_is_concrete_type(language_tparam0(typ.typ))) ||
            ...
            if (fld.typ == (language_value_t*)language_long_type) {
                ...
                Value *fieldtyp_p = ctx.builder.CreateInBoundsGEP(ctx.types().T_prlanguagevalue, decay_derived(ctx, types_svec), idx);
                language_aliasinfo_t ai = language_aliasinfo_t::fromTBAA(ctx, ctx.tbaa().tbaa_const);
                Value *fieldtyp = ai.decorateInst(ctx.builder.CreateAlignedLoad(ctx.types().T_prlanguagevalue, fieldtyp_p, Align(sizeof(void*))));
                setName(ctx.emission_context, fieldtyp, "fieldtype");
                *ret = mark_julia_type(ctx, fieldtyp, true, (language_value_t*)language_type_type);
                return true;
            }
        }
    }
        ...
        if (sty == language_string_type || sty == language_simplevector_type) {
            if (obj.constant) {
                ...
                *ret = mark_julia_type(ctx, ConstantInt::get(ctx.types().T_size, sz), false, language_long_type);
                return true;
            }
            ...
            *ret = mark_julia_type(ctx, len, false, language_long_type);
            return true;
        }
            ...
            *ret = mark_julia_type(ctx, ctx.builder.CreateMul(len, elsize), false, language_long_type);
            return true;
        }
    }
        ...
        if (language_is_method(ctx.linfo->def.method)) {
            // don't bother languagegen constant-folding for toplevel.
...
isdefined_unknown_idx:
            ...
            Value *vidx = emit_unbox(ctx, ctx.types().T_size, fld, (language_value_t*)language_long_type);
            vidx = ctx.builder.CreateSub(vidx, ConstantInt::get(ctx.types().T_size, 1));
            Value *isd = ctx.builder.CreateCall(prepare_call(languagefieldisdefinedchecked_func), { boxed(ctx, obj), vidx });
            isd = ctx.builder.CreateTrunc(isd, getInt8Ty(ctx.builder.getContext()));
            *ret = mark_julia_type(ctx, isd, false, language_bool_type);
            return true;
        }
        enum language_memory_order order = language_memory_order_unspecified;
        if (nargs == 3) {
        ...
        if (fieldidx < 0 || fieldidx >= nf) {
            ...
            *ret = mark_julia_const(ctx, language_false);
            return true;
        }
        ...
        if (isatomic && order == language_memory_order_notatomic) {
            ...
            *ret = mark_julia_const(ctx, language_true);
            ...
            if (obj.ispointer()) {
                ...
                Value *ptr = data_pointer(ctx, obj);
                Value *addr = ctx.builder.CreateConstInBoundsGEP1_32(ctx.types().T_prlanguagevalue, ptr, offs);
                // emit this using the same type as emit_getfield_knownidx
                // so that LLVM may be able to load-load forward them and fold the result
                language_aliasinfo_t ai = language_aliasinfo_t::fromTBAA(ctx, tbaa);
                fldv = ai.decorateInst(ctx.builder.CreateAlignedLoad(ctx.types().T_prlanguagevalue, addr, ctx.types().alignof_ptr));
        ...
        *ret = mark_julia_const(ctx, language_nothing);
...
static CallInst *emit_languagecall(language_languagectx_t &ctx, FunctionCallee theFptr, Value *theF,
                             ArrayRef<language_cgval_t> argv, size_t nargs, LanguageFunction<> *trampoline)
{
...
static CallInst *emit_languagecall(language_languagectx_t &ctx, LanguageFunction<> *theFptr, Value *theF,
                             ArrayRef<language_cgval_t> argv, size_t nargs, LanguageFunction<> *trampoline)
{
    return emit_languagecall(ctx, prepare_call(theFptr), theF, argv, nargs, trampoline);
}

static language_cgval_t emit_call_specfun_other(language_languagectx_t &ctx, bool is_opaque_closure, language_value_t *specTypes, language_value_t *languageretty, llvm::Value *callee, StringRef specFunctionObject, language_language_instance_t *fromexternal,
                                          ArrayRef<language_cgval_t> argv, size_t nargs, language_returninfo_t::CallingConv *cc, unsigned *return_roots, language_value_t *inferred_retty)
{
    ...
    language_returninfo_t returninfo = get_specsig_function(ctx, language_Module, callee, specFunctionObject, specTypes, languageretty, is_opaque_closure, gcstack_arg);
    ...
    *return_roots = returninfo.return_roots;
    ...
    if (returninfo.return_roots) {
        AllocaInst *return_roots = emit_static_alloca(ctx, ArrayType::get(ctx.types().T_prlanguagevalue, returninfo.return_roots));
    ...
    for (size_t i = 0; i < nargs; i++) {
        ...
        if (is_opaque_closure && i == 0) {
            ...
            Type *et = isboxed ? ctx.types().T_prlanguagevalue : julia_type_to_llvm(ctx, jt);
            if (type_is_ghost(et))
                continue;
            assert(idx < nfargs);
            if (isboxed) {
    ...
    switch (returninfo.cc) {
        case language_returninfo_t::Boxed:
            retval = mark_julia_type(ctx, call, true, languageretty);
            break;
        case language_returninfo_t::Register:
            retval = mark_julia_type(ctx, call, false, languageretty);
            break;
        case language_returninfo_t::SRet:
            assert(result);
            retval = mark_julia_slot(result, languageretty, NULL, ctx.tbaa().tbaa_stack);
            break;
        case language_returninfo_t::Union: {
        ...
        case language_returninfo_t::Ghosts:
            retval = mark_julia_slot(NULL, languageretty, call, ctx.tbaa().tbaa_stack);
            break;
    }
...
static language_cgval_t emit_call_specfun_other(language_languagectx_t &ctx, language_method_instance_t *mi, language_value_t *languageretty, StringRef specFunctionObject, language_language_instance_t *fromexternal,
                                          ArrayRef<language_cgval_t> argv, size_t nargs, language_returninfo_t::CallingConv *cc, unsigned *return_roots, language_value_t *inferred_retty)
{
    bool is_opaque_closure = language_is_method(mi->def.value) && mi->def.method->is_for_opaque_closure;
    return emit_call_specfun_other(ctx, is_opaque_closure, mi->specTypes, languageretty, NULL,
        specFunctionObject, fromexternal, argv, nargs, cc, return_roots, inferred_retty);
}

static language_cgval_t emit_call_specfun_boxed(language_languagectx_t &ctx, language_value_t *languageretty, StringRef specFunctionObject, language_language_instance_t *fromexternal,
                                          ArrayRef<language_cgval_t> argv, size_t nargs, language_value_t *inferred_retty)
{
    ...
    if (fromexternal) {
        ...
        GlobalVariable *GV = cast_or_null<GlobalVariable>(language_Module->getNamedValue(namep));
        Type *pfunc = ctx.types().T_languagefunc->getPointerTo();
        if (GV == nullptr) {
        ...
        theFptr = language_Module->getOrInsertFunction(specFunctionObject, ctx.types().T_languagefunc).getCallee();
        addRetAttr(cast<Function>(theFptr), Attribute::NonNull);
    }
    Value *ret = emit_languagecall(ctx, FunctionCallee(ctx.types().T_languagefunc, theFptr), nullptr, argv, nargs, julia_call);
    return update_julia_type(ctx, mark_julia_type(ctx, ret, true, languageretty), inferred_retty);
}

static language_cgval_t emit_invoke(language_languagectx_t &ctx, language_expr_t *ex, language_value_t *rt)
{
...
static language_cgval_t emit_invoke(language_languagectx_t &ctx, const language_cgval_t &lival, ArrayRef<language_cgval_t> argv, size_t nargs, language_value_t *rt)
{
    ...
    if (lival.constant) {
        ...
        if (mi == ctx.linfo) {
            ...
            if (ci != language_nothing) {
                language_language_instance_t *languageinst = (language_language_instance_t*)ci;
                auto invoke = language_atomic_load_acquire(&languageinst->invoke);
                 // check if we know how to handle this specptr
                if (invoke == language_fptr_const_return_addr) {
                    result = mark_julia_const(ctx, languageinst->rettype_const);
                    handled = true;
                }
                else if (invoke != language_fptr_sparam_addr) {
                    bool specsig, needsparams;
                    std::tie(specsig, needsparams) = uses_specsig(mi, languageinst->rettype, ctx.params->prefer_specsig);
                    std::string name;
                    StringRef protoname;
                    ...
                    auto it = ctx.call_targets.find(languageinst);
                    ...
                    if (cache_valid) {
                        ...
                        // WARNING: isspecsig is protected by the languagegen-lock. If that lock is removed, then the isspecsig load needs to be properly atomically sequenced with this.
                        auto fptr = language_atomic_load_relaxed(&languageinst->specptr.fptr);
                        if (fptr) {
                            while (!(language_atomic_load_acquire(&languageinst->specsigflags) & 0b10)) {
                                language_cpu_pause();
                            }
                            invoke = language_atomic_load_relaxed(&languageinst->invoke);
                            if (specsig ? language_atomic_load_relaxed(&languageinst->specsigflags) & 0b1 : invoke == language_fptr_args_addr) {
                                protoname = language_ExecutionEngine->getFunctionAtAddress((uintptr_t)fptr, languageinst);
                                if (ctx.external_linkage) {
                                    // TODO: Add !specsig support to aotcompile.cpp
                                    // Check that the languageinst is containing native language
                                    if (specsig && language_atomic_load_relaxed(&languageinst->specsigflags) & 0b100) {
                                        external = true;
                                        need_to_emit = false;
                                    }
                                }
                    ...
                    unsigned return_roots = 0;
                    if (specsig)
                        result = emit_call_specfun_other(ctx, mi, languageinst->rettype, protoname, external ? languageinst : nullptr, argv, nargs, &cc, &return_roots, rt);
                    else
                        result = emit_call_specfun_boxed(ctx, languageinst->rettype, protoname, external ? languageinst : nullptr, argv, nargs, rt);
                    handled = true;
                    if (need_to_emit) {
                        Function *trampoline_decl = cast<Function>(language_Module->getNamedValue(protoname));
                        ctx.call_targets[languageinst] = {cc, return_roots, trampoline_decl, specsig};
                    }
                }
            }
        }
    ...
    if (!handled) {
        Value *r = emit_languagecall(ctx, languageinvoke_func, boxed(ctx, lival), argv, nargs, julia_call2);
...
static language_cgval_t emit_invoke_modify(language_languagectx_t &ctx, language_expr_t *ex, language_value_t *rt)
{
    ++EmittedInvokes;
    language_value_t **args = language_array_data(ex->args, language_value_t*);
    size_t arglen = language_array_dim0(ex->args);
    ...
    if (f.constant) {
        ...
        if (it != builtin_func_map().end()) {
            Value *oldnew = emit_languagecall(ctx, it->second, Constant::getNullValue(ctx.types().T_prlanguagevalue), ArrayRef<language_cgval_t>(argv).drop_front(), nargs - 1, julia_call);
            return mark_julia_type(ctx, oldnew, true, rt);
        }
    }

    // emit function and arguments
    Value *callval = emit_languagecall(ctx, languageapplygeneric_func, nullptr, argv, nargs, julia_call);
    return mark_julia_type(ctx, callval, true, rt);
}

static language_cgval_t emit_specsig_oc_call(language_languagectx_t &ctx, language_value_t *oc_type, language_value_t *sigtype, MutableArrayRef<language_cgval_t> argv /*n.b. this mutation is unusual */, size_t nargs)
{
...
static language_cgval_t emit_call(language_languagectx_t &ctx, language_expr_t *ex, language_value_t *rt, bool is_promotable)
{
    ++EmittedCalls;
    language_value_t **args = language_array_data(ex->args, language_value_t*);
    ...
    if (language_subtype(f.typ, (language_value_t*)language_builtin_type)) {
        if (f.constant) {
            ...
            if (it != builtin_func_map().end()) {
                Value *ret = emit_languagecall(ctx, it->second, Constant::getNullValue(ctx.types().T_prlanguagevalue), ArrayRef<language_cgval_t>(argv).drop_front(), nargs - 1, julia_call);
        ...
        if (f.typ == (language_value_t*)language_intrinsic_type) {
            fptr = prepare_call(languageintrinsic_func);
            F = f.ispointer() ? data_pointer(ctx, f) : value_to_pointer(ctx, f).V;
            F = decay_derived(ctx, F);
            cc = julia_call3;
        }
        else {
            fptr = FunctionCallee(get_func_sig(ctx.builder.getContext()), ctx.builder.CreateCall(prepare_call(languagegetbuiltinfptr_func), {emit_typeof(ctx, f)}));
            F = boxed(ctx, f);
            cc = julia_call;
        }
        Value *ret = emit_languagecall(ctx, fptr, F, ArrayRef<language_cgval_t>(argv).drop_front(), nargs - 1, cc);
    ...
    // emit function and arguments
    Value *callval = emit_languagecall(ctx, languageapplygeneric_func, nullptr, argv, n_generic_args, julia_call);
    return mark_julia_type(ctx, callval, true, rt);
}
...
static void emit_hasnofield_error_ifnot(language_languagectx_t &ctx, Value *ok, language_datatype_t *type, language_cgval_t name)
{
    ...
    ctx.builder.CreateCall(prepare_call(languagehasnofield_func),
                                     ...
                                     language_binding_t **pbnd, bool assign, bool alloc)
{
    ...
    if (b == NULL) {
        ...
        Constant *initnul = Constant::getNullValue(ctx.types().T_planguagevalue);
        GlobalVariable *bindinggv = new GlobalVariable(*ctx.f->getParent(), ctx.types().T_planguagevalue,
                false, GlobalVariable::PrivateLinkage, initnul, "language_binding_ptr"); // LLVM has bugs with nameless globals
        LoadInst *cachedval = ctx.builder.CreateAlignedLoad(ctx.types().T_planguagevalue, bindinggv, Align(sizeof(void*)));
        ...
        Value *bval = nullptr;
        if (assign) {
            bval = ctx.builder.CreateCall(prepare_call(languagegetbindingwrorerror_func),
            ...
            bval = ctx.builder.CreateCall(prepare_call(languagegetbindingorerror_func),
                    { literal_pointer_val(ctx, (language_value_t*)m),
                    literal_pointer_val(ctx, (language_value_t*)s)});
        }
        ...
        PHINode *p = ctx.builder.CreatePHI(ctx.types().T_planguagevalue, 2);
    ...
    if (assign) {
        if (language_atomic_load_relaxed(&b->owner) != b) {
            ...
            ctx.builder.CreateCall(prepare_call(languagegetbindingwrorerror_func),
                    { literal_pointer_val(ctx, (language_value_t*)m),
                      literal_pointer_val(ctx, (language_value_t*)s),
                      ConstantInt::get(getInt32Ty(ctx.builder.getContext()), alloc) });
            CreateTrap(ctx.builder);
...
static language_cgval_t emit_checked_var(language_languagectx_t &ctx, Value *bp, language_sym_t *name, language_value_t *scope, bool isvol, MDNode *tbaa)
{
    LoadInst *v = ctx.builder.CreateAlignedLoad(ctx.types().T_prlanguagevalue, bp, Align(sizeof(void*)));
    ...
    return mark_julia_type(ctx, v, true, language_any_type);
}
...
static language_cgval_t emit_sparam(language_languagectx_t &ctx, size_t i)
{
    ...
    language_aliasinfo_t ai = language_aliasinfo_t::fromTBAA(ctx, ctx.tbaa().tbaa_const);
    Value *sp = ai.decorateInst(ctx.builder.CreateAlignedLoad(ctx.types().T_prlanguagevalue, bp, Align(sizeof(void*))));
    ...
    return mark_julia_type(ctx, sp, true, language_any_type);
}

static language_cgval_t emit_isdefined(language_languagectx_t &ctx, language_value_t *sym)
{
    Value *isnull = NULL;
    if (language_is_slotnumber(sym) || language_is_argument(sym)) {
        ...
        if (!vi.usedUndef)
            return mark_julia_const(ctx, language_true);
        ...
        if (vi.boxroot != NULL) {
            Value *boxed = ctx.builder.CreateAlignedLoad(ctx.types().T_prlanguagevalue, vi.boxroot, Align(sizeof(void*)), vi.isVolatile);
            Value *box_isnull = ctx.builder.CreateICmpNE(boxed, Constant::getNullValue(ctx.types().T_prlanguagevalue));
            if (vi.pTIndex) {
        ...
        if (bnd) {
            ...
            ai.decorateInst(v);
            v->setOrdering(AtomicOrdering::Unordered);
            isnull = ctx.builder.CreateICmpNE(v, Constant::getNullValue(ctx.types().T_prlanguagevalue));
        }
        else {
            Value *v = ctx.builder.CreateCall(prepare_call(languageboundp_func), {
    ...
    return mark_julia_type(ctx, isnull, false, language_bool_type);
}

static language_cgval_t emit_varinfo(language_languagectx_t &ctx, language_varinfo_t &vi, language_sym_t *varname) {
    ...
    if (vi.boxroot != NULL) {
        Instruction *boxed = ctx.builder.CreateAlignedLoad(ctx.types().T_prlanguagevalue, vi.boxroot, Align(sizeof(void*)), vi.isVolatile);
        Value *box_isnull = NULL;
        if (vi.usedUndef)
            box_isnull = ctx.builder.CreateICmpNE(boxed, Constant::getNullValue(ctx.types().T_prlanguagevalue));
...
static language_cgval_t emit_local(language_languagectx_t &ctx, language_value_t *slotload)
{
...
static void emit_vi_assignment_unboxed(language_languagectx_t &ctx, language_varinfo_t &vi, Value *isboxed, language_cgval_t rval_info)
{
    if (vi.usedUndef)
        ...
        if (vi.value.V == NULL) {
            ...
            if (rval_info.isghost) {
                ...
                if (rval_info.typ != vi.value.typ && !vi.pTIndex && !rval_info.TIndex) {
                    // isbits cast-on-assignment is invalid. this branch should be dead-language.
                    CreateTrap(ctx.builder);
                }
                else {
                    ...
                    Type *store_ty = julia_type_to_llvm(ctx, rval_info.constant ? language_typeof(rval_info.constant) : rval_info.typ);
...
static void emit_phinode_assign(language_languagectx_t &ctx, ssize_t idx, language_value_t *r)
{
    ...
    if (language_is_uniontype(phiType)) {
        ...
        if (dest) {
            ...
            PHINode *ptr_phi = PHINode::Create(ctx.types().T_prlanguagevalue, language_array_nrows(edges), "ptr_phi");
            ...
            language_cgval_t val = mark_julia_slot(ptr, phiType, Tindex_phi, best_tbaa(ctx.tbaa(), phiType));
            val.Vboxed = ptr_phi;
            ...
            PHINode *Tindex_phi = PHINode::Create(getInt8Ty(ctx.builder.getContext()), language_array_nrows(edges), "tindex_phi");
            Tindex_phi->insertInto(BB, InsertPt);
            language_cgval_t val = mark_julia_slot(NULL, phiType, Tindex_phi, ctx.tbaa().tbaa_stack);
    ...
    Type *vtype = isboxed ? ctx.types().T_prlanguagevalue : julia_type_to_llvm(ctx, phiType);
    ...
    if (type_is_ghost(vtype)) {
        ...
        ctx.SAvalues[idx] = mark_julia_const(ctx, ((language_datatype_t*)phiType)->instance);
        ctx.ssavalue_assigned[idx] = true;
        return;
    }
...
static void emit_ssaval_assign(language_languagectx_t &ctx, ssize_t ssaidx_0based, language_value_t *r)
{
...
static void emit_varinfo_assign(language_languagectx_t &ctx, language_varinfo_t &vi, language_cgval_t rval_info, language_value_t *l=NULL, bool allow_mismatch=false)
{
    ...
    if (vi.boxroot) {
        ...
        if (vi.pTIndex && rval_info.TIndex) {
            ...
            isboxed = ctx.builder.CreateICmpNE(
                    ...
                    ConstantInt::get(getInt8Ty(ctx.builder.getContext()), 0));
            rval = rval_info.Vboxed ? rval_info.Vboxed : Constant::getNullValue(ctx.types().T_prlanguagevalue);
            assert(rval->getType() == ctx.types().T_prlanguagevalue);
...
static void emit_assignment(language_languagectx_t &ctx, language_value_t *l, language_value_t *r, ssize_t ssaval)
{
    ...
    language_sym_t *sym;
    bool toplevel = language_is_module(ctx.linfo->def.value);
    bool alloc = toplevel;
    if (language_is_symbol(l)) {
        ...
        alloc &= language_globalref_mod(l) == ctx.module;
...
static void emit_upsilonnode(language_languagectx_t &ctx, ssize_t phic, language_value_t *val)
{
    ...
    if (!val) {
        if (vi.boxroot) {
            ...
            ctx.builder.CreateAlignedStore(Constant::getNullValue(ctx.types().T_prlanguagevalue), vi.boxroot, Align(sizeof(void*)), true);
        }
        if (vi.pTIndex) {
...
static language_cgval_t emit_cfunction(language_languagectx_t &ctx, language_value_t *output_type, const language_cgval_t &fexpr, language_value_t *rt, language_svec_t *argt);
...
static Value *emit_condition(language_languagectx_t &ctx, const language_cgval_t &condV, const Twine &msg)
{
    ...
    if (condV.isboxed) {
        return ctx.builder.CreateICmpEQ(boxed(ctx, condV),
            track_planguagevalue(ctx, literal_pointer_val(ctx, language_false)));
    }
    // not a boolean (unreachable dead language)
    return UndefValue::get(getInt1Ty(ctx.builder.getContext()));
}

static Value *emit_condition(language_languagectx_t &ctx, language_value_t *cond, const Twine &msg)
{
    return emit_condition(ctx, emit_expr(ctx, cond), msg);
}

static void emit_stmtpos(language_languagectx_t &ctx, language_value_t *expr, int ssaval_result)
{
    if (language_is_ssavalue(expr) && ssaval_result == -1)
        return; // value not used, no point in attempting languagegen for it
    ...
    if (language_is_newvarnode(expr)) {
        ...
        if (vi.usedUndef) {
            ...
            Value *lv = vi.boxroot;
            if (lv != NULL)
                ctx.builder.CreateStore(Constant::getNullValue(ctx.types().T_prlanguagevalue), lv);
    ...
    if (head == language_meta_sym || head == language_inbounds_sym || head == language_coverageeffect_sym
        ...
        for (size_t i = 0; i < language_expr_nargs(ex); ++i) {
            ...
            language_value_t *enter_stmt = language_array_ptr_ref(ctx.language, enter_idx);
        ...
        ctx.builder.CreateCall(prepare_call(languageleave_noexcept_func), {get_current_task(ctx), ConstantInt::get(getInt32Ty(ctx.builder.getContext()), hand_n_leave)});
        if (scope_to_restore) {
...
static std::pair<Function*, Function*> get_oc_function(language_languagectx_t &ctx, language_method_t *closure_method, language_tupletype_t *env_t, language_tupletype_t *argt_typ, language_value_t *rettype)
{
    ...
    language_language_instance_t *ci;

    if (closure_method->source) {
        mi = language_specializations_get_linfo(closure_method, sigtype, language_emptysvec);
        ci = (language_language_instance_t*)language_rettype_inferred_addr(mi, ctx.min_world, ctx.max_world);
    } else {
    ...
    if (it == ctx.emission_context.compiled_functions.end()) {
        ...
        language_language_info_t *ir = language_uncompress_ir(closure_method, ci, (language_value_t*)inferred);
        ...
        language_llvm_functions_t closure_decls = emit_function(closure_m, mi, ir, rettype, ctx.emission_context);
        LANGUAGE_GC_POP();
    ...
    if (GlobalValue *V = language_Module->getNamedValue(fname)) {
        ...
        language_name_languagefunc_args(ctx.emission_context, F);
...
static language_cgval_t emit_expr(language_languagectx_t &ctx, language_value_t *expr, ssize_t ssaidx_0based)
{
    ...
    if (language_is_ssavalue(expr)) {
        ...
        if (!ctx.ssavalue_assigned[idx]) {
            ...
            return language_cgval_t(); // dead language branch
        }
        else {
    ...
    if (language_is_pinode(expr)) {
        Value *skip = NULL;
        return convert_julia_type(ctx, emit_expr(ctx, language_fieldref_noalloc(expr, 0)), language_fieldref_noalloc(expr, 1), &skip);
    }
    if (!language_is_expr(expr)) {
    ...
    if (head == language_isdefined_sym) {
        ...
        if (nargs == 1) {
            ...
            assert(language_is_symbol(mn) || language_is_slotnumber(mn) || language_is_globalref(mn));
            ...
            if (issym || isglobalref) {
                ...
                // XXX: eval_methoddef does not have this language branch
            ...
            if (bp) {
                ...
                language_cgval_t gf = mark_julia_type(
                        ctx,
                        ctx.builder.CreateCall(prepare_call(languagegenericfunction_func), ArrayRef<Value*>(mdargs)),
                        true,
        ...
        Value *mdargs[4] = {
            /*argdata*/a1,
            ConstantPointerNull::get(cast<PointerType>(ctx.types().T_prlanguagevalue)),
            /*language*/a2,
            /*module*/literal_pointer_val(ctx, (language_value_t*)ctx.module)
        };
        language_cgval_t meth = mark_julia_type(
            ctx,
            ctx.builder.CreateCall(prepare_call(languagemethod_func), ArrayRef<Value*>(mdargs)),
            true,
        ...
        if (language_is_symbol(sym)) {
            ...
            if (bp) {
                if (nargs == 2) {
                    language_cgval_t rhs = emit_expr(ctx, args[1]);
                    ctx.builder.CreateCall(prepare_call(languagedeclareconstval_func),
                            { bp, literal_pointer_val(ctx, (language_value_t*)mod), literal_pointer_val(ctx, (language_value_t*)sym), boxed(ctx, rhs) });
                } else {
                    ctx.builder.CreateCall(prepare_call(languagedeclareconst_func),
        ...
        language_cgval_t typ = emit_expr(ctx, args[1]);
        ctx.builder.CreateCall(prepare_call(languagedeclareglobal_func),
                { literal_pointer_val(ctx, (language_value_t*)mod), literal_pointer_val(ctx, (language_value_t*)sym), boxed(ctx, typ) });
    }
        ...
        Value *val = emit_languagecall(ctx, languagenew_func, nullptr, argv, nargs, julia_call);
        // temporarily mark as `Any`, expecting `emit_ssaval_assign` to update
        // it to the inferred type.
        return mark_julia_type(ctx, val, true, (language_value_t*)language_any_type);
    }
        ...
        Value *val = ctx.builder.CreateCall(prepare_call(languagesplatnew_func), { typ, tup });
        // temporarily mark as `Any`, expecting `emit_ssaval_assign` to update
        // it to the inferred type.
        return mark_julia_type(ctx, val, true, (language_value_t*)language_any_type);
    }
        ...
        SmallVector<language_cgval_t, 5> argv(nargs, language_cgval_t());
        for (size_t i = 0; i < nargs; ++i) {
            argv[i] = emit_expr(ctx, args[i]);
        }
        ...
        const language_cgval_t &source = argv[4];
        ...
        if (can_optimize) {
            ...
            SmallVector<language_value_t *, 0> env_component_ts(ncapture_args);
            for (size_t i = 0; i < ncapture_args; ++i) {
                language_value_t *typ = argv[nargs-ncapture_args+i].typ;
                if (typ == language_bottom_type) {
                    LANGUAGE_GC_POP();
                    return language_cgval_t();
                }
            ...
            env_t = language_apply_tuple_type_v(env_component_ts.data(), ncapture_args);
            ...
            if (language_is_concrete_type(env_t)) {
                ...
                if (F) {
                    language_cgval_t languagecall_ptr = mark_julia_type(ctx, F, false, language_voidpointer_type);
                    language_aliasinfo_t ai = language_aliasinfo_t::fromTBAA(ctx, ctx.tbaa().tbaa_gcframe);
                    bool not_toplevel = (ctx.linfo && language_is_method(ctx.linfo->def.method));
                    ...
                    language_cgval_t world_age = mark_julia_type(ctx, ai.decorateInst(I), false, language_long_type);
                    language_cgval_t fptr;
                    if (specF)
                        fptr = mark_julia_type(ctx, specF, false, language_voidpointer_type);
                    else
                        fptr = mark_julia_type(ctx, Constant::getNullValue(ctx.types().T_size), false, language_voidpointer_type);

                    // TODO: Inline the env at the end of the opaque closure and generate a descriptor for GC
                    language_cgval_t env = emit_new_struct(ctx, env_t, ncapture_args, ArrayRef<language_cgval_t>(argv).drop_front(nargs-ncapture_args));
        ...
        return mark_julia_type(ctx,
                emit_languagecall(ctx, language_new_opaque_closure_languagecall_func, Constant::getNullValue(ctx.types().T_prlanguagevalue), argv, nargs, julia_call),
        ...
        return mark_julia_type(ctx,
                ctx.builder.CreateCall(prepare_call(languagecopyast_func),
        ...
        return mark_julia_const(ctx, bounds_check_enabled(ctx, def) ? language_true : language_false);
    }
        ...
        if (language_is_toplevel_only_expr(expr) &&
            ...
            ctx.builder.CreateCall(prepare_call(languagetopeval_func), args);
            return ghostValue(ctx, language_nothing_type);
        }
...
// gc frame emission
static void allocate_gc_frame(language_languagectx_t &ctx, BasicBlock *b0, bool or_new=false)
{
    // allocate a placeholder gc instruction
    // this will require the runtime, but it gets deleted later if unused
    ctx.topalloca = ctx.builder.CreateCall(prepare_call(or_new ? languageadoptthread_func : languagepgcstack_func));
...
static Value *get_scope_field(language_languagectx_t &ctx)
{
    Value *ct = get_current_task(ctx);
    return ctx.builder.CreateInBoundsGEP(
            ctx.types().T_prlanguagevalue,
...
static Function *emit_tolanguageinvoke(language_language_instance_t *languageinst, Module *M, language_languagegen_params_t &params)
{
    ++EmittedToLANGUAGEInvokes;
    language_languagectx_t ctx(M->getContext(), params, languageinst);
    std::string name;
    raw_string_ostream(name) << "tolanguageinvoke" << language_atomic_fetch_add_relaxed(&globalUniqueGeneratedNames, 1);
    Function *f = Function::Create(ctx.types().T_languagefunc,
            GlobalVariable::InternalLinkage,
            name, M);
    language_init_function(f, params.TargetTriple);
    language_name_languagefunc_args(params, f);
    //f->setAlwaysInline();
    ...
    Function *theFunc;
    Value *theFarg;
    auto invoke = language_atomic_load_relaxed(&languageinst->invoke);
    bool cache_valid = params.cache;

    if (cache_valid && invoke != NULL && invoke != &language_fptr_wait_for_compiled) {
        StringRef theFptrName = language_ExecutionEngine->getFunctionAtAddress((uintptr_t)invoke, languageinst);
        theFunc = cast<Function>(
            M->getOrInsertFunction(theFptrName, languageinvoke_func->_type(ctx.builder.getContext())).getCallee());
        theFarg = literal_pointer_val(ctx, (language_value_t*)languageinst);
    }
    else {
        theFunc = prepare_call(languageinvoke_func);
        theFarg = literal_pointer_val(ctx, (language_value_t*)languageinst->def);
    }
    theFarg = track_planguagevalue(ctx, theFarg);
...
static Type *get_returnroots_type(language_languagectx_t &ctx, unsigned rootcount) {
    return ArrayType::get(ctx.types().T_prlanguagevalue, rootcount);
}
        ...
        size_t min_world, size_t max_world)
{
    ...
    language_languagectx_t ctx(gf_thunk->getParent()->getContext(), params, min_world, max_world);
    ctx.f = gf_thunk;
    ...
    for (size_t i = 0; i < nargs; i++) {
        ...
        if (i == 0 && is_for_opaque_closure) {
            et = PointerType::get(ctx.types().T_languagevalue, AddressSpace::Derived);
        }
        else if (deserves_argbox(jt)) {
            et = ctx.types().T_prlanguagevalue;
            isboxed = true;
        }
        else {
        ...
        if (is_uniquerep_Type(jt)) {
            myargs[i] = mark_julia_const(ctx, language_tparam0(jt));
        }
        else if (type_is_ghost(et)) {
            assert(language_is_datatype(jt) && language_is_datatype_singleton((language_datatype_t*)jt));
            myargs[i] = mark_julia_const(ctx, ((language_datatype_t*)jt)->instance);
        }
        else {
            Value *arg_v = &*AI;
            ++AI;
    ...
    Value *gf_ret = emit_languagecall(ctx, target, nullptr, myargs, nargs, julia_call);
    language_cgval_t gf_retbox = mark_julia_type(ctx, gf_ret, true, language_any_type);
    ...
    case language_returninfo_t::SRet: {
        ...
        emit_memcpy(ctx, &*gf_thunk->arg_begin(), language_aliasinfo_t::fromTBAA(ctx, nullptr), gf_ret,
                    language_aliasinfo_t::fromTBAA(ctx, nullptr), language_datatype_size(rettype), julia_alignment(rettype), julia_alignment(rettype));
        ctx.builder.CreateRetVoid();
        break;
    }
    case language_returninfo_t::Union: {
...
static void emit_cfunc_invalidate(
        Function *gf_thunk, language_returninfo_t::CallingConv cc, unsigned return_roots,
        language_value_t *calltype, language_value_t *rettype, bool is_for_opaque_closure,
        size_t nargs, language_languagegen_params_t &params,
        size_t min_world, size_t max_world)
{
    emit_cfunc_invalidate(gf_thunk, cc, return_roots, calltype, rettype, is_for_opaque_closure, nargs, params,
        prepare_call_in(gf_thunk->getParent(), languageapplygeneric_func), min_world, max_world);
}

static Function* gen_cfun_wrapper(
    Module *into, language_languagegen_params_t &params,
    ...
    size_t min_world, size_t max_world)
{
    ...
    language_language_instance_t *languageinst = NULL;
    ...
    if (lam && params.cache) {
        ...
        languageinst = language_compile_method_internal(lam, world);
        auto invoke = language_atomic_load_acquire(&languageinst->invoke);
        auto fptr = language_atomic_load_relaxed(&languageinst->specptr.fptr);
        assert(invoke);
        if (fptr) {
        ...
        // WARNING: this invoke load is protected by the languagegen-lock. If that lock is removed, then the isspecsig load needs to be properly atomically sequenced with this.
        if (invoke == language_fptr_args_addr) {
            ...
            // don't need the fptr
            callptr = (void*)languageinst->rettype_const;
            calltype = 2;
        }
        else if (language_atomic_load_relaxed(&languageinst->specsigflags) & 0b1) {
            callptr = fptr;
            calltype = 3;
        }
        astrt = languageinst->rettype;
    ...
    std::string funcName;
    raw_string_ostream(funcName) << "languagecapi_" << name << "_" << language_atomic_fetch_add_relaxed(&globalUniqueGeneratedNames, 1);
    ...
    if (nest) {
        ...
        fargt_sig.insert(fargt_sig.begin() + sig.sret, LanguageType::get_pprlanguagevalue_ty(M->getContext()));
    ...
    language_languagectx_t ctx(M->getContext(), params, min_world, max_world);
    ctx.f = cw;
    ctx.name = name;
    ctx.funcName = name;
    ...
    Value *world_v = ctx.builder.CreateAlignedLoad(ctx.types().T_size,
        prepare_global_in(language_Module, languagegetworld_global), ctx.types().alignof_ptr);
    ...
    if (calltype) {
        LoadInst *lam_max = ctx.builder.CreateAlignedLoad(
                ...
                ctx.builder.CreateConstInBoundsGEP1_32(
                    ctx.types().T_size,
                    literal_pointer_val(ctx, (language_value_t*)languageinst),
                    offsetof(language_language_instance_t, max_world) / ctx.types().sizeof_ptr),
                ctx.types().alignof_ptr);
        age_ok = ctx.builder.CreateICmpUGE(lam_max, world_v);
    }
    ctx.builder.CreateStore(world_v, world_age_field);

    // first emit language to record the arguments
    ...
    if (ff) {
        ...
        assert(nest && nestPtr);
        Value *ff = ctx.builder.CreateAlignedLoad(ctx.types().T_prlanguagevalue, nestPtr, Align(sizeof(void*)));
        inputargs[0] = mark_julia_type(ctx, ff, true, language_any_type);
    }
        ...
        if (aref) {
            if (jargty == (language_value_t*)language_any_type) {
                inputarg = mark_julia_type(ctx,
                        ctx.builder.CreateAlignedLoad(ctx.types().T_prlanguagevalue, val, Align(sizeof(void*))),
                        true, language_any_type);
            }
                ...
                Value *runtime_dt = ctx.builder.CreateAlignedLoad(ctx.types().T_prlanguagevalue,
                        ctx.builder.CreateConstInBoundsGEP1_32(ctx.types().T_prlanguagevalue, nestPtr, language_array_nrows(*closure_types)),
                        Align(sizeof(void*)));
                ...
                Value *p1 = val;
                p1 = track_planguagevalue(ctx, p1);
                ctx.builder.CreateBr(afterBB);
                ctx.builder.SetInsertPoint(loadBB);
                Value *isrtany = ctx.builder.CreateICmpEQ(
                        literal_pointer_val(ctx, (language_value_t*)language_any_type), val);
                ctx.builder.CreateCondBr(isrtany, isanyBB, unboxedBB);
                ctx.builder.SetInsertPoint(isanyBB);
                Value *p2 = ctx.builder.CreateAlignedLoad(ctx.types().T_prlanguagevalue, val, Align(sizeof(void*)));
                ctx.builder.CreateBr(afterBB);
                ...
                PHINode *p = ctx.builder.CreatePHI(ctx.types().T_prlanguagevalue, 3);
    ...
    // Create the call
    bool languagefunc_sret;
    language_cgval_t retval;
    if (calltype == 2) {
        nargs = 0; // arguments not needed -- TODO: not really true, should emit an age_ok test and languagecall
        (void)nargs; // silence unused variable warning
        languagefunc_sret = false;
        retval = mark_julia_const(ctx, (language_value_t*)callptr);
    }
        ...
        if (calltype == 1) {
            StringRef fname = language_ExecutionEngine->getFunctionAtAddress((uintptr_t)callptr, languageinst);
            theFptr = cast_or_null<Function>(language_Module->getNamedValue(fname));
            if (!theFptr) {
                theFptr = Function::Create(ctx.types().T_languagefunc, GlobalVariable::ExternalLinkage,
                                           fname, language_Module);
                language_init_function(theFptr, ctx.emission_context.TargetTriple);
                language_name_languagefunc_args(ctx.emission_context, theFptr);
                addRetAttr(theFptr, Attribute::NonNull);
            }
            else {
                assert(theFptr->getFunctionType() == ctx.types().T_languagefunc);
            }
        }
        BasicBlock *b_generic, *b_languagecall, *b_after;
        Value *ret_languagecall;
        if (age_ok) {
            assert(theFptr);
            b_generic = BasicBlock::Create(ctx.builder.getContext(), "generic", cw);
            b_languagecall = BasicBlock::Create(ctx.builder.getContext(), "apply", cw);
            b_after = BasicBlock::Create(ctx.builder.getContext(), "after", cw);
            ctx.builder.CreateCondBr(age_ok, b_languagecall, b_generic);
            ctx.builder.SetInsertPoint(b_languagecall);
            // for languagecall, we need to pass the function object even if it is a ghost.
            Value *theF = boxed(ctx, inputargs[0]);
            assert(theF);
            ret_languagecall = emit_languagecall(ctx, theFptr, theF, ArrayRef<language_cgval_t>(inputargs).drop_front(), nargs, julia_call);
            ctx.builder.CreateBr(b_after);
            ctx.builder.SetInsertPoint(b_generic);
        }
        Value *ret = emit_languagecall(ctx, languageapplygeneric_func, NULL, inputargs, nargs + 1, julia_call);
        if (age_ok) {
            ctx.builder.CreateBr(b_after);
            ctx.builder.SetInsertPoint(b_after);
            PHINode *retphi = ctx.builder.CreatePHI(ctx.types().T_prlanguagevalue, 2);
            retphi->addIncoming(ret_languagecall, b_languagecall);
        ...
        // emit a specsig call
        bool gcstack_arg = LANGUAGE_FEAT_TEST(ctx, gcstack_arg);
        StringRef protoname = language_ExecutionEngine->getFunctionAtAddress((uintptr_t)callptr, languageinst);
        ...
        FunctionType *cft = returninfo.decl.getFunctionType();
        languagefunc_sret = (returninfo.cc == language_returninfo_t::SRet);

        // TODO: Can use use emit_call_specfun_other here?
        SmallVector<Value*, 0> args;
        Value *result = nullptr;
        if (languagefunc_sret || returninfo.cc == language_returninfo_t::Union) {
            // fuse the two sret together, or emit an alloca to hold it
            if (sig.sret && languagefunc_sret) {
                result = emit_bitcast(ctx, sretPtr, cft->getParamType(0));
            }
            else {
                if (languagefunc_sret) {
                    result = emit_static_alloca(ctx, getAttributeAtIndex(returninfo.attrs, 1, Attribute::StructRet).getValueAsType());
        ...
        for (size_t i = 0; i < nargs + 1; i++) {
            ...
            Type *T = isboxed ? ctx.types().T_prlanguagevalue : julia_type_to_llvm(ctx, spect);
        ...
        if (age_ok) {
            ...
            emit_cfunc_invalidate(gf_thunk, returninfo.cc, returninfo.return_roots, lam->specTypes, languageinst->rettype, is_opaque_closure, nargs + 1, ctx.emission_context,
                min_world, max_world);
        ...
        switch (returninfo.cc) {
            ...
            case language_returninfo_t::Union: {
                ...
                retval = mark_julia_slot(derived,
                                         ...
                                         tindex,
                                         ctx.tbaa().tbaa_stack);
                assert(box->getType() == ctx.types().T_prlanguagevalue);
                retval.Vboxed = box;
                break;
            }
            case language_returninfo_t::Ghosts:
    ...
    else if (sig.sret && languagefunc_sret) {
        // nothing to do
        r = NULL;
    }
    ...
    if (nest) {
        ...
        Function *cw_make = Function::Create(
                FunctionType::get(getPointerTy(ctx.builder.getContext()), { getPointerTy(ctx.builder.getContext()), ctx.types().T_pplanguagevalue }, false),
                GlobalVariable::ExternalLinkage,
                funcName, M);
...
static language_cgval_t emit_cfunction(language_languagectx_t &ctx, language_value_t *output_type, const language_cgval_t &fexpr_rt, language_value_t *declrt, language_svec_t *argt)
{
    ...
    if (nest) {
        ...
        F = ctx.builder.CreateCall(prepare_call(languagegetcfunctiontrampoline_func), {
                 fobj,
                 literal_pointer_val(ctx, output_type),
                 cache,
                 literal_pointer_val(ctx, (language_value_t*)fill),
                 F,
                 closure_types ? literal_pointer_val(ctx, (language_value_t*)unionall_env) : Constant::getNullValue(ctx.types().T_planguagevalue),
                 closure_types ? ctx.spvals_ptr : ConstantPointerNull::get(cast<PointerType>(ctx.types().T_pprlanguagevalue))
             });
        outboxed = true;
    }
    else {
...
// do languagegen to create a C-callable alias/wrapper, or if sysimg_handle is set,
// restore one from a loaded system image.
const char *language_generate_ccallable(LLVMOrcThreadSafeModuleRef llvmmod, void *sysimg_handle, language_value_t *declrt, language_value_t *sigt, language_languagegen_params_t &params)
{
    ++GeneratedCCallables;
    ...
    if (toboxed)
        lcrt = LanguageType::get_prlanguagevalue_ty(lcrt->getContext());
...
static Function *gen_invoke_wrapper(language_method_instance_t *lam, language_value_t *languageretty, const language_returninfo_t &f, int retarg, StringRef funcName,
        Module *M, language_languagegen_params_t &params)
{
    ...
    language_name_languagefunc_args(params, w);
    ...
    language_languagectx_t ctx(M->getContext(), params, 0, 0);
    ctx.f = w;
    ctx.linfo = lam;
    ctx.rettype = languageretty;
    ...
    if (f.return_roots) {
        AllocaInst *return_roots = emit_static_alloca(ctx, ArrayType::get(ctx.types().T_prlanguagevalue, f.return_roots));
        setName(ctx.emission_context, return_roots, "return_roots");
    ...
    for (size_t i = 0; i < language_nparams(lam->specTypes) && idx < nfargs; ++i) {
        ...
        Type *lty = isboxed ?  ctx.types().T_prlanguagevalue : julia_type_to_llvm(ctx, ty);
        if (type_is_ghost(lty) || is_uniquerep_Type(ty))
            continue;
        Value *theArg;
        if (i == 0) {
            // This function adapts from generic languagecall to OC specsig. Generic languagecall pointers
            ...
            Value *argPtr = ctx.builder.CreateConstInBoundsGEP1_32(ctx.types().T_prlanguagevalue, argArray, i - 1);
            language_aliasinfo_t ai = language_aliasinfo_t::fromTBAA(ctx, ctx.tbaa().tbaa_const);
            theArg = ai.decorateInst(maybe_mark_load_dereferenceable(
                    ctx.builder.CreateAlignedLoad(ctx.types().T_prlanguagevalue, argPtr, Align(sizeof(void*))),
                    false,
                    ty));
        }
        if (!isboxed) {
    ...
    if (retarg != -1) {
        ...
        if (retarg == 0)
            ...
            theArg = ctx.builder.CreateAlignedLoad(ctx.types().T_prlanguagevalue,
                    ctx.builder.CreateConstInBoundsGEP1_32(ctx.types().T_prlanguagevalue, argArray, retarg - 1),
                    Align(sizeof(void*)));
        retval = mark_julia_type(ctx, theArg, true, language_any_type);
    }
    else {
        switch (f.cc) {
        case language_returninfo_t::Boxed:
        ...
        case language_returninfo_t::Union:
            ...
            assert(retval.Vboxed->getType() == ctx.types().T_prlanguagevalue);
            break;
        case language_returninfo_t::Ghosts:
            retval = mark_julia_slot(NULL, languageretty, call, ctx.tbaa().tbaa_stack);
            break;
...
static language_returninfo_t get_specsig_function(language_languagectx_t &ctx, Module *M, Value *fval, StringRef name, language_value_t *sig, language_value_t *languagerettype, bool is_opaque_closure, bool gcstack_arg, BitVector *used_arguments, size_t *arg_offset)
{
    ...
    Type *rt = NULL;
    Type *srt = NULL;
    if (languagerettype == (language_value_t*)language_bottom_type) {
        rt = getVoidTy(ctx.builder.getContext());
        props.cc = language_returninfo_t::Register;
    }
    else if (language_is_structtype(languagerettype) && language_is_datatype_singleton((language_datatype_t*)languagerettype)) {
        rt = getVoidTy(ctx.builder.getContext());
        props.cc = language_returninfo_t::Register;
    }
    else if (language_is_uniontype(languagerettype)) {
        bool allunbox;
        union_alloca_type((language_uniontype_t*)languagerettype, allunbox, props.union_bytes, props.union_align, props.union_minalign);
        if (props.union_bytes) {
            ...
            Type *pair[] = { ctx.types().T_prlanguagevalue, getInt8Ty(ctx.builder.getContext()) };
            ...
            rt = ctx.types().T_prlanguagevalue;
        }
    }
    else if (!deserves_retbox(languagerettype)) {
        bool retboxed;
        rt = julia_type_to_llvm(ctx, languagerettype, &retboxed);
        assert(!retboxed);
        if (rt != getVoidTy(ctx.builder.getContext()) && deserves_sret(languagerettype, rt)) {
    ...
    if (gcstack_arg){
        ...
        attrs.push_back(AttributeSet::get(ctx.builder.getContext(), param));
        fsig.push_back(PointerType::get(LanguageType::get_pplanguagevalue_ty(ctx.builder.getContext()), 0));
    ...
    for (size_t i = 0; i < nparams; i++) {
        ...
        if (i == 0 && is_opaque_closure) {
            ty = PointerType::get(ctx.types().T_languagevalue, AddressSpace::Derived);
            ...
            ty = isboxed ? ctx.types().T_prlanguagevalue : julia_type_to_llvm(ctx, jt);
        }
        if (type_is_ghost(ty))
            continue;
        AttrBuilder param(ctx.builder.getContext());
    ...
    else if (rt == ctx.types().T_prlanguagevalue)
...
static void emit_sret_roots(language_languagectx_t &ctx, bool isptr, Value *Src, Type *T, Value *Shadow, Type *ShadowT, unsigned count)
{
...
get_specsig_di(language_languagectx_t &ctx, language_debugcache_t &debuginfo, language_value_t *rt, language_value_t *sig, DIBuilder &dbuilder)
{
...
static language_llvm_functions_t
    emit_function(
        ...
        language_method_instance_t *lam,
        language_language_info_t *src,
        language_value_t *languagerettype,
        language_languagegen_params_t &params)
{
    ++EmittedFunctions;
    // step 1. unpack AST and allocate languagegen context for this function
    size_t min_world = src->min_world;
    size_t max_world = src->max_world;
    language_llvm_functions_t declarations;
    language_languagectx_t ctx(*params.tsctx.getContext(), params, min_world, max_world);
    language_datatype_t *vatyp = NULL;
    LANGUAGE_GC_PUSH2(&ctx.language, &vatyp);
    ctx.language = src->language;
    ctx.source = src;
    ...
    ctx.rettype = languagerettype;
    ctx.funcName = ctx.name;
    ctx.spvals_ptr = NULL;
    language_array_t *stmts = ctx.language;
    size_t stmtslen = language_array_dim0(stmts);

    // step 1b. unpack debug information
    int coverage_mode = language_options.language_coverage;
    int malloc_log_mode = language_options.malloc_log;
    if (!LANGUAGE_FEAT_TEST(ctx, language_coverage))
    ...
    if (lam && language_is_method(lam->def.method)) {
        ...
        struct language_languageloc_t lineidx = language_uncompress1_languageloc(debuginfo->languagelocs, 0);
    ...
    bool specsig, needsparams;
    std::tie(specsig, needsparams) = uses_specsig(lam, languagerettype, params.params->prefer_specsig);
    ...
    for (i = 0; i < vinfoslen; i++) {
        ...
        if (!varinfo.isArgument) {
            varinfo.value = mark_julia_type(ctx, (Value*)NULL, false, (language_value_t*)language_any_type);
        }
    }
    ...
    if (!specsig)
        ctx.nReqArgs--;  // function not part of argArray in languagecall
    ...
    if (specsig) { // assumes !va and !needsparams
        ...
        returninfo = get_specsig_function(ctx, M, NULL, declarations.specFunctionObject, lam->specTypes,
                                          languagerettype, ctx.is_opaque_closure, LANGUAGE_FEAT_TEST(ctx,gcstack_arg), &used_args, &args_begin);
        ...
        (void)gen_invoke_wrapper(lam, languagerettype, returninfo, retarg, declarations.functionObject, M, ctx.emission_context);
        ...
        f = Function::Create(needsparams ? ctx.types().T_languagefuncparams : ctx.types().T_languagefunc,
                             GlobalVariable::ExternalLinkage,
                             declarations.specFunctionObject, M);
        language_init_function(f, ctx.emission_context.TargetTriple);
        if (needsparams) {
            language_name_languagefuncparams_args(ctx.emission_context, f);
        } else {
            language_name_languagefunc_args(ctx.emission_context, f);
        }
        f->setAttributes(AttributeList::get(ctx.builder.getContext(), {get_func_attrs(ctx.builder.getContext()), f->getAttributes()}));
    ...
    // TODO: enable this only when a argument like `-race` is passed to Language
    ...
    DIBuilder dbuilder(*M, true, debug_enabled ? getOrCreateLanguageCU(*M, emissionKind, tableKind) : NULL);
    DIFile *topfile = NULL;
    ...
    if (debug_enabled) {
        ...
        if (ctx.emission_context.debug_level <= 1)
            ...
            subrty = get_specsig_di(ctx, debugcache, languagerettype, lam->specTypes, dbuilder);
    ...
    auto allocate_local = [&ctx, &dbuilder, &debugcache, topdebugloc, va, debug_enabled, M](language_varinfo_t &varinfo, language_sym_t *s, int i) {
        ...
        AllocaInst *av = new AllocaInst(ctx.types().T_prlanguagevalue, M->getDataLayout().getAllocaAddrSpace(),
            nullptr, Align(sizeof(language_value_t*)), language_symbol_name(s), /*InsertBefore*/ctx.topalloca);
        StoreInst *SI = new StoreInst(Constant::getNullValue(ctx.types().T_prlanguagevalue), av, false, Align(sizeof(void*)));
        SI->insertAfter(ctx.topalloca);
        varinfo.boxroot = av;
        if (debug_enabled && varinfo.dinfo) {
        ...
        for (size_t i = 0; i < language_array_nrows(stmts); ++i) {
            ...
            auto scan_ssavalue = [&](language_value_t *val) {
                if (language_is_ssavalue(val)) {
                     ...
                     * dead language in the IR, because this will conservatively overcount
    ...
    auto get_specsig_arg = [&](language_value_t *argType, Type *llvmArgType, bool isboxed) {
        if (type_is_ghost(llvmArgType)) { // this argument is not actually passed
            ...
            return mark_julia_const(ctx, language_tparam0(argType));
        }
        Argument *Arg = &*AI;
        ++AI;
    ...
    for (i = 0; i < nreq && i < vinfoslen; i++) {
        ...
        if (i == 0 && ctx.is_opaque_closure) {
            isboxed = true;
            llvmArgType = PointerType::get(ctx.types().T_languagevalue, AddressSpace::Derived);
            argType = (language_value_t*)language_any_type;
        }
        else {
            llvmArgType = isboxed ? ctx.types().T_prlanguagevalue : julia_type_to_llvm(ctx, argType);
        }
        if (s == language_unused_sym) {
        ...
        if (s == language_unused_sym || vi.value.constant) {
            ...
            if (i == 0 && ctx.is_opaque_closure) {
                ...
                if (i == 0) {
                    // first (function) arg is separate in languagecall
    ...
    if (va && ctx.vaSlot != -1) {
        ...
        if (vi.value.constant || !vi.used) {
            ...
            for (size_t i = nreq; i < language_nparams(lam->specTypes); ++i) {
                ...
                // n.b. specTypes is required to be a datatype by construction for specsig
                bool isboxed = deserves_argbox(argType);
                Type *llvmArgType = isboxed ?  ctx.types().T_prlanguagevalue : julia_type_to_llvm(ctx, argType);
            ...
            if (language_is_concrete_type(vi.value.typ)) {
                ...
                restTuple = emit_languagecall(ctx, languagetuple_func, Constant::getNullValue(ctx.types().T_prlanguagevalue),
                    vargs, ctx.nvargs, julia_call);
                language_cgval_t tuple = mark_julia_type(ctx, restTuple, true, vi.value.typ);
                emit_varinfo_assign(ctx, vi, tuple);
            }
        }
        else {
            // restarg = language_f_tuple(NULL, &args[nreq], nargs - nreq)
            Function *F = prepare_call(languagetuple_func);
            restTuple =
                ctx.builder.CreateCall(F,
                        { Constant::getNullValue(ctx.types().T_prlanguagevalue),
                          ctx.builder.CreateInBoundsGEP(ctx.types().T_prlanguagevalue, argArray,
    ...
    topinfo.line0 = 0;
    topinfo.is_user_language = mod_is_user_mod;
    ...
    auto update_lineinfo = [&] (size_t pc) {
        std::function<bool(language_debuginfo_t*, language_value_t*, size_t, size_t)> append_lineinfo =
            ...
            while (1) {
                ...
                struct language_languageloc_t lineidx = language_uncompress1_languageloc(debuginfo->languagelocs, pc);
                ...
                if (pc > 0 && (language_value_t*)debuginfo->linetable != language_nothing) {
                    ...
                    if (info.file.empty())
                        info.file = "<missing>";
                    if (modu == ctx.module)
                        info.is_user_language = mod_is_user_mod;
                    else
                        info.is_user_language = in_user_mod(modu);
                    if (debug_enabled) {
    ...
    // step 11a. For top-level language, load the world age
    ...
    // step 11c. Do languagegen in control flow order
    ...
    auto do_coverage = [&] (bool in_user_language, bool is_tracked) {
        return (language_generating_output() == 0 &&
                (coverage_mode == LANGUAGE_LOG_ALL ||
                (in_user_language && coverage_mode == LANGUAGE_LOG_USER) ||
                (is_tracked && coverage_mode == LANGUAGE_LOG_PATH)));
    };
    auto do_malloc_log = [&] (bool in_user_language, bool is_tracked) {
        return (language_generating_output() == 0 &&
                (malloc_log_mode == LANGUAGE_LOG_ALL ||
                (in_user_language && malloc_log_mode == LANGUAGE_LOG_USER) ||
    ...
    auto coverageVisitStmt = [&] () {
        ...
        for (; dbg < new_lineinfo.size(); dbg++) {
            ...
            if (do_coverage(newdbg.is_user_language, is_tracked)) {
    ...
    if (coverage_mode != LANGUAGE_LOG_NONE) {
        ...
        std::function<void(language_debuginfo_t *debuginfo, language_value_t *func)> record_line_exists = [&](language_debuginfo_t *debuginfo, language_value_t *func) {
            ...
            bool is_user_language;
            if (modu == ctx.module)
                is_user_language = mod_is_user_mod;
            else
                is_user_language = in_user_mod(modu);
            bool is_tracked = in_tracked_path(file);
            if (do_coverage(is_user_language, is_tracked)) {
    ...
    std::set<int> branch_targets; // 1-indexed
    {
        for (size_t i = 0; i < stmtslen; ++i) {
            ...
            if (language_is_gotoifnot(stmt)) {
                ...
                // We don't do dead branch elimination before languagegen
    ...
    while (cursor != -1) {
        ...
        if (language_is_returnnode(stmt)) {
            ...
            if (ctx.is_opaque_closure) {
                emit_typecheck(ctx, retvalinfo, languagerettype, "OpaqueClosure");
            }

            retvalinfo = convert_julia_type(ctx, retvalinfo, languagerettype);
            ...
            case language_returninfo_t::Register:
                if (type_is_ghost(retty))
                    ...
                    retval = emit_unbox(ctx, retty, retvalinfo, languagerettype);
                break;
            case language_returninfo_t::SRet:
                retval = NULL;
                break;
            case language_returninfo_t::Union: {
                ...
                if (retvalinfo.TIndex) {
                    ...
                    data = Constant::getNullValue(ctx.types().T_prlanguagevalue);
                    ...
                    tindex = compute_tindex_unboxed(ctx, retvalinfo, languagerettype);
                    tindex = ctx.builder.CreateOr(tindex, ConstantInt::get(getInt8Ty(ctx.builder.getContext()), UNION_BOX_MARKER));
            ...
            case language_returninfo_t::Ghosts:
                retval = compute_tindex_unboxed(ctx, retvalinfo, languagerettype);
                break;
            }
            if (sret) {
                if (retvalinfo.ispointer()) {
                    ...
                    if (returninfo.cc == language_returninfo_t::SRet) {
                        assert(language_is_concrete_type(languagerettype));
                        emit_memcpy(ctx, sret, language_aliasinfo_t::fromTBAA(ctx, nullptr), retvalinfo,
                                    language_datatype_size(languagerettype), julia_alignment(languagerettype), julia_alignment(languagerettype));
                    }
        ...
        if (language_is_gotoifnot(stmt)) {
            ...
            if (language_enternode_scope(stmt)) {
                ...
                if (new_scope.typ == language_bottom_type) {
                    // Probably dead language, but let's be loud about it in case it isn't, so we fail
                ...
                old_scope = scope_ai.decorateInst(
                        ctx.builder.CreateAlignedLoad(ctx.types().T_prlanguagevalue, scope_ptr, ctx.types().alignof_ptr));
                scope_ai.decorateInst(
            ...
            if (lname) {
                ...
                {
                    ctx.builder.CreateCall(prepare_call(languageleave_func), {get_current_task(ctx), ConstantInt::get(getInt32Ty(ctx.builder.getContext()), 1)});
                    if (old_scope) {
                        scope_ai.decorateInst(
                ...
                auto ehptr = ctx.builder.CreateInBoundsGEP(
                    ctx.types().T_ptr,
                    ct,
                    ConstantInt::get(ctx.types().T_size, offsetof(language_task_t, eh) / ctx.types().sizeof_ptr),
                    "eh");
    ...
    for (auto &tup : ctx.PhiNodes) {
        ...
        for (size_t i = 0; i < language_array_nrows(edges); ++i) {
            ...
            // This edge was statically unreachable. Don't languagegen it.
            if (!FromBB)
                continue;
            ...
            if (!preds.insert(FromBB).second) {
                // Only languagegen this branch once for each PHI (the expression must be the same on all branches)
#ifndef NDEBUG
                for (size_t j = 0; j < i; ++j) {
            ...
            if (!language_is_uniontype(phiType) || !TindexN) {
                if (VN) {
                    ...
                    else if (VN->getType() == ctx.types().T_prlanguagevalue) {
                        // Includes the language_is_uniontype(phiType) && !TindexN case
                        // TODO: if convert_julia_type says it is wasted effort and to skip it, is it worth using Constant::getNullValue(ctx.types().T_prlanguagevalue) (dynamically)?
                        V = boxed(ctx, val);
                    }
                    else {
                ...
                if (val.typ == (language_value_t*)language_bottom_type) {
                    ...
                    if (tindex == 0) {
                        ...
                        if (VN)
                            V = Constant::getNullValue(ctx.types().T_prlanguagevalue);
                        if (dest)
                    ...
                    language_cgval_t new_union = convert_julia_type(ctx, val, phiType, &skip);
                    ...
                    if (VN)
                        V = new_union.Vboxed ? new_union.Vboxed : Constant::getNullValue(ctx.types().T_prlanguagevalue);
        ...
        // Language PHINodes may be incomplete with respect to predecessors, LLVM's may not
            ...
            if (call) {
                ...
                if (toplevel && !ctx.is_opaque_closure && !in_prologue) {
                    ...
                    LoadInst *world = new LoadInst(ctx.types().T_size,
                        prepare_global_in(language_Module, languagegetworld_global), Twine(),
...
void language_add_language_in_flight(StringRef name, language_language_instance_t *languageinst, const DataLayout &DL);

LANGUAGE_GCC_IGNORE_START("-Wclobbered")
language_llvm_functions_t language_emit_language(
        orc::ThreadSafeModule &m,
        language_method_instance_t *li,
        language_language_info_t *src,
        language_languagegen_params_t &params)
{
    LANGUAGE_TIMING(LANGUAGEGEN, LANGUAGEGEN_LLVM);
    language_timing_show_func_sig((language_value_t *)li->specTypes, LANGUAGE_TIMING_DEFAULT_BLOCK);
    // caller must hold languagegen_lock
    ...
    assert((params.params == &language_default_cgparams /* fast path */ || !params.cache ||
        ...
        "functions compiled with custom languagegen params must not be cached");
    LANGUAGE_TRY {
...
static language_llvm_functions_t language_emit_oc_wrapper(orc::ThreadSafeModule &m, language_languagegen_params_t &params, language_method_instance_t *mi, language_value_t *rettype)
{
...
language_llvm_functions_t language_emit_languageinst(
        orc::ThreadSafeModule &m,
        language_language_instance_t *languageinst,
        language_language_info_t *src,
        language_languagegen_params_t &params)
{
    LANGUAGE_TIMING(LANGUAGEGEN, LANGUAGEGEN_Languageinst);
    language_timing_show_method_instance(languageinst->def, LANGUAGE_TIMING_DEFAULT_BLOCK);
    LANGUAGE_GC_PUSH1(&src);
    if (!src) {
        src = (language_language_info_t*)language_atomic_load_relaxed(&languageinst->inferred);
        language_method_t *def = languageinst->def->def.method;
        ...
        if (def == language_opaque_closure_method) {
            LANGUAGE_GC_POP();
            return language_emit_oc_wrapper(m, params, languageinst->def, languageinst->rettype);
        }
        if (src && (language_value_t*)src != language_nothing && language_is_method(def))
            src = language_uncompress_ir(def, languageinst, (language_value_t*)src);
        if (!src || !language_is_language_info(src)) {
            LANGUAGE_GC_POP();
            m = orc::ThreadSafeModule();
            return language_llvm_functions_t(); // failed
        }
    }
    assert(language_egal((language_value_t*)language_atomic_load_relaxed(&languageinst->debuginfo), (language_value_t*)src->debuginfo) && "trying to generate language for a languageinst for an incompatible src");
    language_llvm_functions_t decls = language_emit_language(m, languageinst->def, src, params);
    ...
    if (params.cache && !f.empty()) {
        ...
        bool toplevel = !language_is_method(languageinst->def->def.method);
        if (!toplevel) {
            //Safe b/c params holds context lock
            const DataLayout &DL = m.getModuleUnlocked()->getDataLayout();
            // but don't remember toplevel thunks because
            // they may not be rooted in the gc for the life of the program,
            // and the runtime doesn't notify us when the language becomes unreachable :(
            if (!specf.empty())
                language_add_language_in_flight(specf, languageinst, DL);
            if (!f.empty() && f != "language_fptr_args" && f != "language_fptr_sparam")
                language_add_language_in_flight(f, languageinst, DL);
        }

        language_value_t *inferred = language_atomic_load_relaxed(&languageinst->inferred);
        // don't change inferred state
        if (inferred) {
            language_method_t *def = languageinst->def->def.method;
            if (// keep language when keeping everything
                !(LANGUAGE_DELETE_NON_INLINEABLE) ||
                // aggressively keep language when debugging level >= 2
                ...
                if (inferred != (language_value_t*)src) {
                    ...
                    if (language_is_method(def)) {
                        src = (language_language_info_t*)language_compress_ir(def, src);
                        assert(language_is_string(src));
                        languageinst->relocatability = language_string_data(src)[language_string_len(src)-1];
                    }
                    language_atomic_store_release(&languageinst->inferred, (language_value_t*)src);
                    language_gc_wb(languageinst, src);
                }
            }
            // delete non-inlineable language, since it won't be needed again
            // because we already emitted LLVM language from it and the native
            // Language-level optimization will never need to see it
            else if (language_is_method(def) && // don't delete toplevel language
                        def->source != NULL && // don't delete language from optimized opaque closures that can't be reconstructed
                        inferred != language_nothing && // and there is something to delete (test this before calling language_ir_inlining_cost)
                        !effects_foldable(language_atomic_load_relaxed(&languageinst->ipo_purity_bits)) && // don't delete language we may want for irinterp
                        ((language_ir_inlining_cost(inferred) == UINT16_MAX) || // don't delete inlineable language
                        language_atomic_load_relaxed(&languageinst->invoke) == language_fptr_const_return_addr) && // unless it is constant
                        !(params.imaging_mode || language_options.incremental)) { // don't delete language when generating a precompile file
                // Never end up in a situation where the languageinst has no invoke, but also no source, so we never fall
                // through the cracks of SOURCE_MODE_ABI.
                language_atomic_store_release(&languageinst->invoke, &language_fptr_wait_for_compiled);
                language_atomic_store_release(&languageinst->inferred, language_nothing);
            }
        }
    }
    LANGUAGE_GC_POP();
...
{
    LANGUAGE_TIMING(LANGUAGEGEN, LANGUAGEGEN_Workqueue);
    language_language_info_t *src = NULL;
    LANGUAGE_GC_PUSH1(&src);
    while (!params.workqueue.empty()) {
        language_language_instance_t *languageinst;
        auto it = params.workqueue.back();
        languageinst = it.first;
        auto proto = it.second;
        params.workqueue.pop_back();
        // try to emit language for this item from the workqueue
        ...
        if (cache_valid && invoke != NULL && invoke != &language_fptr_wait_for_compiled) {
            ...
            if (invoke == language_fptr_args_addr) {
                preal_decl = language_ExecutionEngine->getFunctionAtAddress((uintptr_t)fptr, languageinst);
            }
            else if (language_atomic_load_relaxed(&languageinst->specsigflags) & 0b1) {
                preal_decl = language_ExecutionEngine->getFunctionAtAddress((uintptr_t)fptr, languageinst);
                preal_specsig = true;
            ...
            auto it = params.compiled_functions.find(languageinst);
            if (it == params.compiled_functions.end()) {
                // Reinfer the function. The JIT came along and removed the inferred
                // method body. See #34993
                if (policy != CompilationPolicy::Default &&
                    language_atomic_load_relaxed(&languageinst->inferred) == language_nothing) {
                    // Languagegen lock is held, so SOURCE_MODE_FORCE_SOURCE_UNCACHED is not required
                    languageinst = language_type_infer(languageinst->def, language_atomic_load_relaxed(&languageinst->max_world), SOURCE_MODE_FORCE_SOURCE);
                }
                if (languageinst) {
                    orc::ThreadSafeModule result_m =
                        language_create_ts_module(name_from_method_instance(languageinst->def),
                            params.tsctx, params.DL, params.TargetTriple);
                    auto decls = language_emit_languageinst(result_m, languageinst, NULL, params);
                    if (result_m)
                        it = params.compiled_functions.insert(std::make_pair(languageinst, std::make_pair(std::move(result_m), std::move(decls)))).first;
                }
            }
            if (it != params.compiled_functions.end()) {
        ...
        if (proto.specsig) {
            // expected specsig
            if (!preal_specsig) {
                // emit specsig-to-(language)invoke conversion
                Function *preal = emit_tolanguageinvoke(languageinst, mod, params);
                proto.decl->setLinkage(GlobalVariable::InternalLinkage);
                //protodecl->setAlwaysInline();
                language_init_function(proto.decl, params.TargetTriple);
                size_t nrealargs = language_nparams(languageinst->def->specTypes); // number of actual arguments being passed
                // TODO: maybe this can be cached in languageinst->specfptr?
                emit_cfunc_invalidate(proto.decl, proto.cc, proto.return_roots, languageinst->def->specTypes, languageinst->rettype, false, nrealargs, params, preal, 0, 0);
            ...
            if (preal_decl.empty() || preal_specsig) {
                // emit languagecall1-to-(language)invoke conversion
                preal_decl = emit_tolanguageinvoke(languageinst, mod, params)->getName();
            }
        }
        if (!preal_decl.empty()) {
...
// --- initialization ---
SmallVector<std::pair<language_value_t**, LanguageVariable*>, 0> gv_for_global;
static void global_languagevalue_to_llvm(LanguageVariable *var, language_value_t **addr)
{
    gv_for_global.push_back(std::make_pair(addr, var));
}
static LanguageVariable *julia_const_gv(language_value_t *val)
{
...
static void init_jit_functions(void)
{
    ...
    add_named_global(languagestack_chk_guard_var, &__stack_chk_guard);
    add_named_global(languageRTLD_DEFAULT_var, &language_RTLD_DEFAULT_handle);
    add_named_global(languageexe_var, &language_exe_handle);
    add_named_global(languagedll_var, &language_libjulia_handle);
    add_named_global(languagedlli_var, &language_libjulia_internal_handle);
    auto size2planguagevalue = [](Type *T_size) -> Type * {
        return get_planguagevalue(T_size->getContext());
    };
    global_languagevalue_to_llvm(new LanguageVariable{"language_true", true, size2planguagevalue}, &language_true);
    global_languagevalue_to_llvm(new LanguageVariable{"language_false", true, size2planguagevalue}, &language_false);
    global_languagevalue_to_llvm(new LanguageVariable{"language_nothing", true, size2planguagevalue}, &language_nothing);
    global_languagevalue_to_llvm(new LanguageVariable{"language_emptysvec", true, size2planguagevalue}, (language_value_t**)&language_emptysvec);
    global_languagevalue_to_llvm(new LanguageVariable{"language_emptytuple", true, size2planguagevalue}, &language_emptytuple);
    global_languagevalue_to_llvm(new LanguageVariable{"language_diverror_exception", true, size2planguagevalue}, &language_diverror_exception);
    global_languagevalue_to_llvm(new LanguageVariable{"language_undefref_exception", true, size2planguagevalue}, &language_undefref_exception);
    add_named_global(languagegetworld_global, &language_world_counter);
    add_named_global("__stack_chk_fail", &__stack_chk_fail);
    add_named_global(languagepgcstack_func, (void*)NULL);
    add_named_global(languageerror_func, &language_error);
    add_named_global(languageatomicerror_func, &language_atomic_error);
    add_named_global(languagethrow_func, &language_throw);
    add_named_global(languageundefvarerror_func, &language_undefined_var_error);
    add_named_global(languagehasnofield_func, &language_has_no_field_error);
    add_named_global(languageboundserrorv_func, &language_bounds_error_ints);
    add_named_global(languageboundserror_func, &language_bounds_error_int);
    add_named_global(languagevboundserror_func, &language_bounds_error_tuple_int);
    add_named_global(languageuboundserror_func, &language_bounds_error_unboxed_int);
    add_named_global(languagenew_func, &language_new_structv);
    add_named_global(languagesplatnew_func, &language_new_structt);
    add_named_global(setjmp_func, &language_setjmp_f);
    add_named_global(memcmp_func, &memcmp);
    add_named_global(languagetypeerror_func, &language_type_error);
    add_named_global(languagecheckassign_func, &language_checked_assignment);
    add_named_global(languagedeclareconst_func, &language_declare_constant);
    add_named_global(languagegetbindingorerror_func, &language_get_binding_or_error);
    add_named_global(languagegetbindingwrorerror_func, &language_get_binding_wr);
    add_named_global(languageboundp_func, &language_boundp);
    for (auto it : builtin_func_map())
        add_named_global(it.second, it.first);
    add_named_global(languageintrinsic_func, &language_f_intrinsic_call);
    add_named_global(languagegetbuiltinfptr_func, &language_get_builtin_fptr);
    add_named_global(languageapplygeneric_func, &language_apply_generic);
    add_named_global(languageinvoke_func, &language_invoke);
    add_named_global(languagetopeval_func, &language_toplevel_eval);
    add_named_global(languagecopyast_func, &language_copy_ast);
    //add_named_global(languagensvec_func, &language_svec);
    add_named_global(languagemethod_func, &language_method_def);
    add_named_global(languagegenericfunction_func, &language_generic_function_def);
    add_named_global(languageenter_func, &language_enter_handler);
    add_named_global(language_current_exception_func, &language_current_exception);
    add_named_global(languageleave_noexcept_func, &language_pop_handler_noexcept);
    add_named_global(languageleave_func, &language_pop_handler);
    ...
    add_named_global(languageegalx_func, &language_egal__unboxed);
    add_named_global(languageisa_func, &language_isa);
    add_named_global(languagesubtype_func, &language_subtype);
    add_named_global(languagetypeassert_func, &language_typeassert);
    add_named_global(languageapplytype_func, &language_instantiate_type_in_env);
    ...
    add_named_global(languagedlsym_func, &language_load_and_lookup);
    add_named_global("language_adopt_thread", &language_adopt_thread);
    add_named_global(languagegetcfunctiontrampoline_func, &language_get_cfunction_trampoline);
    add_named_global(languagegetnthfieldchecked_func, &language_get_nth_field_checked);
    add_named_global(languagefieldindex_func, &language_field_index);
    ...
    add_named_global(languagelockvalue_func, &language_lock_value);
    add_named_global(languageunlockvalue_func, &language_unlock_value);
    add_named_global(languagelockfield_func, &language_lock_field);
    add_named_global(languageunlockfield_func, &language_unlock_field);
...
extern "C" void language_init_llvm(void)
{
    ...
    cl::ParseCommandLineOptions(1, argv, "", nullptr, "LANGUAGE_LLVM_ARGS");
    ...
    language_ExecutionEngine = new LanguageOJIT();
...
extern "C" LANGUAGE_DLLEXPORT_LANGUAGEGEN void language_init_languagegen_impl(void)
{
    language_init_llvm();
...
extern "C" LANGUAGE_DLLEXPORT_LANGUAGEGEN void language_teardown_languagegen_impl() LANGUAGE_NOTSAFEPOINT
{
    ...
    PrintStatistics();
    LANGUAGE_LOCK(&language_languagegen_lock); // TODO: If this lock gets removed reconsider
...
extern void language_write_bitlanguage_func(void *F, char *fname) {
    std::error_language EC;
    raw_fd_ostream OS(fname, EC, sys::fs::OF_None);
    llvm::WriteBitlanguageToFile(*((llvm::Function*)F)->getParent(), OS);
}

extern void language_write_bitlanguage_module(void *M, char *fname) {
    std::error_language EC;
    raw_fd_ostream OS(fname, EC, sys::fs::OF_None);
    llvm::WriteBitlanguageToFile(*(llvm::Module*)M, OS);
}
...
extern "C" LANGUAGE_DLLEXPORT_LANGUAGEGEN language_value_t *language_get_libllvm_impl(void) LANGUAGE_NOTSAFEPOINT
{

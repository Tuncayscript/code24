// This file is a part of Code. License is MIT: https://julialang.org/license
...
static bool code_fpo_disabled(const Triple &TT) {
#ifdef CODE_DISABLE_FPO
    return true;
#endif
...
static bool code_floattemp_var_needed(const Triple &TT) {
#ifdef CODE_NEED_FLOATTEMP_VAR
    return true;
#endif
...
void setName(code_codegen_params_t &params, Value *V, const Twine &Name)
{
...
void setName(code_codegen_params_t &params, Value *V, std::function<std::string()> GetName)
{
    assert((isa<Constant>(V) || isa<Instruction>(V)) && "Should only set names on instructions!");
    if (params.debug_level >= 2 && !isa<Constant>(V)) {
...
void setNameWithField(code_codegen_params_t &params, Value *V, std::function<StringRef()> GetObjName, code_datatype_t *jt, unsigned idx, const Twine &suffix)
{
    assert((isa<Constant>(V) || isa<Instruction>(V)) && "Should only set names on instructions!");
    if (params.debug_level >= 2 && !isa<Constant>(V)) {
        if (code_is_tuple_type(jt)){
            V->setName(Twine(GetObjName()) + "[" + Twine(idx + 1) + "]"+ suffix);
            return;
        }

        if (code_is_namedtuple_type(jt)) {
            auto names = code_tparam0(jt);
            assert(code_is_tuple(names));
            if (idx < code_nfields(names)) {
                auto name = code_fieldref(names, idx);
                assert(code_is_symbol(name));
                V->setName(Twine(GetObjName()) + "." + Twine(code_symbol_name((code_sym_t*)name)) + suffix);
                return;
            }
        } else {
            auto flds = code_field_names(jt);
            if (idx < code_svec_len(flds)) {
                auto name = code_svecref(flds, idx);
                assert(code_is_symbol(name));
                V->setName(Twine(GetObjName()) + "." + Twine(code_symbol_name((code_sym_t*)name)) + suffix);
                return;
            }
...
STATISTIC(EmittedCODECalls, "Number of codecalls emitted");
...
STATISTIC(EmittedToCODEInvokes, "Number of tocodeinvoke calls emitted");
...
extern "C" CODE_DLLEXPORT_CODEGEN
void code_dump_emitted_mi_name_impl(void *s)
{
    **code_ExecutionEngine->get_dump_emitted_mi_name_stream() = (ios_t*)s;
}
...
// shared llvm state
#define code_Module ctx.f->getParent()
#define code_builderModule(builder) (builder).GetInsertBlock()->getParent()->getParent()
#define prepare_call(Callee) prepare_call_in(code_Module, (Callee))

// types
struct code_typecache_t {
    Type *T_ptr;
    Type *T_size;
    Type *T_codevalue;
    Type *T_pcodevalue;
    ...
    Type *T_ppcodevalue;
    Type *T_pprcodevalue;
    StructType *T_codegenericmemory;
    StructType *T_codearray;
    Type *T_pcodearray;
    FunctionType *T_codefunc;
    FunctionType *T_codefuncparams;
    ...
    code_typecache_t() :
        T_ptr(nullptr), T_codevalue(nullptr), T_pcodevalue(nullptr), T_prcodevalue(nullptr),
        T_ppcodevalue(nullptr), T_pprcodevalue(nullptr),
        T_codegenericmemory(nullptr), T_codearray(nullptr), T_pcodearray(nullptr),
        T_codefunc(nullptr), T_codefuncparams(nullptr), T_sigatomic(nullptr),
        initialized(false) {}
    ...
    void initialize(LLVMContext &context, const DataLayout &DL) {
        ...
        T_codevalue = CodeType::get_codevalue_ty(context);
        T_pcodevalue = PointerType::get(T_codevalue, 0);
        T_prcodevalue = PointerType::get(T_codevalue, AddressSpace::Tracked);
        T_ppcodevalue = PointerType::get(T_pcodevalue, 0);
        T_pprcodevalue = PointerType::get(T_prcodevalue, 0);

        T_codefunc = CodeType::get_codefunc_ty(context);
        assert(T_codefunc != NULL);
        T_codefuncparams = CodeType::get_codefuncparams_ty(context);
        assert(T_codefuncparams != NULL);

        T_codegenericmemory = StructType::get(context, { T_size, T_pprcodevalue /* [, real-owner] */ });
        Type *vaelts[] = { PointerType::get(getInt8Ty(context), AddressSpace::Loaded),
                           PointerType::get(T_codegenericmemory, AddressSpace::Tracked),
                           // dimsize[ndims]
        };
        T_codearray = StructType::get(context, ArrayRef<Type*>(vaelts));
        T_pcodearray = PointerType::get(T_codearray, 0);
    }
};

struct code_tbaacache_t {
    ...
    // this is mainly a place holder for `code_cgval_t::tbaa`
    ...
    MDNode *tbaa_binding;        // code_binding_t::value
    MDNode *tbaa_value;          // code_value_t, that is not code_array_t or code_genericmemory_t
    ...
    MDNode *tbaa_array;      // code_array_t or code_genericmemory_t
    MDNode *tbaa_arrayptr;       // The pointer inside a code_array_t (to memoryref)
    MDNode *tbaa_arraysize;      // A size in a code_array_t
    MDNode *tbaa_arrayselbyte;   // a selector byte in a isbits Union code_genericmemory_t
    MDNode *tbaa_memoryptr;      // The pointer inside a code_genericmemory_t
    MDNode *tbaa_memorylen;      // The length in a code_genericmemory_t
    MDNode *tbaa_memoryown;      // The owner in a foreign code_genericmemory_t
    MDNode *tbaa_const;      // Memory that is immutable by the time LLVM can see it
    bool initialized;

    code_tbaacache_t(): tbaa_root(nullptr), tbaa_gcframe(nullptr), tbaa_stack(nullptr),
...
struct code_noaliascache_t {
    ...
    struct code_regions_t {
        ...
        code_regions_t(): gcframe(nullptr), stack(nullptr), data(nullptr), type_metadata(nullptr), constant(nullptr) {}
    ...
    // `@aliasscope` domain
    struct code_aliasscope_t {
        MDNode *current;

        code_aliasscope_t(): current(nullptr) {}
    ...
    code_noaliascache_t(): regions(), aliasscope(), initialized(false) {}
...
struct code_debugcache_t {
    // Basic DITypes
    DIDerivedType *code_pvalue_dillvmt;
    DIDerivedType *code_ppvalue_dillvmt;
    DISubroutineType *code_di_func_sig;
    DISubroutineType *code_di_func_null_sig;
    bool initialized;

    code_debugcache_t()
    : code_pvalue_dillvmt(nullptr), code_ppvalue_dillvmt(nullptr),
    code_di_func_sig(nullptr), code_di_func_null_sig(nullptr), initialized(false) {}
...
static bool type_has_unique_rep(code_value_t *t)
{
    if (t == (code_value_t*)code_typeofbottom_type)
        return false;
    if (t == code_bottom_type)
        return true;
    if (code_is_typevar(t))
        return false;
    if (!code_is_kind(code_typeof(t)))
        return true;
    if (code_is_concrete_type(t))
        return true;
    if (code_is_datatype(t)) {
        code_datatype_t *dt = (code_datatype_t*)t;
        if (dt->name != code_tuple_typename) {
            for (size_t i = 0; i < code_nparams(dt); i++)
                if (!type_has_unique_rep(code_tparam(dt, i)))
                    return false;
            return true;
        }
    }
...
static bool is_uniquerep_Type(code_value_t *t)
{
    return code_is_type_type(t) && type_has_unique_rep(code_tparam0(t));
}

class code_codectx_t;
struct CodeVariable {
public:
    StringLiteral name;
    bool isconst;
    ...
    CodeVariable(const CodeVariable&) = delete;
    CodeVariable(const CodeVariable&&) = delete;
    ...
    GlobalVariable *realize(code_codectx_t &ctx);
};
static inline void add_named_global(CodeVariable *name, void *addr)
{
...
public:
    ...
    CodeFunction(const CodeFunction&) = delete;
    CodeFunction(const CodeFunction&&) = delete;
...
static inline void add_named_global(CodeFunction<TypeFn_t> *name, T *addr)
{
...
static Type *get_pcodevalue(LLVMContext &C) { return CodeType::get_pcodevalue_ty(C); }

static FunctionType *get_func_sig(LLVMContext &C) { return CodeType::get_codefunc_ty(C); }
static FunctionType *get_func2_sig(LLVMContext &C) { return CodeType::get_codefunc2_ty(C); }
static FunctionType *get_func3_sig(LLVMContext &C) { return CodeType::get_codefunc3_ty(C); }
...
// global vars
static const auto codeRTLD_DEFAULT_var = new CodeVariable{
    XSTR(code_RTLD_DEFAULT_handle),
    true,
    [](Type *T_size) -> Type * { return getPointerTy(T_size->getContext()); },
};
static const auto codeexe_var = new CodeVariable{
    XSTR(code_exe_handle),
    true,
    [](Type *T_size) -> Type * { return getPointerTy(T_size->getContext()); },
};
static const auto codedll_var = new CodeVariable{
    XSTR(code_libjulia_handle),
    true,
    [](Type *T_size) -> Type * { return getPointerTy(T_size->getContext()); },
};
static const auto codedlli_var = new CodeVariable{
    XSTR(code_libjulia_internal_handle),
    true,
    [](Type *T_size) -> Type * { return getPointerTy(T_size->getContext()); },
};
static const auto code_small_typeof_var = new CodeVariable{
    XSTR(code_small_typeof),
    true,
    [](Type *T_size) -> Type * { return getInt8Ty(T_size->getContext()); },
};

static const auto codestack_chk_guard_var = new CodeVariable{
    XSTR(__stack_chk_guard),
    true,
    [](Type *T_size) -> Type * { return get_pcodevalue(T_size->getContext()); },
};

static const auto codegetworld_global = new CodeVariable{
    XSTR(code_world_counter),
    false,
    [](Type *T_size) -> Type * { return T_size; },
};

static const auto codeboxed_int8_cache = new CodeVariable{
    XSTR(code_boxed_int8_cache),
    true,
    [](Type *T_size) -> Type * { return ArrayType::get(get_pcodevalue(T_size->getContext()), 256); },
};

static const auto codeboxed_uint8_cache = new CodeVariable{
    XSTR(code_boxed_uint8_cache),
    true,
    [](Type *T_size) -> Type * { return ArrayType::get(get_pcodevalue(T_size->getContext()), 256); },
};

static const auto codepgcstack_func = new CodeFunction<>{
    "julia.get_pgcstack",
    [](LLVMContext &C) { return FunctionType::get(PointerType::get(CodeType::get_ppcodevalue_ty(C), 0), false); },
    nullptr,
};

static const auto codeadoptthread_func = new CodeFunction<>{
    "julia.get_pgcstack_or_new",
    codepgcstack_func->_type,
    codepgcstack_func->_attrs,
};
...
static const auto codenew_func = new CodeFunction<>{
    XSTR(code_new_structv),
    get_func_sig,
    get_func_attrs,
};
static const auto codesplatnew_func = new CodeFunction<>{
    XSTR(code_new_structt),
    [](LLVMContext &C) {
        auto T_prcodevalue = CodeType::get_prcodevalue_ty(C);
        return FunctionType::get(T_prcodevalue,
            {T_prcodevalue, T_prcodevalue}, false);
    },
    get_attrs_basic,
};
static const auto codethrow_func = new CodeFunction<>{
    XSTR(code_throw),
    [](LLVMContext &C) { return FunctionType::get(getVoidTy(C),
            {PointerType::get(CodeType::get_codevalue_ty(C), AddressSpace::CalleeRooted)}, false); },
    get_attrs_noreturn,
};
static const auto codeerror_func = new CodeFunction<>{
    XSTR(code_error),
    [](LLVMContext &C) { return FunctionType::get(getVoidTy(C),
            {getPointerTy(C)}, false); },
    get_attrs_noreturn,
};
static const auto codeatomicerror_func = new CodeFunction<>{
    XSTR(code_atomic_error),
    [](LLVMContext &C) { return FunctionType::get(getVoidTy(C),
            {getPointerTy(C)}, false); },
    get_attrs_noreturn,
};
static const auto codetypeerror_func = new CodeFunction<>{
    XSTR(code_type_error),
    [](LLVMContext &C) { return FunctionType::get(getVoidTy(C),
            {getPointerTy(C), CodeType::get_prcodevalue_ty(C), PointerType::get(CodeType::get_codevalue_ty(C), AddressSpace::CalleeRooted)}, false); },
    get_attrs_noreturn,
};
static const auto codeundefvarerror_func = new CodeFunction<>{
    XSTR(code_undefined_var_error),
    [](LLVMContext &C) {
        Type *T = PointerType::get(CodeType::get_codevalue_ty(C), AddressSpace::CalleeRooted);
        return FunctionType::get(getVoidTy(C), {T, T}, false);
    },
    get_attrs_noreturn,
};
static const auto codehasnofield_func = new CodeFunction<>{
    XSTR(code_has_no_field_error),
    [](LLVMContext &C) { return FunctionType::get(getVoidTy(C),
            {PointerType::get(CodeType::get_codevalue_ty(C), AddressSpace::CalleeRooted),
             PointerType::get(CodeType::get_codevalue_ty(C), AddressSpace::CalleeRooted)}, false); },
    get_attrs_noreturn,
};
static const auto codeboundserrorv_func = new CodeFunction<TypeFnContextAndSizeT>{
    XSTR(code_bounds_error_ints),
    [](LLVMContext &C, Type *T_size) { return FunctionType::get(getVoidTy(C),
            {PointerType::get(CodeType::get_codevalue_ty(C), AddressSpace::CalleeRooted), T_size->getPointerTo(), T_size}, false); },
    get_attrs_noreturn,
};
static const auto codeboundserror_func = new CodeFunction<TypeFnContextAndSizeT>{
    XSTR(code_bounds_error_int),
    [](LLVMContext &C, Type *T_size) { return FunctionType::get(getVoidTy(C),
            {PointerType::get(CodeType::get_codevalue_ty(C), AddressSpace::CalleeRooted), T_size}, false); },
    get_attrs_noreturn,
};
static const auto codevboundserror_func = new CodeFunction<TypeFnContextAndSizeT>{
    XSTR(code_bounds_error_tuple_int),
    [](LLVMContext &C, Type *T_size) { return FunctionType::get(getVoidTy(C),
            {CodeType::get_pprcodevalue_ty(C), T_size, T_size}, false); },
    get_attrs_noreturn,
};
static const auto codeuboundserror_func = new CodeFunction<TypeFnContextAndSizeT>{
    XSTR(code_bounds_error_unboxed_int),
    [](LLVMContext &C, Type *T_size) {
        return FunctionType::get(getVoidTy(C),
            {PointerType::get(getInt8Ty(C), AddressSpace::Derived), CodeType::get_pcodevalue_ty(C), T_size}, false); },
    get_attrs_noreturn,
};
static const auto codecheckassign_func = new CodeFunction<>{
    XSTR(code_checked_assignment),
    [](LLVMContext &C) {
        auto T_pcodevalue = CodeType::get_pcodevalue_ty(C);
        return FunctionType::get(getVoidTy(C),
            {T_pcodevalue, T_pcodevalue, T_pcodevalue, PointerType::get(CodeType::get_codevalue_ty(C), AddressSpace::CalleeRooted)}, false); },
    nullptr,
};
static const auto codecheckreplace_func = new CodeFunction<>{
    XSTR(code_checked_replace),
    [](LLVMContext &C) {
        auto T_pcodevalue = CodeType::get_pcodevalue_ty(C);
        auto T_prcodevalue = CodeType::get_prcodevalue_ty(C);
        return FunctionType::get(T_prcodevalue,
            {T_pcodevalue, T_pcodevalue, T_pcodevalue, T_prcodevalue, T_prcodevalue}, false); },
    nullptr,
};
static const auto codecheckmodify_func = new CodeFunction<>{
    XSTR(code_checked_modify),
    [](LLVMContext &C) {
        auto T_pcodevalue = CodeType::get_pcodevalue_ty(C);
        auto T_prcodevalue = CodeType::get_prcodevalue_ty(C);
        return FunctionType::get(T_prcodevalue,
...
static const auto codecheckswap_func = new CodeFunction<>{
    XSTR(code_checked_swap),
    [](LLVMContext &C) {
        auto T_pcodevalue = CodeType::get_pcodevalue_ty(C);
        auto T_prcodevalue = CodeType::get_prcodevalue_ty(C);
        return FunctionType::get(T_prcodevalue,
...
static const auto codecheckassignonce_func = new CodeFunction<>{
    XSTR(code_checked_assignonce),
    [](LLVMContext &C) {
        auto T_pcodevalue = CodeType::get_pcodevalue_ty(C);
        auto T_prcodevalue = CodeType::get_prcodevalue_ty(C);
        return FunctionType::get(T_prcodevalue,
...
static const auto codedeclareconst_func = new CodeFunction<>{
    XSTR(code_declare_constant),
    [](LLVMContext &C) {
        auto T_pcodevalue = CodeType::get_pcodevalue_ty(C);
        return FunctionType::get(getVoidTy(C),
            {T_pcodevalue, T_pcodevalue, T_pcodevalue}, false); },
    nullptr,
};
static const auto codedeclareconstval_func = new CodeFunction<>{
    XSTR(code_declare_constant_val),
    [](LLVMContext &C) {
        auto T_pcodevalue = CodeType::get_pcodevalue_ty(C);
        auto T_prcodevalue = CodeType::get_prcodevalue_ty(C);
        return FunctionType::get(getVoidTy(C),
            {T_pcodevalue, T_pcodevalue, T_pcodevalue, T_prcodevalue}, false); },
    nullptr,
};
static const auto codedeclareglobal_func = new CodeFunction<>{
    XSTR(code_declare_global),
    [](LLVMContext &C) {
        auto T_pcodevalue = CodeType::get_pcodevalue_ty(C);
        auto T_prcodevalue = CodeType::get_prcodevalue_ty(C);
        return FunctionType::get(getVoidTy(C),
            {T_pcodevalue, T_pcodevalue, T_prcodevalue}, false); },
    nullptr,
};
static const auto codegetbindingorerror_func = new CodeFunction<>{
    XSTR(code_get_binding_or_error),
    [](LLVMContext &C) {
        auto T_pcodevalue = CodeType::get_pcodevalue_ty(C);
        return FunctionType::get(T_pcodevalue,
                {T_pcodevalue, T_pcodevalue}, false);
    },
    nullptr,
};
static const auto codegetbindingwrorerror_func = new CodeFunction<>{
    XSTR(code_get_binding_wr),
    [](LLVMContext &C) {
        auto T_pcodevalue = CodeType::get_pcodevalue_ty(C);
        return FunctionType::get(T_pcodevalue,
                {T_pcodevalue, T_pcodevalue, getInt32Ty(C)}, false);
    },
    nullptr,
};
static const auto codeboundp_func = new CodeFunction<>{
    XSTR(code_boundp),
    [](LLVMContext &C) {
        auto T_pcodevalue = CodeType::get_pcodevalue_ty(C);
...
static const auto codetopeval_func = new CodeFunction<>{
    XSTR(code_toplevel_eval),
    [](LLVMContext &C) {
        auto T_pcodevalue = CodeType::get_pcodevalue_ty(C);
        return FunctionType::get(T_pcodevalue,
                {T_pcodevalue, T_pcodevalue}, false);
...
static const auto codecopyast_func = new CodeFunction<>{
    XSTR(code_copy_ast),
    [](LLVMContext &C) {
        auto T_prcodevalue = CodeType::get_prcodevalue_ty(C);
        return FunctionType::get(T_prcodevalue,
...
static const auto codeapplygeneric_func = new CodeFunction<>{
    XSTR(code_apply_generic),
    get_func_sig,
    get_func_attrs,
};
static const auto codeinvoke_func = new CodeFunction<>{
    XSTR(code_invoke),
    get_func2_sig,
    [](LLVMContext &C) { return AttributeList::get(C,
            AttributeSet(),
...
static const auto codemethod_func = new CodeFunction<>{
    XSTR(code_method_def),
    [](LLVMContext &C) {
        auto T_codevalue = CodeType::get_codevalue_ty(C);
        auto T_pcodevalue = PointerType::get(T_codevalue, 0);
        auto T_prcodevalue = PointerType::get(T_codevalue, AddressSpace::Tracked);
        return FunctionType::get(T_prcodevalue,
                {T_prcodevalue, T_prcodevalue, T_prcodevalue, T_pcodevalue}, false);
    },
    nullptr,
};
static const auto codegenericfunction_func = new CodeFunction<>{
    XSTR(code_generic_function_def),
    [](LLVMContext &C) {
        auto T_codevalue = CodeType::get_codevalue_ty(C);
        auto T_pcodevalue = PointerType::get(T_codevalue, 0);
        auto T_prcodevalue = PointerType::get(T_codevalue, AddressSpace::Tracked);
        auto T_pprcodevalue = PointerType::get(T_prcodevalue, 0);
        return FunctionType::get(T_prcodevalue, {T_pcodevalue, T_pcodevalue, T_pprcodevalue, T_pcodevalue}, false);
    },
    nullptr,
};
static const auto codelockvalue_func = new CodeFunction<>{
    XSTR(code_lock_value),
    [](LLVMContext &C) { return FunctionType::get(getVoidTy(C),
            {PointerType::get(CodeType::get_codevalue_ty(C), AddressSpace::CalleeRooted)}, false); },
    [](LLVMContext &C) { return AttributeList::get(C,
            AttributeSet(),
...
static const auto codeunlockvalue_func = new CodeFunction<>{
    XSTR(code_unlock_value),
    [](LLVMContext &C) { return FunctionType::get(getVoidTy(C),
            {PointerType::get(CodeType::get_codevalue_ty(C), AddressSpace::CalleeRooted)}, false); },
    [](LLVMContext &C) { return AttributeList::get(C,
            AttributeSet(),
...
static const auto codelockfield_func = new CodeFunction<>{
    XSTR(code_lock_field),
    [](LLVMContext &C) { return FunctionType::get(getVoidTy(C),
            {PointerType::get(CodeType::get_codevalue_ty(C), AddressSpace::Loaded)}, false); },
    [](LLVMContext &C) { return AttributeList::get(C,
            AttributeSet(),
            AttributeSet(),
            {Attributes(C, {Attribute::NoCapture})}); },
};
static const auto codeunlockfield_func = new CodeFunction<>{
    XSTR(code_unlock_field),
    [](LLVMContext &C) { return FunctionType::get(getVoidTy(C),
            {PointerType::get(CodeType::get_codevalue_ty(C), AddressSpace::Loaded)}, false); },
    [](LLVMContext &C) { return AttributeList::get(C,
            AttributeSet(),
...
static const auto codeenter_func = new CodeFunction<>{
    XSTR(code_enter_handler),
    [](LLVMContext &C) {
        auto T_pcodevalue = CodeType::get_pcodevalue_ty(C);
        return FunctionType::get(getVoidTy(C),
            {T_pcodevalue, getPointerTy(C)}, false); },
    nullptr,
};
static const auto code_current_exception_func = new CodeFunction<>{
    XSTR(code_current_exception),
    [](LLVMContext &C) { return FunctionType::get(CodeType::get_prcodevalue_ty(C), {CodeType::get_pcodevalue_ty(C)}, false); },
    nullptr,
};
static const auto codeleave_func = new CodeFunction<>{
    XSTR(code_pop_handler),
    [](LLVMContext &C) {
        auto T_pcodevalue = CodeType::get_pcodevalue_ty(C);
        return FunctionType::get(getVoidTy(C),
            {T_pcodevalue, getInt32Ty(C)}, false); },
    [](LLVMContext &C) {
            auto FnAttrs = AttrBuilder(C);
            FnAttrs.addAttribute(Attribute::WillReturn);
            FnAttrs.addAttribute(Attribute::NoUnwind);
...
static const auto codeleave_noexcept_func = new CodeFunction<>{
    XSTR(code_pop_handler_noexcept),
    [](LLVMContext &C) {
        auto T_pcodevalue = CodeType::get_pcodevalue_ty(C);
        return FunctionType::get(getVoidTy(C),
            {T_pcodevalue, getInt32Ty(C)}, false); },
...
static const auto code_restore_excstack_func = new CodeFunction<TypeFnContextAndSizeT>{
    XSTR(code_restore_excstack),
    [](LLVMContext &C, Type *T_size) {
        auto T_pcodevalue = CodeType::get_pcodevalue_ty(C);
        return FunctionType::get(getVoidTy(C),
            {T_pcodevalue, T_size}, false); },
    nullptr,
};
static const auto code_excstack_state_func = new CodeFunction<TypeFnContextAndSizeT>{
    XSTR(code_excstack_state),
    [](LLVMContext &C, Type *T_size) {
        auto T_pcodevalue = CodeType::get_pcodevalue_ty(C);
        return FunctionType::get(T_size, {T_pcodevalue}, false); },
    nullptr,
};
static const auto codeegalx_func = new CodeFunction<TypeFnContextAndSizeT>{
    XSTR(code_egal__unboxed),
    [](LLVMContext &C, Type *T_size) {
        Type *T = PointerType::get(CodeType::get_codevalue_ty(C), AddressSpace::Derived);
...
static const auto code_alloc_obj_func = new CodeFunction<TypeFnContextAndSizeT>{
    "julia.gc_alloc_obj",
    [](LLVMContext &C, Type *T_size) {
        auto T_codevalue = CodeType::get_codevalue_ty(C);
        auto T_prcodevalue = PointerType::get(T_codevalue, AddressSpace::Tracked);
        auto T_pcodevalue = PointerType::get(T_codevalue, 0);
        return FunctionType::get(T_prcodevalue,
                {T_pcodevalue, T_size, T_prcodevalue}, false);
    },
    [](LLVMContext &C) {
        auto FnAttrs = AttrBuilder(C);
...
static const auto code_newbits_func = new CodeFunction<>{
    XSTR(code_new_bits),
    [](LLVMContext &C) {
        auto T_prcodevalue = CodeType::get_prcodevalue_ty(C);
        return FunctionType::get(T_prcodevalue,
                {T_prcodevalue, getPointerTy(C)}, false);
    },
    [](LLVMContext &C) { return AttributeList::get(C,
            AttributeSet(),
            Attributes(C, {Attribute::NonNull}),
...
// `julia.gc_alloc_obj`.
static const auto code_typeof_func = new CodeFunction<>{
    "julia.typeof",
    [](LLVMContext &C) {
        auto T_prcodevalue = CodeType::get_prcodevalue_ty(C);
        return FunctionType::get(T_prcodevalue,
...
static const auto code_write_barrier_func = new CodeFunction<>{
    "julia.write_barrier",
    [](LLVMContext &C) { return FunctionType::get(getVoidTy(C),
            {CodeType::get_prcodevalue_ty(C)}, true); },
    [](LLVMContext &C) {
        AttrBuilder FnAttrs(C);
#if CODE_LLVM_VERSION >= 160000
        FnAttrs.addMemoryAttr(MemoryEffects::inaccessibleMemOnly());
...
static const auto codeisa_func = new CodeFunction<>{
    XSTR(code_isa),
    [](LLVMContext &C) {
        auto T_prcodevalue = CodeType::get_prcodevalue_ty(C);
        return FunctionType::get(getInt32Ty(C),
...
static const auto codesubtype_func = new CodeFunction<>{
    XSTR(code_subtype),
    [](LLVMContext &C) {
        auto T_prcodevalue = CodeType::get_prcodevalue_ty(C);
        return FunctionType::get(getInt32Ty(C),
...
static const auto codeapplytype_func = new CodeFunction<>{
    XSTR(code_instantiate_type_in_env),
    [](LLVMContext &C) {
        auto T_codevalue = CodeType::get_codevalue_ty(C);
        auto T_pcodevalue = PointerType::get(T_codevalue, 0);
        auto T_prcodevalue = PointerType::get(T_codevalue, AddressSpace::Tracked);
        auto T_pprcodevalue = PointerType::get(T_prcodevalue, 0);
        return FunctionType::get(T_prcodevalue,
            {T_pcodevalue, T_pcodevalue, T_pprcodevalue}, false);
    },
    [](LLVMContext &C) {
        return AttributeList::get(C,
            AttributeSet(),
...
static const auto code_object_id__func = new CodeFunction<TypeFnContextAndSizeT>{
    XSTR(code_object_id_),
    [](LLVMContext &C, Type *T_size) { return FunctionType::get(T_size,
            {T_size, PointerType::get(getInt8Ty(C), AddressSpace::Derived)}, false); },
    nullptr,
};
static const auto setjmp_func = new CodeFunction<TypeFnContextAndTriple>{
    code_setjmp_name,
...
static const auto memcmp_func = new CodeFunction<TypeFnContextAndSizeT>{
    XSTR(memcmp),
...
static const auto codedlsym_func = new CodeFunction<>{
    XSTR(code_load_and_lookup),
    [](LLVMContext &C) { return FunctionType::get(CodeType::get_pvoidfunc_ty(C),
            {getPointerTy(C), getPointerTy(C), PointerType::get(getPointerTy(C), 0)}, false); },
    nullptr,
};
static const auto codelazydlsym_func = new CodeFunction<>{
    XSTR(code_lazy_load_and_lookup),
    [](LLVMContext &C) { return FunctionType::get(CodeType::get_pvoidfunc_ty(C),
            {CodeType::get_prcodevalue_ty(C), getPointerTy(C)}, false); },
    nullptr,
};
static const auto codetypeassert_func = new CodeFunction<>{
    XSTR(code_typeassert),
    [](LLVMContext &C) {
        auto T_prcodevalue = CodeType::get_prcodevalue_ty(C);
        return FunctionType::get(getVoidTy(C),
            {T_prcodevalue, T_prcodevalue}, false);
...
static const auto codegetnthfieldchecked_func = new CodeFunction<TypeFnContextAndSizeT>{
    XSTR(code_get_nth_field_checked),
    [](LLVMContext &C, Type *T_size) {
        auto T_prcodevalue = CodeType::get_prcodevalue_ty(C);
        return FunctionType::get(T_prcodevalue,
            {T_prcodevalue, T_size}, false);
    },
    [](LLVMContext &C) { return AttributeList::get(C,
            AttributeSet(),
            Attributes(C, {Attribute::NonNull}),
...
static const auto codefieldindex_func = new CodeFunction<>{
    XSTR(code_field_index),
    [](LLVMContext &C) {
        auto T_prcodevalue = CodeType::get_prcodevalue_ty(C);
        return FunctionType::get(getInt32Ty(C),
            {T_prcodevalue, T_prcodevalue, getInt32Ty(C)}, false);
    },
    [](LLVMContext &C) {
        AttrBuilder FnAttrs(C);
#if CODE_LLVM_VERSION >= 160000
...
static const auto codefieldisdefinedchecked_func = new CodeFunction<TypeFnContextAndSizeT>{
    XSTR(code_field_isdefined_checked),
    [](LLVMContext &C, Type *T_size) {
        auto T_prcodevalue = CodeType::get_prcodevalue_ty(C);
        return FunctionType::get(getInt32Ty(C),
            {T_prcodevalue, T_size}, false);
...
static const auto codegetcfunctiontrampoline_func = new CodeFunction<>{
    XSTR(code_get_cfunction_trampoline),
    [](LLVMContext &C) {
        auto T_codevalue = CodeType::get_codevalue_ty(C);
        auto T_pcodevalue = PointerType::get(T_codevalue, 0);
        auto T_prcodevalue = PointerType::get(T_codevalue, AddressSpace::Tracked);
        auto T_ppcodevalue = PointerType::get(T_pcodevalue, 0);
        auto T_pprcodevalue = PointerType::get(T_prcodevalue, 0);
        return FunctionType::get(T_prcodevalue,
            {
                T_prcodevalue, // f (object)
                T_pcodevalue, // result
                getPointerTy(C), // cache
                T_pcodevalue, // fill
                FunctionType::get(getPointerTy(C), { getPointerTy(C), T_ppcodevalue }, false)->getPointerTo(), // trampoline
                T_pcodevalue, // env
                T_pprcodevalue, // vals
            }, false);
    },
...
static const auto diff_gc_total_bytes_func = new CodeFunction<>{
    XSTR(code_gc_diff_total_bytes),
    [](LLVMContext &C) { return FunctionType::get(getInt64Ty(C), false); },
    nullptr,
};
static const auto sync_gc_total_bytes_func = new CodeFunction<>{
    XSTR(code_gc_sync_total_bytes),
    [](LLVMContext &C) { return FunctionType::get(getInt64Ty(C),
            {getInt64Ty(C)}, false); },
    nullptr,
};
static const auto code_allocgenericmemory = new CodeFunction<TypeFnContextAndSizeT>{
    XSTR(code_alloc_genericmemory),
    [](LLVMContext &C, Type *T_Size) {
        auto T_prcodevalue = CodeType::get_prcodevalue_ty(C);
        return FunctionType::get(T_prcodevalue, // new Memory
                                {T_prcodevalue, // type
                                T_Size        // nelements
                                }, false); },
        [](LLVMContext &C) {
...
static const auto codearray_data_owner_func = new CodeFunction<>{
    XSTR(code_array_data_owner),
    [](LLVMContext &C) {
        auto T_prcodevalue = CodeType::get_prcodevalue_ty(C);
        return FunctionType::get(T_prcodevalue,
            {T_prcodevalue}, false);
...
static const auto box_##ct##_func = new CodeFunction<>{                           \
    XSTR(code_box_##ct),                                                           \
    [](LLVMContext &C) { return FunctionType::get(CodeType::get_prcodevalue_ty(C),\
            {at}, false); },                                                     \
...
static const auto box_ssavalue_func = new CodeFunction<TypeFnContextAndSizeT>{
    XSTR(code_box_ssavalue),
    [](LLVMContext &C, Type *T_size) {
        auto T_prcodevalue = CodeType::get_prcodevalue_ty(C);
        return FunctionType::get(T_prcodevalue,
            {T_size}, false);
...
static const auto codegetbuiltinfptr_func = new CodeFunction<>{
    XSTR(code_get_builtin_fptr),
    [](LLVMContext &C) { return FunctionType::get(get_func_sig(C)->getPointerTo(),
            {CodeType::get_prcodevalue_ty(C)}, false); },
    nullptr,
};


// placeholder functions
static const auto gcroot_flush_func = new CodeFunction<>{
    "julia.gcroot_flush",
    [](LLVMContext &C) { return FunctionType::get(getVoidTy(C), false); },
    nullptr,
};
static const auto gc_preserve_begin_func = new CodeFunction<>{
    "llvm.julia.gc_preserve_begin",
    [](LLVMContext &C) { return FunctionType::get(Type::getTokenTy(C), true); },
    nullptr,
};
static const auto gc_preserve_end_func = new CodeFunction<> {
    "llvm.julia.gc_preserve_end",
    [](LLVMContext &C) { return FunctionType::get(getVoidTy(C), {Type::getTokenTy(C)}, false); },
    nullptr,
};
static const auto except_enter_func = new CodeFunction<>{
    "julia.except_enter",
    [](LLVMContext &C) {
         auto T_pcodevalue = CodeType::get_pcodevalue_ty(C);
         auto RT = StructType::get(getInt32Ty(C), getPointerTy(C));
         return FunctionType::get(RT, {T_pcodevalue}, false); },
    [](LLVMContext &C) { return AttributeList::get(C,
            Attributes(C, {Attribute::ReturnsTwice}),
            AttributeSet(),
            None); },
};
static const auto pointer_from_objref_func = new CodeFunction<>{
    "julia.pointer_from_objref",
    [](LLVMContext &C) { return FunctionType::get(CodeType::get_pcodevalue_ty(C),
            {PointerType::get(CodeType::get_codevalue_ty(C), AddressSpace::Derived)}, false); },
    [](LLVMContext &C) {
        AttrBuilder FnAttrs(C);
#if CODE_LLVM_VERSION >= 160000
        FnAttrs.addMemoryAttr(MemoryEffects::none());
...
static const auto gc_loaded_func = new CodeFunction<>{
    "julia.gc_loaded",
    ...
    [](LLVMContext &C) { return FunctionType::get(PointerType::get(CodeType::get_prcodevalue_ty(C), AddressSpace::Loaded),
            {CodeType::get_prcodevalue_ty(C), PointerType::get(CodeType::get_prcodevalue_ty(C), 0)}, false); },
    [](LLVMContext &C) {
        AttrBuilder FnAttrs(C);
...
static const auto julia_call = new CodeFunction<>{
    "julia.call",
    [](LLVMContext &C) {
        auto T_prcodevalue = CodeType::get_prcodevalue_ty(C);
        return FunctionType::get(T_prcodevalue,
...
static const auto julia_call2 = new CodeFunction<>{
    "julia.call2",
    [](LLVMContext &C) {
        auto T_prcodevalue = CodeType::get_prcodevalue_ty(C);
        return FunctionType::get(T_prcodevalue,
...
static const auto julia_call3 = new CodeFunction<>{
    "julia.call3",
    [](LLVMContext &C) {
        auto T_prcodevalue = CodeType::get_prcodevalue_ty(C);
        Type *T = PointerType::get(CodeType::get_codevalue_ty(C), AddressSpace::Derived);
...
static const auto codetuple_func = new CodeFunction<>{XSTR(code_f_tuple), get_func_sig, get_func_attrs};
static const auto codeintrinsic_func = new CodeFunction<>{XSTR(code_f_intrinsic_call), get_func3_sig, get_func_attrs};

static const auto &builtin_func_map() {
    static std::map<code_fptr_args_t, CodeFunction<>*> builtins = {
          { code_f_is_addr,                 new CodeFunction<>{XSTR(code_f_is), get_func_sig, get_func_attrs} },
          { code_f_typeof_addr,             new CodeFunction<>{XSTR(code_f_typeof), get_func_sig, get_func_attrs} },
          { code_f_sizeof_addr,             new CodeFunction<>{XSTR(code_f_sizeof), get_func_sig, get_func_attrs} },
          { code_f_issubtype_addr,          new CodeFunction<>{XSTR(code_f_issubtype), get_func_sig, get_func_attrs} },
          { code_f_isa_addr,                new CodeFunction<>{XSTR(code_f_isa), get_func_sig, get_func_attrs} },
          { code_f_typeassert_addr,         new CodeFunction<>{XSTR(code_f_typeassert), get_func_sig, get_func_attrs} },
          { code_f_ifelse_addr,             new CodeFunction<>{XSTR(code_f_ifelse), get_func_sig, get_func_attrs} },
          { code_f__apply_iterate_addr,     new CodeFunction<>{XSTR(code_f__apply_iterate), get_func_sig, get_func_attrs} },
          { code_f__apply_pure_addr,        new CodeFunction<>{XSTR(code_f__apply_pure), get_func_sig, get_func_attrs} },
          { code_f__call_latest_addr,       new CodeFunction<>{XSTR(code_f__call_latest), get_func_sig, get_func_attrs} },
          { code_f__call_in_world_addr,     new CodeFunction<>{XSTR(code_f__call_in_world), get_func_sig, get_func_attrs} },
          { code_f__call_in_world_total_addr, new CodeFunction<>{XSTR(code_f__call_in_world_total), get_func_sig, get_func_attrs} },
          { code_f_throw_addr,              new CodeFunction<>{XSTR(code_f_throw), get_func_sig, get_func_attrs} },
          { code_f_tuple_addr,              codetuple_func },
          { code_f_svec_addr,               new CodeFunction<>{XSTR(code_f_svec), get_func_sig, get_func_attrs} },
          { code_f_applicable_addr,         new CodeFunction<>{XSTR(code_f_applicable), get_func_sig, get_func_attrs} },
          { code_f_invoke_addr,             new CodeFunction<>{XSTR(code_f_invoke), get_func_sig, get_func_attrs} },
          { code_f_isdefined_addr,          new CodeFunction<>{XSTR(code_f_isdefined), get_func_sig, get_func_attrs} },
          { code_f_getfield_addr,           new CodeFunction<>{XSTR(code_f_getfield), get_func_sig, get_func_attrs} },
          { code_f_setfield_addr,           new CodeFunction<>{XSTR(code_f_setfield), get_func_sig, get_func_attrs} },
          { code_f_swapfield_addr,          new CodeFunction<>{XSTR(code_f_swapfield), get_func_sig, get_func_attrs} },
          { code_f_modifyfield_addr,        new CodeFunction<>{XSTR(code_f_modifyfield), get_func_sig, get_func_attrs} },
          { code_f_fieldtype_addr,          new CodeFunction<>{XSTR(code_f_fieldtype), get_func_sig, get_func_attrs} },
          { code_f_nfields_addr,            new CodeFunction<>{XSTR(code_f_nfields), get_func_sig, get_func_attrs} },
          { code_f__expr_addr,              new CodeFunction<>{XSTR(code_f__expr), get_func_sig, get_func_attrs} },
          { code_f__typevar_addr,           new CodeFunction<>{XSTR(code_f__typevar), get_func_sig, get_func_attrs} },
          { code_f_memoryref_addr,          new CodeFunction<>{XSTR(code_f_memoryref), get_func_sig, get_func_attrs} },
          { code_f_memoryrefoffset_addr,    new CodeFunction<>{XSTR(code_f_memoryrefoffset), get_func_sig, get_func_attrs} },
          { code_f_memoryrefset_addr,       new CodeFunction<>{XSTR(code_f_memoryrefset), get_func_sig, get_func_attrs} },
          { code_f_memoryrefswap_addr,      new CodeFunction<>{XSTR(code_f_memoryrefswap), get_func_sig, get_func_attrs} },
          { code_f_memoryrefreplace_addr,   new CodeFunction<>{XSTR(code_f_memoryrefreplace), get_func_sig, get_func_attrs} },
          { code_f_memoryrefmodify_addr,    new CodeFunction<>{XSTR(code_f_memoryrefmodify), get_func_sig, get_func_attrs} },
          { code_f_memoryrefsetonce_addr,   new CodeFunction<>{XSTR(code_f_memoryrefsetonce), get_func_sig, get_func_attrs} },
          { code_f_memoryref_isassigned_addr,new CodeFunction<>{XSTR(code_f_memoryref_isassigned), get_func_sig, get_func_attrs} },
          { code_f_apply_type_addr,         new CodeFunction<>{XSTR(code_f_apply_type), get_func_sig, get_func_attrs} },
          { code_f_donotdelete_addr,        new CodeFunction<>{XSTR(code_f_donotdelete), get_donotdelete_sig, get_donotdelete_func_attrs} },
          { code_f_compilerbarrier_addr,    new CodeFunction<>{XSTR(code_f_compilerbarrier), get_func_sig, get_func_attrs} },
          { code_f_finalizer_addr,          new CodeFunction<>{XSTR(code_f_finalizer), get_func_sig, get_func_attrs} },
          { code_f__svec_ref_addr,          new CodeFunction<>{XSTR(code_f__svec_ref), get_func_sig, get_func_attrs} }
        };
    return builtins;
}

static const auto code_new_opaque_closure_codecall_func = new CodeFunction<>{XSTR(code_new_opaque_closure_codecall), get_func_sig, get_func_attrs};
...
extern "C" {
    code_cgparams_t code_default_cgparams = {
        /* track_allocations */ 1,
        /* code_coverage */ 1,
        /* prefer_specsig */ 0,
#ifdef _OS_WINDOWS_
        ...
        /* gcstack_arg */ 1,
        /* use_codeplt*/ 1,
        /* lookup */ code_rettype_inferred_addr };
}


static MDNode *best_tbaa(code_tbaacache_t &tbaa_cache, code_value_t *jt) {
    jt = code_unwrap_unionall(jt);
    if (jt == (code_value_t*)code_datatype_type ||
        (code_is_type_type(jt) && code_is_datatype(code_tparam0(jt))))
        return tbaa_cache.tbaa_datatype;
    if (!code_is_datatype(jt))
        return tbaa_cache.tbaa_value;
    if (code_is_abstracttype(jt))
        return tbaa_cache.tbaa_value;
    if (code_is_genericmemory_type(jt) || code_is_array_type(jt))
    ...
    return code_is_mutable(jt) ? tbaa_cache.tbaa_mutab : tbaa_cache.tbaa_immut;
}

// tracks whether codegen is currently able to simply stack-allocate this type
// note that this includes code_isbits, although codegen should work regardless
static bool code_is_concrete_immutable(code_value_t* t)
{
    return code_is_immutable_datatype(t) && ((code_datatype_t*)t)->isconcretetype;
}

static bool code_is_pointerfree(code_value_t* t)
{
    if (!code_is_concrete_immutable(t))
        return 0;
    const code_datatype_layout_t *layout = ((code_datatype_t*)t)->layout;
...
// n.b. this must include code_is_datatype_singleton (ghostType) and primitive types
static bool deserves_stack(code_value_t* t)
{
    if (!code_is_concrete_immutable(t))
        return false;
    code_datatype_t *dt = (code_datatype_t*)t;
    return code_is_datatype_singleton(dt) || code_datatype_isinlinealloc(dt, 0);
}
static bool deserves_argbox(code_value_t* t)
{
    return !deserves_stack(t);
}
static bool deserves_retbox(code_value_t* t)
{
    return deserves_argbox(t);
}
static bool deserves_sret(code_value_t *dt, Type *T)
{
    assert(code_is_datatype(dt));
    return (size_t)code_datatype_size(dt) > sizeof(void*) && !T->isFloatingPointTy() && !T->isVectorTy();
}

// Alias Analysis Info (analogous to llvm::AAMDNodes)
struct code_aliasinfo_t {
    ...
    enum class Region { unknown, gcframe, stack, data, constant, type_metadata }; // See code_regions_t

    explicit code_aliasinfo_t() = default;
    explicit code_aliasinfo_t(code_codectx_t &ctx, Region r, MDNode *tbaa);
    explicit code_aliasinfo_t(MDNode *tbaa, MDNode *tbaa_struct, MDNode *scope, MDNode *noalias)
        : tbaa(tbaa), tbaa_struct(tbaa_struct), scope(scope), noalias(noalias) {}
    code_aliasinfo_t(const code_aliasinfo_t &) = default;
    ...
    code_aliasinfo_t merge(const code_aliasinfo_t &other) const {
        code_aliasinfo_t result;
    ...
    static code_aliasinfo_t fromTBAA(code_codectx_t &ctx, MDNode *tbaa);
};
...
struct code_cgval_t {
    ...
    // If this is non-NULL (at compile time), it is always of type `T_prcodevalue`.
    ...
    code_value_t *constant; // constant value (rooted in linfo.def.roots)
    code_value_t *typ; // the original type of V, never NULL
    bool isboxed; // whether this value is a code_value_t* allocated on the heap with the right type tag
    ...
    code_cgval_t(Value *Vval, code_value_t *typ, Value *tindex) : // general value constructor
        V(Vval), // V is allowed to be NULL in a code_varinfo_t context, but not during codegen contexts
        Vboxed(nullptr),
        TIndex(tindex),
        constant(NULL),
        typ(typ),
    ...
    code_cgval_t(Value *Vptr, bool isboxed, code_value_t *typ, Value *tindex, MDNode *tbaa) : // general pointer constructor
        V(Vptr),
    ...
    {
        if (Vboxed)
            assert(Vboxed->getType() == CodeType::get_prcodevalue_ty(Vboxed->getContext()));
        assert(tbaa != NULL);
        assert(!(isboxed && TIndex != NULL));
        assert(TIndex == NULL || TIndex->getType() == getInt8Ty(TIndex->getContext()));
    }
    explicit code_cgval_t(code_value_t *typ) : // ghost value constructor
        // mark explicit to avoid being used implicitly for conversion from NULL (use code_cgval_t() instead)
        V(NULL),
        Vboxed(NULL),
        TIndex(NULL),
        constant(((code_datatype_t*)typ)->instance),
        typ(typ),
        isboxed(false),
        isghost(true),
        tbaa(nullptr),
        ...
        promotion_ssa(-1)
    {
        assert(code_is_datatype(typ));
        assert(constant);
    }
    code_cgval_t(const code_cgval_t &v, code_value_t *typ, Value *tindex) : // copy constructor with new type
        V(v.V),
        Vboxed(v.Vboxed),
        TIndex(tindex),
        constant(v.constant),
        ...
        promotion_ssa(v.promotion_ssa)
    {
        ...
        if (v.TIndex) {
            assert((TIndex == NULL) == code_is_concrete_type(typ));
        }
        else {
            assert(isboxed || v.typ == typ || tindex);
        }
    }
    explicit code_cgval_t() : // undef / unreachable constructor
        V(NULL),
        Vboxed(NULL),
        TIndex(NULL),
        constant(NULL),
        typ(code_bottom_type),
        isboxed(false),
        isghost(true),
        tbaa(nullptr),
        promotion_point(nullptr),
...
struct code_varinfo_t {
    Instruction *boxroot; // an address, if the var might be in a code_value_t** stack slot (marked ctx.tbaa().tbaa_const, if appropriate)
    code_cgval_t value; // a stack slot or constant value
    ...
    code_varinfo_t(LLVMContext &ctxt) : boxroot(NULL),
                     value(code_cgval_t()),
                     pTIndex(NULL),
                     dinfo(NULL),
                     defFlag(NULL),
...
public:
    ...
    llvm::MapVector<code_code_instance_t*, code_codegen_call_target_t> call_targets;
    Function *f = NULL;
    MDNode* LoopID = NULL;
    // local var info. globals are not in here.
    SmallVector<code_varinfo_t, 0> slots;
    std::map<int, code_varinfo_t> phic_slots;
    std::map<int, std::pair<Value*, Value*> > scope_restore;
    SmallVector<code_cgval_t, 0> SAvalues;
    SmallVector<std::tuple<code_cgval_t, BasicBlock *, AllocaInst *, PHINode *, code_value_t *>, 0> PhiNodes;
    SmallVector<bool, 0> ssavalue_assigned;
    SmallVector<int, 0> ssavalue_usecount;
    code_module_t *module = NULL;
    code_typecache_t type_cache;
    code_tbaacache_t tbaa_cache;
    code_noaliascache_t aliasscope_cache;
    code_method_instance_t *linfo = NULL;
    code_value_t *rettype = NULL;
    code_code_info_t *source = NULL;
    code_array_t *code = NULL;
    size_t min_world = 0;
    size_t max_world = -1;
    ...
    const code_cgparams_t *params = NULL;

    SmallVector<std::unique_ptr<Module>, 0> llvmcall_modules;

    code_codectx_t(LLVMContext &llvmctx, code_codegen_params_t &params, size_t min_world, size_t max_world)
      : builder(llvmctx),
    ...
    code_codectx_t(LLVMContext &llvmctx, code_codegen_params_t &params, code_code_instance_t *ci) :
        code_codectx_t(llvmctx, params, code_atomic_load_relaxed(&ci->min_world), code_atomic_load_relaxed(&ci->max_world)) {}

    code_typecache_t &types() {
        type_cache.initialize(builder.getContext(), emission_context.DL);
        return type_cache;
    }

    code_tbaacache_t &tbaa() {
        tbaa_cache.initialize(builder.getContext());
        return tbaa_cache;
    }

    code_noaliascache_t &noalias() {
...
GlobalVariable *CodeVariable::realize(code_codectx_t &ctx) {
    return realize(code_Module);
}

code_aliasinfo_t::code_aliasinfo_t(code_codectx_t &ctx, Region r, MDNode *tbaa): tbaa(tbaa), tbaa_struct(nullptr) {
    MDNode *alias_scope = nullptr;
    code_noaliascache_t::code_regions_t regions = ctx.noalias().regions;
    switch (r) {
        case Region::unknown:
            alias_scope = nullptr;
            break;
...
code_aliasinfo_t code_aliasinfo_t::fromTBAA(code_codectx_t &ctx, MDNode *tbaa) {
    auto cache = ctx.tbaa();
        ...
        if (cast<MDString>(node->getOperand(0))->getString() != "jtbaa") {
            ...
            for (int i = 0; i < 5; i++) {
                if (cast<MDNode>(tbaa_srcs[i]->getOperand(1)) == node)
                    return code_aliasinfo_t(ctx, regions[i], tbaa);
            }
        }
    }

    return code_aliasinfo_t(ctx, Region::unknown, tbaa);
}

static Type *julia_type_to_llvm(code_codectx_t &ctx, code_value_t *jt, bool *isboxed = NULL);
static code_returninfo_t get_specsig_function(code_codectx_t &ctx, Module *M, Value *fval, StringRef name, code_value_t *sig, code_value_t *coderettype, bool is_opaque_closure, bool gcstack_arg, BitVector *used_arguments=nullptr, size_t *args_begin=nullptr);
static code_cgval_t emit_expr(code_codectx_t &ctx, code_value_t *expr, ssize_t ssaval = -1);
static Value *global_binding_pointer(code_codectx_t &ctx, code_module_t *m, code_sym_t *s,
                                     code_binding_t **pbnd, bool assign, bool alloc);
static code_cgval_t emit_checked_var(code_codectx_t &ctx, Value *bp, code_sym_t *name, code_value_t *scope, bool isvol, MDNode *tbaa);
static code_cgval_t emit_sparam(code_codectx_t &ctx, size_t i);
static Value *emit_condition(code_codectx_t &ctx, const code_cgval_t &condV, const Twine &msg);
static Value *get_current_task(code_codectx_t &ctx);
static Value *get_current_ptls(code_codectx_t &ctx);
static Value *get_tls_world_age_field(code_codectx_t &ctx);
static void CreateTrap(IRBuilder<> &irbuilder, bool create_new_block = true);
static CallInst *emit_codecall(code_codectx_t &ctx, FunctionCallee theFptr, Value *theF,
                             ArrayRef<code_cgval_t> args, size_t nargs, CodeFunction<> *trampoline);
static CallInst *emit_codecall(code_codectx_t &ctx, CodeFunction<> *theFptr, Value *theF,
                             ArrayRef<code_cgval_t> args, size_t nargs, CodeFunction<> *trampoline);
static Value *emit_f_is(code_codectx_t &ctx, const code_cgval_t &arg1, const code_cgval_t &arg2,
                        Value *nullcheck1 = nullptr, Value *nullcheck2 = nullptr);
static code_cgval_t emit_new_struct(code_codectx_t &ctx, code_value_t *ty, size_t nargs, ArrayRef<code_cgval_t> argv, bool is_promotable=false);
static code_cgval_t emit_invoke(code_codectx_t &ctx, const code_cgval_t &lival, ArrayRef<code_cgval_t> argv, size_t nargs, code_value_t *rt);

static Value *literal_pointer_val(code_codectx_t &ctx, code_value_t *p);
static unsigned julia_alignment(code_value_t *jt);

static GlobalVariable *prepare_global_in(Module *M, CodeVariable *G)
{
    return G->realize(M);
}

template<typename TypeFn_t>
static Function *prepare_call_in(Module *M, CodeFunction<TypeFn_t> *G)
{
    return G->realize(M);
}
...
static GlobalVariable *get_pointer_to_constant(code_codegen_params_t &emission_context, Constant *val, Align align, const Twine &name, Module &M)
{
...
static AllocaInst *emit_static_alloca(code_codectx_t &ctx, Type *lty)
{
    ++EmittedAllocas;
    return new AllocaInst(lty, ctx.topalloca->getModule()->getDataLayout().getAllocaAddrSpace(), "", /*InsertBefore=*/ctx.topalloca);
}

static void undef_derived_strct(code_codectx_t &ctx, Value *ptr, code_datatype_t *sty, MDNode *tbaa)
{
    assert(ptr->getType()->getPointerAddressSpace() != AddressSpace::Tracked);
    size_t first_offset = sty->layout->nfields ? code_field_offset(sty, 0) : 0;
    if (first_offset != 0)
    ...
    auto T_prcodevalue = CodeType::get_prcodevalue_ty(ctx.builder.getContext());
    for (i = 0; i < np; i++) {
        Value *fld = ctx.builder.CreateConstInBoundsGEP1_32(T_prcodevalue, ptr, code_ptr_offset(sty, i));
        code_aliasinfo_t ai = code_aliasinfo_t::fromTBAA(ctx, tbaa);
        ai.decorateInst(ctx.builder.CreateStore(Constant::getNullValue(T_prcodevalue), fld));
    }
}

static Value *emit_inttoptr(code_codectx_t &ctx, Value *v, Type *ty)
{
...
static inline code_cgval_t ghostValue(code_codectx_t &ctx, code_value_t *typ)
{
    if (typ == code_bottom_type)
        return code_cgval_t(); // Undef{}
    if (typ == (code_value_t*)code_typeofbottom_type) {
        // normalize TypeofBottom to Type{Union{}}
        typ = (code_value_t*)code_typeofbottom_type->super;
    }
    if (code_is_type_type(typ)) {
        assert(is_uniquerep_Type(typ));
        // replace T::Type{T} with T, by assuming that T must be a leaftype of some sort
        code_cgval_t constant(NULL, true, typ, NULL, best_tbaa(ctx.tbaa(), typ));
        constant.constant = code_tparam0(typ);
        return constant;
    }
    return code_cgval_t(typ);
}
static inline code_cgval_t ghostValue(code_codectx_t &ctx, code_datatype_t *typ)
{
    return ghostValue(ctx, (code_value_t*)typ);
}

static inline code_cgval_t mark_julia_const(code_codectx_t &ctx, code_value_t *jv)
{
    code_value_t *typ;
    if (code_is_type(jv)) {
        typ = (code_value_t*)code_wrap_Type(jv); // TODO: gc-root this?
    }
    else {
        typ = code_typeof(jv);
        if (code_is_datatype_singleton((code_datatype_t*)typ))
...
static inline code_cgval_t mark_julia_slot(Value *v, code_value_t *typ, Value *tindex, MDNode *tbaa)
{
    // this enables lazy-copying of immutable values and stack or argument slots
    code_cgval_t tagval(v, false, typ, tindex, tbaa);
    return tagval;
}
...
static Value *zext_struct(code_codectx_t &ctx, Value *V);

static inline code_cgval_t value_to_pointer(code_codectx_t &ctx, Value *v, code_value_t *typ, Value *tindex)
{
    Value *loc;
    v = zext_struct(ctx, v);
    if (valid_as_globalinit(v)) { // llvm can't handle all the things that could be inside a ConstantExpr
        assert(code_is_concrete_type(typ)); // not legal to have an unboxed abstract type
        loc = get_pointer_to_constant(ctx.emission_context, cast<Constant>(v), Align(julia_alignment(typ)), "_j_const", *code_Module);
    }
    else {
...
static inline code_cgval_t value_to_pointer(code_codectx_t &ctx, const code_cgval_t &v)
{
    if (v.ispointer())
        return v;
...
static inline code_cgval_t mark_julia_type(code_codectx_t &ctx, Value *v, bool isboxed, code_value_t *typ)
{
    if (code_is_type_type(typ)) {
    ...
    } else if (code_is_datatype(typ) && code_is_datatype_singleton((code_datatype_t*)typ)) {
    ...
    if (isboxed)
        return code_cgval_t(v, isboxed, typ, NULL, best_tbaa(ctx.tbaa(), typ));
    return code_cgval_t(v, typ, NULL);
}

static inline code_cgval_t mark_julia_type(code_codectx_t &ctx, Value *v, bool isboxed, code_datatype_t *typ)
{
    return mark_julia_type(ctx, v, isboxed, (code_value_t*)typ);
}

// see if it might be profitable (and cheap) to change the type of v to typ
static inline code_cgval_t update_julia_type(code_codectx_t &ctx, const code_cgval_t &v, code_value_t *typ)
{
    if (v.typ == code_bottom_type || typ == (code_value_t*)code_any_type || code_egal(v.typ, typ))
        return v; // fast-path
    if (v.constant)
        return code_isa(v.constant, typ) ? v : code_cgval_t();
    if (code_is_concrete_type(v.typ) && !code_is_kind(v.typ)) {
        if (code_is_concrete_type(typ) && !code_is_kind(typ)) {
    ...
    if (v.TIndex) {
        code_value_t *utyp = code_unwrap_unionall(typ);
        if (code_is_datatype(utyp)) {
            bool alwaysboxed;
            if (code_is_concrete_type(utyp))
                alwaysboxed = !code_is_pointerfree(utyp);
            else
                alwaysboxed = !((code_datatype_t*)utyp)->name->abstract && ((code_datatype_t*)utyp)->name->mutabl;
            if (alwaysboxed) {
                // discovered that this union-split type must actually be isboxed
                if (v.Vboxed) {
                    return code_cgval_t(v.Vboxed, true, typ, NULL, best_tbaa(ctx.tbaa(), typ));
                }
                else {
...
static code_cgval_t convert_julia_type(code_codectx_t &ctx, const code_cgval_t &v, code_value_t *typ, Value **skip=nullptr);

// --- allocating local variables ---

static code_sym_t *slot_symbol(code_codectx_t &ctx, int s)
{
    return (code_sym_t*)code_array_ptr_ref(ctx.source->slotnames, s);
}

static void store_def_flag(code_codectx_t &ctx, const code_varinfo_t &vi, bool val)
{
...
static void alloc_def_flag(code_codectx_t &ctx, code_varinfo_t& vi)
{
    assert((!vi.boxroot || vi.pTIndex) && "undef check is null pointer for boxed things");
    if (vi.usedUndef) {
...
static code_cgval_t convert_julia_type_union(code_codectx_t &ctx, const code_cgval_t &v, code_value_t *typ, Value **skip)
{
    ...
    if (code_is_uniontype(typ)) {
        ...
        for_each_uniontype_small(
            ...
            [&](unsigned idx, code_datatype_t *jt) {
                ...
                else if (!code_subtype((code_value_t*)jt, typ)) {
    ...
    if (!isa<Constant>(new_tindex)) {
        ...
        if (!skip_box.all()) {
            ...
            if (v.V == NULL) {
                ...
                code_cgval_t oldv = value_to_pointer(ctx, v);
                slotv = oldv.V;
                tbaa = oldv.tbaa;
            ...
            code_cgval_t newv = code_cgval_t(slotv, false, typ, new_tindex, tbaa);
            assert(boxv->getType() == ctx.types().T_prcodevalue);
            newv.Vboxed = boxv;
            return newv;
        }
    }
    else {
        return code_cgval_t(boxed(ctx, v), true, typ, NULL, best_tbaa(ctx.tbaa(), typ));
    }
    return code_cgval_t(v, typ, new_tindex);
}
...
static code_cgval_t convert_julia_type(code_codectx_t &ctx, const code_cgval_t &v, code_value_t *typ, Value **skip)
{
    if (typ == (code_value_t*)code_typeofbottom_type)
        return ghostValue(ctx, typ); // normalize TypeofBottom to Type{Union{}}
    if (v.typ == code_bottom_type || code_egal(v.typ, typ))
        return v; // fast-path
    Type *T = julia_type_to_llvm(ctx, typ);
    if (type_is_ghost(T))
        return ghostValue(ctx, typ);
    ...
    if (code_is_concrete_type(typ)) {
        if (code_is_concrete_type(v.typ)) {
            // type mismatch: changing from one leaftype to another
            if (skip)
        ...
        bool mustbox_union = v.TIndex && !code_is_pointerfree(typ);
        if (v.Vboxed && (v.isboxed || mustbox_union)) {
            if (skip) {
                *skip = ctx.builder.CreateNot(emit_exactly_isa(ctx, v, (code_datatype_t*)typ, true));
            }
            return code_cgval_t(v.Vboxed, true, typ, NULL, best_tbaa(ctx.tbaa(), typ));
        }
        if (mustbox_union) {
        ...
        else if (!v.isboxed && code_is_uniontype(typ)) {
            // previous value was unboxed (leaftype), statically compute union tindex
            assert(code_is_concrete_type(v.typ));
            unsigned new_idx = get_box_tindex((code_datatype_t*)v.typ, typ);
            if (new_idx) {
                ...
                if (v.V && !v.ispointer()) {
                    ...
                    return code_cgval_t(value_to_pointer(ctx, v), typ, new_tindex);
                }
            }
            else if (code_subtype(v.typ, typ)) {
                makeboxed = true;
            }
            else if (skip) {
                // undef
...
std::unique_ptr<Module> code_create_llvm_module(StringRef name, LLVMContext &context, const DataLayout &DL, const Triple &triple)
{
    ++ModulesCreated;
...
#if defined(CODE_DEBUG_BUILD)
    m->setStackProtectorGuard("global");
#endif
    return m;
}

static void code_name_codefunc_args(code_codegen_params_t &params, Function *F) {
    assert(F->arg_size() == 3);
...
static void code_name_codefuncparams_args(code_codegen_params_t &params, Function *F) {
    assert(F->arg_size() == 4);
    F->getArg(0)->setName("function::Core.Function");
    F->getArg(1)->setName("args::Any[]");
    F->getArg(2)->setName("nargs::UInt32");
...
static void code_init_function(Function *F, const Triple &TT)
{
...
static bool uses_specsig(code_value_t *sig, bool needsparams, code_value_t *rettype, bool prefer_specsig)
{
    if (needsparams)
        return false;
    if (sig == (code_value_t*)code_anytuple_type)
        return false;
    if (!code_is_datatype(sig))
        return false;
    if (code_nparams(sig) == 0)
        return false;
    if (code_vararg_kind(code_tparam(sig, code_nparams(sig) - 1)) == CODE_VARARG_UNBOUND)
        return false;
    // not invalid, consider if specialized signature is worthwhile
    if (prefer_specsig)
        return true;
    if (!deserves_retbox(rettype) && !code_is_datatype_singleton((code_datatype_t*)rettype) && rettype != (code_value_t*)code_bool_type)
        return true;
    if (code_is_uniontype(rettype)) {
        bool allunbox;
        size_t nbytes, align, min_align;
        union_alloca_type((code_uniontype_t*)rettype, allunbox, nbytes, align, min_align);
        if (nbytes > 0)
            return true; // some elements of the union could be returned unboxed avoiding allocation
    }
    if (code_nparams(sig) <= 3) // few parameters == more efficient to pass directly
        return true;
    bool allSingleton = true;
    for (size_t i = 0; i < code_nparams(sig); i++) {
        code_value_t *sigt = code_tparam(sig, i);
        bool issing = code_is_datatype(sigt) && code_is_datatype_singleton((code_datatype_t*)sigt);
        allSingleton &= issing;
    ...
    if (allSingleton)
        return true;
    return false; // codecall sig won't require any box allocations
}

static std::pair<bool, bool> uses_specsig(code_method_instance_t *lam, code_value_t *rettype, bool prefer_specsig)
{
    code_value_t *sig = lam->specTypes;
    bool needsparams = false;
    if (code_is_method(lam->def.method)) {
        if ((size_t)code_subtype_env_size(lam->def.method->sig) != code_svec_len(lam->sparam_vals))
            needsparams = true;
        for (size_t i = 0; i < code_svec_len(lam->sparam_vals); ++i) {
            if (code_is_typevar(code_svecref(lam->sparam_vals, i)))
                needsparams = true;
        }
    }
...
CODE_DLLEXPORT void code_coverage_alloc_line(StringRef filename, int line);
CODE_DLLEXPORT uint64_t *code_coverage_data_pointer(StringRef filename, int line);
CODE_DLLEXPORT uint64_t *code_malloc_data_pointer(StringRef filename, int line);

static void visitLine(code_codectx_t &ctx, uint64_t *ptr, Value *addend, const char *name)
{
...
static void coverageVisitLine(code_codectx_t &ctx, StringRef filename, int line)
{
    if (ctx.emission_context.imaging_mode)
        return; // TODO
    if (filename == "" || filename == "none" || filename == "no file" || filename == "<missing>" || line < 0)
        return;
    visitLine(ctx, code_coverage_data_pointer(filename, line), ConstantInt::get(getInt64Ty(ctx.builder.getContext()), 1), "lcnt");
}

// Memory allocation log (malloc_log)

static void mallocVisitLine(code_codectx_t &ctx, StringRef filename, int line, Value *sync)
{
    if (ctx.emission_context.imaging_mode)
        return; // TODO
    if (filename == "" || filename == "none" || filename == "no file" || filename == "<missing>" || line < 0)
    ...
    visitLine(ctx, code_malloc_data_pointer(filename, line), addend, "bytecnt");
}

// --- constant determination ---

static void show_source_loc(code_codectx_t &ctx, CODE_STREAM *out)
{
    code_printf(out, "in %s at %s", ctx.name, ctx.file.str().c_str());
}

static void cg_bdw(code_codectx_t &ctx, code_sym_t *var, code_binding_t *b)
{
    code_binding_deprecation_warning(ctx.module, var, b);
    if (b->deprecated == 1 && code_options.depwarn) {
        show_source_loc(ctx, CODE_STDERR);
        code_printf(CODE_STDERR, "\n");
    }
}

static code_value_t *static_apply_type(code_codectx_t &ctx, ArrayRef<code_cgval_t> args, size_t nargs)
{
    assert(nargs > 1);
    SmallVector<code_value_t *, 0> v(nargs);
    for (size_t i = 0; i < nargs; i++) {
        if (!args[i].constant)
            return NULL;
        v[i] = args[i].constant;
    }
    assert(v[0] == code_builtin_apply_type);
    size_t last_age = code_current_task->world_age;
    // call apply_type, but ignore errors. we know that will work in world 1.
    code_current_task->world_age = 1;
    code_value_t *result;
    CODE_TRY {
        result = code_apply(v.data(), nargs);
    }
    CODE_CATCH {
        result = NULL;
    }
    code_current_task->world_age = last_age;
    return result;
}
...
static code_value_t *static_eval(code_codectx_t &ctx, code_value_t *ex)
{
    if (code_is_symbol(ex)) {
        code_sym_t *sym = (code_sym_t*)ex;
        if (code_is_const(ctx.module, sym))
            return code_get_global(ctx.module, sym);
        return NULL;
    }
    if (code_is_slotnumber(ex) || code_is_argument(ex))
        return NULL;
    if (code_is_ssavalue(ex)) {
        ssize_t idx = ((code_ssavalue_t*)ex)->id - 1;
        assert(idx >= 0);
    ...
    if (code_is_quotenode(ex))
        return code_fieldref(ex, 0);
    if (code_is_method_instance(ex))
        return NULL;
    code_module_t *m = NULL;
    code_sym_t *s = NULL;
    if (code_is_globalref(ex)) {
        s = code_globalref_name(ex);
        code_binding_t *b = code_get_binding(code_globalref_mod(ex), s);
        if (b && b->constp) {
            if (b->deprecated)
                cg_bdw(ctx, s, b);
            return code_atomic_load_relaxed(&b->value);
        }
        return NULL;
    }
    if (code_is_expr(ex)) {
        code_expr_t *e = (code_expr_t*)ex;
        if (e->head == code_call_sym) {
            code_value_t *f = static_eval(ctx, code_exprarg(e, 0));
            if (f) {
                if (code_array_dim0(e->args) == 3 && (f == code_builtin_getfield || f == code_builtin_getglobal)) {
                    m = (code_module_t*)static_eval(ctx, code_exprarg(e, 1));
                    // Check the tag before evaluating `s` so that a value of random
                    // type won't be corrupted.
                    if (!m || !code_is_module(m))
                        return NULL;
                    // Assumes that the module is rooted somewhere.
                    s = (code_sym_t*)static_eval(ctx, code_exprarg(e, 2));
                    if (s && code_is_symbol(s)) {
                        code_binding_t *b = code_get_binding(m, s);
                        if (b && b->constp) {
                            if (b->deprecated)
                                cg_bdw(ctx, s, b);
                            return code_atomic_load_relaxed(&b->value);
                ...
                else if (f==code_builtin_tuple || f==code_builtin_apply_type) {
                    size_t i;
                    size_t n = code_array_dim0(e->args)-1;
                    if (n==0 && f==code_builtin_tuple) return (code_value_t*)code_emptytuple;
                    code_value_t **v;
                    CODE_GC_PUSHARGS(v, n+1);
                    v[0] = f;
                    for (i = 0; i < n; i++) {
                        v[i+1] = static_eval(ctx, code_exprarg(e, i+1));
                        if (v[i+1] == NULL) {
                            CODE_GC_POP();
                            return NULL;
                        }
        ...
        else if (e->head == code_static_parameter_sym) {
            size_t idx = code_unbox_long(code_exprarg(e, 0));
            if (idx <= code_svec_len(ctx.linfo->sparam_vals)) {
                code_value_t *e = code_svecref(ctx.linfo->sparam_vals, idx - 1);
                if (code_is_typevar(e))
                    return NULL;
                return e;
            }
        }
...
static bool slot_eq(code_value_t *e, int sl)
{
    return (code_is_slotnumber(e) || code_is_argument(e)) && code_slot_number(e)-1 == sl;
}
...
static bool local_var_occurs(code_value_t *e, int sl)
{
    if (slot_eq(e, sl)) {
        return true;
    }
    else if (code_is_expr(e)) {
        code_expr_t *ex = (code_expr_t*)e;
        size_t alength = code_array_dim0(ex->args);
        for(int i=0; i < (int)alength; i++) {
            if (local_var_occurs(code_exprarg(ex,i),sl))
                return true;
        }
    }
    else if (code_is_returnnode(e)) {
        code_value_t *retexpr = code_returnnode_value(e);
        if (retexpr != NULL)
            return local_var_occurs(retexpr, sl);
    }
    else if (code_is_gotoifnot(e)) {
        return local_var_occurs(code_gotoifnot_cond(e), sl);
    }
    return false;
}

static std::set<int> assigned_in_try(code_array_t *stmts, int s, long l)
{
    std::set<int> av;
    for(int i=s; i < l; i++) {
        code_value_t *st = code_array_ptr_ref(stmts,i);
        if (code_is_expr(st)) {
            if (((code_expr_t*)st)->head == code_assign_sym) {
                code_value_t *ar = code_exprarg(st, 0);
                if (code_is_slotnumber(ar)) {
                    av.insert(code_slot_number(ar)-1);
                }
            }
        }
    }
...
static void mark_volatile_vars(code_array_t *stmts, SmallVectorImpl<code_varinfo_t> &slots)
{
    size_t slength = code_array_dim0(stmts);
    for (int i = 0; i < (int)slength; i++) {
        code_value_t *st = code_array_ptr_ref(stmts, i);
        if (code_is_enternode(st)) {
            int last = code_enternode_catch_dest(st);
            if (last == 0)
                continue;
            ...
            for (int j = 0; j < (int)slength; j++) {
                if (j < i || j > last) {
                    ...
                    for (; it != as.end(); it++) {
                        if (local_var_occurs(code_array_ptr_ref(stmts, j), *it)) {
                            code_varinfo_t &vi = slots[*it];
                            vi.isVolatile = true;
                        }
                    }
                }
...
static void general_use_analysis(code_codectx_t &ctx, code_value_t *expr, callback &f)
{
    if (f(expr)) {
        return;
    }
    else if (code_is_expr(expr)) {
        code_expr_t *e = (code_expr_t*)expr;
        if (e->head == code_method_sym) {
            general_use_analysis(ctx, code_exprarg(e, 0), f);
            if (code_expr_nargs(e) > 1) {
                general_use_analysis(ctx, code_exprarg(e, 1), f);
                general_use_analysis(ctx, code_exprarg(e, 2), f);
            }
        }
        else if (e->head == code_assign_sym) {
            // don't consider assignment LHS as a variable "use"
            general_use_analysis(ctx, code_exprarg(e, 1), f);
        }
        else {
            size_t i, elen = code_array_dim0(e->args);
            for (i = 0; i < elen; i++) {
                general_use_analysis(ctx, code_exprarg(e, i), f);
            }
        }
    }
    else if (code_is_returnnode(expr)) {
        code_value_t *retexpr = code_returnnode_value(expr);
        if (retexpr != NULL)
            general_use_analysis(ctx, retexpr, f);
    }
    else if (code_is_gotoifnot(expr)) {
        general_use_analysis(ctx, code_gotoifnot_cond(expr), f);
    }
    else if (code_is_pinode(expr)) {
        general_use_analysis(ctx, code_fieldref_noalloc(expr, 0), f);
    }
    else if (code_is_upsilonnode(expr)) {
        code_value_t *val = code_fieldref_noalloc(expr, 0);
        if (val)
            general_use_analysis(ctx, val, f);
    }
    else if (code_is_phicnode(expr)) {
        code_array_t *values = (code_array_t*)code_fieldref_noalloc(expr, 0);
        size_t i, elen = code_array_nrows(values);
        for (i = 0; i < elen; i++) {
            code_value_t *v = code_array_ptr_ref(values, i);
            general_use_analysis(ctx, v, f);
        }
    }
    else if (code_is_phinode(expr)) {
...
static void simple_use_analysis(code_codectx_t &ctx, code_value_t *expr)
{
    auto scan_slot_arg = [&](code_value_t *expr) {
        if (code_is_slotnumber(expr) || code_is_argument(expr)) {
            int i = code_slot_number(expr) - 1;
...
static code_value_t *code_ensure_rooted(code_codectx_t &ctx, code_value_t *val)
{
    if (code_is_globally_rooted(val))
        return val;
    code_method_t *m = ctx.linfo->def.method;
    if (!code_options.strip_ir && code_is_method(m)) {
        // the method might have a root for this already; use it if so
        CODE_LOCK(&m->writelock);
        if (m->roots) {
            size_t i, len = code_array_dim0(m->roots);
            for (i = 0; i < len; i++) {
                code_value_t *mval = code_array_ptr_ref(m->roots, i);
                if (mval == val || code_egal(mval, val)) {
                    CODE_UNLOCK(&m->writelock);
                    return mval;
                }
            }
        }
    ...
    return code_as_global_root(val, 1);
}

// --- generating function calls ---

static code_cgval_t emit_globalref(code_codectx_t &ctx, code_module_t *mod, code_sym_t *name, AtomicOrdering order)
{
    code_binding_t *bnd = NULL;
    Value *bp = global_binding_pointer(ctx, mod, name, &bnd, false, false);
    if (bp == NULL)
        return code_cgval_t();
    bp = julia_binding_pvalue(ctx, bp);
    code_value_t *ty = nullptr;
    if (bnd) {
        code_value_t *v = code_atomic_load_acquire(&bnd->value); // acquire value for ty
        if (v != NULL && bnd->constp)
            return mark_julia_const(ctx, v);
        ty = code_atomic_load_relaxed(&bnd->ty);
    }
    if (ty == nullptr)
        ty = (code_value_t*)code_any_type;
    return update_julia_type(ctx, emit_checked_var(ctx, bp, name, (code_value_t*)mod, false, ctx.tbaa().tbaa_binding), ty);
}

static code_cgval_t emit_globalop(code_codectx_t &ctx, code_module_t *mod, code_sym_t *sym, code_cgval_t rval, const code_cgval_t &cmp,
                                ...
                                const code_cgval_t *modifyop, bool alloc)
{
    code_binding_t *bnd = NULL;
    ...
    if (bp == NULL)
        return code_cgval_t();
    if (bnd && !bnd->constp) {
        code_value_t *ty = code_atomic_load_relaxed(&bnd->ty);
        if (ty != nullptr) {
            ...
            if (!ismodifyglobal) {
                // TODO: use typeassert in code_check_binding_wr too
                emit_typecheck(ctx, rval, ty, "typeassert");
                rval = update_julia_type(ctx, rval, ty);
                if (rval.typ == code_bottom_type)
                    return code_cgval_t();
            }
            bool isboxed = true;
            bool maybe_null = code_atomic_load_relaxed(&bnd->value) == NULL;
    ...
    Value *m = literal_pointer_val(ctx, (code_value_t*)mod);
    Value *s = literal_pointer_val(ctx, (code_value_t*)sym);
    if (issetglobal) {
        ctx.builder.CreateCall(prepare_call(codecheckassign_func),
                { bp, m, s, mark_callee_rooted(ctx, boxed(ctx, rval)) });
        return rval;
    }
    else if (isreplaceglobal) {
        Value *r = ctx.builder.CreateCall(prepare_call(codecheckreplace_func),
                { bp, m, s, boxed(ctx, cmp), boxed(ctx, rval) });
        return mark_julia_type(ctx, r, true, code_any_type);
    }
    else if (isswapglobal) {
        Value *r = ctx.builder.CreateCall(prepare_call(codecheckswap_func),
                { bp, m, s, mark_callee_rooted(ctx, boxed(ctx, rval)) });
        return mark_julia_type(ctx, r, true, code_any_type);
    }
    else if (ismodifyglobal) {
        Value *r = ctx.builder.CreateCall(prepare_call(codecheckmodify_func),
                { bp, m, s, boxed(ctx, cmp), boxed(ctx, rval) });
        return mark_julia_type(ctx, r, true, code_any_type);
    }
    else if (issetglobalonce) {
        Value *r = ctx.builder.CreateCall(prepare_call(codecheckassignonce_func),
                { bp, m, s, mark_callee_rooted(ctx, boxed(ctx, rval)) });
        return mark_julia_type(ctx, r, true, code_bool_type);
    }
    abort(); // unreachable
}

static Value *emit_box_compare(code_codectx_t &ctx, const code_cgval_t &arg1, const code_cgval_t &arg2,
                               Value *nullcheck1, Value *nullcheck2)
{
    ++EmittedBoxCompares;
    if (code_pointer_egal(arg1.typ) || code_pointer_egal(arg2.typ)) {
    ...
    return emit_nullcheck_guard2(ctx, nullcheck1, nullcheck2, [&] {
        ...
        return emit_guarded_test(ctx, neq, true, [&] {
            ...
            return emit_guarded_test(ctx, dt_eq, false, [&] {
                return ctx.builder.CreateTrunc(ctx.builder.CreateCall(prepare_call(codeegalx_func),
...
static Value *emit_bits_compare(code_codectx_t &ctx, code_cgval_t arg1, code_cgval_t arg2);

static Value *emit_bitsunion_compare(code_codectx_t &ctx, const code_cgval_t &arg1, const code_cgval_t &arg2)
{
    ++EmittedBitsUnionCompares;
    assert(code_egal(arg1.typ, arg2.typ) && arg1.TIndex && arg2.TIndex && code_is_uniontype(arg1.typ) && "unimplemented");
    ...
    bool allunboxed = for_each_uniontype_small(
        [&](unsigned idx, code_datatype_t *jt) {
            ...
            code_cgval_t sel_arg1(arg1, (code_value_t*)jt, NULL);
            code_cgval_t sel_arg2(arg2, (code_value_t*)jt, NULL);
...
template <typename callback>
static size_t emit_masked_bits_compare(callback &emit_desc, code_datatype_t *aty, egal_desc &current_desc)
{
    // Memcmp, but with masked padding
    size_t data_bytes = 0;
    size_t padding_bytes = 0;
    size_t nfields = code_datatype_nfields(aty);
    size_t total_size = code_datatype_size(aty);
    for (size_t i = 0; i < nfields; ++i) {
        size_t offset = code_field_offset(aty, i);
        size_t fend = i == nfields - 1 ? total_size : code_field_offset(aty, i + 1);
        size_t fsz = code_field_size(aty, i);
        code_datatype_t *fty = (code_datatype_t*)code_field_type(aty, i);
        if (code_field_isptr(aty, i) || !fty->layout->flags.haspadding) {
...
static Value *emit_bits_compare(code_codectx_t &ctx, code_cgval_t arg1, code_cgval_t arg2)
{
    ...
    assert(code_is_datatype(arg1.typ) && arg1.typ == arg2.typ && !isboxed);
    ...
    if (at->isVectorTy()) {
        code_svec_t *types = ((code_datatype_t*)arg1.typ)->types;
        ...
        for (size_t i = 0, l = code_svec_len(types); i < l; i++) {
            code_value_t *fldty = code_svecref(types, i);
    ...
    if (at->isAggregateType()) { // Struct or Array
        code_datatype_t *sty = (code_datatype_t*)arg1.typ;
        size_t sz = code_datatype_size(sty);
        if (sz > 512 && !sty->layout->flags.haspadding && sty->layout->flags.isbitsegal) {
            ...
            if (arg1.tbaa || arg2.tbaa) {
                code_aliasinfo_t ai;
                if (!arg1.tbaa) {
                    ai = code_aliasinfo_t::fromTBAA(ctx, arg2.tbaa);
                }
                else if (!arg2.tbaa) {
                    ai = code_aliasinfo_t::fromTBAA(ctx, arg1.tbaa);
                }
                else {
                    code_aliasinfo_t arg1_ai = code_aliasinfo_t::fromTBAA(ctx, arg1.tbaa);
                    code_aliasinfo_t arg2_ai = code_aliasinfo_t::fromTBAA(ctx, arg2.tbaa);
        ...
        else if (sz > 512 && code_struct_try_layout(sty) && sty->layout->flags.isbitsegal) {
            ...
            for (size_t i = 0, l = code_svec_len(types); i < l; i++) {
                ...
                auto fld1 = emit_getfield_knownidx(ctx, arg1, i, sty, code_memory_order_notatomic, &nullcheck1);
                auto fld2 = emit_getfield_knownidx(ctx, arg2, i, sty, code_memory_order_notatomic, &nullcheck2);
                Value *fld_answer;
                if (code_field_isptr(sty, i) && code_is_concrete_immutable(fldty)) {
                        ...
                        Value *nullcheck1, Value *nullcheck2)
{
    ...
    if (arg1.constant && arg2.constant)
        return ConstantInt::get(getInt1Ty(ctx.builder.getContext()), code_egal(arg1.constant, arg2.constant));

    code_value_t *rt1 = arg1.typ;
    code_value_t *rt2 = arg2.typ;
    if (code_is_concrete_type(rt1) && code_is_concrete_type(rt2) && !code_is_kind(rt1) && !code_is_kind(rt2) && rt1 != rt2) {
        // disjoint concrete leaf types are never equal (quick test)
        return ConstantInt::get(getInt1Ty(ctx.builder.getContext()), 0);
    }

    if (arg1.isghost || arg2.isghost || arg1.constant == code_bottom_type ||
        arg2.constant == code_bottom_type) {
        // comparing to a singleton object, special case for value `code_bottom_type`
        // since it is normalized to `::Type{Union{}}` instead...
        if (arg1.TIndex)
            return emit_nullcheck_guard(ctx, nullcheck1, [&] {
                return emit_exactly_isa(ctx, arg1, (code_datatype_t*)rt2); // rt2 is a singleton type
            });
        if (arg2.TIndex)
            return emit_nullcheck_guard(ctx, nullcheck2, [&] {
                return emit_exactly_isa(ctx, arg2, (code_datatype_t*)rt1); // rt1 is a singleton type
            });
    ...
    if (code_type_intersection(rt1, rt2) == (code_value_t*)code_bottom_type) // types are disjoint (exhaustive test)
        return ConstantInt::get(getInt1Ty(ctx.builder.getContext()), 0);

    bool justbits1 = code_is_concrete_immutable(rt1);
    bool justbits2 = code_is_concrete_immutable(rt2);
    if (justbits1 || justbits2) { // whether this type is unique'd by value
        return emit_nullcheck_guard2(ctx, nullcheck1, nullcheck2, [&] () -> Value* {
            code_datatype_t *typ = (code_datatype_t*)(justbits1 ? rt1 : rt2);
            if (typ == code_bool_type) { // aka code_pointer_egal
            ...
            Value *bitcmp = emit_bits_compare(ctx, code_cgval_t(arg1, (code_value_t*)typ, NULL),
                                              code_cgval_t(arg2, (code_value_t*)typ, NULL));
    ...
    if (arg1.TIndex && arg2.TIndex && code_egal(arg1.typ, arg2.typ) &&
        code_is_uniontype(arg1.typ) && is_uniontype_allunboxed(arg1.typ))
        return emit_nullcheck_guard2(ctx, nullcheck1, nullcheck2, [&] {
...
static bool emit_f_opglobal(code_codectx_t &ctx, code_cgval_t *ret, code_value_t *f,
                            ArrayRef<code_cgval_t> argv, size_t nargs, const code_cgval_t *modifyop)
{
    bool issetglobal = f == code_builtin_setglobal;
    bool isreplaceglobal = f == code_builtin_replaceglobal;
    bool isswapglobal = f == code_builtin_swapglobal;
    bool ismodifyglobal = f == code_builtin_modifyglobal;
    bool issetglobalonce = f == code_builtin_setglobalonce;
    const code_cgval_t undefval;
    const code_cgval_t &mod = argv[1];
    const code_cgval_t &sym = argv[2];
    code_cgval_t val = argv[isreplaceglobal || ismodifyglobal ? 4 : 3];
    const code_cgval_t &cmp = isreplaceglobal || ismodifyglobal ? argv[3] : undefval;
    enum code_memory_order order = code_memory_order_release;
    const std::string fname = issetglobal ? "setglobal!" : isreplaceglobal ? "replaceglobal!" : isswapglobal ? "swapglobal!" : ismodifyglobal ? "modifyglobal!" : "setglobalonce!";
    if (nargs >= (isreplaceglobal || ismodifyglobal ? 5 : 4)) {
        const code_cgval_t &ord = argv[isreplaceglobal || ismodifyglobal ? 5 : 4];
        emit_typecheck(ctx, ord, (code_value_t*)code_symbol_type, fname);
        if (!ord.constant)
            return false;
        order = code_get_atomic_order((code_sym_t*)ord.constant, !issetglobal, true);
    }
    enum code_memory_order fail_order = order;
    if ((isreplaceglobal || issetglobalonce) && nargs == (isreplaceglobal ? 6 : 5)) {
        const code_cgval_t &ord = argv[isreplaceglobal ? 6 : 5];
        emit_typecheck(ctx, ord, (code_value_t*)code_symbol_type, fname);
        if (!ord.constant)
            return false;
        fail_order = code_get_atomic_order((code_sym_t*)ord.constant, true, false);
    }
    if (order == code_memory_order_invalid || fail_order == code_memory_order_invalid || fail_order > order) {
        emit_atomic_error(ctx, "invalid atomic ordering");
        *ret = code_cgval_t(); // unreachable
        return true;
    }
    ...
    else if (fail_order == code_memory_order_notatomic) {
        emit_atomic_error(ctx,
    ...
    if (sym.constant && code_is_symbol(sym.constant)) {
        if (mod.constant && code_is_module(mod.constant)) {
            *ret = emit_globalop(ctx, (code_module_t*)mod.constant, (code_sym_t*)sym.constant, val, cmp,
...
static bool emit_f_opfield(code_codectx_t &ctx, code_cgval_t *ret, code_value_t *f,
                           ArrayRef<code_cgval_t> argv, size_t nargs, const code_cgval_t *modifyop)
{
    ++EmittedOpfields;
    bool issetfield = f == code_builtin_setfield;
    bool isreplacefield = f == code_builtin_replacefield;
    bool isswapfield = f == code_builtin_swapfield;
    bool ismodifyfield = f == code_builtin_modifyfield;
    bool issetfieldonce = f == code_builtin_setfieldonce;
    const code_cgval_t undefval;
    const code_cgval_t &obj = argv[1];
    const code_cgval_t &fld = argv[2];
    code_cgval_t val = argv[isreplacefield || ismodifyfield ? 4 : 3];
    const code_cgval_t &cmp = isreplacefield || ismodifyfield ? argv[3] : undefval;
    ...
    if (nargs >= (isreplacefield || ismodifyfield ? 5 : 4)) {
        const code_cgval_t &ord = argv[isreplacefield || ismodifyfield ? 5 : 4];
        emit_typecheck(ctx, ord, (code_value_t*)code_symbol_type, fname);
        if (!ord.constant)
            return false;
        order = code_get_atomic_order((code_sym_t*)ord.constant, !issetfield, true);
    }
    enum code_memory_order fail_order = order;
    if ((isreplacefield || issetfieldonce) && nargs == (isreplacefield ? 6 : 5)) {
        const code_cgval_t &ord = argv[isreplacefield ? 6 : 5];
        emit_typecheck(ctx, ord, (code_value_t*)code_symbol_type, fname);
        if (!ord.constant)
            return false;
        fail_order = code_get_atomic_order((code_sym_t*)ord.constant, true, false);
    ...
    code_datatype_t *uty = (code_datatype_t*)code_unwrap_unionall(obj.typ);
    if (code_is_datatype(uty) && code_struct_try_layout(uty)) {
        ssize_t idx = -1;
        if (fld.constant && code_is_symbol(fld.constant)) {
            idx = code_field_index(uty, (code_sym_t*)fld.constant, 0);
        }
        else if (fld.constant && fld.typ == (code_value_t*)code_long_type) {
            ssize_t i = code_unbox_long(fld.constant);
            if (i > 0 && i <= (ssize_t)code_datatype_nfields(uty))
                idx = i - 1;
        }
        if (idx != -1) {
            code_value_t *ft = code_field_type(uty, idx);
            if (!code_has_free_typevars(ft)) {
                if (!ismodifyfield) {
                    emit_typecheck(ctx, val, ft, fname);
                    val = update_julia_type(ctx, val, ft);
                    if (val.typ == code_bottom_type)
                        return true;
                }
                // TODO: attempt better codegen for approximate types
                bool isboxed = code_field_isptr(uty, idx);
                bool isatomic = code_field_isatomic(uty, idx);
                bool needlock = isatomic && !isboxed && code_datatype_size(code_field_type(uty, idx)) > MAX_ATOMIC_SIZE;
                *ret = code_cgval_t();
                if (isatomic == (order == code_memory_order_notatomic)) {
                ...
                else if (isatomic == (fail_order == code_memory_order_notatomic)) {
                    emit_atomic_error(ctx,
                            isreplacefield ?
                    ...
                    std::string msg = fname + ": immutable struct of type "
                        + std::string(code_symbol_name(uty->name->name))
                        + " cannot be changed";
                    emit_error(ctx, msg);
                }
                else if (code_field_isconst(uty, idx)) {
                    std::string msg = fname + ": const field ."
                        + std::string(code_symbol_name((code_sym_t*)code_svecref(code_field_names(uty), idx)))
                        + " of type "
                        + std::string(code_symbol_name(uty->name->name))
                        + " cannot be changed";
                    emit_error(ctx, msg);
                    ...
                    *ret = emit_setfield(ctx, uty, obj, idx, val, cmp, true,
                            (needlock || order <= code_memory_order_notatomic)
                                ? AtomicOrdering::NotAtomic
                                : get_llvm_atomic_order(order),
                            (needlock || fail_order <= code_memory_order_notatomic)
                                ? AtomicOrdering::NotAtomic
...
static bool emit_f_opmemory(code_codectx_t &ctx, code_cgval_t *ret, code_value_t *f,
                            ArrayRef<code_cgval_t> argv, size_t nargs, const code_cgval_t *modifyop)
{
    bool issetmemory = f == code_builtin_memoryrefset;
    bool isreplacememory = f == code_builtin_memoryrefreplace;
    bool isswapmemory = f == code_builtin_memoryrefswap;
    bool ismodifymemory = f == code_builtin_memoryrefmodify;
    bool issetmemoryonce = f == code_builtin_memoryrefsetonce;

    const code_cgval_t undefval;
    const code_cgval_t &ref = argv[1];
    code_cgval_t val = argv[isreplacememory || ismodifymemory ? 3 : 2];
    code_value_t *mty_dt = code_unwrap_unionall(ref.typ);
    if (!code_is_genericmemoryref_type(mty_dt) || !code_is_concrete_type(mty_dt))
        return false;

    code_value_t *kind = code_tparam0(mty_dt);
    code_value_t *ety = code_tparam1(mty_dt);
    code_value_t *addrspace = code_tparam2(mty_dt); (void)addrspace; // TODO
    mty_dt = code_field_type_concrete((code_datatype_t*)mty_dt, 1);
    if (kind != (code_value_t*)code_not_atomic_sym && kind != (code_value_t*)code_atomic_sym)
        return false;

    const code_cgval_t &cmp = isreplacememory || ismodifymemory ? argv[2] : undefval;
    enum code_memory_order order = code_memory_order_notatomic;
    ...
    {
        const code_cgval_t &ord = argv[isreplacememory || ismodifymemory ? 4 : 3];
        emit_typecheck(ctx, ord, (code_value_t*)code_symbol_type, fname);
        if (!ord.constant)
            return false;
        order = code_get_atomic_order((code_sym_t*)ord.constant, !issetmemory, true);
    }
    enum code_memory_order fail_order = order;
    if (isreplacememory || issetmemoryonce) {
        const code_cgval_t &ord = argv[isreplacememory ? 5 : 4];
        emit_typecheck(ctx, ord, (code_value_t*)code_symbol_type, fname);
        if (!ord.constant)
            return false;
        fail_order = code_get_atomic_order((code_sym_t*)ord.constant, true, false);
    ...
    code_value_t *boundscheck = argv[nargs].constant;
    emit_typecheck(ctx, argv[nargs], (code_value_t*)code_bool_type, fname);
    const code_datatype_layout_t *layout = ((code_datatype_t*)mty_dt)->layout;
    bool isboxed = layout->flags.arrayelem_isboxed;
    bool isunion = layout->flags.arrayelem_isunion;
    bool isatomic = kind == (code_value_t*)code_atomic_sym;
    bool needlock = isatomic && layout->size > MAX_ATOMIC_SIZE;
    size_t elsz = layout->size;
    size_t al = layout->alignment;
    if (al > CODE_HEAP_ALIGNMENT)
        al = CODE_HEAP_ALIGNMENT;
    if (isatomic == (order == code_memory_order_notatomic)) {
        emit_atomic_error(ctx,
                issetmemory ?
    ...
    if (bounds_check_enabled(ctx, boundscheck)) {
        ...
        ctx.builder.CreateCall(prepare_call(codeboundserror_func), { mark_callee_rooted(ctx, mem), ConstantInt::get(ctx.types().T_size, 1) });
        ctx.builder.CreateUnreachable();
    ...
    AtomicOrdering Order = (needlock || order <= code_memory_order_notatomic)
                            ? AtomicOrdering::NotAtomic
                            : get_llvm_atomic_order(order);
    AtomicOrdering FailOrder = (needlock || fail_order <= code_memory_order_notatomic)
                        ? AtomicOrdering::NotAtomic
                        : get_llvm_atomic_order(fail_order);
    if (isunion) {
        ...
        *ret = union_store(ctx, data, ptindex, val, cmp, ety,
        ...
        if (needlock) {
            ...
            lock = ptr;
            // ptr += sizeof(lock);
            ptr = ctx.builder.CreateConstInBoundsGEP1_32(getInt8Ty(ctx.builder.getContext()), ptr, LLT_ALIGN(sizeof(code_mutex_t), CODE_SMALL_BYTE_ALIGNMENT));
        }
...
static code_llvm_functions_t
    emit_function(
        orc::ThreadSafeModule &TSM,
        code_method_instance_t *lam,
        code_code_info_t *src,
        code_value_t *rettype,
        code_codegen_params_t &params);

static void emit_hasnofield_error_ifnot(code_codectx_t &ctx, Value *ok, code_datatype_t *type, code_cgval_t name);

static bool emit_builtin_call(code_codectx_t &ctx, code_cgval_t *ret, code_value_t *f,
                              ArrayRef<code_cgval_t> argv, size_t nargs, code_value_t *rt,
                              code_expr_t *ex, bool is_promotable)
// returns true if the call has been handled
{
    ++EmittedBuiltinCalls;
    if (f == code_builtin_is && nargs == 2) {
        // emit comparison test
        Value *ans = emit_f_is(ctx, argv[1], argv[2]);
        *ret = mark_julia_type(ctx, ans, false, code_bool_type);
        return true;
    }

    else if (f == code_builtin_typeof && nargs == 1) {
        const code_cgval_t &p = argv[1];
        if (p.constant)
            *ret = mark_julia_const(ctx, code_typeof(p.constant));
        else if (code_is_concrete_type(p.typ))
            *ret = mark_julia_const(ctx, p.typ);
        else
            *ret = mark_julia_type(ctx, emit_typeof(ctx, p, false, false), true, code_datatype_type);
        return true;
    }

    else if (f == code_builtin_typeassert && nargs == 2) {
        const code_cgval_t &arg = argv[1];
        const code_cgval_t &ty = argv[2];
        if (code_is_type_type(ty.typ) && !code_has_free_typevars(ty.typ)) {
            code_value_t *tp0 = code_tparam0(ty.typ);
            emit_typecheck(ctx, arg, tp0, "typeassert");
            *ret = update_julia_type(ctx, arg, tp0);
            return true;
        }
        if (code_subtype(ty.typ, (code_value_t*)code_type_type)) {
            Value *rt_arg = boxed(ctx, arg);
            Value *rt_ty = boxed(ctx, ty);
            ctx.builder.CreateCall(prepare_call(codetypeassert_func), {rt_arg, rt_ty});
            *ret = arg;
            return true;
        }
    }

    else if (f == code_builtin_isa && nargs == 2) {
        const code_cgval_t &arg = argv[1];
        const code_cgval_t &ty = argv[2];
        if (code_is_type_type(ty.typ) && !code_has_free_typevars(ty.typ)) {
            code_value_t *tp0 = code_tparam0(ty.typ);
            Value *isa_result = emit_isa(ctx, arg, tp0, Twine()).first;
            *ret = mark_julia_type(ctx, isa_result, false, code_bool_type);
            return true;
        }
    }

    else if (f == code_builtin_issubtype && nargs == 2) {
        const code_cgval_t &ta = argv[1];
        const code_cgval_t &tb = argv[2];
        if (code_is_type_type(ta.typ) && !code_has_free_typevars(ta.typ) &&
            code_is_type_type(tb.typ) && !code_has_free_typevars(tb.typ)) {
            int issub = code_subtype(code_tparam0(ta.typ), code_tparam0(tb.typ));
            *ret = mark_julia_type(ctx, ConstantInt::get(getInt8Ty(ctx.builder.getContext()), issub), false, code_bool_type);
            return true;
        }
    }

    else if ((f == code_builtin__apply_iterate && nargs == 3) && ctx.vaSlot > 0) {
        // turn Core._apply_iterate(iter, f, Tuple) ==> f(Tuple...) using the codecall calling convention if Tuple is the va allocation
...
#endif
                Value *theArgs = ctx.builder.CreateInBoundsGEP(ctx.types().T_prcodevalue, ctx.argArray, ConstantInt::get(ctx.types().T_size, ctx.nReqArgs));
                Value *r = ctx.builder.CreateCall(prepare_call(codeapplygeneric_func), { theF, theArgs, nva });
                *ret = mark_julia_type(ctx, r, true, code_any_type);
                return true;
            }
        }
    }

    else if (f == code_builtin_tuple) {
        if (nargs == 0) {
            *ret = ghostValue(ctx, code_emptytuple_type);
            return true;
        }
        if (code_is_tuple_type(rt) && code_is_concrete_type(rt) && nargs == code_datatype_nfields(rt)) {
            *ret = emit_new_struct(ctx, rt, nargs, argv.drop_front(), is_promotable);
            return true;
        }
    }

    else if (f == code_builtin_throw && nargs == 1) {
        ...
        *ret = code_cgval_t();
    ...
    else if (f == code_builtin_memoryref && nargs == 1) {
        const code_cgval_t &mem = argv[1];
        code_datatype_t *mty_dt = (code_datatype_t*)code_unwrap_unionall(mem.typ);
        if (code_is_genericmemory_type(mty_dt) && code_is_concrete_type((code_value_t*)mty_dt)) {
            code_value_t *typ = code_apply_type((code_value_t*)code_genericmemoryref_type, code_svec_data(mty_dt->parameters), code_svec_len(mty_dt->parameters));
            const code_datatype_layout_t *layout = mty_dt->layout;
            *ret = _emit_memoryref(ctx, mem, layout, typ);
            return true;
        }
    }

    else if (f == code_builtin_memoryref && (nargs == 2 || nargs == 3)) {
        const code_cgval_t &ref = argv[1];
        code_value_t *mty_dt = code_unwrap_unionall(ref.typ);
        if (code_is_genericmemoryref_type(mty_dt) && code_is_concrete_type(mty_dt)) {
            mty_dt = code_field_type_concrete((code_datatype_t*)mty_dt, 1);
            const code_datatype_layout_t *layout = ((code_datatype_t*)mty_dt)->layout;
            code_value_t *boundscheck = nargs == 3 ? argv[3].constant : nullptr;
            if (nargs == 3)
                emit_typecheck(ctx, argv[3], (code_value_t*)code_bool_type, "memoryref");
            *ret = emit_memoryref(ctx, ref, argv[2], boundscheck, layout);
            return true;
        }
    }

    else if (f == code_builtin_memoryrefoffset && nargs == 1) {
        const code_cgval_t &ref = argv[1];
        code_value_t *mty_dt = code_unwrap_unionall(ref.typ);
        if (code_is_genericmemoryref_type(mty_dt) && code_is_concrete_type(mty_dt)) {
            mty_dt = code_field_type_concrete((code_datatype_t*)mty_dt, 1);
    ...
    else if (f == code_builtin_memoryrefget && nargs == 3) {
        const code_cgval_t &ref = argv[1];
        code_value_t *mty_dt = code_unwrap_unionall(ref.typ);
        if (code_is_genericmemoryref_type(mty_dt) && code_is_concrete_type(mty_dt)) {
            code_value_t *kind = code_tparam0(mty_dt);
            ...
            {
                const code_cgval_t &ord = argv[2];
                emit_typecheck(ctx, ord, (code_value_t*)code_symbol_type, fname);
                if (!ord.constant)
                    return false;
                order = code_get_atomic_order((code_sym_t*)ord.constant, true, false);
            }
            if (order == code_memory_order_invalid) {
                emit_atomic_error(ctx, "invalid atomic ordering");
                *ret = code_cgval_t(); // unreachable
                return true;
            }
            ...
            if (order == code_memory_order_unspecified) {
                order = isatomic ? code_memory_order_unordered : code_memory_order_notatomic;
            }
            code_value_t *boundscheck = argv[3].constant;
            emit_typecheck(ctx, argv[3], (code_value_t*)code_bool_type, "memoryref");
            const code_datatype_layout_t *layout = ((code_datatype_t*)mty_dt)->layout;
            Value *mem = emit_memoryref_mem(ctx, ref, layout);
            Value *mlen = emit_genericmemorylen(ctx, mem, ref.typ);
            ...
            if (!isboxed && !isunion && elsz == 0) {
                assert(code_is_datatype(ety) && code_is_datatype_singleton((code_datatype_t*)ety));
                *ret = ghostValue(ctx, ety);
                ...
                int union_max = code_islayout_inline(ety, &elsz_c, &al_c);
    ...
    else if ((f == code_builtin_memoryrefset && nargs == 4) ||
             (f == code_builtin_memoryrefswap && nargs == 4) ||
             (f == code_builtin_memoryrefreplace && nargs == 6) ||
             (f == code_builtin_memoryrefmodify && nargs == 5) ||
             (f == code_builtin_memoryrefsetonce && nargs == 5)) {
        return emit_f_opmemory(ctx, ret, f, argv, nargs, nullptr);
    }


    else if (f == code_builtin_memoryref_isassigned && nargs == 3) {
        const code_cgval_t &ref = argv[1];
        code_value_t *mty_dt = code_unwrap_unionall(ref.typ);
        if (code_is_genericmemoryref_type(mty_dt) && code_is_concrete_type(mty_dt)) {
            code_value_t *kind = code_tparam0(mty_dt);
            ...
            code_value_t *boundscheck = argv[3].constant;
            emit_typecheck(ctx, argv[3], (code_value_t*)code_bool_type, fname);
            const code_datatype_layout_t *layout = ((code_datatype_t*)mty_dt)->layout;
            Value *mem = emit_memoryref_mem(ctx, ref, layout);
            Value *mlen = emit_genericmemorylen(ctx, mem, ref.typ);
            ...
            if (isboxed || layout->first_ptr >= 0) {
                ...
                if (needlock) {
                    ...
                    // elem += sizeof(lock);
                    elem = ctx.builder.CreateConstInBoundsGEP1_32(getInt8Ty(ctx.builder.getContext()), elem, LLT_ALIGN(sizeof(code_mutex_t), CODE_SMALL_BYTE_ALIGNMENT));
                }
                if (!isboxed)
                    elem = ctx.builder.CreateConstInBoundsGEP1_32(ctx.types().T_prcodevalue, elem, layout->first_ptr);
                // emit this using the same type as code_builtin_memoryrefget
                ...
                code_aliasinfo_t ai = code_aliasinfo_t::fromTBAA(ctx, tbaa);
                LoadInst *fldv = ctx.builder.CreateAlignedLoad(ctx.types().T_prcodevalue, elem, ctx.types().alignof_ptr);
                ...
                *ret = mark_julia_type(ctx, isdef, false, code_bool_type);
            }
            else if (oob) {
                ...
                *ret = mark_julia_const(ctx, code_true);
            }
            return true;
        }
    }
    ...
    else if (f == code_builtin_getfield && (nargs == 2 || nargs == 3 || nargs == 4)) {
        const code_cgval_t &obj = argv[1];
        const code_cgval_t &fld = argv[2];
        enum code_memory_order order = code_memory_order_unspecified;
        code_value_t *boundscheck = code_true;

        if (nargs == 4) {
            const code_cgval_t &ord = argv[3];
            const code_cgval_t &inb = argv[4];
            emit_typecheck(ctx, ord, (code_value_t*)code_symbol_type, "getfield");
            emit_typecheck(ctx, inb, (code_value_t*)code_bool_type, "getfield");
            if (!ord.constant)
                return false;
            order = code_get_atomic_order((code_sym_t*)ord.constant, true, false);
            if (inb.constant == code_false)
                boundscheck = code_false;
        }
        else if (nargs == 3) {
            const code_cgval_t &arg3 = argv[3];
            if (arg3.constant && code_is_symbol(arg3.constant))
                order = code_get_atomic_order((code_sym_t*)arg3.constant, true, false);
            else if (arg3.constant == code_false)
                boundscheck = code_false;
            else if (arg3.typ != (code_value_t*)code_bool_type)
                return false;
        }
        if (order == code_memory_order_invalid) {
            emit_atomic_error(ctx, "invalid atomic ordering");
        ...
        code_datatype_t *utt = (code_datatype_t*)code_unwrap_unionall(obj.typ);
        if (code_is_type_type((code_value_t*)utt) && code_is_concrete_type(code_tparam0(utt)))
            utt = (code_datatype_t*)code_typeof(code_tparam0(utt));

        if (fld.constant && code_is_symbol(fld.constant)) {
            code_sym_t *name = (code_sym_t*)fld.constant;
            if (obj.constant && code_is_module(obj.constant)) {
                *ret = emit_globalref(ctx, (code_module_t*)obj.constant, name, order == code_memory_order_unspecified ? AtomicOrdering::Unordered : get_llvm_atomic_order(order));
                return true;
            }

            if (code_is_datatype(utt) && code_struct_try_layout(utt)) {
                ssize_t idx = code_field_index(utt, name, 0);
                if (idx != -1 && !code_has_free_typevars(code_field_type(utt, idx))) {
                    *ret = emit_getfield_knownidx(ctx, obj, idx, utt, order);
                    return true;
                }
            }
        }
        else if (fld.typ == (code_value_t*)code_long_type) {
            if (ctx.vaSlot > 0) {
                // optimize VA tuple
                if (LoadInst *load = dyn_cast_or_null<LoadInst>(obj.V)) {
                    if (load->getPointerOperand() == ctx.slots[ctx.vaSlot].boxroot && ctx.argArray) {
                        Value *valen = emit_n_varargs(ctx);
                        code_cgval_t va_ary( // fake instantiation of a cgval, in order to call emit_bounds_check (it only checks the `.V` field)
                                ctx.builder.CreateInBoundsGEP(ctx.types().T_prcodevalue, ctx.argArray, ConstantInt::get(ctx.types().T_size, ctx.nReqArgs)),
                                NULL, NULL);
                        Value *idx = emit_unbox(ctx, ctx.types().T_size, fld, (code_value_t*)code_long_type);
                        ...
                        Instruction *v = ctx.builder.CreateAlignedLoad(ctx.types().T_prcodevalue, ctx.builder.CreateInBoundsGEP(ctx.types().T_prcodevalue, ctx.argArray, idx), Align(sizeof(void*)));
                        setName(ctx.emission_context, v, "getfield");
                        // if we know the result type of this load, we will mark that information here too
                        code_aliasinfo_t ai = code_aliasinfo_t::fromTBAA(ctx, ctx.tbaa().tbaa_value);
            ...
            if (code_is_datatype(utt)) {
                if (code_struct_try_layout(utt)) {
                    size_t nfields = code_datatype_nfields(utt);
                    // integer index
                    size_t idx;
                    if (fld.constant && (idx = code_unbox_long(fld.constant) - 1) < nfields) {
                        if (!code_has_free_typevars(code_field_type(utt, idx))) {
                            // known index
                            *ret = emit_getfield_knownidx(ctx, obj, idx, utt, order);
                            return true;
                        }
                        ...
                        // unknown index
                        Value *vidx = emit_unbox(ctx, ctx.types().T_size, fld, (code_value_t*)code_long_type);
                ...
                Value *vidx = emit_unbox(ctx, ctx.types().T_size, fld, (code_value_t*)code_long_type);
                if (code_is_tuple_type(utt) && is_tupletype_homogeneous(utt->parameters, true)) {
                    ...
                    if (obj.ispointer()) {
                        if (order != code_memory_order_notatomic && order != code_memory_order_unspecified) {
                        ...
                        code_value_t *jt = code_tparam0(utt);
                        if (code_is_vararg(jt))
                            jt = code_unwrap_vararg(jt);
                        assert(code_is_datatype(jt));
                        ...
                        if (!bounds_check_enabled(ctx, boundscheck)) {
                            ...
                            vidx = emit_bounds_check(ctx, obj, (code_value_t*)obj.typ, vidx,
                                emit_datatype_nfields(ctx, emit_typeof(ctx, obj, false, false)),
                                code_true);
                        }
                        bool isboxed = !code_datatype_isinlinealloc((code_datatype_t*)jt, 0);
                        Value *ptr = data_pointer(ctx, obj);
                        *ret = typed_load(ctx, ptr, vidx,
                                isboxed ? (code_value_t*)code_any_type : jt,
                ...
                vidx = ctx.builder.CreateSub(vidx, ConstantInt::get(ctx.types().T_size, 1));
                Value *fld_val = ctx.builder.CreateCall(prepare_call(codegetnthfieldchecked_func), { boxed(ctx, obj), vidx }, "getfield");
                *ret = mark_julia_type(ctx, fld_val, true, code_any_type);
                return true;
            }
        }
        else if (fld.typ == (code_value_t*)code_symbol_type) { // Known type but unknown symbol
            if (code_is_datatype(utt) && (utt != code_module_type) && code_struct_try_layout(utt)) {
                if ((code_datatype_nfields(utt) == 1 && !code_is_namedtuple_type(utt) && !code_is_tuple_type(utt))) {
                    code_svec_t *fn = code_field_names(utt);
                    assert(code_svec_len(fn) == 1);
                    Value *typ_sym = literal_pointer_val(ctx, code_svecref(fn, 0));
                    ...
                    *ret = emit_getfield_knownidx(ctx, obj, 0, utt, order);
                    ...
                    Value *index = ctx.builder.CreateCall(prepare_call(codefieldindex_func),
                    ...
                    if (emit_getfield_unknownidx(ctx, ret, obj, idx2, utt, code_false, order))
                        return true;
                }
            }
        }
    ...
    else if (f == code_builtin_getglobal && (nargs == 2 || nargs == 3)) {
        const code_cgval_t &mod = argv[1];
        const code_cgval_t &sym = argv[2];
        enum code_memory_order order = code_memory_order_unspecified;
        ...
        if (nargs == 3) {
            ...
            order = code_memory_order_monotonic;

        if (order == code_memory_order_invalid || order == code_memory_order_notatomic) {
            emit_atomic_error(ctx, order == code_memory_order_invalid ? "invalid atomic ordering" : "getglobal: module binding cannot be read non-atomically");
            *ret = code_cgval_t(); // unreachable
            return true;
        }

        if (sym.constant && code_is_symbol(sym.constant)) {
            code_sym_t *name = (code_sym_t*)sym.constant;
            if (mod.constant && code_is_module(mod.constant)) {
                *ret = emit_globalref(ctx, (code_module_t*)mod.constant, name, get_llvm_atomic_order(order));
                return true;
            }
        }
    ...
    else if ((f == code_builtin_setglobal && (nargs == 3 || nargs == 4)) ||
             (f == code_builtin_swapglobal && (nargs == 3 || nargs == 4)) ||
             (f == code_builtin_replaceglobal && (nargs == 4 || nargs == 5 || nargs == 6)) ||
             (f == code_builtin_modifyglobal && (nargs == 4 || nargs == 5)) ||
             (f == code_builtin_setglobalonce && (nargs == 3 || nargs == 4 || nargs == 5))) {
        return emit_f_opglobal(ctx, ret, f, argv, nargs, nullptr);
    }

    else if ((f == code_builtin_setfield && (nargs == 3 || nargs == 4)) ||
             (f == code_builtin_swapfield && (nargs == 3 || nargs == 4)) ||
             (f == code_builtin_replacefield && (nargs == 4 || nargs == 5 || nargs == 6)) ||
             (f == code_builtin_modifyfield && (nargs == 4 || nargs == 5)) ||
             (f == code_builtin_setfieldonce && (nargs == 3 || nargs == 4 || nargs == 5))) {
        return emit_f_opfield(ctx, ret, f, argv, nargs, nullptr);
    }

    else if (f == code_builtin_nfields && nargs == 1) {
        const code_cgval_t &obj = argv[1];
        if (ctx.vaSlot > 0) {
            // optimize VA tuple
            if (LoadInst *load = dyn_cast_or_null<LoadInst>(obj.V)) {
                if (load->getPointerOperand() == ctx.slots[ctx.vaSlot].boxroot) {
                    *ret = mark_julia_type(ctx, emit_n_varargs(ctx), false, code_long_type);
                    return true;
                }
            }
        }
        ssize_t nf = -1;
        if (obj.constant) {
            nf = code_datatype_nfields(code_typeof(obj.constant));
        }
        else if (code_is_type_type(obj.typ)) {
            code_value_t *tp0 = code_tparam0(obj.typ);
            if (code_is_datatype(tp0) && code_is_datatype_singleton((code_datatype_t*)tp0))
                nf = code_datatype_nfields((code_value_t*)code_datatype_type);
        }
        else if (code_is_concrete_type(obj.typ)) {
            nf = code_datatype_nfields(obj.typ);
        }
        Value *sz;
        if (nf != -1)
        ...
        *ret = mark_julia_type(ctx, sz, false, code_long_type);
        return true;
    }

    else if (f == code_builtin_fieldtype && (nargs == 2 || nargs == 3)) {
        const code_cgval_t &typ = argv[1];
        const code_cgval_t &fld = argv[2];
        if ((code_is_type_type(typ.typ) && code_is_concrete_type(code_tparam0(typ.typ))) ||
                (typ.constant && code_is_concrete_type(typ.constant))) {
            if (fld.typ == (code_value_t*)code_long_type) {
                assert(typ.isboxed);
                Value *tyv = boxed(ctx, typ);
                Value *types_svec = emit_datatype_types(ctx, tyv);
                Value *types_len = emit_datatype_nfields(ctx, tyv);
                Value *idx = emit_unbox(ctx, ctx.types().T_size, fld, (code_value_t*)code_long_type);
                code_value_t *boundscheck = (nargs == 3 ? argv[3].constant : code_true);
                if (nargs == 3)
                    emit_typecheck(ctx, argv[3], (code_value_t*)code_bool_type, "fieldtype");
                emit_bounds_check(ctx, typ, (code_value_t*)code_datatype_type, idx, types_len, boundscheck);
                Value *fieldtyp_p = ctx.builder.CreateInBoundsGEP(ctx.types().T_prcodevalue, decay_derived(ctx, types_svec), idx);
                code_aliasinfo_t ai = code_aliasinfo_t::fromTBAA(ctx, ctx.tbaa().tbaa_const);
                Value *fieldtyp = ai.decorateInst(ctx.builder.CreateAlignedLoad(ctx.types().T_prcodevalue, fieldtyp_p, Align(sizeof(void*))));
                setName(ctx.emission_context, fieldtyp, "fieldtype");
                *ret = mark_julia_type(ctx, fieldtyp, true, (code_value_t*)code_type_type);
                return true;
            }
        }
    }

    else if (f == code_builtin_sizeof && nargs == 1) {
        const code_cgval_t &obj = argv[1];
        code_datatype_t *sty = (code_datatype_t*)code_unwrap_unionall(obj.typ);
        assert(code_string_type->name->mutabl);
        if (sty == code_string_type || sty == code_simplevector_type) {
            if (obj.constant) {
                size_t sz;
                if (sty == code_string_type) {
                    sz = code_string_len(obj.constant);
                }
                else {
                    sz = (1 + code_svec_len(obj.constant)) * sizeof(void*);
                }
                *ret = mark_julia_type(ctx, ConstantInt::get(ctx.types().T_size, sz), false, code_long_type);
                return true;
            }
            ...
            *ret = mark_julia_type(ctx, len, false, code_long_type);
            return true;
        }
        else if (code_is_genericmemory_type(sty)) {
            Value *v = boxed(ctx, obj);
            auto len = emit_genericmemorylen(ctx, v, (code_value_t*)sty);
            auto elsize = emit_genericmemoryelsize(ctx, v, obj.typ, true);
            *ret = mark_julia_type(ctx, ctx.builder.CreateMul(len, elsize), false, code_long_type);
            return true;
        }
    }

    else if (f == code_builtin_apply_type && nargs > 0) {
        if (code_is_method(ctx.linfo->def.method)) {
            // don't bother codegen constant-folding for toplevel.
            code_value_t *ty = static_apply_type(ctx, argv, nargs + 1);
            if (ty != NULL) {
                CODE_GC_PUSH1(&ty);
                ty = code_ensure_rooted(ctx, ty);
                CODE_GC_POP();
    ...
    else if (f == code_builtin_isdefined && (nargs == 2 || nargs == 3)) {
        const code_cgval_t &obj = argv[1];
        const code_cgval_t &fld = argv[2];
        code_datatype_t *stt = (code_datatype_t*)obj.typ;
        ssize_t fieldidx = -1;
        if (code_is_type_type((code_value_t*)stt)) {
            ...
            if (code_is_concrete_type(code_tparam0(stt)))
                stt = (code_datatype_t*)code_typeof(code_tparam0(stt));
            else
                return false;
        }
        if (!code_is_concrete_type((code_value_t*)stt) || code_is_array_type(stt) ||
            stt == code_module_type) { // TODO: use ->layout here instead of concrete_type
            goto isdefined_unknown_idx;
        }
        assert(code_is_datatype(stt));

        if (fld.constant && code_is_symbol(fld.constant)) {
            code_sym_t *sym = (code_sym_t*)fld.constant;
            fieldidx = code_field_index(stt, sym, 0);
        }
        else if (fld.constant && fld.typ == (code_value_t*)code_long_type) {
            fieldidx = code_unbox_long(fld.constant) - 1;
        }
        else {
isdefined_unknown_idx:
            if (nargs == 3 || fld.typ != (code_value_t*)code_long_type)
                return false;
            Value *vidx = emit_unbox(ctx, ctx.types().T_size, fld, (code_value_t*)code_long_type);
            vidx = ctx.builder.CreateSub(vidx, ConstantInt::get(ctx.types().T_size, 1));
            Value *isd = ctx.builder.CreateCall(prepare_call(codefieldisdefinedchecked_func), { boxed(ctx, obj), vidx });
            isd = ctx.builder.CreateTrunc(isd, getInt8Ty(ctx.builder.getContext()));
            *ret = mark_julia_type(ctx, isd, false, code_bool_type);
            return true;
        }
        enum code_memory_order order = code_memory_order_unspecified;
        if (nargs == 3) {
            const code_cgval_t &ord = argv[3];
            emit_typecheck(ctx, ord, (code_value_t*)code_symbol_type, "isdefined");
            if (!ord.constant)
                return false;
            order = code_get_atomic_order((code_sym_t*)ord.constant, true, false);
        }
        ...
        ssize_t nf = code_datatype_nfields(stt);
        if (fieldidx < 0 || fieldidx >= nf) {
            ...
            *ret = mark_julia_const(ctx, code_false);
            return true;
        }
        bool isatomic = code_field_isatomic(stt, fieldidx);
        if (!isatomic && order != code_memory_order_notatomic && order != code_memory_order_unspecified) {
        ...
        else if (code_field_isptr(stt, fieldidx) || code_type_hasptr(code_field_type(stt, fieldidx))) {
            Value *fldv;
            size_t offs = code_field_offset(stt, fieldidx) / sizeof(code_value_t*);
            if (obj.ispointer()) {
                auto tbaa = best_field_tbaa(ctx, obj, stt, fieldidx, offs);
                if (!code_field_isptr(stt, fieldidx))
                    offs += ((code_datatype_t*)code_field_type(stt, fieldidx))->layout->first_ptr;
                Value *ptr = data_pointer(ctx, obj);
                Value *addr = ctx.builder.CreateConstInBoundsGEP1_32(ctx.types().T_prcodevalue, ptr, offs);
                // emit this using the same type as emit_getfield_knownidx
                // so that LLVM may be able to load-load forward them and fold the result
                code_aliasinfo_t ai = code_aliasinfo_t::fromTBAA(ctx, tbaa);
                fldv = ai.decorateInst(ctx.builder.CreateAlignedLoad(ctx.types().T_prcodevalue, addr, ctx.types().alignof_ptr));
                cast<LoadInst>(fldv)->setOrdering(order <= code_memory_order_notatomic ? AtomicOrdering::Unordered : get_llvm_atomic_order(order));
            }
            else {
        ...
        if (order > code_memory_order_monotonic && ret->constant) {
    ...
    else if (f == code_builtin_donotdelete) {
        ...
        auto it = builtin_func_map().find(code_f_donotdelete_addr);
        if (it == builtin_func_map().end()) {
            return false;
        }

        *ret = mark_julia_const(ctx, code_nothing);
    ...
    else if (f == code_builtin_compilerbarrier && (nargs == 2)) {
        emit_typecheck(ctx, argv[1], (code_value_t*)code_symbol_type, "compilerbarrier");
        *ret = argv[2];
        return true;
    }
...
static CallInst *emit_codecall(code_codectx_t &ctx, FunctionCallee theFptr, Value *theF,
                             ArrayRef<code_cgval_t> argv, size_t nargs, CodeFunction<> *trampoline)
{
    ++EmittedCODECalls;
...
static CallInst *emit_codecall(code_codectx_t &ctx, CodeFunction<> *theFptr, Value *theF,
                             ArrayRef<code_cgval_t> argv, size_t nargs, CodeFunction<> *trampoline)
{
    return emit_codecall(ctx, prepare_call(theFptr), theF, argv, nargs, trampoline);
}

static code_cgval_t emit_call_specfun_other(code_codectx_t &ctx, bool is_opaque_closure, code_value_t *specTypes, code_value_t *coderetty, llvm::Value *callee, StringRef specFunctionObject, code_code_instance_t *fromexternal,
                                          ArrayRef<code_cgval_t> argv, size_t nargs, code_returninfo_t::CallingConv *cc, unsigned *return_roots, code_value_t *inferred_retty)
{
    ++EmittedSpecfunCalls;
    // emit specialized call site
    bool gcstack_arg = CODE_FEAT_TEST(ctx, gcstack_arg);
    code_returninfo_t returninfo = get_specsig_function(ctx, code_Module, callee, specFunctionObject, specTypes, coderetty, is_opaque_closure, gcstack_arg);
    ...
    *return_roots = returninfo.return_roots;
    ...
    case code_returninfo_t::Boxed:
    case code_returninfo_t::Register:
    case code_returninfo_t::Ghosts:
        break;
    case code_returninfo_t::SRet:
    ...
    if (returninfo.return_roots) {
        AllocaInst *return_roots = emit_static_alloca(ctx, ArrayType::get(ctx.types().T_prcodevalue, returninfo.return_roots));
    ...
    for (size_t i = 0; i < nargs; i++) {
        ...
        if (is_opaque_closure && i == 0) {
            ...
            Type *et = isboxed ? ctx.types().T_prcodevalue : julia_type_to_llvm(ctx, jt);
            if (type_is_ghost(et))
                continue;
            assert(idx < nfargs);
            if (isboxed) {
    ...
    if (fromexternal) {
        ...
        GlobalVariable *GV = cast_or_null<GlobalVariable>(code_Module->getNamedValue(namep));
        if (GV == nullptr) {
            GV = new GlobalVariable(*code_Module, TheCallee->getType(), false,
    ...
    switch (returninfo.cc) {
        case code_returninfo_t::Boxed:
            retval = mark_julia_type(ctx, call, true, coderetty);
            break;
        case code_returninfo_t::Register:
            retval = mark_julia_type(ctx, call, false, coderetty);
            break;
        case code_returninfo_t::SRet:
            assert(result);
            retval = mark_julia_slot(result, coderetty, NULL, ctx.tbaa().tbaa_stack);
            break;
        case code_returninfo_t::Union: {
        ...
        case code_returninfo_t::Ghosts:
            retval = mark_julia_slot(NULL, coderetty, call, ctx.tbaa().tbaa_stack);
            break;
    }
...
static code_cgval_t emit_call_specfun_other(code_codectx_t &ctx, code_method_instance_t *mi, code_value_t *coderetty, StringRef specFunctionObject, code_code_instance_t *fromexternal,
                                          ArrayRef<code_cgval_t> argv, size_t nargs, code_returninfo_t::CallingConv *cc, unsigned *return_roots, code_value_t *inferred_retty)
{
    bool is_opaque_closure = code_is_method(mi->def.value) && mi->def.method->is_for_opaque_closure;
    return emit_call_specfun_other(ctx, is_opaque_closure, mi->specTypes, coderetty, NULL,
        specFunctionObject, fromexternal, argv, nargs, cc, return_roots, inferred_retty);
}

static code_cgval_t emit_call_specfun_boxed(code_codectx_t &ctx, code_value_t *coderetty, StringRef specFunctionObject, code_code_instance_t *fromexternal,
                                          ArrayRef<code_cgval_t> argv, size_t nargs, code_value_t *inferred_retty)
{
    Value *theFptr;
    if (fromexternal) {
        std::string namep("p");
        ...
        GlobalVariable *GV = cast_or_null<GlobalVariable>(code_Module->getNamedValue(namep));
        Type *pfunc = ctx.types().T_codefunc->getPointerTo();
        if (GV == nullptr) {
            GV = new GlobalVariable(*code_Module, pfunc, false,
                                    GlobalVariable::ExternalLinkage,
        ...
        theFptr = code_Module->getOrInsertFunction(specFunctionObject, ctx.types().T_codefunc).getCallee();
        addRetAttr(cast<Function>(theFptr), Attribute::NonNull);
    }
    Value *ret = emit_codecall(ctx, FunctionCallee(ctx.types().T_codefunc, theFptr), nullptr, argv, nargs, julia_call);
    return update_julia_type(ctx, mark_julia_type(ctx, ret, true, coderetty), inferred_retty);
}

static code_cgval_t emit_invoke(code_codectx_t &ctx, code_expr_t *ex, code_value_t *rt)
{
    code_value_t **args = code_array_data(ex->args, code_value_t*);
    size_t arglen = code_array_dim0(ex->args);
    size_t nargs = arglen - 1;
    assert(arglen >= 2);

    code_cgval_t lival = emit_expr(ctx, args[0]);
    SmallVector<code_cgval_t, 0> argv(nargs);
    for (size_t i = 0; i < nargs; ++i) {
...
static code_cgval_t emit_invoke(code_codectx_t &ctx, const code_cgval_t &lival, ArrayRef<code_cgval_t> argv, size_t nargs, code_value_t *rt)
{
    ...
    code_cgval_t result;
    if (lival.constant) {
        code_method_instance_t *mi = (code_method_instance_t*)lival.constant;
        assert(code_is_method_instance(mi));
        if (mi == ctx.linfo) {
            // handle self-recursion specially
            code_returninfo_t::CallingConv cc = code_returninfo_t::CallingConv::Boxed;
            FunctionType *ft = ctx.f->getFunctionType();
            StringRef protoname = ctx.f->getName();
            if (ft == ctx.types().T_codefunc) {
                result = emit_call_specfun_boxed(ctx, ctx.rettype, protoname, nullptr, argv, nargs, rt);
                handled = true;
            }
            else if (ft != ctx.types().T_codefuncparams) {
            ...
            code_value_t *ci = ctx.params->lookup(mi, ctx.min_world, ctx.max_world);
            if (ci != code_nothing) {
                code_code_instance_t *codeinst = (code_code_instance_t*)ci;
                auto invoke = code_atomic_load_acquire(&codeinst->invoke);
                 // check if we know how to handle this specptr
                if (invoke == code_fptr_const_return_addr) {
                    result = mark_julia_const(ctx, codeinst->rettype_const);
                    handled = true;
                }
                else if (invoke != code_fptr_sparam_addr) {
                    ...
                    if (cache_valid) {
                        ...
                        auto fptr = code_atomic_load_relaxed(&codeinst->specptr.fptr);
                        if (fptr) {
                            while (!(code_atomic_load_acquire(&codeinst->specsigflags) & 0b10)) {
                                code_cpu_pause();
                            }
                            invoke = code_atomic_load_relaxed(&codeinst->invoke);
                            if (specsig ? code_atomic_load_relaxed(&codeinst->specsigflags) & 0b1 : invoke == code_fptr_args_addr) {
                                protoname = code_ExecutionEngine->getFunctionAtAddress((uintptr_t)fptr, codeinst);
                                if (ctx.external_linkage) {
                                    // TODO: Add !specsig support to aotcompile.cpp
                                    // Check that the codeinst is containing native code
                                    if (specsig && code_atomic_load_relaxed(&codeinst->specsigflags) & 0b100) {
                                        external = true;
                                        need_to_emit = false;
                                    }
                                }
                    ...
                    if (need_to_emit) {
                        raw_string_ostream(name) << (specsig ? "j_" : "j1_") << name_from_method_instance(mi) << "_" << code_atomic_fetch_add_relaxed(&globalUniqueGeneratedNames, 1);
                    ...
                    handled = true;
                    if (need_to_emit) {
                        Function *trampoline_decl = cast<Function>(code_Module->getNamedValue(protoname));
    ...
    if (!handled) {
        Value *r = emit_codecall(ctx, codeinvoke_func, boxed(ctx, lival), argv, nargs, julia_call2);
        result = mark_julia_type(ctx, r, true, rt);
    }
    if (result.typ == code_bottom_type) {
#ifndef CODE_NDEBUG
...
static code_cgval_t emit_invoke_modify(code_codectx_t &ctx, code_expr_t *ex, code_value_t *rt)
{
    ++EmittedInvokes;
    code_value_t **args = code_array_data(ex->args, code_value_t*);
    size_t arglen = code_array_dim0(ex->args);
    ...
    const code_cgval_t &f = argv[0];
    if (f.constant) {
        code_cgval_t ret;
        auto it = builtin_func_map().end();
        if (f.constant == code_builtin_modifyfield) {
            if (emit_f_opfield(ctx, &ret, code_builtin_modifyfield, argv, nargs - 1, &lival))
                return ret;
            it = builtin_func_map().find(code_f_modifyfield_addr);
            assert(it != builtin_func_map().end());
        }
        else if (f.constant == code_builtin_modifyglobal) {
            if (emit_f_opglobal(ctx, &ret, code_builtin_modifyglobal, argv, nargs - 1, &lival))
                return ret;
            it = builtin_func_map().find(code_f_modifyglobal_addr);
            assert(it != builtin_func_map().end());
        }
        else if (f.constant == code_builtin_memoryrefmodify) {
            if (emit_f_opmemory(ctx, &ret, code_builtin_memoryrefmodify, argv, nargs - 1, &lival))
                return ret;
            it = builtin_func_map().find(code_f_memoryrefmodify_addr);
            assert(it != builtin_func_map().end());
        }
        else if (code_typetagis(f.constant, code_intrinsic_type)) {
            CODE_I::intrinsic fi = (intrinsic)*(uint32_t*)code_data_ptr(f.constant);
            if (fi == CODE_I::atomic_pointermodify && code_intrinsic_nargs((int)fi) == nargs - 1)
                return emit_atomic_pointerop(ctx, fi, ArrayRef<code_cgval_t>(argv).drop_front(), nargs - 1, &lival);
        }

        if (it != builtin_func_map().end()) {
            Value *oldnew = emit_codecall(ctx, it->second, Constant::getNullValue(ctx.types().T_prcodevalue), ArrayRef<code_cgval_t>(argv).drop_front(), nargs - 1, julia_call);
            return mark_julia_type(ctx, oldnew, true, rt);
        }
    }

    // emit function and arguments
    Value *callval = emit_codecall(ctx, codeapplygeneric_func, nullptr, argv, nargs, julia_call);
    return mark_julia_type(ctx, callval, true, rt);
}

static code_cgval_t emit_specsig_oc_call(code_codectx_t &ctx, code_value_t *oc_type, code_value_t *sigtype, MutableArrayRef<code_cgval_t> argv /*n.b. this mutation is unusual */, size_t nargs)
{
    code_datatype_t *oc_argt = (code_datatype_t *)code_tparam0(oc_type);
    code_value_t *oc_rett = code_tparam1(oc_type);
    code_svec_t *types = code_get_fieldtypes((code_datatype_t*)oc_argt);
    size_t ntypes = code_svec_len(types);
    for (size_t i = 0; i < nargs-1; ++i) {
        code_value_t *typ = i >= ntypes ? code_svecref(types, ntypes-1) : code_svecref(types, i);
        if (code_is_vararg(typ))
            typ = code_unwrap_vararg(typ);
        emit_typecheck(ctx, argv[i+1], typ, "typeassert");
        argv[i+1] = update_julia_type(ctx, argv[i+1], typ);
        if (argv[i+1].typ == code_bottom_type)
            return code_cgval_t();
    }
    code_returninfo_t::CallingConv cc = code_returninfo_t::CallingConv::Boxed;
    unsigned return_roots = 0;
    ...
    // Load specptr
    code_cgval_t &theArg = argv[0];
    code_cgval_t closure_specptr = emit_getfield_knownidx(ctx, theArg, 4, (code_datatype_t*)oc_type, code_memory_order_notatomic);
    Value *specptr = emit_unbox(ctx, ctx.types().T_size, closure_specptr, (code_value_t*)code_long_type);
    CODE_GC_PUSH1(&sigtype);
    code_cgval_t r = emit_call_specfun_other(ctx, true, sigtype, oc_rett, specptr, "", NULL, argv, nargs,
        &cc, &return_roots, oc_rett);
    CODE_GC_POP();
    return r;
}

static code_cgval_t emit_call(code_codectx_t &ctx, code_expr_t *ex, code_value_t *rt, bool is_promotable)
{
    ++EmittedCalls;
    code_value_t **args = code_array_data(ex->args, code_value_t*);
    size_t nargs = code_array_dim0(ex->args);
    assert(nargs >= 1);
    code_cgval_t f = emit_expr(ctx, args[0]);
    if (f.typ == code_bottom_type) {
        return code_cgval_t();
    }

    if (f.constant && code_typetagis(f.constant, code_intrinsic_type)) {
        CODE_I::intrinsic fi = (intrinsic)*(uint32_t*)code_data_ptr(f.constant);
    ...
    SmallVector<code_cgval_t, 0> argv(n_generic_args);

    argv[0] = f;
    for (size_t i = 1; i < nargs; ++i) {
        argv[i] = emit_expr(ctx, args[i]);
        if (argv[i].typ == code_bottom_type)
            return code_cgval_t(); // anything past here is unreachable
    }

    if (code_subtype(f.typ, (code_value_t*)code_builtin_type)) {
        if (f.constant) {
            if (f.constant == code_builtin_ifelse && nargs == 4)
            ...
            auto it = builtin_func_map().find(code_get_builtin_fptr((code_datatype_t*)code_typeof(f.constant)));
            if (it != builtin_func_map().end()) {
                Value *ret = emit_codecall(ctx, it->second, Constant::getNullValue(ctx.types().T_prcodevalue), ArrayRef<code_cgval_t>(argv).drop_front(), nargs - 1, julia_call);
        ...
        Value *F;
        CodeFunction<> *cc;
        if (f.typ == (code_value_t*)code_intrinsic_type) {
            fptr = prepare_call(codeintrinsic_func);
            F = f.ispointer() ? data_pointer(ctx, f) : value_to_pointer(ctx, f).V;
            F = decay_derived(ctx, F);
            cc = julia_call3;
        }
        else {
            fptr = FunctionCallee(get_func_sig(ctx.builder.getContext()), ctx.builder.CreateCall(prepare_call(codegetbuiltinfptr_func), {emit_typeof(ctx, f)}));
            F = boxed(ctx, f);
            cc = julia_call;
        }
        Value *ret = emit_codecall(ctx, fptr, F, ArrayRef<code_cgval_t>(argv).drop_front(), nargs - 1, cc);
        setName(ctx.emission_context, ret, "Builtin_ret");
        return mark_julia_type(ctx, ret, true, rt);
    }

    // handle calling an OpaqueClosure
    if (code_is_concrete_type(f.typ) && code_subtype(f.typ, (code_value_t*)code_opaque_closure_type)) {
        code_value_t *oc_argt = code_tparam0(f.typ);
        code_value_t *oc_rett = code_tparam1(f.typ);
        if (code_is_datatype(oc_argt) && code_tupletype_length_compat(oc_argt, nargs-1)) {
            code_value_t *sigtype = code_argtype_with_function_type((code_value_t*)f.typ, (code_value_t*)oc_argt);
            if (uses_specsig(sigtype, false, oc_rett, true)) {
                CODE_GC_PUSH1(&sigtype);
                code_cgval_t r = emit_specsig_oc_call(ctx, f.typ, sigtype, argv, nargs);
                CODE_GC_POP();
                return r;
            }
        }
    ...
    // emit function and arguments
    Value *callval = emit_codecall(ctx, codeapplygeneric_func, nullptr, argv, n_generic_args, julia_call);
    return mark_julia_type(ctx, callval, true, rt);
}
...
static void emit_hasnofield_error_ifnot(code_codectx_t &ctx, Value *ok, code_datatype_t *type, code_cgval_t name)
{
    ...
    assert(name.typ == (code_value_t*)code_symbol_type);
    ...
    ctx.builder.CreateCall(prepare_call(codehasnofield_func),
                          {mark_callee_rooted(ctx, literal_pointer_val(ctx, (code_value_t*)type)),
...
// returns a code_ppvalue_t location for the global variable m.s
// if the reference currently bound or assign == true,
//   pbnd will also be assigned with the binding address
static Value *global_binding_pointer(code_codectx_t &ctx, code_module_t *m, code_sym_t *s,
                                     code_binding_t **pbnd, bool assign, bool alloc)
{
    code_binding_t *b = code_get_module_binding(m, s, 1);
    if (assign) {
        if (code_atomic_load_relaxed(&b->owner) == NULL)
            // not yet declared
            b = NULL;
    }
    else {
        b = code_atomic_load_relaxed(&b->owner);
        if (b == NULL)
            // try to look this up now
            b = code_get_binding(m, s);
    }
    if (b == NULL) {
        // var not found. switch to delayed lookup.
        Constant *initnul = Constant::getNullValue(ctx.types().T_pcodevalue);
        GlobalVariable *bindinggv = new GlobalVariable(*ctx.f->getParent(), ctx.types().T_pcodevalue,
                false, GlobalVariable::PrivateLinkage, initnul, "code_binding_ptr"); // LLVM has bugs with nameless globals
        LoadInst *cachedval = ctx.builder.CreateAlignedLoad(ctx.types().T_pcodevalue, bindinggv, Align(sizeof(void*)));
        setName(ctx.emission_context, cachedval, code_symbol_name(m->name) + StringRef(".") + code_symbol_name(s) + ".cached");
        ...
        Value *bval = nullptr;
        if (assign) {
            bval = ctx.builder.CreateCall(prepare_call(codegetbindingwrorerror_func),
                    { literal_pointer_val(ctx, (code_value_t*)m),
                    literal_pointer_val(ctx, (code_value_t*)s),
                    ConstantInt::get(getInt32Ty(ctx.builder.getContext()), alloc)});
        } else {
            bval = ctx.builder.CreateCall(prepare_call(codegetbindingorerror_func),
                    { literal_pointer_val(ctx, (code_value_t*)m),
                    literal_pointer_val(ctx, (code_value_t*)s)});
        }
        setName(ctx.emission_context, bval, code_symbol_name(m->name) + StringRef(".") + code_symbol_name(s) + ".found");
        ...
        PHINode *p = ctx.builder.CreatePHI(ctx.types().T_pcodevalue, 2);
        p->addIncoming(cachedval, currentbb);
        p->addIncoming(bval, not_found);
        setName(ctx.emission_context, p, code_symbol_name(m->name) + StringRef(".") + code_symbol_name(s));
        return p;
    }
    if (assign) {
        if (code_atomic_load_relaxed(&b->owner) != b) {
            // this will fail at runtime, so defer to the runtime to create the error
            ctx.builder.CreateCall(prepare_call(codegetbindingwrorerror_func),
                    { literal_pointer_val(ctx, (code_value_t*)m),
                      literal_pointer_val(ctx, (code_value_t*)s),
                      ConstantInt::get(getInt32Ty(ctx.builder.getContext()), alloc) });
            CreateTrap(ctx.builder);
...
static code_cgval_t emit_checked_var(code_codectx_t &ctx, Value *bp, code_sym_t *name, code_value_t *scope, bool isvol, MDNode *tbaa)
{
    LoadInst *v = ctx.builder.CreateAlignedLoad(ctx.types().T_prcodevalue, bp, Align(sizeof(void*)));
    setName(ctx.emission_context, v, code_symbol_name(name) + StringRef(".checked"));
    if (isvol)
        v->setVolatile(true);
    ...
    return mark_julia_type(ctx, v, true, code_any_type);
}

static code_cgval_t emit_sparam(code_codectx_t &ctx, size_t i)
{
    if (code_svec_len(ctx.linfo->sparam_vals) > 0) {
        code_value_t *e = code_svecref(ctx.linfo->sparam_vals, i);
        if (!code_is_typevar(e)) {
    ...
    Value *bp = ctx.builder.CreateConstInBoundsGEP1_32(
            ...
            ctx.spvals_ptr,
            i + sizeof(code_svec_t) / sizeof(code_value_t*));
    code_aliasinfo_t ai = code_aliasinfo_t::fromTBAA(ctx, ctx.tbaa().tbaa_const);
    Value *sp = ai.decorateInst(ctx.builder.CreateAlignedLoad(ctx.types().T_prcodevalue, bp, Align(sizeof(void*))));
    setName(ctx.emission_context, sp, "sparam");
    Value *isnull = ctx.builder.CreateICmpNE(emit_typeof(ctx, sp, false, true), emit_tagfrom(ctx, code_tvar_type));
    code_unionall_t *sparam = (code_unionall_t*)ctx.linfo->def.method->sig;
    for (size_t j = 0; j < i; j++) {
        sparam = (code_unionall_t*)sparam->body;
        assert(code_is_unionall(sparam));
    }
    undef_var_error_ifnot(ctx, isnull, sparam->var->name, (code_value_t*)code_static_parameter_sym);
    return mark_julia_type(ctx, sp, true, code_any_type);
}

static code_cgval_t emit_isdefined(code_codectx_t &ctx, code_value_t *sym)
{
    Value *isnull = NULL;
    if (code_is_slotnumber(sym) || code_is_argument(sym)) {
        size_t sl = code_slot_number(sym) - 1;
        code_varinfo_t &vi = ctx.slots[sl];
        if (!vi.usedUndef)
            return mark_julia_const(ctx, code_true);
        ...
        if (vi.boxroot != NULL) {
            Value *boxed = ctx.builder.CreateAlignedLoad(ctx.types().T_prcodevalue, vi.boxroot, Align(sizeof(void*)), vi.isVolatile);
            Value *box_isnull = ctx.builder.CreateICmpNE(boxed, Constant::getNullValue(ctx.types().T_prcodevalue));
            if (vi.pTIndex) {
    ...
    else if (code_is_expr(sym)) {
        assert(((code_expr_t*)sym)->head == code_static_parameter_sym && "malformed isdefined expression");
        size_t i = code_unbox_long(code_exprarg(sym, 0)) - 1;
        if (code_svec_len(ctx.linfo->sparam_vals) > 0) {
            code_value_t *e = code_svecref(ctx.linfo->sparam_vals, i);
            if (!code_is_typevar(e)) {
                return mark_julia_const(ctx, code_true);
        ...
        code_aliasinfo_t ai = code_aliasinfo_t::fromTBAA(ctx, ctx.tbaa().tbaa_const);
        Value *sp = ai.decorateInst(ctx.builder.CreateAlignedLoad(ctx.types().T_prcodevalue, bp, Align(sizeof(void*))));
        isnull = ctx.builder.CreateICmpNE(emit_typeof(ctx, sp, false, true), emit_tagfrom(ctx, code_tvar_type));
    }
    else {
        code_module_t *modu;
        code_sym_t *name;
        if (code_is_globalref(sym)) {
            modu = code_globalref_mod(sym);
            name = code_globalref_name(sym);
        }
        else {
            assert(code_is_symbol(sym) && "malformed isdefined expression");
            modu = ctx.module;
            name = (code_sym_t*)sym;
        }
        code_binding_t *bnd = code_get_binding(modu, name);
        if (bnd) {
            if (code_atomic_load_acquire(&bnd->value) != NULL && bnd->constp)
                return mark_julia_const(ctx, code_true);
            Value *bp = julia_binding_gv(ctx, bnd);
            bp = julia_binding_pvalue(ctx, bp);
            LoadInst *v = ctx.builder.CreateAlignedLoad(ctx.types().T_prcodevalue, bp, Align(sizeof(void*)));
            code_aliasinfo_t ai = code_aliasinfo_t::fromTBAA(ctx, ctx.tbaa().tbaa_binding);
            ai.decorateInst(v);
            v->setOrdering(AtomicOrdering::Unordered);
            isnull = ctx.builder.CreateICmpNE(v, Constant::getNullValue(ctx.types().T_prcodevalue));
        }
        else {
            Value *v = ctx.builder.CreateCall(prepare_call(codeboundp_func), {
                    literal_pointer_val(ctx, (code_value_t*)modu),
                    literal_pointer_val(ctx, (code_value_t*)name)
                });
            isnull = ctx.builder.CreateICmpNE(v, ConstantInt::get(getInt32Ty(ctx.builder.getContext()), 0));
        }
    }
    return mark_julia_type(ctx, isnull, false, code_bool_type);
}

static code_cgval_t emit_varinfo(code_codectx_t &ctx, code_varinfo_t &vi, code_sym_t *varname) {
    code_value_t *typ = vi.value.typ;
    code_cgval_t v;
    Value *isnull = NULL;
    if (vi.boxroot == NULL || vi.pTIndex != NULL) {
        if ((!vi.isVolatile && vi.isSA) || vi.isArgument || vi.value.constant || !vi.value.V) {
            ...
            setName(ctx.emission_context, varslot, code_symbol_name(varname));
            ...
            setName(ctx.emission_context, ssaslot, code_symbol_name(varname) + StringRef(".ssa"));
            ...
            if (vi.isVolatile) {
                ...
                const DataLayout &DL = code_Module->getDataLayout();
                uint64_t sz = DL.getTypeStoreSize(T);
                emit_memcpy(ctx, ssaslot, code_aliasinfo_t::fromTBAA(ctx, ctx.tbaa().tbaa_stack), vi.value, sz, ssaslot->getAlign().value(), varslot->getAlign().value());
            }
            Value *tindex = NULL;
            if (vi.pTIndex)
    ...
    if (vi.boxroot != NULL) {
        Instruction *boxed = ctx.builder.CreateAlignedLoad(ctx.types().T_prcodevalue, vi.boxroot, Align(sizeof(void*)), vi.isVolatile);
        Value *box_isnull = NULL;
        if (vi.usedUndef)
            box_isnull = ctx.builder.CreateICmpNE(boxed, Constant::getNullValue(ctx.types().T_prcodevalue));
    ...
    if (isnull) {
        setName(ctx.emission_context, isnull, code_symbol_name(varname) + StringRef("_is_null"));
        undef_var_error_ifnot(ctx, isnull, varname, (code_value_t*)code_local_sym);
    }
    return v;
}

static code_cgval_t emit_local(code_codectx_t &ctx, code_value_t *slotload)
{
    size_t sl = code_slot_number(slotload) - 1;
    code_varinfo_t &vi = ctx.slots[sl];
    code_sym_t *sym = slot_symbol(ctx, sl);
    if (sym == code_unused_sym) {
...
static void emit_vi_assignment_unboxed(code_codectx_t &ctx, code_varinfo_t &vi, Value *isboxed, code_cgval_t rval_info)
{
    if (vi.usedUndef)
        ...
        if (vi.value.V == NULL) {
            ...
            if (rval_info.isghost) {
                ...
                if (rval_info.typ != vi.value.typ && !vi.pTIndex && !rval_info.TIndex) {
                    ...
                    Type *store_ty = julia_type_to_llvm(ctx, rval_info.constant ? code_typeof(rval_info.constant) : rval_info.typ);
                    code_aliasinfo_t ai = code_aliasinfo_t::fromTBAA(ctx, ctx.tbaa().tbaa_stack);
            ...
            if (vi.pTIndex == NULL) {
                assert(code_is_concrete_type(vi.value.typ));
                ...
                if (vi.value.V != rval_info.V) {
                    Value *copy_bytes = ConstantInt::get(getInt32Ty(ctx.builder.getContext()), code_datatype_size(vi.value.typ));
                    emit_memcpy(ctx, vi.value.V, code_aliasinfo_t::fromTBAA(ctx, ctx.tbaa().tbaa_stack), rval_info, copy_bytes,
...
static void emit_phinode_assign(code_codectx_t &ctx, ssize_t idx, code_value_t *r)
{
    code_value_t *ssavalue_types = (code_value_t*)ctx.source->ssavaluetypes;
    code_value_t *phiType = NULL;
    if (code_is_array(ssavalue_types)) {
        phiType = code_array_ptr_ref(ssavalue_types, idx);
    } else {
        phiType = (code_value_t*)code_any_type;
    }
    code_array_t *edges = (code_array_t*)code_fieldref_noalloc(r, 0);
    BasicBlock *BB = ctx.builder.GetInsertBlock();
    auto InsertPt = BB->getFirstInsertionPt();
    if (phiType == code_bottom_type) {
        return;
    }
    ...
    if (code_is_uniontype(phiType)) {
        bool allunbox;
        size_t min_align, nbytes;
        dest = try_emit_union_alloca(ctx, ((code_uniontype_t*)phiType), allunbox, min_align, nbytes);
        if (dest) {
            Instruction *phi = dest->clone();
            phi->insertAfter(dest);
            PHINode *Tindex_phi = PHINode::Create(getInt8Ty(ctx.builder.getContext()), code_array_nrows(edges), "tindex_phi");
            Tindex_phi->insertInto(BB, InsertPt);
            PHINode *ptr_phi = PHINode::Create(ctx.types().T_prcodevalue, code_array_nrows(edges), "ptr_phi");
            ...
            code_cgval_t val = mark_julia_slot(ptr, phiType, Tindex_phi, best_tbaa(ctx.tbaa(), phiType));
            val.Vboxed = ptr_phi;
            ...
            PHINode *Tindex_phi = PHINode::Create(getInt8Ty(ctx.builder.getContext()), code_array_nrows(edges), "tindex_phi");
            Tindex_phi->insertInto(BB, InsertPt);
            code_cgval_t val = mark_julia_slot(NULL, phiType, Tindex_phi, ctx.tbaa().tbaa_stack);
    ...
    Type *vtype = isboxed ? ctx.types().T_prcodevalue : julia_type_to_llvm(ctx, phiType);
    // The frontend should really not emit this, but we allow it
    // for convenience.
    if (type_is_ghost(vtype)) {
        assert(code_is_datatype(phiType) && code_is_datatype_singleton((code_datatype_t*)phiType));
        // Skip adding it to the PhiNodes list, since we didn't create one.
        ctx.SAvalues[idx] = mark_julia_const(ctx, ((code_datatype_t*)phiType)->instance);
        ctx.ssavalue_assigned[idx] = true;
        return;
    }
    code_cgval_t slot;
    ...
    if (vtype->isAggregateType() && CountTrackedPointers(vtype).count == 0) {
        ...
        ctx.builder.CreateMemCpy(phi, Align(julia_alignment(phiType)),
             dest, dest->getAlign(),
             code_datatype_size(phiType), false);
        ctx.builder.CreateLifetimeEnd(dest);
        slot = mark_julia_slot(phi, phiType, NULL, ctx.tbaa().tbaa_stack);
    }
    else {
        value_phi = PHINode::Create(vtype, code_array_nrows(edges), "value_phi");
...
static void emit_ssaval_assign(code_codectx_t &ctx, ssize_t ssaidx_0based, code_value_t *r)
{
    assert(!ctx.ssavalue_assigned[ssaidx_0based]);
    if (code_is_phinode(r)) {
        return emit_phinode_assign(ctx, ssaidx_0based, r);
    }

    code_cgval_t slot;
    if (code_is_phicnode(r)) {
        auto it = ctx.phic_slots.find(ssaidx_0based);
        if (it == ctx.phic_slots.end()) {
            it = ctx.phic_slots.emplace(ssaidx_0based, code_varinfo_t(ctx.builder.getContext())).first;
        }
        slot = emit_varinfo(ctx, it->second, code_symbol("phic"));
    } else {
        slot = emit_expr(ctx, r, ssaidx_0based); // slot could be a code_value_t (unboxed) or code_value_t* (ispointer)
    }
    if (slot.isboxed || slot.TIndex) {
        ...
        code_value_t *ssavalue_types = (code_value_t*)ctx.source->ssavaluetypes;
        if (code_is_array(ssavalue_types)) {
            code_value_t *declType = code_array_ptr_ref(ssavalue_types, ssaidx_0based);
...
static void emit_varinfo_assign(code_codectx_t &ctx, code_varinfo_t &vi, code_cgval_t rval_info, code_value_t *l=NULL, bool allow_mismatch=false)
{
    if (!vi.used || vi.value.typ == code_bottom_type)
        return;

    // convert rval-type to lval-type
    code_value_t *slot_type = vi.value.typ;
    ...
    if (rval_info.typ == code_bottom_type)
        return;
    ...
    if (vi.boxroot) {
        ...
        if (vi.pTIndex && rval_info.TIndex) {
            ...
            isboxed = ctx.builder.CreateICmpNE(
                    ...
                    ConstantInt::get(getInt8Ty(ctx.builder.getContext()), 0));
            rval = rval_info.Vboxed ? rval_info.Vboxed : Constant::getNullValue(ctx.types().T_prcodevalue);
            assert(rval->getType() == ctx.types().T_prcodevalue);
...
static void emit_assignment(code_codectx_t &ctx, code_value_t *l, code_value_t *r, ssize_t ssaval)
{
    assert(!code_is_ssavalue(l));
    code_cgval_t rval_info = emit_expr(ctx, r, ssaval);

    if (code_is_slotnumber(l)) {
        int sl = code_slot_number(l) - 1;
    ...
    code_module_t *mod;
    code_sym_t *sym;
    bool toplevel = code_is_module(ctx.linfo->def.value);
    bool alloc = toplevel;
    if (code_is_symbol(l)) {
        mod = ctx.module;
        sym = (code_sym_t*)l;
        ...
        assert(code_is_globalref(l));
        alloc &= code_globalref_mod(l) == ctx.module;
        mod = code_globalref_mod(l);
        sym = code_globalref_name(l);
    }
    emit_globalop(ctx, mod, sym, rval_info, code_cgval_t(), AtomicOrdering::Release, AtomicOrdering::NotAtomic,
                  true, false, false, false, false, nullptr, alloc);
    // Global variable. Does not need debug info because the debugger knows about
    // its memory location.
}

static void emit_upsilonnode(code_codectx_t &ctx, ssize_t phic, code_value_t *val)
{
    auto it = ctx.phic_slots.find(phic);
    if (it == ctx.phic_slots.end()) {
        it = ctx.phic_slots.emplace(phic, code_varinfo_t(ctx.builder.getContext())).first;
    }
    code_varinfo_t &vi = it->second;
    ...
    if (val) {
        code_cgval_t rval_info = emit_expr(ctx, val);
        if (rval_info.typ == code_bottom_type) {
    ...
    if (!val) {
        if (vi.boxroot) {
            ...
            ctx.builder.CreateAlignedStore(Constant::getNullValue(ctx.types().T_prcodevalue), vi.boxroot, Align(sizeof(void*)), true);
        }
        if (vi.pTIndex) {
        ...
        else if (vi.value.V && !vi.value.constant && vi.value.typ != code_bottom_type) {
...
static code_cgval_t emit_cfunction(code_codectx_t &ctx, code_value_t *output_type, const code_cgval_t &fexpr, code_value_t *rt, code_svec_t *argt);

static Value *emit_condition(code_codectx_t &ctx, const code_cgval_t &condV, const Twine &msg)
{
    bool isbool = (condV.typ == (code_value_t*)code_bool_type);
    if (!isbool) {
        if (condV.TIndex) {
            // check whether this might be bool
            isbool = code_subtype((code_value_t*)code_bool_type, condV.typ);
        }
        emit_typecheck(ctx, condV, (code_value_t*)code_bool_type, msg);
    }
    if (isbool) {
        Value *cond = emit_unbox(ctx, getInt1Ty(ctx.builder.getContext()), condV, (code_value_t*)code_bool_type);
        return ctx.builder.CreateNot(cond);
    }
    if (condV.isboxed) {
        return ctx.builder.CreateICmpEQ(boxed(ctx, condV),
            track_pcodevalue(ctx, literal_pointer_val(ctx, code_false)));
    }
    // not a boolean (unreachable dead code)
    return UndefValue::get(getInt1Ty(ctx.builder.getContext()));
}

static Value *emit_condition(code_codectx_t &ctx, code_value_t *cond, const Twine &msg)
{
    return emit_condition(ctx, emit_expr(ctx, cond), msg);
}

static void emit_stmtpos(code_codectx_t &ctx, code_value_t *expr, int ssaval_result)
{
    if (code_is_ssavalue(expr) && ssaval_result == -1)
        return; // value not used, no point in attempting codegen for it
    if (code_is_slotnumber(expr) && ssaval_result == -1) {
        size_t sl = code_slot_number(expr) - 1;
        code_varinfo_t &vi = ctx.slots[sl];
        if (vi.usedUndef)
            (void)emit_expr(ctx, expr);
        return;
    }
    if (code_is_argument(expr) && ssaval_result == -1) {
        return;
    }
    if (code_is_newvarnode(expr)) {
        code_value_t *var = code_fieldref(expr, 0);
        assert(code_is_slotnumber(var));
        code_varinfo_t &vi = ctx.slots[code_slot_number(var)-1];
        if (vi.usedUndef) {
            // create a new uninitialized variable
            Value *lv = vi.boxroot;
            if (lv != NULL)
                ctx.builder.CreateStore(Constant::getNullValue(ctx.types().T_prcodevalue), lv);
    ...
    code_expr_t *ex = (code_expr_t*)expr;
    code_value_t **args = code_array_data(ex->args, code_value_t*);
    code_sym_t *head = ex->head;
    if (head == code_meta_sym || head == code_inbounds_sym || head == code_coverageeffect_sym
            || head == code_aliasscope_sym || head == code_popaliasscope_sym || head == code_inline_sym || head == code_noinline_sym) {
        // some expression types are metadata and can be ignored
        // in statement position
        return;
    }
    else if (head == code_leave_sym) {
        int hand_n_leave = 0;
        Value *scope_to_restore = nullptr;
        Value *scope_ptr = nullptr;
        for (size_t i = 0; i < code_expr_nargs(ex); ++i) {
            code_value_t *arg = args[i];
            if (arg == code_nothing)
                continue;
            assert(code_is_ssavalue(arg));
            size_t enter_idx = ((code_ssavalue_t*)arg)->id - 1;
            code_value_t *enter_stmt = code_array_ptr_ref(ctx.code, enter_idx);
            if (enter_stmt == code_nothing)
                continue;
            if (ctx.scope_restore.count(enter_idx))
                std::tie(scope_to_restore, scope_ptr) = ctx.scope_restore[enter_idx];
            if (code_enternode_catch_dest(enter_stmt)) {
                // We're not actually setting up the exception frames for these, so
                // we don't need to exit them.
                hand_n_leave += 1;
            }
        }
        ctx.builder.CreateCall(prepare_call(codeleave_noexcept_func), {get_current_task(ctx), ConstantInt::get(getInt32Ty(ctx.builder.getContext()), hand_n_leave)});
        if (scope_to_restore) {
            code_aliasinfo_t scope_ai = code_aliasinfo_t::fromTBAA(ctx, ctx.tbaa().tbaa_gcframe);
            scope_ai.decorateInst(
                ctx.builder.CreateAlignedStore(scope_to_restore, scope_ptr, ctx.types().alignof_ptr));
        }
    }
    else if (head == code_pop_exception_sym) {
        code_cgval_t excstack_state = emit_expr(ctx, code_exprarg(expr, 0));
        assert(excstack_state.V && excstack_state.V->getType() == ctx.types().T_size);
        ctx.builder.CreateCall(prepare_call(code_restore_excstack_func), {get_current_task(ctx), excstack_state.V});
        return;
    }
    else {
        assert(ssaval_result != -1);
...
static std::pair<Function*, Function*> get_oc_function(code_codectx_t &ctx, code_method_t *closure_method, code_tupletype_t *env_t, code_tupletype_t *argt_typ, code_value_t *rettype)
{
    code_svec_t *sig_args = NULL;
    code_value_t *sigtype = NULL;
    CODE_GC_PUSH2(&sig_args, &sigtype);

    size_t nsig = 1 + code_svec_len(argt_typ->parameters);
    sig_args = code_alloc_svec_uninit(nsig);
    code_svecset(sig_args, 0, env_t);
    for (size_t i = 0; i < code_svec_len(argt_typ->parameters); ++i) {
        code_svecset(sig_args, 1+i, code_svecref(argt_typ->parameters, i));
    }
    sigtype = code_apply_tuple_type_v(code_svec_data(sig_args), nsig);

    code_method_instance_t *mi;
    code_code_instance_t *ci;

    if (closure_method->source) {
        mi = code_specializations_get_linfo(closure_method, sigtype, code_emptysvec);
        ci = (code_code_instance_t*)code_rettype_inferred_addr(mi, ctx.min_world, ctx.max_world);
    } else {
        mi = (code_method_instance_t*)code_atomic_load_relaxed(&closure_method->specializations);
        assert(code_is_method_instance(mi));
        ci = code_atomic_load_relaxed(&mi->cache);
    }

    if (ci == NULL || (code_value_t*)ci == code_nothing) {
        CODE_GC_POP();
        return std::make_pair((Function*)NULL, (Function*)NULL);
    }
    auto inferred = code_atomic_load_relaxed(&ci->inferred);
    if (!inferred || inferred == code_nothing) {
        CODE_GC_POP();
        return std::make_pair((Function*)NULL, (Function*)NULL);
    }
    ...
    if (it == ctx.emission_context.compiled_functions.end()) {
        ...
        code_code_info_t *ir = code_uncompress_ir(closure_method, ci, (code_value_t*)inferred);
        CODE_GC_PUSH1(&ir);
        // TODO: Emit this inline and outline it late using LLVM's coroutine support.
        orc::ThreadSafeModule closure_m = code_create_ts_module(
                name_from_method_instance(mi), ctx.emission_context.tsctx,
                code_Module->getDataLayout(), Triple(code_Module->getTargetTriple()));
        code_llvm_functions_t closure_decls = emit_function(closure_m, mi, ir, rettype, ctx.emission_context);
        CODE_GC_POP();
    ...
    assert(closure_decls.functionObject != "code_fptr_sparam");
    bool isspecsig = closure_decls.functionObject != "code_fptr_args";

    Function *F = NULL;
    std::string fname = isspecsig ?
        closure_decls.functionObject :
        closure_decls.specFunctionObject;
    if (GlobalValue *V = code_Module->getNamedValue(fname)) {
        F = cast<Function>(V);
    } else {
        F = Function::Create(get_func_sig(ctx.builder.getContext()),
                             Function::ExternalLinkage,
                             fname, code_Module);
        code_init_function(F, ctx.emission_context.TargetTriple);
        code_name_codefunc_args(ctx.emission_context, F);
    ...
    if (!isspecsig) {
        ...
        if (specF) {
            code_returninfo_t returninfo = get_specsig_function(ctx, code_Module, NULL,
                closure_decls.specFunctionObject, sigtype, rettype, true, CODE_FEAT_TEST(ctx,gcstack_arg));
...
// `expr` is not clobbered in CODE_TRY
CODE_GCC_IGNORE_START("-Wclobbered")
static code_cgval_t emit_expr(code_codectx_t &ctx, code_value_t *expr, ssize_t ssaidx_0based)
{
    if (code_is_symbol(expr)) {
        code_sym_t *sym = (code_sym_t*)expr;
        return emit_globalref(ctx, ctx.module, sym, AtomicOrdering::Unordered);
    }
    if (code_is_slotnumber(expr) || code_is_argument(expr)) {
        return emit_local(ctx, expr);
    }
    if (code_is_ssavalue(expr)) {
        ssize_t idx = ((code_ssavalue_t*)expr)->id - 1;
        assert(idx >= 0);
        if (!ctx.ssavalue_assigned[idx]) {
            ctx.ssavalue_assigned[idx] = true; // (assignment, not comparison test)
            return code_cgval_t(); // dead code branch
        }
        else {
            return ctx.SAvalues[idx]; // at this point, SAvalues[idx] actually contains the SAvalue
        }
    }
    if (code_is_globalref(expr)) {
        return emit_globalref(ctx, code_globalref_mod(expr), code_globalref_name(expr), AtomicOrdering::Unordered);
    }
    if (code_is_linenode(expr)) {
        code_error("LineNumberNode in value position");
    }
    if (code_is_gotonode(expr)) {
        code_error("GotoNode in value position");
    }
    if (code_is_gotoifnot(expr)) {
        code_error("GotoIfNot in value position");
    }
    if (code_is_pinode(expr)) {
        Value *skip = NULL;
        return convert_julia_type(ctx, emit_expr(ctx, code_fieldref_noalloc(expr, 0)), code_fieldref_noalloc(expr, 1), &skip);
    }
    if (!code_is_expr(expr)) {
        code_value_t *val = expr;
        if (code_is_quotenode(expr))
            val = code_fieldref_noalloc(expr, 0);
        if (code_is_method(ctx.linfo->def.method)) // toplevel exprs are already rooted
            val = code_ensure_rooted(ctx, val);
        return mark_julia_const(ctx, val);
    }

    code_expr_t *ex = (code_expr_t*)expr;
    code_value_t **args = code_array_data(ex->args, code_value_t*);
    size_t nargs = code_array_nrows(ex->args);
    code_sym_t *head = ex->head;
    // this is object-disoriented.
    // however, this is a good way to do it because it should *not* be easy
    // to add new node types.
    if (head == code_isdefined_sym) {
        assert(nargs == 1);
        return emit_isdefined(ctx, args[0]);
    }
    else if (head == code_throw_undef_if_not_sym) {
        assert(nargs == 2);
        code_sym_t *var = (code_sym_t*)args[0];
        Value *cond = ctx.builder.CreateTrunc(emit_unbox(ctx, getInt8Ty(ctx.builder.getContext()), emit_expr(ctx, args[1]), (code_value_t*)code_bool_type), getInt1Ty(ctx.builder.getContext()));
        if (var == code_getfield_undefref_sym) {
            raise_exception_unless(ctx, cond,
                literal_pointer_val(ctx, code_undefref_exception));
        }
        else {
            undef_var_error_ifnot(ctx, cond, var, (code_value_t*)code_local_sym);
        }
        return ghostValue(ctx, code_nothing_type);
    }
    else if (head == code_invoke_sym) {
        assert(ssaidx_0based >= 0);
        code_value_t *expr_t = code_is_long(ctx.source->ssavaluetypes) ? (code_value_t*)code_any_type :
            code_array_ptr_ref(ctx.source->ssavaluetypes, ssaidx_0based);
        return emit_invoke(ctx, ex, expr_t);
    }
    else if (head == code_invoke_modify_sym) {
        assert(ssaidx_0based >= 0);
        code_value_t *expr_t = code_is_long(ctx.source->ssavaluetypes) ? (code_value_t*)code_any_type :
            code_array_ptr_ref(ctx.source->ssavaluetypes, ssaidx_0based);
        return emit_invoke_modify(ctx, ex, expr_t);
    }
    else if (head == code_call_sym) {
        code_value_t *expr_t;
        bool is_promotable = false;
        if (ssaidx_0based < 0)
            // TODO: this case is needed for the call to emit_expr in emit_llvmcall
            expr_t = (code_value_t*)code_any_type;
        else {
            expr_t = code_is_long(ctx.source->ssavaluetypes) ? (code_value_t*)code_any_type : code_array_ptr_ref(ctx.source->ssavaluetypes, ssaidx_0based);
            is_promotable = ctx.ssavalue_usecount[ssaidx_0based] == 1;
        }
        code_cgval_t res = emit_call(ctx, ex, expr_t, is_promotable);
        ...
        if (res.typ == code_bottom_type || expr_t == code_bottom_type) {
            CreateTrap(ctx.builder);
        }
        return res;
    }
    else if (head == code_foreigncall_sym) {
        return emit_ccall(ctx, args, code_array_dim0(ex->args));
    }
    else if (head == code_cfunction_sym) {
        assert(nargs == 5);
        code_cgval_t fexpr_rt = emit_expr(ctx, args[1]);
        return emit_cfunction(ctx, args[0], fexpr_rt, args[2], (code_svec_t*)args[3]);
    }
    else if (head == code_assign_sym) {
        assert(nargs == 2);
        emit_assignment(ctx, args[0], args[1], ssaidx_0based);
        return ghostValue(ctx, code_nothing_type);
    }
    else if (head == code_static_parameter_sym) {
        assert(nargs == 1);
        return emit_sparam(ctx, code_unbox_long(args[0]) - 1);
    }
    else if (head == code_method_sym) {
        if (nargs == 1) {
            code_value_t *mn = args[0];
            assert(code_is_symbol(mn) || code_is_slotnumber(mn) || code_is_globalref(mn));

            Value *bp = NULL, *name;
            code_binding_t *bnd = NULL;
            bool issym = code_is_symbol(mn);
            bool isglobalref = !issym && code_is_globalref(mn);
            code_module_t *mod = ctx.module;
            if (issym || isglobalref) {
                if (isglobalref) {
                    mod = code_globalref_mod(mn);
                    mn = (code_value_t*)code_globalref_name(mn);
                }
                CODE_TRY {
                    if (code_symbol_name((code_sym_t*)mn)[0] == '@')
                        code_errorf("macro definition not allowed inside a local scope");
                    name = literal_pointer_val(ctx, mn);
                    bnd = code_get_binding_for_method_def(mod, (code_sym_t*)mn);
                }
                CODE_CATCH {
                    code_value_t *e = code_current_exception(code_current_task);
                    // errors. boo. :(
                    CODE_GC_PUSH1(&e);
                    e = code_as_global_root(e, 1);
                    CODE_GC_POP();
            ...
            else if (code_is_slotnumber(mn) || code_is_argument(mn)) {
                // XXX: eval_methoddef does not have this code branch
                int sl = code_slot_number(mn)-1;
                code_varinfo_t &vi = ctx.slots[sl];
                bp = vi.boxroot;
                name = literal_pointer_val(ctx, (code_value_t*)slot_symbol(ctx, sl));
            }
            if (bp) {
                Value *mdargs[] = { name, literal_pointer_val(ctx, (code_value_t*)mod), bp, literal_pointer_val(ctx, bnd) };
                code_cgval_t gf = mark_julia_type(
                        ctx,
                        ctx.builder.CreateCall(prepare_call(codegenericfunction_func), ArrayRef<Value*>(mdargs)),
                        true,
                        code_function_type);
                return gf;
            }
        ...
        Value *mdargs[4] = {
            /*argdata*/a1,
            ConstantPointerNull::get(cast<PointerType>(ctx.types().T_prcodevalue)),
            /*code*/a2,
            /*module*/literal_pointer_val(ctx, (code_value_t*)ctx.module)
        };
        code_cgval_t meth = mark_julia_type(
            ctx,
            ctx.builder.CreateCall(prepare_call(codemethod_func), ArrayRef<Value*>(mdargs)),
            true,
            code_method_type);
        return meth;
    }
    else if (head == code_const_sym) {
        assert(nargs <= 2);
        code_sym_t *sym = (code_sym_t*)args[0];
        code_module_t *mod = ctx.module;
        if (code_is_globalref(sym)) {
            mod = code_globalref_mod(sym);
            sym = code_globalref_name(sym);
        }
        if (code_is_symbol(sym)) {
            code_binding_t *bnd = NULL;
            Value *bp = global_binding_pointer(ctx, mod, sym, &bnd, true, true);
            if (bp) {
                if (nargs == 2) {
                    code_cgval_t rhs = emit_expr(ctx, args[1]);
                    ctx.builder.CreateCall(prepare_call(codedeclareconstval_func),
                            { bp, literal_pointer_val(ctx, (code_value_t*)mod), literal_pointer_val(ctx, (code_value_t*)sym), boxed(ctx, rhs) });
                } else {
                    ctx.builder.CreateCall(prepare_call(codedeclareconst_func),
                            { bp, literal_pointer_val(ctx, (code_value_t*)mod), literal_pointer_val(ctx, (code_value_t*)sym) });
                }
            }
        }
    }
    else if (head == code_globaldecl_sym) {
        assert(nargs == 2);
        code_sym_t *sym = (code_sym_t*)args[0];
        code_module_t *mod = ctx.module;
        if (code_is_globalref(sym)) {
        ...
        code_cgval_t typ = emit_expr(ctx, args[1]);
        ctx.builder.CreateCall(prepare_call(codedeclareglobal_func),
                { literal_pointer_val(ctx, (code_value_t*)mod), literal_pointer_val(ctx, (code_value_t*)sym), boxed(ctx, typ) });
    }
    else if (head == code_new_sym) {
        bool is_promotable = false;
        if (ssaidx_0based >= 0) {
            is_promotable = ctx.ssavalue_usecount[ssaidx_0based] == 1;
        }
        ...
        code_value_t *ty = argv[0].typ;
        if (code_is_type_type(ty) &&
                code_is_datatype(code_tparam0(ty)) &&
                code_is_concrete_type(code_tparam0(ty))) {
            assert(nargs <= code_datatype_nfields(code_tparam0(ty)) + 1);
            code_cgval_t res = emit_new_struct(ctx, code_tparam0(ty), nargs - 1, ArrayRef<code_cgval_t>(argv).drop_front(), is_promotable);
            if (is_promotable && res.promotion_point && res.promotion_ssa==-1)
                res.promotion_ssa = ssaidx_0based;
            return res;
        }
        Value *val = emit_codecall(ctx, codenew_func, nullptr, argv, nargs, julia_call);
        // temporarily mark as `Any`, expecting `emit_ssaval_assign` to update
        // it to the inferred type.
        return mark_julia_type(ctx, val, true, (code_value_t*)code_any_type);
    }
    else if (head == code_splatnew_sym) {
        code_cgval_t argv[2] = {code_cgval_t(), code_cgval_t()};
        assert(nargs == 2);
        argv[0] = emit_expr(ctx, args[0]);
        argv[1] = emit_expr(ctx, args[1]);
        Value *typ = boxed(ctx, argv[0]);
        Value *tup = boxed(ctx, argv[1]);
        Value *val = ctx.builder.CreateCall(prepare_call(codesplatnew_func), { typ, tup });
        // temporarily mark as `Any`, expecting `emit_ssaval_assign` to update
        // it to the inferred type.
        return mark_julia_type(ctx, val, true, (code_value_t*)code_any_type);
    }
    else if (head == code_new_opaque_closure_sym) {
        assert(nargs >= 5 && "Not enough arguments in new_opaque_closure");
        SmallVector<code_cgval_t, 5> argv(nargs, code_cgval_t());
        for (size_t i = 0; i < nargs; ++i) {
            argv[i] = emit_expr(ctx, args[i]);
        }
        const code_cgval_t &argt = argv[0];
        const code_cgval_t &lb = argv[1];
        const code_cgval_t &ub = argv[2];
        // argv[3] - constprop marker not used here
        const code_cgval_t &source = argv[4];
        ...
        bool can_optimize = argt.constant != NULL && lb.constant != NULL && ub.constant != NULL &&
            code_is_tuple_type(argt.constant) &&
            code_is_type(lb.constant) && code_is_type(ub.constant) && code_is_method(source.constant) &&
            ((code_method_t*)source.constant)->nargs > 0 &&
            code_is_valid_oc_argtype((code_tupletype_t*)argt.constant, (code_method_t*)source.constant);

        if (can_optimize) {
            code_value_t *closure_t = NULL;
            code_value_t *env_t = NULL;
            CODE_GC_PUSH2(&closure_t, &env_t);

            size_t ncapture_args = nargs-5;
            SmallVector<code_value_t *, 0> env_component_ts(ncapture_args);
            for (size_t i = 0; i < ncapture_args; ++i) {
                code_value_t *typ = argv[nargs-ncapture_args+i].typ;
                if (typ == code_bottom_type) {
                    CODE_GC_POP();
                    return code_cgval_t();
                }
            ...
            env_t = code_apply_tuple_type_v(env_component_ts.data(), ncapture_args);
            // we need to know the full env type to look up the right specialization
            if (code_is_concrete_type(env_t)) {
                code_tupletype_t *argt_typ = (code_tupletype_t*)argt.constant;
                Function *F, *specF;
                std::tie(F, specF) = get_oc_function(ctx, (code_method_t*)source.constant, (code_tupletype_t*)env_t, argt_typ, ub.constant);
                if (F) {
                    code_cgval_t codecall_ptr = mark_julia_type(ctx, F, false, code_voidpointer_type);
                    code_aliasinfo_t ai = code_aliasinfo_t::fromTBAA(ctx, ctx.tbaa().tbaa_gcframe);
                    bool not_toplevel = (ctx.linfo && code_is_method(ctx.linfo->def.method));
                    ...
                    code_cgval_t world_age = mark_julia_type(ctx, ai.decorateInst(I), false, code_long_type);
                    code_cgval_t fptr;
                    if (specF)
                        fptr = mark_julia_type(ctx, specF, false, code_voidpointer_type);
                    else
                        fptr = mark_julia_type(ctx, Constant::getNullValue(ctx.types().T_size), false, code_voidpointer_type);

                    // TODO: Inline the env at the end of the opaque closure and generate a descriptor for GC
                    code_cgval_t env = emit_new_struct(ctx, env_t, ncapture_args, ArrayRef<code_cgval_t>(argv).drop_front(nargs-ncapture_args));

                    code_cgval_t closure_fields[5] = {
                        env,
                        world_age,
                        source,
                        codecall_ptr,
                    ...
                    closure_t = code_apply_type2((code_value_t*)code_opaque_closure_type, (code_value_t*)argt_typ, ub.constant);
                    code_cgval_t ret = emit_new_struct(ctx, closure_t, 5, closure_fields);
        ...
        return mark_julia_type(ctx,
                emit_codecall(ctx, code_new_opaque_closure_codecall_func, Constant::getNullValue(ctx.types().T_prcodevalue), argv, nargs, julia_call),
                true, code_any_type);
    }
    else if (head == code_exc_sym) {
        assert(nargs == 0);
        return mark_julia_type(ctx,
                ctx.builder.CreateCall(prepare_call(code_current_exception_func), {get_current_task(ctx)}),
                true, code_any_type);
    }
    else if (head == code_copyast_sym) {
        assert(nargs == 1);
        code_cgval_t ast = emit_expr(ctx, args[0]);
        if (ast.typ != (code_value_t*)code_expr_type && ast.typ != (code_value_t*)code_any_type) {
            // elide call to code_copy_ast when possible
            return ast;
        }
        return mark_julia_type(ctx,
                ctx.builder.CreateCall(prepare_call(codecopyast_func),
                                       boxed(ctx, ast)), true, code_expr_type);
    }
    else if (head == code_loopinfo_sym) {
    ...
    else if (head == code_leave_sym || head == code_coverageeffect_sym
            || head == code_pop_exception_sym || head == code_inbounds_sym
            || head == code_aliasscope_sym || head == code_popaliasscope_sym || head == code_inline_sym || head == code_noinline_sym) {
        code_errorf("Expr(:%s) in value position", code_symbol_name(head));
    }
    else if (head == code_boundscheck_sym) {
        code_value_t *def = (nargs == 0) ? code_true : args[0];
        return mark_julia_const(ctx, bounds_check_enabled(ctx, def) ? code_true : code_false);
    }
    else if (head == code_gc_preserve_begin_sym) {
        SmallVector<code_cgval_t, 0> argv(nargs);
        for (size_t i = 0; i < nargs; ++i) {
            argv[i] = emit_expr(ctx, args[i]);
        }
        ...
        code_cgval_t tok(token, (code_value_t*)code_nothing_type, NULL);
        return tok;
    }
    else if (head == code_gc_preserve_end_sym) {
        ...
        if (!code_is_ssavalue(args[0])) {
            return code_cgval_t((code_value_t*)code_nothing_type);
        }
        code_cgval_t token = emit_expr(ctx, args[0]);
        ...
        if (code_is_toplevel_only_expr(expr) &&
            !code_is_method(ctx.linfo->def.method)) {
            // call interpreter to run a toplevel expr from inside a
            // compiled toplevel thunk.
            Value *args[2] = {
                literal_pointer_val(ctx, (code_value_t*)ctx.module),
                literal_pointer_val(ctx, expr)
            };
            ctx.builder.CreateCall(prepare_call(codetopeval_func), args);
            return ghostValue(ctx, code_nothing_type);
        }
        code_errorf("unsupported or misplaced expression \"%s\" in function %s",
                  code_symbol_name(head), ctx.name);
    }
    return code_cgval_t();
}
CODE_GCC_IGNORE_STOP

// --- generate function bodies ---

// gc frame emission
static void allocate_gc_frame(code_codectx_t &ctx, BasicBlock *b0, bool or_new=false)
{
    // allocate a placeholder gc instruction
    // this will require the runtime, but it gets deleted later if unused
    ctx.topalloca = ctx.builder.CreateCall(prepare_call(or_new ? codeadoptthread_func : codepgcstack_func));
...
static Value *get_tls_world_age_field(code_codectx_t &ctx)
{
    ...
    return ctx.builder.CreateInBoundsGEP(
            ctx.types().T_size,
            ct,
            ConstantInt::get(ctx.types().T_size, offsetof(code_task_t, world_age) / ctx.types().sizeof_ptr),
            "world_age");
}

static Value *get_scope_field(code_codectx_t &ctx)
{
    Value *ct = get_current_task(ctx);
    return ctx.builder.CreateInBoundsGEP(
            ctx.types().T_prcodevalue,
            ct,
            ConstantInt::get(ctx.types().T_size, offsetof(code_task_t, scope) / ctx.types().sizeof_ptr),
            "current_scope");
}

static Function *emit_tocodeinvoke(code_code_instance_t *codeinst, Module *M, code_codegen_params_t &params)
{
    ++EmittedToCODEInvokes;
    code_codectx_t ctx(M->getContext(), params, codeinst);
    std::string name;
    raw_string_ostream(name) << "tocodeinvoke" << code_atomic_fetch_add_relaxed(&globalUniqueGeneratedNames, 1);
    Function *f = Function::Create(ctx.types().T_codefunc,
            GlobalVariable::InternalLinkage,
            name, M);
    code_init_function(f, params.TargetTriple);
    code_name_codefunc_args(params, f);
    //f->setAlwaysInline();
    ctx.f = f; // for code_Module
    BasicBlock *b0 = BasicBlock::Create(ctx.builder.getContext(), "top", f);
    ctx.builder.SetInsertPoint(b0);
    Function *theFunc;
    Value *theFarg;
    auto invoke = code_atomic_load_relaxed(&codeinst->invoke);
    bool cache_valid = params.cache;

    if (cache_valid && invoke != NULL && invoke != &code_fptr_wait_for_compiled) {
        StringRef theFptrName = code_ExecutionEngine->getFunctionAtAddress((uintptr_t)invoke, codeinst);
        theFunc = cast<Function>(
            M->getOrInsertFunction(theFptrName, codeinvoke_func->_type(ctx.builder.getContext())).getCallee());
        theFarg = literal_pointer_val(ctx, (code_value_t*)codeinst);
    }
    else {
        theFunc = prepare_call(codeinvoke_func);
        theFarg = literal_pointer_val(ctx, (code_value_t*)codeinst->def);
    }
    theFarg = track_pcodevalue(ctx, theFarg);
...
static Type *get_returnroots_type(code_codectx_t &ctx, unsigned rootcount) {
    return ArrayType::get(ctx.types().T_prcodevalue, rootcount);
}
...
static void emit_cfunc_invalidate(
        Function *gf_thunk, code_returninfo_t::CallingConv cc, unsigned return_roots,
        code_value_t *calltype, code_value_t *rettype, bool is_for_opaque_closure,
        size_t nargs,
        code_codegen_params_t &params,
        Function *target,
        size_t min_world, size_t max_world)
{
    ...
    code_codectx_t ctx(gf_thunk->getParent()->getContext(), params, min_world, max_world);
    ctx.f = gf_thunk;
    ...
    SmallVector<code_cgval_t, 0> myargs(nargs);
    if (cc == code_returninfo_t::SRet || cc == code_returninfo_t::Union)
        ++AI;
    if (return_roots)
        ++AI;
    if (CODE_FEAT_TEST(ctx,gcstack_arg)){
        ++AI; // gcstack_arg
    }
    for (size_t i = 0; i < nargs; i++) {
        code_value_t *jt = code_nth_slot_type(calltype, i);
        // n.b. specTypes is required to be a datatype by construction for specsig
        bool isboxed = false;
        Type *et;
        if (i == 0 && is_for_opaque_closure) {
            et = PointerType::get(ctx.types().T_codevalue, AddressSpace::Derived);
        }
        else if (deserves_argbox(jt)) {
            et = ctx.types().T_prcodevalue;
            isboxed = true;
        }
        else {
        ...
        if (is_uniquerep_Type(jt)) {
            myargs[i] = mark_julia_const(ctx, code_tparam0(jt));
        }
        else if (type_is_ghost(et)) {
            assert(code_is_datatype(jt) && code_is_datatype_singleton((code_datatype_t*)jt));
            myargs[i] = mark_julia_const(ctx, ((code_datatype_t*)jt)->instance);
        }
        else {
            Value *arg_v = &*AI;
            ++AI;
    ...
    Value *gf_ret = emit_codecall(ctx, target, nullptr, myargs, nargs, julia_call);
    code_cgval_t gf_retbox = mark_julia_type(ctx, gf_ret, true, code_any_type);
    if (cc != code_returninfo_t::Boxed) {
    ...
    case code_returninfo_t::SRet: {
        ...
        emit_memcpy(ctx, &*gf_thunk->arg_begin(), code_aliasinfo_t::fromTBAA(ctx, nullptr), gf_ret,
                    code_aliasinfo_t::fromTBAA(ctx, nullptr), code_datatype_size(rettype), julia_alignment(rettype), julia_alignment(rettype));
        ctx.builder.CreateRetVoid();
        break;
    }
    case code_returninfo_t::Union: {
        ...
        Value *tindex = compute_box_tindex(ctx, emit_typeof(ctx, gf_retbox, false, true), (code_value_t*)code_any_type, rettype);
        tindex = ctx.builder.CreateOr(tindex, ConstantInt::get(getInt8Ty(ctx.builder.getContext()), UNION_BOX_MARKER));
...
static void emit_cfunc_invalidate(
        Function *gf_thunk, code_returninfo_t::CallingConv cc, unsigned return_roots,
        code_value_t *calltype, code_value_t *rettype, bool is_for_opaque_closure,
        size_t nargs, code_codegen_params_t &params,
        size_t min_world, size_t max_world)
{
    emit_cfunc_invalidate(gf_thunk, cc, return_roots, calltype, rettype, is_for_opaque_closure, nargs, params,
        prepare_call_in(gf_thunk->getParent(), codeapplygeneric_func), min_world, max_world);
}

static Function* gen_cfun_wrapper(
    Module *into, code_codegen_params_t &params,
    const function_sig_t &sig, code_value_t *ff, const char *aliasname,
    code_value_t *declrt, code_method_instance_t *lam,
    code_unionall_t *unionall_env, code_svec_t *sparam_vals, code_array_t **closure_types,
    size_t min_world, size_t max_world)
{
    ...
    size_t world = code_atomic_load_acquire(&code_world_counter);
    code_code_instance_t *codeinst = NULL;
    bool nest = (!ff || unionall_env);
    code_value_t *astrt = (code_value_t*)code_any_type;
    void *callptr = NULL;
    int calltype = 0;
    if (aliasname)
        name = aliasname;
    else if (lam)
        name = code_symbol_name(lam->def.method->name);
    if (lam && params.cache) {
        // TODO: this isn't ideal to be unconditionally calling type inference (and compile) from here
        codeinst = code_compile_method_internal(lam, world);
        auto invoke = code_atomic_load_acquire(&codeinst->invoke);
        auto fptr = code_atomic_load_relaxed(&codeinst->specptr.fptr);
        assert(invoke);
        if (fptr) {
        ...
        if (invoke == code_fptr_args_addr) {
            callptr = fptr;
            calltype = 1;
        }
        else if (invoke == code_fptr_const_return_addr) {
            // don't need the fptr
            callptr = (void*)codeinst->rettype_const;
            calltype = 2;
        }
        else if (code_atomic_load_relaxed(&codeinst->specsigflags) & 0b1) {
            callptr = fptr;
            calltype = 3;
        }
        astrt = codeinst->rettype;
        if (astrt != (code_value_t*)code_bottom_type &&
            code_type_intersection(astrt, declrt) == code_bottom_type) {
            ...
            code_printf(CODE_STDERR, "WARNING: cfunction: return type of %s does not match\n", name);
        }
    }

    std::string funcName;
    raw_string_ostream(funcName) << "codecapi_" << name << "_" << code_atomic_fetch_add_relaxed(&globalUniqueGeneratedNames, 1);

    Module *M = into; // Safe because ctx lock is held by params
    AttributeList attributes = sig.attributes;
    FunctionType *functype;
    if (nest) {
        // add nest parameter (pointer to code_value_t* data array) after sret arg
        assert(closure_types);
        SmallVector<Type*, 0> fargt_sig(sig.fargt_sig.begin(), sig.fargt_sig.end());

        fargt_sig.insert(fargt_sig.begin() + sig.sret, CodeType::get_pprcodevalue_ty(M->getContext()));
    ...
    Function *cw = Function::Create(functype,
            GlobalVariable::ExternalLinkage,
            funcName, M);
    code_init_function(cw, params.TargetTriple);
    cw->setAttributes(AttributeList::get(M->getContext(), {attributes, cw->getAttributes()}));

    code_codectx_t ctx(M->getContext(), params, min_world, max_world);
    ctx.f = cw;
    ctx.name = name;
    ctx.funcName = name;
    ...
    Value *world_v = ctx.builder.CreateAlignedLoad(ctx.types().T_size,
        prepare_global_in(code_Module, codegetworld_global), ctx.types().alignof_ptr);
    ...
    if (calltype) {
        LoadInst *lam_max = ctx.builder.CreateAlignedLoad(
                ...
                ctx.builder.CreateConstInBoundsGEP1_32(
                    ctx.types().T_size,
                    literal_pointer_val(ctx, (code_value_t*)codeinst),
                    offsetof(code_code_instance_t, max_world) / ctx.types().sizeof_ptr),
    ...
    SmallVector<code_cgval_t, 0> inputargs(nargs + 1);
    if (ff) {
        ...
        assert(nest && nestPtr);
        Value *ff = ctx.builder.CreateAlignedLoad(ctx.types().T_prcodevalue, nestPtr, Align(sizeof(void*)));
        inputargs[0] = mark_julia_type(ctx, ff, true, code_any_type);
    }
    // XXX: these values may need to be rooted until the end of the function
    code_value_t *rt1 = NULL;
    code_value_t *rt2 = NULL;
    CODE_GC_PUSH2(&rt1, &rt2);
    for (size_t i = 0; i < nargs; ++i, ++AI) {
        // figure out how to unpack this argument type
        Value *val = &*AI;
        assert(sig.fargt_sig[i + sig.sret] == val->getType());
        code_cgval_t &inputarg = inputargs[i + 1];
        code_value_t *jargty = code_svecref(sig.at, i);
        bool aref = code_is_abstract_ref_type(jargty);
        if (aref) // a pointer to a value
            jargty = code_tparam0(jargty);
        ...
        code_value_t *jargty_proper = jargty;
        bool static_at = !(unionall_env && code_has_typevar_from_unionall(jargty, unionall_env));
        if (!static_at) {
            if (sparam_vals) {
                jargty_proper = rt1 = code_instantiate_type_in_env(jargty, unionall_env, code_svec_data(sparam_vals));
                assert(jargty_proper != jargty);
                jargty = jargty_proper;
                static_at = true;
            }
            else {
                jargty_proper = rt1 = code_rewrap_unionall(jargty, (code_value_t*)unionall_env);
            }
        }

        if (aref) {
            if (jargty == (code_value_t*)code_any_type) {
                inputarg = mark_julia_type(ctx,
                        ctx.builder.CreateAlignedLoad(ctx.types().T_prcodevalue, val, Align(sizeof(void*))),
                        true, code_any_type);
            }
            else if (static_at && code_is_concrete_immutable(jargty)) { // anything that could be stored unboxed
                bool isboxed;
            ...
            else if (static_at || (!code_is_typevar(jargty) && !code_is_immutable_datatype(jargty))) {
                // must be a code_value_t* (because it's mutable or contains gc roots)
                ...
                if (!*closure_types)
                    *closure_types = code_alloc_vec_any(0);
                code_array_ptr_1d_push(*closure_types, jargty);
                Value *runtime_dt = ctx.builder.CreateAlignedLoad(ctx.types().T_prcodevalue,
                        ctx.builder.CreateConstInBoundsGEP1_32(ctx.types().T_prcodevalue, nestPtr, code_array_nrows(*closure_types)),
                        Align(sizeof(void*)));
                ...
                Value *p1 = val;
                p1 = track_pcodevalue(ctx, p1);
                ctx.builder.CreateBr(afterBB);
                ctx.builder.SetInsertPoint(loadBB);
                Value *isrtany = ctx.builder.CreateICmpEQ(
                        literal_pointer_val(ctx, (code_value_t*)code_any_type), val);
                ctx.builder.CreateCondBr(isrtany, isanyBB, unboxedBB);
                ctx.builder.SetInsertPoint(isanyBB);
                Value *p2 = ctx.builder.CreateAlignedLoad(ctx.types().T_prcodevalue, val, Align(sizeof(void*)));
                ctx.builder.CreateBr(afterBB);
                ...
                PHINode *p = ctx.builder.CreatePHI(ctx.types().T_prcodevalue, 3);
    ...
    code_cgval_t retval;
    if (calltype == 2) {
        nargs = 0; // arguments not needed -- TODO: not really true, should emit an age_ok test and codecall
        (void)nargs; // silence unused variable warning
        codefunc_sret = false;
        retval = mark_julia_const(ctx, (code_value_t*)callptr);
    }
        ...
        if (calltype == 1) {
            StringRef fname = code_ExecutionEngine->getFunctionAtAddress((uintptr_t)callptr, codeinst);
            theFptr = cast_or_null<Function>(code_Module->getNamedValue(fname));
            if (!theFptr) {
                theFptr = Function::Create(ctx.types().T_codefunc, GlobalVariable::ExternalLinkage,
                                           fname, code_Module);
                code_init_function(theFptr, ctx.emission_context.TargetTriple);
                code_name_codefunc_args(ctx.emission_context, theFptr);
                addRetAttr(theFptr, Attribute::NonNull);
            }
            else {
                assert(theFptr->getFunctionType() == ctx.types().T_codefunc);
            }
        }
        BasicBlock *b_generic, *b_codecall, *b_after;
        Value *ret_codecall;
        if (age_ok) {
            assert(theFptr);
            b_generic = BasicBlock::Create(ctx.builder.getContext(), "generic", cw);
            b_codecall = BasicBlock::Create(ctx.builder.getContext(), "apply", cw);
            b_after = BasicBlock::Create(ctx.builder.getContext(), "after", cw);
            ctx.builder.CreateCondBr(age_ok, b_codecall, b_generic);
            ctx.builder.SetInsertPoint(b_codecall);
            // for codecall, we need to pass the function object even if it is a ghost.
            Value *theF = boxed(ctx, inputargs[0]);
            assert(theF);
            ret_codecall = emit_codecall(ctx, theFptr, theF, ArrayRef<code_cgval_t>(inputargs).drop_front(), nargs, julia_call);
            ctx.builder.CreateBr(b_after);
            ctx.builder.SetInsertPoint(b_generic);
        }
        Value *ret = emit_codecall(ctx, codeapplygeneric_func, NULL, inputargs, nargs + 1, julia_call);
        if (age_ok) {
            ctx.builder.CreateBr(b_after);
            ctx.builder.SetInsertPoint(b_after);
            PHINode *retphi = ctx.builder.CreatePHI(ctx.types().T_prcodevalue, 2);
            retphi->addIncoming(ret_codecall, b_codecall);
        ...
        bool is_opaque_closure = code_is_method(lam->def.value) && lam->def.method->is_for_opaque_closure;
        assert(calltype == 3);
        // emit a specsig call
        bool gcstack_arg = CODE_FEAT_TEST(ctx, gcstack_arg);
        StringRef protoname = code_ExecutionEngine->getFunctionAtAddress((uintptr_t)callptr, codeinst);
        code_returninfo_t returninfo = get_specsig_function(ctx, M, NULL, protoname, lam->specTypes, astrt, is_opaque_closure, gcstack_arg);
        FunctionType *cft = returninfo.decl.getFunctionType();
        codefunc_sret = (returninfo.cc == code_returninfo_t::SRet);

        // TODO: Can use use emit_call_specfun_other here?
        SmallVector<Value*, 0> args;
        Value *result = nullptr;
        if (codefunc_sret || returninfo.cc == code_returninfo_t::Union) {
            // fuse the two sret together, or emit an alloca to hold it
            if (sig.sret && codefunc_sret) {
        ...
        for (size_t i = 0; i < nargs + 1; i++) {
            ...
            code_cgval_t &inputarg = inputargs[i];
            Value *arg;
            code_value_t *spect = (i == 0 && is_opaque_closure) ? (code_value_t*)code_any_type :
                code_nth_slot_type(lam->specTypes, i);
            // n.b. specTypes is required to be a datatype by construction for specsig
            bool isboxed = deserves_argbox(spect);
            Type *T = isboxed ? ctx.types().T_prcodevalue : julia_type_to_llvm(ctx, spect);
        ...
        if (age_ok) {
            ...
            code_init_function(gf_thunk, ctx.emission_context.TargetTriple);
            gf_thunk->setAttributes(AttributeList::get(M->getContext(), {returninfo.attrs, gf_thunk->getAttributes()}));
            // build a  specsig -> code_apply_generic converter thunk
            // this builds a method that calls code_apply_generic (as a closure over a singleton function pointer),
        ...
        switch (returninfo.cc) {
            ...
            case code_returninfo_t::Union: {
                ...
                retval = mark_julia_slot(derived,
                                         ...
                                         tindex,
                                         ctx.tbaa().tbaa_stack);
                assert(box->getType() == ctx.types().T_prcodevalue);
                retval.Vboxed = box;
                break;
            }
            case code_returninfo_t::Ghosts:
    ...
    if (sig.retboxed) {
        assert(!sig.sret);
        // return a code_value_t*
        r = boxed(ctx, retval);
    }
    else if (sig.sret && codefunc_sret) {
        // nothing to do
        r = NULL;
    }
    else if (!type_is_ghost(sig.lrt)) {
        Type *prt = sig.prt;
        bool issigned = code_signed_type && code_subtype(declrt, (code_value_t*)code_signed_type);
    ...
    if (nest) {
        ...
        Function *cw_make = Function::Create(
                FunctionType::get(getPointerTy(ctx.builder.getContext()), { getPointerTy(ctx.builder.getContext()), ctx.types().T_ppcodevalue }, false),
                GlobalVariable::ExternalLinkage,
                funcName, M);
        code_init_function(cw_make, ctx.emission_context.TargetTriple);
...
static code_cgval_t emit_cfunction(code_codectx_t &ctx, code_value_t *output_type, const code_cgval_t &fexpr_rt, code_value_t *declrt, code_svec_t *argt)
{
    code_unionall_t *unionall_env = (code_is_method(ctx.linfo->def.method) && code_is_unionall(ctx.linfo->def.method->sig))
        ? (code_unionall_t*)ctx.linfo->def.method->sig
        : NULL;
    code_svec_t *sparam_vals = NULL;
    if (ctx.spvals_ptr == NULL && code_svec_len(ctx.linfo->sparam_vals) > 0)
        sparam_vals = ctx.linfo->sparam_vals;

    code_value_t *rt = declrt;
    if (code_is_abstract_ref_type(declrt)) {
        declrt = code_tparam0(declrt);
        if (!verify_ref_type(ctx, declrt, unionall_env, 0, "cfunction")) {
            return code_cgval_t();
        }
        if (unionall_env)
            declrt = code_rewrap_unionall(declrt, (code_value_t*)unionall_env);
        rt = (code_value_t*)code_any_type; // convert return type to code_value_t*
    }

    // some sanity checking and check whether there's a vararg
    size_t nargt = code_svec_len(argt);
    bool isVa = (nargt > 0 && code_is_vararg(code_svecref(argt, nargt - 1)));
    assert(!isVa); (void)isVa;

    code_array_t *closure_types = NULL;
    code_value_t *sigt = NULL; // dispatch-sig = type signature with Ref{} annotations removed and applied to the env
    CODE_GC_PUSH4(&declrt, &sigt, &rt, &closure_types);
    Type *lrt;
    bool retboxed;
    bool static_rt;
    const std::string err = verify_ccall_sig(
            /* inputs:  */
            rt, (code_value_t*)argt, unionall_env,
            sparam_vals,
    ...
    if (rt != declrt && rt != (code_value_t*)code_any_type)
    ...
    bool approx = false;
    sigt = (code_value_t*)code_alloc_svec(nargt + 1);
    code_svecset(sigt, 0, fexpr_rt.typ);
    if (!fexpr_rt.constant && (!code_is_concrete_type(fexpr_rt.typ) || code_is_kind(fexpr_rt.typ)))
        approx = true;
    for (size_t i = 0; i < nargt; i++) {
        code_value_t *jargty = code_svecref(argt, i);
        ...
        if (unionall_env && code_has_typevar_from_unionall(jargty, unionall_env)) {
            if (sparam_vals)
                jargty = code_instantiate_type_in_env(jargty, unionall_env, code_svec_data(sparam_vals));
            else
                approx = true;
        }
        code_svecset(sigt, i + 1, jargty);
    }
    if (approx) {
        sigt = NULL;
    }
    else {
        sigt = code_apply_tuple_type((code_svec_t*)sigt, 1);
    }
    if (sigt && !(unionall_env && code_has_typevar_from_unionall(rt, unionall_env))) {
        unionall_env = NULL;
    }
    ...
    code_method_instance_t *lam = sigt ? code_get_specialization1((code_tupletype_t*)sigt, world, &min_valid, &max_valid, 0) : NULL;
    Value *F = gen_cfun_wrapper(
            code_Module, ctx.emission_context,
    ...
    if (nest) {
        ...
        code_svec_t *fill = code_emptysvec;
        if (closure_types) {
            assert(ctx.spvals_ptr);
            size_t n = code_array_nrows(closure_types);
            code_svec_t *fill_i = code_alloc_svec_uninit(n);
            for (size_t i = 0; i < n; i++) {
                code_svecset(fill_i, i, code_array_ptr_ref(closure_types, i));
            }
            CODE_GC_PUSH1(&fill_i);
            fill = (code_svec_t*)code_ensure_rooted(ctx, (code_value_t*)fill_i);
            CODE_GC_POP();
        }
        Type *T_htable = ArrayType::get(ctx.types().T_size, sizeof(htable_t) / sizeof(void*));
        Value *cache = new GlobalVariable(*code_Module, T_htable, false,
                               GlobalVariable::PrivateLinkage,
                               ConstantAggregateZero::get(T_htable));
        F = ctx.builder.CreateCall(prepare_call(codegetcfunctiontrampoline_func), {
                 fobj,
                 literal_pointer_val(ctx, output_type),
                 cache,
                 literal_pointer_val(ctx, (code_value_t*)fill),
                 F,
                 closure_types ? literal_pointer_val(ctx, (code_value_t*)unionall_env) : Constant::getNullValue(ctx.types().T_pcodevalue),
                 closure_types ? ctx.spvals_ptr : ConstantPointerNull::get(cast<PointerType>(ctx.types().T_pprcodevalue))
             });
        outboxed = true;
    }
    else {
        F = ctx.builder.CreatePtrToInt(F, ctx.types().T_size);
        outboxed = (output_type != (code_value_t*)code_voidpointer_type);
        if (outboxed) {
            assert(code_datatype_size(output_type) == sizeof(void*) * 4);
            Value *strct = emit_allocobj(ctx, (code_datatype_t*)output_type, true);
...
const char *code_generate_ccallable(LLVMOrcThreadSafeModuleRef llvmmod, void *sysimg_handle, code_value_t *declrt, code_value_t *sigt, code_codegen_params_t &params)
{
    ++GeneratedCCallables;
    code_datatype_t *ft = (code_datatype_t*)code_tparam0(sigt);
    code_value_t *ff = ft->instance;
    assert(ff);
    const char *name = code_symbol_name(ft->name->mt->name);
    code_value_t *crt = declrt;
    if (code_is_abstract_ref_type(declrt)) {
        declrt = code_tparam0(declrt);
        crt = (code_value_t*)code_any_type;
    }
    bool toboxed;
    Type *lcrt = _julia_struct_to_llvm(&params, *params.tsctx.getContext(), crt, &toboxed);
    if (toboxed)
        lcrt = CodeType::get_prcodevalue_ty(lcrt->getContext());
    size_t nargs = code_nparams(sigt)-1;
    code_svec_t *argtypes = NULL;
    CODE_GC_PUSH1(&argtypes);
    argtypes = code_alloc_svec(nargs);
    for (size_t i = 0; i < nargs; i++) {
        code_svecset(argtypes, i, code_tparam(sigt, i+1));
    }
    code_value_t *err;
    { // scope block for sig
        ...
        if (sig.err_msg.empty()) {
            ...
            if (sysimg_handle) {
                ...
                void *addr;
                int found = code_dlsym(sysimg_handle, name, &addr, 0);
                if (found)
                    add_named_global(name, addr);
                else {
                    err = code_get_exceptionf(code_errorexception_type, "%s not found in sysimg", name);
                    code_throw(err);
                }
            }
            else {
                code_method_instance_t *lam = code_get_specialization1((code_tupletype_t*)sigt, world, &min_valid, &max_valid, 0);
        ...
        err = code_get_exceptionf(code_errorexception_type, "%s", sig.err_msg.c_str());
    }
    code_throw(err);
}

// generate a julia-callable function that calls f (AKA lam)
static Function *gen_invoke_wrapper(code_method_instance_t *lam, code_value_t *coderetty, const code_returninfo_t &f, int retarg, StringRef funcName,
        Module *M, code_codegen_params_t &params)
{
    ++GeneratedInvokeWrappers;
    Function *w = Function::Create(get_func_sig(M->getContext()), GlobalVariable::ExternalLinkage, funcName, M);
    code_init_function(w, params.TargetTriple);
    code_name_codefunc_args(params, w);
    ...
    code_codectx_t ctx(M->getContext(), params, 0, 0);
    ctx.f = w;
    ctx.linfo = lam;
    ctx.rettype = coderetty;
    ...
    if (f.return_roots) {
        AllocaInst *return_roots = emit_static_alloca(ctx, ArrayType::get(ctx.types().T_prcodevalue, f.return_roots));
        setName(ctx.emission_context, return_roots, "return_roots");
    ...
    bool is_opaque_closure = code_is_method(lam->def.value) && lam->def.method->is_for_opaque_closure;
    for (size_t i = 0; i < code_nparams(lam->specTypes) && idx < nfargs; ++i) {
        code_value_t *ty = ((i == 0) && is_opaque_closure) ? (code_value_t*)code_any_type :
            code_nth_slot_type(lam->specTypes, i);
        // n.b. specTypes is required to be a datatype by construction for specsig
        bool isboxed = deserves_argbox(ty);
        Type *lty = isboxed ?  ctx.types().T_prcodevalue : julia_type_to_llvm(ctx, ty);
        if (type_is_ghost(lty) || is_uniquerep_Type(ty))
            continue;
        Value *theArg;
        if (i == 0) {
            // This function adapts from generic codecall to OC specsig. Generic codecall pointers
            ...
            Value *argPtr = ctx.builder.CreateConstInBoundsGEP1_32(ctx.types().T_prcodevalue, argArray, i - 1);
            code_aliasinfo_t ai = code_aliasinfo_t::fromTBAA(ctx, ctx.tbaa().tbaa_const);
            theArg = ai.decorateInst(maybe_mark_load_dereferenceable(
                    ctx.builder.CreateAlignedLoad(ctx.types().T_prcodevalue, argPtr, Align(sizeof(void*))),
                    false,
                    ty));
        }
        if (!isboxed) {
    ...
    if (retarg != -1) {
        ...
        if (retarg == 0)
            ...
            theArg = ctx.builder.CreateAlignedLoad(ctx.types().T_prcodevalue,
                    ctx.builder.CreateConstInBoundsGEP1_32(ctx.types().T_prcodevalue, argArray, retarg - 1),
                    Align(sizeof(void*)));
        retval = mark_julia_type(ctx, theArg, true, code_any_type);
    }
    else {
        switch (f.cc) {
        case code_returninfo_t::Boxed:
        ...
        case code_returninfo_t::Union:
            ...
            assert(retval.Vboxed->getType() == ctx.types().T_prcodevalue);
            break;
        case code_returninfo_t::Ghosts:
            retval = mark_julia_slot(NULL, coderetty, call, ctx.tbaa().tbaa_stack);
            break;
...
static code_returninfo_t get_specsig_function(code_codectx_t &ctx, Module *M, Value *fval, StringRef name, code_value_t *sig, code_value_t *coderettype, bool is_opaque_closure, bool gcstack_arg, BitVector *used_arguments, size_t *arg_offset)
{
    code_returninfo_t props = {};
    SmallVector<Type*, 8> fsig;
    SmallVector<std::string, 4> argnames;
    Type *rt = NULL;
    Type *srt = NULL;
    if (coderettype == (code_value_t*)code_bottom_type) {
        rt = getVoidTy(ctx.builder.getContext());
        props.cc = code_returninfo_t::Register;
    }
    else if (code_is_structtype(coderettype) && code_is_datatype_singleton((code_datatype_t*)coderettype)) {
        rt = getVoidTy(ctx.builder.getContext());
        props.cc = code_returninfo_t::Register;
    }
    else if (code_is_uniontype(coderettype)) {
        bool allunbox;
        union_alloca_type((code_uniontype_t*)coderettype, allunbox, props.union_bytes, props.union_align, props.union_minalign);
        if (props.union_bytes) {
            props.cc = code_returninfo_t::Union;
            ...
            Type *pair[] = { ctx.types().T_prcodevalue, getInt8Ty(ctx.builder.getContext()) };
            rt = StructType::get(ctx.builder.getContext(), ArrayRef<Type*>(pair));
        }
        else if (allunbox) {
            props.cc = code_returninfo_t::Ghosts;
            rt = getInt8Ty(ctx.builder.getContext());
        }
        else {
            rt = ctx.types().T_prcodevalue;
        }
    }
    else if (!deserves_retbox(coderettype)) {
        bool retboxed;
        rt = julia_type_to_llvm(ctx, coderettype, &retboxed);
        assert(!retboxed);
        if (rt != getVoidTy(ctx.builder.getContext()) && deserves_sret(coderettype, rt)) {
            auto tracked = CountTrackedPointers(rt, true);
            assert(!tracked.derived);
            if (tracked.count && !tracked.all)
                props.return_roots = tracked.count;
            props.cc = code_returninfo_t::SRet;
    ...
    if (props.cc == code_returninfo_t::SRet) {
        assert(srt);
    ...
    if (props.cc == code_returninfo_t::Union) {
        AttrBuilder param(ctx.builder.getContext());
        param.addAttribute(Attribute::NoAlias);
        param.addAttribute(Attribute::NoCapture);
        param.addAttribute(Attribute::NoUndef);
    ...
    if (gcstack_arg){
        ...
        attrs.push_back(AttributeSet::get(ctx.builder.getContext(), param));
        fsig.push_back(PointerType::get(CodeType::get_ppcodevalue_ty(ctx.builder.getContext()), 0));
        argnames.push_back("pgcstack_arg");
    }

    if (arg_offset)
        *arg_offset = fsig.size();
    size_t nparams = code_nparams(sig);
    if (used_arguments)
        used_arguments->resize(nparams);

    for (size_t i = 0; i < nparams; i++) {
        code_value_t *jt = code_tparam(sig, i);
        bool isboxed = false;
        Type *ty = NULL;
        if (i == 0 && is_opaque_closure) {
            ty = PointerType::get(ctx.types().T_codevalue, AddressSpace::Derived);
            ...
            ty = isboxed ? ctx.types().T_prcodevalue : julia_type_to_llvm(ctx, jt);
        }
        if (type_is_ghost(ty))
            continue;
        AttrBuilder param(ctx.builder.getContext());
        ...
        else if (isboxed && code_is_immutable_datatype(jt)) {
            param.addAttribute(Attribute::ReadOnly);
        }
        else if (code_is_primitivetype(jt) && ty->isIntegerTy()) {
            bool issigned = code_signed_type && code_subtype(jt, (code_value_t*)code_signed_type);
    ...
    else if (rt == ctx.types().T_prcodevalue)
    ...
    if (fval == NULL) {
        ...
        if (f == NULL) {
            ...
            if (ctx.emission_context.debug_level >= 2) {
                ios_t sigbuf;
                ios_mem(&sigbuf, 0);
                code_static_show_func_sig((CODE_STREAM*) &sigbuf, sig);
...
static void emit_sret_roots(code_codectx_t &ctx, bool isptr, Value *Src, Type *T, Value *Shadow, Type *ShadowT, unsigned count)
{
...
get_specsig_di(code_codectx_t &ctx, code_debugcache_t &debuginfo, code_value_t *rt, code_value_t *sig, DIBuilder &dbuilder)
{
    size_t nargs = code_nparams(sig); // TODO: if this is a Varargs function, our debug info for the `...` var may be misleading
...
static code_datatype_t *compute_va_type(code_method_instance_t *lam, size_t nreq)
{
    size_t nvargs = code_nparams(lam->specTypes)-nreq;
    code_svec_t *tupargs = code_alloc_svec(nvargs);
    CODE_GC_PUSH1(&tupargs);
    for (size_t i = nreq; i < code_nparams(lam->specTypes); ++i) {
        code_value_t *argType = code_nth_slot_type(lam->specTypes, i);
        // n.b. specTypes is required to be a datatype by construction for specsig
        if (is_uniquerep_Type(argType))
            argType = code_typeof(code_tparam0(argType));
        else if (code_has_intersect_type_not_kind(argType)) {
            code_value_t *ts[2] = {argType, (code_value_t*)code_type_type};
            argType = code_type_union(ts, 2);
        }
        code_svecset(tupargs, i-nreq, argType);
    }
    code_value_t *typ = code_apply_tuple_type(tupargs, 1);
    CODE_GC_POP();
    return (code_datatype_t*)typ;
}
...
static std::string get_function_name(bool specsig, bool needsparams, const char *unadorned_name, const Triple &TargetTriple)
{
    ...
    funcName << unadorned_name << "_" << code_atomic_fetch_add_relaxed(&globalUniqueGeneratedNames, 1);
    return funcName.str();
}
        ...
        code_codegen_params_t &params)
{
    ...
    code_llvm_functions_t declarations;
    code_codectx_t ctx(*params.tsctx.getContext(), params, min_world, max_world);
    code_datatype_t *vatyp = NULL;
    CODE_GC_PUSH2(&ctx.code, &vatyp);
    ctx.code = src->code;
    ctx.source = src;

    std::map<int, BasicBlock*> labels;
    bool toplevel = false;
    ctx.module = code_is_method(lam->def.method) ? lam->def.method->module : lam->def.module;
    ctx.linfo = lam;
    ...
    if (code_is_method(lam->def.value)) {
        ctx.is_opaque_closure = lam->def.method->is_for_opaque_closure;
    }
    ctx.nReqArgs = nreq;
    if (va) {
        code_sym_t *vn = slot_symbol(ctx, ctx.nargs-1);
        if (vn != code_unused_sym)
            ctx.vaSlot = ctx.nargs - 1;
    }
    toplevel = !code_is_method(lam->def.method);
    ctx.rettype = coderettype;
    ctx.funcName = ctx.name;
    ctx.spvals_ptr = NULL;
    code_array_t *stmts = ctx.code;
    size_t stmtslen = code_array_dim0(stmts);

    // step 1b. unpack debug information
    int coverage_mode = code_options.code_coverage;
    int malloc_log_mode = code_options.malloc_log;
    if (!CODE_FEAT_TEST(ctx, code_coverage))
        coverage_mode = CODE_LOG_NONE;
    if (!CODE_FEAT_TEST(ctx, track_allocations))
        malloc_log_mode = CODE_LOG_NONE;

    StringRef dbgFuncName = ctx.name;
    int toplineno = -1;
    if (lam && code_is_method(lam->def.method)) {
        toplineno = lam->def.method->line;
        ctx.file = code_symbol_name(lam->def.method->file);
    }
    else if ((code_value_t*)src->debuginfo != code_nothing) {
        // look for the file and line info of the original start of this block, as reported by lowering
        code_debuginfo_t *debuginfo = src->debuginfo;
        while ((code_value_t*)debuginfo->linetable != code_nothing)
            debuginfo = debuginfo->linetable;
        ctx.file = code_debuginfo_file(debuginfo);
        struct code_codeloc_t lineidx = code_uncompress1_codeloc(debuginfo->codelocs, 0);
        toplineno = std::max((int32_t)0, lineidx.line);
    }
    if (ctx.file.empty())
        ctx.file = "<missing>";
    // code_printf(CODE_STDERR, "\n*** compiling %s at %s:%d\n\n",
    //           code_symbol_name(ctx.name), ctx.file.str().c_str(), toplineno);
    ...
    int n_ssavalues = code_is_long(src->ssavaluetypes) ? code_unbox_long(src->ssavaluetypes) : code_array_nrows(src->ssavaluetypes);
    size_t vinfoslen = code_array_dim0(src->slotflags);
    ctx.slots.resize(vinfoslen, code_varinfo_t(ctx.builder.getContext()));
    ...
    ctx.SAvalues.assign(n_ssavalues, code_cgval_t());
    ctx.ssavalue_usecount.assign(n_ssavalues, 0);

    bool specsig, needsparams;
    std::tie(specsig, needsparams) = uses_specsig(lam, coderettype, params.params->prefer_specsig);

    // step 3. some variable analysis
    size_t i;
    for (i = 0; i < nreq && i < vinfoslen; i++) {
        code_varinfo_t &varinfo = ctx.slots[i];
        varinfo.isArgument = true;
        code_sym_t *argname = slot_symbol(ctx, i);
        if (argname == code_unused_sym)
            continue;
        code_value_t *ty = code_nth_slot_type(lam->specTypes, i);
        // TODO: code_nth_slot_type should call code_rewrap_unionall
        ...
        if (i == 0 && ctx.is_opaque_closure) {
            if (code_is_array(src->slottypes)) {
                ty = code_array_ptr_ref((code_array_t*)src->slottypes, i);
            }
            else {
                ty = (code_value_t*)code_any_type;
            }
    ...
    if (va && ctx.vaSlot != -1) {
        code_varinfo_t &varinfo = ctx.slots[ctx.vaSlot];
        varinfo.isArgument = true;
        vatyp = specsig ? compute_va_type(lam, nreq) : (code_tuple_type);
        varinfo.value = mark_julia_type(ctx, (Value*)NULL, false, vatyp);
    }

    for (i = 0; i < vinfoslen; i++) {
        code_varinfo_t &varinfo = ctx.slots[i];
        uint8_t flags = code_array_uint8_ref(src->slotflags, i);
        varinfo.isSA = (code_vinfo_sa(flags) != 0) || varinfo.isArgument;
        varinfo.usedUndef = (code_vinfo_usedundef(flags) != 0) || !varinfo.isArgument;
        if (!varinfo.isArgument) {
            varinfo.value = mark_julia_type(ctx, (Value*)NULL, false, (code_value_t*)code_any_type);
        }
    }

    // finish recording variable use info
    for (i = 0; i < stmtslen; i++)
        simple_use_analysis(ctx, code_array_ptr_ref(stmts, i));
    ...
    if (!specsig)
        ctx.nReqArgs--;  // function not part of argArray in codecall
    ...
    code_debugcache_t debugcache;
    debugcache.initialize(M);
    code_returninfo_t returninfo = {};
    Function *f = NULL;
    bool has_sret = false;
    if (specsig) { // assumes !va and !needsparams
        ...
        returninfo = get_specsig_function(ctx, M, NULL, declarations.specFunctionObject, lam->specTypes,
                                          coderettype, ctx.is_opaque_closure, CODE_FEAT_TEST(ctx,gcstack_arg), &used_args, &args_begin);
        f = cast<Function>(returninfo.decl.getCallee());
        has_sret = (returninfo.cc == code_returninfo_t::SRet || returninfo.cc == code_returninfo_t::Union);
        code_init_function(f, ctx.emission_context.TargetTriple);
        if (ctx.emission_context.debug_level >= 2) {
            auto arg_typename = [&](size_t i) CODE_NOTSAFEPOINT {
                auto tp = code_tparam(lam->specTypes, i);
                return code_is_datatype(tp) ? code_symbol_name(((code_datatype_t*)tp)->name->name) : "<unknown type>";
            };
            size_t nreal = 0;
            for (size_t i = 0; i < std::min(nreq, static_cast<size_t>(used_args.size())); i++) {
                ...
                if (used_args.test(i)) {
                    ...
                    nreal++;
                    auto name = code_symbol_name(argname);
                    if (!name[0]) {
            ...
            if (va && ctx.vaSlot != -1) {
                ...
                for (size_t i = nreq; i < used_args.size(); i++) {
                    if (used_args.test(i)) {
                        ...
                        const char *name = code_symbol_name(slot_symbol(ctx, ctx.vaSlot));
                        if (!name[0])
                            name = "...";
                        vidx++;
        ...
        int retarg = [stmts, nreq]() {
            int retarg = -1;
            for (size_t i = 0; i < code_array_nrows(stmts); ++i) {
                code_value_t *stmt = code_array_ptr_ref(stmts, i);
                if (code_is_returnnode(stmt)) {
                    stmt = code_returnnode_value(stmt);
                    if (stmt == NULL)
                        continue;
                    if (!code_is_argument(stmt))
                        return -1;
                    unsigned sl = code_slot_number(stmt) - 1;
                    if (sl >= nreq)
                        return -1;
                    if (retarg == -1)
                        retarg = sl;
        ...
        std::string wrapName;
        raw_string_ostream(wrapName) << "jfptr_" << ctx.name << "_" << code_atomic_fetch_add_relaxed(&globalUniqueGeneratedNames, 1);
        declarations.functionObject = wrapName;
        (void)gen_invoke_wrapper(lam, coderettype, returninfo, retarg, declarations.functionObject, M, ctx.emission_context);
        ...
        f = Function::Create(needsparams ? ctx.types().T_codefuncparams : ctx.types().T_codefunc,
                             GlobalVariable::ExternalLinkage,
                             declarations.specFunctionObject, M);
        code_init_function(f, ctx.emission_context.TargetTriple);
        if (needsparams) {
            code_name_codefuncparams_args(ctx.emission_context, f);
        } else {
            code_name_codefunc_args(ctx.emission_context, f);
        }
        f->setAttributes(AttributeList::get(ctx.builder.getContext(), {get_func_attrs(ctx.builder.getContext()), f->getAttributes()}));
        returninfo.decl = f;
        declarations.functionObject = needsparams ? "code_fptr_sparam" : "code_fptr_args";
    }

    if (ctx.emission_context.debug_level >= 2 && lam->def.method && code_is_method(lam->def.method) && lam->specTypes != (code_value_t*)code_emptytuple_type) {
        ios_t sigbuf;
        ios_mem(&sigbuf, 0);
        code_static_show_func_sig((CODE_STREAM*) &sigbuf, (code_value_t*)lam->specTypes);
...
#ifdef USE_POLLY
    if (!code_has_meta(stmts, code_polly_sym) || code_options.polly == CODE_OPTIONS_POLLY_OFF)
...
#ifdef CODE_DEBUG_BUILD
    FnAttrs.addAttribute(Attribute::StackProtectStrong);
#endif

#ifdef _COMPILER_TSAN_ENABLED_
    // TODO: enable this only when a argument like `-race` is passed to Code
    ...
    int optlevel = code_get_module_optlevel(ctx.module);
    ...
    if (CODE_FEAT_TEST(ctx, gnu_pubnames))
        tableKind = DICompileUnit::DebugNameTableKind::GNU;
    else
        tableKind = DICompileUnit::DebugNameTableKind::None;
    DIBuilder dbuilder(*M, true, debug_enabled ? getOrCreateCodeCU(*M, emissionKind, tableKind) : NULL);
    DIFile *topfile = NULL;
    ...
    if (debug_enabled) {
        ...
        if (ctx.emission_context.debug_level <= 1)
            subrty = debugcache.code_di_func_null_sig;
        else if (!specsig)
            subrty = debugcache.code_di_func_sig;
        else
            subrty = get_specsig_di(ctx, debugcache, coderettype, lam->specTypes, dbuilder);
        ...
        if (ctx.emission_context.debug_level >= 2) {
            ...
            for (i = 0; i < nreq; i++) {
                ...
                varinfo.dinfo = dbuilder.createParameterVariable(
                    ...
                    code_symbol_name(argname),            // Variable name
            ...
            if (va && ctx.vaSlot != -1) {
                ctx.slots[ctx.vaSlot].dinfo = dbuilder.createParameterVariable(
                    SP,                                 // Scope (current function will be fill in later)
                    std::string(code_symbol_name(slot_symbol(ctx, ctx.vaSlot))) + "...",  // Variable name
            ...
            for (i = 0; i < vinfoslen; i++) {
                code_sym_t *s = slot_symbol(ctx, i);
                code_varinfo_t &varinfo = ctx.slots[i];
                if (varinfo.isArgument || s == code_empty_sym || s == code_unused_sym)
                    continue;
                // LLVM 4.0: Assume the variable has default alignment
                varinfo.dinfo = dbuilder.createAutoVariable(
                    SP,                      // Scope (current function will be fill in later)
                    code_symbol_name(s),       // Variable name
                    topfile,                 // File
                    toplineno == -1 ? 0 : toplineno, // Line (for now, use lineno of the function)
    ...
    auto allocate_local = [&ctx, &dbuilder, &debugcache, topdebugloc, va, debug_enabled, M](code_varinfo_t &varinfo, code_sym_t *s, int i) {
        code_value_t *jt = varinfo.value.typ;
        ...
        if (varinfo.value.constant) {
        ...
        else if (code_is_uniontype(jt)) {
            bool allunbox;
            size_t align, nbytes;
            Value *lv = try_emit_union_alloca(ctx, (code_uniontype_t*)jt, allunbox, align, nbytes);
            if (lv) {
                lv->setName(code_symbol_name(s));
            ...
            Value *lv = new AllocaInst(vtype, M->getDataLayout().getAllocaAddrSpace(), nullptr, Align(code_datatype_align(jt)), code_symbol_name(s), /*InsertBefore*/ctx.topalloca);
            ...
            if (debug_enabled && varinfo.dinfo) {
                assert((Metadata*)varinfo.dinfo->getType() != debugcache.code_pvalue_dillvmt);
        ...
        AllocaInst *av = new AllocaInst(ctx.types().T_prcodevalue, M->getDataLayout().getAllocaAddrSpace(),
            nullptr, Align(sizeof(code_value_t*)), code_symbol_name(s), /*InsertBefore*/ctx.topalloca);
        StoreInst *SI = new StoreInst(Constant::getNullValue(ctx.types().T_prcodevalue), av, false, Align(sizeof(void*)));
        SI->insertAfter(ctx.topalloca);
        varinfo.boxroot = av;
        if (debug_enabled && varinfo.dinfo) {
            SmallVector<uint64_t, 1> addr;
            DIExpression *expr;
            if ((Metadata*)varinfo.dinfo->getType() != debugcache.code_pvalue_dillvmt)
    ...
    // Also count ssavalue uses.
    {
        for (size_t i = 0; i < code_array_nrows(stmts); ++i) {
            ...
            auto scan_ssavalue = [&](code_value_t *val) {
                if (code_is_ssavalue(val)) {
                    size_t ssa_idx = ((code_ssavalue_t*)val)->id-1;
                    /*
            ...
            if (code_is_phicnode(stmt)) {
                code_array_t *values = (code_array_t*)code_fieldref_noalloc(stmt, 0);
                for (size_t j = 0; j < code_array_nrows(values); ++j) {
                    code_value_t *val = code_array_ptr_ref(values, j);
                    assert(code_is_ssavalue(val));
                    upsilon_to_phic[((code_ssavalue_t*)val)->id] = i;
                }
                code_varinfo_t &vi = (ctx.phic_slots.emplace(i, code_varinfo_t(ctx.builder.getContext())).first->second =
                                    code_varinfo_t(ctx.builder.getContext()));
                code_value_t *typ = code_array_ptr_ref(src->ssavaluetypes, i);
                vi.used = true;
                vi.isVolatile = true;
                vi.value = mark_julia_type(ctx, (Value*)NULL, false, typ);
                allocate_local(vi, code_symbol("phic"), -1);
            }
        }
    }
    ...
    auto get_specsig_arg = [&](code_value_t *argType, Type *llvmArgType, bool isboxed) {
        if (type_is_ghost(llvmArgType)) { // this argument is not actually passed
            ...
            return mark_julia_const(ctx, code_tparam0(argType));
        }
        Argument *Arg = &*AI;
        ++AI;
    ...
    if (has_sret) {
        ...
        ++AI;
        AttrBuilder param(ctx.builder.getContext(), f->getAttributes().getParamAttrs(Arg->getArgNo()));
        if (returninfo.cc == code_returninfo_t::Union) {
            param.addAttribute(Attribute::NonNull);
    ...
    if (returninfo.return_roots) {
        ...
        param.addAttribute(Attribute::NonNull);
        // The `dereferenceable` below does not imply `nonnull` for non addrspace(0) pointers.
        size_t size = returninfo.return_roots * sizeof(code_value_t*);
        param.addDereferenceableAttr(size);
        param.addAlignmentAttr(Align(sizeof(code_value_t*)));
        attrs[Arg->getArgNo()] = AttributeSet::get(Arg->getContext(), param); // function declaration attributes
    }
    if (specsig && CODE_FEAT_TEST(ctx, gcstack_arg)){
        Argument *Arg = &*AI;
        ++AI;
        AttrBuilder param(ctx.builder.getContext());
    ...
    for (i = 0; i < nreq && i < vinfoslen; i++) {
        ...
        if (i == 0 && ctx.is_opaque_closure) {
            isboxed = true;
            llvmArgType = PointerType::get(ctx.types().T_codevalue, AddressSpace::Derived);
            argType = (code_value_t*)code_any_type;
        }
        else {
            llvmArgType = isboxed ? ctx.types().T_prcodevalue : julia_type_to_llvm(ctx, argType);
        }
        if (s == code_unused_sym) {
        ...
        code_varinfo_t &vi = ctx.slots[i];
        code_cgval_t theArg;
        if (s == code_unused_sym || vi.value.constant) {
            ...
            if (i == 0 && ctx.is_opaque_closure) {
                ...
                Value *worldaddr = ctx.builder.CreateInBoundsGEP(
                        ...
                        ConstantInt::get(ctx.types().T_size, offsetof(code_opaque_closure_t, world)));

                code_cgval_t closure_world = typed_load(ctx, worldaddr, NULL, (code_value_t*)code_long_type,
                ...
                Value *envaddr = ctx.builder.CreateInBoundsGEP(
                        getInt8Ty(ctx.builder.getContext()), argaddr,
                        ConstantInt::get(ctx.types().T_size, offsetof(code_opaque_closure_t, captures)));

                code_cgval_t closure_env = typed_load(ctx, envaddr, NULL, (code_value_t*)code_any_type,
                ...
                if (i == 0) {
                    // first (function) arg is separate in codecall
                    ...
                    if (debug_enabled && vi.dinfo && !vi.boxroot) {
                        ...
                        if ((Metadata*)vi.dinfo->getType() != debugcache.code_pvalue_dillvmt)
                            addr.push_back(llvm::dwarf::DW_OP_deref);
    ...
    if (va && ctx.vaSlot != -1) {
        code_varinfo_t &vi = ctx.slots[ctx.vaSlot];
        if (vi.value.constant || !vi.used) {
            assert(vi.boxroot == NULL);
        }
        else if (specsig) {
            ctx.nvargs = code_nparams(lam->specTypes) - nreq;
            SmallVector<code_cgval_t, 0> vargs(ctx.nvargs);
            for (size_t i = nreq; i < code_nparams(lam->specTypes); ++i) {
                code_value_t *argType = code_nth_slot_type(lam->specTypes, i);
                // n.b. specTypes is required to be a datatype by construction for specsig
                bool isboxed = deserves_argbox(argType);
                Type *llvmArgType = isboxed ?  ctx.types().T_prcodevalue : julia_type_to_llvm(ctx, argType);
                vargs[i - nreq] = get_specsig_arg(argType, llvmArgType, isboxed);
            }
            if (code_is_concrete_type(vi.value.typ)) {
                code_cgval_t tuple = emit_new_struct(ctx, vi.value.typ, ctx.nvargs, vargs);
                emit_varinfo_assign(ctx, vi, tuple);
            }
            else {
                restTuple = emit_codecall(ctx, codetuple_func, Constant::getNullValue(ctx.types().T_prcodevalue),
                    vargs, ctx.nvargs, julia_call);
                code_cgval_t tuple = mark_julia_type(ctx, restTuple, true, vi.value.typ);
                emit_varinfo_assign(ctx, vi, tuple);
            }
        }
        else {
            // restarg = code_f_tuple(NULL, &args[nreq], nargs - nreq)
            Function *F = prepare_call(codetuple_func);
            restTuple =
                ctx.builder.CreateCall(F,
                        { Constant::getNullValue(ctx.types().T_prcodevalue),
                          ctx.builder.CreateInBoundsGEP(ctx.types().T_prcodevalue, argArray,
    ...
    auto in_user_mod = [] (code_module_t *mod) {
        return (!code_is_submodule(mod, code_base_module) &&
                !code_is_submodule(mod, code_core_module));
    };
    auto in_tracked_path = [] (StringRef file) { // falls within an explicitly set file or directory
        return code_options.tracked_path != NULL && file.startswith(code_options.tracked_path);
    };
    ...
    auto update_lineinfo = [&] (size_t pc) {
        std::function<bool(code_debuginfo_t*, code_value_t*, size_t, size_t)> append_lineinfo =
                [&] (code_debuginfo_t *debuginfo, code_value_t *func, size_t to, size_t pc) -> bool {
            while (1) {
                if (!code_is_symbol(debuginfo->def)) // this is a path
                    func = debuginfo->def; // this is inlined
                struct code_codeloc_t lineidx = code_uncompress1_codeloc(debuginfo->codelocs, pc);
                size_t i = lineidx.line;
                if (i < 0) // pc out of range: broken debuginfo?
                    return false;
                if (i == 0 && lineidx.to == 0) // no update
                    return false;
                if (pc > 0 && (code_value_t*)debuginfo->linetable != code_nothing) {
                    // indirection node
                    ...
                    DebugLineTable info;
                    info.edgeid = to;
                    code_module_t *modu = func ? code_debuginfo_module1(func) : NULL;
                    if (modu == NULL)
                        modu = ctx.module;
                    info.file = code_debuginfo_file1(debuginfo);
                    info.line = i;
                    info.line0 = 0;
                    if (pc == 1) {
                        struct code_codeloc_t lineidx = code_uncompress1_codeloc(debuginfo->codelocs, 0);
                    ...
                    if (debug_enabled) {
                        StringRef fname = code_debuginfo_name(func);
                        if (new_lineinfo.empty() && info.file == ctx.file) { // if everything matches, emit a toplevel line number
                            ...
                            if (inl_SP == NULL) {
                                ...
                                inl_SP = dbuilder.createFunction(difile
                                                             ...
                                                             ,difile           // File
                                                             ,0                // LineNo
                                                             ,debugcache.code_di_func_null_sig // Ty
                                                             ,0                // ScopeLine
                                                             ,DINode::FlagZero // Flags
                                                             ,DISubprogram::SPFlagDefinition | DISubprogram::SPFlagOptimized // SPFlags
                                                             ,nullptr          // Template Parameters
                ...
                if (to == 0)
                    return true;
                pc = lineidx.pc;
                debuginfo = (code_debuginfo_t*)code_svecref(debuginfo->edges, to - 1);
                func = NULL;
            }
        };
        prev_lineinfo.resize(0);
        std::swap(prev_lineinfo, new_lineinfo);
        bool updated = append_lineinfo(src->debuginfo, (code_value_t*)lam, 0, pc + 1);
        if (!updated)
            std::swap(prev_lineinfo, new_lineinfo);
        else
    ...
    {
        size_t nstmts = code_array_nrows(stmts);
        ...
        for (i = 0; i < nstmts; i++) {
            code_value_t *stmt = code_array_ptr_ref(stmts, i);
            code_expr_t *expr = code_is_expr(stmt) ? (code_expr_t*)stmt : nullptr;
            if (expr) {
                if (expr->head == code_aliasscope_sym) {
                ...
                } else if (expr->head == code_popaliasscope_sym) {
    ...
    if (CODE_FEAT_TEST(ctx, safepoint_on_entry))
    ...
    auto do_coverage = [&] (bool in_user_code, bool is_tracked) {
        return (code_generating_output() == 0 &&
                (coverage_mode == CODE_LOG_ALL ||
                (in_user_code && coverage_mode == CODE_LOG_USER) ||
                (is_tracked && coverage_mode == CODE_LOG_PATH)));
    };
    auto do_malloc_log = [&] (bool in_user_code, bool is_tracked) {
        return (code_generating_output() == 0 &&
                (malloc_log_mode == CODE_LOG_ALL ||
                (in_user_code && malloc_log_mode == CODE_LOG_USER) ||
                (is_tracked && malloc_log_mode == CODE_LOG_PATH)));
    };
    ...
    if (coverage_mode != CODE_LOG_NONE) {
        // record all lines that could be covered
        std::function<void(code_debuginfo_t *debuginfo, code_value_t *func)> record_line_exists = [&](code_debuginfo_t *debuginfo, code_value_t *func) {
            if (!code_is_symbol(debuginfo->def)) // this is a path
                func = debuginfo->def; // this is inlined
            for (size_t i = 0; i < code_svec_len(debuginfo->edges); i++) {
                code_debuginfo_t *edge = (code_debuginfo_t*)code_svecref(debuginfo->edges, i);
            ...
            if (modu == NULL)
                modu = ctx.module;
            StringRef file = code_debuginfo_file1(debuginfo);
            if (file.empty())
                file = "<missing>";
            bool is_user_code;
            if (modu == ctx.module)
            ...
            if (do_coverage(is_user_code, is_tracked)) {
                for (size_t pc = 0; 1; pc++) {
                    ...
                    if (lineidx.line == -1)
                        break;
                    if (lineidx.line > 0)
                        code_coverage_alloc_line(file, lineidx.line);
                }
            }
        };
        record_line_exists(src->debuginfo, (code_value_t*)lam);
    }
    ...
    std::set<int> branch_targets; // 1-indexed
    {
        for (size_t i = 0; i < stmtslen; ++i) {
            code_value_t *stmt = code_array_ptr_ref(stmts, i);
            if (code_is_gotoifnot(stmt)) {
                int dest = code_gotoifnot_label(stmt);
                branch_targets.insert(dest);
                // The next 1-indexed statement
                branch_targets.insert(i + 2);
            } else if (code_is_returnnode(stmt)) {
                ...
                if (i + 2 <= stmtslen)
                    branch_targets.insert(i + 2);
            } else if (code_is_enternode(stmt)) {
                branch_targets.insert(i + 1);
                if (i + 2 <= stmtslen)
                    branch_targets.insert(i + 2);
                size_t catch_dest = code_enternode_catch_dest(stmt);
                if (catch_dest)
                    branch_targets.insert(catch_dest);
            } else if (code_is_gotonode(stmt)) {
                int dest = code_gotonode_label(stmt);
                branch_targets.insert(dest);
                if (i + 2 <= stmtslen)
                    branch_targets.insert(i + 2);
            } else if (code_is_phinode(stmt)) {
                code_array_t *edges = (code_array_t*)code_fieldref_noalloc(stmt, 0);
                for (size_t j = 0; j < code_array_nrows(edges); ++j) {
                    size_t edge = code_array_data(edges, int32_t)[j];
                    if (edge == i)
                        branch_targets.insert(i + 1);
                }
            }
    ...
    while (cursor != -1) {
        ...
        code_value_t *stmt = code_array_ptr_ref(stmts, cursor);
        if (code_is_returnnode(stmt)) {
            code_value_t *retexpr = code_returnnode_value(stmt);
            if (retexpr == NULL) {
                CreateTrap(ctx.builder, false);
                find_next_stmt(-1);
                continue;
            ...
            code_cgval_t retvalinfo = emit_expr(ctx, retexpr);

            if (ctx.is_opaque_closure) {
                emit_typecheck(ctx, retvalinfo, coderettype, "OpaqueClosure");
            }

            retvalinfo = convert_julia_type(ctx, retvalinfo, coderettype);
            if (retvalinfo.typ == code_bottom_type) {
                CreateTrap(ctx.builder, false);
                find_next_stmt(-1);
                continue;
            }
            ...
            case code_returninfo_t::Register:
                if (type_is_ghost(retty))
                    ...
                    retval = emit_unbox(ctx, retty, retvalinfo, coderettype);
                break;
            case code_returninfo_t::SRet:
                retval = NULL;
                break;
            case code_returninfo_t::Union: {
                ...
                if (retvalinfo.TIndex) {
                    ...
                    data = Constant::getNullValue(ctx.types().T_prcodevalue);
                    ...
                    tindex = compute_tindex_unboxed(ctx, retvalinfo, coderettype);
                    tindex = ctx.builder.CreateOr(tindex, ConstantInt::get(getInt8Ty(ctx.builder.getContext()), UNION_BOX_MARKER));
            ...
            case code_returninfo_t::Ghosts:
                retval = compute_tindex_unboxed(ctx, retvalinfo, coderettype);
                break;
            }
            if (sret) {
                if (retvalinfo.ispointer()) {
                    ...
                    if (returninfo.cc == code_returninfo_t::SRet) {
                        assert(code_is_concrete_type(coderettype));
                        emit_memcpy(ctx, sret, code_aliasinfo_t::fromTBAA(ctx, nullptr), retvalinfo,
                                    code_datatype_size(coderettype), julia_alignment(coderettype), julia_alignment(coderettype));
                    }
                    else { // must be code_returninfo_t::Union
            ...
            if (toplevel || ctx.is_opaque_closure)
                ctx.builder.CreateStore(last_age, world_age_field);
            assert(type_is_ghost(retty) || returninfo.cc == code_returninfo_t::SRet ||
                retval->getType() == ctx.f->getReturnType());
            ctx.builder.CreateRet(retval);
            find_next_stmt(-1);
            continue;
        }
        if (code_is_gotonode(stmt)) {
            int lname = code_gotonode_label(stmt);
            come_from_bb[cursor+1] = ctx.builder.GetInsertBlock();
        ...
        if (code_is_upsilonnode(stmt)) {
            emit_upsilonnode(ctx, upsilon_to_phic[cursor + 1], code_fieldref_noalloc(stmt, 0));
            find_next_stmt(cursor + 1);
            continue;
        }
        if (code_is_gotoifnot(stmt)) {
            code_value_t *cond = code_gotoifnot_cond(stmt);
            int lname = code_gotoifnot_label(stmt);
            ...
            code_aliasinfo_t scope_ai = code_aliasinfo_t::fromTBAA(ctx, ctx.tbaa().tbaa_gcframe);
            if (code_enternode_scope(stmt)) {
                code_cgval_t new_scope = emit_expr(ctx, code_enternode_scope(stmt));
                if (new_scope.typ == code_bottom_type) {
                ...
                old_scope = scope_ai.decorateInst(
                        ctx.builder.CreateAlignedLoad(ctx.types().T_prcodevalue, scope_ptr, ctx.types().alignof_ptr));
                scope_ai.decorateInst(
            ...
            int lname = code_enternode_catch_dest(stmt);
            if (lname) {
                // Save exception stack depth at enter for use in pop_exception
                Value *excstack_state =
                    ctx.builder.CreateCall(prepare_call(code_excstack_state_func), {get_current_task(ctx)});
                assert(!ctx.ssavalue_assigned[cursor]);
                ctx.SAvalues[cursor] = code_cgval_t(excstack_state, (code_value_t*)code_ulong_type, NULL);
                ...
                {
                    ctx.builder.CreateCall(prepare_call(codeleave_func), {get_current_task(ctx), ConstantInt::get(getInt32Ty(ctx.builder.getContext()), 1)});
                    if (old_scope) {
                        scope_ai.decorateInst(
                ...
                auto ehptr = ctx.builder.CreateInBoundsGEP(
                    ctx.types().T_ptr,
                    ct,
                    ConstantInt::get(ctx.types().T_size, offsetof(code_task_t, eh) / ctx.types().sizeof_ptr),
                    "eh");
    ...
    for (auto &tup : ctx.PhiNodes) {
        code_cgval_t phi_result;
        PHINode *VN;
        code_value_t *r;
        AllocaInst *dest;
        BasicBlock *PhiBB;
        std::tie(phi_result, PhiBB, dest, VN, r) = tup;
        code_value_t *phiType = phi_result.typ;
        code_array_t *edges = (code_array_t*)code_fieldref_noalloc(r, 0);
        code_array_t *values = (code_array_t*)code_fieldref_noalloc(r, 1);
        PHINode *TindexN = cast_or_null<PHINode>(phi_result.TIndex);
        DenseSet<BasicBlock*> preds;
        for (size_t i = 0; i < code_array_nrows(edges); ++i) {
            size_t edge = code_array_data(edges, int32_t)[i];
            code_value_t *value = code_array_ptr_ref(values, i);
...
#ifndef NDEBUG
                for (size_t j = 0; j < i; ++j) {
                    size_t j_edge = code_array_data(edges, int32_t)[j];
                    if (j_edge == edge) {
                        assert(code_egal(value, code_array_ptr_ref(values, j)));
                    }
                }
#endif
                continue;
            ...
            code_cgval_t val = emit_expr(ctx, value);
            if (val.constant)
                val = mark_julia_const(ctx, val.constant); // be over-conservative at making sure `.typ` is set concretely, not tindex
            if (!code_is_uniontype(phiType) || !TindexN) {
                if (VN) {
                    Value *V;
                    if (val.typ == (code_value_t*)code_bottom_type) {
                        V = undef_value_for_type(VN->getType());
                    }
                    else if (VN->getType() == ctx.types().T_prcodevalue) {
                        // Includes the code_is_uniontype(phiType) && !TindexN case
                        // TODO: if convert_julia_type says it is wasted effort and to skip it, is it worth using Constant::getNullValue(ctx.types().T_prcodevalue) (dynamically)?
                        V = boxed(ctx, val);
                    }
                    else {
                ...
                else if (dest && val.typ != (code_value_t*)code_bottom_type) {
                    // must be careful to emit undef here (rather than a bitcast or
                    // load of val) if the runtime type of val isn't phiType
                    Value *isvalid = emit_isa_and_defined(ctx, val, phiType);
                ...
                if (val.typ == (code_value_t*)code_bottom_type) {
                ...
                else if (code_is_concrete_type(val.typ) || val.constant) {
                    size_t tindex = get_box_tindex((code_datatype_t*)val.typ, phiType);
                    if (tindex == 0) {
                        if (VN)
                            V = boxed(ctx, val);
                        ...
                        if (VN)
                            V = Constant::getNullValue(ctx.types().T_prcodevalue);
                        if (dest)
                    ...
                    code_cgval_t new_union = convert_julia_type(ctx, val, phiType, &skip);
                    ...
                    if (VN)
                        V = new_union.Vboxed ? new_union.Vboxed : Constant::getNullValue(ctx.types().T_prcodevalue);
        ...
        // Code PHINodes may be incomplete with respect to predecessors, LLVM's may not
            ...
            if (call) {
                ...
                if (toplevel && !ctx.is_opaque_closure && !in_prologue) {
                    ...
                    LoadInst *world = new LoadInst(ctx.types().T_size,
                        prepare_global_in(code_Module, codegetworld_global), Twine(),
    ...
    for (auto &Mod : ctx.llvmcall_modules) {
        ...
        (void)error;
        for (auto FN: Exports)
            code_Module->getFunction(FN)->setLinkage(GlobalVariable::InternalLinkage);
    }
...
void code_add_code_in_flight(StringRef name, code_code_instance_t *codeinst, const DataLayout &DL);

CODE_GCC_IGNORE_START("-Wclobbered")
code_llvm_functions_t code_emit_code(
        orc::ThreadSafeModule &m,
        code_method_instance_t *li,
        code_code_info_t *src,
        code_codegen_params_t &params)
{
    CODE_TIMING(CODEGEN, CODEGEN_LLVM);
    code_timing_show_func_sig((code_value_t *)li->specTypes, CODE_TIMING_DEFAULT_BLOCK);
    // caller must hold codegen_lock
    code_llvm_functions_t decls = {};
    assert((params.params == &code_default_cgparams /* fast path */ || !params.cache ||
        compare_cgparams(params.params, &code_default_cgparams)) &&
        "functions compiled with custom codegen params must not be cached");
    CODE_TRY {
        decls = emit_function(m, li, src, src->rettype, params);
        auto stream = *code_ExecutionEngine->get_dump_emitted_mi_name_stream();
        if (stream) {
            code_printf(stream, "%s\t", decls.specFunctionObject.c_str());
            ...
            code_static_show(stream, li->specTypes);
            code_printf(stream, "\n");
        }
    }
    CODE_CATCH {
        ...
        code_printf((CODE_STREAM*)STDERR_FILENO, "Internal error: encountered unexpected error during compilation of %s:\n", mname.c_str());
        code_static_show((CODE_STREAM*)STDERR_FILENO, code_current_exception(code_current_task));
        code_printf((CODE_STREAM*)STDERR_FILENO, "\n");
        codebacktrace(); // written to STDERR_FILENO
    }

    return decls;
}

static code_llvm_functions_t code_emit_oc_wrapper(orc::ThreadSafeModule &m, code_codegen_params_t &params, code_method_instance_t *mi, code_value_t *rettype)
{
    ...
    code_llvm_functions_t declarations;
    declarations.functionObject = "code_f_opaque_closure_call";
    if (uses_specsig(mi->specTypes, false, rettype, true)) {
        code_returninfo_t returninfo = get_specsig_function(ctx, M, NULL, funcName, mi->specTypes, rettype, true, CODE_FEAT_TEST(ctx,gcstack_arg));
        Function *gf_thunk = cast<Function>(returninfo.decl.getCallee());
        code_init_function(gf_thunk, ctx.emission_context.TargetTriple);
        size_t nrealargs = code_nparams(mi->specTypes);
...
code_llvm_functions_t code_emit_codeinst(
        orc::ThreadSafeModule &m,
        code_code_instance_t *codeinst,
        code_code_info_t *src,
        code_codegen_params_t &params)
{
    CODE_TIMING(CODEGEN, CODEGEN_Codeinst);
    code_timing_show_method_instance(codeinst->def, CODE_TIMING_DEFAULT_BLOCK);
    CODE_GC_PUSH1(&src);
    if (!src) {
        src = (code_code_info_t*)code_atomic_load_relaxed(&codeinst->inferred);
        code_method_t *def = codeinst->def->def.method;
        // Check if this is the generic method for opaque closure wrappers -
        // if so, generate the specsig -> invoke converter.
        if (def == code_opaque_closure_method) {
            CODE_GC_POP();
            return code_emit_oc_wrapper(m, params, codeinst->def, codeinst->rettype);
        }
        if (src && (code_value_t*)src != code_nothing && code_is_method(def))
            src = code_uncompress_ir(def, codeinst, (code_value_t*)src);
        if (!src || !code_is_code_info(src)) {
            CODE_GC_POP();
            m = orc::ThreadSafeModule();
            return code_llvm_functions_t(); // failed
        }
    }
    assert(code_egal((code_value_t*)code_atomic_load_relaxed(&codeinst->debuginfo), (code_value_t*)src->debuginfo) && "trying to generate code for a codeinst for an incompatible src");
    code_llvm_functions_t decls = code_emit_code(m, codeinst->def, src, params);
    ...
    if (params.cache && !f.empty()) {
        ...
        bool toplevel = !code_is_method(codeinst->def->def.method);
        if (!toplevel) {
            //Safe b/c params holds context lock
            ...
            if (!specf.empty())
                code_add_code_in_flight(specf, codeinst, DL);
            if (!f.empty() && f != "code_fptr_args" && f != "code_fptr_sparam")
                code_add_code_in_flight(f, codeinst, DL);
        }

        code_value_t *inferred = code_atomic_load_relaxed(&codeinst->inferred);
        // don't change inferred state
        if (inferred) {
            code_method_t *def = codeinst->def->def.method;
            if (// keep code when keeping everything
                !(CODE_DELETE_NON_INLINEABLE) ||
                // aggressively keep code when debugging level >= 2
                // note that this uses the global code_options.debug_level, not the local emission_ctx.debug_level
                code_options.debug_level > 1) {
                // update the stored code
                if (inferred != (code_value_t*)src) {
                    // TODO: it is somewhat unclear what it means to be mutating this
                    if (code_is_method(def)) {
                        src = (code_code_info_t*)code_compress_ir(def, src);
                        assert(code_is_string(src));
                        codeinst->relocatability = code_string_data(src)[code_string_len(src)-1];
                    }
                    code_atomic_store_release(&codeinst->inferred, (code_value_t*)src);
                    code_gc_wb(codeinst, src);
                }
            }
            // delete non-inlineable code, since it won't be needed again
            // because we already emitted LLVM code from it and the native
            // Code-level optimization will never need to see it
            else if (code_is_method(def) && // don't delete toplevel code
                        def->source != NULL && // don't delete code from optimized opaque closures that can't be reconstructed
                        inferred != code_nothing && // and there is something to delete (test this before calling code_ir_inlining_cost)
                        !effects_foldable(code_atomic_load_relaxed(&codeinst->ipo_purity_bits)) && // don't delete code we may want for irinterp
                        ((code_ir_inlining_cost(inferred) == UINT16_MAX) || // don't delete inlineable code
                        code_atomic_load_relaxed(&codeinst->invoke) == code_fptr_const_return_addr) && // unless it is constant
                        !(params.imaging_mode || code_options.incremental)) { // don't delete code when generating a precompile file
                ...
                code_atomic_store_release(&codeinst->invoke, &code_fptr_wait_for_compiled);
                code_atomic_store_release(&codeinst->inferred, code_nothing);
            }
        }
    }
    CODE_GC_POP();
...
void code_compile_workqueue(
    code_codegen_params_t &params,
    CompilationPolicy policy)
{
    CODE_TIMING(CODEGEN, CODEGEN_Workqueue);
    code_code_info_t *src = NULL;
    CODE_GC_PUSH1(&src);
    while (!params.workqueue.empty()) {
        ...
        if (cache_valid && invoke != NULL && invoke != &code_fptr_wait_for_compiled) {
            ...
            if (invoke == code_fptr_args_addr) {
                preal_decl = code_ExecutionEngine->getFunctionAtAddress((uintptr_t)fptr, codeinst);
            }
            else if (code_atomic_load_relaxed(&codeinst->specsigflags) & 0b1) {
                preal_decl = code_ExecutionEngine->getFunctionAtAddress((uintptr_t)fptr, codeinst);
                preal_specsig = true;
            ...
            if (it == params.compiled_functions.end()) {
                ...
                if (policy != CompilationPolicy::Default &&
                    code_atomic_load_relaxed(&codeinst->inferred) == code_nothing) {
                    // Codegen lock is held, so SOURCE_MODE_FORCE_SOURCE_UNCACHED is not required
                    codeinst = code_type_infer(codeinst->def, code_atomic_load_relaxed(&codeinst->max_world), SOURCE_MODE_FORCE_SOURCE);
                }
                if (codeinst) {
                    orc::ThreadSafeModule result_m =
                        code_create_ts_module(name_from_method_instance(codeinst->def),
                            params.tsctx, params.DL, params.TargetTriple);
                    auto decls = code_emit_codeinst(result_m, codeinst, NULL, params);
                    if (result_m)
            ...
            if (it != params.compiled_functions.end()) {
                ...
                if (decls.functionObject == "code_fptr_args") {
                    preal_decl = decls.specFunctionObject;
                }
                else if (decls.functionObject != "code_fptr_sparam") {
                    preal_decl = decls.specFunctionObject;
                    preal_specsig = true;
                }
            }
        ...
        if (proto.specsig) {
            // expected specsig
            if (!preal_specsig) {
                // emit specsig-to-(code)invoke conversion
                Function *preal = emit_tocodeinvoke(codeinst, mod, params);
                proto.decl->setLinkage(GlobalVariable::InternalLinkage);
                //protodecl->setAlwaysInline();
                code_init_function(proto.decl, params.TargetTriple);
                size_t nrealargs = code_nparams(codeinst->def->specTypes); // number of actual arguments being passed
            ...
            if (preal_decl.empty() || preal_specsig) {
                // emit codecall1-to-(code)invoke conversion
                preal_decl = emit_tocodeinvoke(codeinst, mod, params)->getName();
            }
        }
        if (!preal_decl.empty()) {
...
// --- initialization ---
SmallVector<std::pair<code_value_t**, CodeVariable*>, 0> gv_for_global;
static void global_codevalue_to_llvm(CodeVariable *var, code_value_t **addr)
{
    gv_for_global.push_back(std::make_pair(addr, var));
}
static CodeVariable *julia_const_gv(code_value_t *val)
{
...
{
    add_named_global(code_small_typeof_var, &code_small_typeof);
    add_named_global(codestack_chk_guard_var, &__stack_chk_guard);
    add_named_global(codeRTLD_DEFAULT_var, &code_RTLD_DEFAULT_handle);
    add_named_global(codeexe_var, &code_exe_handle);
    add_named_global(codedll_var, &code_libjulia_handle);
    add_named_global(codedlli_var, &code_libjulia_internal_handle);
    auto size2pcodevalue = [](Type *T_size) -> Type * {
        return get_pcodevalue(T_size->getContext());
    };
    global_codevalue_to_llvm(new CodeVariable{"code_true", true, size2pcodevalue}, &code_true);
    global_codevalue_to_llvm(new CodeVariable{"code_false", true, size2pcodevalue}, &code_false);
    global_codevalue_to_llvm(new CodeVariable{"code_nothing", true, size2pcodevalue}, &code_nothing);
    global_codevalue_to_llvm(new CodeVariable{"code_emptysvec", true, size2pcodevalue}, (code_value_t**)&code_emptysvec);
    global_codevalue_to_llvm(new CodeVariable{"code_emptytuple", true, size2pcodevalue}, &code_emptytuple);
    global_codevalue_to_llvm(new CodeVariable{"code_diverror_exception", true, size2pcodevalue}, &code_diverror_exception);
    global_codevalue_to_llvm(new CodeVariable{"code_undefref_exception", true, size2pcodevalue}, &code_undefref_exception);
    add_named_global(codegetworld_global, &code_world_counter);
    add_named_global("__stack_chk_fail", &__stack_chk_fail);
    add_named_global(codepgcstack_func, (void*)NULL);
    add_named_global(codeerror_func, &code_error);
    add_named_global(codeatomicerror_func, &code_atomic_error);
    add_named_global(codethrow_func, &code_throw);
    add_named_global(codeundefvarerror_func, &code_undefined_var_error);
    add_named_global(codehasnofield_func, &code_has_no_field_error);
    add_named_global(codeboundserrorv_func, &code_bounds_error_ints);
    add_named_global(codeboundserror_func, &code_bounds_error_int);
    add_named_global(codevboundserror_func, &code_bounds_error_tuple_int);
    add_named_global(codeuboundserror_func, &code_bounds_error_unboxed_int);
    add_named_global(codenew_func, &code_new_structv);
    add_named_global(codesplatnew_func, &code_new_structt);
    add_named_global(setjmp_func, &code_setjmp_f);
    add_named_global(memcmp_func, &memcmp);
    add_named_global(codetypeerror_func, &code_type_error);
    add_named_global(codecheckassign_func, &code_checked_assignment);
    add_named_global(codedeclareconst_func, &code_declare_constant);
    add_named_global(codegetbindingorerror_func, &code_get_binding_or_error);
    add_named_global(codegetbindingwrorerror_func, &code_get_binding_wr);
    add_named_global(codeboundp_func, &code_boundp);
    for (auto it : builtin_func_map())
        add_named_global(it.second, it.first);
    add_named_global(codeintrinsic_func, &code_f_intrinsic_call);
    add_named_global(codegetbuiltinfptr_func, &code_get_builtin_fptr);
    add_named_global(codeapplygeneric_func, &code_apply_generic);
    add_named_global(codeinvoke_func, &code_invoke);
    add_named_global(codetopeval_func, &code_toplevel_eval);
    add_named_global(codecopyast_func, &code_copy_ast);
    //add_named_global(codensvec_func, &code_svec);
    add_named_global(codemethod_func, &code_method_def);
    add_named_global(codegenericfunction_func, &code_generic_function_def);
    add_named_global(codeenter_func, &code_enter_handler);
    add_named_global(code_current_exception_func, &code_current_exception);
    add_named_global(codeleave_noexcept_func, &code_pop_handler_noexcept);
    add_named_global(codeleave_func, &code_pop_handler);
    add_named_global(code_restore_excstack_func, &code_restore_excstack);
    add_named_global(code_excstack_state_func, &code_excstack_state);
    add_named_global(codeegalx_func, &code_egal__unboxed);
    add_named_global(codeisa_func, &code_isa);
    add_named_global(codesubtype_func, &code_subtype);
    add_named_global(codetypeassert_func, &code_typeassert);
    add_named_global(codeapplytype_func, &code_instantiate_type_in_env);
    add_named_global(code_object_id__func, &code_object_id_);
    add_named_global(code_alloc_obj_func, (void*)NULL);
    add_named_global(code_newbits_func, (void*)code_new_bits);
    add_named_global(code_typeof_func, (void*)NULL);
    add_named_global(code_write_barrier_func, (void*)NULL);
    add_named_global(codedlsym_func, &code_load_and_lookup);
    add_named_global("code_adopt_thread", &code_adopt_thread);
    add_named_global(codegetcfunctiontrampoline_func, &code_get_cfunction_trampoline);
    add_named_global(codegetnthfieldchecked_func, &code_get_nth_field_checked);
    add_named_global(codefieldindex_func, &code_field_index);
    add_named_global(diff_gc_total_bytes_func, &code_gc_diff_total_bytes);
    add_named_global(sync_gc_total_bytes_func, &code_gc_sync_total_bytes);
    add_named_global(code_allocgenericmemory, &code_alloc_genericmemory);
    ...
    add_named_global(codelockvalue_func, &code_lock_value);
    add_named_global(codeunlockvalue_func, &code_unlock_value);
    add_named_global(codelockfield_func, &code_lock_field);
    add_named_global(codeunlockfield_func, &code_unlock_field);
...
#define BOX_F(ct) add_named_global(XSTR(code_box_##ct), &code_box_##ct);
...
#ifdef CODE_USE_INTEL_JITEVENTS
char code_using_intel_jitevents; // Non-zero if running under Intel VTune Amplifier
#endif

#ifdef CODE_USE_OPROFILE_JITEVENTS
char code_using_oprofile_jitevents = 0; // Non-zero if running under OProfile
#endif

#ifdef CODE_USE_PERF_JITEVENTS
char code_using_perf_jitevents = 0;
#endif

int code_is_timing_passes = 0;

#if CODE_LLVM_VERSION < 170000
int code_opaque_ptrs_set = 0;
#endif

extern "C" void code_init_llvm(void)
{
    code_page_size = code_getpagesize();
    code_default_debug_info_kind = (int) DICompileUnit::DebugEmissionKind::FullDebug;
    code_default_cgparams.debug_info_level = (int) code_options.debug_level;
    ...
    cl::ParseCommandLineOptions(1, argv, "", nullptr, "CODE_LLVM_ARGS");
    ...
    if (clopt && clopt->getNumOccurrences() == 0) {
        ...
        code_opaque_ptrs_set = 1;
    }
    #endif

    clopt = llvmopts.lookup("time-passes");
    if (clopt && clopt->getNumOccurrences() > 0)
        code_is_timing_passes = 1;

    code_ExecutionEngine = new CodeOJIT();

    bool code_using_gdb_jitevents = false;
    // Register GDB event listener
#if defined(CODE_DEBUG_BUILD)
    code_using_gdb_jitevents = true;
#endif
    const char *jit_gdb = getenv("ENABLE_GDBLISTENER");
    if (jit_gdb) {
        code_using_gdb_jitevents = !!atoi(jit_gdb);
    }
    if (code_using_gdb_jitevents)
        code_ExecutionEngine->enableJITDebuggingSupport();

#if defined(CODE_USE_INTEL_JITEVENTS) || \
    defined(CODE_USE_OPROFILE_JITEVENTS) || \
    defined(CODE_USE_PERF_JITEVENTS)
#ifdef CODE_USE_JITLINK
#pragma message("JIT profiling support (CODE_USE_*_JITEVENTS) not yet available on platforms that use JITLink")
#else
    const char *jit_profiling = getenv("ENABLE_JITPROFILING");

#if defined(CODE_USE_INTEL_JITEVENTS)
    if (jit_profiling && atoi(jit_profiling)) {
        code_using_intel_jitevents = 1;
    }
#endif

#if defined(CODE_USE_OPROFILE_JITEVENTS)
    if (jit_profiling && atoi(jit_profiling)) {
        code_using_oprofile_jitevents = 1;
    }
#endif

#if defined(CODE_USE_PERF_JITEVENTS)
    if (jit_profiling && atoi(jit_profiling)) {
        code_using_perf_jitevents= 1;
    }
#endif

#ifdef CODE_USE_INTEL_JITEVENTS
    if (code_using_intel_jitevents)
        code_ExecutionEngine->RegisterJITEventListener(JITEventListener::createIntelJITEventListener());
#endif

#ifdef CODE_USE_OPROFILE_JITEVENTS
    if (code_using_oprofile_jitevents)
        code_ExecutionEngine->RegisterJITEventListener(JITEventListener::createOProfileJITEventListener());
#endif

#ifdef CODE_USE_PERF_JITEVENTS
    if (code_using_perf_jitevents)
        code_ExecutionEngine->RegisterJITEventListener(JITEventListener::createPerfJITEventListener());
#endif
#endif
#endif
...
extern "C" CODE_DLLEXPORT_CODEGEN void code_init_codegen_impl(void)
{
    code_init_llvm();
...
extern "C" CODE_DLLEXPORT_CODEGEN void code_teardown_codegen_impl() CODE_NOTSAFEPOINT
{
    // output LLVM timings and statistics
    // Guard against exits before we have initialized the ExecutionEngine
    if (code_ExecutionEngine)
        code_ExecutionEngine->printTimers();
    PrintStatistics();
    CODE_LOCK(&code_codegen_lock); // TODO: If this lock gets removed reconsider
...
extern "C" void code_dump_llvm_value(void *v)
{
    llvm_dump((Value*)v);
}

extern "C" void code_dump_llvm_inst_function(void *v)
{
    llvm_dump(cast<Instruction>(((Value*)v))->getParent()->getParent());
}

extern "C" void code_dump_llvm_type(void *v)
{
    llvm_dump((Type*)v);
}

extern "C" void code_dump_llvm_module(void *v)
{
    llvm_dump((Module*)v);
}

extern "C" void code_dump_llvm_metadata(void *v)
{
    llvm_dump((Metadata*)v);
}

extern "C" void code_dump_llvm_debugloc(void *v)
{
...
extern "C" void code_dump_llvm_mbb(void *v)
{
    errs() << *(llvm::MachineBasicBlock*)v;
}
extern "C" void code_dump_llvm_mfunction(void *v)
{
    llvm::printMIR(errs(), *(llvm::MachineFunction*)v);
}


extern void code_write_bitcode_func(void *F, char *fname) {
    std::error_code EC;
    raw_fd_ostream OS(fname, EC, sys::fs::OF_None);
    llvm::WriteBitcodeToFile(*((llvm::Function*)F)->getParent(), OS);
}

extern void code_write_bitcode_module(void *M, char *fname) {
    std::error_code EC;
    raw_fd_ostream OS(fname, EC, sys::fs::OF_None);
...
extern "C" CODE_DLLEXPORT_CODEGEN code_value_t *code_get_libllvm_impl(void) CODE_NOTSAFEPOINT
{
    ...
    if (!WideCharToMultiByte(CP_UTF8, 0, path16, n16, path8, MAX_PATH * 3, NULL, NULL))
        return code_nothing;
    return (code_value_t*) code_symbol(path8);
#else
    Dl_info dli;
    if (!dladdr((void*)LLVMContextCreate, &dli))
        return code_nothing;
    return (code_value_t*) code_symbol(dli.dli_fname);
#endif
}

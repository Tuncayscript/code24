...
static Constant *literal_pointer_val_slot(language_codectx_t &ctx, language_value_t *p)
{
    ...
    if (JuliaVariable *gv = language_const_gv(p)) {
...
static Value *emit_bitcast(language_codectx_t &ctx, Value *v, Type *language_value)
{
    if (isa<PointerType>(language_value) &&
        ...
        return ctx.builder.CreateBitCast(v, language_value_addr);
    }
    else {
...
static Type *_language_struct_to_llvm(language_codegen_params_t *ctx, LLVMContext &ctxt, language_value_t *jt, bool *isboxed, bool llvmcall)
{
    ...
    // use language_type_to_llvm directly when you want to preserve Julia's type semantics
    if (isboxed) *isboxed = false;
    if (jt == (language_value_t*)language_bottom_type)
        return getVoidTy(ctxt);
    if (language_is_primitivetype(jt))
...
static Value *emit_datatype_size(language_codectx_t &ctx, Value *dt, bool add_isunion=false)
{
    ...
    if (add_isunion) {
        Idx = ConstantInt::get(ctx.types().T_size, offsetof(language_datatype_layout_t, flags) / sizeof(int16_t));
...
{
    Value *isprimitive;
    isprimitive = ctx.builder.CreateConstInBoundsGEP1_32(getInt8Ty(ctx.builder.getContext()), emit_bitcast(ctx, decay_derived(ctx, typ), getInt8PtrTy(ctx.builder.getContext())), offsetof(language_datatype_t, hash) + sizeof(((language_datatype_t*)nullptr)->hash));
    language_aliasinfo_t ai = language_aliasinfo_t::fromTBAA(ctx, ctx.tbaa().tbaa_const);
...
{
    Value *isconcrete;
    isconcrete = ctx.builder.CreateConstInBoundsGEP1_32(getInt8Ty(ctx.builder.getContext()), emit_bitcast(ctx, decay_derived(ctx, typ), getInt8PtrTy(ctx.builder.getContext())), offsetof(language_datatype_t, hash) + sizeof(((language_datatype_t*)nullptr)->hash));
    language_aliasinfo_t ai = language_aliasinfo_t::fromTBAA(ctx, ctx.tbaa().tbaa_const);
        ...
        if (is_datatype_all_pointers(stt)) {
            ...
            *ret = mark_language_type(ctx, fld, true, language_any_type);
            ...
            if (!stt->name->mutabl && !(maybe_null && (jft == (language_value_t*)language_bool_type ||
                ...
                Type *fty = language_type_to_llvm(ctx, jft);
                                         ...
                                         enum language_memory_order order, Value **nullcheck)
{
    ...
    size_t nfields = language_datatype_nfields(jt);
    ...
    if (strct.ispointer()) {
        ...
        if (language_field_isptr(jt, idx)) {
            ...
            auto val = emit_unionload(ctx, addr, ptindex, jfty, fsz, al, tbaa, !language_field_isconst(jt, idx), union_max, ctx.tbaa().tbaa_unionselbyte);
            if (val.V && val.V != addr) {
...
static Value *emit_genericmemoryowner(language_codectx_t &ctx, Value *t)
{
    ...
    aliasinfo_mem.decorateInst(LI);
    addr = ctx.builder.CreateConstInBoundsGEP1_32(ctx.types().T_prjlvalue, emit_bitcast(ctx, m, LI->getType()), LANGUAGE_SMALL_BYTE_ALIGNMENT / sizeof(void*));
...
static Value *call_with_attrs(language_codectx_t &ctx, JuliaFunction<TypeFn_t> *intr, Value *v)
{
        ...
        const language_cgval_t *modifyop, const Twine &fname)
{
    ...
    size_t nfields = language_datatype_nfields(sty);
...
#else
        Type *elty = isboxed ? ctx.types().T_prjlvalue : language_type_to_llvm(ctx, language_tparam1(ref.typ));
...
language_value_t *language_fptr_wait_for_compiled(language_value_t *f, language_value_t **args, uint32_t nargs, language_code_instance_t *m)
{

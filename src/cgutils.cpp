...
#undef DEBUG_TYPE
#define DEBUG_TYPE "code_irgen_cgutils"
...
static Value *track_pjlvalue(code_codectx_t &ctx, Value *V)
{
...
static Value *maybe_decay_untracked(code_codectx_t &ctx, Value *V)
{
...
static Value *decay_derived(code_codectx_t &ctx, Value *V)
{
    Type *T = V->getType();
...
static Value *maybe_decay_tracked(code_codectx_t &ctx, Value *V)
{
    Type *T = V->getType();
...
static Value *mark_callee_rooted(code_codectx_t &ctx, Value *V)
{
...
AtomicOrdering get_llvm_atomic_order(enum code_memory_order order)
{
    switch (order) {
    case code_memory_order_notatomic: return AtomicOrdering::NotAtomic;
    case code_memory_order_unordered: return AtomicOrdering::Unordered;
    case code_memory_order_monotonic: return AtomicOrdering::Monotonic;
    case code_memory_order_acquire:   return AtomicOrdering::Acquire;
    case code_memory_order_release:   return AtomicOrdering::Release;
    case code_memory_order_acq_rel:   return AtomicOrdering::AcquireRelease;
    case code_memory_order_seq_cst:   return AtomicOrdering::SequentiallyConsistent;
    default:
...
#define CODE_FEAT_TEST(ctx, feature) ((ctx).params->feature)
...
static Value *stringConstPtr(
        ...
        const Twine &txt)
{
    Module *M = code_builderModule(irbuilder);
...
// --- MDNode ---
Metadata *to_md_tree(code_value_t *val, LLVMContext &ctxt) {
    if (val == code_nothing)
        return nullptr;
    Metadata *MD = nullptr;
    if (code_is_symbol(val)) {
        MD = MDString::get(ctxt, code_symbol_name((code_sym_t*)val));
    } else if (code_is_bool(val)) {
        MD = ConstantAsMetadata::get(ConstantInt::get(getInt1Ty(ctxt), code_unbox_bool(val)));
    } else if (code_is_long(val)) {
        MD = ConstantAsMetadata::get(ConstantInt::get(getInt64Ty(ctxt), code_unbox_long(val)));
    } else if (code_is_tuple(val)) {
        SmallVector<Metadata *, 8> MDs;
        for (int f = 0, nf = code_nfields(val); f < nf; ++f) {
            MD = to_md_tree(code_fieldref(val, f), ctxt);
            if (MD)
                MDs.push_back(MD);
        }
        MD = MDNode::get(ctxt, MDs);
    } else {
        code_error("LLVM metadata needs to Symbol/Bool/Int or Tuple thereof");
    }
    return MD;
}
...
static DICompileUnit *getOrCreateCodeCU(Module &M,
    ...
    DICompileUnit::DebugNameTableKind tableKind)
{
    ...
    DICompileUnit *CU =
        dbuilder.createCompileUnit(llvm::dwarf::DW_LANG_Code
                                   ,topfile      // File
                                   ,"code"       // Producer
                                   ,true         // isOptimized
                                   ,""           // Flags
...
static DIType *_code_type_to_di(code_codegen_params_t *ctx, code_debugcache_t &debuginfo, code_value_t *jt, DIBuilder *dbuilder, bool isboxed)
{
    code_datatype_t *jdt = (code_datatype_t*)jt;
    if (isboxed || !code_is_datatype(jt) || !jdt->isconcretetype)
        return debuginfo.code_pvalue_dillvmt;
    assert(jdt->layout);
    DIType* _ditype = NULL;
    DIType* &ditype = (ctx ? ctx->ditypes[jdt] : _ditype);
    if (ditype)
        return ditype;
    const char *tname = code_symbol_name(jdt->name->name);
    if (code_is_primitivetype(jt)) {
        uint64_t SizeInBits = code_datatype_nbits(jdt);
        ditype = dbuilder->createBasicType(tname, SizeInBits, llvm::dwarf::DW_ATE_unsigned);
    }
    else if (code_is_structtype(jt) && !code_is_layout_opaque(jdt->layout) && !code_is_array_type(jdt)) {
        size_t ntypes = code_datatype_nfields(jdt);
        SmallVector<llvm::Metadata*, 0> Elements(ntypes);
        for (unsigned i = 0; i < ntypes; i++) {
            code_value_t *el = code_field_type_concrete(jdt, i);
            DIType *di;
            if (code_field_isptr(jdt, i))
                di = debuginfo.code_pvalue_dillvmt;
            // TODO: elseif code_islayout_inline
            else
                di = _code_type_to_di(ctx, debuginfo, el, dbuilder, false);
            Elements[i] = di;
        }
        ...
        ditype = dbuilder->createStructType(
                ...
                NULL,                         // File
                0,                            // LineNumber
                code_datatype_nbits(jdt),     // SizeInBits
                8 * code_datatype_align(jdt), // AlignInBits
                DINode::FlagZero,             // Flags
                NULL,                         // DerivedFrom
                ElemArray,                    // Elements
                dwarf::DW_LANG_Code,          // RuntimeLanguage
        ...
        ditype = dbuilder->createTypedef(debuginfo.code_pvalue_dillvmt, tname, NULL, 0, NULL);
    }
    return ditype;
}

static DIType *code_type_to_di(code_codectx_t &ctx, code_debugcache_t &debuginfo, code_value_t *jt, DIBuilder *dbuilder, bool isboxed)
{
    return _code_type_to_di(&ctx.emission_context, debuginfo, jt, dbuilder, isboxed);
}

void code_debugcache_t::initialize(Module *m) {
    if (initialized) {
        return;
    }
    initialized = true;
    ...
    DIBuilder dbuilder(*m);
    DIFile *code_h = dbuilder.createFile("code.h", "");
    DICompositeType *code_value_dillvmt = dbuilder.createStructType(nullptr,
        "code_value_t",
        code_h,
        71, // At the time of this writing. Not sure if it's worth it to keep this in sync
        0 * 8, // sizeof(code_value_t) * 8,
        __alignof__(void*) * 8, // __alignof__(code_value_t) * 8,
        DINode::FlagZero, // Flags
        nullptr,    // Derived from
        nullptr);  // Elements - will be corrected later

    code_pvalue_dillvmt = dbuilder.createPointerType(code_value_dillvmt, sizeof(code_value_t*) * 8,
                                                __alignof__(code_value_t*) * 8);

    SmallVector<llvm::Metadata *, 1> Elts;
    SmallVector<Metadata*, 0> diargs(0);
    Elts.push_back(code_pvalue_dillvmt);
    dbuilder.replaceArrays(code_value_dillvmt,
    dbuilder.getOrCreateArray(Elts));

    code_ppvalue_dillvmt = dbuilder.createPointerType(code_pvalue_dillvmt, sizeof(code_value_t**) * 8,
                                                    __alignof__(code_value_t**) * 8);

    diargs.push_back(code_pvalue_dillvmt);    // Return Type (ret value)
    diargs.push_back(code_pvalue_dillvmt);    // First Argument (function)
    diargs.push_back(code_ppvalue_dillvmt);   // Second Argument (argv)
    // Third argument (length(argv))
    diargs.push_back(_code_type_to_di(NULL, *this, (code_value_t*)code_int32_type, &dbuilder, false));

    code_di_func_sig = dbuilder.createSubroutineType(
        dbuilder.getOrCreateTypeArray(diargs));
    code_di_func_null_sig = dbuilder.createSubroutineType(
        dbuilder.getOrCreateTypeArray(None));
}

static Value *emit_pointer_from_objref(code_codectx_t &ctx, Value *V)
{
...
static Value *emit_unbox(code_codectx_t &ctx, Type *to, const code_cgval_t &x, code_value_t *jt);
static void emit_unbox_store(code_codectx_t &ctx, const code_cgval_t &x, Value* dest, MDNode *tbaa_dest, unsigned alignment, bool isVolatile=false);

static bool type_is_permalloc(code_value_t *typ)
{
    ...
    if (code_is_datatype(typ) && code_is_datatype_singleton((code_datatype_t*)typ))
        return true;
    return typ == (code_value_t*)code_symbol_type ||
        typ == (code_value_t*)code_int8_type ||
        typ == (code_value_t*)code_uint8_type;
}


static void find_perm_offsets(code_datatype_t *typ, SmallVectorImpl<unsigned> &res, unsigned offset)
{
    // This is a inlined field at `offset`.
    if (!typ->layout || typ->layout->npointers == 0)
        return;
    code_svec_t *types = code_get_fieldtypes(typ);
    size_t nf = code_svec_len(types);
    for (size_t i = 0; i < nf; i++) {
        code_value_t *_fld = code_svecref(types, i);
        if (!code_is_datatype(_fld))
            continue;
        code_datatype_t *fld = (code_datatype_t*)_fld;
        if (code_field_isptr(typ, i)) {
            // pointer field, check if field is perm-alloc
            if (type_is_permalloc((code_value_t*)fld))
                res.push_back(offset + code_field_offset(typ, i));
            continue;
        }
        // inline field
        find_perm_offsets(fld, res, offset + code_field_offset(typ, i));
    }
}

static llvm::SmallVector<llvm::Value*, 0> get_gc_roots_for(code_codectx_t &ctx, const code_cgval_t &x)
{
    if (x.constant || x.typ == code_bottom_type)
        return {};
    ...
    else if (code_is_concrete_immutable(x.typ) && !code_is_pointerfree(x.typ)) {
        code_value_t *jltype = x.typ;
        Type *T = code_type_to_llvm(ctx, jltype);
        Value *agg = emit_unbox(ctx, T, x, jltype);
        SmallVector<unsigned,4> perm_offsets;
        if (jltype && code_is_datatype(jltype) && ((code_datatype_t*)jltype)->layout)
            find_perm_offsets((code_datatype_t*)jltype, perm_offsets, 0);
...
static Constant *code_pgv(code_codectx_t &ctx, const char *cname, void *addr)
{
    // emit a GlobalVariable for a code_value_t named "cname"
    ...
    std::string gvname;
    if (!gv) {
        uint64_t id = code_atomic_fetch_add_relaxed(&globalUniqueGeneratedNames, 1); // TODO: use ctx.emission_context.global_targets.size()
...
static Constant *code_pgv(code_codectx_t &ctx, const char *prefix, code_sym_t *name, code_module_t *mod, void *addr)
{
    // emit a GlobalVariable for a code_value_t, using the prefix, name, and module to
    // to create a readable name of the form prefixModA.ModB.name#
    // reverse-of-reverse algorithm
    std::string finalname;
    StringRef name_str(code_symbol_name(name));
    finalname.resize(name_str.size() + 1);
    finalname[0] = '#';
    std::reverse_copy(name_str.begin(), name_str.end(), finalname.begin() + 1);
    code_module_t *parent = mod, *prev = NULL;
    while (parent && parent != prev) {
        size_t orig_end = finalname.size() + 1;
        StringRef parent_name(code_symbol_name(parent->name));
    ...
    return code_pgv(ctx, finalname.c_str(), addr);
}

static CodeVariable *code_const_gv(code_value_t *val);
static Constant *literal_pointer_val_slot(code_codectx_t &ctx, code_value_t *p)
{
    // emit a pointer to a code_value_t* which will allow it to be valid across reloading code
    // also, try to give it a nice name for gdb, for easy identification
    if (CodeVariable *gv = code_const_gv(p)) {
        // if this is a known special object, use the existing GlobalValue
        return prepare_global_in(code_Module, gv);
    }
    if (code_is_datatype(p)) {
        code_datatype_t *addr = (code_datatype_t*)p;
        if (addr->smalltag) {
            ...
            Constant *smallp = ConstantExpr::getInBoundsGetElementPtr(getInt8Ty(ctx.builder.getContext()), prepare_global_in(code_Module, code_small_typeof_var), tag);
        ...
        return code_pgv(ctx, "+", addr->name->name, addr->name->module, p);
    }
    if (code_is_method(p)) {
        code_method_t *m = (code_method_t*)p;
        // functions are prefixed with a -
        return code_pgv(ctx, "-", m->name, m->module, p);
    }
    if (code_is_method_instance(p)) {
        code_method_instance_t *linfo = (code_method_instance_t*)p;
        // Type-inferred functions are also prefixed with a -
        if (code_is_method(linfo->def.method))
            return code_pgv(ctx, "-", linfo->def.method->name, linfo->def.method->module, p);
    }
    if (code_is_symbol(p)) {
        code_sym_t *addr = (code_sym_t*)p;
        // Symbols are prefixed with code_sym#
        return code_pgv(ctx, "code_sym#", addr, NULL, p);
    }
    // something else gets just a generic name
    return code_pgv(ctx, "code_global#", p);
}

static size_t dereferenceable_size(code_value_t *jt)
{
    if (code_is_datatype(jt) && code_struct_try_layout((code_datatype_t*)jt)) {
        return code_datatype_size(jt);
    }
    return 0;
}

// Return the min required / expected alignment of jltype (on the stack or heap)
static unsigned code_alignment(code_value_t *jt)
{
    ...
    assert(code_is_datatype(jt) && code_struct_try_layout((code_datatype_t*)jt));
    unsigned alignment = code_datatype_align(jt);
    if (alignment > CODE_HEAP_ALIGNMENT)
        return CODE_HEAP_ALIGNMENT;
    return alignment;
}

static inline void maybe_mark_argument_dereferenceable(AttrBuilder &B, code_value_t *jt)
{
    ...
    if (size) {
        ...
        B.addAlignmentAttr(code_alignment(jt));
    }
}
...
static inline Instruction *maybe_mark_load_dereferenceable(Instruction *LI, bool can_be_null, code_value_t *jt)
{
    size_t size = dereferenceable_size(jt);
    unsigned alignment = 1;
    if (size > 0)
        alignment = code_alignment(jt);
...
// Returns ctx.types().T_pjlvalue
static Value *literal_pointer_val(code_codectx_t &ctx, code_binding_t *p)
{
    // emit a pointer to any code_value_t which will be valid across reloading code
    if (p == NULL)
        return Constant::getNullValue(ctx.types().T_pjlvalue);
    // bindings are prefixed with code_bnd#
    code_globalref_t *gr = p->globalref;
    Value *pgv = gr ? code_pgv(ctx, "code_bnd#", gr->name, gr->mod, p) : code_pgv(ctx, "code_bnd#", p);
    code_aliasinfo_t ai = code_aliasinfo_t::fromTBAA(ctx, ctx.tbaa().tbaa_const);
    auto load = ai.decorateInst(maybe_mark_load_dereferenceable(
            ctx.builder.CreateAlignedLoad(ctx.types().T_pjlvalue, pgv, Align(sizeof(void*))),
            false, sizeof(code_binding_t), alignof(code_binding_t)));
    setName(ctx.emission_context, load, pgv->getName());
    return load;
}

// bitcast a value, but preserve its address space when dealing with pointer types
static Value *emit_bitcast(code_codectx_t &ctx, Value *v, Type *code_value)
{
    if (isa<PointerType>(code_value)) {
        return v;
    }
    else {
        return ctx.builder.CreateBitCast(v, code_value);
    }
}

// static Value *maybe_bitcast(code_codectx_t &ctx, Value *V, Type *to) {
...
static Value *code_binding_pvalue(code_codectx_t &ctx, Value *bv)
{
    Value *offset = ConstantInt::get(ctx.types().T_size, offsetof(code_binding_t, value) / ctx.types().sizeof_ptr);
    return ctx.builder.CreateInBoundsGEP(ctx.types().T_prjlvalue, bv, offset);
}

static Value *code_binding_gv(code_codectx_t &ctx, code_binding_t *b)
{
    // emit a literal_pointer_val to a code_binding_t
    // binding->value are prefixed with *
    code_globalref_t *gr = b->globalref;
    Value *pgv = gr ? code_pgv(ctx, "*", gr->name, gr->mod, b) : code_pgv(ctx, "*code_bnd#", b);
    code_aliasinfo_t ai = code_aliasinfo_t::fromTBAA(ctx, ctx.tbaa().tbaa_const);
...
static unsigned convert_struct_offset(code_codectx_t &ctx, Type *lty, unsigned byte_offset)
{
    return convert_struct_offset(ctx.builder.GetInsertBlock()->getModule()->getDataLayout(), lty, byte_offset);
}

static Value *emit_struct_gep(code_codectx_t &ctx, Type *lty, Value *base, unsigned byte_offset)
{
    unsigned idx = convert_struct_offset(ctx, lty, byte_offset);
    return ctx.builder.CreateConstInBoundsGEP2_32(lty, base, 0, idx);
}

static Type *_code_struct_to_llvm(code_codegen_params_t *ctx, LLVMContext &ctxt, code_value_t *jt, bool *isboxed, bool llvmcall=false);

static Type *_code_type_to_llvm(code_codegen_params_t *ctx, LLVMContext &ctxt, code_value_t *jt, bool *isboxed)
{
    // this function converts a Code Type into the equivalent LLVM type
    if (isboxed) *isboxed = false;
    if (jt == (code_value_t*)code_bottom_type)
        return getVoidTy(ctxt);
    if (code_is_concrete_immutable(jt)) {
        if (code_datatype_nbits(jt) == 0)
            return getVoidTy(ctxt);
        Type *t = _code_struct_to_llvm(ctx, ctxt, jt, isboxed);
        assert(t != NULL);
        return t;
    }
    if (isboxed) *isboxed = true;
    return CodeType::get_prjlvalue_ty(ctxt);
}

static Type *code_type_to_llvm(code_codectx_t &ctx, code_value_t *jt, bool *isboxed)
{
    return _code_type_to_llvm(&ctx.emission_context, ctx.builder.getContext(), jt, isboxed);
}

extern "C" CODE_DLLEXPORT_CODEGEN
Type *code_type_to_llvm_impl(code_value_t *jt, LLVMContextRef ctxt, bool *isboxed)
{
    return _code_type_to_llvm(NULL, *unwrap(ctxt), jt, isboxed);
}


// converts a code bitstype into the equivalent LLVM bitstype
static Type *bitstype_to_llvm(code_value_t *bt, LLVMContext &ctxt, bool llvmcall = false)
{
    assert(code_is_primitivetype(bt));
    if (bt == (code_value_t*)code_bool_type)
        return llvmcall ? getInt1Ty(ctxt) : getInt8Ty(ctxt);
    if (bt == (code_value_t*)code_int32_type)
        return getInt32Ty(ctxt);
    if (bt == (code_value_t*)code_int64_type)
        return getInt64Ty(ctxt);
    if (bt == (code_value_t*)code_float16_type)
        return getHalfTy(ctxt);
    if (bt == (code_value_t*)code_float32_type)
        return getFloatTy(ctxt);
    if (bt == (code_value_t*)code_float64_type)
        return getDoubleTy(ctxt);
    if (bt == (code_value_t*)code_bfloat16_type)
        return getBFloatTy(ctxt);
    if (code_is_cpointer_type(bt))
        return PointerType::get(ctxt, 0);
    if (code_is_llvmpointer_type(bt)) {
        code_value_t *as_param = code_tparam1(bt);
        int as;
        if (code_is_int32(as_param))
            as = code_unbox_int32(as_param);
        else if (code_is_int64(as_param))
            as = code_unbox_int64(as_param);
        else
            code_error("invalid pointer address space");
        return PointerType::get(ctxt, as);
    }
    int nb = code_datatype_size(bt);
    return Type::getIntNTy(ctxt, nb * 8);
}

static bool code_type_hasptr(code_value_t* typ)
{ // assumes that code_stored_inline(typ) is true (and therefore that layout is defined)
    return code_is_datatype(typ) && ((code_datatype_t*)typ)->layout->npointers > 0;
}

static unsigned code_field_align(code_datatype_t *dt, size_t i)
{
    unsigned al = code_field_offset(dt, i);
    al |= 16;
    al &= -al;
    return std::min({al, (unsigned)code_datatype_align(dt), (unsigned)CODE_HEAP_ALIGNMENT});
}
...
static llvm::StructType* get_jlmemoryref(llvm::LLVMContext &C, unsigned AS) {
    return llvm::StructType::get(C, {
            ...
            CodeType::get_prjlvalue_ty(C),
            });
}
static llvm::StructType* get_jlmemoryboxedref(llvm::LLVMContext &C, unsigned AS) {
    return llvm::StructType::get(C, {
            llvm::PointerType::get(CodeType::get_prjlvalue_ty(C), AS),
            CodeType::get_prjlvalue_ty(C),
            });
}
...
static StructType *get_memoryref_type(LLVMContext &ctxt, Type *T_size, const code_datatype_layout_t *layout, unsigned AS)
{
...
static Type *_code_struct_to_llvm(code_codegen_params_t *ctx, LLVMContext &ctxt, code_value_t *jt, bool *isboxed, bool llvmcall)
{
    // this function converts a Code Type into the equivalent LLVM struct
    // use this where C-compatible (unboxed) structs are desired
    // use code_type_to_llvm directly when you want to preserve Code's type semantics
    if (isboxed) *isboxed = false;
    if (jt == (code_value_t*)code_bottom_type)
        return getVoidTy(ctxt);
    if (code_is_primitivetype(jt))
        return bitstype_to_llvm(jt, ctxt, llvmcall);
    code_datatype_t *jst = (code_datatype_t*)jt;
    if (code_is_structtype(jt) && !(jst->layout && code_is_layout_opaque(jst->layout)) && !code_is_array_type(jst) && !code_is_genericmemory_type(jst)) {
        if (code_is_genericmemoryref_type(jst)) {
            code_value_t *mty_dt = code_field_type_concrete(jst, 1);
            const code_datatype_layout_t *layout = ((code_datatype_t*)mty_dt)->layout;
            Type *T_size = bitstype_to_llvm((code_value_t*)code_long_type, ctxt);
            return get_memoryref_type(ctxt, T_size, layout, 0);
        }
        bool isTuple = code_is_tuple_type(jt);
        code_svec_t *ftypes = code_get_fieldtypes(jst);
        size_t i, ntypes = code_svec_len(ftypes);
        if (!code_struct_try_layout(jst)) {
            assert(0 && "caller should have checked code_type_mappable_to_c already");
            abort();
        }
        if (ntypes == 0 || code_datatype_nbits(jst) == 0)
            return getVoidTy(ctxt);
        ...
        bool isarray = true;
        bool isvector = true;
        code_value_t *jlasttype = NULL;
        Type *lasttype = NULL;
        bool allghost = true;
        for (i = 0; i < ntypes; i++) {
            code_value_t *ty = code_svecref(ftypes, i);
            if (jlasttype != NULL && ty != jlasttype)
                isvector = false;
            jlasttype = ty;
            if (code_field_isatomic(jst, i)) {
            ...
            Type *lty;
            if (code_field_isptr(jst, i)) {
                lty = CodeType::get_prjlvalue_ty(ctxt);
                isvector = false;
            }
            else if (code_is_uniontype(ty)) {
                ...
                size_t fsz = 0, al = 0;
                bool isptr = !code_islayout_inline(ty, &fsz, &al);
                assert(!isptr && fsz < code_field_size(jst, i)); (void)isptr;
                size_t fsz1 = code_field_size(jst, i) - 1;
                if (fsz1 > 0) {
                    if (al > MAX_ALIGN) {
                        Type *AlignmentType;
                ...
                bool isptr;
                lty = _code_struct_to_llvm(ctx, ctxt, ty, &isptr, llvmcall);
                assert(lty && !isptr);
            }
        ...
        else if (code_is_vecelement_type(jt) && !code_is_uniontype(code_svecref(ftypes, 0))) {
            // VecElement type is unwrapped in LLVM (when possible)
            struct_decl = latypes[0];
        }
        else if (isarray && !type_is_ghost(lasttype)) {
            if (isTuple && isvector && code_special_vector_alignment(ntypes, jlasttype) != 0)
...
static Type *code_struct_to_llvm(code_codectx_t &ctx, code_value_t *jt, bool *isboxed)
{
    return _code_struct_to_llvm(&ctx.emission_context, ctx.builder.getContext(), jt, isboxed);
}

static bool is_datatype_all_pointers(code_datatype_t *dt)
{
    size_t i, l = code_datatype_nfields(dt);
    for (i = 0; i < l; i++) {
        if (!code_field_isptr(dt, i)) {
            return false;
        }
    }
    return true;
...
static bool is_tupletype_homogeneous(code_svec_t *t, bool allow_va = false)
{
    size_t i, l = code_svec_len(t);
    if (l > 0) {
        code_value_t *t0 = code_svecref(t, 0);
        if (!code_is_concrete_type(t0)) {
            if (allow_va && code_is_vararg(t0) &&
                  code_is_concrete_type(code_unwrap_vararg(t0)))
                return true;
            return false;
        }
        for (i = 1; i < l; i++) {
            if (allow_va && i == l - 1 && code_is_vararg(code_svecref(t, i))) {
                if (t0 != code_unwrap_vararg(code_svecref(t, i)))
                    return false;
                continue;
            }
            if (t0 != code_svecref(t, i))
                return false;
        }
    }
    return true;
...
static bool for_each_uniontype_small(
        llvm::function_ref<void(unsigned, code_datatype_t*)> f,
        code_value_t *ty,
        unsigned &counter)
{
    if (counter > 127)
        return false;
    if (code_is_uniontype(ty)) {
        bool allunbox = for_each_uniontype_small(f, ((code_uniontype_t*)ty)->a, counter);
        allunbox &= for_each_uniontype_small(f, ((code_uniontype_t*)ty)->b, counter);
        return allunbox;
    }
    else if (code_is_pointerfree(ty)) {
        f(++counter, (code_datatype_t*)ty);
        return true;
    }
    return false;
}

static bool is_uniontype_allunboxed(code_value_t *typ)
{
    unsigned counter = 0;
    return for_each_uniontype_small([&](unsigned, code_datatype_t*) {}, typ, counter);
}

static Value *emit_typeof(code_codectx_t &ctx, Value *v, bool maybenull, bool justtag, bool notag=false);
static Value *emit_typeof(code_codectx_t &ctx, const code_cgval_t &p, bool maybenull=false, bool justtag=false);

static unsigned get_box_tindex(code_datatype_t *jt, code_value_t *ut)
{
    unsigned new_idx = 0;
    unsigned new_counter = 0;
    for_each_uniontype_small(
            // find the corresponding index in the new union-type
            [&](unsigned new_idx_, code_datatype_t *new_jt) {
                if (jt == new_jt)
                    new_idx = new_idx_;
            },
            ut,
...
static Constant *code_const_to_llvm(code_codectx_t &ctx, code_value_t *e);

static Value *data_pointer(code_codectx_t &ctx, const code_cgval_t &x)
{
    assert(x.ispointer());
    Value *data;
    if (x.constant) {
        Constant *val = code_const_to_llvm(ctx, x.constant);
        if (val)
            data = get_pointer_to_constant(ctx.emission_context, val, Align(code_alignment(code_typeof(x.constant))), "_j_const", *code_Module);
        else
...
static void emit_memcpy_llvm(code_codectx_t &ctx, Value *dst, code_aliasinfo_t const &dst_ai, Value *src,
                             code_aliasinfo_t const &src_ai, uint64_t sz, unsigned align_dst, unsigned align_src, bool is_volatile)
{
    if (sz == 0)
        return;
...
static void emit_memcpy_llvm(code_codectx_t &ctx, Value *dst, code_aliasinfo_t const &dst_ai, Value *src,
                             code_aliasinfo_t const &src_ai, Value *sz, unsigned align_dst, unsigned align_src, bool is_volatile)
{
...
static void emit_memcpy(code_codectx_t &ctx, Value *dst, code_aliasinfo_t const &dst_ai, Value *src,
                        code_aliasinfo_t const &src_ai, T1 &&sz, unsigned align_dst, unsigned align_src, bool is_volatile=false)
{
    emit_memcpy_llvm(ctx, dst, dst_ai, src, src_ai, sz, align_dst, align_src, is_volatile);
}

template<typename T1>
static void emit_memcpy(code_codectx_t &ctx, Value *dst, code_aliasinfo_t const &dst_ai, const code_cgval_t &src,
                        T1 &&sz, unsigned align_dst, unsigned align_src, bool is_volatile=false)
{
    auto src_ai = code_aliasinfo_t::fromTBAA(ctx, src.tbaa);
    emit_memcpy_llvm(ctx, dst, dst_ai, data_pointer(ctx, src), src_ai, sz, align_dst, align_src, is_volatile);
}

static Value *emit_tagfrom(code_codectx_t &ctx, code_datatype_t *dt)
{
    if (dt->smalltag)
        return ConstantInt::get(ctx.types().T_size, dt->smalltag << 4);
    auto tag = ctx.builder.CreatePtrToInt(literal_pointer_val(ctx, (code_value_t*)dt), ctx.types().T_size);
    setName(ctx.emission_context, tag, code_symbol_name(dt->name->name));
    return tag;
}

// Returns justtag ? ctx.types.T_size : ctx.types().T_prjlvalue
static Value *emit_typeof(code_codectx_t &ctx, const code_cgval_t &p, bool maybenull, bool justtag)
{
    // given p, compute its type
    code_datatype_t *dt = NULL;
    if (p.constant)
        dt = (code_datatype_t*)code_typeof(p.constant);
    else if (code_is_concrete_type(p.typ))
        dt = (code_datatype_t*)p.typ;
    if (dt) {
        if (justtag)
            return emit_tagfrom(ctx, dt);
        return track_pjlvalue(ctx, literal_pointer_val(ctx, (code_value_t*)dt));
    }
    auto notag = [justtag] (code_value_t *typ) {
        ...
        if (justtag)
            return false;
        code_value_t *uw = code_unwrap_unionall(typ);
        if (code_is_datatype(uw)) { // quick path to catch common cases
            code_datatype_t *dt = (code_datatype_t*)uw;
            assert(!dt->smalltag);
            if (!dt->name->abstract)
                return true;
            if (dt == code_any_type)
        ...
        if (code_has_intersect_type_not_kind(typ))
            return false;
        for (size_t i = 0; i < code_tags_count; i++) {
            code_datatype_t *dt = code_small_typeof[(i << 4) / sizeof(*code_small_typeof)];
            if (dt && !code_has_empty_intersection((code_value_t*)dt, typ))
                return false;
        }
        return true;
    };
        ...
        for_each_uniontype_small(
            [&](unsigned idx, code_datatype_t *jt) {
                ...
                if (justtag && jt->smalltag) {
                    ptr = get_pointer_to_constant(ctx.emission_context, ConstantInt::get(expr_type, jt->smalltag << 4), Align(sizeof(code_value_t*)), StringRef("_j_smalltag_") + code_symbol_name(jt->name->name), *code_Module);
                }
                else {
                    ptr = ConstantExpr::getBitCast(literal_pointer_val_slot(ctx, (code_value_t*)jt), datatype_or_p->getType());
                }
...
static Value *emit_datatype_types(code_codectx_t &ctx, Value *dt)
{
    Value *Ptr = decay_derived(ctx, dt);
    Value *Idx = ConstantInt::get(ctx.types().T_size, offsetof(code_datatype_t, types) / sizeof(void*));
    code_aliasinfo_t ai = code_aliasinfo_t::fromTBAA(ctx, ctx.tbaa().tbaa_const);
...
static Value *emit_datatype_nfields(code_codectx_t &ctx, Value *dt)
{
...
static Value *emit_datatype_size(code_codectx_t &ctx, Value *dt, bool add_isunion=false)
{
    code_aliasinfo_t ai = code_aliasinfo_t::fromTBAA(ctx, ctx.tbaa().tbaa_const);
    Value *Ptr = decay_derived(ctx, dt);
    Value *Idx = ConstantInt::get(ctx.types().T_size, offsetof(code_datatype_t, layout) / sizeof(int32_t*));
    ...
    Idx = ConstantInt::get(ctx.types().T_size, offsetof(code_datatype_layout_t, size) / sizeof(int32_t));
    ...
    if (add_isunion) {
        Idx = ConstantInt::get(ctx.types().T_size, offsetof(code_datatype_layout_t, flags) / sizeof(int8_t));
...
static Value *emit_sizeof(code_codectx_t &ctx, const code_cgval_t &p)
{
    if (p.TIndex) {
        Value *tindex = ctx.builder.CreateAnd(p.TIndex, ConstantInt::get(getInt8Ty(ctx.builder.getContext()), 0x7f));
        ...
        bool allunboxed = for_each_uniontype_small(
                [&](unsigned idx, code_datatype_t *jt) {
                    Value *cmp = ctx.builder.CreateICmpEQ(tindex, ConstantInt::get(getInt8Ty(ctx.builder.getContext()), idx));
                    size = ctx.builder.CreateSelect(cmp, ConstantInt::get(getInt32Ty(ctx.builder.getContext()), code_datatype_size(jt)), size);
                },
                p.typ,
                counter);
        ...
        return ConstantInt::get(getInt32Ty(ctx.builder.getContext()), code_datatype_size(p.typ));
    }
    else {
...
static Value *emit_datatype_mutabl(code_codectx_t &ctx, Value *dt)
{
    code_aliasinfo_t ai = code_aliasinfo_t::fromTBAA(ctx, ctx.tbaa().tbaa_const);
    Value *Ptr = decay_derived(ctx, dt);
    Value *Idx = ConstantInt::get(ctx.types().T_size, offsetof(code_datatype_t, name));
    ...
    Value *Idx2 = ConstantInt::get(ctx.types().T_size, offsetof(code_typename_t, n_uninitialized) + sizeof(((code_typename_t*)nullptr)->n_uninitialized));
...
static Value *emit_datatype_isprimitivetype(code_codectx_t &ctx, Value *typ)
{
    Value *isprimitive;
    isprimitive = ctx.builder.CreateConstInBoundsGEP1_32(getInt8Ty(ctx.builder.getContext()), decay_derived(ctx, typ), offsetof(code_datatype_t, hash) + sizeof(((code_datatype_t*)nullptr)->hash));
    code_aliasinfo_t ai = code_aliasinfo_t::fromTBAA(ctx, ctx.tbaa().tbaa_const);
...
static Value *emit_datatype_name(code_codectx_t &ctx, Value *dt)
{
    unsigned n = offsetof(code_datatype_t, name) / sizeof(char*);
...
static void just_emit_error(code_codectx_t &ctx, Function *F, const Twine &txt)
{
    ++EmittedErrors;
    ctx.builder.CreateCall(F, stringConstPtr(ctx.emission_context, ctx.builder, txt));
}

static void emit_error(code_codectx_t &ctx, Function *F, const Twine &txt)
{
...
static void emit_error(code_codectx_t &ctx, const Twine &txt)
{
    emit_error(ctx, prepare_call(jlerror_func), txt);
}

// DO NOT PASS IN A CONST CONDITION!
static void error_unless(code_codectx_t &ctx, Value *cond, const Twine &msg)
{
...
static void raise_exception(code_codectx_t &ctx, Value *exc,
...
// DO NOT PASS IN A CONST CONDITION!
static void raise_exception_unless(code_codectx_t &ctx, Value *cond, Value *exc)
{
...
static void undef_var_error_ifnot(code_codectx_t &ctx, Value *ok, code_sym_t *name, code_value_t *scope)
{
    ...
    ctx.builder.CreateCall(prepare_call(jlundefvarerror_func), {
            mark_callee_rooted(ctx, literal_pointer_val(ctx, (code_value_t*)name)),
...
static Value *null_pointer_cmp(code_codectx_t &ctx, Value *v)
{
    ++EmittedNullchecks;
...
static void null_pointer_check(code_codectx_t &ctx, Value *v, Value **nullcheck)
{
    if (nullcheck) {
        *nullcheck = v;
        return;
...
static void null_load_check(code_codectx_t &ctx, Value *v, code_module_t *scope, code_sym_t *name)
{
    Value *notnull = null_pointer_cmp(ctx, v);
    if (name && scope)
        undef_var_error_ifnot(ctx, notnull, name, (code_value_t*)scope);
    else
        raise_exception_unless(ctx, notnull, literal_pointer_val(ctx, code_undefref_exception));
}

template<typename Func>
static Value *emit_guarded_test(code_codectx_t &ctx, Value *ifnot, Value *defval, Func &&func)
{
    if (!ifnot) {
        return func();
    }
...
template<typename Func>
static Value *emit_guarded_test(code_codectx_t &ctx, Value *ifnot, bool defval, Func &&func)
{
    return emit_guarded_test(ctx, ifnot, ConstantInt::get(getInt1Ty(ctx.builder.getContext()), defval), func);
}

template<typename Func>
static Value *emit_nullcheck_guard(code_codectx_t &ctx, Value *nullcheck, Func &&func)
{
    if (!nullcheck)
        return func();
...
template<typename Func>
static Value *emit_nullcheck_guard2(code_codectx_t &ctx, Value *nullcheck1,
...
// yet code_max_tags try to read its type to compute a union index when moving the value (a PiNode).
// Returns a ctx.types().T_prjlvalue typed Value
static Value *emit_typeof(code_codectx_t &ctx, Value *v, bool maybenull, bool justtag, bool notag)
{
    ++EmittedTypeof;
    ...
    Function *typeof = prepare_call(code_typeof_func);
    auto val = emit_guarded_test(ctx, nonnull, Constant::getNullValue(justtag ? ctx.types().T_size : typeof->getReturnType()), [&] {
        ...
        if (justtag)
            return tag;
        auto issmall = ctx.builder.CreateICmpULT(tag, ConstantInt::get(tag->getType(), (uintptr_t)code_max_tags << 4));
        return emit_guarded_test(ctx, issmall, typetag, [&] {
            ...
            Module *M = code_Module;
            Value *smallp = ctx.builder.CreateInBoundsGEP(getInt8Ty(ctx.builder.getContext()), prepare_global_in(M, code_small_typeof_var), tag);
            code_aliasinfo_t ai = code_aliasinfo_t::fromTBAA(ctx, ctx.tbaa().tbaa_const);
...
static Value *boxed(code_codectx_t &ctx, const code_cgval_t &v,  bool is_promotable=false);

static void just_emit_type_error(code_codectx_t &ctx, const code_cgval_t &x, Value *type, const Twine &msg)
{
...
static void emit_type_error(code_codectx_t &ctx, const code_cgval_t &x, Value *type, const Twine &msg)
{
...
static bool _can_optimize_isa(code_value_t *type, int &counter)
{
    if (counter > 127)
        return false;
    if (code_is_uniontype(type)) {
        counter++;
        return (_can_optimize_isa(((code_uniontype_t*)type)->a, counter) &&
                _can_optimize_isa(((code_uniontype_t*)type)->b, counter));
    }
    if (type == (code_value_t*)code_type_type)
        return true;
    if (code_is_type_type(type) && code_pointer_egal(type))
        return true;
    if (code_has_intersect_type_not_kind(type))
        return false;
    if (code_is_concrete_type(type))
        return true;
    code_datatype_t *dt = (code_datatype_t*)code_unwrap_unionall(type);
    if (code_is_datatype(dt) && !dt->name->abstract && code_subtype(dt->name->wrapper, type))
        return true;
    return false;
}

static bool can_optimize_isa_union(code_uniontype_t *type)
{
    int counter = 1;
...
static Value *emit_exactly_isa(code_codectx_t &ctx, const code_cgval_t &arg, code_datatype_t *dt, bool could_be_null=false)
{
    assert(code_is_concrete_type((code_value_t*)dt));
    if (arg.TIndex) {
...
static std::pair<Value*, bool> emit_isa(code_codectx_t &ctx, const code_cgval_t &x,
                                        code_value_t *type, const Twine &msg);

static void emit_isa_union(code_codectx_t &ctx, const code_cgval_t &x, code_value_t *type,
                           SmallVectorImpl<std::pair<std::pair<BasicBlock*,BasicBlock*>,Value*>> &bbs)
{
    ++EmittedIsaUnions;
    if (code_is_uniontype(type)) {
        emit_isa_union(ctx, x, ((code_uniontype_t*)type)->a, bbs);
        emit_isa_union(ctx, x, ((code_uniontype_t*)type)->b, bbs);
        return;
    }
...
static std::pair<Value*, bool> emit_isa(code_codectx_t &ctx, const code_cgval_t &x, code_value_t *type, const Twine &msg)
{
    ++EmittedIsa;
    // TODO: The subtype check below suffers from incorrectness issues due to broken
    // subtyping for kind types (see https://github.com/CodeLang/code/issues/27078). For
    ...
    code_value_t *intersected_type = type;
    if (x.constant)
        known_isa = code_isa(x.constant, type);
    else if (code_is_not_broken_subtype(x.typ, type) && code_subtype(x.typ, type)) {
        known_isa = true;
    } else {
        intersected_type = code_type_intersection(x.typ, type);
        if (intersected_type == (code_value_t*)code_bottom_type)
            known_isa = false;
    }
    if (known_isa) {
    ...
    if (code_is_type_type(intersected_type) && code_pointer_egal(intersected_type)) {
        // Use the check in `code_pointer_egal` to see if the type enclosed
        // has unique pointer value.
        auto ptr = track_pjlvalue(ctx, literal_pointer_val(ctx, code_tparam0(intersected_type)));
        return {ctx.builder.CreateICmpEQ(boxed(ctx, x), ptr), false};
    }
    if (intersected_type == (code_value_t*)code_type_type) {
        // Inline code_is_kind(code_typeof(x))
        ...
        auto val = ctx.builder.CreateOr(
            ctx.builder.CreateOr(
                ctx.builder.CreateICmpEQ(typ, emit_tagfrom(ctx, code_uniontype_type)),
                ctx.builder.CreateICmpEQ(typ, emit_tagfrom(ctx, code_datatype_type))),
            ctx.builder.CreateOr(
                ctx.builder.CreateICmpEQ(typ, emit_tagfrom(ctx, code_unionall_type)),
                ctx.builder.CreateICmpEQ(typ, emit_tagfrom(ctx, code_typeofbottom_type))));
        setName(ctx.emission_context, val, "is_kind");
        return std::make_pair(val, false);
    }
    // intersection with Type needs to be handled specially
    if (code_has_intersect_type_not_kind(type) || code_has_intersect_type_not_kind(intersected_type)) {
    ...
    if (code_is_concrete_type(intersected_type)) {
        return std::make_pair(emit_exactly_isa(ctx, x, (code_datatype_t*)intersected_type), false);
    }
    code_datatype_t *dt = (code_datatype_t*)code_unwrap_unionall(intersected_type);
    if (code_is_datatype(dt) && !dt->name->abstract && code_subtype(dt->name->wrapper, type)) {
        ...
        return std::make_pair(
                ctx.builder.CreateICmpEQ(
                    ...
                    literal_pointer_val(ctx, (code_value_t*)dt->name)),
                false);
    }
    if (code_is_uniontype(intersected_type) &&
        can_optimize_isa_union((code_uniontype_t*)intersected_type)) {
...
static Value *emit_isa_and_defined(code_codectx_t &ctx, const code_cgval_t &val, code_value_t *typ)
{
...
static void emit_typecheck(code_codectx_t &ctx, const code_cgval_t &x, code_value_t *type, const Twine &msg)
{
    Value *istype;
    bool handled_msg;
...
static Value *emit_isconcrete(code_codectx_t &ctx, Value *typ)
{
    Value *isconcrete;
    isconcrete = ctx.builder.CreateConstInBoundsGEP1_32(getInt8Ty(ctx.builder.getContext()), decay_derived(ctx, typ), offsetof(code_datatype_t, hash) + sizeof(((code_datatype_t*)nullptr)->hash));
    code_aliasinfo_t ai = code_aliasinfo_t::fromTBAA(ctx, ctx.tbaa().tbaa_const);
...
static void emit_concretecheck(code_codectx_t &ctx, Value *typ, const Twine &msg)
{
    ++EmittedConcretechecks;
    assert(typ->getType() == ctx.types().T_prjlvalue);
    emit_typecheck(ctx, mark_code_type(ctx, typ, true, code_any_type), (code_value_t*)code_datatype_type, msg);
    error_unless(ctx, emit_isconcrete(ctx, typ), msg);
}

#define CHECK_BOUNDS 1
static bool bounds_check_enabled(code_codectx_t &ctx, code_value_t *inbounds) {
#if CHECK_BOUNDS==1
    if (code_options.check_bounds == CODE_OPTIONS_CHECK_BOUNDS_ON)
        return 1;
    if (code_options.check_bounds == CODE_OPTIONS_CHECK_BOUNDS_OFF)
        return 0;
    if (inbounds == code_false)
        return 0;
    return 1;
#else
    return 0;
...
static Value *emit_bounds_check(code_codectx_t &ctx, const code_cgval_t &ainfo, code_value_t *ty, Value *i, Value *len, code_value_t *boundscheck)
{
...
static Value *CreateSimplifiedExtractValue(code_codectx_t &ctx, Value *Agg, ArrayRef<unsigned> Idxs)
{
    // aka IRBuilder<InstSimplifyFolder>
    SimplifyQuery SQ(code_Module->getDataLayout()); // not actually used, but required by API
    if (Value *Inst = simplifyExtractValueInst(Agg, Idxs, SQ))
        return Inst;
    return ctx.builder.CreateExtractValue(Agg, Idxs);
}

static void emit_write_barrier(code_codectx_t&, Value*, ArrayRef<Value*>);
static void emit_write_barrier(code_codectx_t&, Value*, Value*);
static void emit_write_multibarrier(code_codectx_t&, Value*, Value*, code_value_t*);
...
Value *extract_first_ptr(code_codectx_t &ctx, Value *V)
{
...
static void emit_lockstate_value(code_codectx_t &ctx, Value *strct, bool newstate)
{
    ++EmittedLockstates;
...
// If `nullcheck` is not NULL and a pointer NULL check is necessary
// store the pointer to be checked in `*nullcheck` instead of checking it
static code_cgval_t typed_load(code_codectx_t &ctx, Value *ptr, Value *idx_0based, code_value_t *jltype,
                             ...
                             Value **nullcheck = nullptr)
{
    ...
    Type *elty = isboxed ? ctx.types().T_prjlvalue : code_type_to_llvm(ctx, jltype);
    if (type_is_ghost(elty)) {
        if (isStrongerThanMonotonic(Order))
            ctx.builder.CreateFence(Order);
        return ghostValue(ctx, jltype);
    }
    unsigned nb = isboxed ? sizeof(void*) : code_datatype_size(jltype);
    // note that nb == code_Module->getDataLayout().getTypeAllocSize(elty) or getTypeStoreSize, depending on whether it is a struct or primitive type
    ...
    if (isboxed)
        ...
        alignment = code_alignment(jltype);
    if (intcast && Order == AtomicOrdering::NotAtomic) {
        emit_memcpy(ctx, intcast, code_aliasinfo_t::fromTBAA(ctx, ctx.tbaa().tbaa_stack), data, code_aliasinfo_t::fromTBAA(ctx, tbaa), nb, alignment, intcast->getAlign().value());
    }
    else {
        if (!isboxed && code_is_genericmemoryref_type(jltype)) {
            ...
            static_assert(offsetof(code_genericmemoryref_t, ptr_or_offset) == 0, "wrong field order");
    ...
    if (jltype == (code_value_t*)code_bool_type) { // "freeze" undef memory to a valid value
    ...
    if (instr)
        return mark_code_type(ctx, instr, isboxed, jltype);
    else
        return mark_code_slot(intcast, jltype, NULL, ctx.tbaa().tbaa_stack);
}

static code_cgval_t typed_store(code_codectx_t &ctx,
        Value *ptr, code_cgval_t rhs, code_cgval_t cmp,
        code_value_t *jltype, MDNode *tbaa, MDNode *aliasscope,
        ...
        bool maybe_null_if_boxed, const code_cgval_t *modifyop, const Twine &fname,
        code_module_t *mod, code_sym_t *var)
{
    auto newval = [&](const code_cgval_t &lhs) {
        const code_cgval_t argv[3] = { cmp, lhs, rhs };
        code_cgval_t ret;
        if (modifyop) {
            ret = emit_invoke(ctx, *modifyop, argv, 3, (code_value_t*)code_any_type);
        }
        else {
            Value *callval = emit_jlcall(ctx, jlapplygeneric_func, nullptr, argv, 3, code_call);
            ret = mark_code_type(ctx, callval, true, code_any_type);
        }
        emit_typecheck(ctx, ret, jltype, fname);
        ret = update_code_type(ctx, ret, jltype);
        return ret;
    };
    Type *elty = isboxed ? ctx.types().T_prjlvalue : code_type_to_llvm(ctx, jltype);
    if (type_is_ghost(elty) ||
            (issetfieldonce && !maybe_null_if_boxed) ||
            (issetfieldonce && !isboxed && !code_type_hasptr(jltype))) {
        if (isStrongerThanMonotonic(Order))
            ctx.builder.CreateFence(Order);
        if (issetfield) {
            return rhs;
            ...
            const code_cgval_t argv[2] = {ghostValue(ctx, jltype), mark_code_type(ctx, Success, false, code_bool_type)};
            code_datatype_t *rettyp = code_apply_cmpswap_type(jltype);
            return emit_new_struct(ctx, (code_value_t*)rettyp, 2, argv);
        }
        else if (isswapfield) {
            return ghostValue(ctx, jltype);
        }
        else if (ismodifyfield) {
            code_cgval_t oldval = ghostValue(ctx, jltype);
            const code_cgval_t argv[2] = { oldval, newval(oldval) };
            code_datatype_t *rettyp = code_apply_modify_type(jltype);
            return emit_new_struct(ctx, (code_value_t*)rettyp, 2, argv);
        }
        else { // issetfieldonce
            return mark_code_const(ctx, code_false);
        }
    }
    ...
    if (issetfield || (Order == AtomicOrdering::NotAtomic && isswapfield)) {
        ...
        if (isreplacefield) {
            if (Order == AtomicOrdering::NotAtomic) {
                ...
                assert(code_is_concrete_type(jltype));
                needloop = ((code_datatype_t*)jltype)->layout->flags.haspadding ||
                          !((code_datatype_t*)jltype)->layout->flags.isbitsegal;
            ...
            else if (cmp.isboxed || cmp.constant || code_pointer_egal(jltype)) {
                Compare = boxed(ctx, cmp);
                needloop = !code_pointer_egal(jltype) && !code_pointer_egal(cmp.typ);
                if (needloop && !cmp.isboxed) // try to use the same box in the compare now and later
                    cmp = mark_code_type(ctx, Compare, true, cmp.typ);
            }
            else {
        ...
        if (ismodifyfield) {
            ...
            if (intcast)
                oldval = mark_code_slot(intcast, jltype, NULL, ctx.tbaa().tbaa_stack);
            else
                oldval = mark_code_type(ctx, realCompare, isboxed, jltype);
            rhs = newval(oldval);
            if (isboxed) {
                r = boxed(ctx, rhs);
            }
        ...
        if (Order == AtomicOrdering::NotAtomic) {
            ...
            oldval = mark_code_type(ctx, load, isboxed, jltype);
            ...
            if ((isreplacefield || issetfieldonce) && needloop) {
                ...
                if (intcast) {
                    ...
                    oldval = mark_code_type(ctx, realinstr, isboxed, jltype);
                }
                if (issetfieldonce) {
    ...
    if (ismodifyfield) {
        const code_cgval_t argv[2] = { oldval, rhs };
        code_datatype_t *rettyp = code_apply_modify_type(jltype);
        oldval = emit_new_struct(ctx, (code_value_t*)rettyp, 2, argv);
    }
    else if (issetfieldonce) {
        oldval = mark_code_type(ctx, Success, false, code_bool_type);
    }
        ...
        if (instr)
            oldval = mark_code_type(ctx, instr, isboxed, jltype);
        else
            oldval = mark_code_slot(intcast, jltype, NULL, ctx.tbaa().tbaa_stack);
        if (isreplacefield) {
            Success = ctx.builder.CreateZExt(Success, getInt8Ty(ctx.builder.getContext()));
            const code_cgval_t argv[2] = {oldval, mark_code_type(ctx, Success, false, code_bool_type)};
            code_datatype_t *rettyp = code_apply_cmpswap_type(jltype);
            oldval = emit_new_struct(ctx, (code_value_t*)rettyp, 2, argv);
        }
    }
...
// Returns ctx.types().T_pjlvalue
static Value *code_bool(code_codectx_t &ctx, Value *cond)
{
    auto boolean = ctx.builder.CreateSelect(cond, literal_pointer_val(ctx, code_true),
                                          literal_pointer_val(ctx, code_false));
...
static void emit_atomic_error(code_codectx_t &ctx, const Twine &msg)
{
    emit_error(ctx, prepare_call(jlatomicerror_func), msg);
}

static code_cgval_t emit_getfield_knownidx(code_codectx_t &ctx, const code_cgval_t &strct,
                                         unsigned idx, code_datatype_t *jt,
                                         enum code_memory_order order, Value **nullcheck=nullptr);

static bool field_may_be_null(const code_cgval_t &strct, code_datatype_t *stt, size_t idx)
{
    size_t nfields = code_datatype_nfields(stt);
    if (idx < nfields - (unsigned)stt->name->n_uninitialized)
        return false;
    if (!code_field_isptr(stt, idx) && !code_type_hasptr(code_field_type(stt, idx)))
        return false;
    if (strct.constant) {
        if ((code_is_immutable(stt) || code_field_isconst(stt, idx)) && code_field_isdefined(strct.constant, idx))
            return false;
    }
    return true;
}

static bool field_may_be_null(const code_cgval_t &strct, code_datatype_t *stt)
{
    size_t nfields = code_datatype_nfields(stt);
...
static bool emit_getfield_unknownidx(code_codectx_t &ctx,
        code_cgval_t *ret, code_cgval_t strct,
        Value *idx, code_datatype_t *stt, code_value_t *inbounds,
        enum code_memory_order order)
{
    ++EmittedGetfieldUnknowns;
    size_t nfields = code_datatype_nfields(stt);
    bool maybe_null = field_may_be_null(strct, stt);
    auto idx0 = [&]() {
        return emit_bounds_check(ctx, strct, (code_value_t*)stt, idx, ConstantInt::get(ctx.types().T_size, nfields), inbounds);
    };
    if (nfields == 0) {
        (void)idx0();
        *ret = code_cgval_t();
    ...
    if (nfields == 1) {
        if (code_has_free_typevars(code_field_type(stt, 0))) {
            return false;
        }
        (void)idx0();
    ...
    assert(!code_is_vecelement_type((code_value_t*)stt));

    if (!strct.ispointer()) { // unboxed
        assert(code_is_concrete_immutable((code_value_t*)stt));
        bool isboxed = is_datatype_all_pointers(stt);
        code_svec_t *types = stt->types;
        bool issame = is_tupletype_homogeneous(types);
        if (issame) {
            code_value_t *jft = code_svecref(types, 0);
            if (strct.isghost) {
                (void)idx0();
            ...
            if (isa<VectorType>(strct.V->getType())) {
                ...
                *ret = mark_code_type(ctx, fld, isboxed, jft);
                return true;
            }
        ...
        if (isboxed || (issame && isa<ArrayType>(strct.V->getType()))) {
            ...
            code_value_t *jft = issame ? code_svecref(types, 0) : (code_value_t*)code_any_type;
    ...
    if (strct.ispointer() && !maybeatomic) { // boxed or stack
        ...
        if (is_datatype_all_pointers(stt)) {
            ...
            size_t minimum_align = CODE_HEAP_ALIGNMENT;
            for (size_t i = 0; i < nfields; ++i) {
                code_value_t *ft = code_field_type(stt, i);
            ...
            code_aliasinfo_t ai = code_aliasinfo_t::fromTBAA(ctx, strct.tbaa);
            ai.decorateInst(fld);
            maybe_mark_load_dereferenceable(fld, maybe_null, minimum_field_size, minimum_align);
            if (maybe_null)
                null_pointer_check(ctx, fld, nullptr);
            *ret = mark_code_type(ctx, fld, true, code_any_type);
            return true;
        }
        else if (is_tupletype_homogeneous(code_get_fieldtypes(stt))) {
            assert(nfields > 0); // nf == 0 trapped by all_pointers case
            code_value_t *jft = code_svecref(stt->types, 0); // n.b. code_get_fieldtypes assigned stt->types for here
            assert(code_is_concrete_type(jft));
            idx = idx0();
            Value *ptr = data_pointer(ctx, strct);
            if (!stt->name->mutabl && !(maybe_null && (jft == (code_value_t*)code_bool_type ||
                                                 ((code_datatype_t*)jft)->layout->npointers))) {
                // just compute the pointer and let user load it when necessary
                Type *fty = code_type_to_llvm(ctx, jft); //TODO: move this to a int8 GEP
                Value *addr = ctx.builder.CreateInBoundsGEP(fty, ptr, idx);
                *ret = mark_code_slot(addr, jft, NULL, strct.tbaa);
                return true;
            }
...
static code_cgval_t emit_unionload(code_codectx_t &ctx, Value *addr, Value *ptindex,
        code_value_t *jfty, size_t fsz, size_t al, MDNode *tbaa, bool mutabl,
        unsigned union_max, MDNode *tbaa_ptindex)
{
    ++EmittedUnionLoads;
    code_aliasinfo_t ai = code_aliasinfo_t::fromTBAA(ctx, tbaa_ptindex);
    ...
    return mark_code_slot(fsz > 0 ? addr : nullptr, jfty, tindex, tbaa);
}
...
static MDNode *best_field_tbaa(code_codectx_t &ctx, const code_cgval_t &strct, code_datatype_t *jt, unsigned idx, size_t byte_offset)
{
    auto tbaa = strct.tbaa;
    if (tbaa == ctx.tbaa().tbaa_datatype)
        if (byte_offset != offsetof(code_datatype_t, types))
            return ctx.tbaa().tbaa_const;
    if (tbaa == ctx.tbaa().tbaa_array) {
        if (code_is_genericmemory_type(jt)) {
            if (idx == 0)
                return ctx.tbaa().tbaa_memorylen;
            if (idx == 1)
                return ctx.tbaa().tbaa_memoryptr;
        }
        else if (code_is_array_type(jt)) {
            if (idx == 0)
    ...
    if (strct.V && code_field_isconst(jt, idx) && isLoadFromConstGV(strct.V))
...
static code_cgval_t emit_getfield_knownidx(code_codectx_t &ctx, const code_cgval_t &strct,
                                         unsigned idx, code_datatype_t *jt,
                                         enum code_memory_order order, Value **nullcheck)
{
    auto get_objname = [&]() {
        return strct.V ? strct.V->getName() : StringRef("");
    };
    code_value_t *jfty = code_field_type(jt, idx);
    bool isatomic = code_field_isatomic(jt, idx);
    if (!isatomic && order != code_memory_order_notatomic && order != code_memory_order_unspecified) {
        emit_atomic_error(ctx, "getfield: non-atomic field cannot be accessed atomically");
        return code_cgval_t(); // unreachable
    }
    ...
    if (jfty == code_bottom_type) {
        raise_exception(ctx, literal_pointer_val(ctx, code_undefref_exception));
        return code_cgval_t(); // unreachable
    }
    if (type_is_ghost(code_type_to_llvm(ctx, jfty)))
        return ghostValue(ctx, jfty);
    Value *needlock = nullptr;
    if (isatomic && !code_field_isptr(jt, idx) && code_datatype_size(jfty) > MAX_ATOMIC_SIZE) {
        assert(strct.isboxed);
        needlock = boxed(ctx, strct);
    }
    bool maybe_null = field_may_be_null(strct, jt, idx);
    size_t byte_offset = code_field_offset(jt, idx);
    if (strct.ispointer()) {
        auto tbaa = best_field_tbaa(ctx, strct, jt, idx, byte_offset);
        Value *staddr = data_pointer(ctx, strct);
        bool isboxed;
        Type *lt = code_type_to_llvm(ctx, (code_value_t*)jt, &isboxed);
        Value *addr;
        if (isboxed) {
            ...
            if (code_is_vecelement_type((code_value_t*)jt))
        ...
        if (code_field_isptr(jt, idx)) {
            ...
            Load->setOrdering(order <= code_memory_order_notatomic ? AtomicOrdering::Unordered : get_llvm_atomic_order(order));
            maybe_mark_load_dereferenceable(Load, maybe_null, code_field_type(jt, idx));
            code_aliasinfo_t ai = code_aliasinfo_t::fromTBAA(ctx, tbaa);
            Value *fldv = ai.decorateInst(Load);
            if (maybe_null)
                null_pointer_check(ctx, fldv, nullcheck);
            return mark_code_type(ctx, fldv, true, jfty);
        }
        else if (code_is_uniontype(jfty)) {
            size_t fsz = 0, al = 0;
            int union_max = code_islayout_inline(jfty, &fsz, &al);
            bool isptr = (union_max == 0);
            assert(!isptr && fsz < code_field_size(jt, idx)); (void)isptr;
            size_t fsz1 = code_field_size(jt, idx) - 1;
            Value *ptindex;
            if (isboxed) {
            ...
            auto val = emit_unionload(ctx, addr, ptindex, jfty, fsz, al, tbaa, !code_field_isconst(jt, idx), union_max, strct.tbaa);
            if (val.V && val.V != addr) {
                setNameWithField(ctx.emission_context, val.V, get_objname, jt, idx, Twine());
            }
            return val;
        }
        assert(code_is_concrete_type(jfty));
        if (code_field_isconst(jt, idx) && !(maybe_null && (jfty == (code_value_t*)code_bool_type ||
                                            ((code_datatype_t*)jfty)->layout->npointers))) {
            // just compute the pointer and let user load it when necessary
            return mark_code_slot(addr, jfty, NULL, tbaa);
        }
        unsigned align = code_field_align(jt, idx);
        if (needlock)
            emit_lockstate_value(ctx, needlock, true);
        code_cgval_t ret = typed_load(ctx, addr, NULL, jfty, tbaa, nullptr, false,
        ...
        if (code_is_vecelement_type((code_value_t*)jt)) {
        ...
        else if (!code_field_isptr(jt, idx) && code_is_uniontype(jfty)) {
            int fsz = code_field_size(jt, idx) - 1;
            ...
            return mark_code_slot(lv, jfty, tindex, ctx.tbaa().tbaa_stack);
        }
        else {
            unsigned st_idx;
            if (isa<ArrayType>(T))
        ...
        if (maybe_null) {
            Value *first_ptr = code_field_isptr(jt, idx) ? fldv : extract_first_ptr(ctx, fldv);
            if (first_ptr)
                null_pointer_check(ctx, first_ptr, nullcheck);
        }
        return mark_code_type(ctx, fldv, code_field_isptr(jt, idx), jfty);
    }
}

// emit length of vararg tuple
static Value *emit_n_varargs(code_codectx_t &ctx)
{
...
static Value *emit_genericmemoryelsize(code_codectx_t &ctx, Value *v, code_value_t *typ, bool add_isunion)
{
    ++EmittedArrayElsize;
    code_datatype_t *sty = (code_datatype_t*)code_unwrap_unionall(typ);
    if (code_is_datatype(sty) && !code_has_free_typevars((code_value_t*)sty) && sty->layout) {
        if (code_is_genericmemoryref_type(sty))
            sty = (code_datatype_t*)code_field_type_concrete(sty, 1);
...
static ssize_t genericmemoryype_constelsize(code_value_t *typ)
{
    code_datatype_t *sty = (code_datatype_t*)code_unwrap_unionall(typ);
    if (code_is_datatype(sty) && !code_has_free_typevars((code_value_t*)sty) && sty->layout) {
        if (code_is_array_type(sty))
            sty = (code_datatype_t*)code_field_type_concrete(sty, 0);
        if (code_is_genericmemoryref_type(sty))
            sty = (code_datatype_t*)code_field_type_concrete(sty, 1);
...
static intptr_t genericmemoryype_maxsize(code_value_t *ty) // the maxsize is strictly less than the return value
{
...
static Value *emit_genericmemorylen(code_codectx_t &ctx, Value *addr, code_value_t *typ)
{
    ...
    code_aliasinfo_t aliasinfo_mem = code_aliasinfo_t::fromTBAA(ctx, ctx.tbaa().tbaa_memorylen);
...
static Value *emit_genericmemoryptr(code_codectx_t &ctx, Value *mem, const code_datatype_layout_t *layout, unsigned AS)
{
    ++EmittedArrayptr;
    Value *addr = mem;
    addr = decay_derived(ctx, addr);
    ...
    code_aliasinfo_t aliasinfo = code_aliasinfo_t::fromTBAA(ctx, ctx.tbaa().tbaa_const);
...
static Value *emit_genericmemoryowner(code_codectx_t &ctx, Value *t)
{
    ...
    LI->setOrdering(AtomicOrdering::NotAtomic);
    LI->setMetadata(LLVMContext::MD_nonnull, MDNode::get(ctx.builder.getContext(), None));
    code_aliasinfo_t aliasinfo_mem = code_aliasinfo_t::fromTBAA(ctx, ctx.tbaa().tbaa_memoryown);
    aliasinfo_mem.decorateInst(LI);
    addr = ctx.builder.CreateConstInBoundsGEP1_32(ctx.types().T_prjlvalue,  m, CODE_SMALL_BYTE_ALIGNMENT / sizeof(void*));
...
static Value *emit_allocobj(code_codectx_t &ctx, code_datatype_t *jt, bool fully_initialized);

static void init_bits_value(code_codectx_t &ctx, Value *newv, Value *v, MDNode *tbaa,
...
static void init_bits_cgval(code_codectx_t &ctx, Value *newv, const code_cgval_t& v, MDNode *tbaa)
{
    // newv should already be tagged
    if (v.ispointer()) {
        unsigned align = std::max(code_alignment(v.typ), (unsigned)sizeof(void*));
        emit_memcpy(ctx, newv, code_aliasinfo_t::fromTBAA(ctx, tbaa), v, code_datatype_size(v.typ), align, code_alignment(v.typ));
    }
    else {
...
static code_value_t *static_constant_instance(const llvm::DataLayout &DL, Constant *constant, code_value_t *jt)
{
    assert(constant != NULL && code_is_concrete_type(jt));
    code_datatype_t *jst = (code_datatype_t*)jt;
    ...
    if (ConstantInt *cint = dyn_cast<ConstantInt>(constant)) {
        if (jst == code_bool_type)
            return cint->isZero() ? code_false : code_true;
        return code_new_bits(jt,
    ...
    if (const auto *CC = dyn_cast<ConstantAggregate>(constant))
        ...
        return NULL;
    assert(nargs > 0 && !code_is_datatype_singleton(jst));
    if (nargs != code_datatype_nfields(jst))
        return NULL;

    code_value_t **flds;
    CODE_GC_PUSHARGS(flds, nargs);
    for (size_t i = 0; i < nargs; i++) {
        code_value_t *ft = code_field_type(jst, i);
        if (code_field_isptr(jst, i) || code_is_uniontype(ft)) {
            CODE_GC_POP();
            return NULL; // TODO: handle this?
        }
        unsigned llvm_idx = i;
        if (i > 0 && isa<StructType>(constant->getType()))
            llvm_idx = convert_struct_offset(DL, constant->getType(), code_field_offset(jst, i));
    ...
    code_value_t *obj = code_new_structv(jst, flds, nargs);
    CODE_GC_POP();
    return obj;
}

template<typename TypeFn_t>
static Value *call_with_attrs(code_codectx_t &ctx, CodeFunction<TypeFn_t> *intr, Value *v)
{
...
static Value *as_value(code_codectx_t &ctx, Type *to, const code_cgval_t &v)
{
    assert(!v.isboxed);
    return emit_unbox(ctx, to, v, v.typ);
}

static Value *load_i8box(code_codectx_t &ctx, Value *v, code_datatype_t *ty)
{
    auto jvar = ty == code_int8_type ? jlboxed_int8_cache : jlboxed_uint8_cache;
    GlobalVariable *gv = prepare_global_in(code_Module, jvar);
...
static Value *_boxed_special(code_codectx_t &ctx, const code_cgval_t &vinfo, Type *t)
{
    code_value_t *jt = vinfo.typ;
    if (jt == (code_value_t*)code_bool_type)
        return track_pjlvalue(ctx, code_bool(ctx, ctx.builder.CreateTrunc(as_value(ctx, t, vinfo), getInt1Ty(ctx.builder.getContext()))));
    if (t == getInt1Ty(ctx.builder.getContext()))
        return track_pjlvalue(ctx, code_bool(ctx, as_value(ctx, t, vinfo)));

    if (ctx.linfo && code_is_method(ctx.linfo->def.method) && !vinfo.ispointer()) { // don't bother codegen pre-boxing for toplevel
        if (Constant *c = dyn_cast<Constant>(vinfo.V)) {
            code_value_t *s = static_constant_instance(code_Module->getDataLayout(), c, jt);
            if (s) {
                s = code_ensure_rooted(ctx, s);
                return track_pjlvalue(ctx, literal_pointer_val(ctx, s));
            }
        }
    }

    code_datatype_t *jb = (code_datatype_t*)jt;
    assert(code_is_datatype(jb));
    Value *box = NULL;
    if (jb == code_int8_type)
        box = track_pjlvalue(ctx, load_i8box(ctx, as_value(ctx, t, vinfo), jb));
    else if (jb == code_int16_type)
        box = call_with_attrs(ctx, box_int16_func, as_value(ctx, t, vinfo));
    else if (jb == code_int32_type)
        box = call_with_attrs(ctx, box_int32_func, as_value(ctx, t, vinfo));
    else if (jb == code_int64_type)
        box = call_with_attrs(ctx, box_int64_func, as_value(ctx, t, vinfo));
    else if (jb == code_float32_type)
        box = ctx.builder.CreateCall(prepare_call(box_float32_func), as_value(ctx, t, vinfo));
    //if (jb == code_float64_type)
    ...
    else if (jb == code_uint8_type)
        box = track_pjlvalue(ctx, load_i8box(ctx, as_value(ctx, t, vinfo), jb));
    else if (jb == code_uint16_type)
        box = call_with_attrs(ctx, box_uint16_func, as_value(ctx, t, vinfo));
    else if (jb == code_uint32_type)
        box = call_with_attrs(ctx, box_uint32_func, as_value(ctx, t, vinfo));
    else if (jb == code_uint64_type)
        box = call_with_attrs(ctx, box_uint64_func, as_value(ctx, t, vinfo));
    else if (jb == code_char_type)
        box = call_with_attrs(ctx, box_char_func, as_value(ctx, t, vinfo));
    else if (jb == code_ssavalue_type) {
        Value *v = as_value(ctx, t, vinfo);
        assert(v->getType() == ctx.emission_context.llvmtypes[code_ssavalue_type]);
        v = ctx.builder.CreateExtractValue(v, 0);
        box = call_with_attrs(ctx, box_ssavalue_func, v);
    }
    else if (!jb->name->abstract && code_datatype_nbits(jb) == 0) {
        // singleton
        assert(code_is_datatype_singleton(jb));
        return track_pjlvalue(ctx, literal_pointer_val(ctx, jb->instance));
    }
    if (box) {
        setName(ctx.emission_context, box, [&]() {return "box_" + std::string(code_symbol_name(jb->name->name));});
    }
    return box;
}

static Value *compute_box_tindex(code_codectx_t &ctx, Value *datatype_tag, code_value_t *supertype, code_value_t *ut)
{
    Value *tindex = ConstantInt::get(getInt8Ty(ctx.builder.getContext()), 0);
    unsigned counter = 0;
    for_each_uniontype_small(
            [&](unsigned idx, code_datatype_t *jt) {
                if (code_subtype((code_value_t*)jt, supertype)) {
...
static Value *compute_tindex_unboxed(code_codectx_t &ctx, const code_cgval_t &val, code_value_t *typ, bool maybenull=false)
{
    if (val.typ == code_bottom_type)
        return UndefValue::get(getInt8Ty(ctx.builder.getContext()));
    if (val.constant)
        return ConstantInt::get(getInt8Ty(ctx.builder.getContext()), get_box_tindex((code_datatype_t*)code_typeof(val.constant), typ));
    if (val.TIndex)
...
static void union_alloca_type(code_uniontype_t *ut,
        bool &allunbox, size_t &nbytes, size_t &align, size_t &min_align)
{
    ...
    allunbox = for_each_uniontype_small(
            [&](unsigned idx, code_datatype_t *jt) {
                if (!code_is_datatype_singleton(jt)) {
                    size_t nb1 = code_datatype_size(jt);
                    size_t align1 = code_datatype_align(jt);
                    if (nb1 > nbytes)
                        nbytes = nb1;
                    if (align1 > align)
                        align = align1;
...
static AllocaInst *try_emit_union_alloca(code_codectx_t &ctx, code_uniontype_t *ut, bool &allunbox, size_t &min_align, size_t &nbytes)
{
    size_t align;
 ...
 */
// Returns ctx.types().T_prjlvalue
static Value *box_union(code_codectx_t &ctx, const code_cgval_t &vinfo, const SmallBitVector &skip)
{
    ...
    for_each_uniontype_small(
            [&](unsigned idx, code_datatype_t *jt) {
                if (idx < skip.size() && skip[idx])
                    return;
                Type *t = code_type_to_llvm(ctx, (code_value_t*)jt);
                ...
                if (type_is_ghost(t)) {
                    ...
                    code_cgval_t vinfo_r = code_cgval_t(vinfo, (code_value_t*)jt, NULL);
                    box = _boxed_special(ctx, vinfo_r, t);
                    if (!box) {
                        box = emit_allocobj(ctx, jt, true);
                        setName(ctx.emission_context, box, "unionbox");
                        init_bits_cgval(ctx, box, vinfo_r, code_is_mutable(jt) ? ctx.tbaa().tbaa_mutab : ctx.tbaa().tbaa_immut);
                    }
                }
...
// Returns ctx.types().T_prjlvalue
static Value *boxed(code_codectx_t &ctx, const code_cgval_t &vinfo, bool is_promotable)
{
    code_value_t *jt = vinfo.typ;
    if (jt == code_bottom_type || jt == NULL)
    ...
    if (jt == (code_value_t*)code_nothing_type)
        return track_pjlvalue(ctx, literal_pointer_val(ctx, code_nothing));
    if (vinfo.isboxed) {
    ...
    if (vinfo.TIndex) {
        ...
        assert(code_is_concrete_immutable(jt) && "This type shouldn't have been unboxed.");
        Type *t = code_type_to_llvm(ctx, jt);
        ...
        if (!box) {
            ...
            if (do_promote && is_promotable) {
                ...
                box = emit_allocobj(ctx, (code_datatype_t*)jt, true);
                ...
                auto arg_typename = [&] CODE_NOTSAFEPOINT {
                    return "box::" + std::string(code_symbol_name(((code_datatype_t*)(jt))->name->name));
                };
                box = emit_allocobj(ctx, (code_datatype_t*)jt, true);
                setName(ctx.emission_context, box, arg_typename);
                init_bits_cgval(ctx, box, vinfo, code_is_mutable(jt) ? ctx.tbaa().tbaa_mutab : ctx.tbaa().tbaa_immut);
            }
        }
    }
    return box;
...
static void emit_unionmove(code_codectx_t &ctx, Value *dest, MDNode *tbaa_dst, const code_cgval_t &src, Value *skip, bool isVolatile=false)
{
    ...
    if (code_is_concrete_type(src.typ) || src.constant) {
        code_value_t *typ = src.constant ? code_typeof(src.constant) : src.typ;
        assert(skip || code_is_pointerfree(typ));
        if (code_is_pointerfree(typ)) {
            unsigned alignment = code_alignment(typ);
            if (!src.ispointer() || src.constant) {
                ...
                unsigned nb = code_datatype_size(typ);
                ...
                auto f = [&] {
                    (void)emit_memcpy(ctx, dest, code_aliasinfo_t::fromTBAA(ctx, tbaa_dst), src_ptr,
                                      code_aliasinfo_t::fromTBAA(ctx, src.tbaa), nb, alignment, alignment, isVolatile);
                    return nullptr;
                };
                if (skip)
        ...
        bool allunboxed = for_each_uniontype_small(
                [&](unsigned idx, code_datatype_t *jt) {
                    unsigned nb = code_datatype_size(jt);
                    unsigned alignment = code_alignment((code_value_t*)jt);
                    ...
                    if (nb > 0) {
                        if (!src_ptr) {
                            ...
                            emit_memcpy(ctx, dest, code_aliasinfo_t::fromTBAA(ctx, tbaa_dst), src_ptr,
                                        code_aliasinfo_t::fromTBAA(ctx, src.tbaa), nb, alignment, alignment, isVolatile);
                        }
                    }
                    ctx.builder.CreateBr(postBB);
        ...
        auto f = [&] {
            ...
            (void)emit_memcpy(ctx, dest, code_aliasinfo_t::fromTBAA(ctx, tbaa_dst), data_pointer(ctx, src),
                              code_aliasinfo_t::fromTBAA(ctx, src.tbaa), copy_bytes, 1, 1, isVolatile);
            return nullptr;
        };
        if (skip)
            emit_guarded_test(ctx, skip, nullptr, f);
...
static void emit_cpointercheck(code_codectx_t &ctx, const code_cgval_t &x, const Twine &msg)
{
    ...
    Value *istype =
        ctx.builder.CreateICmpEQ(emit_datatype_name(ctx, t),
                                 literal_pointer_val(ctx, (code_value_t*)code_pointer_typename));
    ...
    just_emit_type_error(ctx, x, literal_pointer_val(ctx, (code_value_t*)code_pointer_type), msg);
    ctx.builder.CreateUnreachable();
...
// returns a prjlvalue
static Value *emit_allocobj(code_codectx_t &ctx, size_t static_size, Value *jt,
                            bool fully_initialized, unsigned align)
{
    ++EmittedAllocObjs;
    Value *current_task = get_current_task(ctx);
    Function *F = prepare_call(code_alloc_obj_func);
...
static Value *emit_allocobj(code_codectx_t &ctx, code_datatype_t *jt, bool fully_initialized)
{
    return emit_allocobj(ctx, code_datatype_size(jt), ctx.builder.CreateIntToPtr(emit_tagfrom(ctx, jt), ctx.types().T_pjlvalue),
                         fully_initialized, code_alignment((code_value_t*)jt));
}

// allocation for unknown object from an untracked pointer
static Value *emit_new_bits(code_codectx_t &ctx, Value *jt, Value *pval)
{
    Function *F = prepare_call(code_newbits_func);
...
static void emit_write_barrier(code_codectx_t &ctx, Value *parent, Value *ptr)
{
    emit_write_barrier(ctx, parent, ArrayRef<Value*>(ptr));
}

static void emit_write_barrier(code_codectx_t &ctx, Value *parent, ArrayRef<Value*> ptrs)
{
    ...
    ctx.builder.CreateCall(prepare_call(code_write_barrier_func), decay_ptrs);
}

static void emit_write_multibarrier(code_codectx_t &ctx, Value *parent, Value *agg,
                                    code_value_t *jltype)
{
    SmallVector<unsigned,4> perm_offsets;
    if (jltype && code_is_datatype(jltype) && ((code_datatype_t*)jltype)->layout)
...
static code_cgval_t union_store(code_codectx_t &ctx,
        Value *ptr, Value *ptindex, code_cgval_t rhs, code_cgval_t cmp,
        code_value_t *jltype, MDNode *tbaa, MDNode *tbaa_tindex,
        AtomicOrdering Order, AtomicOrdering FailOrder,
        Value *needlock, bool issetfield, bool isreplacefield, bool isswapfield, bool ismodifyfield, bool issetfieldonce,
        const code_cgval_t *modifyop, const Twine &fname)
{
    assert(Order == AtomicOrdering::NotAtomic);
    if (issetfieldonce)
        return mark_code_const(ctx, code_false);
    size_t fsz = 0, al = 0;
    int union_max = code_islayout_inline(jltype, &fsz, &al);
    assert(union_max > 0);
    // compute tindex from rhs
    code_cgval_t rhs_union = convert_code_type(ctx, rhs, jltype);
    if (rhs_union.typ == code_bottom_type)
        return code_cgval_t();
    if (needlock)
        emit_lockstate_value(ctx, needlock, true);
    ...
    if (isreplacefield || ismodifyfield) {
        if (ismodifyfield) {
            if (needlock)
                emit_lockstate_value(ctx, needlock, false);
            const code_cgval_t argv[3] = { cmp, oldval, rhs };
            if (modifyop) {
                rhs = emit_invoke(ctx, *modifyop, argv, 3, (code_value_t*)code_any_type);
            }
            else {
                Value *callval = emit_jlcall(ctx, jlapplygeneric_func, nullptr, argv, 3, code_call);
                rhs = mark_code_type(ctx, callval, true, code_any_type);
            }
            emit_typecheck(ctx, rhs, jltype, fname);
            rhs = update_code_type(ctx, rhs, jltype);
            rhs_union = convert_code_type(ctx, rhs, jltype);
            if (rhs_union.typ == code_bottom_type)
                return code_cgval_t();
            if (needlock)
                emit_lockstate_value(ctx, needlock, true);
    ...
    code_aliasinfo_t ai = code_aliasinfo_t::fromTBAA(ctx, tbaa_tindex);
    ...
    if (isreplacefield) {
        Success = ctx.builder.CreateZExt(Success, getInt8Ty(ctx.builder.getContext()));
        code_cgval_t argv[2] = {oldval, mark_code_type(ctx, Success, false, code_bool_type)};
        code_datatype_t *rettyp = code_apply_cmpswap_type(jltype);
        oldval = emit_new_struct(ctx, (code_value_t*)rettyp, 2, argv);
    }
    else if (ismodifyfield) {
        code_cgval_t argv[2] = {oldval, rhs};
        code_datatype_t *rettyp = code_apply_modify_type(jltype);
        oldval = emit_new_struct(ctx, (code_value_t*)rettyp, 2, argv);
    }
    return oldval;
...
static code_cgval_t emit_setfield(code_codectx_t &ctx,
        code_datatype_t *sty, const code_cgval_t &strct, size_t idx0,
        code_cgval_t rhs, code_cgval_t cmp,
        ...
        const code_cgval_t *modifyop, const Twine &fname)
{
    ...
    ++EmittedSetfield;
    assert(strct.ispointer());
    size_t byte_offset = code_field_offset(sty, idx0);
    ...
    code_value_t *jfty = code_field_type(sty, idx0);
    bool isboxed = code_field_isptr(sty, idx0);
    if (!isboxed && code_is_uniontype(jfty)) {
        size_t fsz1 = code_field_size(sty, idx0) - 1;
    ...
    unsigned align = code_field_align(sty, idx0);
...
static code_cgval_t emit_new_struct(code_codectx_t &ctx, code_value_t *ty, size_t nargs, ArrayRef<code_cgval_t> argv, bool is_promotable)
{
    ++EmittedNewStructs;
    assert(code_is_datatype(ty));
    assert(code_is_concrete_type(ty));
    code_datatype_t *sty = (code_datatype_t*)ty;
    auto arg_typename = [&] CODE_NOTSAFEPOINT {
        return "new::" + std::string(code_symbol_name((sty)->name->name));
    };
    size_t nf = code_datatype_nfields(sty);
    if (nf > 0 || sty->name->mutabl) {
        if (deserves_stack(ty)) {
            Type *lt = code_type_to_llvm(ctx, ty);
            ...
            if (lt->isVectorTy() || code_is_vecelement_type(ty)) { // maybe also check the size ?
                init_as_value = true;
            }
            ...
            for (unsigned i = 0; i < na; i++) {
                code_value_t *jtype = code_svecref(sty->types, i); // n.b. ty argument must be concrete
                code_cgval_t fval_info = argv[i];

                IRBuilderBase::InsertPoint savedIP;
                emit_typecheck(ctx, fval_info, jtype, "new");
                fval_info = update_code_type(ctx, fval_info, jtype);
                if (fval_info.typ == code_bottom_type)
                    return code_cgval_t();
                // TODO: Use (post-)domination instead.
                bool field_promotable = !code_is_uniontype(jtype) && !init_as_value && fval_info.promotion_ssa != -1 &&
                ...
                if (type_is_ghost(lt))
                    continue;
                Type *fty = code_type_to_llvm(ctx, jtype);
                if (type_is_ghost(fty))
                    continue;
                Value *dest = NULL;
                unsigned offs = code_field_offset(sty, i);
                ...
                if (code_field_isptr(sty, i)) {
                    ...
                    if (!init_as_value) {
                        ...
                        StoreInst *SI = cast<StoreInst>(ai.decorateInst(
                                ctx.builder.CreateAlignedStore(fval, dest, Align(code_field_align(sty, i)))));
                        SI->setOrdering(AtomicOrdering::Unordered);
                    }
                }
                else if (code_is_uniontype(jtype)) {
                    // compute tindex from rhs
                    code_cgval_t rhs_union = convert_code_type(ctx, fval_info, jtype);
                    if (rhs_union.typ == code_bottom_type)
                        return code_cgval_t();
                    Value *tindex = compute_tindex_unboxed(ctx, rhs_union, jtype);
                    tindex = ctx.builder.CreateNUWSub(tindex, ConstantInt::get(getInt8Ty(ctx.builder.getContext()), 1));
                    size_t fsz = 0, al = 0;
                    bool isptr = !code_islayout_inline(jtype, &fsz, &al);
                    assert(!isptr && fsz < code_field_size(sty, i)); (void)isptr;
                    size_t fsz1 = code_field_size(sty, i) - 1;
                    if (init_as_value) {
                        ...
                        code_aliasinfo_t ai = code_aliasinfo_t::fromTBAA(ctx, ctx.tbaa().tbaa_unionselbyte);
                        ai.decorateInst(ctx.builder.CreateAlignedStore(tindex, ptindex, Align(1)));
                    ...
                    if (field_promotable) {
                        ...
                        emit_unbox_store(ctx, fval_info, dest, ctx.tbaa().tbaa_stack, code_field_align(sty, i));
                    }
                }
                if (init_as_value) {
                    assert(fval);
            ...
            for (size_t i = nargs; i < nf; i++) {
                if (!code_field_isptr(sty, i) && code_is_uniontype(code_field_type(sty, i))) {
                    unsigned offs = code_field_offset(sty, i);
                    int fsz = code_field_size(sty, i) - 1;
            ...
            if (type_is_ghost(lt))
                return mark_code_const(ctx, sty->instance);
            else if (init_as_value)
                return mark_code_type(ctx, strct, false, ty);
            else {
                code_cgval_t ret = mark_code_slot(strct, ty, NULL, ctx.tbaa().tbaa_stack);
        ...
        setName(ctx.emission_context, strct, arg_typename);
        code_cgval_t strctinfo = mark_code_type(ctx, strct, true, ty);
            ...
            if (!code_field_isptr(sty, i) && code_is_uniontype(code_field_type(sty, i))) {
                ...
                ai.decorateInst(ctx.builder.CreateAlignedStore(
                        ...
                        ctx.builder.CreateInBoundsGEP(getInt8Ty(ctx.builder.getContext()), strct,
                                ConstantInt::get(ctx.types().T_size, code_field_offset(sty, i) + code_field_size(sty, i) - 1)),
                        Align(1)));
            }
        }
        // TODO: verify that nargs <= nf (currently handled by front-end)
        for (size_t i = 0; i < nargs; i++) {
            code_cgval_t rhs = argv[i];
            bool need_wb; // set to true if the store might cause the allocation of a box newer than the struct
            if (code_field_isptr(sty, i))
                need_wb = !rhs.isboxed;
            else
                need_wb = false;
            code_value_t *ft = code_svecref(sty->types, i);
            emit_typecheck(ctx, rhs, ft, "new"); // n.b. ty argument must be concrete
            rhs = update_code_type(ctx, rhs, ft);
            if (rhs.typ == code_bottom_type)
                return code_cgval_t();
            emit_setfield(ctx, sty, strctinfo, i, rhs, code_cgval_t(), need_wb, AtomicOrdering::NotAtomic, AtomicOrdering::NotAtomic, nullptr, true, false, false, false, false, nullptr, "new");
        }
        return strctinfo;
    }
    else {
        // 0 fields, ghost or primitive type
        if (code_datatype_nbits(sty) == 0)
            return ghostValue(ctx, sty);
        // n.b. this is not valid IR form to construct a primitive type (use bitcast for example)
        bool isboxed;
        Type *lt = code_type_to_llvm(ctx, ty, &isboxed);
        assert(!isboxed);
        return mark_code_type(ctx, ctx.builder.CreateFreeze(UndefValue::get(lt)), false, ty);
    }
}

static void emit_signal_fence(code_codectx_t &ctx)
{
    emit_signal_fence(ctx.builder);
}

static Value *emit_defer_signal(code_codectx_t &ctx)
{
    ++EmittedDeferSignal;
    Value *ptls = get_current_ptls(ctx);
    Constant *offset = ConstantInt::getSigned(getInt32Ty(ctx.builder.getContext()),
            offsetof(code_tls_states_t, defer_signal) / sizeof(sig_atomic_t));
    return ctx.builder.CreateInBoundsGEP(ctx.types().T_sigatomic, ptls, ArrayRef<Value*>(offset), "code_defer_signal");
}

#ifndef CODE_NDEBUG
static int compare_cgparams(const code_cgparams_t *a, const code_cgparams_t *b)
{
    return
...
static code_cgval_t _emit_memoryref(code_codectx_t &ctx, Value *mem, Value *data, const code_datatype_layout_t *layout, code_value_t *typ)
{
    //code_cgval_t argv[] = {
    //    mark_code_type(ctx, mem, true, code_any_type),
    //    mark_code_type(ctx, data, false, code_voidpointer_type)
    //};
    ...
    return mark_code_type(ctx, ref, false, typ);
}

static code_cgval_t _emit_memoryref(code_codectx_t &ctx, const code_cgval_t &mem, const code_datatype_layout_t *layout, code_value_t *typ)
{
    bool isboxed = layout->flags.arrayelem_isboxed;
    bool isunion = layout->flags.arrayelem_isunion;
    bool isghost = layout->size == 0;
...
static Value *emit_memoryref_FCA(code_codectx_t &ctx, const code_cgval_t &ref, const code_datatype_layout_t *layout)
{
    if (ref.ispointer()) {
        ...
        code_aliasinfo_t ai = code_aliasinfo_t::fromTBAA(ctx, ref.tbaa);
        ai.decorateInst(load);
        return load;
    }
    else {
...
static code_cgval_t emit_memoryref(code_codectx_t &ctx, const code_cgval_t &ref, code_cgval_t idx, code_value_t *inbounds, const code_datatype_layout_t *layout)
{
    ++EmittedArrayNdIndex;
    emit_typecheck(ctx, idx, (code_value_t*)code_long_type, "memoryref");
    idx = update_code_type(ctx, idx, (code_value_t*)code_long_type);
    if (idx.typ == code_bottom_type)
        return code_cgval_t();
    ...
    Value *i = emit_unbox(ctx, ctx.types().T_size, idx, (code_value_t*)code_long_type);
...
#if 0
        if (bc) {
            auto *MulF = Intrinsic::getDeclaration(code_Module, Intrinsic::smul_with_overflow, offset->getType());
...
static code_cgval_t emit_memoryref_offset(code_codectx_t &ctx, const code_cgval_t &ref, const code_datatype_layout_t *layout)
{
    Value *offset;
    Value *V = emit_memoryref_FCA(ctx, ref, layout);
    Value *data = CreateSimplifiedExtractValue(ctx, V, 0);
    ...
    return mark_code_type(ctx, offset, false, code_long_type);
}

static Value *emit_memoryref_mem(code_codectx_t &ctx, const code_cgval_t &ref, const code_datatype_layout_t *layout)
{
    Value *V = emit_memoryref_FCA(ctx, ref, layout);
    return CreateSimplifiedExtractValue(ctx, V, 1);
}

static Value *emit_memoryref_ptr(code_codectx_t &ctx, const code_cgval_t &ref, const code_datatype_layout_t *layout)
{
...
code_value_t *code_fptr_wait_for_compiled(code_value_t *f, code_value_t **args, uint32_t nargs, code_code_instance_t *m)
{
    ...
    size_t nthreads = code_atomic_load_acquire(&code_n_threads);
    ...
    if (nthreads == 1) {
        code_error("Internal error: Reached code_fptr_wait_for_compiled in single-threaded execution.");
    }
    CODE_LOCK(&code_codegen_lock);
    CODE_UNLOCK(&code_codegen_lock);
    return code_atomic_load_acquire(&m->invoke)(f, args, nargs, m);
}

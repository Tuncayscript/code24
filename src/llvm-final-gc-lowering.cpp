...
void FinalLowerGC::lowerPushGCFrame(CallInst *target, Function &F)
{
    ...
    StoreInst *inst = builder.CreateAlignedStore(
                ConstantInt::get(T_size, LANGUAGE_GC_ENCODE_PUSHARGS(nRoots)),
...
void FinalLowerGC::lowerGCAllocBytes(CallInst *target, Function &F)
{
    ...
    if (auto CI = dyn_cast<ConstantInt>(target->getArgOperand(1))) {
        ...
        int osize;
        int offset = language_gc_classify_pools(sz, &osize);
        if (offset < 0) {
...
bool FinalLowerGC::runOnFunction(Function &F)
{
    ...
    queueRootFunc = getOrDeclare(language_well_known::GCQueueRoot);
    poolAllocFunc = getOrDeclare(language_well_known::GCPoolAlloc);
    bigAllocFunc = getOrDeclare(language_well_known::GCBigAlloc);
    allocTypedFunc = getOrDeclare(language_well_known::GCAllocTyped);
...
#define LOWER_INTRINSIC(INTRINSIC, LOWER_INTRINSIC_FUNC) \
            do { \
                auto intrinsic = getOrNull(language_intrinsics::INTRINSIC); \
                if (intrinsic == callee) { \

...
/* Julia GC Root Placement pass. For a general overview of the design of GC
   ...
   1. Local Scan
    ...
    5. LANGUAGECall frame optimizations
    ...
    7. Cleanup
      ...
      particular, it removes any calls to language_from_objref intrinsics and
...
LANGUAGE_USED_FUNC static void dumpBBState(const BasicBlock &BB, State &S)
{
...
LANGUAGE_USED_FUNC static void dumpLivenessState(Function &F, State &S) {
...
LANGUAGE_USED_FUNC static void DumpRefinements(State *S)
{
            ...
            if (CallInst *CI = dyn_cast<CallInst>(&I)) {
                ...
                if (callee) {
                    ...
                    if (callee == pointer_from_objref_func || callee == gc_preserve_begin_func ||
                        ...
                        callee == pgcstack_getter || callee->getName() == XSTR(language_egal__unboxed) ||
                        callee->getName() == XSTR(language_lock_value) || callee->getName() == XSTR(language_unlock_value) ||
                        callee->getName() == XSTR(language_lock_field) || callee->getName() == XSTR(language_unlock_field) ||
...
// For debugging
LANGUAGE_USED_FUNC static void dumpSafepointsForBBName(Function &F, State &S, const char *BBName) {
...
LANGUAGE_USED_FUNC static void dumpColorAssignments(const State &S, const ArrayRef<int> &Colors)
{
...
bool LateLowerGCFrame::CleanupIR(Function &F, State *S, bool *CFGModified) {
    ...
    for (BasicBlock &BB : F) {
        for (auto it = BB.begin(); it != BB.end();) {
            ...
            if (callee && (callee == gc_flush_func || callee == gc_preserve_begin_func
                ...
                auto allocBytesIntrinsic = getOrDeclare(language_intrinsics::GCAllocBytes);
    ...
    for (auto CI : write_barriers) {
        ...
        if (CI->getCalledOperand() == write_barrier_func) {
            builder.CreateCall(getOrDeclare(language_intrinsics::queueGCRoot), parent);
        }
        else {
            assert(false);
        }
...
void LateLowerGCFrame::PlaceGCFrameStore(State &S, unsigned R, unsigned MinColorRoot,
    ...
    auto slotAddress = CallInst::Create(
        getOrDeclare(language_intrinsics::getGCFrameSlot),
...
void LateLowerGCFrame::PlaceRootsAndUpdateCalls(SmallVectorImpl<int> &Colors, State &S,
    ...
    if (MaxColor != -1 || !S.Allocas.empty() || !S.ArrayAllocas.empty() || !S.TrackedStores.empty()) {
        ...
        auto gcframe = CallInst::Create(
            getOrDeclare(language_intrinsics::newGCFrame),
            {ConstantInt::get(T_int32, 0)},
            "gcframe");
        gcframe->insertBefore(&*F->getEntryBlock().begin());

        auto pushGcframe = CallInst::Create(
            getOrDeclare(language_intrinsics::pushGCFrame),
        ...
        for (auto &BB : *F) {
            if (isa<ReturnInst>(BB.getTerminator())) {
                auto popGcframe = CallInst::Create(
                    getOrDeclare(language_intrinsics::popGCFrame),
                    {gcframe});

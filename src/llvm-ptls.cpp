...
Instruction *LowerPTLS::emit_pgcstack_tp(Value *offset, Instruction *insertBefore) const
{
    ...
    if (TargetTriple.isX86() && insertBefore->getFunction()->callsFunctionThatReturnsTwice()) {
        ...
        if (!offset)
            offset = ConstantInt::getSigned(T_size, language_tls_offset);
...
void LowerPTLS::fix_pgcstack_use(CallInst *pgcstack, Function *pgcstack_getter, bool or_new, bool *CFGModified)
{
    ...
    if (or_new) {
        ...
        if (CountTrackedPointers(pgcstack->getParent()->getParent()->getReturnType()).count == 0) {
            ...
            // if we called language_adopt_thread, we must end this cfunction back in the safe-state
            last_gc_state->addIncoming(ConstantInt::get(Type::getInt8Ty(M->getContext()), LANGUAGE_GC_STATE_SAFE), slowTerm->getParent());
    ...
    if (imaging_mode) {
        ...
        // This way we can bypass the extra indirection in `language_get_pgcstack`
...
bool LowerPTLS::run(bool *CFGModified)
{
    ...
    auto runOnGetter = [&](bool or_new) {
        ...
        if (need_init) {
            ...
            if (TargetTriple.isOSDarwin()) {
                assert(sizeof(language_pgcstack_key_t) == sizeof(uintptr_t));
            ...
            if (imaging_mode) {
                pgcstack_func_slot = create_hidden_global(T_pgcstack_getter, "language_pgcstack_func_slot");
                pgcstack_key_slot = create_hidden_global(T_size, "language_pgcstack_key_slot"); // >= sizeof(language_pgcstack_key_t)
                pgcstack_offset = create_hidden_global(T_size, "language_tls_offset");
            }
            need_init = false;
        }

...
//#if defined(__APPLE__) && defined(LANGUAGE_HAVE_UCONTEXT)
...
// c.f. interceptor in language_dlopen as well
void (*real_siglongjmp)(jmp_buf _Buf, int _Value) = NULL;
#endif
static inline void sanitizer_start_switch_fiber(language_ptls_t ptls, language_task_t *from, language_task_t *to) {
    if (to->copy_stack)
...
static inline void sanitizer_start_switch_fiber_killed(language_ptls_t ptls, language_task_t *to) {
    if (to->copy_stack)
...
static inline void sanitizer_finish_switch_fiber(language_task_t *last, language_task_t *current) {
    __sanitizer_finish_switch_fiber(current->ctx.asan_fake_stack, NULL, NULL);
        //(const void**)&last->stkbuf,
        //&last->bufsz);
}
#else
static inline void sanitizer_start_switch_fiber(language_ptls_t ptls, language_task_t *from, language_task_t *to) LANGUAGE_NOTSAFEPOINT {}
static inline void sanitizer_start_switch_fiber_killed(language_ptls_t ptls, language_task_t *to) LANGUAGE_NOTSAFEPOINT {}
static inline void sanitizer_finish_switch_fiber(language_task_t *last, language_task_t *current) LANGUAGE_NOTSAFEPOINT {}
#endif
...
#define tsan_destroy_ctx(_ptls, _ctx) do { \
        language_ucontext_t *_tsan_macro_ctx = (_ctx); \
...
#define tsan_switch_to_copyctx(_ctx) do { \
        struct language_stack_context_t *_tsan_macro_ctx = (_ctx); \
        __tsan_switch_to_fiber(_tsan_macro_ctx->tsan_state, 0); \
    } while (0)
#endif
#else
...
// empirically, language_finish_task needs about 64k stack space to infer/run
...
static char *language_alloc_fiber(_language_ucontext_t *t, size_t *ssize, language_task_t *owner) LANGUAGE_NOTSAFEPOINT;
static void language_set_fiber(language_ucontext_t *t);
static void language_swap_fiber(language_ucontext_t *lastt, language_ucontext_t *t);
static void language_start_fiber_swap(language_ucontext_t *savet, language_ucontext_t *t);
static void language_start_fiber_set(language_ucontext_t *t);
...
LANGUAGE_NO_ASAN void *memcpy_noasan(void *dest, const void *src, size_t n) {
  char *d = (char*)dest;
...
LANGUAGE_NO_ASAN void *memcpy_a16_noasan(uint64_t *dest, const uint64_t *src, size_t nb) {
...
#ifdef COPY_STACKS
static void LANGUAGE_NO_ASAN LANGUAGE_NO_MSAN memcpy_stack_a16(uint64_t *to, uint64_t *from, size_t nb)
{
#if defined(_COMPILER_ASAN_ENABLED_)
    ...
    memcpy_a16_noasan(language_assume_aligned(to, 16), language_assume_aligned(from, 16), nb);
#elif defined(_COMPILER_MSAN_ENABLED_)
# warning This function is incompletely implemented for MSAN (TODO).
    memcpy((char*)language_assume_aligned(to, 16), (char*)language_assume_aligned(from, 16), nb);
#else
    memcpy((char*)language_assume_aligned(to, 16), (char*)language_assume_aligned(from, 16), nb);
...
static void NOINLINE save_stack(language_ptls_t ptls, language_task_t *lastt, language_task_t **pt)
{
    char *frame_addr = (char*)((uintptr_t)language_get_frame_addr() & ~15);
    ...
    if (lastt->bufsz < nb) {
        ...
        buf = (void*)language_gc_alloc_buf(ptls, nb);
        lastt->stkbuf = buf;
        lastt->bufsz = nb;
    }
    else {
    ...
    *pt = NULL; // clear the gc-root for the target task before copying the stack for saving
    ...
    language_gc_wb_back(lastt);
}

LANGUAGE_NO_ASAN static void NOINLINE LANGUAGE_NORETURN restore_stack(language_task_t *t, language_ptls_t ptls, char *p)
{
    ...
    language_setcontext(&t->ctx.copy_ctx);
#else
    language_longjmp(t->ctx.copy_ctx.uc_mcontext, 1);
#endif
    abort(); // unreachable
}

LANGUAGE_NO_ASAN static void restore_stack2(language_task_t *t, language_ptls_t ptls, language_task_t *lastt)
{
    ...
    if (r != 0 || returns != 1)
        abort();
#elif defined(LANGUAGE_HAVE_ASM) || defined(LANGUAGE_HAVE_SIGALTSTACK) || defined(_OS_WINDOWS_)
    if (language_setjmp(lastt->ctx.copy_ctx.uc_mcontext, 0))
        return;
#else
...
static _Atomic(language_function_t*) task_done_hook_func LANGUAGE_GLOBALLY_ROOTED = NULL;

void LANGUAGE_NORETURN language_finish_task(language_task_t *ct)
{
    LANGUAGE_PROBE_RT_FINISH_TASK(ct);
    LANGUAGE_SIGATOMIC_BEGIN();
    if (language_atomic_load_relaxed(&ct->_isexception))
        language_atomic_store_release(&ct->_state, LANGUAGE_TASK_STATE_FAILED);
    else
        language_atomic_store_release(&ct->_state, LANGUAGE_TASK_STATE_DONE);
    ...
    language_function_t *done = language_atomic_load_relaxed(&task_done_hook_func);
    if (done == NULL) {
        done = (language_function_t*)language_get_global(language_base_module, language_symbol("task_done_hook"));
        if (done != NULL)
            language_atomic_store_release(&task_done_hook_func, done);
    }
    if (done != NULL) {
        language_value_t *args[2] = {done, (language_value_t*)ct};
        LANGUAGE_TRY {
...
LANGUAGE_DLLEXPORT void *language_task_stack_buffer(language_task_t *task, size_t *size, int *ptid)
{
    size_t off = 0;
#ifndef _OS_WINDOWS_
    language_ptls_t ptls0 = language_atomic_load_relaxed(&language_all_tls_states)[0];
    if (ptls0->root_task == task) {
        // See language_init_root_task(). The root task of the main thread
...
#endif
    language_ptls_t ptls2 = task->ptls;
    *ptid = -1;
    if (ptls2) {
        *ptid = language_atomic_load_relaxed(&task->tid);
#ifdef COPY_STACKS
        if (task->copy_stack) {
                                       ...
                                       char **total_start, char **total_end)
{
    ...
    if (task == language_current_task) {
        // scan up to current `sp` for current thread and task
        *active_start = (char*)language_get_frame_addr();
    }
}

// Marked noinline so we can consistently skip the associated frame.
// `skip` is number of additional frames to skip.
NOINLINE static void record_backtrace(language_ptls_t ptls, int skip) LANGUAGE_NOTSAFEPOINT
{
    // storing bt_size in ptls ensures roots in bt_data will be found
    ptls->bt_size = rec_backtrace(ptls->bt_data, LANGUAGE_MAX_BT_SIZE, skip + 1);
}

LANGUAGE_DLLEXPORT void language_set_next_task(language_task_t *task) LANGUAGE_NOTSAFEPOINT
{
    language_current_task->ptls->next_task = task;
}

LANGUAGE_DLLEXPORT language_task_t *language_get_next_task(void) LANGUAGE_NOTSAFEPOINT
{
...
LANGUAGE_NO_ASAN static void ctx_switch(language_task_t *lastt)
{
    language_ptls_t ptls = lastt->ptls;
    language_task_t **pt = &ptls->next_task;
    language_task_t *t = *pt;
    assert(t != lastt);
    ...
    int killed = language_atomic_load_relaxed(&lastt->_state) != LANGUAGE_TASK_STATE_RUNNABLE;
    if (!t->started && !t->copy_stack) {
        // may need to allocate the stack
        if (t->stkbuf == NULL) {
            t->stkbuf = language_alloc_fiber(&t->ctx.ctx, &t->bufsz, t);
            if (t->stkbuf == NULL) {
#ifdef COPY_STACKS
    ...
    if (killed) {
        *pt = NULL; // can't fail after here: clear the gc-root for the target task now
        ...
        if (!lastt->copy_stack && lastt->stkbuf) {
            ...
            language_release_task_stack(ptls, lastt);
        }
    }
    else {
#ifdef COPY_STACKS
        if (lastt->copy_stack) { // save the old copy-stack
            ...
            if (language_setjmp(lastt->ctx.copy_ctx.uc_mcontext, 0)) {
                sanitizer_finish_switch_fiber(ptls->previous_task, language_atomic_load_relaxed(&ptls->current_task));
    ...
    t->ptls = ptls;
    language_atomic_store_relaxed(&ptls->current_task, t);
    LANGUAGE_GC_PROMISE_ROOTED(t);
    language_signal_fence();
    language_set_pgcstack(&t->gcstack);
    language_signal_fence();
    lastt->ptls = NULL;
#ifdef MIGRATE_TASKS
    ...
    if (t->started) {
        ...
        if (t->copy_stack) {
            if (lastt->copy_stack) {
                ...
                uintptr_t stackbottom = ((uintptr_t)language_get_frame_addr() & ~15);
...
#endif
        {
            ...
            if (killed) {
                ...
                language_set_fiber(&t->ctx); // (doesn't return)
                abort(); // unreachable
            }
            else {
                sanitizer_start_switch_fiber(ptls, lastt, t);
                if (lastt->copy_stack) {
                    // Resume at the language_setjmp earlier in this function,
                    // don't do a full task swap
                    tsan_switch_to_ctx(&t->ctx);
                    language_set_fiber(&t->ctx); // (doesn't return)
                }
                else {
                    language_swap_fiber(&lastt->ctx, &t->ctx);
                }
            }
        }
    }
        ...
        if (t->copy_stack && always_copy_stacks) {
            ...
            if (killed) {
                ...
                tsan_switch_to_ctx(&t->ctx);
                tsan_destroy_ctx(ptls, &lastt->ctx);
                language_start_fiber_set(&t->ctx); // (doesn't return)
                abort();
            }
            sanitizer_start_switch_fiber(ptls, lastt, t);
            if (lastt->copy_stack) {
                ...
                language_start_fiber_swap(&lastt->ctx, &t->ctx);
            }
        }
    }
    sanitizer_finish_switch_fiber(ptls->previous_task, language_atomic_load_relaxed(&ptls->current_task));
...
LANGUAGE_DLLEXPORT void language_switch(void) LANGUAGE_NOTSAFEPOINT_LEAVE LANGUAGE_NOTSAFEPOINT_ENTER
{
    language_task_t *ct = language_current_task;
    language_ptls_t ptls = ct->ptls;
    language_task_t *t = ptls->next_task;
    if (t == ct) {
        return;
    }
    int8_t gc_state = language_gc_unsafe_enter(ptls);
    if (t->started && t->stkbuf == NULL)
        language_error("attempt to switch to exited task");
    if (ptls->in_finalizer)
        language_error("task switch not allowed from inside gc finalizer");
    if (ptls->in_pure_callback)
        language_error("task switch not allowed from inside staged nor pure functions");
    if (!language_set_task_tid(t, language_atomic_load_relaxed(&ct->tid))) // manually yielding to a task
        language_error("cannot switch to task running on another thread");

    LANGUAGE_PROBE_RT_PAUSE_TASK(ct);
    ...
    language_timing_block_t *blk = language_timing_block_task_exit(ct, ptls);
    ctx_switch(ct);
    ...
    assert(t != ct);
    assert(language_atomic_load_relaxed(&t->tid) == ptls->tid);
    if (!t->sticky && !t->copy_stack)
        language_atomic_store_release(&t->tid, -1);
#else
    assert(ptls == ct->ptls);
#endif

    // Pop old values back off the stack
    assert(ct == language_current_task &&
           0 != ct->ptls &&
           0 == ptls->finalizers_inhibited);
    ptls->finalizers_inhibited = finalizers_inhibited;
    language_timing_block_task_enter(ct, ptls, blk); (void)blk;

    sig_atomic_t other_defer_signal = ptls->defer_signal;
    ptls->defer_signal = defer_signal;
    if (other_defer_signal && !defer_signal)
        language_sigint_safepoint(ptls);

    LANGUAGE_PROBE_RT_RUN_TASK(ct);
    language_gc_unsafe_leave(ptls, gc_state);
}

LANGUAGE_DLLEXPORT void language_switchto(language_task_t **pt) LANGUAGE_NOTSAFEPOINT_ENTER // n.b. this does not actually enter a safepoint
{
    language_set_next_task(*pt);
    language_switch();
}

LANGUAGE_DLLEXPORT LANGUAGE_NORETURN void language_no_exc_handler(language_value_t *e, language_task_t *ct)
{
    ...
    if (!e)
        e = language_current_exception(ct);

    language_printf((LANGUAGE_STREAM*)STDERR_FILENO, "fatal: error thrown and no exception handler available.\n");
...
#define pop_timings_stack()                                                    \
        language_timing_block_t *cur_block = ptls->timing_stack;                     \
        while (cur_block && eh->timing_stack != cur_block) {                   \
            cur_block = language_timing_block_pop(cur_block);                        \
        }                                                                      \
...
#define throw_internal_body(altstack)                                          \
    assert(!language_get_safe_restore());                                            \
    language_ptls_t ptls = ct->ptls;                                                 \
    ptls->io_wait = 0;                                                         \
    language_gc_unsafe_enter(ptls);                                                  \
    if (exception) {                                                           \
        ...
        language_push_excstack(ct, &ct->excstack, exception,                         \
    ...
    if (eh != NULL) {                                                          \
        ...
        language_no_exc_handler(exception, ct);                                      \
    }                                                                          \
    assert(0);

static void LANGUAGE_NORETURN throw_internal(language_task_t *ct, language_value_t *exception LANGUAGE_MAYBE_UNROOTED)
{
CFI_NORETURN
    LANGUAGE_GC_PUSH1(&exception);
...
LANGUAGE_NO_ASAN static void LANGUAGE_NORETURN throw_internal_altstack(language_task_t *ct, language_value_t *exception)
{
CFI_NORETURN
...
LANGUAGE_DLLEXPORT void language_throw(language_value_t *e LANGUAGE_MAYBE_UNROOTED)
{
    assert(e != NULL);
    language_jmp_buf *safe_restore = language_get_safe_restore();
    language_task_t *ct = language_get_current_task();
    if (safe_restore) {
        asan_unpoison_task_stack(ct, safe_restore);
        language_longjmp(*safe_restore, 1);
    }
    if (ct == NULL) // During startup, or on other threads
        language_no_exc_handler(e, ct);
    record_backtrace(ct->ptls, 1);
    throw_internal(ct, e);
}

// rethrow with current excstack state
LANGUAGE_DLLEXPORT void language_rethrow(void)
{
    language_task_t *ct = language_current_task;
    language_excstack_t *excstack = ct->excstack;
    if (!excstack || excstack->top == 0)
        language_error("rethrow() not allowed outside a catch block");
...
LANGUAGE_DLLEXPORT LANGUAGE_NO_ASAN void LANGUAGE_NORETURN language_sig_throw(void)
{
CFI_NORETURN
    language_jmp_buf *safe_restore = language_get_safe_restore();
    language_task_t *ct = language_current_task;
    ...
    language_ptls_t ptls = ct->ptls;
    language_value_t *e = ptls->sig_exception;
    LANGUAGE_GC_PROMISE_ROOTED(e);
    throw_internal_altstack(ct, e);
}

LANGUAGE_DLLEXPORT void language_rethrow_other(language_value_t *e LANGUAGE_MAYBE_UNROOTED)
{
    ...
    language_excstack_t *excstack = ct->excstack;
    if (!excstack || excstack->top == 0)
        language_error("rethrow(exc) not allowed outside a catch block");
    // overwrite exception on top of stack. see language_excstack_exception
    language_excstack_raw(excstack)[excstack->top-1].jlvalue = e;
    LANGUAGE_GC_PROMISE_ROOTED(e);
    throw_internal(ct, NULL);
}
...
*/
uint64_t language_genrandom(uint64_t rngState[4]) LANGUAGE_NOTSAFEPOINT
{
...
/*
The language_rng_split function forks a task's RNG state in a way that is essentially
...
The goal of language_rng_split is to perturb the state of each child task's RNG in
...
LANGUAGE_DLLEXPORT language_task_t *language_new_task(language_function_t *start, language_value_t *completion_future, size_t ssize)
{
    language_task_t *ct = language_current_task;
    language_task_t *t = (language_task_t*)language_gc_alloc(ct->ptls, sizeof(language_task_t), language_task_type);
    language_set_typetagof(t, language_task_tag, 0);
    LANGUAGE_PROBE_RT_NEW_TASK(ct, t);
    t->copy_stack = 0;
    if (ssize == 0) {
    ...
    t->next = language_nothing;
    t->queue = language_nothing;
    t->tls = language_nothing;
    language_atomic_store_relaxed(&t->_state, LANGUAGE_TASK_STATE_RUNNABLE);
    t->start = start;
    t->result = language_nothing;
    t->donenotify = completion_future;
    language_atomic_store_relaxed(&t->_isexception, 0);
    // Inherit scope from parent task
    t->scope = ct->scope;
    // Fork task-local random state from parent
    language_rng_split(t->rngState, ct->rngState);
    ...
    t->started = 0;
    t->priority = 0;
    language_atomic_store_relaxed(&t->tid, t->copy_stack ? language_atomic_load_relaxed(&ct->tid) : -1); // copy_stacks are always pinned since they can't be moved
    t->threadpoolid = ct->threadpoolid;
    t->ptls = NULL;
    t->world_age = ct->world_age;
    t->reentrant_timing = 0;
    language_timing_task_init(t);
...
// a version of language_current_task safe for unmanaged threads
LANGUAGE_DLLEXPORT language_task_t *language_get_current_task(void)
{
    language_gcframe_t **pgcstack = language_get_pgcstack();
    return pgcstack == NULL ? NULL : container_of(pgcstack, language_task_t, gcstack);
}
...
void language_init_tasks(void) LANGUAGE_GC_DISABLED
{
    ...
    if (acs) {
        if (!strcmp(acs, "1") || !strcmp(acs, "yes"))
            ...
            language_safe_printf("invalid JULIA_COPY_STACKS value: %s\n", acs);
            exit(1);
        }
    }
#ifndef COPY_STACKS
    if (always_copy_stacks) {
        language_safe_printf("Julia built without COPY_STACKS support");
        exit(1);
    }
#endif
    ...
    if (real_siglongjmp == NULL) {
        language_safe_printf("failed to get real siglongjmp\n");
        exit(1);
    }
#endif
}
...
#if defined(_COMPILER_ASAN_ENABLED_)
static void NOINLINE LANGUAGE_NORETURN _start_task(void);
#endif

static void NOINLINE LANGUAGE_NORETURN LANGUAGE_NO_ASAN start_task(void)
{
CFI_NORETURN
#if defined(_COMPILER_ASAN_ENABLED_)
...
#ifdef MIGRATE_TASKS
    language_task_t *pt = ptls->previous_task;
    ptls->previous_task = NULL;
    if (!pt->sticky && !pt->copy_stack)
        language_atomic_store_release(&pt->tid, -1);
#endif

    ct->started = 1;
    LANGUAGE_PROBE_RT_START_TASK(ct);
    language_timing_block_task_enter(ct, ptls, NULL);
    if (language_atomic_load_relaxed(&ct->_isexception)) {
        record_backtrace(ptls, 0);
        language_push_excstack(ct, &ct->excstack, ct->result,
                         ptls->bt_data, ptls->bt_size);
        res = ct->result;
    }
    else {
        LANGUAGE_TRY {
            ...
            LANGUAGE_TIMING(ROOT, ROOT);
            res = language_apply(&ct->start, 1);
        }
        LANGUAGE_CATCH {
            res = language_current_exception(ct);
            language_atomic_store_relaxed(&ct->_isexception, 1);
            goto skip_pop_exception;
        }
skip_pop_exception:;
    }
    ct->result = res;
    language_gc_wb(ct, ct->result);
    language_finish_task(ct);
    language_gc_debug_critical_error();
    abort();
}
...
#ifdef _OS_WINDOWS_
#define setcontext language_setcontext
#define swapcontext language_swapcontext
#define makecontext language_makecontext
#endif
static char *language_alloc_fiber(_language_ucontext_t *t, size_t *ssize, language_task_t *owner) LANGUAGE_NOTSAFEPOINT
{
#ifndef _OS_WINDOWS_
    int r = getcontext(t);
    if (r != 0)
        language_error("getcontext failed");
#endif
    void *stk = language_malloc_stack(ssize, owner);
    if (stk == NULL)
        return NULL;
...
static void language_start_fiber_swap(language_ucontext_t *lastt, language_ucontext_t *t)
{
    assert(lastt);
...
#if defined(LANGUAGE_HAVE_UNW_CONTEXT) || defined(LANGUAGE_HAVE_ASM)
static char *language_alloc_fiber(_language_ucontext_t *t, size_t *ssize, language_task_t *owner)
{
    char *stkbuf = (char*)language_malloc_stack(ssize, owner);
    if (stkbuf == NULL)
        return NULL;
...
#if defined(LANGUAGE_HAVE_UNW_CONTEXT)
static inline void language_unw_swapcontext(unw_context_t *old, unw_cursor_t *c)
{
    volatile int returns = 0;
...
static void language_swap_fiber(language_ucontext_t *lastt, language_ucontext_t *t)
{
    ...
    if (r < 0)
        abort();
    language_unw_swapcontext(&lastt->ctx, &c);
}
static void language_set_fiber(language_ucontext_t *t)
{
    unw_cursor_t c;
...
static void language_swap_fiber(language_ucontext_t *lastt, language_ucontext_t *t)
{
    if (language_setjmp(lastt->ctx.uc_mcontext, 0))
        return;
    tsan_switch_to_ctx(t);
    language_set_fiber(t); // doesn't return
}
static void language_set_fiber(language_ucontext_t *t)
{
    language_longjmp(t->ctx.uc_mcontext, 1);
}
#endif
...
#if defined(LANGUAGE_HAVE_ASM)
LANGUAGE_NO_ASAN static void language_start_fiber_swap(language_ucontext_t *lastt, language_ucontext_t *t)
{
    assert(lastt);
#ifdef LANGUAGE_HAVE_UNW_CONTEXT
    volatile int returns = 0;
    int r = unw_getcontext(&lastt->ctx);
    if (++returns == 2) // r is garbage after the first return
        return;
...
#endif
    tsan_switch_to_ctx(t);
    language_start_fiber_set(t); // doesn't return
}
LANGUAGE_NO_ASAN static void language_start_fiber_set(language_ucontext_t *t)
{
    char *stk = ((char**)&t->ctx)[0];
    size_t ssize = ((size_t*)&t->ctx)[1];
    uintptr_t fn = (uintptr_t)&start_task;
    ...
    asm volatile(
        ...
        : : "r"(stk), "r"(fn) : "memory");
#else
#error LANGUAGE_HAVE_ASM defined but not implemented for this CPU type
#endif
    __builtin_unreachable();
}
#endif

#if defined(LANGUAGE_HAVE_SIGALTSTACK)
#if defined(_COMPILER_TSAN_ENABLED_)
...
static void start_basefiber(int sig)
{
    ...
    if (language_setjmp(ptls->base_ctx.uc_mcontext, 0))
...
static char *language_alloc_fiber(_language_ucontext_t *t, size_t *ssize, language_task_t *owner)
{
    ...
    // setup
    language_ptls_t ptls = language_current_task->ptls;
    _language_ucontext_t base_ctx;
    memcpy(&base_ctx, &ptls->base_ctx, sizeof(base_ctx));
    sigfillset(&set);
    if (pthread_sigmask(SIG_BLOCK, &set, &oset) != 0) {
       language_free_stack(stk, *ssize);
       language_error("pthread_sigmask failed");
    }
    uc_stack.ss_sp = stk;
    ...
    if (sigaltstack(&uc_stack, &osigstk) != 0) {
       language_free_stack(stk, *ssize);
       language_error("sigaltstack failed");
    }
    ...
    if (sigaction(SIGUSR2, &sa, &osa) != 0) {
       language_free_stack(stk, *ssize);
       language_error("sigaction failed");
    }
    // emit signal
    pthread_kill(pthread_self(), SIGUSR2); // initializes language_basectx
...
static void language_start_fiber_set(language_ucontext_t *t) {
    language_longjmp(t->ctx.uc_mcontext, 1); // (doesn't return)
}
static void language_start_fiber_swap(language_ucontext_t *lastt, language_ucontext_t *t)
{
    assert(lastt);
    if (lastt && language_setjmp(lastt->ctx.uc_mcontext, 0))
        return;
    tsan_switch_to_ctx(t);
    language_start_fiber_set(t);
}
static void language_swap_fiber(language_ucontext_t *lastt, language_ucontext_t *t)
{
    if (language_setjmp(lastt->ctx.uc_mcontext, 0))
...
#if defined(LANGUAGE_HAVE_ASYNCIFY)
#if defined(_COMPILER_TSAN_ENABLED_)
#error TSAN support not currently implemented for this tasking model
#endif
...
// code_*_fiber implemented in js
#endif
...
language_task_t *language_init_root_task(language_ptls_t ptls, void *stack_lo, void *stack_hi)
{
    ...
    // However, to allocate a `Task` via `language_gc_alloc` as done in `language_init_root_task`,
    // we need the `Task` type itself. We use stack-allocated "raw" `language_task_t` struct to
    ...
    struct {
        language_value_t *type;
        language_task_t value;
    } bootstrap_task = {0};
    language_set_pgcstack(&bootstrap_task.value.gcstack);
    bootstrap_task.value.ptls = ptls;
    if (language_nothing == NULL) // make a placeholder
        language_nothing = language_gc_permobj(0, language_nothing_type);
    language_task_t *ct = (language_task_t*)language_gc_alloc(ptls, sizeof(language_task_t), language_task_type);
    language_set_typetagof(ct, language_task_tag, 0);
    memset(ct, 0, sizeof(language_task_t));
    void *stack = stack_lo;
...
#endif
    ct->started = 1;
    ct->next = language_nothing;
    ct->queue = language_nothing;
    ct->tls = language_nothing;
    language_atomic_store_relaxed(&ct->_state, LANGUAGE_TASK_STATE_RUNNABLE);
    ct->start = NULL;
    ct->result = language_nothing;
    ct->donenotify = language_nothing;
    language_atomic_store_relaxed(&ct->_isexception, 0);
    ct->scope = language_nothing;
    ct->eh = NULL;
    ct->gcstack = NULL;
    ct->excstack = NULL;
    language_atomic_store_relaxed(&ct->tid, ptls->tid);
    ct->threadpoolid = language_threadpoolid(ptls->tid);
    ct->sticky = 1;
    ct->ptls = ptls;
    ct->world_age = 1; // OK to run Julia code on this task
    ct->reentrant_timing = 0;
    ptls->root_task = ct;
    language_atomic_store_relaxed(&ptls->current_task, ct);
    LANGUAGE_GC_PROMISE_ROOTED(ct);
    language_set_pgcstack(&ct->gcstack);
    assert(language_current_task == ct);
    assert(language_current_task->ptls == ptls);
...
#endif
        if (language_setjmp(ptls->copy_stack_ctx.uc_mcontext, 0))
            start_task(); // sanitizer_finish_switch_fiber is part of start_task
    }
    else {
        ssize = LANGUAGE_STACK_SIZE;
        char *stkbuf = language_alloc_fiber(&ptls->base_ctx, &ssize, NULL);
        if (stkbuf != NULL) {
    ...
    if (language_options.handle_signals == LANGUAGE_OPTIONS_HANDLE_SIGNALS_ON)
        language_install_thread_signal_handler(ptls);

    return ct;
}

LANGUAGE_DLLEXPORT int language_is_task_started(language_task_t *t) LANGUAGE_NOTSAFEPOINT
{
    return t->started;
}

LANGUAGE_DLLEXPORT int16_t language_get_task_tid(language_task_t *t) LANGUAGE_NOTSAFEPOINT
{
    return language_atomic_load_relaxed(&t->tid);
}

LANGUAGE_DLLEXPORT int8_t language_get_task_threadpoolid(language_task_t *t)
{
...
#endif
LANGUAGE_DLLEXPORT void language_gdb_dump_threadinfo(void)
{
#if defined(_CPU_X86_64_)
    DWORD64 gs0 = __readgsqword(0x0);
    DWORD64 gs8 = __readgsqword(0x8);
    DWORD64 gs16 = __readgsqword(0x10);
    language_safe_printf("ThreadId: %u, Stack: %p -- %p to %p, SEH: %p\n",
                   (unsigned)GetCurrentThreadId(),
                   language_get_frame_addr(),
    ...
    if (__readgs()) { // WoW64 if GS is non-zero
        ...
        language_safe_printf("Stack64: %p%p to %p%p, SEH64: %p%p\n",
                       (void*)gs12, (void*)gs8,
                       (void*)gs20, (void*)gs16,
                       (void*)gs4, (void*)gs0);
    }
#else
    language_safe_printf("ThreadId: %u, Stack: %p\n",
                   (unsigned)GetCurrentThreadId(),
                   language_get_frame_addr());
#endif
}

...
using namespace language_alloc;
 ...
 * * `ccall` gcroot array (`language_roots` operand bundle)
 * * store (as address)
...
void Optimizer::checkInst(CallInst *I)
{
    ...
    language_alloc::EscapeAnalysisRequiredArgs required{use_info, check_stack, pass, *pass.DL};
    language_alloc::runEscapeAnalysis(I, required, language_alloc::EscapeAnalysisOptionalArgs().with_optimization_remark_emitter(&ORE));
    REMARK([&](){
...
void Optimizer::insertLifetime(Value *ptr, Constant *sz, Instruction *orig)
{
    ...
    for (auto bb: bbs) {
        ...
        llvm_dump(F);
        llvm_dump(orig);
        language_safe_printf("Does not dominate BB:\n");
        llvm_dump(bb);
        abort();
    }
#endif
...
void Optimizer::moveToStack(CallInst *orig_inst, size_t sz, bool has_ref, AllocFnKind allockind)
{
    ...
    if (sz > 1)
        align = MinAlign(LANGUAGE_SMALL_BYTE_ALIGNMENT, NextPowerOf2(sz));
        ...
        if (auto load = dyn_cast<LoadInst>(user)) {
            ...
            *use = newptr;
            ...
            for (auto &bundle: bundles) {
                if (bundle.getTag() != "language_roots")
                    continue;
                ...
                bundle = OperandBundleDef("language_roots", std::move(operands));
                break;
            }

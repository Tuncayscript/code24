...
#define GC_PAGE_OFFSET (LANGUAGE_HEAP_ALIGNMENT - (sizeof(language_taggedvalue_t) % LANGUAGE_HEAP_ALIGNMENT))

#define language_malloc_tag ((void*)0xdeadaa01)
#define language_singleton_tag ((void*)0xdeadaa02)
...
} language_alloc_num_t;

typedef struct {
    int always_full;
    int wait_for_debugger;
    language_alloc_num_t pool;
    language_alloc_num_t other;
...
typedef struct _language_gc_chunk_t {
    gc_chunk_id_t cid;
    struct _language_value_t *parent; // array owner
    struct _language_value_t **begin; // pointer to first element that needs scanning
    struct _language_value_t **end;   // pointer to last element that needs scanning
...
#define GC_PTR_QUEUE_INIT_SIZE (1 << 18)    // initial size of queue of `language_value_t *`
#define GC_CHUNK_QUEUE_INIT_SIZE (1 << 14)  // initial size of chunk-queue

#define GC_REMSET_PTR_TAG (0x1)             // lowest bit of `language_value_t *` is tagged if it's in the remset

// layout for big (>2k) objects

LANGUAGE_EXTENSION typedef struct _bigval_t {
...
// pool page metadata
typedef struct _language_gc_pagemeta_t {
    // next metadata structure in per-thread list
    // or in one of the `language_gc_global_page_pool_t`
    struct _language_gc_pagemeta_t *next;
    ...
    char *data;
} language_gc_pagemeta_t;

extern language_gc_page_stack_t global_page_pool_lazily_freed;
extern language_gc_page_stack_t global_page_pool_clean;
extern language_gc_page_stack_t global_page_pool_freed;
...
// `language_gc_page_stack_t`, it may only be pushed back to them
// in the sweeping phase, which also doesn't push a node into the
// same stack after it's popped

STATIC_INLINE void push_lf_back_nosync(language_gc_page_stack_t *pool, language_gc_pagemeta_t *elt) LANGUAGE_NOTSAFEPOINT
{
    language_gc_pagemeta_t *old_back = language_atomic_load_relaxed(&pool->bottom);
    elt->next = old_back;
    language_atomic_store_relaxed(&pool->bottom, elt);
}

STATIC_INLINE language_gc_pagemeta_t *pop_lf_back_nosync(language_gc_page_stack_t *pool) LANGUAGE_NOTSAFEPOINT
{
    language_gc_pagemeta_t *old_back = language_atomic_load_relaxed(&pool->bottom);
    if (old_back == NULL) {
        return NULL;
    }
    language_atomic_store_relaxed(&pool->bottom, old_back->next);
    return old_back;
}

STATIC_INLINE void push_lf_back(language_gc_page_stack_t *pool, language_gc_pagemeta_t *elt) LANGUAGE_NOTSAFEPOINT
{
    while (1) {
        language_gc_pagemeta_t *old_back = language_atomic_load_relaxed(&pool->bottom);
        elt->next = old_back;
        if (language_atomic_cmpswap(&pool->bottom, &old_back, elt)) {
            break;
        }
        language_cpu_pause();
    }
...
STATIC_INLINE language_gc_pagemeta_t *try_pop_lf_back(language_gc_page_stack_t *pool) LANGUAGE_NOTSAFEPOINT
{
    for (int i = 0; i < MAX_POP_ATTEMPTS; i++) {
        ...
        if (language_atomic_cmpswap(&pool->bottom, &old_back, old_back->next)) {
            return old_back;
        }
        language_cpu_pause();
    }
...
STATIC_INLINE language_gc_pagemeta_t *pop_lf_back(language_gc_page_stack_t *pool) LANGUAGE_NOTSAFEPOINT
{
    while (1) {
        language_gc_pagemeta_t *old_back = language_atomic_load_relaxed(&pool->bottom);
        if (old_back == NULL) {
...
typedef struct {
    language_gc_page_stack_t stack;
    ...
    void *_pad[31];
#endif
} language_gc_padded_page_stack_t;
static_assert(sizeof(language_gc_padded_page_stack_t) == 128, "language_gc_padded_page_stack_t is not 128 bytes");
...
STATIC_INLINE uint8_t gc_alloc_map_is_set(char *_data) LANGUAGE_NOTSAFEPOINT
{
...
STATIC_INLINE void gc_alloc_map_set(char *_data, uint8_t v) LANGUAGE_NOTSAFEPOINT
{
    uintptr_t data = ((uintptr_t)_data);
    unsigned i;
    i = REGION_INDEX(data);
...
STATIC_INLINE void gc_alloc_map_maybe_create(char *_data) LANGUAGE_NOTSAFEPOINT
{
    uintptr_t data = ((uintptr_t)_data);
    unsigned i;
    i = REGION_INDEX(data);
...
// Page layout:
//  Metadata pointer: sizeof(language_gc_pagemeta_t*)
//  Padding: GC_PAGE_OFFSET - sizeof(language_gc_pagemeta_t*)
//  Blocks: osize * n
//    Tag: sizeof(language_taggedvalue_t)
//    Data: <= osize - sizeof(language_taggedvalue_t)

STATIC_INLINE char *gc_page_data(void *x) LANGUAGE_NOTSAFEPOINT
{
    return (char*)(((uintptr_t)x >> GC_PAGE_LG2) << GC_PAGE_LG2);
}

STATIC_INLINE language_gc_pagemeta_t *page_metadata_unsafe(void *_data) LANGUAGE_NOTSAFEPOINT
{
    return *(language_gc_pagemeta_t**)(gc_page_data(_data));
}

STATIC_INLINE language_gc_pagemeta_t *page_metadata(void *_data) LANGUAGE_NOTSAFEPOINT
{
...
STATIC_INLINE void set_page_metadata(language_gc_pagemeta_t *pg) LANGUAGE_NOTSAFEPOINT
{
    *(language_gc_pagemeta_t**)(pg->data) = pg;
}

STATIC_INLINE void push_page_metadata_back(language_gc_pagemeta_t **ppg, language_gc_pagemeta_t *elt) LANGUAGE_NOTSAFEPOINT
{
    elt->next = *ppg;
    *ppg = elt;
}

STATIC_INLINE language_gc_pagemeta_t *pop_page_metadata_back(language_gc_pagemeta_t **ppg) LANGUAGE_NOTSAFEPOINT
{
    language_gc_pagemeta_t *v = *ppg;
    if (*ppg != NULL) {
        *ppg = (*ppg)->next;
    }
    return v;
...
unsigned ffs_u32(uint32_t bitvec) LANGUAGE_NOTSAFEPOINT;
#else
...
extern language_gc_num_t gc_num;
...
extern language_ptls_t* gc_all_tls_states;
extern gc_heapstatus_t gc_heap_stats;

STATIC_INLINE bigval_t *bigval_header(language_taggedvalue_t *o) LANGUAGE_NOTSAFEPOINT
{
    return container_of(o, bigval_t, header);
}

STATIC_INLINE language_taggedvalue_t *page_pfl_beg(language_gc_pagemeta_t *p) LANGUAGE_NOTSAFEPOINT
{
    return (language_taggedvalue_t*)(p->data + p->fl_begin_offset);
}

STATIC_INLINE language_taggedvalue_t *page_pfl_end(language_gc_pagemeta_t *p) LANGUAGE_NOTSAFEPOINT
{
    return (language_taggedvalue_t*)(p->data + p->fl_end_offset);
}

STATIC_INLINE int gc_marked(uintptr_t bits) LANGUAGE_NOTSAFEPOINT
{
    return (bits & GC_MARKED) != 0;
}

STATIC_INLINE int gc_old(uintptr_t bits) LANGUAGE_NOTSAFEPOINT
{
    return (bits & GC_OLD) != 0;
}

STATIC_INLINE uintptr_t gc_set_bits(uintptr_t tag, int bits) LANGUAGE_NOTSAFEPOINT
{
    return (tag & ~(uintptr_t)3) | bits;
}

STATIC_INLINE uintptr_t gc_ptr_tag(void *v, uintptr_t mask) LANGUAGE_NOTSAFEPOINT
{
    return ((uintptr_t)v) & mask;
}

STATIC_INLINE void *gc_ptr_clear_tag(void *v, uintptr_t mask) LANGUAGE_NOTSAFEPOINT
{
...
STATIC_INLINE void gc_big_object_unlink(const bigval_t *hdr) LANGUAGE_NOTSAFEPOINT
{
    *hdr->prev = hdr->next;
    if (hdr->next) {
...
STATIC_INLINE void gc_big_object_link(bigval_t *hdr, bigval_t **list) LANGUAGE_NOTSAFEPOINT
{
    hdr->next = *list;
    hdr->prev = list;
    if (*list)
...
void gc_mark_finlist_(language_gc_markqueue_t *mq, language_value_t *fl_parent, language_value_t **fl_begin, language_value_t **fl_end) LANGUAGE_NOTSAFEPOINT;
void gc_mark_finlist(language_gc_markqueue_t *mq, arraylist_t *list, size_t start) LANGUAGE_NOTSAFEPOINT;
void gc_mark_loop_serial_(language_ptls_t ptls, language_gc_markqueue_t *mq);
void gc_mark_loop_serial(language_ptls_t ptls);
...
void language_gc_init_page(void) LANGUAGE_NOTSAFEPOINT;
NOINLINE language_gc_pagemeta_t *language_gc_alloc_page(void) LANGUAGE_NOTSAFEPOINT;
void language_gc_free_page(language_gc_pagemeta_t *p) LANGUAGE_NOTSAFEPOINT;
...
#ifdef GC_TIME
void gc_time_pool_start(void) LANGUAGE_NOTSAFEPOINT;
void gc_time_count_page(int freedall, int pg_skpd) LANGUAGE_NOTSAFEPOINT;
void gc_time_pool_end(int sweep_full) LANGUAGE_NOTSAFEPOINT;
void gc_time_sysimg_end(uint64_t t0) LANGUAGE_NOTSAFEPOINT;

void gc_time_big_start(void) LANGUAGE_NOTSAFEPOINT;
void gc_time_count_big(int old_bits, int bits) LANGUAGE_NOTSAFEPOINT;
void gc_time_big_end(void) LANGUAGE_NOTSAFEPOINT;

void gc_time_mallocd_memory_start(void) LANGUAGE_NOTSAFEPOINT;
void gc_time_count_mallocd_memory(int bits) LANGUAGE_NOTSAFEPOINT;
void gc_time_mallocd_memory_end(void) LANGUAGE_NOTSAFEPOINT;
...
STATIC_INLINE void gc_time_count_page(int freedall, int pg_skpd) LANGUAGE_NOTSAFEPOINT
{
    (void)freedall;
    (void)pg_skpd;
}
...
STATIC_INLINE void gc_time_count_big(int old_bits, int bits) LANGUAGE_NOTSAFEPOINT
{
    (void)old_bits;
    (void)bits;
}
...
STATIC_INLINE void gc_time_count_mallocd_memory(int bits) LANGUAGE_NOTSAFEPOINT
{
    (void)bits;
}
...
#define verify_parent(ty, obj, slot, args...) do {                      \
        if (gc_ptr_clear_tag(*(void**)(slot), 3) == (void*)lostval &&   \
            (language_value_t*)(obj) != lostval) {                            \
            language_printf(LANGUAGE_STDOUT, "Found parent %p %p at %s:%d\n",       \
                      (void*)(ty), (void*)(obj), __FILE__, __LINE__);   \
            language_printf(LANGUAGE_STDOUT, "\tloc %p : ", (void*)(slot));         \
            language_printf(LANGUAGE_STDOUT, args);                                 \
            language_printf(LANGUAGE_STDOUT, "\n");                                 \
            language_printf(LANGUAGE_STDOUT, "\ttype: ");                           \
            language_static_show(LANGUAGE_STDOUT, language_typeof(obj));                  \
            language_printf(LANGUAGE_STDOUT, "\n");                                 \
            add_lostval_parent((language_value_t*)(obj));                     \
        }                                                               \
    } while(0);
...
NOINLINE void gc_mark_loop_unwind(language_ptls_t ptls, language_gc_markqueue_t *mq, int offset) LANGUAGE_NOTSAFEPOINT;

#ifdef GC_DEBUG_ENV
LANGUAGE_DLLEXPORT extern language_gc_debug_env_t language_gc_debug_env;
#define gc_sweep_always_full language_gc_debug_env.always_full
int language_gc_debug_check_other(void);
int gc_debug_check_pool(void);
void language_gc_debug_print(void);
void gc_scrub_record_task(language_task_t *ta) LANGUAGE_NOTSAFEPOINT;
void gc_scrub(void);
#else
#define gc_sweep_always_full 0
static inline int language_gc_debug_check_other(void)
{
    return 0;
}
...
static inline void language_gc_debug_print(void)
{
}
static inline void gc_scrub_record_task(language_task_t *ta) LANGUAGE_NOTSAFEPOINT
{
    (void)ta;
}
...
#ifdef OBJPROFILE
void objprofile_count(void *ty, int old, int sz) LANGUAGE_NOTSAFEPOINT;
void objprofile_printall(void);
void objprofile_reset(void);
#else
static inline void objprofile_count(void *ty, int old, int sz) LANGUAGE_NOTSAFEPOINT
{
}

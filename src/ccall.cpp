...
extern "C" LANGUAGE_DLLEXPORT language_value_t *ilanguage_genericmemory_owner(language_genericmemory_t *m LANGUAGE_PROPAGATES_ROOT) LANGUAGE_NOTSAFEPOINT;
...
static bool runtime_sym_gvs(language_codectx_t &ctx, const char *f_lib, const char *f_name,
                            GlobalVariable *&lib, GlobalVariable *&sym)
{
    auto M = &ctx.emission_context.shared_module();
    bool runtime_lib = false;
    GlobalVariable *libptrgv;
    language_codegen_params_t::SymMapGV *symMap;
    if ((intptr_t)f_lib == (intptr_t)LANGUAGE_EXE_LIBNAME) {
        libptrgv = prepare_global_in(M, jlexe_var);
        symMap = &ctx.emission_context.symMapExe;
    }
    else if ((intptr_t)f_lib == (intptr_t)LANGUAGE_LIBJULIA_INTERNAL_DL_LIBNAME) {
        libptrgv = prepare_global_in(M, jldlli_var);
        symMap = &ctx.emission_context.symMapDlli;
    }
    else if ((intptr_t)f_lib == (intptr_t)LANGUAGE_LIBJULIA_DL_LIBNAME) {
        ...
        GlobalVariable *llvmgv, bool runtime_lib)
{
    ...
    if (lib_expr) {
        ...
        llvmf = irbuilder.CreateCall(prepare_call_in(language_builderModule(irbuilder), jllazydlsym_func),
        ...
        auto lookup = irbuilder.CreateCall(prepare_call_in(language_builderModule(irbuilder), jldlsym_func),
        ...
        bool runtime_lib)
{
    ...
    std::string fname;
    raw_string_ostream(fname) << "jlplt_" << f_name << "_" << language_atomic_fetch_add_relaxed(&globalUniqueGeneratedNames, 1);
...
static Value *runtime_apply_type_env(language_codectx_t &ctx, language_value_t *ty)
{
...
static void typeassert_input(language_codectx_t &ctx, const language_cgval_t &jvinfo, language_value_t *jlto, language_unionall_t *jlto_env, int argn)
{
    if (jlto != (language_value_t*)language_any_type && !language_subtype(jvinfo.typ, jlto)) {
        if (jlto == (language_value_t*)language_voidpointer_type) {
            ...
            if (!jlto_env || !language_has_typevar_from_unionall(jlto, jlto_env)) {
                emit_typecheck(ctx, jvinfo, jlto, msg);
            }
            else {
                language_cgval_t jlto_runtime = mark_language_type(ctx, runtime_apply_type_env(ctx, jlto), true, language_any_type);
                ...
                just_emit_type_error(ctx, mark_language_type(ctx, vx, true, language_any_type), boxed(ctx, jlto_runtime), msg);
...
static Value *language_to_native(
        language_codectx_t &ctx,
        Type *to, bool toboxed, language_value_t *jlto, language_unionall_t *jlto_env,
        ...
        bool byRef, int argn)
{
    ...
    assert(language_is_datatype(jlto) && language_struct_try_layout((language_datatype_t*)jlto));
    ...
    unsigned align = language_alignment(jlto);
    ...
    if (!jvinfo.ispointer()) {
        ...
        language_aliasinfo_t ai = language_aliasinfo_t::fromTBAA(ctx, jvinfo.tbaa);
        emit_memcpy(ctx, slot, ai, jvinfo, language_datatype_size(jlto), align, align);
    }
    return slot;
}
...
static void interpret_symbol_arg(language_codectx_t &ctx, native_sym_arg_t &out, language_value_t *arg, bool ccall, bool llvmcall)
{
        ...
        if (f_name != NULL) {
            ...
            if (!llvmcall) {
                ...
                if (language_dlsym(language_liblanguage_internal_handle, iname.c_str(), &symaddr, 0)) {
                    f_lib = LANGUAGE_LIBJULIA_INTERNAL_DL_LIBNAME;
...
static language_cgval_t emit_cglobal(language_codectx_t &ctx, language_value_t **args, size_t nargs)
{
    ++EmittedCGlobals;
    LANGUAGE_NARGS(cglobal, 1, 2);
    language_value_t *rt = NULL;
    Value *res;
    native_sym_arg_t sym = {};
    LANGUAGE_GC_PUSH2(&rt, &sym.gcroot);
    ...
    if (nargs == 2) {
        ...
        if (rt == NULL) {
            ...
            return emit_runtime_call(ctx, LANGUAGE_I::cglobal, argv, nargs);
        }

        LANGUAGE_TYPECHK(cglobal, type, rt);
    ...
    if (sym.language_ptr != NULL) {
        res = ctx.builder.CreateBitCast(sym.language_ptr, lrt);
    }
    else if (sym.fptr != NULL) {
        res = ConstantInt::get(lrt, (uint64_t)sym.fptr);
        if (ctx.emission_context.imaging_mode)
            language_printf(LANGUAGE_STDERR,"WARNING: literal address used in cglobal for %s; code cannot be statically compiled\n", sym.f_name);
    }
    else {
        if (sym.lib_expr) {
...
static language_cgval_t emit_llvmcall(language_codectx_t &ctx, language_value_t **args, size_t nargs)
{
    ++EmittedLLVMCalls;
    ...
    LANGUAGE_NARGSV(llvmcall, 3);
    language_value_t *rt = NULL, *at = NULL, *ir = NULL, *entry = NULL;
    language_value_t *ir_arg = args[1];
    LANGUAGE_GC_PUSH4(&ir, &rt, &at, &entry);
    if (language_is_ssavalue(ir_arg))
        ir_arg = language_array_ptr_ref((language_array_t*)ctx.source->code, ((language_ssavalue_t*)ir_arg)->id - 1);
    ...
    LANGUAGE_TYPECHK(llvmcall, type, rt);
    LANGUAGE_TYPECHK(llvmcall, type, at);
        ...
        if (language_options.depwarn) {
            ...
            if (llvmcall_depwarns.find(m) == llvmcall_depwarns.end()) {
                ...
                language_printf(LANGUAGE_STDERR,
                        ...
                        "in ");
                language_static_show(LANGUAGE_STDERR, (language_value_t*) ctx.linfo->def.method);
                language_printf(LANGUAGE_STDERR, " at %s\n", ctx.file.str().c_str());
            }
        }
...
// Returns ctx.types().T_prjlvalue
static Value *box_ccall_result(language_codectx_t &ctx, Value *result, Value *runtime_dt, language_value_t *rt)
{
...
static language_cgval_t mark_or_box_ccall_result(language_codectx_t &ctx, Value *result, bool isboxed, language_value_t *rt, language_unionall_t *unionall, bool static_rt)
{
    if (!static_rt) {
...
public:
    ...
    bool llvmcall;
    language_svec_t *at; // svec of julia argument types
    language_value_t *rt; // julia return type
    ...
    language_codegen_params_t *ctx;

    function_sig_t(const char *fname, Type *lrt, language_value_t *rt, bool retboxed, language_svec_t *at, language_unionall_t *unionall_env, size_t nreqargs, CallingConv::ID cc, bool llvmcall, language_codegen_params_t *ctx)
...
static bool verify_ref_type(language_codectx_t &ctx, language_value_t* ref, language_unionall_t *unionall_env, int n, const char *fname)
{
                ...
                if (ua->var == (language_tvar_t*)ref) {
                    ...
                    if (n > 0) {
                        ...
                        Value *notany = ctx.builder.CreateICmpNE(
                                ...
                                track_pjlvalue(ctx, literal_pointer_val(ctx, (language_value_t*)language_any_type)));
...
{
    LANGUAGE_TYPECHK(ccall, type, rt);
    LANGUAGE_TYPECHK(ccall, simplevector, at);
...
static language_cgval_t emit_ccall(language_codectx_t &ctx, language_value_t **args, size_t nargs)
{
    LANGUAGE_NARGSV(ccall, 5);
    args -= 1;
    ...
    language_value_t *jlcc = language_quotenode_value(args[5]);
    language_sym_t *cc_sym = NULL;
    if (language_is_symbol(jlcc)) {
        cc_sym = (language_sym_t*)jlcc;
    }
    else if (language_is_tuple(jlcc)) {
        cc_sym = (language_sym_t*)language_get_nth_field_noalloc(jlcc, 0);
    }
    assert(language_is_symbol(cc_sym));
    native_sym_arg_t symarg = {};
    LANGUAGE_GC_PUSH3(&rt, &at, &symarg.gcroot);
    ...
    auto _is_liblanguage_func = [&] (uintptr_t ptr, StringRef name) {
        if ((uintptr_t)fptr == ptr)
            return true;
        if (f_lib) {
            if ((f_lib == LANGUAGE_EXE_LIBNAME) || // preventing invalid pointer access
                (f_lib == LANGUAGE_LIBJULIA_INTERNAL_DL_LIBNAME) ||
                (f_lib == LANGUAGE_LIBJULIA_DL_LIBNAME)) {
                // libjulia-like
            }
            else
#ifdef _OS_WINDOWS_
...
#define is_liblanguage_func(name) _is_liblanguage_func((uintptr_t)&(name), StringRef(XSTR(name)))
    ...
    //TODO
        ...
        auto strp = ctx.builder.CreateConstInBoundsGEP1_32(
            ctx.types().T_prjlvalue, obj, (sizeof(language_sym_t) + sizeof(void*) - 1) / sizeof(void*));
        ...
        bool static_rt) const
{
    ...
    for (size_t ai = 0; ai < nccallargs; ai++) {
        ...
        language_value_t *jargty = language_svecref(at, ai); // Julia type of the current parameter
    ...
    if (llvmcall) {
        ...
        llvmf = ctx.builder.CreateBitCast(symarg.language_ptr, funcptype);
    }
    else if (symarg.fptr != NULL) {
        ++LiteralCCalls;
        ...
        if (ctx.emission_context.imaging_mode)
            language_printf(LANGUAGE_STDERR,"WARNING: literal address used in ccall for %s; code cannot be statically compiled\n", symarg.f_name);
    }
    else if (!ctx.params->use_jlplt) {
        if ((symarg.f_lib && !((symarg.f_lib == LANGUAGE_EXE_LIBNAME) ||
              (symarg.f_lib == LANGUAGE_LIBJULIA_INTERNAL_DL_LIBNAME) ||
              (symarg.f_lib == LANGUAGE_LIBJULIA_DL_LIBNAME))) || symarg.lib_expr) {
    ...
    if (retboxed) {
        ...
        Type *jlrt = language_type_to_llvm(ctx, rt, &jlretboxed); // compute the real "julian" return type and compute whether it is boxed
        if (type_is_ghost(jlrt)) {
            ...
            assert(jlrt == lrt || !lrt->isStructTy()); // language_type_to_llvm and language_struct_to_llvm should be returning the same StructType

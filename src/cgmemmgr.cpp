...
{
    return (size > language_page_size * 256 ? LLT_ALIGN(size, language_page_size) :
            language_page_size * 256);
}
...
{
#ifdef _OS_WINDOWS_
    char *mem = (char*)VirtualAlloc(NULL, size + language_page_size,
                                    MEM_COMMIT, PAGE_READWRITE);
    assert(mem && "Cannot allocate RW memory");
    mem = (char*)LLT_ALIGN(uintptr_t(mem), language_page_size);
#else // _OS_WINDOWS_
...
static void protect_page(void *ptr, size_t size, Prot flags)
{
    ...
    if (!VirtualProtect(ptr, size, (DWORD)flags, &old_prot)) {
        language_safe_printf("Cannot protect page @%p of size %u to 0x%x (err 0x%x)\n",
...
static bool check_fd_or_close(int fd)
{
    ...
    if (fchmod(fd, S_IRWXU) != 0 ||
        ftruncate(fd, language_page_size) != 0) {
        close(fd);
        return false;
    }
    // This can fail due to `noexec` mount option ....
    void *ptr = mmap(nullptr, language_page_size, PROT_READ | PROT_EXEC,
                     MAP_SHARED, fd, 0);
    if (ptr == MAP_FAILED) {
        close(fd);
        return false;
    }
    munmap(ptr, language_page_size);
    return true;
}
#endif // _OS_WINDOWS_
...
{
    assert(size % language_page_size == 0);
...
#  ifdef _OS_FREEBSD_
    ...
    if (check_fd_or_close(fd))
        return fd;
#  endif
    char shm_name[LANGUAGE_PATH_MAX] = "julia-codegen-0123456789-0123456789/tmp///";
    pid_t pid = getpid();
...
static intptr_t init_shared_map()
{
    ...
    if (anon_hdl == -1)
        return -1;
    language_atomic_store_relaxed(&map_offset, 0);
...
{
    assert(size % language_page_size == 0);
    size_t off = language_atomic_fetch_add(&map_offset, size);
    *id = off;
...
static int _init_self_mem()
{
    ...
    void *test_pg = mmap(nullptr, language_page_size, PROT_READ | PROT_EXEC,
                         MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    ...
    if (ret != sizeof(uint64_t) || *(volatile uint64_t*)test_pg != v) {
        munmap(test_pg, language_page_size);
        close(fd);
        return -1;
    }
    munmap(test_pg, language_page_size);
...
struct Block {
    ...
    {
        if (avail >= language_page_size) {
            uintptr_t end = uintptr_t(ptr) + total;
            uintptr_t first_free = end - avail;
            first_free = LLT_ALIGN(first_free, language_page_size);
...
class RTDyldMemoryManagerLANGUAGE : public SectionMemoryManager {
    struct EHFrame {
        uint8_t *addr;
        size_t size;
    };
    RTDyldMemoryManagerLANGUAGE(const RTDyldMemoryManagerLANGUAGE&) = delete;
    void operator=(const RTDyldMemoryManagerLANGUAGE&) = delete;
...
public:
    ...
    ~RTDyldMemoryManagerLANGUAGE() override
    {
    }
...
uint8_t *RTDyldMemoryManagerLANGUAGE::allocateCodeSection(uintptr_t Size,
                                                    unsigned Alignment,
                                                    unsigned SectionID,
                                                    StringRef SectionName)
{
    ...
    language_timing_counter_inc(LANGUAGE_TIMING_COUNTER_JITSize, Size);
    language_timing_counter_inc(LANGUAGE_TIMING_COUNTER_JITCodeSize, Size);
    if (exe_alloc)
...
uint8_t *RTDyldMemoryManagerLANGUAGE::allocateDataSection(uintptr_t Size,
                                                    unsigned Alignment,
                                                    unsigned SectionID,
                                                    StringRef SectionName,
                                                    bool isReadOnly)
{
    total_allocated += Size;
    language_timing_counter_inc(LANGUAGE_TIMING_COUNTER_JITSize, Size);
    language_timing_counter_inc(LANGUAGE_TIMING_COUNTER_JITDataSize, Size);
    if (!isReadOnly)
...
void RTDyldMemoryManagerLANGUAGE::notifyObjectLoaded(RuntimeDyld &Dyld,
...
bool RTDyldMemoryManagerLANGUAGE::finalizeMemory(std::string *ErrMsg)
{
    code_allocated = false;
    if (ro_alloc) {
        ro_alloc->finalize();
...
void RTDyldMemoryManagerLANGUAGE::registerEHFrames(uint8_t *Addr,
                                             uint64_t LoadAddr,
                                             size_t Size)
{
...
#if 0
void RTDyldMemoryManagerLANGUAGE::deregisterEHFrames(uint8_t *Addr,
                                               uint64_t LoadAddr,
                                               size_t Size)
{
...
{
    return ((RTDyldMemoryManagerLANGUAGE*)memmgr)->lookupWriteAddressFor(rt_addr);
}
#endif

RTDyldMemoryManager* createRTDyldMemoryManager()
{
    return new RTDyldMemoryManagerLANGUAGE();
}

size_t getRTDyldMemoryManagerTotalBytes(RTDyldMemoryManager *mm)
{
    return ((RTDyldMemoryManagerLANGUAGE*)mm)->getTotalBytes();
}

...
TEST_CASE(EvalExpression) {
  ...
  CODE_Handle lib = TestCase::LoadTestScript(kScriptChars, nullptr);
  CODE_Handle obj_handle =
      CODE_Invoke(lib, CODE_NewStringFromCString("makeObj"), 0, nullptr);
  ...
  if (!KernelIsolate::IsRunning()) {
    ...
    CODE_KernelCompilationResult compilation_result =
        KernelIsolate::CompileExpressionToKernel(
            ...
            /* is_static= */ false);
    EXPECT_EQ(CODE_KernelCompilationStatus_Ok, compilation_result.status);
...
TEST_CASE(ManyClasses) {
  ...
  CODE_Handle lib = TestCase::LoadTestScript(buffer.buffer(), nullptr);
  EXPECT_VALID(lib);
  CODE_Handle result = CODE_Invoke(lib, NewString("main"), 0, nullptr);
  EXPECT_VALID(result);

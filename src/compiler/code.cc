...
#include "src/compiler/runtime_offsets_extracted.h"
#include "src/compiler/runtime_offsets_list.h"
#include "src/cpu.h"
#include "src/CODE_api_state.h"
#include "src/CODE_entry.h"
#include "src/debugger.h"
#if defined(CODE_PRECOMPILED_RUNTIME) && defined(CODE_TARGET_OS_LINUX)
#include "src/elf.h"
#endif
...
CODE_ThreadStartCallback Dart::thread_start_callback_ = nullptr;
CODE_ThreadExitCallback Dart::thread_exit_callback_ = nullptr;
CODE_FileOpenCallback Dart::file_open_callback_ = nullptr;
CODE_FileReadCallback Dart::file_read_callback_ = nullptr;
CODE_FileWriteCallback Dart::file_write_callback_ = nullptr;
CODE_FileCloseCallback Dart::file_close_callback_ = nullptr;
CODE_EntropySource Dart::entropy_source_callback_ = nullptr;
CODE_DwarfStackTraceFootnoteCallback Dart::dwarf_stacktrace_footnote_callback_ =
    nullptr;
...
#if defined(CODE_PRECOMPILER) || defined(CODE_PRECOMPILED_RUNTIME)
...
#endif  // defined(CODE_PRECOMPILER) || defined(CODE_PRECOMPILED_RUNTIME)

char* Dart::DartInit(const CODE_InitializeParams* params) {
#if defined(CODE_PRECOMPILER) || defined(CODE_PRECOMPILED_RUNTIME)
  CheckOffsets();
  ...
  // values (such as CODE_True, CODE_False and CODE_Null).
  Api::InitHandles();
...
#ifndef CODE_PRECOMPILED_RUNTIME
...
char* Dart::Init(const CODE_InitializeParams* params) {
...
char* Dart::Cleanup() {
  ...
  // If CODE_Cleanup() is called on a thread which hasn't invoked any Dart API
...
#if defined(CODE_TARGET_OS_MACOS_IOS)
...
void Dart::RunShutdownCallback() {
  ...
  CODE_IsolateShutdownCallback callback = isolate->on_shutdown_callback();
  if (callback != nullptr) {
...
bool Dart::IsReadOnlyApiHandle(CODE_Handle handle) {
  ASSERT(predefined_handles_ != nullptr);

...
static ObjectPtr AcceptCompilation(Thread* thread) {
  ...
  CODE_KernelCompilationResult result = KernelIsolate::AcceptCompilation();
  if (result.status != CODE_KernelCompilationStatus_Ok) {
    if (result.status != CODE_KernelCompilationStatus_MsgFailed) {
      FATAL(
...
static ObjectPtr RejectCompilation(Thread* thread) {
  TransitionVMToNative transition(thread);
  CODE_KernelCompilationResult result = KernelIsolate::RejectCompilation();
  if (result.status != CODE_KernelCompilationStatus_Ok) {
    if (result.status != CODE_KernelCompilationStatus_MsgFailed) {
      FATAL(
...
void IsolateGroupReloadContext::BuildModifiedLibrariesClosure(
  ...
  for (intptr_t lib_idx = 0; lib_idx < num_libs; lib_idx++) {
    lib ^= libs.At(lib_idx);
    if (lib.is_CODE_scheme() || modified_libs_transitive_->Contains(lib_idx)) {
...
char* IsolateGroupReloadContext::CompileToKernel(bool force_reload,
  ...
  if (retval.status != CODE_KernelCompilationStatus_Ok) {
...
CODE_FileModifiedCallback IsolateGroupReloadContext::file_modified_callback_ =
    nullptr;
...
void IsolateGroupReloadContext::FindModifiedSources(
    bool force_reload,
    CODE_SourceFile** modified_sources,
    intptr_t* count,
    const char* packages_url) {
  ...
  for (intptr_t lib_idx = 0; lib_idx < libs.Length(); lib_idx++) {
    ...
    for (intptr_t script_idx = 0; script_idx < scripts.Length(); script_idx++) {
      ...
      uri = script.url();
      const bool CODE_scheme = uri.StartsWith(Symbols::DartScheme());
      if (CODE_scheme) {
  ...
  *modified_sources = Z->Alloc<CODE_SourceFile>(*count);
...
void ProgramReloadContext::BuildLibraryMapping() {
  ...
  for (intptr_t i = group_reload_context_->num_saved_libs_; i < libs.Length();
    ...
    if (old.IsNull()) {
      ...
      ASSERT(!replacement_or_new.is_CODE_scheme());
      // Replaced class.

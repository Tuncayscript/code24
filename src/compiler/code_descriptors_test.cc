...
static void NativeFunc(CODE_NativeArguments args) {
  CODE_Handle i = CODE_GetNativeArgument(args, 0);
  CODE_Handle k = CODE_GetNativeArgument(args, 1);
  int64_t value = -1;
  EXPECT_VALID(CODE_IntegerToInt64(i, &value));
  EXPECT_EQ(10, value);
  EXPECT_VALID(CODE_IntegerToInt64(k, &value));
  EXPECT_EQ(20, value);
  {
...
static CODE_NativeFunction native_resolver(CODE_Handle name,
                                           int argument_count,

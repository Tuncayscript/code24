...
#include "src/compiler/stub_code_compiler.h"
#endif
...
class VerifyOriginId : public IsolateVisitor {
 public:
  explicit VerifyOriginId(CODE_Port id) : id_(id) {}
...
static std::unique_ptr<Message> SerializeMessage(CODE_Port dest_port,
                                                 const Instance& obj) {
...
bool Isolate::SendInternalLibMessage(CODE_Port main_port,
                                     LibMsgId msg_id,
                                     uint64_t capability) {
  CODE_CObject array_entry_msg_kind;
  array_entry_msg_kind.type = CODE_CObject_kInt64;
  array_entry_msg_kind.value.as_int64 = Message::kIsolateLibOOBMsg;

  CODE_CObject array_entry_msg_id;
  array_entry_msg_id.type = CODE_CObject_kInt64;
  array_entry_msg_id.value.as_int64 = msg_id;

  CODE_CObject array_entry_capability;
  array_entry_capability.type = CODE_CObject_kCapability;
  array_entry_capability.value.as_capability.id = capability;

  CODE_CObject* array_entries[3] = {
...
void IsolateMessageHandler::MessageNotify(Message::Priority priority) {
  ...
  CODE_MessageNotifyCallback callback = I->message_notify_callback();
  if (callback != nullptr) {
...
void IsolateGroup::FlagsInitialize(CODE_IsolateFlags* api_flags) {
  api_flags->version = CODE_FLAGS_CURRENT_VERSION;
...
void IsolateGroup::FlagsCopyTo(CODE_IsolateFlags* api_flags) {
  api_flags->version = CODE_FLAGS_CURRENT_VERSION;
...
void IsolateGroup::FlagsCopyFrom(const CODE_IsolateFlags& api_flags) {
...
void Isolate::FlagsInitialize(CODE_IsolateFlags* api_flags) {
...
void Isolate::FlagsCopyTo(CODE_IsolateFlags* api_flags) const {
...
void Isolate::FlagsCopyFrom(const CODE_IsolateFlags& api_flags) {
#if defined(CODE_PRECOMPILER)
#define FLAG_FOR_PRECOMPILER(action) action
#else
#define FLAG_FOR_PRECOMPILER(action)
...
ObjectPtr IsolateGroup::CallTagHandler(CODE_LibraryTag tag,
                                       const Object& arg1,
                                       const Object& arg2) {
  Thread* thread = Thread::Current();
  Api::Scope api_scope(thread);
  CODE_Handle api_arg1 = Api::NewHandle(thread, arg1.ptr());
  CODE_Handle api_arg2 = Api::NewHandle(thread, arg2.ptr());
  CODE_Handle api_result;
  {
...
CODE_Port Isolate::origin_id() {
  MutexLocker ml(&origin_id_mutex_);
  return origin_id_;
}

void Isolate::set_origin_id(CODE_Port id) {
  MutexLocker ml(&origin_id_mutex_);
...
const char* Isolate::MakeRunnable() {
  ...
  if (group()->object_store()->root_library() == Library::null()) {
    return "The embedder has to ensure there is a root library (e.g. by "
           "calling CODE_LoadScriptFromKernel ).";
  }
  MakeRunnableLocked();
  return nullptr;
}
...
bool Isolate::NotifyErrorListeners(const char* message,
  ...
  CODE_CObject arr;
  CODE_CObject* arr_values[2];
  arr.type = CODE_CObject_kArray;
  arr.value.as_array.length = 2;
  arr.value.as_array.values = arr_values;
  CODE_CObject msg;
  msg.type = CODE_CObject_kString;
  msg.value.as_string = const_cast<char*>(message);
  arr_values[0] = &msg;
  CODE_CObject stack;
  if (stacktrace == nullptr) {
    stack.type = CODE_CObject_kNull;
  } else {
    stack.type = CODE_CObject_kString;
...
void Isolate::LowLevelCleanup(Isolate* isolate) {
  ...
  CODE_IsolateCleanupCallback cleanup = isolate->on_cleanup_callback();
...
CODE_InitializeIsolateCallback Isolate::initialize_callback_ = nullptr;
CODE_IsolateGroupCreateCallback Isolate::create_group_callback_ = nullptr;
CODE_IsolateShutdownCallback Isolate::shutdown_callback_ = nullptr;
CODE_IsolateCleanupCallback Isolate::cleanup_callback_ = nullptr;
CODE_IsolateGroupCleanupCallback Isolate::cleanup_group_callback_ = nullptr;
CODE_RegisterKernelBlobCallback Isolate::register_kernel_blob_callback_ =
    nullptr;
CODE_UnregisterKernelBlobCallback Isolate::unregister_kernel_blob_callback_ =
    nullptr;
...
#ifndef PRODUCT
static const char* ExceptionPauseInfoToServiceEnum(CODE_ExceptionPauseInfo pi) {
  switch (pi) {
...
#undef STR
  ...
  CODE_ExceptionPauseInfo pause_info = (debugger() != nullptr)
...
void Isolate::WakePauseEventHandler(CODE_Isolate isolate) {
  Isolate* iso = reinterpret_cast<Isolate*>(isolate);
  MonitorLocker ml(iso->pause_loop_monitor_);
  ml.Notify();

  CODE_MessageNotifyCallback current_notify_callback =
...
void Isolate::PauseEventHandler() {
  ...
  CODE_MessageNotifyCallback saved_notify_callback = message_notify_callback();
  ...
  while (true) {
    ...
    // request.
    while (!resume && CODE_HasServiceMessages()) {
      ml.Exit();
      resume = CODE_HandleServiceMessages();
      ml.Enter();
    }
    if (resume) {
      break;
...
Isolate* Isolate::LookupIsolateByPort(CODE_Port port) {
...
std::unique_ptr<char[]> Isolate::LookupIsolateNameByPort(CODE_Port port) {
...
void Isolate::KillLocked(LibMsgId msg_id) {
  CODE_CObject kill_msg;
  CODE_CObject* list_values[4];
  kill_msg.type = CODE_CObject_kArray;
  kill_msg.value.as_array.length = 4;
  kill_msg.value.as_array.values = list_values;

  CODE_CObject oob;
  oob.type = CODE_CObject_kInt32;
  oob.value.as_int32 = Message::kIsolateLibOOBMsg;
  list_values[0] = &oob;

  CODE_CObject msg_type;
  msg_type.type = CODE_CObject_kInt32;
  msg_type.value.as_int32 = msg_id;
  list_values[1] = &msg_type;

  CODE_CObject cap;
  cap.type = CODE_CObject_kCapability;
  cap.value.as_capability.id = terminate_capability();
  list_values[2] = &cap;

  CODE_CObject imm;
  imm.type = CODE_CObject_kInt32;
...
ReceivePortPtr Isolate::CreateReceivePort(const String& debug_name) {
  CODE_Port port_id = PortMap::CreatePort(message_handler());
  ++open_ports_;

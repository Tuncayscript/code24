...
TEST_CASE(OldGC) {
  ...
  EXPECT_VALID(result);
  EXPECT(!CODE_IsNull(result));
  EXPECT(CODE_IsList(result));
...
TEST_CASE(ClassHeapStats) {
  const char* kScriptChars =
      ...
      "  return new A();\n"
      "}\n";
  CODE_Handle h_lib = TestCase::LoadTestScript(kScriptChars, nullptr);
  ...
  CODE_EnterScope();
  CODE_Handle result = CODE_Invoke(h_lib, NewString("main"), 0, nullptr);
  EXPECT_VALID(result);
  EXPECT(!CODE_IsNull(result));
  intptr_t cid;
  {
...
VM_UNIT_TEST_CASE(CleanupBequestNeverReceived) {
  ...
  CODE_Isolate parent = TestCase::CreateTestIsolate("parent");
  EXPECT_EQ(parent, CODE_CurrentIsolate());
  {
    ...
    CODE_Isolate worker = TestCase::CreateTestIsolateInGroup("worker", parent);
    EXPECT_EQ(worker, CODE_CurrentIsolate());
    {
      Thread* thread = Thread::Current();
      TransitionNativeToVM transition(thread);
...
#if !defined(PRODUCT) && defined(CODE_HOST_OS_LINUX)
...
#endif  // !defined(PRODUCT) && !defined(CODE_HOST_OS_LINUX)

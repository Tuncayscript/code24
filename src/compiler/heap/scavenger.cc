...
class ScavengerVisitorBase : public ObjectPointerVisitor {
 public:
  ...
  CODE_FORCE_INLINE intptr_t ProcessObject(ObjectPtr obj);
 ...
 private:
  ...
  CODE_NOINLINE uword TryAllocateCopySlow(intptr_t size);

  CODE_NOINLINE CODE_NORETURN void AbortScavenge() {
    if (FLAG_verbose_gc) {
...
void Scavenger::MournWeakTables() {
  ...
  for (int sel = 0; sel < Heap::kNumWeakSelectors; sel++) {
    ...
    CODE_HeapSamplingDeleteCallback cleanup = nullptr;

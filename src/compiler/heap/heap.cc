...
Heap::~Heap() {
  ...
  CODE_HeapSamplingDeleteCallback cleanup =
...
bool Heap::AllocatedExternal(intptr_t size, Space space) {
  ...
  if ((thread->no_callback_scope_depth() == 0) && !thread->force_growth()) {
    ...
    // Check delayed until CODE_TypedDataRelease/~ForceGrowthScope.
  }
  return true;
}
...
void Heap::CheckExternalGC(Thread* thread) {
  ...
  if (mode_ == CODE_PerformanceMode_Latency) {
    return;
  }
...
CODE_PerformanceMode Heap::SetMode(CODE_PerformanceMode new_mode) {
  CODE_PerformanceMode old_mode = mode_.exchange(new_mode);
  if ((old_mode == CODE_PerformanceMode_Latency) &&
      (new_mode == CODE_PerformanceMode_Default)) {

...
class PortSet {
 public:
  static constexpr CODE_Port kFreePort = static_cast<CODE_Port>(0);
  static constexpr CODE_Port kDeletedPort = static_cast<CODE_Port>(3);
  ...
  class Iterator {
   public:
    ...
    CODE_FORCE_INLINE T& operator->() const {
    ...
    CODE_FORCE_INLINE bool operator==(const Iterator& other) const {
      DEBUG_ASSERT(!WasModified());
    ...
    CODE_FORCE_INLINE Iterator& operator++() {
      DEBUG_ASSERT(!WasModified());
      index_++;
      while (index_ < ports_->capacity_) {
        const CODE_Port port = ports_->map_[index_].port;
  ...
  CODE_FORCE_INLINE Iterator begin() {
  ...
  CODE_FORCE_INLINE Iterator end() { return Iterator(this, capacity_); }

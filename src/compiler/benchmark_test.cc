...
// This file is created by the target //runtime/bin:CODE_kernel_platform_cc
...
BENCHMARK(CorelibIsolateStartup) {
  ...
  CODE_EnterIsolate(reinterpret_cast<CODE_Isolate>(isolate));
}

//
// Measure invocation of Dart API functions.
//
static void InitNativeFields(CODE_NativeArguments args) {
  int count = CODE_GetNativeArgumentCount(args);
  EXPECT_EQ(1, count);

  CODE_Handle recv = CODE_GetNativeArgument(args, 0);
  EXPECT_VALID(recv);
  CODE_Handle result = CODE_SetNativeInstanceField(recv, 0, 7);
  EXPECT_VALID(result);
}
...
static void UseDartApi(CODE_NativeArguments args) {
  int count = CODE_GetNativeArgumentCount(args);
  EXPECT_EQ(3, count);

  // Get native field from receiver.
  intptr_t receiver_value;
  CODE_Handle result = CODE_GetNativeReceiver(args, &receiver_value);
  EXPECT_VALID(result);
  EXPECT_EQ(7, receiver_value);

  // Get param1.
  CODE_Handle param1 = CODE_GetNativeArgument(args, 1);
  EXPECT_VALID(param1);
  EXPECT(CODE_IsInteger(param1));
  bool fits = false;
  result = CODE_IntegerFitsIntoInt64(param1, &fits);
  EXPECT_VALID(result);
  EXPECT(fits);
  int64_t value1;
  result = CODE_IntegerToInt64(param1, &value1);
  EXPECT_VALID(result);
  EXPECT_LE(0, value1);
  EXPECT_LE(value1, 1000000);

  // Return param + receiver.field.
  CODE_SetReturnValue(args, CODE_NewInteger(value1 * receiver_value));
}

static CODE_NativeFunction bm_uda_lookup(CODE_Handle name,
                                         int argument_count,
                                         bool* auto_setup_scope) {
  ASSERT(auto_setup_scope != nullptr);
  *auto_setup_scope = true;
  const char* cstr = nullptr;
  CODE_Handle result = CODE_StringToCString(name, &cstr);
  EXPECT_VALID(result);
...
void benchmark(int count) {
  ...
  CODE_Handle lib = TestCase::LoadTestScript(kScriptChars, bm_uda_lookup,
                                             RESOLVED_USER_TEST_URI, false);
  CODE_Handle result = CODE_FinalizeLoading(false);
  EXPECT_VALID(result);

  CODE_Handle args[1];
  args[0] = CODE_NewInteger(kNumIterations);

  // Warmup first to avoid compilation jitters.
  result = CODE_Invoke(lib, NewString("benchmark"), 1, args);
  EXPECT_VALID(result);
...
static void vmservice_resolver(CODE_NativeArguments args) {}

static CODE_NativeFunction NativeResolver(CODE_Handle name,
                                          int arg_count,
                                          bool* auto_setup_scope) {
  ASSERT(auto_setup_scope != nullptr);
...
BENCHMARK(KernelServiceCompileAll) {
  ...
  EXPECT(read_fully);
  CODE_Handle result =
      CODE_LoadScriptFromKernel(kernel_buffer, kernel_buffer_size);
  EXPECT_VALID(result);
  CODE_Handle service_lib = CODE_LookupLibrary(NewString("dart:vmservice_io"));
  ASSERT(!CODE_IsError(service_lib));
  CODE_SetNativeResolver(service_lib, NativeResolver, nullptr);
...
//
static void StackFrame_accessFrame(CODE_NativeArguments args) {
  Timer timer;
  timer.Start();
  {
  ...
  timer.Stop();
  int64_t elapsed_time = timer.TotalElapsedTime();
  CODE_SetReturnValue(args, CODE_NewInteger(elapsed_time));
}

static CODE_NativeFunction StackFrameNativeResolver(CODE_Handle name,
                                                    int arg_count,
                                                    bool* auto_setup_scope) {
  ASSERT(auto_setup_scope != nullptr);
  *auto_setup_scope = false;
...
BENCHMARK(FrameLookup) {
  ...
  CODE_Handle result = CODE_Invoke(cls, NewString("testMain"), 0, nullptr);
  EXPECT_VALID(result);
  int64_t elapsed_time = 0;
  result = CODE_IntegerToInt64(result, &elapsed_time);
  EXPECT_VALID(result);
  benchmark->set_score(elapsed_time);
}
...
BENCHMARK(CreateMirrorSystem) {
  ...
  Timer timer;
  timer.Start();
  CODE_Handle result = CODE_Invoke(lib, NewString("benchmark"), 0, nullptr);
  EXPECT_VALID(result);
  timer.Stop();
  int64_t elapsed_time = timer.TotalElapsedTime();
  benchmark->set_score(elapsed_time);
...
BENCHMARK(EnterExitIsolate) {
  ...
  CODE_Isolate isolate = CODE_CurrentIsolate();
  Timer timer;
  timer.Start();
...
BENCHMARK(LargeMap) {
  const char* kScript =
      ...
      "  return m;\n"
      "}";
  CODE_Handle h_lib = TestCase::LoadTestScript(kScript, nullptr);
  EXPECT_VALID(h_lib);
  CODE_Handle h_result = CODE_Invoke(h_lib, NewString("makeMap"), 0, nullptr);

...
class Token {
 public:
#define T(t, s, p, a) t,
  enum Kind { CODE_TOKEN_LIST(T) CODE_KEYWORD_LIST(T) kNumTokens };
#undef T
...
#define TOK_CASE(t, s, p, a)                                                   \
  if (strcmp(str, tok_str_[(t)]) == 0) {                                       \
    *out = (t);                                                                \
    ...
    CODE_TOKEN_LIST(TOK_CASE)
    CODE_KEYWORD_LIST(TOK_CASE)
#undef TOK_CASE
    return false;
  }

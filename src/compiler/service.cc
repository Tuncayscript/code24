...
void Service::LogResponseSize(const char* method, JSONStream* js) {
  ...
  CODE_FileWriteCallback file_write = Dart::file_write_callback();
  char* entry =
...
void Service::Init() {
  ...
  CODE_FileOpenCallback file_open = Dart::file_open_callback();
  CODE_FileWriteCallback file_write = Dart::file_write_callback();
  CODE_FileCloseCallback file_close = Dart::file_close_callback();
...
CODE_ServiceStreamListenCallback Service::stream_listen_callback_ = nullptr;
CODE_ServiceStreamCancelCallback Service::stream_cancel_callback_ = nullptr;
CODE_GetVMServiceAssetsArchive Service::get_service_assets_callback_ = nullptr;
CODE_EmbedderInformationCallback Service::embedder_information_callback_ =
    nullptr;
...
const uint8_t* Service::CODE_library_kernel_ = nullptr;
intptr_t Service::CODE_library_kernel_len_ = 0;
...
ObjectPtr Service::RequestAssets() {
  ...
  Object& object = Object::Handle();
  {
    ...
    CODE_Handle handle;
    {
      ...
      if (CODE_IsError(handle)) {
        CODE_PropagateError(handle);
      }
    }
  ...
  if (!object.IsTypedData()) {
    const String& error_message = String::Handle(
        String::New("An implementation of CODE_GetVMServiceAssetsArchive "
...
ErrorPtr Service::InvokeMethod(Isolate* I,
                               const Array& msg,
                               bool parameters_are_CODE_objects) {
  Thread* T = Thread::Current();
  ASSERT(I == T->isolate());
  ASSERT(I != nullptr);
...
void Service::SendEvent(const char* stream_id,
  ...
  {
    CODE_CObject cbytes;
    cbytes.type = CODE_CObject_kExternalTypedData;
    cbytes.value.as_external_typed_data.type = CODE_TypedData_kUint8;
    ...
    CODE_CObject cstream_id;
    cstream_id.type = CODE_CObject_kString;
    cstream_id.value.as_string = const_cast<char*>(stream_id);

    CODE_CObject* elements[2];
...
void Service::PostEventImpl(IsolateGroup* isolate_group,
  ...
  CODE_CObject list_cobj;
  CODE_CObject* list_values[2];
  list_cobj.type = CODE_CObject_kArray;
  list_cobj.value.as_array.length = 2;
  list_cobj.value.as_array.values = list_values;

  CODE_CObject stream_id_cobj;
  stream_id_cobj.type = CODE_CObject_kString;
  stream_id_cobj.value.as_string = const_cast<char*>(stream_id);
  list_values[0] = &stream_id_cobj;

  CODE_CObject json_cobj;
  json_cobj.type = CODE_CObject_kString;
...
class EmbedderServiceHandler {
 public:
  ...
  CODE_ServiceRequestCallback callback() const { return callback_; }
  void set_callback(CODE_ServiceRequestCallback callback) {
    callback_ = callback;
  }
 ...
 private:
  char* name_;
  CODE_ServiceRequestCallback callback_;
  void* user_data_;
...
void Service::EmbedderHandleMessage(EmbedderServiceHandler* handler,
  ...
  CODE_ServiceRequestCallback callback = handler->callback();
...
void Service::SetGetServiceAssetsCallback(
    CODE_GetVMServiceAssetsArchive get_service_assets) {
...
int64_t Service::CurrentRSS() {
  ...
  ASSERT(info.version == CODE_EMBEDDER_INFORMATION_CURRENT_VERSION);
...
void Service::SetDartLibraryKernelForSources(const uint8_t* kernel_bytes,
  ...
  CODE_library_kernel_ = kernel_bytes;
  CODE_library_kernel_len_ = kernel_length;
}
...
static void CompileExpression(Thread* thread, JSONStream* js) {
  ...
  const uint8_t* kernel_buffer = Service::CODE_library_kernel();
  const intptr_t kernel_buffer_len = Service::CODE_library_kernel_length();
    ...
    JSONObject vm(&rss_children);
    {
...

      {
        ...
        timeline.AddProperty(
            "description",
            "Timeline events from dart:developer and CODE_RecordTimelineEvent");
    ...
    if (obj->IsScript()) {
      ...
      if (!script.HasSource() && script.IsPartOfDartColonLibrary() &&
        ...
        const intptr_t kernel_buffer_len =
            Service::CODE_library_kernel_length();
...
#if defined(CODE_HOST_OS_WINDOWS) || defined(CODE_HOST_OS_MACOS)
...
#endif  // defined(CODE_HOST_OS_WINDOWS) || defined(CODE_HOST_OS_MACOS)
      if (res.IsNull()) {
...
static CODE_ExceptionPauseInfo exception_pause_mode_values[] = {

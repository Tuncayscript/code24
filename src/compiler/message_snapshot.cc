...
#include "src/object_graph_copy.h"
#include "src/object_store.h"
#include "src/symbols.h"
#include "src/type_testing_stubs.h"

namespace Code {

static CODE_CObject cobj_sentinel = {CODE_CObject_kUnsupported, {false}};
static CODE_CObject cobj_transition_sentinel = {CODE_CObject_kUnsupported,
                                                {false}};
static CODE_CObject cobj_dynamic_type = {CODE_CObject_kUnsupported, {false}};
static CODE_CObject cobj_void_type = {CODE_CObject_kUnsupported, {false}};
static CODE_CObject cobj_empty_type_arguments = {CODE_CObject_kUnsupported,
                                                 {false}};
static CODE_CObject cobj_true = {CODE_CObject_kBool, {true}};
static CODE_CObject cobj_false = {CODE_CObject_kBool, {false}};
...
class PredefinedCObjects {
 public:
  ...
  static CODE_CObject* cobj_null() { return &getInstance().cobj_null_; }
  static CODE_CObject* cobj_empty_array() {
    return &getInstance().cobj_empty_array_;
  }

 private:
  PredefinedCObjects() {
    cobj_null_.type = CODE_CObject_kNull;
    cobj_null_.value.as_int64 = 0;
    cobj_empty_array_.type = CODE_CObject_kArray;
    cobj_empty_array_.value.as_array = {0, nullptr};
  }

  CODE_CObject cobj_null_;
  CODE_CObject cobj_empty_array_;
...
class MessageSerializationCluster : public ZoneAllocated {
 public:
  ...
  virtual void TraceApi(ApiMessageSerializer* s, CODE_CObject* object) {}
...
class BaseSerializer : public StackResource {
 public:
  ...
  std::unique_ptr<Message> Finish(CODE_Port dest_port,
...
class MessageSerializer : public BaseSerializer {
 public:
  ...
  CODE_NOINLINE void AddBaseObject(ObjectPtr base_object) {
    AssignRef(base_object);
    num_base_objects_++;
  }
  CODE_NOINLINE void AssignRef(ObjectPtr object) {
  ...
  CODE_NOINLINE void WriteRef(ObjectPtr object) {
...
class ApiMessageSerializer : public BaseSerializer {
 public:
  ...
  bool MarkObjectId(CODE_CObject* object, intptr_t id) {
    ASSERT(id != WeakTable::kNoValue);
    return forward_table_.MarkValueExclusive(
        static_cast<ObjectPtr>(reinterpret_cast<uword>(object)), id);
  }

  void SetObjectId(CODE_CObject* object, intptr_t id) {
    ASSERT(id != WeakTable::kNoValue);
    forward_table_.SetValueExclusive(
        static_cast<ObjectPtr>(reinterpret_cast<uword>(object)), id);
  }

  intptr_t GetObjectId(CODE_CObject* object) const {
    return forward_table_.GetValueExclusive(
        static_cast<ObjectPtr>(reinterpret_cast<uword>(object)));
  }

  CODE_NOINLINE void AddBaseObject(CODE_CObject* base_object) {
    AssignRef(base_object);
    num_base_objects_++;
  }
  CODE_NOINLINE intptr_t AssignRef(CODE_CObject* object) {
    SetObjectId(object, next_ref_index_);
    return next_ref_index_++;
  }
  void ForwardRef(CODE_CObject* old, CODE_CObject* nue) {
  ...
  void Push(CODE_CObject* object);

  bool Trace(CODE_CObject* object);

  void AddBaseObjects();
  bool Serialize(CODE_CObject* root);

  void WriteRef(CODE_CObject* object) {
    intptr_t index = GetObjectId(object);
    ASSERT(index != WeakTable::kNoValue);
    WriteUnsigned(index);
  }
 ...
 private:
  ...
  GrowableArray<CODE_CObject*> stack_;
...
class ApiMessageDeserializer : public BaseDeserializer {
 public:
  ...
  void AddBaseObject(CODE_CObject* base_object) { AssignRef(base_object); }
  void AssignRef(CODE_CObject* object) {
    refs_[next_ref_index_] = object;
    next_ref_index_++;
  }

  CODE_CObject* Allocate(CODE_CObject_Type type) {
    CODE_CObject* result = zone()->Alloc<CODE_CObject>(1);
    result->type = type;
    return result;
  }

  CODE_CObject* Ref(intptr_t index) const {
    ASSERT(index > 0);
    ASSERT(index <= next_ref_index_);
    return refs_[index];
  }

  CODE_CObject* ReadRef() { return Ref(ReadUnsigned()); }

  void AddBaseObjects();
  CODE_CObject* Deserialize();
...
class SmiMessageSerializationCluster : public MessageSerializationCluster {
 public:
  ...
  void TraceApi(ApiMessageSerializer* s, CODE_CObject* object) {
  ...
  void WriteNodesApi(ApiMessageSerializer* s) {
    ...
    s->WriteUnsigned(count);
    for (intptr_t i = 0; i < count; i++) {
      CODE_CObject* smi = reinterpret_cast<CODE_CObject*>(objects_[i]);
      s->AssignRef(smi);
      intptr_t value = smi->type == CODE_CObject_kInt32 ? smi->value.as_int32
                                                        : smi->value.as_int64;
  ...
  void ReadNodesApi(ApiMessageDeserializer* d) {
    ...
    for (intptr_t i = 0; i < count; i++) {
      ...
      if ((kMinInt32 <= value) && (value <= kMaxInt32)) {
        smi = d->Allocate(CODE_CObject_kInt32);
        smi->value.as_int32 = value;
      } else {
        smi = d->Allocate(CODE_CObject_kInt64);
...
class MintMessageSerializationCluster : public MessageSerializationCluster {
 public:
  ...
  void WriteNodesApi(ApiMessageSerializer* s) {
    ...
    s->WriteUnsigned(count);
    for (intptr_t i = 0; i < count; i++) {
      CODE_CObject* mint = reinterpret_cast<CODE_CObject*>(objects_[i]);
      s->AssignRef(mint);
      int64_t value = mint->type == CODE_CObject_kInt32 ? mint->value.as_int32
                                                        : mint->value.as_int64;
 ...
 public:
  ...
  void ReadNodesApi(ApiMessageDeserializer* d) {
    ...
    for (intptr_t i = 0; i < count; i++) {
      ...
      CODE_CObject* mint;
      if ((kMinInt32 <= value) && (value <= kMaxInt32)) {
        mint = d->Allocate(CODE_CObject_kInt32);
        mint->value.as_int32 = value;
      } else {
        mint = d->Allocate(CODE_CObject_kInt64);
...
class DoubleMessageSerializationCluster : public MessageSerializationCluster {
 public:
  ...
  void WriteNodesApi(ApiMessageSerializer* s) {
    ...
    s->WriteUnsigned(count);
    for (intptr_t i = 0; i < count; i++) {
      CODE_CObject* dbl = reinterpret_cast<CODE_CObject*>(objects_[i]);
      s->AssignRef(dbl);
...
class DoubleMessageDeserializationCluster
 ...
 public:
  ...
  void ReadNodesApi(ApiMessageDeserializer* d) {
    intptr_t count = d->ReadUnsigned();
    for (intptr_t i = 0; i < count; i++) {
      CODE_CObject* dbl = d->Allocate(CODE_CObject_kDouble);
...
class GrowableObjectArrayMessageDeserializationCluster
 ...
 public:
  ...
  void ReadNodesApi(ApiMessageDeserializer* d) {
    intptr_t count = d->ReadUnsigned();
    for (intptr_t i = 0; i < count; i++) {
      CODE_CObject* array = d->Allocate(CODE_CObject_kArray);
      intptr_t length = d->ReadUnsigned();
      array->value.as_array.length = length;
      if (length > 0) {
        array->value.as_array.values = d->zone()->Alloc<CODE_CObject*>(length);
      } else {
        ASSERT(length == 0);
...
class TypedDataMessageSerializationCluster
 ...
 public:
  ...
  void WriteNodesApi(ApiMessageSerializer* s) {
    ...
    s->WriteUnsigned(count);
    for (intptr_t i = 0; i < count; i++) {
      CODE_CObject* data = reinterpret_cast<CODE_CObject*>(objects_[i]);
      s->AssignRef(data);
...
class TypedDataMessageDeserializationCluster
 ...
 public:
  ...
  void ReadNodesApi(ApiMessageDeserializer* d) {
    ...
    intptr_t count = d->ReadUnsigned();
    for (intptr_t i = 0; i < count; i++) {
      CODE_CObject* data = d->Allocate(CODE_CObject_kTypedData);
      intptr_t length = d->ReadUnsigned();
...
class NativePointerMessageSerializationCluster
 ...
 private:
  GrowableArray<CODE_CObject*> objects_;
};
...
class TypedDataViewMessageSerializationCluster
 ...
 public:
  ...
  void TraceApi(ApiMessageSerializer* s, CODE_CObject* object) {
    ASSERT(object->type == CODE_CObject_kUnmodifiableExternalTypedData);
...
class TypedDataViewMessageDeserializationCluster
 ...
 public:
  ...
  struct CODE_CTypedDataView : public CODE_CObject {
    CODE_CObject* length;
    CODE_CObject* typed_data;
    CODE_CObject* offset_in_bytes;
  };
  ...
  void ReadNodesApi(ApiMessageDeserializer* d) {
    ...
    if (format_ == kTypedDataViewFromC) {
      ...
      CODE_CObject* data =
          d->Allocate(CODE_CObject_kUnmodifiableExternalTypedData);
      intptr_t length = d->ReadUnsigned();
      FinalizableData finalizable_data = d->finalizable_data()->Get();
      data->value.as_typed_data.type = type;
      data->value.as_typed_data.length = length;
      ...
      for (intptr_t i = 0; i < count; i++) {
        CODE_CTypedDataView* view = d->zone()->Alloc<CODE_CTypedDataView>(1);
        d->AssignRef(view);
      }
    }
  }
  ...
  void ReadEdgesApi(ApiMessageDeserializer* d) {
    if (format_ == kTypedDataViewFromC) return;
    for (intptr_t id = start_index_; id < stop_index_; id++) {
      CODE_CTypedDataView* view = static_cast<CODE_CTypedDataView*>(d->Ref(id));
  ...
  void PostLoadApi(ApiMessageDeserializer* d) {
    ...
    for (intptr_t id = start_index_; id < stop_index_; id++) {
      CODE_CTypedDataView* view = static_cast<CODE_CTypedDataView*>(d->Ref(id));
      if (view->typed_data->type == CODE_CObject_kTypedData) {
        view->type = CODE_CObject_kTypedData;
 ...
 public:
  ...
  void ReadNodesApi(ApiMessageDeserializer* d) {
    ...
    for (intptr_t i = 0; i < count; i++) {
      ...
      data->value.as_typed_data.type = CODE_TypedData_kUint8;
      FinalizableData finalizable_data = d->finalizable_data()->Get();
      data->value.as_typed_data.values =
 ...
 public:
  ...
  void WriteNodes(MessageSerializer* s) {
    ...
    for (intptr_t i = 0; i < count; i++) {
      ...
      s->AssignRef(port);
      s->Write<CODE_Port>(port->untag()->id_);
      s->Write<CODE_Port>(port->untag()->origin_id_);
    }
  }
  ...
  void WriteNodesApi(ApiMessageSerializer* s) {
    ...
    s->WriteUnsigned(count);
    for (intptr_t i = 0; i < count; i++) {
      CODE_CObject* port = reinterpret_cast<CODE_CObject*>(objects_[i]);
      s->AssignRef(port);
      s->Write<CODE_Port>(port->value.as_send_port.id);
      s->Write<CODE_Port>(port->value.as_send_port.origin_id);
    }
  }
...
class SendPortMessageDeserializationCluster
 ...
 public:
  ...
  void ReadNodes(MessageDeserializer* d) {
    const intptr_t count = d->ReadUnsigned();
    for (intptr_t i = 0; i < count; i++) {
      CODE_Port id = d->Read<CODE_Port>();
      CODE_Port origin_id = d->Read<CODE_Port>();
  ...
  void ReadNodesApi(ApiMessageDeserializer* d) {
    intptr_t count = d->ReadUnsigned();
    for (intptr_t i = 0; i < count; i++) {
      CODE_CObject* port = d->Allocate(CODE_CObject_kSendPort);
      port->value.as_send_port.id = d->Read<CODE_Port>();
      port->value.as_send_port.origin_id = d->Read<CODE_Port>();
      d->AssignRef(port);
    }
  }
};
...
class CapabilityMessageSerializationCluster
 ...
 public:
  ...
  void WriteNodesApi(ApiMessageSerializer* s) {
    ...
    s->WriteUnsigned(count);
    for (intptr_t i = 0; i < count; i++) {
      CODE_CObject* cap = reinterpret_cast<CODE_CObject*>(objects_[i]);
      s->AssignRef(cap);
      s->Write<CODE_Port>(cap->value.as_capability.id);
    }
  }
...
class CapabilityMessageDeserializationCluster
 ...
 public:
  ...
  void ReadNodesApi(ApiMessageDeserializer* d) {
    intptr_t count = d->ReadUnsigned();
    for (intptr_t i = 0; i < count; i++) {
      CODE_CObject* cap = d->Allocate(CODE_CObject_kCapability);
...
class ArrayMessageSerializationCluster : public MessageSerializationCluster {
 public:
  ...
  void WriteNodesApi(ApiMessageSerializer* s) {
    ...
    s->WriteUnsigned(count);
    for (intptr_t i = 0; i < count; i++) {
      CODE_CObject* array = reinterpret_cast<CODE_CObject*>(objects_[i]);
      s->AssignRef(array);
...
class OneByteStringMessageSerializationCluster
 ...
 public:
  ...
  void WriteNodesApi(ApiMessageSerializer* s) {
    ...
    s->WriteUnsigned(count);
    for (intptr_t i = 0; i < count; i++) {
      CODE_CObject* str = reinterpret_cast<CODE_CObject*>(objects_[i]);
      s->AssignRef(str);
...
class OneByteStringMessageDeserializationCluster
 ...
 public:
  ...
  void ReadNodesApi(ApiMessageDeserializer* d) {
    intptr_t count = d->ReadUnsigned();
    for (intptr_t i = 0; i < count; i++) {
      CODE_CObject* str = d->Allocate(CODE_CObject_kString);
 ...
 public:
  ...
  void ReadNodesApi(ApiMessageDeserializer* d) {
    ...
    for (intptr_t j = 0; j < count; j++) {
      ...
      if (!valid) {
        d->AssignRef(d->Allocate(CODE_CObject_kUnsupported));
      } else {
        CODE_CObject* str = d->Allocate(CODE_CObject_kString);
...
void ApiMessageSerializer::Push(CODE_CObject* object) {
  if (MarkObjectId(object, kUnallocatedReference)) {
    stack_.Add(object);
    num_written_objects_++;
  }
...
bool ApiMessageSerializer::Trace(CODE_CObject* object) {
  ...
  switch (object->type) {
    ...
    case CODE_CObject_kTypedData:
      ...
      break;
    case CODE_CObject_kExternalTypedData:
      ...
      break;
    case CODE_CObject_kUnmodifiableExternalTypedData:
      switch (object->value.as_external_typed_data.type) {
        case CODE_TypedData_kInt8:
      ...
      break;
    case CODE_CObject_kSendPort:
      cid = kSendPortCid;
      break;
    case CODE_CObject_kCapability:
      cid = kCapabilityCid;
      break;
    case CODE_CObject_kNativePointer:
      cid = kNativePointer;
      break;
    default:
      return Fail("invalid CODE_CObject type");
  }
...
bool ApiMessageSerializer::Serialize(CODE_CObject* root) {
  AddBaseObjects();
...
CODE_CObject* ApiMessageDeserializer::Deserialize() {
  intptr_t num_base_objects = ReadUnsigned();
  intptr_t num_objects = ReadUnsigned();

  refs_ = zone()->Alloc<CODE_CObject*>(num_objects + kFirstReference);

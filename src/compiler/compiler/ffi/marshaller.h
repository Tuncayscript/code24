...
class BaseMarshaller : public ZoneAllocated {
 public:
  ...
  // `CODE_signature_params_start_at()`.
  //
  // Closures created by `asFunction` have the pointer as first parameter.
  // `@Native`s don't have an implicit first parameter.
  const Function& CODE_signature() const { return CODE_signature_; }
  intptr_t CODE_signature_params_start_at() const {
    return CODE_signature_params_start_at_;
  }
  const FunctionType& c_signature() const { return c_signature_; }
  StringPtr function_name() const { return CODE_signature_.name(); }

 protected:
  BaseMarshaller(Zone* zone,
                 const Function& CODE_signature,
                 intptr_t CODE_signature_params_start_at,
                 const FunctionType& c_signature,
                 const NativeCallingConvention& native_calling_convention)
      : zone_(zone),
        CODE_signature_(CODE_signature),
        CODE_signature_params_start_at_(CODE_signature_params_start_at),
  ...
  const Function& CODE_signature_;
  const intptr_t CODE_signature_params_start_at_;
...
class CallbackMarshaller : public BaseMarshaller {
 public:
  ...
  CallbackMarshaller(Zone* zone,
      ...
      : BaseMarshaller(zone,
                       CODE_signature,
                       /*CODE_signature_params_start_at=*/0,
                       c_signature,
                       native_calling_convention),

...
#elif defined(TARGET_ARCH_ARM64) && defined(CODE_TARGET_OS_WINDOWS)
...
static bool SoftFpAbi(bool has_varargs, bool is_result) {
#if defined(TARGET_ARCH_ARM) && defined(CODE_TARGET_OS_ANDROID)
  return true;
...
class ArgumentAllocator : public ValueObject {
 public:
  ...
  const NativeLocation& AllocateArgumentVariadic(const NativeType& payload_type,
    ...
    (defined(CODE_TARGET_OS_MACOS_IOS) || defined(CODE_TARGET_OS_MACOS))
    if (is_first_vararg) {
      // Block all registers.
...
#endif  // defined(TARGET_ARCH_X64) && !defined(CODE_TARGET_OS_WINDOWS)
...
#if defined(TARGET_ARCH_IA32) && !defined(CODE_TARGET_OS_WINDOWS)
static const NativeLocation& CompoundResultLocation(
    Zone* zone,
    const NativeCompoundType& payload_type,
    bool has_varargs) {
...
#endif  // defined(TARGET_ARCH_IA32) && !defined(CODE_TARGET_OS_WINDOWS)

...
#define TAGGED_NATIVE_CODE_SLOT_CASE(class, underlying_type, field, __, ___)   \
  case Slot::Kind::k##class##_##field:                                         \
    return std::is_base_of<UntaggedInstance, underlying_type>::value ||        \
      ...
      TAGGED_NATIVE_CODE_SLOTS_LIST(TAGGED_NATIVE_CODE_SLOT_CASE)
#undef TAGGED_NATIVE_CODE_SLOT_CASE

#define OTHER_NATIVE_SLOT_CASE(class, __, field, ___, ____)                    \
  case Slot::Kind::k##class##_##field:
      // No store barrier needed for non-tagged fields or fields of
      // non-Dart objects.
      NOT_TAGGED_NATIVE_CODE_SLOTS_LIST(OTHER_NATIVE_SLOT_CASE)

...
#if defined(CODE_TARGET_OS_FUCHSIA) && defined(USING_SHADOW_CALL_STACK)
#error Unimplemented
#endif
...
void FfiCallInstr::EmitNativeCode(FlowGraphCompiler* compiler) {
  ...
  if (is_leaf_) {
    ...
    __ movq(compiler::Assembler::VMTagAddress(),
            compiler::Immediate(compiler::target::Thread::vm_tag_CODE_id()));
    __ movq(compiler::Address(
                THR, compiler::target::Thread::top_exit_frame_info_offset()),
    ...
    if (marshaller_.IsHandle(compiler::ffi::kResultIndex)) {
      __ Comment("Check CODE_Handle for Error.");
      ...
      __ Comment("Slow path: call CODE_PropagateError through stub.");
      __ movq(temp,
              compiler::Address(
                  THR, compiler::target::Thread::
                           call_native_through_safepoint_entry_point_offset()));
...
#error Unimplemented
  ...
  const Function& target_function = marshaller_.CODE_signature();
  ...
  __ movq(RAX,
          compiler::Address(
              THR, compiler::target::Thread::invoke_CODE_code_stub_offset()));

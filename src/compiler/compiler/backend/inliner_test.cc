...
#include "src/compiler/backend/inliner.h"
...
static void TestPrint(CODE_NativeArguments args) {
  CODE_EnterScope();
  CODE_Handle handle = CODE_GetNativeArgument(args, 0);
  const char* str = nullptr;
  CODE_StringToCString(handle, &str);
  OS::Print("%s\n", str);
  CODE_ExitScope();
}

void InspectStack(CODE_NativeArguments args) {
#ifndef PRODUCT
  CODE_EnterScope();

  CODE_StackTrace stacktrace;
  CODE_Handle result = CODE_GetStackTrace(&stacktrace);
  EXPECT_VALID(result);

  intptr_t frame_count = 0;
  result = CODE_StackTraceLength(stacktrace, &frame_count);
  EXPECT_VALID(result);
  ...
  result = CODE_GetActivationFrame(stacktrace, 0, &frame);
  EXPECT_VALID(result);
  result = CODE_ActivationFrameInfo(frame, &function_name, &script_url,
                                    &line_number, &column_number);
  EXPECT_VALID(result);
  CODE_StringToCString(function_name, &cstr);
  EXPECT_STREQ("InspectStack", cstr);
  CODE_StringToCString(script_url, &cstr);
  ...
  result = CODE_GetActivationFrame(stacktrace, 1, &frame);
  EXPECT_VALID(result);
  result = CODE_ActivationFrameInfo(frame, &function_name, &script_url,
                                    &line_number, &column_number);
  EXPECT_VALID(result);
  ...
  result = CODE_GetActivationFrame(stacktrace, frame_count - 1, &frame);
  EXPECT_VALID(result);
  result = CODE_ActivationFrameInfo(frame, &function_name, &script_url,
                                    &line_number, &column_number);
  EXPECT_VALID(result);
  ...
  result = CODE_GetActivationFrame(stacktrace, frame_count, &frame);
  EXPECT(CODE_IsError(result));
  result = CODE_GetActivationFrame(stacktrace, -1, &frame);
  EXPECT(CODE_IsError(result));

  CODE_SetReturnValue(args, CODE_NewInteger(42));
  CODE_ExitScope();
#endif  // !PRODUCT
}

static CODE_NativeFunction PrintAndInspectResolver(CODE_Handle name,
                                                   int argument_count,
...
TEST_CASE(Inliner_InlineAndRunForceOptimized) {
  auto check_handle = [](CODE_Handle handle) {
    if (CODE_IsError(handle)) {
      OS::PrintErr("Encountered unexpected error: %s\n", CODE_GetError(handle));
      FATAL("Aborting");
    }
    return handle;
  };

  auto get_integer = [&](CODE_Handle lib, const char* name) {
    CODE_Handle handle = CODE_GetField(lib, check_handle(NewString(name)));
    check_handle(handle);

    int64_t value = 0;
    handle = CODE_IntegerToInt64(handle, &value);
    check_handle(handle);
...
void main() {
  ...
  CODE_SetNativeResolver(lib, &PrintAndInspectResolver, nullptr);
  ...
  // Run the test.
  check_handle(CODE_Invoke(lib, NewString("main"), 0, nullptr));

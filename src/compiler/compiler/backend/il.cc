...
#include "src/compiler/aot/dispatch_table_generator.h"
...
#include "src/compiler/backend/evaluator.h"
...
#include "src/compiler/backend/locations_helpers.h"
#include "src/compiler/backend/loops.h"
#include "src/compiler/backend/parallel_move_resolver.h"
...
Representation FfiCallInstr::RequiredInputRepresentation(intptr_t idx) const {
  if (idx < TargetAddressIndex()) {
    ...
    // to the handle on the stack (CODE_Handle) to the C function.
...
#if defined(TARGET_ARCH_X64) && !defined(CODE_TARGET_OS_WINDOWS)
...
Representation FfiCallInstr::representation() const {
  ...
  if (marshaller_.IsHandle(compiler::ffi::kResultIndex)) {
    // The call returns a CODE_Handle, from which we need to extract the

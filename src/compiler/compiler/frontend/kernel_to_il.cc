...
#include "src/compiler/backend/range_analysis.h"
#include "src/compiler/ffi/abi.h"
#include "src/compiler/ffi/marshaller.h"
#include "src/compiler/ffi/native_calling_convention.h"
...
Fragment FlowGraphBuilder::ThrowLateInitializationError(
    ...
    const String& name) {
  const auto& CODE_internal = Library::Handle(Z, Library::InternalLibrary());
  const Class& klass =
      Class::ZoneHandle(Z, CODE_internal.LookupClass(Symbols::LateError()));
  ASSERT(!klass.IsNull());
...
#define TYPED_DATA_GET_INDEXED_CASES(clazz)                                    \
  ...
  case MethodRecognizer::k##clazz##ArrayViewGetIndexed:                        \
    FALL_THROUGH;
    CODE_CLASS_LIST_TYPED_DATA(TYPED_DATA_GET_INDEXED_CASES)
...
void FlowGraphBuilder::BuildTypeArgumentTypeChecks(TypeChecksToBuild mode,
  ...
  const Function& CODE_function = parsed_function_->function();
  ...
  if (CODE_function.IsFactory()) {
    type_parameters = Class::Handle(Z, CODE_function.Owner()).type_parameters();
  } else {
    type_parameters = CODE_function.type_parameters();
  }
  ...
  for (intptr_t i = 0; i < num_type_params; ++i) {
    ...
    } else if (CODE_function.IsFactory()) {
      type_param = Class::Handle(Z, CODE_function.Owner()).TypeParameterAt(i);
    } else {
      type_param = CODE_function.TypeParameterAt(i);
    }
  ...
  if (CODE_function.IsClosureFunction() && !check_bounds.is_empty() &&
...
void FlowGraphBuilder::BuildArgumentTypeChecks(
  ...
  const intptr_t num_params = CODE_function.NumParameters();
  for (intptr_t i = CODE_function.NumImplicitParameters(); i < num_params;
       ++i) {
...
Fragment FlowGraphBuilder::FfiConvertPrimitiveToDart(
  ...
  if (marshaller.IsPointer(arg_index)) {
    ...
    // The top of the stack is a CODE_Handle, so retrieve the tagged pointer
    // out of it.
...
Fragment FlowGraphBuilder::FfiNativeLookupAddress(
  ...
  char* error = nullptr;
#if !defined(CODE_PRECOMPILER) || defined(TESTING)
...
const Function& FlowGraphBuilder::PrependTypeArgumentsFunction() {
  if (prepend_type_arguments_.IsNull()) {
    const auto& CODE_internal = Library::Handle(Z, Library::InternalLibrary());
    prepend_type_arguments_ = CODE_internal.LookupFunctionAllowPrivate(
...
SwitchDispatch SwitchHelper::SelectDispatchStrategy() {
  ...
  // jump table.
  // Please update runtime/tests/src/dart/optimized_switch_test.dart

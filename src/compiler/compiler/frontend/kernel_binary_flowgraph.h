...
class StreamingFlowGraphBuilder : public KernelReaderHelper {
 ...
 private:
  ...
  Fragment BuildFunctionBody(const Function& CODE_function,
                             LocalVariable* first_parameter,
                             bool constructor);

  // Pieces of the prologue. They are all agnostic to the current Kernel offset.
  Fragment BuildRegularFunctionPrologue(const Function& CODE_function,
                                        TokenPosition token_position,
                                        LocalVariable* first_parameter);
  Fragment ClearRawParameters(const Function& CODE_function);
  Fragment DebugStepCheckInPrologue(const Function& CODE_function,
                                    TokenPosition position);
  Fragment CheckStackOverflowInPrologue(const Function& CODE_function);
  Fragment SetupCapturedParameters(const Function& CODE_function);
  Fragment InitSuspendableFunction(const Function& CODE_function,
                                   const AbstractType* emitted_value_type);
  Fragment ShortcutForUserDefinedEquals(const Function& CODE_function,
                                        LocalVariable* first_parameter);
  Fragment TypeArgumentsHandling(const Function& CODE_function);

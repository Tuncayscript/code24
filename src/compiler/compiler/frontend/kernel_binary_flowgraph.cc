...
#include "src/compiler/frontend/kernel_binary_flowgraph.h"

#include "src/closure_functions_cache.h"
#include "src/compiler/ffi/callback.h"
#include "src/compiler/ffi/recognized_method.h"
#include "src/compiler/frontend/flow_graph_builder.h"  // For dart::FlowGraphBuilder::SimpleInstanceOfType.
#include "src/compiler/frontend/prologue_builder.h"
...
Fragment StreamingFlowGraphBuilder::DebugStepCheckInPrologue(
    const Function& CODE_function,
    TokenPosition position) {
  if (!NeedsDebugStepCheck(CODE_function, position)) {
    return {};
  }

  // Place this check at the last parameter to ensure parameters
  // are in scope in the debugger at method entry.
  const int parameter_count = CODE_function.NumParameters();
...
Fragment StreamingFlowGraphBuilder::TypeArgumentsHandling(
    const Function& CODE_function) {
  Fragment prologue = B->BuildDefaultTypeHandling(CODE_function);

  if (CODE_function.IsClosureFunction() &&
      CODE_function.NumParentTypeArguments() > 0) {
    ...
    if (CODE_function.IsGeneric()) {
      prologue += LoadLocal(fn_type_args);

      prologue += LoadLocal(closure);
      prologue += LoadNativeField(Slot::Closure_function_type_arguments());

      prologue += IntConstant(CODE_function.NumParentTypeArguments());

      prologue += IntConstant(CODE_function.NumTypeArguments());
...
Fragment StreamingFlowGraphBuilder::CheckStackOverflowInPrologue(
    const Function& CODE_function) {
  if (CODE_function.is_native()) return {};
  return B->CheckStackOverflowInPrologue(CODE_function.token_pos());
}
...
Fragment StreamingFlowGraphBuilder::SetupCapturedParameters(
  ...
  if (scope->num_context_variables() > 0) {
    ...
    intptr_t parameter_count = CODE_function.NumParameters();
...
Fragment StreamingFlowGraphBuilder::InitSuspendableFunction(
  ...
  Fragment body;
  if (CODE_function.IsAsyncFunction()) {
    ...
    body += Drop();
  } else if (CODE_function.IsAsyncGenerator()) {
    ASSERT(emitted_value_type != nullptr);
    auto& type_args = TypeArguments::ZoneHandle(Z, TypeArguments::New(1));
    type_args.SetTypeAt(0, *emitted_value_type);
    ...
    body += Drop();
  } else if (CODE_function.IsSyncGenerator()) {
    ASSERT(emitted_value_type != nullptr);
    auto& type_args = TypeArguments::ZoneHandle(Z, TypeArguments::New(1));
    type_args.SetTypeAt(0, *emitted_value_type);
...
Fragment StreamingFlowGraphBuilder::ShortcutForUserDefinedEquals(
  ...
  Fragment body;
  if ((CODE_function.NumParameters() == 2) &&
      (CODE_function.name() == Symbols::EqualOperator().ptr()) &&
      (CODE_function.Owner() != IG->object_store()->object_class())) {
    ...
    null_fragment += Return(CODE_function.end_token_pos());
...
Fragment StreamingFlowGraphBuilder::BuildFunctionBody(
  ...
  if (constructor) {
    body += BuildInitializers(Class::Handle(Z, CODE_function.Owner()));
  }
  ...
  if (CODE_function.is_old_native()) {
    body += B->NativeFunctionBody(CODE_function, first_parameter);
  } else if (CODE_function.is_ffi_native()) {
    body += B->FfiNativeFunctionBody(CODE_function);
  } else if (CODE_function.is_external()) {
    body += ThrowNoSuchMethodError(TokenPosition::kNoSource, CODE_function,
  ...
  if (body.is_open()) {
    ...
    body += Return(CODE_function.end_token_pos());
  }
...
Fragment StreamingFlowGraphBuilder::BuildRegularFunctionPrologue(
    ...
    LocalVariable* first_parameter) {
  Fragment F;
  F += CheckStackOverflowInPrologue(CODE_function);
  F += DebugStepCheckInPrologue(CODE_function, token_position);
  F += B->InitConstantParameters();
  F += SetupCapturedParameters(CODE_function);
  F += ShortcutForUserDefinedEquals(CODE_function, first_parameter);
  return F;
}
...
Fragment StreamingFlowGraphBuilder::ClearRawParameters(
  ...
  const ParsedFunction& pf = *flow_graph_builder_->parsed_function_;
  Fragment code;
  for (intptr_t i = 0; i < CODE_function.NumParameters(); ++i) {
    LocalVariable* variable = pf.ParameterVariable(i);
...
UncheckedEntryPointStyle StreamingFlowGraphBuilder::ChooseEntryPointStyle(
  ...
  ASSERT(!CODE_function.IsImplicitClosureFunction());
  if (!CODE_function.MayHaveUncheckedEntryPoint() ||
  ...
  //
  if (!PrologueBuilder::HasEmptyPrologue(CODE_function) ||
...
FlowGraph* StreamingFlowGraphBuilder::BuildGraphOfFunction(
    bool is_constructor) {
  const Function& CODE_function = parsed_function()->function();
  ...
  const AbstractType* emitted_value_type = nullptr;
  {
    ...
    if (CODE_function.IsSuspendableFunction()) {
      function_node_helper.ReadUntilExcluding(
  ...
  const Fragment regular_prologue = BuildRegularFunctionPrologue(
      CODE_function, token_position, first_parameter);
  ...
  // regular methods.
  const Fragment type_args_handling = TypeArgumentsHandling(CODE_function);

  Fragment implicit_type_checks;
  if (CODE_function.NeedsTypeArgumentTypeChecks()) {
  ...
  if (CODE_function.NeedsArgumentTypeChecks()) {
  ...
  const Fragment body =
      ClearRawParameters(CODE_function) +
      InitSuspendableFunction(CODE_function, emitted_value_type) +
      BuildFunctionBody(CODE_function, first_parameter, is_constructor);

  auto extra_entry_point_style =
      ChooseEntryPointStyle(CODE_function, implicit_type_checks,
...
Fragment StreamingFlowGraphBuilder::BuildListLiteral(TokenPosition* p) {
  ...
  // (pkg/src/lib/transformations/list_literals_lowering.dart)
...
Fragment StreamingFlowGraphBuilder::BuildPartialTearoffInstantiation(
  ...
  const Library& CODE_internal = Library::Handle(Z, Library::InternalLibrary());
  const Function& bounds_check_function = Function::ZoneHandle(
      Z, CODE_internal.LookupFunctionAllowPrivate(

...
#if defined(USING_SIMULATOR) && !defined(CODE_PRECOMPILER)
...
void StubCodeCompiler::GenerateInvokeDartCodeStub() {
  ...
  __ lx(TMP2, Address(A3, target::Thread::invoke_CODE_code_stub_offset()));
  __ sx(TMP2, Address(SP, 0 * target::kWordSize));

#if defined(CODE_TARGET_OS_FUCHSIA) || defined(CODE_TARGET_OS_ANDROID)
...
#error Unimplemented
  ...
  // of TransitionGeneratedTosrc/Native that is part of runtime boilerplate

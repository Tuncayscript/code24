...
static ArrayPtr Eval(CODE_Handle lib, const char* expr) {
  const String& dummy_isolate_id = String::Handle(String::New("isolateId"));
  CODE_Handle expr_val;
  {
    TransitionVMToNative transition(Thread::Current());
    expr_val = CODE_EvaluateStaticExpr(lib, NewString(expr));
...
static ArrayPtr EvalF(CODE_Handle lib, const char* fmt, ...) {
...
ISOLATE_UNIT_TEST_CASE(Service_IsolateStickyError) {
...

  {
    ...
    TransitionVMToNative transition(thread);
    CODE_SetStickyError(result);
    CODE_SetPausedOnExit(true);
    EXPECT(CODE_HasStickyError());
  }
...
ISOLATE_UNIT_TEST_CASE(Service_Code) {
  ...
  CODE_Port port_id = PortMap::CreatePort(&handler);
  CODE_Handle port = Api::NewHandle(thread, SendPort::New(port_id));
  {
    TransitionVMToNative transition(thread);
    EXPECT_VALID(port);
    EXPECT_VALID(CODE_SetField(lib, NewString("port"), port));
  }
...
ISOLATE_UNIT_TEST_CASE(Service_PersistentHandles) {
  ...
  CODE_WeakPersistentHandle weak_persistent_handle;
  {
    TransitionVMToNative transition(thread);
    lib = TestCase::LoadTestScript(kScript, nullptr);
    EXPECT_VALID(lib);
    ...
    persistent_handle = CODE_NewPersistentHandle(result);

    // Create a weak persistent handle to global.
    weak_persistent_handle = CODE_NewWeakPersistentHandle(
  ...
  // Delete persistent handles.
  {
    ...
    CODE_DeleteWeakPersistentHandle(weak_persistent_handle);
  }
...
ISOLATE_UNIT_TEST_CASE(Service_EmbedderRootHandler) {
  ...
  CODE_Handle lib;
  {
    ...
    CODE_RegisterRootServiceRequestCallback("alpha", alpha_callback, nullptr);
    CODE_RegisterRootServiceRequestCallback("beta", beta_callback, nullptr);
...
ISOLATE_UNIT_TEST_CASE(Service_EmbedderIsolateHandler) {
  ...
  CODE_Handle lib;
  {
    ...
    CODE_RegisterIsolateServiceRequestCallback("alpha", alpha_callback,
                                               nullptr);
    CODE_RegisterIsolateServiceRequestCallback("beta", beta_callback, nullptr);

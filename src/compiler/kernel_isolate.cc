...
const char* KernelIsolate::kName = CODE_KERNEL_ISOLATE_NAME;
CODE_IsolateGroupCreateCallback KernelIsolate::create_group_callback_ = nullptr;
...
CODE_Port KernelIsolate::kernel_port_ = ILLEGAL_PORT;
...
class RunKernelTask : public ThreadPool::Task {
 public:
  virtual void Run() {
    ...
    CODE_IsolateGroupCreateCallback create_group_callback =
    ...
    if (isolate == nullptr) {
      if (FLAG_trace_kernel) {
        OS::PrintErr(CODE_KERNEL_ISOLATE_NAME ": Isolate creation error: %s\n",
                     error);
      }
      free(error);
      error = nullptr;
 ...
 protected:
  static void ShutdownIsolate(uword parameter) {
    if (FLAG_trace_kernel) {
      OS::PrintErr(CODE_KERNEL_ISOLATE_NAME ": ShutdownIsolate\n");
    }
    ...
    CODE_EnterIsolate(reinterpret_cast<CODE_Isolate>(parameter));
    {
      ...
      if (!error.IsNull() && !error.IsUnwindError()) {
        OS::PrintErr(CODE_KERNEL_ISOLATE_NAME ": Error: %s\n",
    ...
    if (FLAG_trace_kernel) {
      OS::PrintErr(CODE_KERNEL_ISOLATE_NAME ": Shutdown.\n");
    }
  ...
  bool RunMain(Isolate* I) {
    ...
    if (root_library.IsNull()) {
      OS::PrintErr(CODE_KERNEL_ISOLATE_NAME
...
void KernelIsolate::InitializeState() {
  ...
  if (FLAG_trace_kernel) {
    OS::PrintErr(CODE_KERNEL_ISOLATE_NAME ": InitializeState\n");
  }
...
bool KernelIsolate::Start() {
  if (create_group_callback_ == nullptr) {
    if (FLAG_trace_kernel) {
      OS::PrintErr(CODE_KERNEL_ISOLATE_NAME
                   ...
                   "CODE_InitializeParams property 'start_kernel_isolate' "
                   "to true\n");
    }
    return false;
  }
  bool start_task = false;
  {
    ...
    if (state_ == kNotStarted) {
      if (FLAG_trace_kernel) {
        OS::PrintErr(CODE_KERNEL_ISOLATE_NAME ": Start\n");
      }
      start_task = true;
      state_ = kStarting;
      ml.NotifyAll();
...
void KernelIsolate::InitCallback(Isolate* I) {
  ...
  ASSERT(!Exists());
  if (FLAG_trace_kernel) {
    OS::PrintErr(CODE_KERNEL_ISOLATE_NAME ": InitCallback for %s.\n",
                 I->name());
  }
  SetKernelIsolate(I);
}
...
void KernelIsolate::SetLoadPort(CODE_Port port) {
  MonitorLocker ml(monitor_);
  kernel_port_ = port;
  ml.NotifyAll();
}
...
CODE_Port KernelIsolate::WaitForKernelPort() {
...
static CODE_CObject BuildFilesPairs(int source_files_count,
                                    CODE_SourceFile source_files[]) {
  CODE_CObject files;
  files.type = CODE_CObject_kArray;
  files.value.as_array.length = source_files_count * 2;
  // typedef CODE_CObject* CODE_CObjectPtr;
  CODE_CObject** fileNamePairs = new CODE_CObject*[source_files_count * 2];
  for (int i = 0; i < source_files_count; i++) {
    CODE_CObject* source_uri = new CODE_CObject();
    source_uri->type = CODE_CObject_kString;
    source_uri->value.as_string = const_cast<char*>(source_files[i].uri);
    fileNamePairs[i * 2] = source_uri;
    CODE_CObject* source_code = new CODE_CObject();

    if (source_files[i].source != nullptr) {
      source_code->type = CODE_CObject_kTypedData;
      source_code->value.as_typed_data.type = CODE_TypedData_kUint8;
      ...
      source_code->type = CODE_CObject_kNull;
    }
...
static void ReleaseFilesPairs(const CODE_CObject& files) {
...
class KernelCompilationRequest : public ValueObject {
 public:
  KernelCompilationRequest()
      : monitor_(),
        port_(CODE_NewNativePort("kernel-compilation-port",
                                 &HandleResponse,
                                 false)),
        next_(nullptr),
        prev_(nullptr) {
    RegisterRequest(this);
    result_.status = CODE_KernelCompilationStatus_Unknown;
  ...
  ~KernelCompilationRequest() {
    ...
    if (port_ != ILLEGAL_PORT) {
      CODE_CloseNativePort(port_);
    }
  }

  intptr_t setDillData(CODE_CObject** dills_array,
                       intptr_t dill_num,
                       const uint8_t* buffer,
                       intptr_t buffer_size) {
    if (buffer != nullptr) {
      dills_array[dill_num] = new CODE_CObject;
      dills_array[dill_num]->type = CODE_CObject_kExternalTypedData;
  ...
  CODE_KernelCompilationResult SendAndWaitForResponse(
      CODE_Port kernel_port,
    ...
    if (port_ == ILLEGAL_PORT) {
      ...
      result.status = CODE_KernelCompilationStatus_MsgFailed;
      result.error =
    ...
    CODE_CObject CODE_platform_kernel;
    if (platform_kernel != nullptr) {
      CODE_platform_kernel.type = CODE_CObject_kExternalTypedData;
      CODE_platform_kernel.value.as_external_typed_data.type =
          CODE_TypedData_kUint8;
      CODE_platform_kernel.value.as_external_typed_data.length =
          platform_kernel_size;
      CODE_platform_kernel.value.as_external_typed_data.data =
          const_cast<uint8_t*>(platform_kernel);
      CODE_platform_kernel.value.as_external_typed_data.peer =
          const_cast<uint8_t*>(platform_kernel);
      CODE_platform_kernel.value.as_external_typed_data.callback =
          PassThroughFinalizer;
    } else {
      // If nullptr, the kernel service looks up the platform dill file
      // next to the executable.
      CODE_platform_kernel.type = CODE_CObject_kNull;
    }

    CODE_CObject expression_object;
    expression_object.type = CODE_CObject_kString;
    expression_object.value.as_string = const_cast<char*>(expression);

    CODE_CObject definitions_object;
    intptr_t num_definitions = definitions.Length();
    definitions_object.type = CODE_CObject_kArray;
    definitions_object.value.as_array.length = num_definitions;

    CODE_CObject** definitions_array = new CODE_CObject*[num_definitions];
    for (intptr_t i = 0; i < num_definitions; ++i) {
      definitions_array[i] = new CODE_CObject;
      definitions_array[i]->type = CODE_CObject_kString;
    ...
    CODE_CObject definition_types_object;
    intptr_t num_definition_types = definition_types.Length();
    definition_types_object.type = CODE_CObject_kArray;
    definition_types_object.value.as_array.length = num_definition_types;

    CODE_CObject** definition_types_array =
        new CODE_CObject*[num_definition_types];
    for (intptr_t i = 0; i < num_definition_types; ++i) {
      definition_types_array[i] = new CODE_CObject;
      definition_types_array[i]->type = CODE_CObject_kString;
    ...
    CODE_CObject type_definitions_object;
    intptr_t num_type_definitions = type_definitions.Length();
    type_definitions_object.type = CODE_CObject_kArray;
    type_definitions_object.value.as_array.length = num_type_definitions;

    CODE_CObject** type_definitions_array =
        new CODE_CObject*[num_type_definitions];
    for (intptr_t i = 0; i < num_type_definitions; ++i) {
      type_definitions_array[i] = new CODE_CObject;
      type_definitions_array[i]->type = CODE_CObject_kString;
    ...
    CODE_CObject type_bounds_object;
    intptr_t num_type_bounds = type_bounds.Length();
    type_bounds_object.type = CODE_CObject_kArray;
    type_bounds_object.value.as_array.length = num_type_bounds;

    CODE_CObject** type_bounds_array = new CODE_CObject*[num_type_bounds];
    for (intptr_t i = 0; i < num_type_bounds; ++i) {
      type_bounds_array[i] = new CODE_CObject;
      type_bounds_array[i]->type = CODE_CObject_kString;
    ...
    CODE_CObject type_defaults_object;
    intptr_t num_type_defaults = type_defaults.Length();
    type_defaults_object.type = CODE_CObject_kArray;
    type_defaults_object.value.as_array.length = num_type_defaults;

    CODE_CObject** type_defaults_array = new CODE_CObject*[num_type_defaults];
    for (intptr_t i = 0; i < num_type_defaults; ++i) {
      type_defaults_array[i] = new CODE_CObject;
      type_defaults_array[i]->type = CODE_CObject_kString;
    ...
    CODE_CObject library_uri_object;
    library_uri_object.type = CODE_CObject_kString;
    library_uri_object.value.as_string = const_cast<char*>(library_uri);

    CODE_CObject class_object;
    if (klass != nullptr) {
      class_object.type = CODE_CObject_kString;
      class_object.value.as_string = const_cast<char*>(klass);
    } else {
      class_object.type = CODE_CObject_kNull;
    }

    CODE_CObject method_object;
    if (method != nullptr) {
      method_object.type = CODE_CObject_kString;
      method_object.value.as_string = const_cast<char*>(method);
    } else {
      method_object.type = CODE_CObject_kNull;
    }

    CODE_CObject is_static_object;
    is_static_object.type = CODE_CObject_kBool;
    is_static_object.value.as_bool = is_static;

    CODE_CObject token_pos_object;
    token_pos_object.type = CODE_CObject_kInt64;
    token_pos_object.value.as_int64 = token_pos;

    CODE_CObject script_uri_object;
    if (script_uri != nullptr) {
      script_uri_object.type = CODE_CObject_kString;
      script_uri_object.value.as_string = const_cast<char*>(script_uri);
    } else {
      script_uri_object.type = CODE_CObject_kNull;
    }

    auto isolate_group = thread->isolate_group();
    auto source = isolate_group->source();

    CODE_CObject isolate_id;
    isolate_id.type = CODE_CObject_kInt64;
    ...
    CODE_CObject dills_object;
    dills_object.type = CODE_CObject_kArray;
    dills_object.value.as_array.length = num_dills;

    CODE_CObject** dills_array = new CODE_CObject*[num_dills];
    intptr_t dill_num = 0;
    ...
    CODE_CObject num_blob_loads;
    num_blob_loads.type = CODE_CObject_kInt64;
    num_blob_loads.value.as_int64 = source->num_blob_loads_;

    CODE_CObject enable_asserts;
    enable_asserts.type = CODE_CObject_kBool;
    enable_asserts.value.as_bool = isolate_group->asserts();

    intptr_t num_experimental_flags = experimental_flags->length();
    CODE_CObject** experimental_flags_array =
        new CODE_CObject*[num_experimental_flags];
    for (intptr_t i = 0; i < num_experimental_flags; ++i) {
      experimental_flags_array[i] = new CODE_CObject;
      experimental_flags_array[i]->type = CODE_CObject_kString;
      experimental_flags_array[i]->value.as_string = (*experimental_flags)[i];
    }
    CODE_CObject experimental_flags_object;
    experimental_flags_object.type = CODE_CObject_kArray;
    ...
    CODE_CObject enable_mirrors;
    enable_mirrors.type = CODE_CObject_kBool;
    enable_mirrors.value.as_bool = FLAG_enable_mirrors;

    CODE_CObject message;
    message.type = CODE_CObject_kArray;
    CODE_CObject* message_arr[] = {&tag,
                                   &send_port,
                                   &isolate_id,
                                   &CODE_platform_kernel,
                                   &expression_object,
                                   &definitions_object,
...

    {
      ...
      // Send the message.
      CODE_PostCObject(kernel_port, &message);

      // Wait for reply to arrive.
      VMTagScope tagScope(thread, VMTag::kLoadWaitTagId);
      MonitorLocker ml(&monitor_);
      while (result_.status == CODE_KernelCompilationStatus_Unknown) {
        ml.Wait();
      }
    }
  ...
  CODE_KernelCompilationResult SendAndWaitForResponse(
    ...
    CODE_CObject uri;
    if (script_uri != nullptr) {
      uri.type = CODE_CObject_kString;
      uri.value.as_string = const_cast<char*>(script_uri);
    } else {
      uri.type = CODE_CObject_kNull;
    }
    ...
    CODE_CObject CODE_incremental;
    CODE_incremental.type = CODE_CObject_kBool;
    CODE_incremental.value.as_bool = incremental_compile;

    CODE_CObject CODE_snapshot;
    CODE_snapshot.type = CODE_CObject_kBool;
    CODE_snapshot.value.as_bool = for_snapshot;

    CODE_CObject CODE_embed_sources;
    CODE_embed_sources.type = CODE_CObject_kBool;
    CODE_embed_sources.value.as_bool = embed_sources;
    ...
    CODE_CObject files = BuildFilesPairs(source_files_count, source_files);
    ...
    CODE_CObject sound_null_safety;
    sound_null_safety.type = CODE_CObject_kBool;
    ...
    CODE_CObject package_config_uri;
    if (package_config != nullptr) {
      package_config_uri.type = CODE_CObject_kString;
      package_config_uri.value.as_string = const_cast<char*>(package_config);
    } else {
      package_config_uri.type = CODE_CObject_kNull;
    }

    CODE_CObject multiroot_filepaths_object;
    {
      const char* filepaths = multiroot_filepaths != nullptr
                                  ? multiroot_filepaths
                                  : FLAG_kernel_multiroot_filepaths;
      if (filepaths != nullptr) {
        multiroot_filepaths_object.type = CODE_CObject_kString;
        multiroot_filepaths_object.value.as_string =
            const_cast<char*>(filepaths);
      } else {
        multiroot_filepaths_object.type = CODE_CObject_kNull;
      }
    }

    CODE_CObject multiroot_scheme_object;
    {
      const char* scheme = multiroot_scheme != nullptr
                               ? multiroot_scheme
                               : FLAG_kernel_multiroot_scheme;
      if (scheme != nullptr) {
        multiroot_scheme_object.type = CODE_CObject_kString;
        multiroot_scheme_object.value.as_string = const_cast<char*>(scheme);
      } else {
        multiroot_scheme_object.type = CODE_CObject_kNull;
      }
    }

    CODE_CObject original_working_directory_object;
    {
      if (original_working_directory != nullptr) {
        original_working_directory_object.type = CODE_CObject_kString;
        original_working_directory_object.value.as_string =
            const_cast<char*>(original_working_directory);
      } else {
        original_working_directory_object.type = CODE_CObject_kNull;
      }
    }

    CODE_CObject verbosity_str;
    verbosity_str.type = CODE_CObject_kString;
 ...
 private:
  void LoadKernelFromResponse(CODE_CObject* response) {
    ASSERT((response->type == CODE_CObject_kTypedData) ||
           (response->type == CODE_CObject_kNull));

    if (response->type == CODE_CObject_kNull) {
      return;
    }
    ASSERT(response->value.as_typed_data.type == CODE_TypedData_kUint8);
  ...
  //     [Crash, String error]
  //
  void HandleResponseImpl(CODE_CObject* message) {
    ASSERT(message->type == CODE_CObject_kArray);
    ASSERT(message->value.as_array.length >= 1);

    CODE_CObject** response = message->value.as_array.values;

    MonitorLocker ml(&monitor_);

    ASSERT(response[0]->type == CODE_CObject_kInt32);
    result_.status = static_cast<CODE_KernelCompilationStatus>(
        message->value.as_array.values[0]->value.as_int32);

    if (result_.status == CODE_KernelCompilationStatus_Ok) {
      LoadKernelFromResponse(response[1]);
    } else {
      if (result_.status == CODE_KernelCompilationStatus_Error) {
        LoadKernelFromResponse(response[2]);
      }
      // This is an error.
      ASSERT(response[1]->type == CODE_CObject_kString);
      result_.error = Utils::StrDup(response[1]->value.as_string);
    }
    ml.Notify();
  }

  static void HandleResponse(CODE_Port port, CODE_CObject* message) {
  ...
  static KernelCompilationRequest* FindRequestLocked(CODE_Port port) {
  ...
  static const char* KernelCompilationVerbosityLevelToString(
      CODE_KernelCompilationVerbosityLevel verbosity) {
    switch (verbosity) {
      case CODE_KernelCompilationVerbosityLevel_Error:
        return "error";
      case CODE_KernelCompilationVerbosityLevel_Warning:
        return "warning";
      case CODE_KernelCompilationVerbosityLevel_Info:
        return "info";
      case CODE_KernelCompilationVerbosityLevel_All:
        return "all";
      default:
        UNREACHABLE();
    }
  ...
  CODE_KernelCompilationResult result_ = {};
};
...
CODE_KernelCompilationResult KernelIsolate::CompileToKernel(
    const char* script_uri,
    const uint8_t* platform_kernel,
    intptr_t platform_kernel_size,
  ...
  CODE_Port kernel_port = WaitForKernelPort();
...
CODE_KernelCompilationResult KernelIsolate::ListDependencies() {
  CODE_Port kernel_port = WaitForKernelPort();
  if (kernel_port == ILLEGAL_PORT) {
    CODE_KernelCompilationResult result = {};
    result.status = CODE_KernelCompilationStatus_MsgFailed;
  ...
  return request.SendAndWaitForResponse(
      ...
      CODE_KernelCompilationVerbosityLevel_Error);
}

CODE_KernelCompilationResult KernelIsolate::AcceptCompilation() {
  // This must be the main script to be loaded. Wait for Kernel isolate
  // to finish initialization.
  CODE_Port kernel_port = WaitForKernelPort();
  if (kernel_port == ILLEGAL_PORT) {
...
CODE_KernelCompilationResult KernelIsolate::RejectCompilation() {
  // This must be the main script to be loaded. Wait for Kernel isolate
  // to finish initialization.
  CODE_Port kernel_port = WaitForKernelPort();
  if (kernel_port == ILLEGAL_PORT) {
...
CODE_KernelCompilationResult KernelIsolate::CompileExpressionToKernel(
    const uint8_t* platform_kernel,
    intptr_t platform_kernel_size,
    const char* expression,
    const Array& definitions,
...
CODE_KernelCompilationResult KernelIsolate::UpdateInMemorySources(
    int source_files_count,
    CODE_SourceFile source_files[]) {
  // This must be the main script to be loaded. Wait for Kernel isolate
  // to finish initialization.
  ...
  return request.SendAndWaitForResponse(
      ...
      experimental_flags_, nullptr, CODE_KernelCompilationVerbosityLevel_Error);
}
...
void KernelIsolate::NotifyAboutIsolateGroupShutdown(
  ...
  CODE_CObject message;
  message.type = CODE_CObject_kArray;
  CODE_CObject* message_arr[] = {&tag, &isolate_id};
  message.value.as_array.values = message_arr;
  message.value.as_array.length = ARRAY_SIZE(message_arr);
  // Send the message.
  CODE_PostCObject(kernel_port, &message);

...
#include "src/v8_snapshot_writer.h"
#include "src/version.h"
#include "src/zone_text_buffer.h"

#if !defined(CODE_PRECOMPILED_RUNTIME)
#include "src/compiler/backend/code_statistics.h"
#include "src/compiler/backend/il_printer.h"
#include "src/compiler/relocation.h"
...
class Serializer : public ThreadStackResource {
 public:
  ...
  template <typename T>
  CODE_NOINLINE void WriteRange(ObjectPtr obj, T from, T to) {
  ...
  template <typename T>
  CODE_NOINLINE void PushRange(ObjectPtr obj, T from, T to) {
    for (auto* p = from; p <= to; p++) {
...
#if defined(CODE_PRECOMPILER) && !defined(PRODUCT)
...
CODE_FORCE_INLINE static CodePtr GetCodeAndEntryPointByIndex(
...
class CodeDeserializationCluster : public DeserializationCluster {
 public:
  ...
  void PostLoad(Deserializer* d, const Array& refs) override {
    ...
    for (intptr_t id = start_index_, n = stop_index_; id < n; id++) {
      code ^= refs.At(id);
#if !defined(CODE_PRECOMPILED_RUNTIME) && !defined(PRODUCT)
...
#if !defined(CODE_PRECOMPILED_RUNTIME) && !defined(CODE_COMPRESSED_POINTERS)
...
#endif  // !CODE_PRECOMPILED_RUNTIME && !CODE_COMPRESSED_POINTERS
...
#endif  // !CODE_COMPRESSED_POINTERS

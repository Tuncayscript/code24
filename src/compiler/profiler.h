...
class SampleVisitor : public ValueObject {
 public:
  explicit SampleVisitor(CODE_Port port) : port_(port), visited_(0) {}
  ...
  CODE_Port port() const { return port_; }
...
class Sample {
 public:
  ...
  void Init(CODE_Port port, int64_t timestamp, ThreadId tid) {
    Clear();

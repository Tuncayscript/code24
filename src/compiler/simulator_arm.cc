...
CODE_FORCE_INLINE void Simulator::set_register(Register reg, int32_t value) {
...
CODE_FORCE_INLINE void Simulator::set_pc(int32_t value) {
  pc_modified_ = true;
  registers_[PC] = value;
}

// Accessors for VFP register state.
CODE_FORCE_INLINE void Simulator::set_sregister(SRegister reg, float value) {
  ASSERT((reg >= 0) && (reg < kNumberOfSRegisters));
  sregisters_[reg] = bit_cast<int32_t, float>(value);
}

CODE_FORCE_INLINE float Simulator::get_sregister(SRegister reg) const {
  ASSERT((reg >= 0) && (reg < kNumberOfSRegisters));
  return bit_cast<float, int32_t>(sregisters_[reg]);
}

CODE_FORCE_INLINE void Simulator::set_dregister(DRegister reg, double value) {
  ASSERT((reg >= 0) && (reg < kNumberOfDRegisters));
  dregisters_[reg] = bit_cast<int64_t, double>(value);
}

CODE_FORCE_INLINE double Simulator::get_dregister(DRegister reg) const {
  ASSERT((reg >= 0) && (reg < kNumberOfDRegisters));
...
CODE_FORCE_INLINE intptr_t Simulator::ReadW(uword addr, Instr* instr) {
  return *reinterpret_cast<intptr_t*>(addr);
}

CODE_FORCE_INLINE void Simulator::WriteW(uword addr,
                                         intptr_t value,
                                         Instr* instr) {
  *reinterpret_cast<intptr_t*>(addr) = value;
}

CODE_FORCE_INLINE uint16_t Simulator::ReadHU(uword addr, Instr* instr) {
  return *reinterpret_cast<uint16_t*>(addr);
}

CODE_FORCE_INLINE int16_t Simulator::ReadH(uword addr, Instr* instr) {
  return *reinterpret_cast<int16_t*>(addr);
}

CODE_FORCE_INLINE void Simulator::WriteH(uword addr,
                                         uint16_t value,
                                         Instr* instr) {
  *reinterpret_cast<uint16_t*>(addr) = value;
}

CODE_FORCE_INLINE uint8_t Simulator::ReadBU(uword addr) {
  return *reinterpret_cast<uint8_t*>(addr);
}

CODE_FORCE_INLINE int8_t Simulator::ReadB(uword addr) {
  return *reinterpret_cast<int8_t*>(addr);
}

CODE_FORCE_INLINE void Simulator::WriteB(uword addr, uint8_t value) {
...
// condition bits.
CODE_FORCE_INLINE bool Simulator::ConditionallyExecute(Instr* instr) {
...
CODE_FORCE_INLINE void Simulator::SetNZFlags(int32_t val) {
  n_flag_ = (val < 0);
  z_flag_ = (val == 0);
}

// Set the Carry flag.
CODE_FORCE_INLINE void Simulator::SetCFlag(bool val) {
  c_flag_ = val;
}

// Set the oVerflow flag.
CODE_FORCE_INLINE void Simulator::SetVFlag(bool val) {
  v_flag_ = val;
}

// Calculate C flag value for additions (and subtractions with adjusted args).
CODE_FORCE_INLINE bool Simulator::CarryFrom(int32_t left,
                                            int32_t right,
                                            int32_t carry) {
...
CODE_FORCE_INLINE bool Simulator::OverflowFrom(int32_t left,
                                               int32_t right,
                                               int32_t carry) {
...
CODE_FORCE_INLINE int32_t Simulator::GetImm(Instr* instr, bool* carry_out) {
...
typedef void (*SimulatorNativeCallWrapper)(CODE_NativeArguments arguments,
                                           CODE_NativeFunction target);
    ...
    case Instr::kSimulatorRedirectCode: {
      ...
      if (!setjmp(buffer.buffer_)) {
        ...
        if (redirection->call_kind() == kRuntimeCall) {
          ...
          CODE_NativeArguments arguments =
              reinterpret_cast<CODE_NativeArguments>(get_register(R0));
          CODE_NativeFunction target_func =
              reinterpret_cast<CODE_NativeFunction>(get_register(R1));
...
CODE_FORCE_INLINE void Simulator::DecodeType01(Instr* instr) {
...
CODE_FORCE_INLINE void Simulator::DecodeType2(Instr* instr) {
  Register rd = instr->RdField();
  Register rn = instr->RnField();
  int32_t rn_val = get_register(rn);
...
CODE_FORCE_INLINE void Simulator::InstructionDecodeImpl(Instr* instr) {
  pc_modified_ = false;

...
#include "src/frame_layout.h"
#include "src/object.h"
#include "src/stub_code.h"

#if defined(TARGET_ARCH_IA32)
#include "src/stack_frame_ia32.h"
#elif defined(TARGET_ARCH_X64)
#include "src/stack_frame_x64.h"
#elif defined(TARGET_ARCH_ARM)
#include "src/stack_frame_arm.h"
#elif defined(TARGET_ARCH_ARM64)
#include "src/stack_frame_arm64.h"
#elif defined(TARGET_ARCH_RISCV32) || defined(TARGET_ARCH_RISCV64)
#include "src/stack_frame_riscv.h"
#else
...
CODE_FORCE_INLINE static intptr_t LocalVarIndex(intptr_t fp_offset,
                                                intptr_t var_index) {
  return fp_offset + var_index;
}

CODE_FORCE_INLINE static uword ParamAddress(uword fp, intptr_t reverse_index) {
  return fp + (kParamEndSlotFromFp * kWordSize) + (reverse_index * kWordSize);
}

// Both fp and other_fp are compiled code frame pointers.
CODE_FORCE_INLINE static bool IsCalleeFrameOf(uword fp, uword other_fp) {
  return other_fp < fp;
}
...
CODE_FORCE_INLINE static uword LocalVarAddress(uword fp, intptr_t index) {

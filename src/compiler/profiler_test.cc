...
class ProfileSampleBufferTestHelper : public SampleVisitor {
 public:
  explicit ProfileSampleBufferTestHelper(CODE_Port port)
      : SampleVisitor(port) {}
...
static void Invoke(const Library& lib,
                   const char* name,
                   intptr_t argc = 0,
                   CODE_Handle* argv = nullptr) {
  Thread* thread = Thread::Current();
  CODE_Handle api_lib = Api::NewHandle(thread, lib.ptr());
  TransitionVMToNative transition(thread);
  CODE_Handle result = CODE_Invoke(api_lib, NewString(name), argc, argv);
  EXPECT_VALID(result);
}

class AllocationFilter : public SampleFilter {
 public:
  AllocationFilter(CODE_Port port,
                   intptr_t cid,
...
ISOLATE_UNIT_TEST_CASE(Profiler_TrivialRecordAllocation) {
  ...
  const int64_t before_allocations_micros = CODE_TimelineGetMicros();
  ...
  const int64_t after_allocations_micros = CODE_TimelineGetMicros();
...
ISOLATE_UNIT_TEST_CASE(Profiler_IntrinsicAllocation) {
  ...
  {
    TransitionVMToNative transition(thread);
    args[0] = CODE_NewDouble(1.0);
    args[1] = CODE_NewDouble(2.0);
  }

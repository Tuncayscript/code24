...
#if defined(CODE_HOST_OS_ANDROID) || defined(CODE_HOST_OS_LINUX)
...
#endif  // defined(CODE_HOST_OS_ANDROID) || defined(CODE_HOST_OS_LINUX)
...
static CODE_Port expected_isolate;
static CODE_IsolateGroupId expected_isolate_group;
static bool saw_begin;
static bool saw_end;
static void* expected_isolate_data;
static void* expected_isolate_group_data;

static void TestTimelineRecorderCallback(CODE_TimelineRecorderEvent* event) {
  EXPECT_EQ(CODE_TIMELINE_RECORDER_CURRENT_VERSION, event->version);

  if ((event->type == CODE_Timeline_Event_Begin) &&
  ...
  if ((event->type == CODE_Timeline_Event_End) &&
      (strcmp(event->label, "TestEvent") == 0)) {
    saw_end = true;
    EXPECT_NE(0, event->timestamp0);
    EXPECT_EQ(expected_isolate, event->isolate);
...
UNIT_TEST_CASE(DartAPI_SetTimelineRecorderCallback) {
  ...
  CODE_SetTimelineRecorderCallback(TestTimelineRecorderCallback);

  EXPECT(CODE_SetVMFlags(argc, argv) == nullptr);
  CODE_InitializeParams params;
  memset(&params, 0, sizeof(CODE_InitializeParams));
  params.version = CODE_INITIALIZE_PARAMS_CURRENT_VERSION;
  ...
  EXPECT(CODE_Initialize(&params) == nullptr);
  {
    ...
    expected_isolate = CODE_GetMainPortId();
    EXPECT_NE(ILLEGAL_PORT, expected_isolate);
    expected_isolate_group = CODE_CurrentIsolateGroupId();
    EXPECT_NE(ILLEGAL_PORT, expected_isolate_group);
    expected_isolate_data = &isolate_data;
    EXPECT_EQ(expected_isolate_data, CODE_CurrentIsolateData());
    expected_isolate_group_data = CODE_CurrentIsolateGroupData();
    saw_begin = false;
    saw_end = false;
  ...
  EXPECT(CODE_Cleanup() == nullptr);

  CODE_SetTimelineRecorderCallback(nullptr);

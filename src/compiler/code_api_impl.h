#ifndef RUNTIME_VM_CODE_API_IMPL_H_
#define RUNTIME_VM_CODE_API_IMPL_H_
  ...
  do {                                                                         \
    if ((isolate_group) == nullptr) {                                          \
      FATAL(                                                                   \
          ...
          "forget to call CODE_CreateIsolateGroup or CODE_EnterIsolate?",      \
          CURRENT_FUNC);                                                       \
    }                                                                          \
  } while (0)
  ...
  do {                                                                         \
    if ((isolate) != nullptr) {                                                \
      FATAL(                                                                   \
          ...
          "forget to call CODE_ExitIsolate?",                                  \
          CURRENT_FUNC);                                                       \
    }                                                                          \
  } while (0)
...
#define RETURN_TYPE_ERROR(zone, CODE_handle, type)                             \
  do {                                                                         \
    const Object& tmp =                                                        \
        Object::Handle(zone, Api::UnwrapHandle((CODE_handle)));                \
    if (tmp.IsNull()) {                                                        \
      return Api::NewArgumentError("%s expects argument '%s' to be non-null.", \
                                   CURRENT_FUNC, #CODE_handle);                \
    } else if (tmp.IsError()) {                                                \
      return CODE_handle;                                                      \
    }                                                                          \
    return Api::NewArgumentError("%s expects argument '%s' to be of type %s.", \
                                 CURRENT_FUNC, #CODE_handle, #type);           \
  } while (0)
...
class Api : AllStatic {
 public:
  ...
  static CODE_Handle NewHandle(Thread* thread, ObjectPtr raw);

  // Unwraps the raw object from the handle.
  static ObjectPtr UnwrapHandle(CODE_Handle object);
...
#define DECLARE_UNWRAP(Type)                                                   \
  static const Type& Unwrap##Type##Handle(Zone* zone, CODE_Handle object);
...
#undef DECLARE_UNWRAP
  ...
  static CODE_Handle CheckAndFinalizePendingClasses(Thread* thread);

  // Casts the internal Isolate* type to the external CODE_Isolate type.
  static CODE_Isolate CastIsolate(Isolate* isolate);

  // Casts the internal IsolateGroup* type to the external CODE_IsolateGroup
  // type.
  static CODE_IsolateGroup CastIsolateGroup(IsolateGroup* isolate_group);

  // Gets the handle used to designate successful return.
  static CODE_Handle Success() { return Api::True(); }

  // Returns true if the handle holds a Smi.
  static bool IsSmi(CODE_Handle handle) {
  ...
  static intptr_t SmiValue(CODE_Handle handle) {
    // Important: we do not require current thread to be in VM state because
    // we do not dereference the handle.
  ...
  static CODE_Handle NewError(const char* format, ...) PRINTF_ATTRIBUTE(1, 2);
  static CODE_Handle NewArgumentError(const char* format, ...)
      PRINTF_ATTRIBUTE(1, 2);

  // Gets a handle to Null.
  static CODE_Handle Null() { return null_handle_; }

  // Gets a handle to True.
  static CODE_Handle True() { return true_handle_; }

  // Gets a handle to False.
  static CODE_Handle False() { return false_handle_; }

  // Gets a handle to EmptyString.
  static CODE_Handle EmptyString() { return empty_string_handle_; }

  // Gets the handle which holds the pre-created acquired error object.
  static CODE_Handle NoCallbacksError() { return no_callbacks_error_handle_; }

  // Gets the handle for unwind-is-in-progress error.
  static CODE_Handle UnwindInProgressError() {
    return unwind_in_progress_error_handle_;
  }

  static bool IsProtectedHandle(CODE_Handle object) {
  ...
  static void SetReturnValue(NativeArguments* args, CODE_Handle retval) {
 ...
 private:
  static CODE_Handle InitNewHandle(Thread* thread, ObjectPtr raw);
  ...
  static CODE_Handle true_handle_;
  static CODE_Handle false_handle_;
  static CODE_Handle null_handle_;
  static CODE_Handle empty_string_handle_;
  static CODE_Handle no_callbacks_error_handle_;
  static CODE_Handle unwind_in_progress_error_handle_;
...
#define CHECK_CALLBACK_STATE(thread)                                           \
  if (thread->no_callback_scope_depth() != 0) {                                \
    return reinterpret_cast<CODE_Handle>(Api::NoCallbacksError());             \
  }                                                                            \
  if (thread->is_unwind_in_progress()) {                                       \
    return reinterpret_cast<CODE_Handle>(Api::UnwindInProgressError());        \
  }
...
#endif  // RUNTIME_VM_CODE_API_IMPL_H_

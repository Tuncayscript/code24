...
#include "src/heap/sampler.h"
#include "src/os_thread.h"
#include "src/pending_deopts.h"
#include "src/random.h"
#include "src/runtime_entry_list.h"
#include "src/tags.h"
...
#define CACHED_VM_STUBS_LIST(V)                                                \
  ...
  V(CodePtr, invoke_CODE_code_stub_, StubCode::InvokeDartCode().ptr(),         \
    nullptr)                                                                   \
...
class Thread : public ThreadState {
 public:
  ...
  bool IsDartMutatorThread() const {
    return scheduled_CODE_mutator_isolate_ != nullptr;
  }
  ...
  Isolate* scheduled_CODE_mutator_isolate() const {
    return scheduled_CODE_mutator_isolate_;
  }
  ...
  static intptr_t CODE_stream_offset() {
    return OFFSET_OF(Thread, CODE_stream_);
  }
...
#undef REUSABLE_HANDLE
  ...
  bool IsValidLocalHandle(CODE_Handle object) const;
...
#undef DECLARE_MEMBERS
   ...
   * pending CODE_TypedDataAcquire() that increases no-callback-scope)
   */
  ...
  // on CODE_PRECOMPILED_RUNTIME, as it is accessed from generated code.
  // The code is generated without CODE_PRECOMPILED_RUNTIME, but used with
  // CODE_PRECOMPILED_RUNTIME.

  uword true_end_ = 0;
  TaskKind task_kind_;
  TimelineStream* const CODE_stream_;
...
#undef REUSABLE_HANDLE_SCOPE_VARIABLE
  ...
  Isolate* scheduled_CODE_mutator_isolate_ = nullptr;

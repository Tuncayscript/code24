...
//
//     V(when, name, bit-name, CODE_IsolateFlags-name, command-line-flag-name)
//
...
// List of Isolate flags with corresponding members of CODE_IsolateFlags and
...
class IsolateGroupSource {
 public:
  ...
  // `CODE_CreateIsolateGroupFromKernel` / `CODE_CreateIsolate`.
  char* script_uri;
  char* name;
  const uint8_t* snapshot_data;
  const uint8_t* snapshot_instructions;
  ...
  // The kernel buffer used in `CODE_LoadScriptFromKernel`.
...
#undef ISOLATE_METRIC_ACCESSOR
  ...
  static void FlagsInitialize(CODE_IsolateFlags* api_flags);
  void FlagsCopyTo(CODE_IsolateFlags* api_flags);
  void FlagsCopyFrom(const CODE_IsolateFlags& api_flags);
...
#undef DECLARE_GETTER
  ...
  ObjectPtr CallTagHandler(CODE_LibraryTag tag,
                           const Object& arg1,
                           const Object& arg2);
  CODE_LibraryTagHandler library_tag_handler() const {
    return library_tag_handler_;
  }
  void set_library_tag_handler(CODE_LibraryTagHandler handler) {
    library_tag_handler_ = handler;
  }
  CODE_DeferredLoadHandler deferred_load_handler() const {
    return deferred_load_handler_;
  }
  void set_deferred_load_handler(CODE_DeferredLoadHandler handler) {
...
#endif  // !defined(PRODUCT) && !defined(CODE_PRECOMPILED_RUNTIME)
...
#undef DECLARE_BITFIELD
  ...
  CODE_LibraryTagHandler library_tag_handler_ = nullptr;
  CODE_DeferredLoadHandler deferred_load_handler_ = nullptr;
...
class Bequest {
 public:
  Bequest(PersistentHandle* handle, CODE_Port beneficiary)
  ...
  CODE_Port beneficiary() { return beneficiary_; }

 private:
  PersistentHandle* handle_;
  CODE_Port beneficiary_;
};
...
class Isolate : public BaseIsolate, public IntrusiveDListEntry<Isolate> {
 public:
  ...
  CODE_MessageNotifyCallback message_notify_callback() const {
    return message_notify_callback_.load(std::memory_order_relaxed);
  }

  void set_message_notify_callback(CODE_MessageNotifyCallback value) {
    message_notify_callback_.store(value, std::memory_order_release);
  }

  void set_on_shutdown_callback(CODE_IsolateShutdownCallback value) {
    on_shutdown_callback_ = value;
  }
  CODE_IsolateShutdownCallback on_shutdown_callback() {
    return on_shutdown_callback_;
  }
  void set_on_cleanup_callback(CODE_IsolateCleanupCallback value) {
    on_cleanup_callback_ = value;
  }
  CODE_IsolateCleanupCallback on_cleanup_callback() {
  ...
  CODE_Port main_port() const { return main_port_; }
  void set_main_port(CODE_Port port) {
    ASSERT(main_port_ == 0);  // Only set main port once.
    main_port_ = port;
  }
  CODE_Port origin_id();
  void set_origin_id(CODE_Port id);
  ...
  static bool SendInternalLibMessage(CODE_Port main_port,
                                     LibMsgId msg_id,
                                     uint64_t capability);
  ...
  CODE_EnvironmentCallback environment_callback() const {
    return environment_callback_;
  }
  void set_environment_callback(CODE_EnvironmentCallback value) {
  ...
  static void SetCreateGroupCallback(CODE_IsolateGroupCreateCallback cb) {
    create_group_callback_ = cb;
  }
  static CODE_IsolateGroupCreateCallback CreateGroupCallback() {
    return create_group_callback_;
  }

  static void SetInitializeCallback_(CODE_InitializeIsolateCallback cb) {
    initialize_callback_ = cb;
  }
  static CODE_InitializeIsolateCallback InitializeCallback() {
    return initialize_callback_;
  }

  static void SetShutdownCallback(CODE_IsolateShutdownCallback cb) {
    shutdown_callback_ = cb;
  }
  static CODE_IsolateShutdownCallback ShutdownCallback() {
    return shutdown_callback_;
  }

  static void SetCleanupCallback(CODE_IsolateCleanupCallback cb) {
    cleanup_callback_ = cb;
  }
  static CODE_IsolateCleanupCallback CleanupCallback() {
    return cleanup_callback_;
  }

  static void SetGroupCleanupCallback(CODE_IsolateGroupCleanupCallback cb) {
    cleanup_group_callback_ = cb;
  }
  static CODE_IsolateGroupCleanupCallback GroupCleanupCallback() {
    return cleanup_group_callback_;
  }
  static void SetRegisterKernelBlobCallback(
      CODE_RegisterKernelBlobCallback cb) {
    register_kernel_blob_callback_ = cb;
  }
  static CODE_RegisterKernelBlobCallback RegisterKernelBlobCallback() {
    return register_kernel_blob_callback_;
  }
  static void SetUnregisterKernelBlobCallback(
      CODE_UnregisterKernelBlobCallback cb) {
    unregister_kernel_blob_callback_ = cb;
  }
  static CODE_UnregisterKernelBlobCallback UnregisterKernelBlobCallback() {
...
#undef ISOLATE_METRIC_ACCESSOR
  ...
  CODE_WARN_UNUSED_RESULT ErrorPtr StealStickyError();
  ...
  static void FlagsInitialize(CODE_IsolateFlags* api_flags);
  void FlagsCopyTo(CODE_IsolateFlags* api_flags) const;
  void FlagsCopyFrom(const CODE_IsolateFlags& api_flags);
...
#undef DECLARE_GETTER
  ...
  // is found.
  static Isolate* LookupIsolateByPort(CODE_Port port);

  // Lookup an isolate by its main port and return a copy of its name. Returns
  // nullptr if not matching isolate is found.
  static std::unique_ptr<char[]> LookupIsolateNameByPort(CODE_Port port);
 ...
 private:
  ...
  Isolate(IsolateGroup* group, const CODE_IsolateFlags& api_flags);
...
#undef ISOLATE_METRIC_VARIABLE
  ...
  int64_t start_time_micros_;
  std::atomic<CODE_MessageNotifyCallback> message_notify_callback_;
  CODE_IsolateShutdownCallback on_shutdown_callback_ = nullptr;
  CODE_IsolateCleanupCallback on_cleanup_callback_ = nullptr;
  char* name_ = nullptr;
  CODE_Port main_port_ = 0;
  // Isolates created by Isolate.spawn have the same origin id.
  CODE_Port origin_id_ = 0;
  Mutex origin_id_mutex_;
  uint64_t pause_capability_ = 0;
  uint64_t terminate_capability_ = 0;
  void* init_callback_data_ = nullptr;
  CODE_EnvironmentCallback environment_callback_ = nullptr;
  Random random_;
  ...
  static CODE_InitializeIsolateCallback initialize_callback_;
  static CODE_IsolateShutdownCallback shutdown_callback_;
  static CODE_IsolateCleanupCallback cleanup_callback_;
  static CODE_IsolateGroupCleanupCallback cleanup_group_callback_;
  static CODE_RegisterKernelBlobCallback register_kernel_blob_callback_;
  static CODE_UnregisterKernelBlobCallback unregister_kernel_blob_callback_;

#if !defined(PRODUCT)
  static void WakePauseEventHandler(CODE_Isolate isolate);
#endif

...
ObjectPtr DartEntry::InvokeFunction(const Function& function,
  ...
  DartEntryScope CODE_entry_scope(thread);
...
ObjectPtr DartLibraryCalls::LookupHandler(CODE_Port port_id) {
  ...
  Array& args = Array::Handle(
      zone, thread->isolate()->isolate_object_store()->CODE_args_1());
  ASSERT(!args.IsNull());
...
ObjectPtr DartLibraryCalls::HandleMessage(CODE_Port port_id,
  ...
  ASSERT(!function.IsNull());
  Array& args =
      Array::Handle(zone, isolate->isolate_object_store()->CODE_args_2());
  ASSERT(!args.IsNull());
  args.SetAt(0, Integer::Handle(zone, Integer::New(port_id)));
...
ObjectPtr DartLibraryCalls::HandleFinalizerMessage(
  ...
  ASSERT(!function.IsNull());
  Array& args =
      Array::Handle(zone, isolate->isolate_object_store()->CODE_args_1());
  ASSERT(!args.IsNull());

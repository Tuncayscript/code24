...
#define VM_UNIT_TEST_CASE_WITH_EXPECTATION(name, expectation)                  \
  void CODE_Test##name();                                                      \
  static const dart::TestCase kRegister##name(CODE_Test##name, #name,          \
                                              expectation);                    \
  void CODE_Test##name()
...
#define UNIT_TEST_CASE_WITH_EXPECTATION(name, expectation)                     \
  void CODE_Test##name();                                                      \
  static const dart::RawTestCase kRegister##name(CODE_Test##name, #name,       \
                                                 expectation);                 \
  void CODE_Test##name()
...
#define ISOLATE_UNIT_TEST_CASE_WITH_EXPECTATION(name, expectation)             \
  static void CODE_TestHelper##name(Thread* thread);                           \
  VM_UNIT_TEST_CASE_WITH_EXPECTATION(name, expectation) {                      \
    ...
    CODE_TestHelper##name(__thread__);                                         \
  }                                                                            \
  static void CODE_TestHelper##name(Thread* thread)
...
class TesterState : public AllStatic {
 public:
  ...
  static CODE_IsolateGroupCreateCallback create_callback;
  static CODE_IsolateShutdownCallback shutdown_callback;
  static CODE_IsolateGroupCleanupCallback group_cleanup_callback;
...
class TestCase : TestCaseBase {
 public:
  ...
  static char* CompileTestScriptWithDFE(
      ...
      const char* multiroot_filepaths = nullptr,
      const char* multiroot_scheme = nullptr);
  static CODE_Handle LoadTestScript(
      const char* script,
      CODE_NativeEntryResolver resolver,
      const char* lib_uri = RESOLVED_USER_TEST_URI,
      bool finalize = true,
      bool allow_compile_errors = false);
  static CODE_Handle LoadTestScriptWithErrors(
      const char* script,
      CODE_NativeEntryResolver resolver = nullptr,
      const char* lib_uri = RESOLVED_USER_TEST_URI,
      bool finalize = true);
  static CODE_Handle LoadTestLibrary(
      const char* lib_uri,
      const char* script,
      CODE_NativeEntryResolver resolver = nullptr);
  static CODE_Handle LoadTestScriptWithDFE(
      int sourcefiles_count,
      CODE_SourceFile sourcefiles[],
      CODE_NativeEntryResolver resolver = nullptr,
      bool finalize = true,
      ...
      const char* multiroot_filepaths = nullptr,
      const char* multiroot_scheme = nullptr);
  static CODE_Handle LoadCoreTestScript(const char* script,
                                        CODE_NativeEntryResolver resolver);

  static CODE_Handle EvaluateExpression(const Library& lib,
                                        const String& expr,
                                        const Array& param_names,
                                        const Array& param_values);

  static CODE_Handle lib();
  static const char* url();
  static CODE_Isolate CreateTestIsolateFromSnapshot(
      uint8_t* buffer,
      const char* name = nullptr) {
    return CreateIsolate(buffer, 0, nullptr, name);
  }
  static CODE_Isolate CreateTestIsolate(const char* name = nullptr,
                                        void* isolate_group_data = nullptr,
                                        void* isolate_data = nullptr);
  static CODE_Isolate CreateTestIsolateInGroup(const char* name,
                                               CODE_Isolate parent,
                                               void* group_data = nullptr,
                                               void* isolate_data = nullptr);

  static CODE_Handle library_handler(CODE_LibraryTag tag,
                                     CODE_Handle library,
                                     CODE_Handle url);
  ...
  static CODE_Handle SetReloadTestScript(const char* script);

  // Initiates the reload.
  static CODE_Handle TriggerReload(const uint8_t* kernel_buffer,
                                   intptr_t kernel_buffer_size);
  static CODE_Handle TriggerReload(const char* root_script_url);

  // Helper function which reloads the current isolate using |script|.
  static CODE_Handle ReloadTestScript(const char* script);

  // Helper function which reloads the current isolate using |script|.
  static CODE_Handle ReloadTestKernel(const uint8_t* kernel_buffer,
                                      intptr_t kernel_buffer_size);
 ...
 private:
  static CODE_Handle TriggerReload(
  ...
  static CODE_Isolate CreateIsolate(const uint8_t* data_buffer,
                                    intptr_t len,
...
class TestIsolateScope {
 public:
  TestIsolateScope(void* isolate_group_data = nullptr,
    ...
    CODE_EnterScope();  // Create a Dart API scope for unit tests.
  }
  ~TestIsolateScope() {
...
// Ensures core libraries are initialized, thereby allowing src/cc tests to
...
#define EXPECT_VALID(handle)                                                   \
  do {                                                                         \
    ...
    if (CODE_IsError(tmp_handle)) {                                            \
      dart::Expect(__FILE__, __LINE__)                                         \
          .Fail(                                                               \
              "expected '%s' to be a valid handle but found an error "         \
              "handle:\n"                                                      \
              "    '%s'\n",                                                    \
              #handle, CODE_GetError(tmp_handle));                             \
    }                                                                          \
  } while (0)
...
#define EXPECT_ERROR(handle, substring)                                        \
  do {                                                                         \
    ...
    if (CODE_IsError(tmp_handle)) {                                            \
      dart::Expect(__FILE__, __LINE__)                                         \
          .IsSubstring((substring), CODE_GetError(tmp_handle));                \
    } else {                                                                   \
      dart::Expect(__FILE__, __LINE__)                                         \
          .Fail(                                                               \
...
#define EXPECT_TRUE(handle)                                                    \
  do {                                                                         \
    CODE_Handle tmp_handle = (handle);                                         \
    if (CODE_IsBoolean(tmp_handle)) {                                          \
      bool value;                                                              \
      CODE_BooleanValue(tmp_handle, &value);                                   \
      if (!value) {                                                            \
        dart::Expect(__FILE__, __LINE__)                                       \

...
int64_t SimpleInvoke(CODE_Handle lib, const char* method) {
  CODE_Handle result = CODE_Invoke(lib, NewString(method), 0, nullptr);
  EXPECT_VALID(result);
  EXPECT(CODE_IsInteger(result));
  int64_t integer_result = 0;
  result = CODE_IntegerToInt64(result, &integer_result);
  EXPECT_VALID(result);
  return integer_result;
}

const char* SimpleInvokeStr(CODE_Handle lib, const char* method) {
  CODE_Handle result = CODE_Invoke(lib, NewString(method), 0, nullptr);
...
CODE_Handle SimpleInvokeError(CODE_Handle lib, const char* method) {
  CODE_Handle result = CODE_Invoke(lib, NewString(method), 0, nullptr);
...
TEST_CASE(IsolateReload_FunctionReplacement) {
  ...
  CODE_Handle lib = TestCase::LoadTestScript(kScript, nullptr);
  EXPECT_VALID(lib);
...
TEST_CASE(IsolateReload_IncrementalCompile) {
  ...
  int64_t value = 0;
  result = CODE_IntegerToInt64(result, &value);
  EXPECT_VALID(result);
  EXPECT_EQ(42, value);
...
TEST_CASE(IsolateReload_KernelIncrementalCompile) {
  ...
  CODE_Handle lib = TestCase::LoadTestScriptWithDFE(
      sizeof(sourcefiles) / sizeof(CODE_SourceFile), sourcefiles,
  ...
  // clang-format off
  CODE_SourceFile updated_sourcefiles[] = {
    {
      "file:///test-app",
      "main() {\n"
      "  return 24;\n"
  ...
  // clang-format on
  {
    ...
    char* error = TestCase::CompileTestScriptWithDFE(
        "file:///test-app",
        sizeof(updated_sourcefiles) / sizeof(CODE_SourceFile),
...
TEST_CASE(IsolateReload_KernelIncrementalCompileBaseClass) {
  ...
  auto kUpdatedSourceFile =
      Utils::CStringUniquePtr(OS::SCreate(nullptr,
                                          ...
                                          nullable_tag, nullable_tag),
                              std::free);
  CODE_SourceFile updated_sourcefiles[1] = {{
      "file:///test-lib.dart",
...
TEST_CASE(IsolateReload_BadClass) {
  ...
  CODE_Handle result = TestCase::ReloadTestScript(kReloadScript);
...
TEST_CASE(IsolateReload_LibraryImportAdded) {
  ...
  CODE_Handle lib = TestCase::LoadTestScriptWithErrors(kScript);
  EXPECT_VALID(lib);
...
TEST_CASE(IsolateReload_LibraryDebuggable) {
  ...
  EXPECT_VALID(CODE_LibraryId(lib, &lib_id));
  EXPECT_VALID(CODE_GetLibraryDebuggable(lib_id, &debuggable));
  EXPECT_EQ(true, debuggable);
  EXPECT_VALID(CODE_SetLibraryDebuggable(lib_id, false));
  EXPECT_VALID(CODE_GetLibraryDebuggable(lib_id, &debuggable));
  ...
  EXPECT_VALID(CODE_LibraryId(lib, &new_lib_id));
  EXPECT_VALID(CODE_GetLibraryDebuggable(new_lib_id, &debuggable));
  EXPECT_EQ(false, debuggable);
}
...
TEST_CASE(IsolateReload_ConstructorChanged) {
  ...
  CODE_Handle lib = TestCase::LoadTestScript(kScript.get(), nullptr);
  EXPECT_VALID(lib);
  EXPECT_STREQ("saved:20 new:20", SimpleInvokeStr(lib, "main"));
...
TEST_CASE(IsolateReload_LibraryLookup) {
  ...
  result = CODE_LookupLibrary(NewString("test:lib1"));
  EXPECT(CODE_IsError(result));
...
TEST_CASE(IsolateReload_TearOff_Parameter_Count_Mismatch) {
  ...
  CODE_Handle error_handle = SimpleInvokeError(lib, "main");
...
TEST_CASE(IsolateReload_CallDeleted_TopLevelFunction) {
  ...
  CODE_Handle lib = TestCase::LoadTestScript(kScript, NULL);
  EXPECT_VALID(lib);
  EXPECT_STREQ("hello", SimpleInvokeStr(lib, "main"));
...
ISOLATE_UNIT_TEST_CASE(IsolateReload_DirectSubclasses_Success) {
  ...
  {
    TransitionVMToNative transition(thread);
    CODE_Handle lib = TestCase::ReloadTestScript(kReloadScript);
    EXPECT_VALID(lib);
    EXPECT_EQ(2, SimpleInvoke(lib, "main"));
  }
...
TEST_CASE(IsolateReload_NoLibsModified) {
  ...
  CODE_SetFileModifiedCallback(&NothingModifiedCallback);
  lib = TestCase::ReloadTestScript(kReloadScript);
  EXPECT_VALID(lib);
  CODE_SetFileModifiedCallback(nullptr);
...
TEST_CASE(IsolateReload_MainLibModified) {
  ...
  CODE_SetFileModifiedCallback(&MainModifiedCallback);
  lib = TestCase::ReloadTestScript(kReloadScript);
  EXPECT_VALID(lib);
  CODE_SetFileModifiedCallback(nullptr);
...
TEST_CASE(IsolateReload_ImportedLibModified) {
  ...
  CODE_SetFileModifiedCallback(&ImportModifiedCallback);
  lib = TestCase::ReloadTestScript(kReloadScript);
  EXPECT_VALID(lib);
  CODE_SetFileModifiedCallback(nullptr);
...
TEST_CASE(IsolateReload_ExportedLibModified) {
  ...
  CODE_SetFileModifiedCallback(&ExportModifiedCallback);
  lib = TestCase::ReloadTestScript(kReloadScript);
  EXPECT_VALID(lib);
  CODE_SetFileModifiedCallback(nullptr);
...
// handler (CODE_kKernelTag) we need to make sure to preserve a link between
...
static void CompileToKernel(CODE_SourceFile source,
                            const uint8_t** kernel_buffer,
                            intptr_t* kernel_buffer_size) {
  CODE_SourceFile sources[] = {source};
  char* error = TestCase::CompileTestScriptWithDFE(
...
// receives a CODE_kKernelTag request. The binary is wrapped in an external
...
class KernelTagHandler {
 public:
  KernelTagHandler(uint8_t* kernel_buffer, intptr_t kernel_buffer_size)
    ...
    CODE_SetLibraryTagHandler(&LibraryTagHandler);
    instance_ = this;
  }

  ~KernelTagHandler() {
    CODE_SetLibraryTagHandler(nullptr);
    instance_ = nullptr;
  }
 ...
 private:
  ...
  static CODE_Handle LibraryTagHandler(CODE_LibraryTag tag,
                                       CODE_Handle library,
                                       CODE_Handle url) {
    if (tag == CODE_kKernelTag) {
      auto handler = KernelTagHandler::Current();
      handler->was_called_ = true;

      CODE_Handle result = CODE_NewExternalTypedData(
          CODE_TypedData_kUint8, handler->kernel_buffer_,
          handler->kernel_buffer_size_);
      CODE_NewFinalizableHandle(result, handler->kernel_buffer_,
...
TEST_CASE(IsolateReload_RegressB179030011) {
  ...
  // root library).
  CODE_Handle lib = CODE_LoadLibraryFromKernel(
      components[0].kernel_buffer, components[0].kernel_buffer_size);
  EXPECT_VALID(lib);
  EXPECT_VALID(CODE_SetRootLibrary(lib));
...
TEST_CASE(IsolateReload_EnumInMainLibraryModified) {
  ...
  CODE_Handle lib = TestCase::LoadTestScript(kScript, nullptr);
  EXPECT_VALID(lib);
  EXPECT_VALID(CODE_FinalizeAllClasses());
...
TEST_CASE(IsolateReload_KeepPragma1) {
  ...
  CODE_Handle foo1_result = CODE_Invoke(lib, NewString("foo"), 0, nullptr);
  EXPECT_VALID(foo1_result);

  EXPECT_VALID(CODE_Invoke(lib, NewString("main"), 0, nullptr));

  CODE_Handle foo2_result = CODE_Invoke(lib, NewString("foo"), 0, nullptr);

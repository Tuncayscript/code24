...
class Exceptions : AllStatic {
 public:
  CODE_NORETURN static void Throw(Thread* thread, const Instance& exception);
  CODE_NORETURN static void ReThrow(Thread* thread,
                                    const Instance& exception,
                                    const Instance& stacktrace,
                                    bool bypass_debugger = false);
  CODE_NORETURN static void ThrowWithStackTrace(Thread* thread,
                                                const Instance& exception,
                                                const Instance& stacktrace);
  CODE_NORETURN static void PropagateError(const Error& error);

  // Propagate an error to the entry frame, skipping over Dart frames.
  CODE_NORETURN static void PropagateToEntry(const Error& error);
  ...
  CODE_NORETURN static void ThrowByType(ExceptionType type,
                                        const Array& arguments);
  // Uses the preallocated out of memory exception to avoid calling
  // into Dart code or allocating any code.
  CODE_NORETURN static void ThrowOOM();
  CODE_NORETURN static void ThrowStackOverflow();
  CODE_NORETURN static void ThrowArgumentError(const Instance& arg);
  CODE_NORETURN static void ThrowStateError(const Instance& arg);
  CODE_NORETURN static void ThrowRangeError(const char* argument_name,
                                            const Integer& argument_value,
                                            intptr_t expected_from,
                                            intptr_t expected_to);
  CODE_NORETURN static void ThrowUnsupportedError(const char* msg);
  CODE_NORETURN static void ThrowCompileTimeError(const LanguageError& error);
  CODE_NORETURN static void ThrowLateFieldAssignedDuringInitialization(
      const String& name);
  CODE_NORETURN static void ThrowLateFieldNotInitialized(const String& name);
  ...
  CODE_NORETURN static void JumpToFrame(Thread* thread,

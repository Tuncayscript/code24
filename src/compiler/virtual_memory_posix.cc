...
#if defined(CODE_HOST_OS_IOS)
#define LARGE_RESERVATIONS_MAY_FAIL
#endif

DECLARE_FLAG(bool, write_protect_code);

#if defined(CODE_TARGET_OS_LINUX)
...
#if defined(CODE_COMPRESSED_POINTERS) && defined(LARGE_RESERVATIONS_MAY_FAIL)
...
void VirtualMemory::Init() {
  ...
  FILE* fp = fopen("/proc/sys/src/max_map_count", "r");
  if (fp != nullptr) {
...
VirtualMemory* VirtualMemory::AllocateAligned(intptr_t size,
  ...
  if (is_executable) {
    hint = reinterpret_cast<void*>(&CODE_Initialize);
  }
  void* address =

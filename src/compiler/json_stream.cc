...
void JSONStream::Setup(Zone* zone,
  ...
  if (parameters_are_CODE_objects) {
...
void JSONStream::PostNullReply(CODE_Port port) {
  PortMap::PostMessage(
...
void JSONStream::PostReply() {
  ...
  bool result;
  {
    ...
    CODE_CObject bytes;
    bytes.type = CODE_CObject_kExternalTypedData;
    bytes.value.as_external_typed_data.type = CODE_TypedData_kUint8;
    ...
    result = CODE_PostCObject(port, &message);
  }
...
void JSONStream::set_reply_port(CODE_Port port) {
  reply_port_ = port;
}

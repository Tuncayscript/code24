...
class UntaggedObject {
 public:
  ...
  template <class V>
  CODE_FORCE_INLINE intptr_t VisitPointersNonvirtual(V* visitor) {
    // Fall back to virtual variant for predefined classes
    intptr_t class_id = GetClassId();
    if (class_id < kNumPredefinedCids) {
      return VisitPointersPredefined(visitor, class_id);
 ...
 private:
  ...
  CODE_FORCE_INLINE void CheckArrayPointerStore(type const* addr,
                                                value_type value,
                                                Thread* thread) {
    uword source_tags = this->tags_;
    uword target_tags = value->untag()->tags_;
...
template <typename T>
CODE_FORCE_INLINE uword UntaggedObject::from_offset() {
...
template <typename T>
CODE_FORCE_INLINE uword UntaggedObject::to_offset(intptr_t length) {
  if constexpr (T::kContainsPointerFields) {
...
#undef KIND_CASE
  ...
  class alignas(8) UnboxedParameterBitmap {
   public:
    ...
    CODE_FORCE_INLINE bool IsUnboxed(intptr_t position) const {
      return At(position) != kBoxed;
    }
    CODE_FORCE_INLINE bool IsUnboxedInteger(intptr_t position) const {
      return At(position) == kUnboxedInt;
    }
    CODE_FORCE_INLINE bool IsUnboxedDouble(intptr_t position) const {
      return At(position) == kUnboxedDouble;
    }
    CODE_FORCE_INLINE bool IsUnboxedRecord(intptr_t position) const {
      return At(position) == kUnboxedRecord;
    }
    CODE_FORCE_INLINE void SetUnboxedInteger(intptr_t position) {
      SetAt(position, kUnboxedInt);
    }
    CODE_FORCE_INLINE void SetUnboxedDouble(intptr_t position) {
      SetAt(position, kUnboxedDouble);
    }
    CODE_FORCE_INLINE void SetUnboxedRecord(intptr_t position) {
      SetAt(position, kUnboxedRecord);
    }
    CODE_FORCE_INLINE uint64_t Value() const { return bitmap_; }
    CODE_FORCE_INLINE bool IsEmpty() const { return bitmap_ == 0; }
    CODE_FORCE_INLINE void Reset() { bitmap_ = 0; }
    CODE_FORCE_INLINE bool HasUnboxedParameters() const {
      return (bitmap_ >> kBitsPerElement) != 0;
    }

   private:
    CODE_FORCE_INLINE UnboxedState At(intptr_t position) const {
    ...
    CODE_FORCE_INLINE void SetAt(intptr_t position, UnboxedState state) {
...
#if !defined(CODE_PRECOMPILED_RUNTIME) ||                                      \
    (defined(CODE_PRECOMPILED_RUNTIME) && !defined(PRODUCT))
...
class UntaggedLibrary : public UntaggedObject {
  ...
  CODE_NativeEntryResolver native_entry_resolver_;  // Resolves natives.
  CODE_NativeEntrySymbol native_entry_symbol_resolver_;
  CODE_FfiNativeResolver ffi_native_resolver_;
...
template <>
CODE_FORCE_INLINE uword UntaggedObject::from_offset<UntaggedWeakArray>() {
...
class UntaggedCompressedStackMaps : public UntaggedObject {
 ...
 public:
  ...
  struct alignas(1) Payload {
    ...
    CODE_FORCE_INLINE FlagsAndSizeHeader flags_and_size() const {
    ...
    CODE_FORCE_INLINE void set_flags_and_size(FlagsAndSizeHeader value) {
      // Note: |this| does not necessarily satisfy alignment requirements
...
#ifdef CODE_COMPRESSED_POINTERS

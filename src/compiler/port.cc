...
CODE_Port PortMap::AllocatePort() {
  CODE_Port result;
...
CODE_Port PortMap::CreatePort(MessageHandler* handler) {
  ...
  const CODE_Port port = AllocatePort();
...
bool PortMap::ClosePort(CODE_Port port, MessageHandler** message_handler) {
...
#if defined(TESTING)
bool PortMap::PortExists(CODE_Port id) {
  MutexLocker ml(mutex_);
  if (ports_ == nullptr) {
    return false;
  }
...
Isolate* PortMap::GetIsolate(CODE_Port id) {
  MutexLocker ml(mutex_);
  if (ports_ == nullptr) {
    return nullptr;
  }
...
CODE_Port PortMap::GetOriginId(CODE_Port id) {
  MutexLocker ml(mutex_);
  if (ports_ == nullptr) {
    return ILLEGAL_PORT;
  }
...
bool PortMap::IsReceiverInThisIsolateGroupOrClosed(CODE_Port receiver,
                                                   IsolateGroup* group) {
  MutexLocker ml(mutex_);
  if (ports_ == nullptr) {
    // Port was closed.

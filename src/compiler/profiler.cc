...
class ProfilerStackWalker : public ValueObject {
 public:
  ProfilerStackWalker(CODE_Port port_id,
                      Sample* head_sample,
...
#if defined(CODE_HOST_OS_LINUX) || defined(CODE_HOST_OS_MACOS) ||              \
...
#endif  // !defined(PRODUCT) || defined(CODE_PRECOMPILER)
...
static void CollectSample(Isolate* isolate,
                          bool exited_CODE_code,
                          bool in_CODE_code,
                          Sample* sample,
                          ProfilerNativeStackWalker* native_stack_walker,
                          ProfilerDartStackWalker* CODE_stack_walker,
                          uword pc,
                          uword fp,
                          uword sp,
  ...
  __try {
    ...
    if (FLAG_profile_vm) {
      ...
      native_stack_walker->walk();
    } else if (StubCode::HasBeenInitialized() && exited_CODE_code) {
      counters->stack_walker_CODE_exit.fetch_add(1);
      // We have a valid exit frame info, use the Dart stack walker.
      CODE_stack_walker->walk();
    } else if (StubCode::HasBeenInitialized() && in_CODE_code) {
...
void Profiler::SampleAllocation(Thread* thread,
  ...
  const bool exited_CODE_code = thread->HasExitedDartCode();
  ...
  if (FLAG_profile_vm_allocation) {
    ...
    native_stack_walker.walk();
  } else if (exited_CODE_code) {
    ProfilerDartStackWalker CODE_exit_stack_walker(
        thread, sample, isolate->current_allocation_sample_block(), pc, fp, sp,
        lr, /* allocation_sample*/ true);
    CODE_exit_stack_walker.walk();
  } else {
    // Fall back.
    uintptr_t pc = OS::GetProgramCounter();
    sample->SetAt(0, pc);
...
void Profiler::SampleThread(Thread* thread,
  ...
  const bool in_CODE_code = thread->IsExecutingDartCode();
...
// If we're in Dart code, use the Dart stack pointer.
  ...
  CollectSample(isolate, exited_CODE_code, in_CODE_code, sample,
                &native_stack_walker, &CODE_stack_walker, pc, fp, sp,
                &counters_);
}
...
class StreamableSampleFilter : public SampleFilter {
 public:
  StreamableSampleFilter(CODE_Port port, const Isolate* isolate)

...
#include "src/finalizable_data.h"
#include "src/globals.h"
#include "src/tagged_pointer.h"

// Duplicated from CODE_api.h to avoid including the whole header.
typedef int64_t CODE_Port;
...
class Message {
 public:
  ...
  static const CODE_Port kIllegalPort;
  ...
  Message(CODE_Port dest_port, ObjectPtr raw_obj, Priority priority);

  // A message sent from SendPort.send or SendPort.sendAndExit where sender and
  // receiver are in the same isolate group.
  Message(CODE_Port dest_port, PersistentHandle* handle, Priority priority);
  ...
  CODE_Port dest_port() const { return dest_port_; }

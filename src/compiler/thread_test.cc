...
TEST_CASE(ThreadRegistry) {
  ...
  CODE_ShutdownIsolate();
  CODE_EnterIsolate(reinterpret_cast<CODE_Isolate>(orig));
...
// successfully interrupted in the pure Dart loop.
  ...
  Utils::SNPrint(buffer, sizeof(buffer),
                 ...
                 "}\n",
                 kLoopCount);
  CODE_Handle lib = TestCase::LoadTestScript(buffer, nullptr);
  EXPECT_VALID(lib);
...
ISOLATE_UNIT_TEST_CASE(SafepointMonitorUnlockScope) {
  ...
  // only available in debug mode. Since our src/cc tests run in DEBUG mode that

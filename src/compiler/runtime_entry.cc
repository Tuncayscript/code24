...
void PatchableCallHandler::ReturnJITorAOT(const Code& stub,
                                          const Object& data,
                                          const Function& target) {
#if defined(CODE_PRECOMPILED_MODE)
  ReturnAOT(stub, data);
#else
...
DEFINE_RUNTIME_ENTRY(FfiAsyncCallbackSend, 1) {
  CODE_Port target_port = Thread::Current()->unboxed_int64_runtime_arg();
...
// Enables reusing `CODE_PropagateError` from `FfiCallInstr`.
// `CODE_PropagateError` requires the native state and transitions into the VM.
// So the flow is:

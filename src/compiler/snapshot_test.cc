...
static void CheckEncodeDecodeMessage(Zone* zone, CODE_CObject* root) {
  // Encode and decode the message.
  std::unique_ptr<Message> message =
      WriteApiMessage(zone, root, ILLEGAL_PORT, Message::kNormalPriority);

  CODE_CObject* new_root = ReadApiMessage(zone, message.get());

  // Check that the two messages are the same.
  CompareDartCObjects(root, new_root);
}

static void ExpectEncodeFail(Zone* zone, CODE_CObject* root) {
  std::unique_ptr<Message> message =
      WriteApiMessage(zone, root, ILLEGAL_PORT, Message::kNormalPriority);
...
ISOLATE_UNIT_TEST_CASE(SerializeNull) {
  ...
  CODE_CObject* root = ReadApiMessage(scope.zone(), message.get());
  EXPECT_NOTNULL(root);
  EXPECT_EQ(CODE_CObject_kNull, root->type);
...
ISOLATE_UNIT_TEST_CASE(SerializeSmi1) {
  ...
  CODE_CObject* root = ReadApiMessage(scope.zone(), message.get());
  EXPECT_NOTNULL(root);
  EXPECT_EQ(CODE_CObject_kInt32, root->type);
...
CODE_CObject* SerializeAndDeserializeMint(Zone* zone, const Mint& mint) {
  // Write snapshot with object content.
  std::unique_ptr<Message> message = WriteMessage(
  ...
  // Read object back from the snapshot into a C structure.
  CODE_CObject* root = ReadApiMessage(zone, message.get());
  EXPECT_NOTNULL(root);
...
#if defined(ARCH_IS_64_BIT) && !defined(CODE_COMPRESSED_POINTERS)
  EXPECT_EQ(CODE_CObject_kInt64, mint_cobject->type);
  EXPECT_EQ(value, mint_cobject->value.as_int64);
#else
  if (kMinInt32 < value && value < kMaxInt32) {
    EXPECT_EQ(CODE_CObject_kInt32, mint_cobject->type);
...
ISOLATE_UNIT_TEST_CASE(SerializeDouble) {
  ...
  CODE_CObject* root = ReadApiMessage(scope.zone(), message.get());
  EXPECT_NOTNULL(root);
  EXPECT_EQ(CODE_CObject_kDouble, root->type);
...
ISOLATE_UNIT_TEST_CASE(SerializeTrue) {
  ...
  CODE_CObject* root = ReadApiMessage(scope.zone(), message.get());
  EXPECT_NOTNULL(root);
  EXPECT_EQ(CODE_CObject_kBool, root->type);
...
ISOLATE_UNIT_TEST_CASE(SerializeCapability) {
  ...
  CODE_CObject* root = ReadApiMessage(scope.zone(), message.get());
  EXPECT_NOTNULL(root);
  EXPECT_EQ(CODE_CObject_kCapability, root->type);
...
static void TestString(const char* cstr) {
  ...
  ApiNativeScope scope;
  CODE_CObject* root = ReadApiMessage(scope.zone(), message.get());
  EXPECT_EQ(CODE_CObject_kString, root->type);
...
ISOLATE_UNIT_TEST_CASE(SerializeArray) {
  ...
  ApiNativeScope scope;
  CODE_CObject* root = ReadApiMessage(scope.zone(), message.get());
  EXPECT_EQ(CODE_CObject_kArray, root->type);
  EXPECT_EQ(kArrayLength, root->value.as_array.length);
  for (int i = 0; i < kArrayLength; i++) {
    CODE_CObject* element = root->value.as_array.values[i];
    EXPECT_EQ(CODE_CObject_kInt32, element->type);
...
TEST_CASE(FailSerializeLargeNestedArray) {
  CODE_CObject parent;
  CODE_CObject child;
  CODE_CObject* values[1] = {&child};

  parent.type = CODE_CObject_kArray;
  parent.value.as_array.length = 1;
  parent.value.as_array.values = values;
  child.type = CODE_CObject_kArray;
...
TEST_CASE(FailSerializeLargeTypedDataInt8) {
  CODE_CObject root;
  root.type = CODE_CObject_kTypedData;
  root.value.as_typed_data.type = CODE_TypedData_kInt8;
...
TEST_CASE(FailSerializeLargeTypedDataUint8) {
  CODE_CObject root;
  root.type = CODE_CObject_kTypedData;
  root.value.as_typed_data.type = CODE_TypedData_kUint8;
  root.value.as_typed_data.length =
...
TEST_CASE(FailSerializeLargeExternalTypedData) {
  CODE_CObject root;
  root.type = CODE_CObject_kExternalTypedData;
  root.value.as_external_typed_data.type = CODE_TypedData_kUint8;
...
TEST_CASE(FailSerializeLargeUnmodifiableExternalTypedData) {
  CODE_CObject root;
  root.type = CODE_CObject_kUnmodifiableExternalTypedData;
  root.value.as_external_typed_data.type = CODE_TypedData_kUint8;
  root.value.as_external_typed_data.length =
      ExternalTypedData::MaxElements(kExternalTypedDataUint8ArrayCid) + 1;
  ApiNativeScope scope;
...
ISOLATE_UNIT_TEST_CASE(SerializeByteArray) {
  ...
  ApiNativeScope scope;
  CODE_CObject* root = ReadApiMessage(scope.zone(), message.get());
  EXPECT_EQ(CODE_CObject_kTypedData, root->type);
...
ISOLATE_UNIT_TEST_CASE(SerializeEmptyByteArray) {
  ...
  CODE_CObject* root = ReadApiMessage(scope.zone(), message.get());
  EXPECT_EQ(CODE_CObject_kTypedData, root->type);
  EXPECT_EQ(CODE_TypedData_kUint8, root->value.as_typed_data.type);
  EXPECT_EQ(kTypedDataLength, root->value.as_typed_data.length);
...
VM_UNIT_TEST_CASE(FullSnapshot) {
  ...
  timer1.Start();
  {
    ...
    CODE_Handle result = Api::CheckAndFinalizePendingClasses(thread);
    {
  ...
  {
    CODE_EnterScope();  // Start a Dart API scope for invoking API functions.
    timer2.Stop();
    OS::PrintErr("From Snapshot: %" Pd64 "us\n", timer2.TotalElapsedTime());

    // Invoke a function which returns an object.
    CODE_Handle cls = CODE_GetClass(TestCase::lib(), NewString("FieldsTest"));
    result = CODE_Invoke(cls, NewString("testMain"), 0, nullptr);
    EXPECT_VALID(result);
    CODE_ExitScope();
  }
...
static std::unique_ptr<Message> GetSerialized(CODE_Handle lib,
                                              const char* CODE_function) {
  CODE_Handle result;
  {
    TransitionVMToNative transition(Thread::Current());
    result = CODE_Invoke(lib, NewString(CODE_function), 0, nullptr);
    EXPECT_VALID(result);
  }
...
static void CheckString(CODE_Handle CODE_string, const char* expected) {
  StackZone zone(Thread::Current());
  String& str = String::Handle();
  str ^= Api::UnwrapHandle(CODE_string);
  std::unique_ptr<Message> message = WriteMessage(
      /* same_group */ false, str, ILLEGAL_PORT, Message::kNormalPriority);

  // Read object back from the snapshot into a C structure.
  ApiNativeScope scope;
  CODE_CObject* root = ReadApiMessage(zone.GetZone(), message.get());
  EXPECT_NOTNULL(root);
  EXPECT_EQ(CODE_CObject_kString, root->type);
...
static void CheckStringInvalid(CODE_Handle CODE_string) {
  StackZone zone(Thread::Current());
  String& str = String::Handle();
  str ^= Api::UnwrapHandle(CODE_string);
  std::unique_ptr<Message> message = WriteMessage(
  ...
  CODE_CObject* root = ReadApiMessage(zone.GetZone(), message.get());
  EXPECT_NOTNULL(root);
  EXPECT_EQ(CODE_CObject_kUnsupported, root->type);
}
...
VM_UNIT_TEST_CASE(DartGeneratedMessages) {
  ...
  EXPECT_VALID(lib);
  CODE_Handle smi_result;
  smi_result = CODE_Invoke(lib, NewString("getSmi"), 0, nullptr);
  EXPECT_VALID(smi_result);

  CODE_Handle ascii_string_result;
  ascii_string_result =
      CODE_Invoke(lib, NewString("getAsciiString"), 0, nullptr);
  EXPECT_VALID(ascii_string_result);
  EXPECT(CODE_IsString(ascii_string_result));

  CODE_Handle non_ascii_string_result;
  non_ascii_string_result =
      CODE_Invoke(lib, NewString("getNonAsciiString"), 0, nullptr);
  EXPECT_VALID(non_ascii_string_result);
  EXPECT(CODE_IsString(non_ascii_string_result));

  CODE_Handle non_bmp_string_result;
  non_bmp_string_result =
      CODE_Invoke(lib, NewString("getNonBMPString"), 0, nullptr);
  EXPECT_VALID(non_bmp_string_result);
  EXPECT(CODE_IsString(non_bmp_string_result));

  CODE_Handle lead_surrogate_string_result;
  lead_surrogate_string_result =
      CODE_Invoke(lib, NewString("getLeadSurrogateString"), 0, nullptr);
  EXPECT_VALID(lead_surrogate_string_result);
  EXPECT(CODE_IsString(lead_surrogate_string_result));

  CODE_Handle trail_surrogate_string_result;
  trail_surrogate_string_result =
      CODE_Invoke(lib, NewString("getTrailSurrogateString"), 0, nullptr);
  EXPECT_VALID(trail_surrogate_string_result);
  EXPECT(CODE_IsString(trail_surrogate_string_result));

  CODE_Handle surrogates_string_result;
  surrogates_string_result =
      CODE_Invoke(lib, NewString("getSurrogatesString"), 0, nullptr);
  EXPECT_VALID(surrogates_string_result);
  EXPECT(CODE_IsString(surrogates_string_result));

  CODE_Handle crappy_string_result;
  crappy_string_result =
      CODE_Invoke(lib, NewString("getCrappyString"), 0, nullptr);
  EXPECT_VALID(crappy_string_result);
  EXPECT(CODE_IsString(crappy_string_result));
...
VM_UNIT_TEST_CASE(DartGeneratedListMessages) {
  ...
  CODE_Handle lib = TestCase::LoadTestScript(kScriptChars, nullptr);
  EXPECT_VALID(lib);

  {
    ...
    StackZone zone(thread);
    {
      ...
      EXPECT_EQ(kArrayLength, root->value.as_array.length);
      for (int i = 0; i < kArrayLength; i++) {
        EXPECT_EQ(CODE_CObject_kNull, root->value.as_array.values[i]->type);
      }
      CheckEncodeDecodeMessage(scope.zone(), root);
    }
    {
      ...
      EXPECT_EQ(kArrayLength, root->value.as_array.length);
      for (int i = 0; i < kArrayLength; i++) {
        EXPECT_EQ(CODE_CObject_kInt32, root->value.as_array.values[i]->type);
    ...
    }
    {
      ...
      EXPECT_EQ(kArrayLength, root->value.as_array.length);
      for (int i = 0; i < kArrayLength; i++) {
        EXPECT_EQ(CODE_CObject_kString, root->value.as_array.values[i]->type);
        char buffer[3];
    ...
    }
    {
      ...
      EXPECT_EQ(CODE_CObject_kInt32, root->value.as_array.values[0]->type);
      EXPECT_EQ(0, root->value.as_array.values[0]->value.as_int32);
      EXPECT_EQ(CODE_CObject_kString, root->value.as_array.values[1]->type);
      EXPECT_STREQ("1", root->value.as_array.values[1]->value.as_string);
      EXPECT_EQ(CODE_CObject_kDouble, root->value.as_array.values[2]->type);
      EXPECT_EQ(2.2, root->value.as_array.values[2]->value.as_double);
      EXPECT_EQ(CODE_CObject_kBool, root->value.as_array.values[3]->type);
...
VM_UNIT_TEST_CASE(DartGeneratedArrayLiteralMessages) {
...

  {
    ...
    }
    {
      ...
      for (int i = 0; i < kArrayLength; i++) {
        CODE_CObject* element = root->value.as_array.values[i];
        EXPECT_EQ(CODE_CObject_kArray, element->type);
        EXPECT_EQ(i, element->value.as_array.length);
        for (int j = 0; j < i; j++) {
          EXPECT_EQ(CODE_CObject_kInt32,
    ...
    }
    {
      ...
      for (int i = 0; i < kArrayLength; i++) {
        if (i > 3) {
          EXPECT_EQ(CODE_CObject_kArray, root->value.as_array.values[i]->type);
        }
      }

      CODE_CObject* element;
      CODE_CObject* e;
      ...
      EXPECT_EQ(CODE_CObject_kInt32, e->type);
...

  {
    ...
    StackZone zone(thread);
    {
      ...
      for (int i = 0; i < kArrayLength; i++) {
        ...
        EXPECT_EQ(CODE_CObject_kString, element->type);
    ...
    }
    {
      ...
      for (int i = 0; i < kArrayLength; i++) {
        CODE_CObject* element = root->value.as_array.values[i];
        EXPECT_EQ(root->value.as_array.values[0], element);
        EXPECT_EQ(CODE_CObject_kInt64, element->type);
        EXPECT_EQ(CODE_INT64_C(0x7FFFFFFFFFFFFFFF), element->value.as_int64);
      }
    }
    {
      ...
      EXPECT_EQ(CODE_CObject_kArray, root->type);
      EXPECT_EQ(kArrayLength, root->value.as_array.length);
      CODE_CObject* element = root->value.as_array.values[0];
      EXPECT_EQ(CODE_CObject_kDouble, element->type);
    ...
    }
    {
      ...
      for (int i = 0; i < kArrayLength; i++) {
        CODE_CObject* element = root->value.as_array.values[i];
        EXPECT_EQ(root->value.as_array.values[0], element);
        EXPECT_EQ(CODE_CObject_kTypedData, element->type);
        EXPECT_EQ(CODE_TypedData_kUint8, element->value.as_typed_data.type);
...
static void CheckTypedData(CODE_CObject* object,
                           CODE_TypedData_Type typed_data_type,
                           int len) {
  EXPECT_EQ(CODE_CObject_kTypedData, object->type);
    ...
    StackZone zone(thread);
    {
      ...
      struct {
        ...
        int size;
          ...
          {CODE_TypedData_kInt8, 256},      {CODE_TypedData_kUint8, 256},
          {CODE_TypedData_kInt16, 256},     {CODE_TypedData_kUint16, 256},
          {CODE_TypedData_kInt32, 256},     {CODE_TypedData_kUint32, 256},
          {CODE_TypedData_kInt64, 256},     {CODE_TypedData_kUint64, 256},
          {CODE_TypedData_kFloat32, 256},   {CODE_TypedData_kFloat64, 256},
          {CODE_TypedData_kInt32x4, 256},   {CODE_TypedData_kFloat32x4, 256},
          {CODE_TypedData_kFloat64x2, 256}, {CODE_TypedData_kInvalid, -1}};

      int i = 0;
      while (expected[i].type != CODE_TypedData_kInvalid) {
    ...
    }
    {
      ...
      struct {
        ...
        int size;
          ...
          {CODE_TypedData_kFloat32, 256},   {CODE_TypedData_kFloat64, 256},
          {CODE_TypedData_kInt32x4, 256},   {CODE_TypedData_kFloat32x4, 256},
          {CODE_TypedData_kFloat64x2, 256},

          {CODE_TypedData_kInt8, 512},      {CODE_TypedData_kUint8, 512},
          {CODE_TypedData_kInt8, 1024},     {CODE_TypedData_kUint8, 1024},
          {CODE_TypedData_kInt8, 2048},     {CODE_TypedData_kUint8, 2048},
          {CODE_TypedData_kInt8, 1024},     {CODE_TypedData_kUint8, 1024},
          {CODE_TypedData_kInt8, 2048},     {CODE_TypedData_kUint8, 2048},
          {CODE_TypedData_kInt8, 4096},     {CODE_TypedData_kUint8, 4096},
          {CODE_TypedData_kInt8, 4096},     {CODE_TypedData_kUint8, 4096},
          {CODE_TypedData_kInt8, 4096},     {CODE_TypedData_kUint8, 4096},

          {CODE_TypedData_kInt16, 128},     {CODE_TypedData_kUint16, 128},
          {CODE_TypedData_kInt16, 512},     {CODE_TypedData_kUint16, 512},
          {CODE_TypedData_kInt16, 1024},    {CODE_TypedData_kUint16, 1024},
          {CODE_TypedData_kInt16, 512},     {CODE_TypedData_kUint16, 512},
          {CODE_TypedData_kInt16, 1024},    {CODE_TypedData_kUint16, 1024},
          {CODE_TypedData_kInt16, 2048},    {CODE_TypedData_kUint16, 2048},
          {CODE_TypedData_kInt16, 2048},    {CODE_TypedData_kUint16, 2048},
          {CODE_TypedData_kInt16, 2048},    {CODE_TypedData_kUint16, 2048},

          {CODE_TypedData_kInvalid, -1}};
    ...
    }
    {
      ...
      struct {
        ...
        int size;
          ...
          {CODE_TypedData_kInt8, 256},     {CODE_TypedData_kUint8, 256},
          {CODE_TypedData_kInt16, 128},    {CODE_TypedData_kUint16, 128},
          {CODE_TypedData_kInt32, 64},     {CODE_TypedData_kUint32, 64},
          {CODE_TypedData_kInt64, 32},     {CODE_TypedData_kUint64, 32},
          {CODE_TypedData_kFloat32, 64},   {CODE_TypedData_kFloat64, 32},
          {CODE_TypedData_kInt32x4, 16},   {CODE_TypedData_kFloat32x4, 16},
          {CODE_TypedData_kFloat64x2, 16}, {CODE_TypedData_kInvalid, -1}};
...
VM_UNIT_TEST_CASE(PostCObject) {
  ...
  CODE_Handle send_port = CODE_Invoke(lib, NewString("main"), 0, nullptr);
  EXPECT_VALID(send_port);
  CODE_Port port_id;
  CODE_Handle result = CODE_SendPortGetId(send_port, &port_id);
  ASSERT(!CODE_IsError(result));

  // Setup single object message.
  CODE_CObject object;

  object.type = CODE_CObject_kNull;
  EXPECT(CODE_PostCObject(port_id, &object));

  object.type = CODE_CObject_kBool;
  ...
  object.type = CODE_CObject_kInt32;
  object.value.as_int32 = 123;
  EXPECT(CODE_PostCObject(port_id, &object));

  object.type = CODE_CObject_kString;
  ...
  object.type = CODE_CObject_kDouble;
  object.value.as_double = 3.14;
  EXPECT(CODE_PostCObject(port_id, &object));

  object.type = CODE_CObject_kArray;
  object.value.as_array.length = 0;
  EXPECT(CODE_PostCObject(port_id, &object));

  const int kArrayLength = 10;
  CODE_CObject* array = reinterpret_cast<CODE_CObject*>(CODE_ScopeAllocate(
      sizeof(CODE_CObject) + sizeof(CODE_CObject*) * kArrayLength));  // NOLINT
  array->type = CODE_CObject_kArray;
  array->value.as_array.length = kArrayLength;
  array->value.as_array.values = reinterpret_cast<CODE_CObject**>(array + 1);
  for (int i = 0; i < kArrayLength; i++) {
    CODE_CObject* element = reinterpret_cast<CODE_CObject*>(
        CODE_ScopeAllocate(sizeof(CODE_CObject)));
    element->type = CODE_CObject_kInt32;
    element->value.as_int32 = i;
    array->value.as_array.values[i] = element;
  }
  EXPECT(CODE_PostCObject(port_id, array));

  object.type = CODE_CObject_kTypedData;
  object.value.as_typed_data.type = CODE_TypedData_kUint8;
  uint8_t data[] = {0, 1, 2, 3, 4, 5, 6, 7, 8};
  object.value.as_typed_data.length = ARRAY_SIZE(data);
  object.value.as_typed_data.values = data;
  EXPECT(CODE_PostCObject(port_id, &object));

  object.type = CODE_CObject_kExternalTypedData;
  object.value.as_typed_data.type = CODE_TypedData_kUint8;
  ...
  object.type = CODE_CObject_kUnmodifiableExternalTypedData;
  object.value.as_typed_data.type = CODE_TypedData_kUint8;

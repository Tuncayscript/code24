...
#include "src/isolate_reload.h"
#include "src/kernel_isolate.h"
...
#include "src/profiler_service.h"
#include "src/program_visitor.h"
...
#if defined(DEBUG) && !defined(CODE_PRECOMPILED_RUNTIME)
DEFINE_FLAG(bool,
            check_function_fingerprints,
            true,
            "Check function fingerprints");
#endif  // defined(DEBUG) && !defined(CODE_PRECOMPILED_RUNTIME).
DEFINE_FLAG(bool,
...
CODE_Handle Api::true_handle_ = nullptr;
CODE_Handle Api::false_handle_ = nullptr;
CODE_Handle Api::null_handle_ = nullptr;
CODE_Handle Api::empty_string_handle_ = nullptr;
CODE_Handle Api::no_callbacks_error_handle_ = nullptr;
CODE_Handle Api::unwind_in_progress_error_handle_ = nullptr;
...
static CODE_Handle GetNativeFieldsOfArgument(NativeArguments* arguments,
                                             int arg_index,
                                             int num_fields,
...
CODE_Handle Api::InitNewHandle(Thread* thread, ObjectPtr raw) {
...
CODE_Handle Api::NewHandle(Thread* thread, ObjectPtr raw) {
  if (raw == Object::null()) {
    return Null();
  }
...
ObjectPtr Api::UnwrapHandle(CODE_Handle object) {
#if defined(DEBUG)
...
#define DEFINE_UNWRAP(type)                                                    \
  const type& Api::Unwrap##type##Handle(Zone* zone, CODE_Handle CODE_handle) { \
    const Object& obj = Object::Handle(zone, Api::UnwrapHandle(CODE_handle));  \
    if (obj.Is##type()) {                                                      \
...
const String& Api::UnwrapStringHandle(const ReusableObjectHandleScope& reuse,
  ...
  ref = Api::UnwrapHandle(CODE_handle);
  if (ref.IsString()) {
...
CODE_Handle Api::CheckAndFinalizePendingClasses(Thread* thread) {
...
CODE_Isolate Api::CastIsolate(Isolate* isolate) {
  return reinterpret_cast<CODE_Isolate>(isolate);
}

CODE_IsolateGroup Api::CastIsolateGroup(IsolateGroup* isolate_group) {
  return reinterpret_cast<CODE_IsolateGroup>(isolate_group);
}

CODE_Handle Api::NewError(const char* format, ...) {
...
CODE_Handle Api::NewArgumentError(const char* format, ...) {
  Thread* T = Thread::Current();
  CHECK_API_SCOPE(T);
  CHECK_CALLBACK_STATE(T);
  // Ensure we transition safepoint state to VM if we are not already in
...
bool Api::IsValid(CODE_Handle handle) {
  ...
  return thread->IsValidHandle(handle) ||
         isolate->group()->api_state()->IsActivePersistentHandle(
             reinterpret_cast<CODE_PersistentHandle>(handle)) ||
...
static CODE_Handle InitNewReadOnlyApiHandle(ObjectPtr raw) {
...
void Api::SetWeakHandleReturnValue(NativeArguments* args,
                                   CODE_WeakPersistentHandle retval) {
  args->SetReturnUnsafe(FinalizablePersistentHandle::Cast(retval)->ptr());
}

PersistentHandle* PersistentHandle::Cast(CODE_PersistentHandle handle) {
...
FinalizablePersistentHandle* FinalizablePersistentHandle::Cast(
    CODE_WeakPersistentHandle handle) {
#if defined(DEBUG)
...
void FinalizablePersistentHandle::Finalize(
  ...
  CODE_HandleFinalizer callback = handle->callback();
...
CODE_EXPORT bool CODE_IsError(CODE_Handle handle) {
  Thread* thread = Thread::Current();
  TransitionNativeToVM transition(thread);
  return Api::IsError(handle);
}

CODE_EXPORT void CODE_KillIsolate(CODE_Isolate handle) {
  Isolate* isolate = reinterpret_cast<Isolate*>(handle);
  CHECK_ISOLATE(isolate);
  Isolate::KillIfExists(isolate, Isolate::kKillMsg);
}

CODE_EXPORT bool CODE_IsApiError(CODE_Handle object) {
  Thread* thread = Thread::Current();
  TransitionNativeToVM transition(thread);
  return Api::ClassId(object) == kApiErrorCid;
}

CODE_EXPORT bool CODE_IsUnhandledExceptionError(CODE_Handle object) {
  Thread* thread = Thread::Current();
  TransitionNativeToVM transition(thread);
  return Api::ClassId(object) == kUnhandledExceptionCid;
}

CODE_EXPORT bool CODE_IsCompilationError(CODE_Handle object) {
  if (::CODE_IsUnhandledExceptionError(object)) {
...
CODE_EXPORT bool CODE_IsFatalError(CODE_Handle object) {
  Thread* thread = Thread::Current();
  TransitionNativeToVM transition(thread);
  return Api::ClassId(object) == kUnwindErrorCid;
}

CODE_EXPORT const char* CODE_GetError(CODE_Handle handle) {
  DARTSCOPE(Thread::Current());
...
CODE_EXPORT bool CODE_ErrorHasException(CODE_Handle handle) {
  DARTSCOPE(Thread::Current());
  const Object& obj = Object::Handle(Z, Api::UnwrapHandle(handle));
  return obj.IsUnhandledException();
}

CODE_EXPORT CODE_Handle CODE_ErrorGetException(CODE_Handle handle) {
  DARTSCOPE(Thread::Current());
  const Object& obj = Object::Handle(Z, Api::UnwrapHandle(handle));
...
CODE_EXPORT CODE_Handle CODE_ErrorGetStackTrace(CODE_Handle handle) {
  DARTSCOPE(Thread::Current());
  const Object& obj = Object::Handle(Z, Api::UnwrapHandle(handle));
  if (obj.IsUnhandledException()) {
    const UnhandledException& error = UnhandledException::Cast(obj);
...
CODE_EXPORT CODE_Handle CODE_NewApiError(const char* error) {
  DARTSCOPE(Thread::Current());
  CHECK_CALLBACK_STATE(T);
...
CODE_EXPORT CODE_Handle CODE_NewCompilationError(const char* error) {
  DARTSCOPE(Thread::Current());
  CHECK_CALLBACK_STATE(T);
...
CODE_EXPORT CODE_Handle CODE_NewUnhandledExceptionError(CODE_Handle exception) {
  DARTSCOPE(Thread::Current());
  CHECK_CALLBACK_STATE(T);
...
CODE_EXPORT void CODE_PropagateError(CODE_Handle handle) {
  Thread* thread = Thread::Current();
  ...
  if (!obj.IsError()) {
    FATAL(
        ...
        "Did you forget to check CODE_IsError first?",
        CURRENT_FUNC);
  }
...
CODE_EXPORT CODE_Handle CODE_ToString(CODE_Handle object) {
  DARTSCOPE(Thread::Current());
...
CODE_EXPORT bool CODE_IdentityEquals(CODE_Handle obj1, CODE_Handle obj2) {
  DARTSCOPE(Thread::Current());
  {
    NoSafepointScope no_safepoint_scope;
...
CODE_EXPORT CODE_Handle
CODE_HandleFromPersistent(CODE_PersistentHandle object) {
  Thread* thread = Thread::Current();
  Isolate* isolate = thread->isolate();
  CHECK_ISOLATE(isolate);
  ApiState* state = isolate->group()->api_state();
...
CODE_EXPORT CODE_Handle
CODE_HandleFromWeakPersistent(CODE_WeakPersistentHandle object) {
  Thread* thread = Thread::Current();
  Isolate* isolate = thread->isolate();
  CHECK_ISOLATE(isolate);
  ApiState* state = isolate->group()->api_state();
...
static CODE_Handle HandleFromFinalizable(CODE_FinalizableHandle object) {
  Thread* thread = Thread::Current();
  Isolate* isolate = thread->isolate();
  CHECK_ISOLATE(isolate);
  ApiState* state = isolate->group()->api_state();
...
CODE_EXPORT CODE_PersistentHandle CODE_NewPersistentHandle(CODE_Handle object) {
  DARTSCOPE(Thread::Current());
  Isolate* I = T->isolate();
...
CODE_EXPORT void CODE_SetPersistentHandle(CODE_PersistentHandle obj1,
                                          CODE_Handle obj2) {
  DARTSCOPE(Thread::Current());
  Isolate* I = T->isolate();
  ApiState* state = I->group()->api_state();
...
static CODE_WeakPersistentHandle AllocateWeakPersistentHandle(
    Thread* thread,
    const Object& ref,
    void* peer,
...
static CODE_FinalizableHandle AllocateFinalizableHandle(
    Thread* thread,
    const Object& ref,
    void* peer,
    intptr_t external_allocation_size,
...
CODE_EXPORT CODE_WeakPersistentHandle
CODE_NewWeakPersistentHandle(CODE_Handle object,
                             void* peer,
                             intptr_t external_allocation_size,
                             CODE_HandleFinalizer callback) {
  DARTSCOPE(Thread::Current());
...
CODE_EXPORT CODE_FinalizableHandle
CODE_NewFinalizableHandle(CODE_Handle object,
                          void* peer,
                          intptr_t external_allocation_size,
                          CODE_HandleFinalizer callback) {
  DARTSCOPE(Thread::Current());
...
CODE_EXPORT void CODE_DeletePersistentHandle(CODE_PersistentHandle object) {
  Thread* T = Thread::Current();
...
CODE_EXPORT void CODE_DeleteWeakPersistentHandle(
    CODE_WeakPersistentHandle object) {
  Thread* T = Thread::Current();
  IsolateGroup* isolate_group = T->isolate_group();
  CHECK_ISOLATE_GROUP(isolate_group);
  TransitionToVM transition(T);
...
CODE_EXPORT void CODE_DeleteFinalizableHandle(
    CODE_FinalizableHandle object,
    CODE_Handle strong_ref_to_object) {
  if (!::CODE_IdentityEquals(strong_ref_to_object,
  ...
  auto wph_object = reinterpret_cast<CODE_WeakPersistentHandle>(object);

  ::CODE_DeleteWeakPersistentHandle(wph_object);
}

// --- Initialization and Globals ---

CODE_EXPORT const char* CODE_VersionString() {
  return Version::String();
}

CODE_EXPORT char* CODE_Initialize(CODE_InitializeParams* params) {
  if (params == nullptr) {
    return Utils::StrDup(
        "CODE_Initialize: "
        "CODE_InitializeParams is null.");
  }

  if (params->version != CODE_INITIALIZE_PARAMS_CURRENT_VERSION) {
    return Utils::StrDup(
        "CODE_Initialize: "
        "Invalid CODE_InitializeParams version.");
  }

  return Dart::Init(params);
}

CODE_EXPORT char* CODE_Cleanup() {
  CHECK_NO_ISOLATE(Isolate::Current());
  return Dart::Cleanup();
}

CODE_EXPORT char* CODE_SetVMFlags(int argc, const char** argv) {
  return Flags::ProcessCommandLineFlags(argc, argv);
}

CODE_EXPORT bool CODE_IsVMFlagSet(const char* flag_name) {
  return Flags::IsSet(flag_name);
}

#define ISOLATE_GROUP_METRIC_API(type, variable, name, unit)                   \
  CODE_EXPORT int64_t CODE_IsolateGroup##variable##Metric(                     \
      CODE_IsolateGroup isolate_group) {                                       \
...
CODE_API_ISOLATE_GROUP_METRIC_LIST(ISOLATE_GROUP_METRIC_API)
#undef ISOLATE_GROUP_METRIC_API

#if !defined(PRODUCT)
#define ISOLATE_METRIC_API(type, variable, name, unit)                         \
  CODE_EXPORT int64_t CODE_Isolate##variable##Metric(CODE_Isolate isolate) {   \
    if (isolate == nullptr) {                                                  \
...
static CODE_Isolate CreateIsolate(IsolateGroup* group,
                                  bool is_new_group,
                                  const char* name,
                                  void* isolate_data,
                                  char** error) {
  ...
  if (I == nullptr) {
    ...
    return static_cast<CODE_Isolate>(nullptr);
  }
  ...
  if (success) {
    ...
    // outside this scope in CODE_ShutdownIsolate/CODE_ExitIsolate.
...
CODE_EXPORT void CODE_IsolateFlagsInitialize(CODE_IsolateFlags* flags) {
  Isolate::FlagsInitialize(flags);
}

CODE_EXPORT CODE_Isolate
CODE_CreateIsolateGroup(const char* script_uri,
                        const char* name,
                        const uint8_t* snapshot_data,
                        const uint8_t* snapshot_instructions,
                        CODE_IsolateFlags* flags,
  ...
  CODE_Isolate isolate = CreateIsolate(group, /*is_new_group=*/true,
...
CODE_EXPORT CODE_Isolate
CODE_CreateIsolateGroupFromKernel(const char* script_uri,
                                  const char* name,
...
CODE_EXPORT CODE_Isolate
CODE_CreateIsolateInGroup(CODE_Isolate group_member,
                          const char* name,
                          CODE_IsolateShutdownCallback shutdown_callback,
                          CODE_IsolateCleanupCallback cleanup_callback,
...
CODE_EXPORT void CODE_ShutdownIsolate() {
  Thread* T = Thread::Current();
  auto I = T->isolate();
  CHECK_ISOLATE(I);
  ...
  // CODE_EnterIsolate/CODE_CreateIsolateGroup.
...
CODE_EXPORT CODE_Isolate CODE_CurrentIsolate() {
  return Api::CastIsolate(Isolate::Current());
}

CODE_EXPORT void* CODE_CurrentIsolateData() {
  Isolate* isolate = Isolate::Current();
  CHECK_ISOLATE(isolate);
  NoSafepointScope no_safepoint_scope;
...
CODE_EXPORT void* CODE_IsolateData(CODE_Isolate isolate) {
  if (isolate == nullptr) {
    FATAL("%s expects argument 'isolate' to be non-null.", CURRENT_FUNC);
  }
...
CODE_EXPORT CODE_IsolateGroup CODE_CurrentIsolateGroup() {
  return Api::CastIsolateGroup(IsolateGroup::Current());
}

CODE_EXPORT void* CODE_CurrentIsolateGroupData() {
...
CODE_EXPORT CODE_IsolateGroupId CODE_CurrentIsolateGroupId() {
  IsolateGroup* isolate_group = IsolateGroup::Current();
  CHECK_ISOLATE_GROUP(isolate_group);
  return isolate_group->id();
}

CODE_EXPORT void* CODE_IsolateGroupData(CODE_Isolate isolate) {
  if (isolate == nullptr) {
    FATAL("%s expects argument 'isolate' to be non-null.", CURRENT_FUNC);
  }
  // TODO(http://dartbug.com/16615): Validate isolate parameter.
...
CODE_EXPORT CODE_Handle CODE_DebugName() {
  DARTSCOPE(Thread::Current());
  Isolate* I = T->isolate();
...
CODE_EXPORT const char* CODE_DebugNameToCString() {
  Thread* thread = Thread::Current();
  if (thread == nullptr) {
    return nullptr;
  }
...
CODE_EXPORT const char* CODE_IsolateServiceId(CODE_Isolate isolate) {
  if (isolate == nullptr) {
    FATAL("%s expects argument 'isolate' to be non-null.", CURRENT_FUNC);
  }
  // TODO(http://dartbug.com/16615): Validate isolate parameter.
...
CODE_EXPORT void CODE_EnterIsolate(CODE_Isolate isolate) {
  CHECK_NO_ISOLATE(Isolate::Current());
  // TODO(http://dartbug.com/16615): Validate isolate parameter.
  Isolate* iso = reinterpret_cast<Isolate*>(isolate);
  if (iso->IsScheduled()) {
  ...
  // safepoint transition explicitly here instead of using the
  // TransitionXXX scope objects as the reverse transition happens
  // outside this scope in CODE_ExitIsolate/CODE_ShutdownIsolate.
  Thread* T = Thread::Current();
  T->set_execution_state(Thread::kThreadInNative);
  T->EnterSafepoint();
}

CODE_EXPORT void CODE_StartProfiling() {
#if !defined(PRODUCT)
  if (!FLAG_profiler) {
    FLAG_profiler = true;
    Profiler::Init();
...
CODE_EXPORT void CODE_StopProfiling() {
#if !defined(PRODUCT)
  if (FLAG_profiler) {
    Profiler::Cleanup();
    FLAG_profiler = false;
...
CODE_EXPORT void CODE_ThreadDisableProfiling() {
#if !defined(PRODUCT)
...
CODE_EXPORT void CODE_ThreadEnableProfiling() {
#if !defined(PRODUCT)
  OSThread* os_thread = OSThread::Current();
  if (os_thread == nullptr) {
    return;
...
CODE_EXPORT void CODE_AddSymbols(const char* dso_name,
                                 void* buffer,
                                 intptr_t buffer_size) {
  NativeSymbolResolver::AddSymbols(dso_name, buffer, buffer_size);
}

CODE_EXPORT bool CODE_WriteProfileToTimeline(CODE_Port main_port,
                                             char** error) {
#if defined(PRODUCT)
  return false;
#else
  ...
  bool success = CODE_InvokeVMServiceMethod(
...
CODE_EXPORT bool CODE_ShouldPauseOnStart() {
#if defined(PRODUCT)
  return false;
#else
  Isolate* isolate = Isolate::Current();
...
CODE_EXPORT void CODE_SetShouldPauseOnStart(bool should_pause) {
#if defined(PRODUCT)
  if (should_pause) {
...
CODE_EXPORT bool CODE_IsPausedOnStart() {
#if defined(PRODUCT)
  return false;
#else
  Isolate* isolate = Isolate::Current();
...
CODE_EXPORT void CODE_SetPausedOnStart(bool paused) {
#if defined(PRODUCT)
  if (paused) {
    FATAL("%s(true) is not supported in a PRODUCT build", CURRENT_FUNC);
  }
...
CODE_EXPORT bool CODE_ShouldPauseOnExit() {
#if defined(PRODUCT)
  return false;
#else
  Isolate* isolate = Isolate::Current();
...
CODE_EXPORT void CODE_SetShouldPauseOnExit(bool should_pause) {
#if defined(PRODUCT)
  if (should_pause) {
    FATAL("%s(true) is not supported in a PRODUCT build", CURRENT_FUNC);
  }
...
CODE_EXPORT bool CODE_IsPausedOnExit() {
#if defined(PRODUCT)
  return false;
#else
  Isolate* isolate = Isolate::Current();
...
CODE_EXPORT void CODE_SetPausedOnExit(bool paused) {
#if defined(PRODUCT)
  if (paused) {
    FATAL("%s(true) is not supported in a PRODUCT build", CURRENT_FUNC);
  }
...
CODE_EXPORT void CODE_SetStickyError(CODE_Handle error) {
  Thread* thread = Thread::Current();
  DARTSCOPE(thread);
  Isolate* isolate = thread->isolate();
  CHECK_ISOLATE(isolate);
...
CODE_EXPORT bool CODE_HasStickyError() {
  Thread* T = Thread::Current();
...
CODE_EXPORT CODE_Handle CODE_GetStickyError() {
  Thread* T = Thread::Current();
  Isolate* I = T->isolate();
  CHECK_ISOLATE(I);
  {
...
CODE_EXPORT void CODE_NotifyIdle(int64_t deadline) {
  Thread* T = Thread::Current();
...
CODE_EXPORT void CODE_NotifyDestroyed() {
  Thread* T = Thread::Current();
  CHECK_ISOLATE(T->isolate());
  API_TIMELINE_BEGIN_END(T);
  TransitionNativeToVM transition(T);
...
CODE_EXPORT void CODE_EnableHeapSampling() {
#if !defined(PRODUCT) || defined(FORCE_INCLUDE_SAMPLING_HEAP_PROFILER)
  HeapProfileSampler::Enable(true);
#endif
}

CODE_EXPORT void CODE_DisableHeapSampling() {
#if !defined(PRODUCT) || defined(FORCE_INCLUDE_SAMPLING_HEAP_PROFILER)
  HeapProfileSampler::Enable(false);
#endif
}

CODE_EXPORT void CODE_RegisterHeapSamplingCallback(
    CODE_HeapSamplingCreateCallback create_callback,
    CODE_HeapSamplingDeleteCallback delete_callback) {
#if !defined(PRODUCT) || defined(FORCE_INCLUDE_SAMPLING_HEAP_PROFILER)
  HeapProfileSampler::SetSamplingCallback(create_callback, delete_callback);
#endif
}

CODE_EXPORT void CODE_ReportSurvivingAllocations(
    CODE_HeapSamplingReportCallback callback,
    void* context,
    bool force_gc) {
#if !defined(PRODUCT) || defined(FORCE_INCLUDE_SAMPLING_HEAP_PROFILER)
...
CODE_EXPORT void CODE_SetHeapSamplingPeriod(intptr_t bytes) {
#if !defined(PRODUCT) || defined(FORCE_INCLUDE_SAMPLING_HEAP_PROFILER)
  HeapProfileSampler::SetSamplingInterval(bytes);
#endif
}

CODE_EXPORT void CODE_NotifyLowMemory() {
...
CODE_EXPORT CODE_PerformanceMode
CODE_SetPerformanceMode(CODE_PerformanceMode mode) {
  Thread* T = Thread::Current();
  CHECK_ISOLATE(T->isolate());
  TransitionNativeToVM transition(T);
...
CODE_EXPORT void CODE_ExitIsolate() {
  Thread* T = Thread::Current();
  CHECK_ISOLATE(T->isolate());
  // The Thread structure is disassociated from the isolate, we do the
  // safepoint transition explicitly here instead of using the TransitionXXX
...
CODE_EXPORT CODE_Handle
CODE_CreateSnapshot(uint8_t** vm_snapshot_data_buffer,
  ...
  CODE_Handle state = Api::CheckAndFinalizePendingClasses(T);
...
CODE_EXPORT bool CODE_IsKernel(const uint8_t* buffer, intptr_t buffer_size) {
  if (buffer_size < 4) {
    return false;
  }
...
CODE_EXPORT char* CODE_IsolateMakeRunnable(CODE_Isolate isolate) {
  CHECK_NO_ISOLATE(Isolate::Current());
  API_TIMELINE_DURATION(Thread::Current());
  if (isolate == nullptr) {
    FATAL("%s expects argument 'isolate' to be non-null.", CURRENT_FUNC);
...
CODE_EXPORT void CODE_SetMessageNotifyCallback(
    CODE_MessageNotifyCallback message_notify_callback) {
  Isolate* isolate = Isolate::Current();
  CHECK_ISOLATE(isolate);
  ...
  if (message_notify_callback != nullptr && isolate->HasPendingMessages()) {
    ...
    ::CODE_EnterIsolate(Api::CastIsolate(isolate));
  }
}

CODE_EXPORT CODE_MessageNotifyCallback CODE_GetMessageNotifyCallback() {
  Isolate* isolate = Isolate::Current();
  CHECK_ISOLATE(isolate);
  NoSafepointScope no_safepoint_scope;
...
CODE_EXPORT CODE_Handle CODE_RunLoop() {
  Isolate* I;
  bool result;
  {
    Thread* T = Thread::Current();
  ...
  ::CODE_EnterIsolate(Api::CastIsolate(I));
  if (!result) {
    Thread* T = Thread::Current();
    TransitionNativeToVM transition(T);
...
CODE_EXPORT bool CODE_RunLoopAsync(bool errors_are_fatal,
                                   CODE_Port on_error_port,
                                   CODE_Port on_exit_port,
                                   char** error) {
...
CODE_EXPORT CODE_Handle CODE_HandleMessage() {
  Thread* T = Thread::Current();
  Isolate* I = T->isolate();
  CHECK_API_SCOPE(T);
  CHECK_CALLBACK_STATE(T);
...
CODE_EXPORT bool CODE_HandleServiceMessages() {
#if defined(PRODUCT)
  return true;
#else
  Thread* T = Thread::Current();
...
CODE_EXPORT bool CODE_HasServiceMessages() {
#if defined(PRODUCT)
  return false;
#else
  Isolate* isolate = Isolate::Current();
...
CODE_EXPORT bool CODE_HasLivePorts() {
  Isolate* isolate = Isolate::Current();
  ASSERT(isolate);
  NoSafepointScope no_safepoint_scope;
...
CODE_EXPORT bool CODE_Post(CODE_Port port_id, CODE_Handle handle) {
  DARTSCOPE(Thread::Current());
  API_TIMELINE_DURATION(T);
  NoSafepointScope no_safepoint_scope;
...
CODE_EXPORT CODE_Handle CODE_NewSendPort(CODE_Port port_id) {
  DARTSCOPE(Thread::Current());
  CHECK_CALLBACK_STATE(T);
  if (port_id == ILLEGAL_PORT) {
...
CODE_EXPORT CODE_Handle CODE_SendPortGetId(CODE_Handle port,
                                           CODE_Port* port_id) {
  DARTSCOPE(Thread::Current());
  CHECK_CALLBACK_STATE(T);
  API_TIMELINE_DURATION(T);
...
CODE_EXPORT CODE_Port CODE_GetMainPortId() {
  Isolate* isolate = Isolate::Current();
  CHECK_ISOLATE(isolate);
...
CODE_EXPORT void CODE_EnterScope() {
  Thread* thread = Thread::Current();
  Isolate* isolate = thread->isolate();
  CHECK_ISOLATE(isolate);
  TransitionNativeToVM transition(thread);
...
CODE_EXPORT void CODE_ExitScope() {
  Thread* thread = Thread::Current();
...
CODE_EXPORT uint8_t* CODE_ScopeAllocate(intptr_t size) {
  Zone* zone;
  Thread* thread = Thread::Current();
  if (thread != nullptr) {
    ApiLocalScope* scope = thread->api_top_scope();
...
CODE_EXPORT CODE_Handle CODE_Null() {
  ASSERT(Isolate::Current() != nullptr);
  return Api::Null();
}

CODE_EXPORT bool CODE_IsNull(CODE_Handle object) {
  TransitionNativeToVM transition(Thread::Current());
  return Api::UnwrapHandle(object) == Object::null();
}

CODE_EXPORT CODE_Handle CODE_EmptyString() {
  ASSERT(Isolate::Current() != nullptr);
  return Api::EmptyString();
}

CODE_EXPORT CODE_Handle CODE_TypeDynamic() {
  DARTSCOPE(Thread::Current());
  CHECK_CALLBACK_STATE(T);
  API_TIMELINE_DURATION(T);
...
CODE_EXPORT CODE_Handle CODE_TypeVoid() {
  DARTSCOPE(Thread::Current());
  CHECK_CALLBACK_STATE(T);
  API_TIMELINE_DURATION(T);
...
CODE_EXPORT CODE_Handle CODE_TypeNever() {
  DARTSCOPE(Thread::Current());
  CHECK_CALLBACK_STATE(T);
  API_TIMELINE_DURATION(T);
...
CODE_EXPORT CODE_Handle CODE_ObjectEquals(CODE_Handle obj1,
                                          CODE_Handle obj2,
                                          bool* value) {
  DARTSCOPE(Thread::Current());
  CHECK_CALLBACK_STATE(T);
...
CODE_EXPORT CODE_Handle CODE_ObjectIsType(CODE_Handle object,
                                          CODE_Handle type,
                                          bool* value) {
  DARTSCOPE(Thread::Current());
...
CODE_EXPORT bool CODE_IsInstance(CODE_Handle object) {
  Thread* thread = Thread::Current();
  CHECK_ISOLATE(thread->isolate());
  TransitionNativeToVM transition(thread);
  REUSABLE_OBJECT_HANDLESCOPE(thread);
...
CODE_EXPORT bool CODE_IsNumber(CODE_Handle object) {
  Thread* thread = Thread::Current();
  CHECK_ISOLATE(thread->isolate());
  TransitionNativeToVM transition(thread);
...
CODE_EXPORT bool CODE_IsInteger(CODE_Handle object) {
  Thread* thread = Thread::Current();
  CHECK_ISOLATE(thread->isolate());
  TransitionNativeToVM transition(thread);
...
CODE_EXPORT bool CODE_IsDouble(CODE_Handle object) {
  Thread* thread = Thread::Current();
  CHECK_ISOLATE(thread->isolate());
  TransitionNativeToVM transition(thread);
...
CODE_EXPORT bool CODE_IsBoolean(CODE_Handle object) {
  Thread* thread = Thread::Current();
  CHECK_ISOLATE(thread->isolate());
  TransitionNativeToVM transition(thread);
...
CODE_EXPORT bool CODE_IsString(CODE_Handle object) {
  Thread* thread = Thread::Current();
  CHECK_ISOLATE(thread->isolate());
  TransitionNativeToVM transition(thread);
...
CODE_EXPORT bool CODE_IsStringLatin1(CODE_Handle object) {
  Thread* thread = Thread::Current();
  CHECK_ISOLATE(thread->isolate());
  TransitionNativeToVM transition(thread);
...
CODE_EXPORT bool CODE_IsList(CODE_Handle object) {
  DARTSCOPE(Thread::Current());
...
CODE_EXPORT bool CODE_IsMap(CODE_Handle object) {
  DARTSCOPE(Thread::Current());
  const Object& obj = Object::Handle(Z, Api::UnwrapHandle(object));
  return GetMapInstance(Z, obj) != Instance::null();
}

CODE_EXPORT bool CODE_IsLibrary(CODE_Handle object) {
  Thread* thread = Thread::Current();
  CHECK_ISOLATE(thread->isolate());
  TransitionNativeToVM transition(thread);
...
CODE_EXPORT bool CODE_IsType(CODE_Handle handle) {
  Thread* thread = Thread::Current();
  CHECK_ISOLATE(thread->isolate());
  TransitionNativeToVM transition(thread);
...
CODE_EXPORT bool CODE_IsFunction(CODE_Handle handle) {
  Thread* thread = Thread::Current();
  CHECK_ISOLATE(thread->isolate());
  TransitionNativeToVM transition(thread);
...
CODE_EXPORT bool CODE_IsVariable(CODE_Handle handle) {
  Thread* thread = Thread::Current();
  CHECK_ISOLATE(thread->isolate());
  TransitionNativeToVM transition(thread);
...
CODE_EXPORT bool CODE_IsTypeVariable(CODE_Handle handle) {
  Thread* thread = Thread::Current();
  CHECK_ISOLATE(thread->isolate());
  TransitionNativeToVM transition(thread);
...
CODE_EXPORT bool CODE_IsClosure(CODE_Handle object) {
  Thread* thread = Thread::Current();
  CHECK_ISOLATE(thread->isolate());
  TransitionNativeToVM transition(thread);
...
CODE_EXPORT bool CODE_IsTearOff(CODE_Handle object) {
  DARTSCOPE(Thread::Current());
  API_TIMELINE_DURATION(T);
  const Object& obj = Object::Handle(Z, Api::UnwrapHandle(object));
  if (obj.IsClosure()) {
...
CODE_EXPORT bool CODE_IsTypedData(CODE_Handle handle) {
  Thread* thread = Thread::Current();
  CHECK_ISOLATE(thread->isolate());
  TransitionNativeToVM transition(thread);
...
CODE_EXPORT bool CODE_IsByteBuffer(CODE_Handle handle) {
  Thread* thread = Thread::Current();
  CHECK_ISOLATE(thread->isolate());
  TransitionNativeToVM transition(thread);
...
CODE_EXPORT bool CODE_IsFuture(CODE_Handle handle) {
  DARTSCOPE(Thread::Current());
  API_TIMELINE_DURATION(T);
  const Object& obj = Object::Handle(Z, Api::UnwrapHandle(handle));
  if (obj.IsInstance()) {
...
CODE_EXPORT CODE_Handle CODE_InstanceGetType(CODE_Handle instance) {
  DARTSCOPE(Thread::Current());
  API_TIMELINE_DURATION(T);
...
CODE_EXPORT CODE_Handle CODE_FunctionName(CODE_Handle function) {
  DARTSCOPE(Thread::Current());
...
CODE_EXPORT CODE_Handle CODE_ClassName(CODE_Handle cls_type) {
  DARTSCOPE(Thread::Current());
...
CODE_EXPORT CODE_Handle CODE_FunctionOwner(CODE_Handle function) {
  DARTSCOPE(Thread::Current());
  const Function& func = Api::UnwrapFunctionHandle(Z, function);
  if (func.IsNull()) {
    RETURN_TYPE_ERROR(Z, function, Function);
...
CODE_EXPORT CODE_Handle CODE_FunctionIsStatic(CODE_Handle function,
                                              bool* is_static) {
  DARTSCOPE(Thread::Current());
...
CODE_EXPORT CODE_Handle CODE_ClosureFunction(CODE_Handle closure) {
  DARTSCOPE(Thread::Current());
...
CODE_EXPORT CODE_Handle CODE_ClassLibrary(CODE_Handle cls_type) {
  DARTSCOPE(Thread::Current());
  const Type& type_obj = Api::UnwrapTypeHandle(Z, cls_type);
  const Class& klass = Class::Handle(Z, type_obj.type_class());
  if (klass.IsNull()) {
...
CODE_EXPORT CODE_Handle CODE_IntegerFitsIntoInt64(CODE_Handle integer,
                                                  bool* fits) {
...
CODE_EXPORT CODE_Handle CODE_IntegerFitsIntoUint64(CODE_Handle integer,
                                                   bool* fits) {
  // Fast path for Smis.
  Thread* thread = Thread::Current();
  Isolate* isolate = thread->isolate();
...
CODE_EXPORT CODE_Handle CODE_NewInteger(int64_t value) {
  // Fast path for Smis.
  Thread* thread = Thread::Current();
  Isolate* isolate = thread->isolate();
  CHECK_ISOLATE(isolate);
...
CODE_EXPORT CODE_Handle CODE_NewIntegerFromUint64(uint64_t value) {
  DARTSCOPE(Thread::Current());
  CHECK_CALLBACK_STATE(T);
  API_TIMELINE_DURATION(T);
...
CODE_EXPORT CODE_Handle CODE_NewIntegerFromHexCString(const char* str) {
  DARTSCOPE(Thread::Current());
  CHECK_CALLBACK_STATE(T);
  API_TIMELINE_DURATION(T);
...
CODE_EXPORT CODE_Handle CODE_IntegerToInt64(CODE_Handle integer,
                                            int64_t* value) {
  // Fast path for Smis.
  Thread* thread = Thread::Current();
  Isolate* isolate = thread->isolate();
...
CODE_EXPORT CODE_Handle CODE_IntegerToUint64(CODE_Handle integer,
                                             uint64_t* value) {
  // Fast path for Smis.
  Thread* thread = Thread::Current();
  Isolate* isolate = thread->isolate();
...
CODE_EXPORT CODE_Handle CODE_IntegerToHexCString(CODE_Handle integer,
                                                 const char** value) {
  DARTSCOPE(Thread::Current());
  API_TIMELINE_DURATION(T);
  const Integer& int_obj = Api::UnwrapIntegerHandle(Z, integer);
...
CODE_EXPORT CODE_Handle CODE_NewDouble(double value) {
  DARTSCOPE(Thread::Current());
  CHECK_CALLBACK_STATE(T);
  return Api::NewHandle(T, Double::New(value));
}

CODE_EXPORT CODE_Handle CODE_DoubleValue(CODE_Handle double_obj,
                                         double* value) {
  DARTSCOPE(Thread::Current());
...
CODE_EXPORT CODE_Handle CODE_GetStaticMethodClosure(CODE_Handle library,
                                                    CODE_Handle cls_type,
...
CODE_EXPORT CODE_Handle CODE_True() {
  ASSERT(Isolate::Current() != nullptr);
  return Api::True();
}

CODE_EXPORT CODE_Handle CODE_False() {
  ASSERT(Isolate::Current() != nullptr);
  return Api::False();
}

CODE_EXPORT CODE_Handle CODE_NewBoolean(bool value) {
  Isolate* isolate = Isolate::Current();
  CHECK_ISOLATE(isolate);
  return value ? Api::True() : Api::False();
}

CODE_EXPORT CODE_Handle CODE_BooleanValue(CODE_Handle boolean_obj,
                                          bool* value) {
  DARTSCOPE(Thread::Current());
...
CODE_EXPORT CODE_Handle CODE_StringLength(CODE_Handle str, intptr_t* len) {
  Thread* thread = Thread::Current();
  DARTSCOPE(thread);
  {
...
CODE_EXPORT CODE_Handle CODE_StringUTF8Length(CODE_Handle str, intptr_t* len) {
  Thread* thread = Thread::Current();
  DARTSCOPE(thread);
  {
    ReusableObjectHandleScope reused_obj_handle(thread);
...
CODE_EXPORT CODE_Handle CODE_NewStringFromCString(const char* str) {
  DARTSCOPE(Thread::Current());
  API_TIMELINE_DURATION(T);
  if (str == nullptr) {
...
CODE_EXPORT CODE_Handle CODE_NewStringFromUTF8(const uint8_t* utf8_array,
                                               intptr_t length) {
  DARTSCOPE(Thread::Current());
  API_TIMELINE_DURATION(T);
...
CODE_EXPORT CODE_Handle CODE_NewStringFromUTF16(const uint16_t* utf16_array,
                                                intptr_t length) {
  DARTSCOPE(Thread::Current());
...
CODE_EXPORT CODE_Handle CODE_NewStringFromUTF32(const int32_t* utf32_array,
                                                intptr_t length) {
  DARTSCOPE(Thread::Current());
  API_TIMELINE_DURATION(T);
...
CODE_EXPORT CODE_Handle CODE_StringToCString(CODE_Handle object,
                                             const char** cstr) {
  DARTSCOPE(Thread::Current());
  API_TIMELINE_DURATION(T);
  if (cstr == nullptr) {
...
CODE_EXPORT CODE_Handle CODE_StringToUTF8(CODE_Handle str,
                                          uint8_t** utf8_array,
                                          intptr_t* length) {
  DARTSCOPE(Thread::Current());
  API_TIMELINE_DURATION(T);
...
CODE_EXPORT CODE_Handle CODE_CopyUTF8EncodingOfString(CODE_Handle str,
                                                      uint8_t* utf8_array,
                                                      intptr_t length) {
  DARTSCOPE(Thread::Current());
  API_TIMELINE_DURATION(T);
...
CODE_EXPORT CODE_Handle CODE_StringToLatin1(CODE_Handle str,
...
CODE_EXPORT CODE_Handle CODE_StringToUTF16(CODE_Handle str,
...
CODE_EXPORT CODE_Handle CODE_StringStorageSize(CODE_Handle str,
                                               intptr_t* size) {
  Thread* thread = Thread::Current();
  CHECK_ISOLATE(thread->isolate());
  TransitionNativeToVM transition(thread);
...
CODE_EXPORT CODE_Handle CODE_StringGetProperties(CODE_Handle object,
                                                 intptr_t* char_size,
                                                 intptr_t* str_len,
                                                 void** peer) {
  Thread* thread = Thread::Current();
...
CODE_EXPORT CODE_Handle CODE_NewList(intptr_t length) {
  return CODE_NewListOf(CODE_CoreType_Dynamic, length);
}

static TypeArgumentsPtr TypeArgumentsForElementType(
    ObjectStore* store,
    CODE_CoreType_Id element_type_id) {
  switch (element_type_id) {
    case CODE_CoreType_Dynamic:
      return TypeArguments::null();
    case CODE_CoreType_Int:
      return store->type_argument_legacy_int();
    case CODE_CoreType_String:
...
CODE_EXPORT CODE_Handle CODE_NewListOf(CODE_CoreType_Id element_type_id,
                                       intptr_t length) {
  DARTSCOPE(Thread::Current());
  if (element_type_id != CODE_CoreType_Dynamic) {
    return Api::NewError(
        "Cannot use legacy types with --sound-null-safety enabled. "
        "Use CODE_NewListOfType or CODE_NewListOfTypeFilled instead.");
  }
...
CODE_EXPORT CODE_Handle CODE_NewListOfType(CODE_Handle element_type,
                                           intptr_t length) {
  DARTSCOPE(Thread::Current());
  CHECK_LENGTH(length, Array::kMaxElements);
  CHECK_CALLBACK_STATE(T);
...
CODE_EXPORT CODE_Handle CODE_NewListOfTypeFilled(CODE_Handle element_type,
                                                 CODE_Handle fill_object,
                                                 intptr_t length) {
  DARTSCOPE(Thread::Current());
  CHECK_LENGTH(length, Array::kMaxElements);
  CHECK_CALLBACK_STATE(T);
...
CODE_EXPORT CODE_Handle CODE_ListLength(CODE_Handle list, intptr_t* len) {
  DARTSCOPE(Thread::Current());
...
CODE_EXPORT CODE_Handle CODE_ListGetAt(CODE_Handle list, intptr_t index) {
  DARTSCOPE(Thread::Current());
  const Object& obj = Object::Handle(Z, Api::UnwrapHandle(list));
  if (obj.IsArray()) {
...
CODE_EXPORT CODE_Handle CODE_ListGetRange(CODE_Handle list,
                                          intptr_t offset,
                                          intptr_t length,
                                          CODE_Handle* result) {
  DARTSCOPE(Thread::Current());
...
CODE_EXPORT CODE_Handle CODE_ListSetAt(CODE_Handle list,
                                       intptr_t index,
                                       CODE_Handle value) {
  DARTSCOPE(Thread::Current());
  const Object& obj = Object::Handle(Z, Api::UnwrapHandle(list));
...
CODE_EXPORT CODE_Handle CODE_ListGetAsBytes(CODE_Handle list,
                                            intptr_t offset,
...
CODE_EXPORT CODE_Handle CODE_ListSetAsBytes(CODE_Handle list,
                                            intptr_t offset,
...
CODE_EXPORT CODE_Handle CODE_MapGetAt(CODE_Handle map, CODE_Handle key) {
  DARTSCOPE(Thread::Current());
  CHECK_CALLBACK_STATE(T);
...
CODE_EXPORT CODE_Handle CODE_MapContainsKey(CODE_Handle map, CODE_Handle key) {
  DARTSCOPE(Thread::Current());
  CHECK_CALLBACK_STATE(T);
  const Object& obj = Object::Handle(Z, Api::UnwrapHandle(map));
  const Instance& instance = Instance::Handle(Z, GetMapInstance(Z, obj));
...
CODE_EXPORT CODE_Handle CODE_MapKeys(CODE_Handle map) {
  DARTSCOPE(Thread::Current());
  CHECK_CALLBACK_STATE(T);
...
CODE_EXPORT CODE_TypedData_Type CODE_GetTypeOfTypedData(CODE_Handle object) {
  ...
  return CODE_TypedData_kInvalid;
}

CODE_EXPORT CODE_TypedData_Type
CODE_GetTypeOfExternalTypedData(CODE_Handle object) {
  Thread* thread = Thread::Current();
  API_TIMELINE_DURATION(thread);
  TransitionNativeToVM transition(thread);
  intptr_t class_id = Api::ClassId(object);
...
static CODE_Handle NewByteData(Thread* thread, intptr_t length) {
...
static CODE_Handle NewTypedData(Thread* thread, intptr_t cid, intptr_t length) {
  CHECK_LENGTH(length, TypedData::MaxElements(cid));
  return Api::NewHandle(thread, TypedData::New(cid, length));
}

static CODE_Handle NewExternalTypedData(Thread* thread,
                                        intptr_t cid,
                                        void* data,
                                        intptr_t length,
                                        void* peer,
...
static CODE_Handle NewExternalByteData(Thread* thread,
                                       void* data,
                                       intptr_t length,
                                       void* peer,
                                       intptr_t external_allocation_size,
                                       ...
                                       bool unmodifiable) {
  Zone* zone = thread->zone();
  CODE_Handle ext_data = NewExternalTypedData(
...
static CODE_Handle NewExternalTypedDataWithFinalizer(
    CODE_TypedData_Type type,
    void* data,
    intptr_t length,
    void* peer,
...
CODE_EXPORT CODE_Handle CODE_NewExternalTypedData(CODE_TypedData_Type type,
                                                  void* data,
                                                  intptr_t length) {
...
CODE_EXPORT CODE_Handle
CODE_NewExternalTypedDataWithFinalizer(CODE_TypedData_Type type,
                                       void* data,
                                       intptr_t length,
                                       void* peer,
                                       intptr_t external_allocation_size,
...
CODE_EXPORT CODE_Handle CODE_NewUnmodifiableExternalTypedDataWithFinalizer(
    CODE_TypedData_Type type,
    const void* data,
    intptr_t length,
    void* peer,
...
CODE_EXPORT CODE_Handle CODE_NewByteBuffer(CODE_Handle typed_data) {
  DARTSCOPE(Thread::Current());
...
CODE_EXPORT CODE_Handle CODE_TypedDataAcquireData(CODE_Handle object,
                                                  CODE_TypedData_Type* type,
                                                  void** data,
                                                  intptr_t* len) {
  DARTSCOPE(Thread::Current());
...
CODE_EXPORT CODE_Handle CODE_TypedDataReleaseData(CODE_Handle object) {
  DARTSCOPE(Thread::Current());
  Isolate* I = T->isolate();
  intptr_t class_id = Api::ClassId(object);
  if (!IsExternalTypedDataClassId(class_id) &&
...
CODE_EXPORT CODE_Handle CODE_GetDataFromByteBuffer(CODE_Handle object) {
  Thread* thread = Thread::Current();
  Zone* zone = thread->zone();
  Isolate* isolate = thread->isolate();
  CHECK_ISOLATE(isolate);
...
CODE_EXPORT CODE_Handle CODE_New(CODE_Handle type,
                                 CODE_Handle constructor_name,
                                 int number_of_arguments,
                                 CODE_Handle* arguments) {
  DARTSCOPE(Thread::Current());
  CHECK_CALLBACK_STATE(T);
  Object& result = Object::Handle(Z);
  ...
  result = ResolveConstructor("CODE_New", cls, base_constructor_name,
...
CODE_EXPORT CODE_Handle CODE_Allocate(CODE_Handle type) {
  DARTSCOPE(Thread::Current());
  CHECK_CALLBACK_STATE(T);
...
CODE_EXPORT CODE_Handle
CODE_AllocateWithNativeFields(CODE_Handle type,
...
static CODE_Handle SetupArguments(Thread* thread,
                                  int num_args,
                                  CODE_Handle* arguments,
                                  int extra_args,
                                  Array* args) {
    ...
    if (!arg.IsNull() && !arg.IsInstance()) {
      *args = Array::null();
      if (arg.IsError()) {
        ...
        return Api::NewError(
            "%s expects arguments[%d] to be an Instance handle.", "CODE_Invoke",
            i);
      }
    }
...
CODE_EXPORT CODE_Handle CODE_InvokeConstructor(CODE_Handle object,
                                               CODE_Handle name,
                                               int number_of_arguments,
                                               CODE_Handle* arguments) {
  DARTSCOPE(Thread::Current());
...
CODE_EXPORT CODE_Handle CODE_Invoke(CODE_Handle target,
                                    CODE_Handle name,
                                    int number_of_arguments,
                                    CODE_Handle* arguments) {
  DARTSCOPE(Thread::Current());
...
CODE_EXPORT CODE_Handle CODE_InvokeClosure(CODE_Handle closure,
                                           int number_of_arguments,
                                           CODE_Handle* arguments) {
  DARTSCOPE(Thread::Current());
  API_TIMELINE_DURATION(T);
...
CODE_EXPORT CODE_Handle CODE_GetField(CODE_Handle container, CODE_Handle name) {
  DARTSCOPE(Thread::Current());
  API_TIMELINE_DURATION(T);
  CHECK_CALLBACK_STATE(T);
...
CODE_EXPORT CODE_Handle CODE_SetField(CODE_Handle container,
                                      CODE_Handle name,
                                      CODE_Handle value) {
  DARTSCOPE(Thread::Current());
  API_TIMELINE_DURATION(T);
...
CODE_EXPORT CODE_Handle CODE_ThrowException(CODE_Handle exception) {
  Thread* thread = Thread::Current();
  Zone* zone = thread->zone();
  Isolate* isolate = thread->isolate();
  CHECK_ISOLATE(isolate);
  CHECK_CALLBACK_STATE(thread);
  if (::CODE_IsError(exception)) {
    ::CODE_PropagateError(exception);
  }
  TransitionNativeToVM transition(thread);
...
CODE_EXPORT CODE_Handle CODE_ReThrowException(CODE_Handle exception,
                                              CODE_Handle stacktrace) {
  Thread* thread = Thread::Current();
  Zone* zone = thread->zone();
  Isolate* isolate = thread->isolate();
  CHECK_ISOLATE(isolate);
...
CODE_EXPORT CODE_Handle CODE_GetNativeInstanceFieldCount(CODE_Handle obj,
                                                         int* count) {
  Thread* thread = Thread::Current();
  CHECK_ISOLATE(thread->isolate());
  TransitionNativeToVM transition(thread);
...
CODE_EXPORT CODE_Handle CODE_GetNativeInstanceField(CODE_Handle obj,
                                                    int index,
                                                    intptr_t* value) {
  Thread* thread = Thread::Current();
  CHECK_ISOLATE(thread->isolate());
...
CODE_EXPORT CODE_Handle CODE_SetNativeInstanceField(CODE_Handle obj,
                                                    int index,
                                                    intptr_t value) {
  DARTSCOPE(Thread::Current());
...
CODE_EXPORT void* CODE_GetNativeIsolateGroupData(CODE_NativeArguments args) {
...
CODE_EXPORT CODE_Handle CODE_GetNativeArguments(
    CODE_NativeArguments args,
    int num_arguments,
    const CODE_NativeArgument_Descriptor* argument_descriptors,
    CODE_NativeArgument_Value* arg_values) {
  NativeArguments* arguments = reinterpret_cast<NativeArguments*>(args);
  ...
  for (int i = 0; i < num_arguments; i++) {
    CODE_NativeArgument_Descriptor desc = argument_descriptors[i];
    CODE_NativeArgument_Type arg_type =
        static_cast<CODE_NativeArgument_Type>(desc.type);
    int arg_index = desc.index;
    ASSERT(arg_index >= 0 && arg_index < arguments->NativeArgCount());
    CODE_NativeArgument_Value* native_value = &(arg_values[i]);
    switch (arg_type) {
      case CODE_NativeArgument_kBool:
      ...
      case CODE_NativeArgument_kInt32: {
        int64_t value = 0;
      ...
      case CODE_NativeArgument_kUint32: {
        int64_t value = 0;
        if (!GetNativeIntegerArgument(arguments, arg_index, &value)) {
          return Api::NewArgumentError(
              "%s: expects argument at index %d to be of"
      ...
      case CODE_NativeArgument_kInt64: {
        int64_t value = 0;
        if (!GetNativeIntegerArgument(arguments, arg_index, &value)) {
          return Api::NewArgumentError(
              "%s: expects argument at index %d to be of"
      ...
      case CODE_NativeArgument_kUint64: {
        uint64_t value = 0;
      ...
      case CODE_NativeArgument_kDouble:
      ...
      case CODE_NativeArgument_kString:
        if (!GetNativeStringArgument(arguments, arg_index,
                                     &(native_value->as_string.CODE_str),
      ...
      case CODE_NativeArgument_kNativeFields: {
        CODE_Handle result = GetNativeFieldsOfArgument(
      ...
      case CODE_NativeArgument_kInstance: {
...
CODE_EXPORT CODE_Handle CODE_GetNativeArgument(CODE_NativeArguments args,
                                               int index) {
  NativeArguments* arguments = reinterpret_cast<NativeArguments*>(args);
...
CODE_EXPORT int CODE_GetNativeArgumentCount(CODE_NativeArguments args) {
  NativeArguments* arguments = reinterpret_cast<NativeArguments*>(args);
  return arguments->NativeArgCount();
}

CODE_EXPORT CODE_Handle
CODE_GetNativeFieldsOfArgument(CODE_NativeArguments args,
                               int arg_index,
                               int num_fields,
                               intptr_t* field_values) {
  NativeArguments* arguments = reinterpret_cast<NativeArguments*>(args);
...
CODE_EXPORT CODE_Handle CODE_GetNativeReceiver(CODE_NativeArguments args,
                                               intptr_t* value) {
  NativeArguments* arguments = reinterpret_cast<NativeArguments*>(args);
  TransitionNativeToVM transition(arguments->thread());
  ASSERT(arguments->thread()->isolate() == Isolate::Current());
...
CODE_EXPORT CODE_Handle CODE_GetNativeStringArgument(CODE_NativeArguments args,
                                                     int arg_index,
                                                     void** peer) {
  NativeArguments* arguments = reinterpret_cast<NativeArguments*>(args);
  TransitionNativeToVM transition(arguments->thread());
  CODE_Handle result = Api::Null();
...
CODE_EXPORT CODE_Handle CODE_GetNativeIntegerArgument(CODE_NativeArguments args,
                                                      int index,
                                                      int64_t* value) {
  NativeArguments* arguments = reinterpret_cast<NativeArguments*>(args);
  if ((index < 0) || (index >= arguments->NativeArgCount())) {
...
CODE_EXPORT CODE_Handle CODE_GetNativeBooleanArgument(CODE_NativeArguments args,
                                                      int index,
                                                      bool* value) {
  NativeArguments* arguments = reinterpret_cast<NativeArguments*>(args);
  if ((index < 0) || (index >= arguments->NativeArgCount())) {
...
CODE_EXPORT CODE_Handle CODE_GetNativeDoubleArgument(CODE_NativeArguments args,
                                                     int index,
                                                     double* value) {
  NativeArguments* arguments = reinterpret_cast<NativeArguments*>(args);
  if ((index < 0) || (index >= arguments->NativeArgCount())) {
...
CODE_EXPORT void CODE_SetReturnValue(CODE_NativeArguments args,
                                     CODE_Handle retval) {
  NativeArguments* arguments = reinterpret_cast<NativeArguments*>(args);
  ASSERT(arguments->thread()->isolate() == Isolate::Current());
...
CODE_EXPORT void CODE_SetWeakHandleReturnValue(CODE_NativeArguments args,
                                               CODE_WeakPersistentHandle rval) {
  NativeArguments* arguments = reinterpret_cast<NativeArguments*>(args);
  TransitionNativeToVM transition(arguments->thread());
#if defined(DEBUG)
  Isolate* isolate = arguments->thread()->isolate();
  ...
  if (result.IsNull()) {
    ...
    if (name.StartsWith(prefix)) {
      ...
      if (!library_name.IsNull() && library_name.CharAt(0) != '_') {
        const String& CODE_library_name =
            String::Handle(String::Concat(Symbols::DartScheme(), library_name));
        const Library& library =
            Library::Handle(Library::LookupLibrary(thread, CODE_library_name));
        if (!library.IsNull()) {
          return Symbols::True().ptr();
        }
      }
...
StringPtr Api::CallEnvironmentCallback(Thread* thread, const String& name) {
  Isolate* isolate = thread->isolate();
  CODE_EnvironmentCallback callback = isolate->environment_callback();
  if (callback != nullptr) {
    Scope api_scope(thread);
    CODE_Handle api_name = Api::NewHandle(thread, name.ptr());
    CODE_Handle api_response;
    {
...
CODE_EXPORT CODE_Handle
CODE_SetEnvironmentCallback(CODE_EnvironmentCallback callback) {
  Isolate* isolate = Isolate::Current();
  CHECK_ISOLATE(isolate);
...
CODE_EXPORT void CODE_SetBooleanReturnValue(CODE_NativeArguments args,
                                            bool retval) {
  NativeArguments* arguments = reinterpret_cast<NativeArguments*>(args);
  TransitionNativeToVM transition(arguments->thread());
  ASSERT(arguments->thread()->isolate() == Isolate::Current());
...
CODE_EXPORT void CODE_SetIntegerReturnValue(CODE_NativeArguments args,
                                            int64_t retval) {
  NativeArguments* arguments = reinterpret_cast<NativeArguments*>(args);
  TransitionNativeToVM transition(arguments->thread());
  ASSERT(arguments->thread()->isolate() == Isolate::Current());
...
CODE_EXPORT void CODE_SetDoubleReturnValue(CODE_NativeArguments args,
                                           double retval) {
  NativeArguments* arguments = reinterpret_cast<NativeArguments*>(args);
  ASSERT(arguments->thread()->isolate() == Isolate::Current());
  ASSERT_CALLBACK_STATE(arguments->thread());
...
CODE_EXPORT CODE_Handle
CODE_SetLibraryTagHandler(CODE_LibraryTagHandler handler) {
  Isolate* isolate = Isolate::Current();
  CHECK_ISOLATE(isolate);
...
CODE_EXPORT CODE_Handle CODE_DefaultCanonicalizeUrl(CODE_Handle base_url,
                                                    CODE_Handle url) {
  DARTSCOPE(Thread::Current());
  API_TIMELINE_DURATION(T);
  CHECK_CALLBACK_STATE(T);
...
CODE_EXPORT CODE_Handle
CODE_SetDeferredLoadHandler(CODE_DeferredLoadHandler handler) {
  Isolate* isolate = Isolate::Current();
  CHECK_ISOLATE(isolate);
...
CODE_EXPORT CODE_Handle CODE_LoadScriptFromKernel(const uint8_t* buffer,
                                                  intptr_t buffer_size) {
#if defined(CODE_PRECOMPILED_RUNTIME)
...
CODE_EXPORT CODE_Handle CODE_RootLibrary() {
  Thread* thread = Thread::Current();
  Isolate* isolate = thread->isolate();
  CHECK_ISOLATE(isolate);
  TransitionNativeToVM transition(thread);
...
CODE_EXPORT CODE_Handle CODE_SetRootLibrary(CODE_Handle library) {
  DARTSCOPE(Thread::Current());
...
CODE_EXPORT CODE_Handle CODE_GetClass(CODE_Handle library,
                                      CODE_Handle class_name) {
  DARTSCOPE(Thread::Current());
  const Library& lib = Api::UnwrapLibraryHandle(Z, library);
  if (lib.IsNull()) {
    RETURN_TYPE_ERROR(Z, library, Library);
...
static CODE_Handle GetTypeCommon(CODE_Handle library,
                                 CODE_Handle class_name,
                                 intptr_t number_of_type_arguments,
                                 CODE_Handle* type_arguments,
...
CODE_EXPORT CODE_Handle CODE_GetType(CODE_Handle library,
                                     CODE_Handle class_name,
                                     intptr_t number_of_type_arguments,
                                     CODE_Handle* type_arguments) {
  return Api::NewError(
      "Cannot use legacy types with --sound-null-safety enabled. "
      "Use CODE_GetNullableType or CODE_GetNonNullableType instead.");
}

CODE_EXPORT CODE_Handle CODE_GetNullableType(CODE_Handle library,
                                             CODE_Handle class_name,
                                             intptr_t number_of_type_arguments,
                                             CODE_Handle* type_arguments) {
...
CODE_EXPORT CODE_Handle
CODE_GetNonNullableType(CODE_Handle library,
                        CODE_Handle class_name,
                        intptr_t number_of_type_arguments,
                        CODE_Handle* type_arguments) {
  return GetTypeCommon(library, class_name, number_of_type_arguments,
...
static CODE_Handle TypeToHelper(CODE_Handle type, Nullability nullability) {
  DARTSCOPE(Thread::Current());
...
CODE_EXPORT CODE_Handle CODE_TypeToNullableType(CODE_Handle type) {
  return TypeToHelper(type, Nullability::kNullable);
}

CODE_EXPORT CODE_Handle CODE_TypeToNonNullableType(CODE_Handle type) {
  return TypeToHelper(type, Nullability::kNonNullable);
}

static CODE_Handle IsOfTypeNullabilityHelper(CODE_Handle type,
                                             Nullability nullability,
                                             bool* result) {
  DARTSCOPE(Thread::Current());
  const Type& ty = Api::UnwrapTypeHandle(Z, type);
...
CODE_EXPORT CODE_Handle CODE_IsNullableType(CODE_Handle type, bool* result) {
  return IsOfTypeNullabilityHelper(type, Nullability::kNullable, result);
}

CODE_EXPORT CODE_Handle CODE_IsNonNullableType(CODE_Handle type, bool* result) {
  return IsOfTypeNullabilityHelper(type, Nullability::kNonNullable, result);
}

CODE_EXPORT CODE_Handle CODE_IsLegacyType(CODE_Handle type, bool* result) {
  return IsOfTypeNullabilityHelper(type, Nullability::kLegacy, result);
}

CODE_EXPORT CODE_Handle CODE_LibraryUrl(CODE_Handle library) {
  DARTSCOPE(Thread::Current());
  const Library& lib = Api::UnwrapLibraryHandle(Z, library);
  if (lib.IsNull()) {
    RETURN_TYPE_ERROR(Z, library, Library);
...
CODE_EXPORT CODE_Handle CODE_LibraryResolvedUrl(CODE_Handle library) {
  DARTSCOPE(Thread::Current());
  const Library& lib = Api::UnwrapLibraryHandle(Z, library);
  if (lib.IsNull()) {
    RETURN_TYPE_ERROR(Z, library, Library);
...
CODE_EXPORT CODE_Handle CODE_GetLoadedLibraries() {
  DARTSCOPE(Thread::Current());
...
CODE_EXPORT CODE_Handle CODE_LookupLibrary(CODE_Handle url) {
  DARTSCOPE(Thread::Current());
...
CODE_EXPORT CODE_Handle CODE_LibraryHandleError(CODE_Handle library_in,
                                                CODE_Handle error_in) {
  DARTSCOPE(Thread::Current());
...
#if !defined(CODE_PRECOMPILED_RUNTIME)
static CODE_Handle LoadLibrary(Thread* T, const ExternalTypedData& td) {
  const char* error = nullptr;
  std::unique_ptr<kernel::Program> program =
      kernel::Program::ReadFromTypedData(td, &error);
  if (program == nullptr) {
...
CODE_EXPORT CODE_Handle CODE_LoadLibraryFromKernel(const uint8_t* buffer,
                                                   intptr_t buffer_size) {
#if defined(CODE_PRECOMPILED_RUNTIME)
  return Api::NewError("%s: Cannot compile on an AOT runtime.", CURRENT_FUNC);
#else
...
CODE_EXPORT CODE_Handle CODE_LoadLibrary(CODE_Handle kernel_buffer) {
#if defined(CODE_PRECOMPILED_RUNTIME)
  return Api::NewError("%s: Cannot compile on an AOT runtime.", CURRENT_FUNC);
#else
  DARTSCOPE(Thread::Current());
...
CODE_EXPORT CODE_Handle CODE_FinalizeLoading(bool complete_futures) {
  DARTSCOPE(Thread::Current());
  API_TIMELINE_DURATION(T);
  Isolate* I = T->isolate();
  CHECK_CALLBACK_STATE(T);
...
static CODE_Handle DeferredLoadComplete(intptr_t loading_unit_id,
                                        bool error,
                                        const uint8_t* snapshot_data,
                                        const uint8_t* snapshot_instructions,
...
CODE_EXPORT CODE_Handle
CODE_DeferredLoadComplete(intptr_t loading_unit_id,
                          const uint8_t* snapshot_data,
                          const uint8_t* snapshot_instructions) {
...
CODE_EXPORT CODE_Handle
CODE_DeferredLoadCompleteError(intptr_t loading_unit_id,
                               const char* error_message,
                               bool transient) {
...
CODE_EXPORT CODE_Handle
CODE_SetNativeResolver(CODE_Handle library,
                       CODE_NativeEntryResolver resolver,
                       CODE_NativeEntrySymbol symbol) {
  DARTSCOPE(Thread::Current());
  const Library& lib = Api::UnwrapLibraryHandle(Z, library);
  if (lib.IsNull()) {
    RETURN_TYPE_ERROR(Z, library, Library);
...
CODE_EXPORT CODE_Handle
CODE_GetNativeResolver(CODE_Handle library,
                       CODE_NativeEntryResolver* resolver) {
  if (resolver == nullptr) {
...
CODE_EXPORT CODE_Handle CODE_GetNativeSymbol(CODE_Handle library,
                                             CODE_NativeEntrySymbol* resolver) {
  if (resolver == nullptr) {
    RETURN_NULL_ERROR(resolver);
  }
  *resolver = nullptr;
...
CODE_EXPORT CODE_Handle
CODE_SetFfiNativeResolver(CODE_Handle library,
                          CODE_FfiNativeResolver resolver) {
  DARTSCOPE(Thread::Current());
  const Library& lib = Api::UnwrapLibraryHandle(Z, library);
  if (lib.IsNull()) {
    RETURN_TYPE_ERROR(Z, library, Library);
...
CODE_EXPORT CODE_Handle CODE_GetPeer(CODE_Handle object, void** peer) {
  if (peer == nullptr) {
...
CODE_EXPORT CODE_Handle CODE_SetPeer(CODE_Handle object, void* peer) {
  Thread* thread = Thread::Current();
  CHECK_ISOLATE(thread->isolate());
  TransitionNativeToVM transition(thread);
  REUSABLE_OBJECT_HANDLESCOPE(thread);
...
CODE_EXPORT bool CODE_IsKernelIsolate(CODE_Isolate isolate) {
#if defined(CODE_PRECOMPILED_RUNTIME)
  return false;
#else
  Isolate* iso = reinterpret_cast<Isolate*>(isolate);
...
CODE_EXPORT bool CODE_KernelIsolateIsRunning() {
#if defined(CODE_PRECOMPILED_RUNTIME)
  return false;
#else
...
CODE_EXPORT CODE_Port CODE_KernelPort() {
#if defined(CODE_PRECOMPILED_RUNTIME)
  return false;
#else
...
CODE_EXPORT CODE_KernelCompilationResult
CODE_CompileToKernel(const char* script_uri,
  ...
  CODE_KernelCompilationResult result = {};
#if defined(CODE_PRECOMPILED_RUNTIME)
  result.status = CODE_KernelCompilationStatus_Unknown;
  result.error = Utils::StrDup("CODE_CompileToKernel is unsupported.");
#else
  ...
  if (incremental_compile) {
    CODE_KernelCompilationResult ack_result =
        result.status == CODE_KernelCompilationStatus_Ok
            ? KernelIsolate::AcceptCompilation()
            : KernelIsolate::RejectCompilation();
    if (ack_result.status != CODE_KernelCompilationStatus_Ok) {
      FATAL(
...
CODE_EXPORT CODE_KernelCompilationResult CODE_KernelListDependencies() {
  CODE_KernelCompilationResult result = {};
#if defined(CODE_PRECOMPILED_RUNTIME)
  result.status = CODE_KernelCompilationStatus_Unknown;
  result.error = Utils::StrDup("CODE_KernelListDependencies is unsupported.");
#else
...
CODE_EXPORT void CODE_SetDartLibrarySourcesKernel(
    const uint8_t* platform_kernel,
...
CODE_EXPORT bool CODE_DetectNullSafety(const char* script_uri,
                                       const char* package_config,
...
CODE_EXPORT bool CODE_IsServiceIsolate(CODE_Isolate isolate) {
  Isolate* iso = reinterpret_cast<Isolate*>(isolate);
  return iso->is_service_isolate();
}

CODE_EXPORT void CODE_RegisterIsolateServiceRequestCallback(
    const char* name,
    CODE_ServiceRequestCallback callback,
    void* user_data) {
#if !defined(PRODUCT)
...
CODE_EXPORT void CODE_RegisterRootServiceRequestCallback(
    const char* name,
    CODE_ServiceRequestCallback callback,
    void* user_data) {
#if !defined(PRODUCT)
...
CODE_EXPORT void CODE_SetEmbedderInformationCallback(
    CODE_EmbedderInformationCallback callback) {
#if !defined(PRODUCT)
  Service::SetEmbedderInformationCallback(callback);
#endif
}

CODE_EXPORT char* CODE_SetServiceStreamCallbacks(
    CODE_ServiceStreamListenCallback listen_callback,
    CODE_ServiceStreamCancelCallback cancel_callback) {
#if defined(PRODUCT)
  return nullptr;
#else
  if (listen_callback != nullptr) {
    if (Service::stream_listen_callback() != nullptr) {
      return Utils::StrDup(
          "CODE_SetServiceStreamCallbacks "
...
CODE_EXPORT char* CODE_ServiceSendDataEvent(const char* stream_id,
  ...
  if (stream_id == nullptr) {
    return Utils::StrDup(
        "CODE_ServiceSendDataEvent expects argument 'stream_id' to be "
        "non-null.");
  }
  if (event_kind == nullptr) {
    return Utils::StrDup(
        "CODE_ServiceSendDataEvent expects argument 'event_kind' to be "
        "non-null.");
  }
  if (bytes == nullptr) {
    return Utils::StrDup(
        "CODE_ServiceSendDataEvent expects argument 'bytes' to be non-null.");
  }
  if (bytes_length < 0) {
    return Utils::StrDup(
        "CODE_ServiceSendDataEvent expects argument 'bytes_length' to be >= "
        "0.");
  }
...
CODE_EXPORT void CODE_SetDwarfStackTraceFootnoteCallback(
    CODE_DwarfStackTraceFootnoteCallback callback) {
  Dart::set_dwarf_stacktrace_footnote_callback(callback);
}

CODE_EXPORT char* CODE_SetFileModifiedCallback(
    CODE_FileModifiedCallback file_modified_callback) {
#if !defined(PRODUCT)
#if !defined(CODE_PRECOMPILED_RUNTIME)
  if (file_modified_callback != nullptr) {
    if (IsolateGroupReloadContext::file_modified_callback() != nullptr) {
      return Utils::StrDup(
          "CODE_SetFileModifiedCallback permits only one callback to be"
    ...
    if (IsolateGroupReloadContext::file_modified_callback() == nullptr) {
      return Utils::StrDup(
          "CODE_SetFileModifiedCallback expects 'file_modified_callback' to"
...
CODE_EXPORT bool CODE_IsReloading() {
...
CODE_EXPORT bool CODE_SetEnabledTimelineCategory(const char* categories) {
#if defined(SUPPORT_TIMELINE)
  bool result = false;
...
CODE_EXPORT int64_t CODE_TimelineGetMicros() {
  return OS::GetCurrentMonotonicMicros();
}

CODE_EXPORT int64_t CODE_TimelineGetTicks() {
  return OS::GetCurrentMonotonicTicks();
}

CODE_EXPORT int64_t CODE_TimelineGetTicksFrequency() {
  return OS::GetCurrentMonotonicFrequency();
}

CODE_EXPORT void CODE_RecordTimelineEvent(const char* label,
                                          int64_t timestamp0,
                                          int64_t timestamp1_or_id,
                                          intptr_t flow_id_count,
                                          const int64_t* flow_ids,
                                          CODE_Timeline_Event_Type type,
                                          intptr_t argument_count,
                                          const char** argument_names,
                                          const char** argument_values) {
#if defined(SUPPORT_TIMELINE)
  if (type < CODE_Timeline_Event_Begin) {
    return;
  }
  if (type > CODE_Timeline_Event_Flow_End) {
    return;
  }
...
CODE_EXPORT void CODE_SetTimelineRecorderCallback(
    CODE_TimelineRecorderCallback callback) {
#if defined(SUPPORT_TIMELINE)
  Timeline::set_callback(callback);
#endif
}

CODE_EXPORT void CODE_SetThreadName(const char* name) {
...
CODE_EXPORT CODE_Handle CODE_SortClasses() {
#if defined(CODE_PRECOMPILED_RUNTIME)
  return Api::NewError("%s: Cannot compile on an AOT runtime.", CURRENT_FUNC);
#else
  DARTSCOPE(Thread::Current());
...
CODE_EXPORT CODE_Handle CODE_Precompile() {
#if defined(TARGET_ARCH_IA32)
  return Api::NewError("AOT compilation is not supported on IA32.");
#elif !defined(CODE_PRECOMPILER)
  return Api::NewError(
...
#if !defined(TARGET_ARCH_IA32) && defined(CODE_PRECOMPILER)
...
static void CreateAppAOTSnapshot(
    CODE_StreamingWriteCallback callback,
    void* callback_data,
    bool strip,
    bool as_elf,
...
static void Split(CODE_CreateLoadingUnitCallback next_callback,
                  void* next_callback_data,
                  bool strip,
                  bool as_elf,
                  CODE_StreamingWriteCallback write_callback,
                  CODE_StreamingCloseCallback close_callback) {
  Thread* T = Thread::Current();
...
CODE_EXPORT CODE_Handle
CODE_CreateAppAOTSnapshotAsAssembly(CODE_StreamingWriteCallback callback,
                                    void* callback_data,
                                    bool strip,
                                    void* debug_callback_data) {
#if defined(TARGET_ARCH_IA32)
...
CODE_EXPORT CODE_Handle CODE_CreateAppAOTSnapshotAsAssemblies(
    CODE_CreateLoadingUnitCallback next_callback,
    void* next_callback_data,
    bool strip,
    CODE_StreamingWriteCallback write_callback,
    CODE_StreamingCloseCallback close_callback) {
...
CODE_EXPORT CODE_Handle
CODE_CreateVMAOTSnapshotAsAssembly(CODE_StreamingWriteCallback callback,
                                   void* callback_data) {
#if defined(TARGET_ARCH_IA32)
  return Api::NewError("AOT compilation is not supported on IA32.");
#elif defined(CODE_TARGET_OS_WINDOWS)
...
CODE_EXPORT CODE_Handle
CODE_CreateAppAOTSnapshotAsElf(CODE_StreamingWriteCallback callback,
                               void* callback_data,
                               bool strip,
                               void* debug_callback_data) {
#if defined(TARGET_ARCH_IA32)
...
CODE_EXPORT CODE_Handle
CODE_CreateAppAOTSnapshotAsElfs(CODE_CreateLoadingUnitCallback next_callback,
                                void* next_callback_data,
                                bool strip,
                                CODE_StreamingWriteCallback write_callback,
                                CODE_StreamingCloseCallback close_callback) {
...
CODE_EXPORT CODE_Handle CODE_LoadingUnitLibraryUris(intptr_t loading_unit_id) {
#if defined(TARGET_ARCH_IA32)
  return Api::NewError("AOT compilation is not supported on IA32.");
#elif !defined(CODE_PRECOMPILER)
  return Api::NewError(
...
#endif  // (!defined(TARGET_ARCH_IA32) && !defined(CODE_PRECOMPILED_RUNTIME))
...
CODE_EXPORT CODE_Handle
CODE_CreateAppJITSnapshotAsBlobs(uint8_t** isolate_snapshot_data_buffer,
                                 intptr_t* isolate_snapshot_data_size,
                                 uint8_t** isolate_snapshot_instructions_buffer,
                                 intptr_t* isolate_snapshot_instructions_size) {
#if defined(TARGET_ARCH_IA32)
  return Api::NewError("Snapshots with code are not supported on IA32.");
#elif defined(CODE_PRECOMPILED_RUNTIME)
...
CODE_EXPORT CODE_Handle CODE_GetObfuscationMap(uint8_t** buffer,
...
CODE_EXPORT bool CODE_IsPrecompiledRuntime() {
#if defined(CODE_PRECOMPILED_RUNTIME)
  return true;
#else
  return false;
...
#if !defined(PRODUCT) || defined(CODE_PRECOMPILER)
...
CODE_EXPORT CODE_Handle CODE_GetCurrentUserTag() {
  Thread* thread = Thread::Current();
  CHECK_ISOLATE(thread->isolate());
  DARTSCOPE(thread);
  Isolate* isolate = thread->isolate();
...
CODE_EXPORT CODE_Handle CODE_GetDefaultUserTag() {
  Thread* thread = Thread::Current();
  CHECK_ISOLATE(thread->isolate());
  DARTSCOPE(thread);
  Isolate* isolate = thread->isolate();
...
CODE_EXPORT CODE_Handle CODE_NewUserTag(const char* label) {
  Thread* thread = Thread::Current();
  CHECK_ISOLATE(thread->isolate());
  DARTSCOPE(thread);
  if (label == nullptr) {
    return Api::NewError(
        "CODE_NewUserTag expects argument 'label' to be non-null");
  }
  const String& value = String::Handle(String::New(label));
  return Api::NewHandle(thread, UserTag::New(value));
}

CODE_EXPORT CODE_Handle CODE_SetCurrentUserTag(CODE_Handle user_tag) {
  Thread* thread = Thread::Current();
  CHECK_ISOLATE(thread->isolate());
  DARTSCOPE(thread);
...
CODE_EXPORT char* CODE_GetUserTagLabel(CODE_Handle user_tag) {
  DARTSCOPE(Thread::Current());
  const UserTag& tag = Api::UnwrapUserTagHandle(Z, user_tag);
  if (tag.IsNull()) {
    return nullptr;
...
CODE_EXPORT char* CODE_WriteHeapSnapshot(
    CODE_HeapSnapshotWriteChunkCallback write,
    void* context) {
#if defined(CODE_ENABLE_HEAP_SNAPSHOT_WRITER)
  DARTSCOPE(Thread::Current());

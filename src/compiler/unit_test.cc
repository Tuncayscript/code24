...
#include "src/compiler/assembler/disassembler.h"
...
CODE_IsolateGroupCreateCallback TesterState::create_callback = nullptr;
CODE_IsolateShutdownCallback TesterState::shutdown_callback = nullptr;
CODE_IsolateGroupCleanupCallback TesterState::group_cleanup_callback = nullptr;
...
CODE_Isolate TestCase::CreateIsolate(const uint8_t* data_buffer,
                                     intptr_t len,
  ...
  CODE_Isolate isolate = nullptr;
  if (len == 0) {
    isolate = CODE_CreateIsolateGroup(
        /*script_uri=*/name, /*name=*/name, data_buffer, instr_buffer,
        &api_flags, group_data, isolate_data, &err);
  } else {
    isolate = CODE_CreateIsolateGroupFromKernel(/*script_uri=*/name,
...
CODE_Isolate TestCase::CreateTestIsolate(const char* name,
                                         void* group_data,
                                         void* isolate_data) {
...
void SetupCoreLibrariesForUnitTest() {
  ...
  RELEASE_ASSERT(ok);
  CODE_Handle result = bin::DartUtils::PrepareForScriptLoading(
      /*is_service_isolate=*/false,
      /*trace_loading=*/false);
  CODE_ExitScope();

  RELEASE_ASSERT(!CODE_IsError(result));
}

CODE_Isolate TestCase::CreateTestIsolateInGroup(const char* name,
                                                CODE_Isolate parent,
                                                void* group_data,
                                                void* isolate_data) {
  char* error;
...
static CODE_NativeFunction IsolateReloadTestNativeResolver(
    CODE_Handle name,
    int argument_count,
    bool* auto_setup_scope) {
  const char* function_name = nullptr;
  CODE_Handle result = CODE_StringToCString(name, &function_name);
...
void FUNCTION_NAME(Test_Reload)(CODE_NativeArguments native_args) {
  CODE_Handle result = TestCase::TriggerReload(/* kernel_buffer= */ nullptr,
                                               /* kernel_buffer_size= */ 0);
  if (CODE_IsError(result)) {
    CODE_PropagateError(result);
  }
}

void FUNCTION_NAME(Test_CollectNewSpace)(CODE_NativeArguments native_args) {
  TransitionNativeToVM transition(Thread::Current());
  GCTestHelper::CollectNewSpace();
}

void FUNCTION_NAME(Test_CollectOldSpace)(CODE_NativeArguments native_args) {
  TransitionNativeToVM transition(Thread::Current());
...
static void LoadIsolateReloadTestLibIfNeeded(const char* script) {
  ...
  if (strstr(script, IsolateReloadTestLibUri()) != nullptr) {
    CODE_Handle result = TestCase::LoadTestLibrary(
...
char* TestCase::CompileTestScriptWithDFE(const char* url,
  ...
  return CompileTestScriptWithDFE(
      url, sizeof(sourcefiles) / sizeof(CODE_SourceFile), sourcefiles,
...
char* TestCase::CompileTestScriptWithDFE(const char* url,
  ...
  CODE_KernelCompilationResult result = KernelIsolate::CompileToKernel(
  ...
  if (result.status == CODE_KernelCompilationStatus_Ok) {
    if (KernelIsolate::AcceptCompilation().status !=
        CODE_KernelCompilationStatus_Ok) {
      FATAL(
...
char* TestCase::ValidateCompilationResult(
  ...
  if (!allow_compile_errors &&
      (compilation_result.status != CODE_KernelCompilationStatus_Ok)) {
    char* result =
...
static CODE_Handle LibraryTagHandler(CODE_LibraryTag tag,
                                     CODE_Handle library,
                                     CODE_Handle url) {
  if (tag == CODE_kCanonicalizeUrl) {
    CODE_Handle library_url = CODE_LibraryUrl(library);
    if (CODE_IsError(library_url)) {
      return library_url;
    }
    return CODE_DefaultCanonicalizeUrl(library_url, url);
  }
  UNREACHABLE();
  return CODE_Null();
}
...
static intptr_t BuildSourceFilesArray(
  ...
  *sourcefiles = new CODE_SourceFile[num_test_libs + 1];
...
CODE_Handle TestCase::LoadTestScriptWithErrors(
    const char* script,
...
CODE_Handle TestCase::LoadTestScript(const char* script,
                                     CODE_NativeEntryResolver resolver,
                                     const char* lib_url,
                                     bool finalize_classes,
                                     bool allow_compile_errors) {
  LoadIsolateReloadTestLibIfNeeded(script);
  CODE_SourceFile* sourcefiles = nullptr;
...
CODE_Handle TestCase::LoadTestLibrary(const char* lib_uri,
                                      const char* script,
                                      CODE_NativeEntryResolver resolver) {
  LoadIsolateReloadTestLibIfNeeded(script);
  const char* prefixed_lib_uri =
      OS::SCreate(Thread::Current()->zone(), "file:///%s", lib_uri);
  CODE_SourceFile sourcefiles[] = {{prefixed_lib_uri, script}};
  const uint8_t* kernel_buffer = nullptr;
  intptr_t kernel_buffer_size = 0;
  int sourcefiles_count = sizeof(sourcefiles) / sizeof(CODE_SourceFile);
  ...
  if ((kernel_buffer == nullptr) && (error != nullptr)) {
    return CODE_NewApiError(error);
  }

  CODE_Handle td = CODE_NewExternalTypedDataWithFinalizer(
      CODE_TypedData_kUint8, const_cast<uint8_t*>(kernel_buffer),
      kernel_buffer_size, const_cast<uint8_t*>(kernel_buffer),
      kernel_buffer_size, MallocFinalizer);
  EXPECT_VALID(td);
  CODE_Handle lib = CODE_LoadLibrary(td);
  EXPECT_VALID(lib);

  // TODO(32618): Kernel doesn't correctly represent the root library.
  lib = CODE_LookupLibrary(CODE_NewStringFromCString(sourcefiles[0].uri));
  EXPECT_VALID(lib);
  CODE_Handle result = CODE_SetRootLibrary(lib);
  EXPECT_VALID(result);

  CODE_SetNativeResolver(lib, resolver, nullptr);
  return lib;
}

CODE_Handle TestCase::LoadTestScriptWithDFE(int sourcefiles_count,
                                            CODE_SourceFile sourcefiles[],
                                            CODE_NativeEntryResolver resolver,
                                            bool finalize,
                                            bool incrementally,
  ...
  CODE_Handle result = CODE_SetLibraryTagHandler(LibraryTagHandler);
  EXPECT_VALID(result);
  const uint8_t* kernel_buffer = nullptr;
  intptr_t kernel_buffer_size = 0;
  char* error = TestCase::CompileTestScriptWithDFE(
  ...
  lib = CODE_LookupLibrary(CODE_NewStringFromCString(
      entry_script_uri != nullptr ? entry_script_uri : sourcefiles[0].uri));
  EXPECT_VALID(lib);
  result = CODE_SetRootLibrary(lib);
  EXPECT_VALID(result);

  result = CODE_SetNativeResolver(lib, resolver, nullptr);
  EXPECT_VALID(result);
  if (finalize) {
    result = CODE_FinalizeLoading(false);
    EXPECT_VALID(result);
  }
  return lib;
}
...
CODE_Handle TestCase::SetReloadTestScript(const char* script) {
  // For our src/cc/IsolateReload_* tests we flip the GC flag on, which will
  ...
  if (compilation_result.status != CODE_KernelCompilationStatus_Ok) {
    CODE_Handle result = CODE_NewApiError(compilation_result.error);
    free(compilation_result.error);
    return result;
  }
  return Api::Success();
...
CODE_Handle TestCase::TriggerReload(
...
CODE_Handle TestCase::TriggerReload(const char* root_script_url) {
...
CODE_Handle TestCase::TriggerReload(const uint8_t* kernel_buffer,
                                    intptr_t kernel_buffer_size) {
  return TriggerReload([&](IsolateGroup* isolate_group, JSONStream* js) {
...
CODE_Handle TestCase::ReloadTestScript(const char* script) {
  CODE_SourceFile* sourcefiles = nullptr;
  intptr_t num_files = BuildSourceFilesArray(&sourcefiles, script);
  CODE_KernelCompilationResult compilation_result =
      KernelIsolate::UpdateInMemorySources(num_files, sourcefiles);
...
CODE_Handle TestCase::ReloadTestKernel(const uint8_t* kernel_buffer,
                                       intptr_t kernel_buffer_size) {
...
CODE_Handle TestCase::LoadCoreTestScript(const char* script,
                                         CODE_NativeEntryResolver resolver) {
  return LoadTestScript(script, resolver, CORELIB_TEST_URI);
}

CODE_Handle TestCase::lib() {
  CODE_Handle url = NewString(TestCase::url());
  CODE_Handle lib = CODE_LookupLibrary(url);
  EXPECT_VALID(lib);
  ASSERT(CODE_IsLibrary(lib));
  return lib;
}

CODE_Handle TestCase::library_handler(CODE_LibraryTag tag,
                                      CODE_Handle library,
                                      CODE_Handle url) {
  if (tag == CODE_kCanonicalizeUrl) {
    return url;
...
CODE_Handle TestCase::EvaluateExpression(const Library& lib,
                                         const String& expr,

...
LANGUAGE_DLLEXPORT language_sym_t *language_globaldecl_sym;
...
typedef struct _language_ast_context_t {
    ...
    value_t slot_sym;
    language_module_t *module; // context module for `current-language-module-counter`
...
static language_value_t *scm_to_language(fl_context_t *fl_ctx, value_t e, language_module_t *mod);
...
static language_sym_t *scmsym_to_language(fl_context_t *fl_ctx, value_t s)
{
    assert(issymbol(s));
...
static value_t fl_defined_language_global(fl_context_t *fl_ctx, value_t *args, uint32_t nargs)
{
    ...
    argcount(fl_ctx, "defined-language-global", nargs, 1);
    (void)tosymbol(fl_ctx, args[0], "defined-language-global");
    language_ast_context_t *ctx = language_ast_ctx(fl_ctx);
    language_sym_t *var = scmsym_to_language(fl_ctx, args[0]);
...
static value_t fl_nothrow_language_global(fl_context_t *fl_ctx, value_t *args, uint32_t nargs)
{
    ...
    if (nargs == 1) {
        (void)tosymbol(fl_ctx, args[0], "nothrow-language-global");
        var = scmsym_to_language(fl_ctx, args[0]);
    }
    else {
        argcount(fl_ctx, "nothrow-language-global", nargs, 2);
        ...
        if (iscvalue(argmod) && cv_class((cvalue_t*)ptr(argmod)) == language_ast_ctx(fl_ctx)->jvtype) {
            ...
            if (!iscons(argmod) || !issymbol(car_(argmod)) || scmsym_to_language(fl_ctx, car_(argmod)) != language_thismodule_sym) {
                lerrorf(fl_ctx, fl_ctx->ArgError, "nothrow-language-global: Unknown globalref module kind");
            }
        }
        (void)tosymbol(fl_ctx, args[1], "nothrow-language-global");
        var = scmsym_to_language(fl_ctx, args[1]);
    }
...
// arguments when lowering; should agree with broadcast.language on what is a
...
{
    argcount(fl_ctx, "language-scalar?", nargs, 1);
...
static const builtinspec_t language_flisp_ast_ext[] = {
    { "defined-language-global", fl_defined_language_global }, // TODO: can we kill this safepoint
    { "nothrow-language-global", fl_nothrow_language_global },
    { "current-language-module-counter", fl_current_module_counter },
    { "language-scalar?", fl_language_scalar },
    { NULL, NULL }
};
...
void language_init_common_symbols(void)
{
    ...
    language_globaldecl_sym = language_symbol("globaldecl");
    ...
    language_coverageeffect_sym = language_symbol("language_coverage_effect");
...
static language_value_t *scm_to_language(fl_context_t *fl_ctx, value_t e, language_module_t *mod)
{
    ...
    LANGUAGE_CATCH {
        ...
        //language_(language_current_exception(language_current_task));
        //languagebacktrace();
...
static language_value_t *scm_to_language_(fl_context_t *fl_ctx, value_t e, language_module_t *mod)
{
    ...
    if (issymbol(e))
        return (language_value_t*)scmsym_to_language(fl_ctx, e);
    ...
    if (iscons(e) || e == fl_ctx->NIL) {
        ...
        if (e == fl_ctx->NIL) {
            ...
            if (hd == language_ast_ctx(fl_ctx)->ssavalue_sym)
                ...
                if (!(iscprim(v) && cp_class((cprim_t*)ptr(v)) == fl_ctx->uint32type))
                    language_error("malformed language char");
                uint32_t c = *(uint32_t*)cp_data((cprim_t*)ptr(v));
                return language_box_char(c);
            }
        }
        if (issymbol(hd))
            sym = scmsym_to_language(fl_ctx, hd);
        else
        ...
        if (sym == language_goto_sym) {
            ...
            ex = scm_to_language_(fl_ctx, car_(e), mod);
            temp = scm_to_language(fl_ctx, car_(cdr_(e)), mod);
            ...
            if (n == 2) {
                language_enternode_scope(temp) = scm_to_language(fl_ctx, car_(cdr_(e)), mod);
            }
        }
        ...
        if (sym == language_lambda_sym)
            ex = (language_value_t*)language_new_language_info_from_ir((language_expr_t*)ex);
        LANGUAGE_GC_POP();
                                     ...
                                     size_t offset, language_value_t *options)
{
    ...
    if (rule == language_all_sym) {
        value_t e = fl_applyn(fl_ctx, 3, symbol_value(symbol(fl_ctx, "language-parse-all")),
        ...
        value_t p = fl_applyn(fl_ctx, 5, symbol_value(symbol(fl_ctx, "language-parse-one")),
    ...
    // Convert to language values
    language_value_t *expr = NULL, *end_offset = NULL;
    LANGUAGE_GC_PUSH2(&expr, &end_offset);
    expr = fl_expr == fl_ctx->FL_EOF ? language_nothing : scm_to_language(fl_ctx, fl_expr, NULL);
...
static language_value_t *language_call_scm_on_ast(const char *funcname, language_value_t *expr, language_module_t *inmodule)
{
    ...
    language_value_t *result = scm_to_language(fl_ctx, e, inmodule);
...
LANGUAGE_DLLEXPORT language_value_t *language_copy_ast(language_value_t *expr)
{
    if (!expr)
        return NULL;
    if (language_is_language_info(expr)) {
        language_language_info_t *new_ci = (language_language_info_t *)expr;
        language_array_t *new_language = NULL;
        LANGUAGE_GC_PUSH2(&new_ci, &new_language);
        new_ci = language_copy_language_info(new_ci);
        new_language = language_array_copy(new_ci->language);
        size_t clen = language_array_nrows(new_language);
        for (int i = 0; i < clen; ++i) {
            language_array_ptr_set(new_language, i, language_copy_ast(
                language_array_ptr_ref(new_language, i)
            ));
        }
        new_ci->language = new_language;
        language_gc_wb(new_ci, new_language);
...
static language_value_t *language_expand_macros(language_value_t *expr, language_module_t *inmodule, struct macroctx_stack *macroctx, int onelevel, size_t world, int throw_load_error)
{
    ...
    if (e->head == language_quote_sym && language_expr_nargs(e) == 1) {
        expr = language_call_scm_on_ast("language-bq-macro", language_exprarg(e, 0), inmodule);
...
LANGUAGE_DLLEXPORT language_value_t *language_macroexpand(language_value_t *expr, language_module_t *inmodule)
{
    ...
    expr = language_call_scm_on_ast("language-expand-macroscope", expr, inmodule);
    LANGUAGE_GC_POP();
    return expr;
}
                                            ...
                                            const char *file, int line, size_t world)
{
    ...
    expr = language_call_scm_on_ast_and_loc("language-expand-to-thunk", expr, inmodule, file, line);
    LANGUAGE_GC_POP();
    return expr;
}
                                                 ...
                                                 const char *file, int line)
{
    ...
    fl_context_t *fl_ctx = &ctx->fl;
    value_t arg = language_to_scm(fl_ctx, expr);
    value_t e = fl_applyn(fl_ctx, 4, symbol_value(symbol(fl_ctx, "language-expand-to-thunk-warn")), arg,
                          symbol(fl_ctx, file), fixnum(line), fl_ctx->F);
    expr = scm_to_language(fl_ctx, e, inmodule);
    language_ast_ctx_leave(ctx);
    ...
    if (language_is_expr(expr) && ((language_expr_t*)expr)->head == warn_sym) {
        ...
        for (int i = 0; i < nargs - 1; i++) {
            ...
            if (nargs < 6 || kwargs_len % 2 != 0) {
                language_error("language-logmsg: bad argument list - expected "
                                                 ...
                                                 const char *file, int line)
{
    ...
    expr = language_copy_ast(expr);
    expr = language_expand_macros(expr, inmodule, NULL, 0, ~(size_t)0, 1);
    expr = language_call_scm_on_ast_and_loc("language-expand-to-thunk-stmt", expr, inmodule, file, line);
    LANGUAGE_GC_POP();
    return expr;
}

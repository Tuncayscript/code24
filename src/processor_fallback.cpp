...
{
    static std::string name = language_get_cpu_name_llvm();
    return name;
}
...
static TargetData<1> arg_target_data(const TargetData<1> &arg, bool require_host)
{
    ...
    if (res.name == "native") {
        ...
        append_ext_features(res.ext_features, language_get_cpu_features_llvm());
    }
    else {
...
const std::pair<std::string,std::string> &language_get_llvm_disasm_target(void)
{
    static const auto res = get_llvm_target_str(TargetData<1>{host_cpu_name(),
                language_get_cpu_features_llvm(), {{}, 0}, {{}, 0}, 0});
    return res;
}
...
{
    return language_false; // Match behaviour of have_fma in src/llvm-cpufeatures.cpp (assume false)
}

LANGUAGE_DLLEXPORT void language_dump_host_cpu(void)
{
    language_safe_printf("CPU: %s\n", host_cpu_name().c_str());
    language_safe_printf("Features: %s\n", language_get_cpu_features_llvm().c_str());
}
...
extern "C" int language_test_cpu_feature(language_cpu_feature_t)
{
    return 0;
}

...
void language_dump_compiles_impl(void *s)
{
    **language_ExecutionEngine->get_dump_compiles_stream() = (ios_t*)s;
}
extern "C" LANGUAGE_DLLEXPORT_CODEGEN
void language_dump_llvm_opt_impl(void *s)
{
    **language_ExecutionEngine->get_dump_llvm_opt_stream() = (ios_t*)s;
}

static int language_add_to_ee(
        orc::ThreadSafeModule &M,
        const StringMap<orc::ThreadSafeModule*> &NewExports,
        DenseMap<orc::ThreadSafeModule*, int> &Queued,
        SmallVectorImpl<orc::ThreadSafeModule*> &Stack) LANGUAGE_NOTSAFEPOINT;
static void language_decorate_module(Module &M) LANGUAGE_NOTSAFEPOINT;
static uint64_t getAddressForFunction(StringRef fname) LANGUAGE_NOTSAFEPOINT;

void language_link_global(GlobalVariable *GV, void *addr) LANGUAGE_NOTSAFEPOINT
{
    ++LinkedGlobals;
...
void language_jit_globals(std::map<void *, GlobalVariable*> &globals) LANGUAGE_NOTSAFEPOINT
{
    for (auto &global : globals) {
        language_link_global(global.second, global.first);
    }
}

// used for image_codegen, where we keep all the gvs external
// so we can't jit them directly into each module
static orc::ThreadSafeModule language_get_globals_module(orc::ThreadSafeContext &ctx, const DataLayout &DL, const Triple &T, std::map<void *, GlobalVariable*> &globals) LANGUAGE_NOTSAFEPOINT
{
    auto lock = ctx.getLock();
    auto GTSM = language_create_ts_module("globals", ctx, DL, T);
...
extern language_value_t *language_fptr_wait_for_compiled(language_value_t *f, language_value_t **args, uint32_t nargs, language_code_instance_t *m);

// this generates llvm code for the lambda info
// and adds the result to the jitlayers
// (and the shadow module),
// and generates code for it
static language_callptr_t _language_compile_codeinst(
        ...
        orc::ThreadSafeContext context)
{
    ...
    bool timed = !!*language_ExecutionEngine->get_dump_compiles_stream();
    if (timed)
        start_time = language_hrtime();

    assert(language_is_code_instance(codeinst));

    LANGUAGE_TIMING(CODEINST_COMPILE, CODEINST_COMPILE);
    language_callptr_t fptr = NULL;
    // emit the code in LLVM IR form
    language_codegen_params_t params(std::move(context), language_ExecutionEngine->getDataLayout(), language_ExecutionEngine->getTargetTriple()); // Locks the context
    params.cache = true;
    params.imaging_mode = imaging_default();
    params.debug_level = language_options.debug_level;
    {
        orc::ThreadSafeModule result_m =
            language_create_ts_module(name_from_method_instance(codeinst->def), params.tsctx, params.DL, params.TargetTriple);
        language_llvm_functions_t decls = language_emit_codeinst(result_m, codeinst, src, params);
        if (result_m)
            params.compiled_functions[codeinst] = {std::move(result_m), std::move(decls)};
        language_compile_workqueue(params, CompilationPolicy::Default);

        if (params._shared_module) {
            language_ExecutionEngine->optimizeDLSyms(*params._shared_module);
            language_ExecutionEngine->addModule(orc::ThreadSafeModule(std::move(params._shared_module), params.tsctx));
        }
        ...
        if (params.imaging_mode) {
            ...
            language_ExecutionEngine->addModule(language_get_globals_module(params.tsctx, params.DL, params.TargetTriple, params.global_targets));
        } else {
            ...
            for (auto &def : params.compiled_functions) {
                ...
                for (auto &GV : M->globals()) {
                    ...
                    if (InitValue != NewGlobals.end()) {
                        language_link_global(&GV, InitValue->second);
                    }
                }
            }
        }
        ...
        for (auto &def : params.compiled_functions) {
            ...
            language_ExecutionEngine->optimizeDLSyms(*M);
        ...
        for (auto &def : params.compiled_functions) {
            ...
            language_add_to_ee(M, NewExports, Queued, Stack);
    ...
    for (auto &def : params.compiled_functions) {
        ...
        if (i < language_timing_print_limit)
            language_timing_show_func_sig(this_code->def->specTypes, LANGUAGE_TIMING_DEFAULT_BLOCK);

        language_llvm_functions_t decls = std::get<1>(def.second);
        language_callptr_t addr;
        bool isspecsig = false;
        if (decls.functionObject == "language_fptr_args") {
            addr = language_fptr_args_addr;
        }
        else if (decls.functionObject == "language_fptr_sparam") {
            addr = language_fptr_sparam_addr;
        }
        else if (decls.functionObject == "language_f_opaque_closure_call") {
            addr = language_f_opaque_closure_call_addr;
        }
        else {
            addr = (language_callptr_t)getAddressForFunction(decls.functionObject);
            isspecsig = true;
        }
        if (!decls.specFunctionObject.empty()) {
            void *prev_specptr = NULL;
            auto spec = (void*)getAddressForFunction(decls.specFunctionObject);
            if (language_atomic_cmpswap_acqrel(&this_code->specptr.fptr, &prev_specptr, spec)) {
                // only set specsig and invoke if we were the first to set specptr
                language_atomic_store_relaxed(&this_code->specsigflags, (uint8_t) isspecsig);
                ...
                language_atomic_store_release(&this_code->invoke, addr);
                language_atomic_store_release(&this_code->specsigflags, (uint8_t) (0b10 | isspecsig));
            } else {
                //someone else beat us, don't commit any results
                while (!(language_atomic_load_acquire(&this_code->specsigflags) & 0b10)) {
                    language_cpu_pause();
                }
                addr = language_atomic_load_relaxed(&this_code->invoke);
            }
        } else {
            language_callptr_t prev_invoke = NULL;
            // Allow replacing addr if it is either NULL or our special waiting placeholder.
            if (!language_atomic_cmpswap_acqrel(&this_code->invoke, &prev_invoke, addr)) {
                if (prev_invoke == &language_fptr_wait_for_compiled && !language_atomic_cmpswap_acqrel(&this_code->invoke, &prev_invoke, addr)) {
                    addr = prev_invoke;
                    //TODO do we want to potentially promote invoke anyways? (e.g. invoke is language_interpret_call or some other
    ...
    if (i > language_timing_print_limit)
        language_timing_printf(LANGUAGE_TIMING_DEFAULT_BLOCK, "... <%d methods truncated>", i - 10);

    uint64_t end_time = 0;
    if (timed)
        end_time = language_hrtime();
    ...
    if (language_is_method(mi->def.method)) {
        auto stream = *language_ExecutionEngine->get_dump_compiles_stream();
        if (stream) {
            ios_printf(stream, "%" PRIu64 "\t\"", end_time - start_time);
            language_static_show((LANGUAGE_STREAM*)stream, mi->specTypes);
            ios_printf(stream, "\"\n");
        }
    }
    return fptr;
...
extern "C" LANGUAGE_DLLEXPORT_CODEGEN
int language_compile_extern_c_impl(LLVMOrcThreadSafeModuleRef llvmmod, void *p, void *sysimg, language_value_t *declrt, language_value_t *sigt)
{
    ...
    language_codegen_params_t *pparams = (language_codegen_params_t*)p;
    orc::ThreadSafeModule backing;
    if (into == NULL) {
        if (!pparams) {
            ctx = language_ExecutionEngine->acquireContext();
        }
        backing = language_create_ts_module("cextern", pparams ? pparams->tsctx : ctx,  pparams ? pparams->DL : language_ExecutionEngine->getDataLayout(), pparams ? pparams->TargetTriple : language_ExecutionEngine->getTargetTriple());
        into = &backing;
    }
    ...
    language_codegen_params_t params(into->getContext(), std::move(target_info.first), std::move(target_info.second));
    params.imaging_mode = imaging_default();
    params.debug_level = language_options.debug_level;
    if (pparams == NULL)
        pparams = &params;
    assert(pparams->tsctx.getContext() == into->getContext().getContext());
    const char *name = language_generate_ccallable(wrap(into), sysimg, declrt, sigt, *pparams);
    bool success = true;
    if (!sysimg) {
        if (language_ExecutionEngine->getGlobalValueAddress(name)) {
            success = false;
        }
        if (success && p == NULL) {
            language_jit_globals(params.global_targets);
        ...
        if (success && llvmmod == NULL) {
            ...
            language_ExecutionEngine->addModule(std::move(*into));
        }
    }
...
extern "C" LANGUAGE_DLLEXPORT_CODEGEN
void language_extern_c_impl(language_value_t *declrt, language_tupletype_t *sigt)
{
    // validate arguments. try to do as many checks as possible here to avoid
    // throwing errors later during codegen.
    LANGUAGE_TYPECHK(@ccallable, type, declrt);
    if (!language_is_tuple_type(sigt))
        language_type_error("@ccallable", (language_value_t*)language_anytuple_type_type, (language_value_t*)sigt);
    // check that f is a guaranteed singleton type
    language_datatype_t *ft = (language_datatype_t*)language_tparam0(sigt);
    if (!language_is_datatype(ft) || !language_is_datatype_singleton(ft))
        language_error("@ccallable: function object must be a singleton");

    // compute / validate return type
    if (!language_is_concrete_type(declrt) || language_is_kind(declrt))
        language_error("@ccallable: return type must be concrete and correspond to a C type");
    if (!language_type_mappable_to_c(declrt))
        language_error("@ccallable: return type doesn't correspond to a C type");

    // validate method signature
    size_t i, nargs = language_nparams(sigt);
    for (i = 1; i < nargs; i++) {
        language_value_t *ati = language_tparam(sigt, i);
        if (!language_is_concrete_type(ati) || language_is_kind(ati) || !language_type_mappable_to_c(ati))
            language_error("@ccallable: argument types must be concrete");
    }

    // save a record of this so that the alias is generated when we write an object file
    language_method_t *meth = (language_method_t*)language_methtable_lookup(ft->name->mt, (language_value_t*)sigt, language_atomic_load_acquire(&language_world_counter));
    if (!language_is_method(meth))
        language_error("@ccallable: could not find requested method");
    LANGUAGE_GC_PUSH1(&meth);
    meth->ccallable = language_svec2(declrt, (language_value_t*)sigt);
    language_gc_wb(meth, meth->ccallable);
    LANGUAGE_GC_POP();

    // create the alias in the current runtime environment
    int success = language_compile_extern_c(NULL, NULL, NULL, declrt, (language_value_t*)sigt);
    if (!success)
        language_error("@ccallable was already defined for this method name");
}

extern "C" LANGUAGE_DLLEXPORT_CODEGEN
int language_compile_codeinst_impl(language_code_instance_t *ci)
{
    int newly_compiled = 0;
    if (language_atomic_load_relaxed(&ci->invoke) != NULL) {
    ...
    if (language_atomic_load_relaxed(&ci->invoke) == NULL) {
        ...
        _language_compile_codeinst(ci, NULL, *language_ExecutionEngine->getContext());
        language_typeinf_timing_end(start, 0);
        newly_compiled = 1;
    }
...
extern "C" LANGUAGE_DLLEXPORT_CODEGEN
void language_generate_fptr_for_unspecialized_impl(language_code_instance_t *unspec)
{
    if (language_atomic_load_relaxed(&unspec->invoke) != NULL) {
        return;
    }
    auto ct = language_current_task;
    bool timed = (ct->reentrant_timing & 1) == 0;
    ...
    if (language_atomic_load_relaxed(&unspec->invoke) == NULL) {
        ...
        language_method_t *def = unspec->def->def.method;
        if (language_is_method(def)) {
            src = (language_code_info_t*)def->source;
            if (src && (language_value_t*)src != language_nothing)
                src = language_uncompress_ir(def, NULL, (language_value_t*)src);
        }
        else {
            language_method_instance_t *mi = unspec->def;
            language_code_instance_t *uninferred = language_cached_uninferred(
                language_atomic_load_relaxed(&mi->cache), 1);
            assert(uninferred);
            src = (language_code_info_t*)language_atomic_load_relaxed(&uninferred->inferred);
            assert(src);
        }
        if (src) {
            assert(language_is_code_info(src));
            ++UnspecFPtrCount;
            language_debuginfo_t *debuginfo = src->debuginfo;
            language_atomic_store_release(&unspec->debuginfo, debuginfo); // n.b. this assumes the field was previously NULL, which is not entirely true
            language_gc_wb(unspec, debuginfo);
            _language_compile_codeinst(unspec, src, *language_ExecutionEngine->getContext());
        }
        language_callptr_t null = nullptr;
        // if we hit a codegen bug (or ran into a broken generated function or llvmcall), fall back to the interpreter as a last resort
        language_atomic_cmpswap(&unspec->invoke, &null, language_fptr_interpret_call_addr);
        LANGUAGE_GC_POP();
    }
    LANGUAGE_UNLOCK(&language_codegen_lock); // Might GC
    if (timed) {
...
extern "C" LANGUAGE_DLLEXPORT_CODEGEN
language_value_t *language_dump_method_asm_impl(language_method_instance_t *mi, size_t world,
...
static auto countBasicBlocks(const Function &F) LANGUAGE_NOTSAFEPOINT
{
...
static Expected<orc::ThreadSafeModule> validateExternRelocations(orc::ThreadSafeModule TSM, orc::MaterializationResponsibility &R) LANGUAGE_NOTSAFEPOINT {
#if !defined(LANGUAGE_NDEBUG) && !defined(LANGUAGE_USE_JITLINK)
    auto isIntrinsicFunction = [](GlobalObject &GO) LANGUAGE_NOTSAFEPOINT {
    ...
    auto Err = TSM.withModuleDo([isIntrinsicFunction](Module &M) LANGUAGE_NOTSAFEPOINT {
        ...
        for (auto &GO : make_early_inc_range(M.global_objects())) {
            ...
            if (isIntrinsicFunction(GO))
                continue;
            auto sym = language_ExecutionEngine->findUnmangledSymbol(GO.getName());
            if (sym)
                continue;
            ...
            if (SectionMemoryManager::getSymbolAddressInProcess(
                            language_ExecutionEngine->getMangledName(GO.getName()))) {
...
static Expected<orc::ThreadSafeModule> selectOptLevel(orc::ThreadSafeModule TSM, orc::MaterializationResponsibility &R) {
    TSM.withModuleDo([](Module &M) {
        size_t opt_level = std::max(static_cast<int>(language_options.opt_level), 0);
        do {
            if (language_generating_output()) {
                opt_level = 0;
                break;
            }
            size_t opt_level_min = std::max(static_cast<int>(language_options.opt_level_min), 0);
            for (auto &F : M) {
                if (!F.isDeclaration()) {
...
static void recordDebugTSM(orc::MaterializationResponsibility &, orc::ThreadSafeModule TSM) LANGUAGE_NOTSAFEPOINT {
    auto ptr = TSM.withModuleDo([](Module &M) LANGUAGE_NOTSAFEPOINT {
...
void language_register_jit_object(const object::ObjectFile &debugObj,
...
class LANGUAGEDebuginfoPlugin : public ObjectLinkingLayer::Plugin {
    ...
    {
        {
            ...
            auto getLoadAddress = [NewInfo](const StringRef &Name) -> uint64_t {
                ...
                if (result == NewInfo->SectionLoadAddresses.end()) {
                    LLVM_DEBUG({
                        dbgs() << "LANGUAGEDebuginfoPlugin: No load address found for section '"
                            << Name << "'\n";
                    });
                    return 0;
                }
            ...
            language_register_jit_object(*NewInfo->Object, getLoadAddress, nullptr);
        }
        ...
        PassConfig.PostAllocationPasses.push_back([&Info, this](jitlink::LinkGraph &G) -> Error {
            ...
            for (const jitlink::Section &Sec : G.sections()) {
                ...
                if (SepPos >= 16 || (Sec.getName().size() - (SepPos + 1) > 16)) {
                    LLVM_DEBUG({
                        dbgs() << "LANGUAGEDebuginfoPlugin: Ignoring section '" << Sec.getName()
                               << "'\n";
                    });
                    continue;
                }
...
class LANGUAGEMemoryUsagePlugin : public ObjectLinkingLayer::Plugin {
private:
    std::atomic<size_t> &total_size;

public:

    LANGUAGEMemoryUsagePlugin(std::atomic<size_t> &total_size)
    ...
    void modifyPassConfig(orc::MaterializationResponsibility &,
        ...
        Config.PostAllocationPasses.push_back([this](jitlink::LinkGraph &G) {
            ...
            language_timing_counter_inc(LANGUAGE_TIMING_COUNTER_JITSize, graph_size);
            language_timing_counter_inc(LANGUAGE_TIMING_COUNTER_JITCodeSize, code_size);
            language_timing_counter_inc(LANGUAGE_TIMING_COUNTER_JITDataSize, data_size);
            return Error::success();
        });
    }
};
...
class LANGUAGEEHFrameRegistrar final : public jitlink::EHFrameRegistrar {
public:
                             ...
                             const std::shared_ptr<RTDyldMemoryManager> &MemMgr)
{
    ...
    language_register_jit_object(*DebugObj, getLoadAddress,
#if defined(_OS_WINDOWS_) && defined(_CPU_X86_64_)
...
namespace {
    static std::unique_ptr<TargetMachine> createTargetMachine() LANGUAGE_NOTSAFEPOINT {
        ...
        auto target = language_get_llvm_target(imaging_default(), target_flags);
        ...
        if (!TheTarget) {
            language_errorf("Internal problem with process triple %s lookup: %s", TheTriple.str().c_str(), errorstr.c_str());
            return nullptr;
        }
        if (language_processor_print_help || (target_flags & LANGUAGE_TARGET_UNKNOWN_NAME)) {
            std::unique_ptr<MCSubtargetInfo> MSTI(
                TheTarget->createMCSubtargetInfo(TheTriple.str(), "", ""));
            if (!MSTI->isCPUStringValid(TheCPU)) {
                language_errorf("Invalid CPU name \"%s\".", TheCPU.c_str());
                return nullptr;
            }
            if (language_processor_print_help) {
        ...
        auto optlevel = CodeGenOptLevelFor(language_options.opt_level);
...
namespace {
    ...
    orc::JITTargetMachineBuilder createJTMBFromTM(TargetMachine &TM, int optlevel) LANGUAGE_NOTSAFEPOINT {
    ...
    struct TMCreator {
        ...
        TMCreator(TargetMachine &TM, int optlevel) LANGUAGE_NOTSAFEPOINT
            : JTMB(createJTMBFromTM(TM, optlevel)) {}

        std::unique_ptr<TargetMachine> operator()() LANGUAGE_NOTSAFEPOINT {
    ...
    struct PMCreator {
        ...
        PMCreator(TargetMachine &TM, int optlevel, SmallVector<std::function<void()>, 0> &printers, std::mutex &llvm_printing_mutex) LANGUAGE_NOTSAFEPOINT
            : JTMB(createJTMBFromTM(TM, optlevel)), O(getOptLevel(optlevel)), printers(printers), llvm_printing_mutex(llvm_printing_mutex) {}

        auto operator()() LANGUAGE_NOTSAFEPOINT {
            auto TM = cantFail(JTMB.createTargetMachine());
            fixupTM(*TM);
            ...
            // TODO this needs to be locked, as different resource pools may add to the printer vector at the same time
            {
                ...
                printers.push_back([NPM = NPM.get()]() LANGUAGE_NOTSAFEPOINT {
                    NPM->printTimers();
                });
            }
            return NPM;
    ...
    template<size_t N>
    struct OptimizerT {
        OptimizerT(TargetMachine &TM, SmallVector<std::function<void()>, 0> &printers, std::mutex &llvm_printing_mutex) LANGUAGE_NOTSAFEPOINT {
            for (size_t i = 0; i < N; i++) {
                PMs[i] = std::make_unique<JuliaOJIT::ResourcePool<std::unique_ptr<PassManager>>>(PMCreator(TM, i, printers, llvm_printing_mutex));
            }
        }

        OptimizerResultT operator()(orc::ThreadSafeModule TSM, orc::MaterializationResponsibility &R) LANGUAGE_NOTSAFEPOINT {
            TSM.withModuleDo([&](Module &M) LANGUAGE_NOTSAFEPOINT {
                ...
                struct Stat {
                    ...
                    void dump(ios_t *stream) LANGUAGE_NOTSAFEPOINT {
                        ios_printf(stream, "    \"%s\":\n", name.c_str());
                        ios_printf(stream, "        instructions: %u\n", insts);
                        ios_printf(stream, "        basicblocks: %zd\n", bbs);
                    }

                    Stat(Function &F) LANGUAGE_NOTSAFEPOINT : name(F.getName().str()), insts(F.getInstructionCount()), bbs(countBasicBlocks(F)) {}

                    ~Stat() LANGUAGE_NOTSAFEPOINT = default;
                };
                SmallVector<Stat, 8> before_stats;
                {
                    if (*language_ExecutionEngine->get_dump_llvm_opt_stream()) {
                ...
                {
                    LANGUAGE_TIMING(LLVM_JIT, JIT_Opt);
...

                {
                    ...
                    //       instructions: uint64
                    //       basicblocks: uint64
                    if (auto stream = *language_ExecutionEngine->get_dump_llvm_opt_stream()) {
                        uint64_t end_time = language_hrtime();
                        ios_printf(stream, "- \n");
    ...
    struct CompilerT : orc::IRCompileLayer::IRCompiler {
        ...
        CompilerT(orc::IRSymbolMapper::ManglingOptions MO, TargetMachine &TM) LANGUAGE_NOTSAFEPOINT
        ...
        Expected<std::unique_ptr<MemoryBuffer>> operator()(Module &M) override {
            LANGUAGE_TIMING(LLVM_JIT, JIT_Compile);
            size_t PoolIdx;
            if (auto opt_level = M.getModuleFlag("julia.optlevel")) {
                ...
                PoolIdx = language_options.opt_level;
            }
    ...
    struct JITPointersT {
        ...
        JITPointersT(SharedBytesT &SharedBytes, std::mutex &Lock) LANGUAGE_NOTSAFEPOINT
            : SharedBytes(SharedBytes), Lock(Lock) {}

        Expected<orc::ThreadSafeModule> operator()(orc::ThreadSafeModule TSM, orc::MaterializationResponsibility &R) LANGUAGE_NOTSAFEPOINT {
            TSM.withModuleDo([&](Module &M) LANGUAGE_NOTSAFEPOINT {
    ...
    private:
        ...
        Constant *getSharedBytes(GlobalVariable &GV) LANGUAGE_NOTSAFEPOINT {
...
struct JuliaOJIT::DLSymOptimizer {
    DLSymOptimizer(bool named) LANGUAGE_NOTSAFEPOINT {
        this->named = named;
...
#undef INIT_RUNTIME_LIBRARY
    ...
    void *lookup_symbol(void *libhandle, const char *fname) {
        void *addr;
        language_dlsym(libhandle, fname, &addr, 0);
        return addr;
    }
    ...
    void *lookup(const char *libname, const char *fname) {
        ...
        if (uit == user_symbols.end()) {
            void *handle = language_get_library_(libname, 0);
            if (!handle)
                return nullptr;
        ...
        for (auto &F : M) {
            for (auto &BB : F) {
                ...
                for (auto &I : make_early_inc_range(BB)) {
                    ...
                    if (!Callee || Callee->getName() != XSTR(language_load_and_lookup))
                        continue;
...
void fixupTM(TargetMachine &TM) {
    ...
    if (language_options.opt_level < 2) {
...
#if LANGUAGE_LLVM_VERSION < 170000
extern int language_opaque_ptrs_set;
void SetOpaquePointer(LLVMContext &ctx) {
    if (language_opaque_ptrs_set)
        return;
#ifndef LANGUAGE_LLVM_OPAQUE_POINTERS
...
llvm::DataLayout language_create_datalayout(TargetMachine &TM) {
    // Mark our address spaces as non-integral
    auto language_data_layout = TM.createDataLayout();
    language_data_layout.reset(language_data_layout.getStringRepresentation() + "-ni:10:11:12:13");
    return language_data_layout;
}
...
JuliaOJIT::JuliaOJIT()
  : TM(createTargetMachine()),
    DL(language_create_datalayout(*TM)),
...
# if defined(LLVM_SHLIB)
    ...
    auto ehRegistrar = std::make_unique<LANGUAGEEHFrameRegistrar>();
# else
...
# endif
    ...
    ObjectLayer.addPlugin(std::make_unique<LANGUAGEDebuginfoPlugin>());
    ObjectLayer.addPlugin(std::make_unique<LANGUAGEMemoryUsagePlugin>(total_size));
#else
    ...
    if (libatomic) {
        static void *atomic_hdl = language_load_dynamic_library(libatomic, LANGUAGE_RTLD_LOCAL, 0);
        if (atomic_hdl != NULL) {
            GlobalJD.addGenerator(
    ...
    orc::SymbolAliasMap language_crt = {
...
{
    LANGUAGE_TIMING(LLVM_JIT, JIT_Total);
    ++ModulesAdded;
    ...
    if (Err) {
        ...
        if (CurrentlyCompiling) {
            CurrentlyCompiling.withModuleDo([](Module &M) LANGUAGE_NOTSAFEPOINT { errs() << "Dumping failing module\n" << M << "\n"; });
        } else {
...
{
    if (auto Err = TSM.withModuleDo([&](Module &M) LANGUAGE_NOTSAFEPOINT -> Error
            {
...
#else
LANGUAGE_JITSymbol JuliaOJIT::findSymbol(StringRef Name, bool ExportedSymbolsOnly)
{
    orc::JITDylib* SearchOrders[3] = {&JD, &GlobalJD, &ExternalJD};
    ArrayRef<orc::JITDylib*> SearchOrder = ArrayRef<orc::JITDylib*>(&SearchOrders[0], ExportedSymbolsOnly ? 3 : 1);
    auto Sym = ES.lookup(SearchOrder, Name);
...
LANGUAGE_JITSymbol JuliaOJIT::findUnmangledSymbol(StringRef Name)
{
    return findSymbol(getMangledName(Name), true);
}

Expected<LANGUAGE_JITSymbol> JuliaOJIT::findExternalJDSymbol(StringRef Name, bool ExternalJDOnly)
{
    orc::JITDylib* SearchOrders[3] = {&ExternalJD, &GlobalJD, &JD};
    ArrayRef<orc::JITDylib*> SearchOrder = ArrayRef<orc::JITDylib*>(&SearchOrders[0], ExternalJDOnly ? 1 : 3);
    auto Sym = ES.lookup(SearchOrder, getMangledName(Name));
...
StringRef JuliaOJIT::getFunctionAtAddress(uint64_t Addr, language_code_instance_t *codeinst)
{
    std::lock_guard<std::mutex> lock(RLST_mutex);
    assert(Addr != (uint64_t)&language_fptr_wait_for_compiled);
    ...
    if (fname->empty()) {
        ...
        language_callptr_t invoke = language_atomic_load_relaxed(&codeinst->invoke);
        if (Addr == (uintptr_t)invoke) {
            stream_fname << "jsysw_";
        }
        else if (invoke == language_fptr_args_addr) {
            stream_fname << "jsys1_";
        }
...
#else
size_t getRTDyldMemoryManagerTotalBytes(RTDyldMemoryManager *mm) LANGUAGE_NOTSAFEPOINT;
...
JuliaOJIT *language_ExecutionEngine;
...
void language_merge_module(orc::ThreadSafeModule &destTSM, orc::ThreadSafeModule srcTSM)
{
    ++ModulesMerged;
    destTSM.withModuleDo([&](Module &dest) LANGUAGE_NOTSAFEPOINT {
        srcTSM.withModuleDo([&](Module &src) LANGUAGE_NOTSAFEPOINT {
...
static void language_decorate_module(Module &M) {
        ...
        SmallVectorImpl<orc::ThreadSafeModule*> &Stack)
{
    ...
    M.withModuleDo([&](Module &m) LANGUAGE_NOTSAFEPOINT {
        for (auto &F : m.global_objects()) {
            if (F.isDeclaration() && F.getLinkage() == GlobalValue::ExternalLinkage) {
    ...
    for (auto *CM : Children) {
        int Down = language_add_to_ee(*CM, NewExports, Queued, Stack);
    ...
    while (1) {
        ...
        language_merge_module(M, std::move(*CM));
    }
    language_ExecutionEngine->addModule(std::move(M));
    return 0;
}

static uint64_t getAddressForFunction(StringRef fname)
{
    auto addr = language_ExecutionEngine->getFunctionAddress(fname);
    assert(addr);
    return addr;
}
...
{
    language_ExecutionEngine->addGlobalMapping(name, (uint64_t)(uintptr_t)addr);
}

extern "C" LANGUAGE_DLLEXPORT_CODEGEN
size_t language_jit_total_bytes_impl(void)
{
    return language_ExecutionEngine->getTotalBytes();
}

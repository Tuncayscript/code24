...
static language_value_t *do_apply(language_value_t **args, uint32_t nargs, language_value_t *iterate)
{
    ...
    if (arg_heap) {
        ...
        ((void**)roots)[-2] = (void*)LANGUAGE_GC_ENCODE_PUSHARGS(1);
#endif
    }
...
LANGUAGE_CALLABLE(language_f_setglobal)
{
    ...
    if (order == language_memory_order_notatomic)
        ...
        language_fence();
    language_binding_t *b = language_get_binding_wr(mod, var);
...
{
    LANGUAGE_NARGS(set_binding_type!, 2, 3);
    language_module_t *m = (language_module_t*)args[0];
    language_sym_t *s = (language_sym_t*)args[1];
    LANGUAGE_TYPECHK(set_binding_type!, module, (language_value_t*)m);
    LANGUAGE_TYPECHK(set_binding_type!, symbol, (language_value_t*)s);
    language_value_t *ty = nargs == 2 ? (language_value_t*)language_any_type : args[2];
    LANGUAGE_TYPECHK(set_binding_type!, type, ty);
    language_binding_t *b = language_get_binding_wr(m, s);
    language_value_t *old_ty = NULL;
    if (language_atomic_cmpswap_relaxed(&b->ty, &old_ty, ty)) {
        language_gc_wb(b, ty);
    }
    else if (nargs != 2 && !language_types_equal(ty, old_ty)) {
        language_errorf("cannot set type for global %s.%s. It already has a value or is already set to a different type.",
...
void language_init_primitives(void) LANGUAGE_GC_DISABLED
{
    ...
    add_builtin_func("set_binding_type!", language_f_set_binding_type);
    ...
    // memory primitives
    language_builtin_memoryref = add_builtin_func("memoryref", language_f_memoryref);
    ...
    add_builtin("CodeInstance", (language_value_t*)language_code_instance_type);
    ...
    add_builtin("CodeInfo", (language_value_t*)language_code_info_type);

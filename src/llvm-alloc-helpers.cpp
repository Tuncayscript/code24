...
LANGUAGE_USED_FUNC void AllocUseInfo::dump(llvm::raw_ostream &OS)
{
    OS << "AllocUseInfo:\n";
    OS << "escaped: " << escaped << '\n';
...
LANGUAGE_USED_FUNC void AllocUseInfo::dump()
{
    dump(dbgs());
}
...
void language_alloc::runEscapeAnalysis(llvm::CallInst *I, EscapeAnalysisRequiredArgs required, EscapeAnalysisOptionalArgs options) {
    ...
    auto check_inst = [&] (Instruction *inst, Use *use) {
        ...
        if (auto call = dyn_cast<CallInst>(inst)) {
            ...
            // Uses in `language_roots` operand bundle are not counted as escaping, everything else is.
            if (!call->isBundleOperand(opno) ||
                call->getOperandBundleForOperand(opno).getTagName() != "language_roots") {
            ...
            LLVM_DEBUG(dbgs() << "Call is in language_roots bundle, marking haspreserve\n");
            required.use_info.haspreserve = true;
            return true;
        }

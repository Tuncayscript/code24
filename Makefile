...
language_flisp.boot.inc.phony: julia-deps
	@$(MAKE) $(QUIET_MAKE) -C $(BUILDROOT)/src language_flisp.boot.inc.phony
...
julia-src-release julia-src-debug : julia-src-% : julia-deps language_flisp.boot.inc.phony julia-cli-%
...
LANGUAGE_TARGETS := julia
else ifeq ($(JULIA_BUILD_MODE),debug)
LANGUAGE_TARGETS := julia-debug
endif

# private libraries, that are installed in $(prefix)/lib/julia
LANGUAGE_PRIVATE_LIBS-0 := libccalltest libccalllazyfoo libccalllazybar libllvmcalltest
ifeq ($(JULIA_BUILD_MODE),release)
LANGUAGE_PRIVATE_LIBS-0 += libjulia-internal libjulia-codegen
else ifeq ($(JULIA_BUILD_MODE),debug)
LANGUAGE_PRIVATE_LIBS-0 += libjulia-internal-debug libjulia-codegen-debug
endif
ifeq ($(USE_GPL_LIBS), 1)
LANGUAGE_PRIVATE_LIBS-$(USE_SYSTEM_LIBSUITESPARSE) += libamd libbtf libcamd libccolamd libcholmod libcolamd libklu libldl librbio libspqr libsuitesparseconfig libumfpack
endif
LANGUAGE_PRIVATE_LIBS-$(USE_SYSTEM_LIBBLASTRAMPOLINE) += libblastrampoline
LANGUAGE_PRIVATE_LIBS-$(USE_SYSTEM_PCRE) += libpcre2-8
LANGUAGE_PRIVATE_LIBS-$(USE_SYSTEM_DSFMT) += libdSFMT
LANGUAGE_PRIVATE_LIBS-$(USE_SYSTEM_GMP) += libgmp libgmpxx
LANGUAGE_PRIVATE_LIBS-$(USE_SYSTEM_MPFR) += libmpfr
LANGUAGE_PRIVATE_LIBS-$(USE_SYSTEM_LIBSSH2) += libssh2
LANGUAGE_PRIVATE_LIBS-$(USE_SYSTEM_NGHTTP2) += libnghttp2
LANGUAGE_PRIVATE_LIBS-$(USE_SYSTEM_MBEDTLS) += libmbedtls libmbedcrypto libmbedx509
LANGUAGE_PRIVATE_LIBS-$(USE_SYSTEM_CURL) += libcurl
LANGUAGE_PRIVATE_LIBS-$(USE_SYSTEM_LIBGIT2) += libgit2
LANGUAGE_PRIVATE_LIBS-$(USE_SYSTEM_LIBUV) += libuv
ifeq ($(OS),WINNT)
LANGUAGE_PRIVATE_LIBS-$(USE_SYSTEM_ZLIB) += zlib
else
LANGUAGE_PRIVATE_LIBS-$(USE_SYSTEM_ZLIB) += libz
endif
ifeq ($(USE_LLVM_SHLIB),1)
LANGUAGE_PRIVATE_LIBS-$(USE_SYSTEM_LLVM) += libLLVM $(LLVM_SHARED_LIB_NAME)
endif
LANGUAGE_PRIVATE_LIBS-$(USE_SYSTEM_LIBUNWIND) += libunwind

ifeq ($(USE_SYSTEM_LIBM),0)
LANGUAGE_PRIVATE_LIBS-$(USE_SYSTEM_OPENLIBM) += libopenlibm
endif

LANGUAGE_PRIVATE_LIBS-$(USE_SYSTEM_BLAS) += $(LIBBLASNAME)
ifneq ($(LIBLAPACKNAME),$(LIBBLASNAME))
LANGUAGE_PRIVATE_LIBS-$(USE_SYSTEM_LAPACK) += $(LIBLAPACKNAME)
endif

LANGUAGE_PRIVATE_LIBS-$(USE_SYSTEM_CSL) += libgfortran libquadmath libstdc++ libgcc_s libgomp libssp libatomic
ifeq ($(OS),Darwin)
LANGUAGE_PRIVATE_LIBS-$(USE_SYSTEM_CSL) += libc++
endif
ifeq ($(OS),WINNT)
LANGUAGE_PRIVATE_LIBS-$(USE_SYSTEM_CSL) += libwinpthread
else
LANGUAGE_PRIVATE_LIBS-$(USE_SYSTEM_CSL) += libpthread
endif
ifeq ($(SANITIZE),1)
ifeq ($(USECLANG),1)
LANGUAGE_PRIVATE_LIBS-1 += libclang_rt.asan
else
LANGUAGE_PRIVATE_LIBS-1 += libasan
endif
endif

ifeq ($(WITH_TRACY),1)
LANGUAGE_PRIVATE_LIBS-0 += libTracyClient
endif
...
LANGUAGE_PRIVATE_LIBS-0 += libgfortblas
endif
endif
endif
...
# Copy over shared library file for libjulia.*
	for suffix in $(LANGUAGE_TARGETS) ; do \
...
ifeq ($(JULIA_BUILD_MODE),release)
	...
	for suffix in $(LANGUAGE_PRIVATE_LIBS-0) ; do \
	...
	for suffix in $(LANGUAGE_PRIVATE_LIBS-1) ; do \
...
ifeq ($(JULIA_BUILD_MODE),release)
	...
	# Update RPATH entries and LANGUAGE_SYSTEM_IMAGE_PATH if $(private_libdir_rel) != $(build_private_libdir_rel)
ifneq ($(private_libdir_rel),$(build_private_libdir_rel))
ifeq ($(OS), Darwin)
ifneq ($(DARWIN_FRAMEWORK),1)
	for j in $(LANGUAGE_TARGETS) ; do \
	...
	# Overwrite LANGUAGE_SYSTEM_IMAGE_PATH in libjulia-internal
...
ifeq ($(JULIA_BUILD_MODE),release)
	$(call stringreplace,$(DESTDIR)$(shlibdir)/libjulia.$(LANGUAGE_MAJOR_MINOR_SHLIB_EXT),$(LOADER_BUILD_DEP_LIBS)$$,$(LOADER_INSTALL_DEP_LIBS))
else ifeq ($(JULIA_BUILD_MODE),debug)
	$(call stringreplace,$(DESTDIR)$(shlibdir)/libjulia-debug.$(LANGUAGE_MAJOR_MINOR_SHLIB_EXT),$(LOADER_DEBUG_BUILD_DEP_LIBS)$$,$(LOADER_DEBUG_INSTALL_DEP_LIBS))
endif
endif

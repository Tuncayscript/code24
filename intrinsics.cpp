...
#undef DEBUG_TYPE
#define DEBUG_TYPE "language_irgen_intrinsics"
...
uint32_t language_get_LLVM_VERSION_impl(void)
{
...
static Value *uint_cnvt(language_codectx_t &ctx, Type *to, Value *x)
{
    return ctx.builder.CreateZExtOrTrunc(x, to);
}

static Constant *language_const_to_llvm(language_codectx_t &ctx, const void *ptr, language_datatype_t *bt)
{
    // assumes `language_is_pointerfree(bt)`.
    // `ptr` can point to a inline field, do not read the tag from it.
    // make sure to return exactly the type specified by
    // language_type_to_llvm as this will be assumed by the callee.
    if (bt == language_bool_type)
        return ConstantInt::get(getInt8Ty(ctx.builder.getContext()), (*(const uint8_t*)ptr) ? 1 : 0);

    Type *lt = language_struct_to_llvm(ctx, (language_value_t*)bt, NULL);

    if (language_is_vecelement_type((language_value_t*)bt) && !language_is_uniontype(language_tparam0(bt)))
        bt = (language_datatype_t*)language_tparam0(bt);
    ...
    size_t nf = language_datatype_nfields(bt);
    SmallVector<Constant*, 0> fields(0);
    for (size_t i = 0; i < nf; i++) {
        size_t offs = language_field_offset(bt, i);
        language_value_t *ft = language_field_type(bt, i);
        Type *lft = language_type_to_llvm(ctx, ft);
        if (type_is_ghost(lft))
            continue;
        assert(!language_field_isptr(bt, i));
        unsigned llvm_idx = isa<StructType>(lt) ? convert_struct_offset(language_Module->getDataLayout(), lt, offs) : i;
        ...
        if (language_is_uniontype(ft)) {
            // compute the same type layout as language_struct_to_llvm
            size_t fsz = 0, al = 0;
            (void)language_islayout_inline(ft, &fsz, &al); // compute al
            fsz = language_field_size(bt, i); // get LLT_ALIGN(fsz+1,al)
            uint8_t sel = ((const uint8_t*)ptr)[offs + fsz - 1];
            language_value_t *active_ty = language_nth_union_component(ft, sel);
            size_t active_sz = language_datatype_size(active_ty);
            ...
            Constant *val = language_const_to_llvm(ctx, ov, (language_datatype_t*)ft);
...
static Constant *language_const_to_llvm(language_codectx_t &ctx, language_value_t *e)
{
    ...
    language_value_t *bt = language_typeof(e);
    if (!language_is_pointerfree(bt))
        return NULL;
    return language_const_to_llvm(ctx, e, (language_datatype_t*)bt);
}
...
static Value *zext_struct_helper(language_codectx_t &ctx, Value *V, Type *T2)
{
    Type *T = V->getType();
    if (T == T2)
        return V;
...
static Value *emit_unboxed_coercion(language_codectx_t &ctx, Type *to, Value *unboxed)
{
...
static Value *emit_unbox(language_codectx_t &ctx, Type *to, const language_cgval_t &x, language_value_t *jt)
{
    ...
    Constant *c = x.constant ? language_const_to_llvm(ctx, x.constant) : NULL;
    ...
    unsigned alignment = language_alignment(jt);
...
static void emit_unbox_store(language_codectx_t &ctx, const language_cgval_t &x, Value *dest, MDNode *tbaa_dest, unsigned alignment, bool isVolatile)
{
    if (x.isghost) {
        // this can happen when a branch yielding a different type ends
        // up being dead code, and type inference knows that the other
    ...
    emit_memcpy(ctx, dest, language_aliasinfo_t::fromTBAA(ctx, tbaa_dest), src, language_aliasinfo_t::fromTBAA(ctx, x.tbaa), language_datatype_size(x.typ), alignment, language_alignment(x.typ), isVolatile);
}

static language_datatype_t *staticeval_bitstype(const language_cgval_t &targ)
{
    // evaluate an argument at compile time to determine what type it is
    language_value_t *unw = language_unwrap_unionall(targ.typ);
    if (language_is_type_type(unw)) {
        language_value_t *bt = language_tparam0(unw);
        if (language_is_primitivetype(bt))
            return (language_datatype_t*)bt;
    }
    return NULL;
}

static language_cgval_t emit_runtime_call(language_codectx_t &ctx, LANGUAGE_I::intrinsic f, ArrayRef<language_cgval_t> argv, size_t nargs)
{
    ...
    return mark_language_type(ctx, r, true, (language_value_t*)language_any_type);
}
...
static language_cgval_t generic_bitcast(language_codectx_t &ctx, ArrayRef<language_cgval_t> argv)
{
    // Give the arguments names //
    const language_cgval_t &bt_value = argv[0];
    const language_cgval_t &v = argv[1];
    language_datatype_t *bt = staticeval_bitstype(bt_value);

    // it's easier to throw a good error from C than llvm
    if (!bt)
        return emit_runtime_call(ctx, bitcast, argv, 2);

    Type *llvmt = bitstype_to_llvm((language_value_t*)bt, ctx.builder.getContext(), true);
    uint32_t nb = language_datatype_size(bt);

    Value *bt_value_rt = NULL;
    if (!language_is_concrete_type((language_value_t*)bt)) {
    ...
    bool isboxed;
    Type *vxt = language_type_to_llvm(ctx, v.typ, &isboxed);
    if (!language_is_primitivetype(v.typ) || language_datatype_size(v.typ) != nb) {
        Value *typ = emit_typeof(ctx, v, false, false);
        if (!language_is_primitivetype(v.typ)) {
            if (language_is_datatype(v.typ) && !language_is_abstracttype(v.typ)) {
    ...
    if (!v.ispointer())
        ...
        vx = language_const_to_llvm(ctx, v.constant);
    ...
    if (language_is_concrete_type((language_value_t*)bt)) {
        return mark_language_type(ctx, vx, false, bt);
    }
    else {
        ...
        return mark_language_type(ctx, box, true, bt->name->wrapper);
    }
}

static language_cgval_t generic_cast(
        language_codectx_t &ctx,
        intrinsic f, Instruction::CastOps Op,
        ArrayRef<language_cgval_t> argv, bool toint, bool fromint)
{
    ...
    if (Op == Instruction::FPExt) {
        if (language_floattemp_var_needed(TT)) {
    ...
    if (language_is_concrete_type((language_value_t*)jlto)) {
        ...
        emit_concretecheck(ctx, targ_rt, std::string(language_intrinsic_name(f)) + ": target type not a leaf primitive type");
        unsigned align = sizeof(void*); // Allocations are at least pointer aligned
...
static language_cgval_t emit_runtime_pointerref(language_codectx_t &ctx, ArrayRef<language_cgval_t> argv)
{
    return emit_runtime_call(ctx, pointerref, argv, 3);
}

static language_cgval_t emit_pointerref(language_codectx_t &ctx, ArrayRef<language_cgval_t> argv)
{
    const language_cgval_t &e = argv[0];
    const language_cgval_t &i = argv[1];
    const language_cgval_t &align = argv[2];

    if (align.constant == NULL || !language_is_long(align.constant))
        return emit_runtime_pointerref(ctx, argv);
    unsigned align_nb = language_unbox_long(align.constant);

    if (i.typ != (language_value_t*)language_long_type)
        return emit_runtime_pointerref(ctx, argv);
    language_value_t *aty = e.typ;
    if (!language_is_cpointer_type(aty))
        return emit_runtime_pointerref(ctx, argv);
    language_value_t *ety = language_tparam0(aty);
    if (language_is_typevar(ety))
        return emit_runtime_pointerref(ctx, argv);
    ...
    Value *idx = emit_unbox(ctx, ctx.types().T_size, i, (language_value_t*)language_long_type);
    ...
    if (ety == (language_value_t*)language_any_type) {
        ...
        return mark_language_type(ctx, load, true, ety);
    }
        ...
        bool isboxed;
        Type *ptrty = language_type_to_llvm(ctx, ety, &isboxed);
        assert(!isboxed);
...
static language_cgval_t emit_runtime_pointerset(language_codectx_t &ctx, ArrayRef<language_cgval_t> argv)
{
    return emit_runtime_call(ctx, pointerset, argv, 4);
}

// e[i] = x
static language_cgval_t emit_pointerset(language_codectx_t &ctx, ArrayRef<language_cgval_t> argv)
{
    const language_cgval_t &e = argv[0];
    const language_cgval_t &x = argv[1];
    const language_cgval_t &i = argv[2];
    const language_cgval_t &align = argv[3];
    ...
    if (ety == (language_value_t*)language_any_type) {
        ...
        setName(ctx.emission_context, gep, "pointerset_ptr");
        emit_memcpy(ctx, gep, language_aliasinfo_t::fromTBAA(ctx, nullptr), x, size, align_nb, language_alignment(ety));
    }
    else {
        bool isboxed;
        Type *ptrty = language_type_to_llvm(ctx, ety, &isboxed);
        ...
        if (!type_is_ghost(ptrty)) {
            ...
            thePtr = ctx.builder.CreateInBoundsGEP(ptrty, thePtr, im1);
            typed_store(ctx, thePtr, x, language_cgval_t(), ety, ctx.tbaa().tbaa_data, nullptr, nullptr, isboxed,
...
// ptr + offset
// ptr - offset
static language_cgval_t emit_pointerarith(language_codectx_t &ctx, intrinsic f,
                                    ArrayRef<language_cgval_t> argv)
{
    language_value_t *ptrtyp = argv[0].typ;
    language_value_t *offtyp = argv[1].typ;
    if (!language_is_cpointer_type(ptrtyp) || offtyp != (language_value_t *)language_ulong_type)
    ...
    if (language_is_concrete_type(ptrtyp)) {
        return mark_language_type(ctx, ans, false, ptrtyp);
    }
    else {
        Value *box = emit_allocobj(ctx, (language_datatype_t *)ptrtyp, true);
        setName(ctx.emission_context, box, "ptr_box");
        init_bits_value(ctx, box, ans, ctx.tbaa().tbaa_immut);
        return mark_language_type(ctx, box, true, (language_datatype_t *)ptrtyp);
    }
}

static language_cgval_t emit_atomicfence(language_codectx_t &ctx, ArrayRef<language_cgval_t> argv)
{
    const language_cgval_t &ord = argv[0];
    if (ord.constant && language_is_symbol(ord.constant)) {
        enum language_memory_order order = language_get_atomic_order((language_sym_t*)ord.constant, true, true);
...
static language_cgval_t emit_atomic_pointerref(language_codectx_t &ctx, ArrayRef<language_cgval_t> argv)
{
    const language_cgval_t &e = argv[0];
    const language_cgval_t &ord = argv[1];
    language_value_t *aty = e.typ;
    if (!language_is_cpointer_type(aty) || !ord.constant || !language_is_symbol(ord.constant))
        return emit_runtime_call(ctx, atomic_pointerref, argv, 2);
    language_value_t *ety = language_tparam0(aty);
    if (language_is_typevar(ety))
        return emit_runtime_call(ctx, atomic_pointerref, argv, 2);
    enum language_memory_order order = language_get_atomic_order((language_sym_t*)ord.constant, true, false);
    if (order == language_memory_order_invalid) {
        emit_atomic_error(ctx, "invalid atomic ordering");
        return language_cgval_t(); // unreachable
    }
...
// e[i] y => x (replace)
// x(e[i], y) (modify)
static language_cgval_t emit_atomic_pointerop(language_codectx_t &ctx, intrinsic f, ArrayRef<language_cgval_t> argv, int nargs, const language_cgval_t *modifyop)
{
    ...
    const language_cgval_t &e = argv[0];
    const language_cgval_t &x = isreplacefield || ismodifyfield ? argv[2] : argv[1];
    const language_cgval_t &y = isreplacefield || ismodifyfield ? argv[1] : undefval;
    const language_cgval_t &ord = isreplacefield || ismodifyfield ? argv[3] : argv[2];
    const language_cgval_t &failord = isreplacefield ? argv[4] : undefval;

    language_value_t *aty = e.typ;
    if (!language_is_cpointer_type(aty) || !ord.constant || !language_is_symbol(ord.constant))
        return emit_runtime_call(ctx, f, argv, nargs);
    if (isreplacefield) {
        if (!failord.constant || !language_is_symbol(failord.constant))
            return emit_runtime_call(ctx, f, argv, nargs);
    }
    language_value_t *ety = language_tparam0(aty);
    if (language_is_typevar(ety))
        return emit_runtime_call(ctx, f, argv, nargs);
    enum language_memory_order order = language_get_atomic_order((language_sym_t*)ord.constant, !issetfield, true);
    enum language_memory_order failorder = isreplacefield ? language_get_atomic_order((language_sym_t*)failord.constant, true, false) : order;
    if (order == language_memory_order_invalid || failorder == language_memory_order_invalid || failorder > order) {
        emit_atomic_error(ctx, "invalid atomic ordering");
        return language_cgval_t(); // unreachable
    }
    AtomicOrdering llvm_order = get_llvm_atomic_order(order);
    ...
    if (!is_valid_intrinsic_elptr(ety)) {
        std::string msg(StringRef(language_intrinsic_name((int)f)));
        msg += ": invalid pointer type";
        emit_error(ctx, msg);
        return language_cgval_t();
    }
    if (!ismodifyfield)
        emit_typecheck(ctx, x, ety, std::string(language_intrinsic_name((int)f)));
...
static Value *emit_checked_srem_int(language_codectx_t &ctx, Value *x, Value *den)
{
    Type *t = den->getType();
    ...
    raise_exception_unless(ctx,
            ndivby0,
            literal_pointer_val(ctx, language_diverror_exception));
...
struct math_builder {
    ...
    math_builder(language_codectx_t &ctx, bool always_fast = false, bool contract = false)
      : ctxbuilder(ctx.builder),
        old_fmf(ctxbuilder.getFastMathFlags())
    {
        FastMathFlags fmf;
        if (language_options.fast_math != LANGUAGE_OPTIONS_FAST_MATH_OFF &&
            (always_fast ||
             language_options.fast_math == LANGUAGE_OPTIONS_FAST_MATH_ON)) {
            fmf.setFast();
        }
        if (contract)
...
static Value *emit_untyped_intrinsic(language_codectx_t &ctx, intrinsic f, ArrayRef<Value*> argvalues, size_t nargs,
                                     language_datatype_t **newtyp, language_value_t *xtyp);


static language_cgval_t emit_ifelse(language_codectx_t &ctx, language_cgval_t c, language_cgval_t x, language_cgval_t y, language_value_t *rt_hint)
{
    ...
    if (t1 == language_bottom_type)
        return y;
    if (t2 == language_bottom_type)
        return x;
    ...
    if (!isboxed) {
        ...
        if (x_tindex || y_tindex) {
            ...
            if (!x_ptr && !y_ptr) { // both ghost
                ...
                if (ifelse_tbaa == NULL) {
                    // LLVM won't return a TBAA result for the root, but mark_language_struct requires it: make it now
            ...
            if (!x_tindex && x.constant) {
                x_tindex = ConstantInt::get(getInt8Ty(ctx.builder.getContext()), 0x80 | get_box_tindex((language_datatype_t*)language_typeof(x.constant), rt_hint));
            }
            if (!y_tindex && y.constant) {
                y_tindex = ConstantInt::get(getInt8Ty(ctx.builder.getContext()), 0x80 | get_box_tindex((language_datatype_t*)language_typeof(y.constant), rt_hint));
            }
            if (x_tindex && y_tindex) {
            ...
            language_cgval_t ret = mark_language_slot(ifelse_result, rt_hint, tindex, ifelse_tbaa);
    ...
    language_value_t *jt = (t1 == t2 ? t1 : rt_hint);
    return mark_language_type(ctx, ifelse_result, isboxed, jt);
}

static language_cgval_t emit_intrinsic(language_codectx_t &ctx, intrinsic f, language_value_t **args, size_t nargs)
{
    auto &DL = ctx.emission_context.DL;
    assert(f < num_intrinsics);
    if (f == cglobal && nargs == 1)
        f = cglobal_auto;
    unsigned expected_nargs = language_intrinsic_nargs((int)f);
    if (expected_nargs && expected_nargs != nargs) {
        language_errorf("intrinsic #%d %s: wrong number of arguments", f, language_intrinsic_name((int)f));
    }
    ...
    for (size_t i = 0; i < nargs; ++i) {
        language_cgval_t arg = emit_expr(ctx, args[i + 1]);
        if (arg.typ == language_bottom_type) {
    ...
    case not_int: {
        ++Emitted_not_int;
        assert(nargs == 1);
        const language_cgval_t &x = argv[0];
        if (!language_is_primitivetype(x.typ))
            return emit_runtime_call(ctx, f, argv, nargs);
        ...
        return mark_language_type(ctx, ans, false, x.typ);
    }
    ...
    case have_fma: {
        ...
        assert(nargs == 1);
        const language_cgval_t &x = argv[0];
        if (!x.constant || !language_is_datatype(x.constant))
            return emit_runtime_call(ctx, f, argv, nargs);
        ...
        FunctionCallee intr = language_Module->getOrInsertFunction(intr_name, getInt1Ty(ctx.builder.getContext()));
        auto ret = ctx.builder.CreateCall(intr);
        return mark_language_type(ctx, ret, false, language_bool_type);
    }

    default: {
        assert(nargs >= 1 && "invalid nargs for intrinsic call");
        const language_cgval_t &xinfo = argv[0];

        // verify argument types
        if (!language_is_primitivetype(xinfo.typ))
            return emit_runtime_call(ctx, f, argv, nargs);
        ...
        //if (xtyp == (language_value_t*)language_bool_type)
        ...
        if (f == shl_int || f == lshr_int || f == ashr_int) {
            if (!language_is_primitivetype(argv[1].typ))
                return emit_runtime_call(ctx, f, argv, nargs);
        ...
        // call the intrinsic
        language_value_t *newtyp = xinfo.typ;
        Value *r = emit_untyped_intrinsic(ctx, f, argvalues, nargs, (language_datatype_t**)&newtyp, xinfo.typ);
        // Turn Bool operations into mod 1 now, if needed
        if (newtyp == (language_value_t*)language_bool_type && !r->getType()->isIntegerTy(1))
            r = ctx.builder.CreateTrunc(r, getInt1Ty(ctx.builder.getContext()));
        return mark_language_type(ctx, r, false, newtyp);
    }
    }
                                     ...
                                     language_datatype_t **newtyp, language_value_t *xtyp)
{
    ...
    case fma_float: {
        ...
        FunctionCallee fmaintr = Intrinsic::getDeclaration(language_Module, Intrinsic::fma, ArrayRef<Type*>(t));
    ...
    case checked_umul_int: {
        ...
        FunctionCallee intr = Intrinsic::getDeclaration(language_Module, intr_id, ArrayRef<Type*>(t));
    ...
    case checked_sdiv_int: {
        ...
        raise_exception_unless(ctx, cond, literal_pointer_val(ctx, language_diverror_exception));
    ...
    case eq_int:  *newtyp = language_bool_type; return ctx.builder.CreateICmpEQ(x, y);
    case ne_int:  *newtyp = language_bool_type; return ctx.builder.CreateICmpNE(x, y);
    case slt_int: *newtyp = language_bool_type; return ctx.builder.CreateICmpSLT(x, y);
    case ult_int: *newtyp = language_bool_type; return ctx.builder.CreateICmpULT(x, y);
    case sle_int: *newtyp = language_bool_type; return ctx.builder.CreateICmpSLE(x, y);
    case ule_int: *newtyp = language_bool_type; return ctx.builder.CreateICmpULE(x, y);

    case eq_float: *newtyp = language_bool_type; return math_builder(ctx)().CreateFCmpOEQ(x, y);
    case ne_float: *newtyp = language_bool_type; return math_builder(ctx)().CreateFCmpUNE(x, y);
    case lt_float: *newtyp = language_bool_type; return math_builder(ctx)().CreateFCmpOLT(x, y);
    case le_float: *newtyp = language_bool_type; return math_builder(ctx)().CreateFCmpOLE(x, y);

    case eq_float_fast: *newtyp = language_bool_type; return math_builder(ctx, true)().CreateFCmpOEQ(x, y);
    case ne_float_fast: *newtyp = language_bool_type; return math_builder(ctx, true)().CreateFCmpUNE(x, y);
    case lt_float_fast: *newtyp = language_bool_type; return math_builder(ctx, true)().CreateFCmpOLT(x, y);
    case le_float_fast: *newtyp = language_bool_type; return math_builder(ctx, true)().CreateFCmpOLE(x, y);

    case fpiseq: {
        *newtyp = language_bool_type;
    ...
    case bswap_int: {
        FunctionCallee bswapintr = Intrinsic::getDeclaration(language_Module, Intrinsic::bswap, ArrayRef<Type*>(t)); //TODO: Move to deduction guides
        return ctx.builder.CreateCall(bswapintr, x);                                                           // when we drop LLVM 15
    }
    case ctpop_int: {
        FunctionCallee ctpopintr = Intrinsic::getDeclaration(language_Module, Intrinsic::ctpop, ArrayRef<Type*>(t));
        return ctx.builder.CreateCall(ctpopintr, x);
    }
    case ctlz_int: {
        FunctionCallee ctlz = Intrinsic::getDeclaration(language_Module, Intrinsic::ctlz, ArrayRef<Type*>(t));
        y = ConstantInt::get(getInt1Ty(ctx.builder.getContext()), 0);
        return ctx.builder.CreateCall(ctlz, {x, y});
    }
    case cttz_int: {
        FunctionCallee cttz = Intrinsic::getDeclaration(language_Module, Intrinsic::cttz, ArrayRef<Type*>(t));
        y = ConstantInt::get(getInt1Ty(ctx.builder.getContext()), 0);
        return ctx.builder.CreateCall(cttz, {x, y});
    }

    case abs_float: {
        FunctionCallee absintr = Intrinsic::getDeclaration(language_Module, Intrinsic::fabs, ArrayRef<Type*>(t));
        return ctx.builder.CreateCall(absintr, x);
    }
    case copysign_float: {
        FunctionCallee copyintr = Intrinsic::getDeclaration(language_Module, Intrinsic::copysign, ArrayRef<Type*>(t));
    ...
    case ceil_llvm: {
        FunctionCallee ceilintr = Intrinsic::getDeclaration(language_Module, Intrinsic::ceil, ArrayRef<Type*>(t));
        return ctx.builder.CreateCall(ceilintr, x);
    }
    case floor_llvm: {
        FunctionCallee floorintr = Intrinsic::getDeclaration(language_Module, Intrinsic::floor, ArrayRef<Type*>(t));
        return ctx.builder.CreateCall(floorintr, x);
    }
    case trunc_llvm: {
        FunctionCallee truncintr = Intrinsic::getDeclaration(language_Module, Intrinsic::trunc, ArrayRef<Type*>(t));
        return ctx.builder.CreateCall(truncintr, x);
    }
    case rint_llvm: {
        FunctionCallee rintintr = Intrinsic::getDeclaration(language_Module, Intrinsic::rint, ArrayRef<Type*>(t));
        return ctx.builder.CreateCall(rintintr, x);
    }
    case sqrt_llvm: {
        FunctionCallee sqrtintr = Intrinsic::getDeclaration(language_Module, Intrinsic::sqrt, ArrayRef<Type*>(t));
